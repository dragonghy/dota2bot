...
spanned <=1.1s on an 8-game corpus and registered the filter this tool
implements: **span >= 2.0s first, then compare arms, and always report the
gate-OFF null channel alongside**.
...
RESOLVABLE_SPAN = 2.0          # 20:49Z §2.3 registered filter
...
def _verify():
    ...
    print("lion_drain_census --verify: 18 asserts OK")
...
def main():
    ...
    for k in ("all", "lion_cand_side", "lion_base_side"):
        ...
        print("%-15s games %3d  channels %4d (hero-target %3d, zero-snap %3d)  "
              "in-domain %3d (leak-only %d)  span>=2.0s %3d  RESOLVABLE %3d"
              % (k, v["games"], v["channels"], v["hero_target"], v["zero_snap"],
                 v["in_domain"], v["in_domain_leak_only"], v["span_ge_2"], v["resolvable"]))
    if a.json:
