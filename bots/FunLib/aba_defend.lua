...
function ____exports.ShouldDefend(bot, hBuilding, nRadius)
    ...
    if not result then
        ...
        -- branch for 0. Everything it declines lands here, and this last
        -- clause carries no threat precondition whatsoever. So on a building
        -- with nothing near it, the closest of {2, 3} is still named its
        -- defender, every frame, for the whole game. That answer is not
        -- cosmetic in GetDefendDesireHelper: `shouldDef` there skips the
        -- bail-out to VeryLow, adds +0.1 to the desire cap (capBoost) and
        -- lifts the desire floor from VeryLow to Low (baseFloor).
        --
        -- The clause is kept verbatim for nNearby >= 1, where it is doing the
        -- job it was written for -- naming one core when the role ladder found
        -- nobody. Armed (turbo only) it simply stops firing on a quiet
        -- building. This is a precondition, not a knob: it can only ever
        ...
        -- would move every rung. This disjunct only ever restores shipped
        -- behaviour, so the one-directional property above still holds
        -- verbatim.
        --
        ...
        -- repo can run -- which is also why arming this id is bit-identical on
        -- the corpus before and after this change. The arithmetic above, not a
        -- corpus reading, is what this disjunct rests on; the real-frame half
        -- is tests/test_defquiet_creep_siege.lua, which feeds ShouldDefend the
        -- fixture's OWN creep sample.
        --
        -- NOT TOUCHED, and deliberately so: the travel-boots/tinker escalation
        -- directly above has the same missing precondition and additionally
        -- writes `travel_boots_defender` state on a quiet frame. One lever at
        -- a time -- that one is registered in this round's report, not fixed
        -- here. Measured this round and registered rather than fixed: it has a
        -- constructive zero domain in this corpus -- 0 of 1120 (fixture, hero)
        ...
        if (not bDefQuiet or nNearby >= 1 or creepWeights > 0) and not result and pos == GetClosestAllyPos(
            {2, 3},
            hBuilding:GetLocation()
        ) then
            result = true
...
function ____exports.DefendThink(bot, lane)
    ...
    local ds = getDefendState(bot)
    -- [defstale] The guard below reads "more enemies on my PATH than IN RANGE",
    ...
    -- early returns -- one of which (`#closeEnemiesDefend > 0 and
    -- #closeAlliesDefend >= #closeEnemiesDefend`) fires on exactly the frames
    -- this Think runs on, i.e. whenever a fight is happening within 900. So
    -- during a fight the right-hand side freezes at whatever it held before the
    -- fight started, or -- if no helper call ever reached the bottom -- at the
    -- initial empty table. The comparison can therefore only ever be bought by
    -- that staleness, never by a real path/range difference, and it aborts every
    -- other DefendThink branch for the frame.
