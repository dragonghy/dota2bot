...
| `f_260909_215040_wk_blast_lane_67.lua`, `..._lane_121.lua`, `..._mid_269.lua`, `..._lion_480.lua`, `..._sb_661.lua`, `..._sb_1052.lua` | queue `hero-54`'s Wraith King half (`5313f5 / 20260909_215040_slot1`, t = 67.1 / 121.5 / 269.1 / 480.6 / 661.3 / 1052.0). Each frame is cut AT a `skeleton_king_hellfire_blast` combat-log entry, and **three of the six drive the shipped tree to order that spell** -- against 0 of 36 for Wraith King in the whole corpus. Used by `tests/test_focus_cast_instant_frames.lua`. Staging price **1 file** (paid); admission price **17 of 19 census files** -- see the section below. ⚠ **A SECOND file was silently short of this directory and the staging measurement could not have found it** (hero 2026-09-18): `tests/test_wk_q_aim_preflight.lua` adopted TREE scope on 2026-08-28 (it appends the `iterations/pending/` frame by name) and never adopted the enumeration, so from the commit that staged these frames it went on publishing "not one frame reaches the branch" while `..._lane_121` and `..._sb_1052` (plus `f_260909_215227_zeus_arc_od_79`) reach it. ⛔ This is **not** a miss by the staging round: the prescribed check `rg -l 'tests/frames' tests/` finds the scans that ALREADY enumerate this directory and **structurally cannot find one that should and does not** -- the omission is invisible to a grep for the string being omitted. Repaired by widening that file's own enumeration and re-deriving its verdict in the same round (verdict unchanged, for a narrower reason); **nothing was admitted to `tests/fixtures/`**. |
...
| `f_260904_125801_campbind_poke_3307.lua`, `..._3347.lua` | The soak candidate `campbind`'s condition-(a) pair, and the instant `test_set.md §FX.5` named when it moved that condition off wave corpora and onto a fixture (`20260904_125801_slot6` spirit_breaker, t = 330.7 / 334.7; W46, run tail `b77771`). Recovered 2026-09-17 from the **21-day `dem21/` bulk prefix** with 8 days left on its lifecycle -- `soak/<run>/` holds no `.dem` at `REC_SLOTS>1`, so the ruling's open premise ("is the `.dem` still in S3") resolves YES and the row does **not** convert to permanent 退集. Dumped at `-creep-interval 0.25`, so the neutral sample sits dt = 0.1 from each instant. The 330.7 frame is the only place in the tree where two neutral camps sit inside one `GetNearbyNeutralCreeps(1400)` and **partition exactly** under the helper's own 1200u radius (6 troll-box / 4 kobold-box / **0 in both**) -- which is the readable surface §FX.4 measured as 1-and-unattributable over 97 games. The 334.7 frame is its counterexample: 3 units inside both boxes, and the nearest one traces back to a 3-second-motionless position 34.6u from the troll box. Used by `tests/test_campbind_poke_real_frame.lua` (by name). Staging price **measured against a clean tree and PAID in the same round**: 4 files / 5 assertions -- `test_axe_hunger_camp_reach` (corpus 142->144, unit rows 1420->1440), `test_chasering_target_in_ring` and `test_divepocket_target_in_pocket` (142->144; divepocket's live subject-team rows 657->665), `test_cm_kill_confirm_quantifier` (1420->1440). **Every one is a denominator; VERDICT UNCHANGED in all four** -- the draft is centaur/lich/necrolyte/obsidian_destroyer/pudge/sniper/spirit_breaker/sven/venomancer/viper, so no Axe, CM, Lion, Zeus or WK row exists to move a hero zero. Admission price to `tests/fixtures/` **measured, NOT paid**: at least 3 files / 5 assertions (`test_axe_hunger_camp_reach` 2, `test_cm_kill_confirm_quantifier` 1, `test_fieldcreep_magnitude_operand` 2), measured by moving the pair in, running, and moving it back. |
