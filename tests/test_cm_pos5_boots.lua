...
-- Bearing terminus that had to go with it.  Pure build change, no gate, so it
-- is LIVE -- which is exactly why the reasoning is pinned here as assertions.
--
...
-- ---------------------------------------------------------------------------
-- 1. The change itself, asserted against the source.  Prose about a line must
--    not be the only thing holding that line (the 2026-08-22 21:48Z lesson).

tests['[hero] pos_5 opens on the arcane outfit, and Bearing is gone with it'] = function()
    local tPos5 = pos5_list()
    assert(has(tPos5, 'item_crystal_maiden_outfit'),
        'pos_5 must reference item_crystal_maiden_outfit; got ' .. table.concat(tPos5, ','))
    assert(not has(tPos5, 'item_mage_outfit'),
        'pos_5 still carries the tranquil-boots mage outfit as well')
    assert(not has(tPos5, 'item_boots_of_bearing'),
        'Boots of Bearing is back in the pos_5 list. Its recipe consumes a pair '
        .. 'of TRANQUIL boots, so with arcane in the opener it buys a second '
        .. 'pair and movement speed does not stack. Re-read this file before '
        .. 'restoring it.')
end

tests['[hero] the swap moves the boots and nothing else'] = function()
    local tMage, tCM = outfit('item_mage_outfit'), outfit('item_crystal_maiden_outfit')
    assert(#tMage == #tCM,
        'the two outfits must stay entry-for-entry comparable so the boots are '
        .. 'the only lever; got ' .. #tMage .. ' vs ' .. #tCM)
    local nDiff = 0
    for i = 1, #tMage do if tMage[i] ~= tCM[i] then nDiff = nDiff + 1 end end
    assert(nDiff == 1, 'exactly one entry may differ between the two outfits; '
        .. nDiff .. ' do. mage=' .. table.concat(tMage, ',')
        .. ' cm=' .. table.concat(tCM, ','))
    assert(has(tMage, 'item_tranquil_boots') and has(tCM, 'item_arcane_boots'),
        'and the one entry must be the boots')
    assert(not has(tCM, 'item_tranquil_boots'),
        'the CM outfit must not carry both pairs')
end

tests['[hero] the outfit-complete sentinel follows its boots'] = function()
    local src = read_file(ITEMS)
    local sSentinel = src:match("%['item_crystal_maiden_outfit'%]%s*=%s*\"(item_[%w_]+)\"")
    assert(sSentinel == 'item_arcane_boots',
        'tDefineItemRealName decides when an outfit counts as owned; mage points '
        .. 'at its tranquil boots and priest at its arcane boots. This one reads '
        .. tostring(sSentinel))
end
