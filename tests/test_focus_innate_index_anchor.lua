...
--
--      ⭐ AMENDED 2026-08-26 (GH #209): section 5 still holds and still ratchets,
--      but its thinnest leg no longer has to carry anything.  It refuted "the
--      dumped array is slot order" by observing Axe's ultimate on cooldown on
--      1 of 26 frames; the game's own npc_heroes.txt now says so directly --
--      Axe's ultimate is at engine slot 5 with two `generic_hidden` placeholders
--      ahead of it, which the dumper filters out, which is exactly why his
--      dumped array is four entries long.  The slot map is
--      tests/mock/hero_slots.lua (generated by tools/agent/hero_slot_map.py) and
--      the reading is pinned in tests/test_hero_slot_order_anchor.lua.  Note
--      what that does NOT change: a slot map read from the KV is still not
--      permission to derive an index map from this CORPUS, which is what
--      section 5 forbids.
--
-- HONEST BOUNDS
