...
-- ---------------------------------------------------------------- section 7 --
-- Where the miscount lived, read out of the two instruments rather than
-- retyped, so a drift in either turns this red instead of stale.
--
-- ⭐ 2026-09-07 (replay-check): THE SCANNER IS FIXED.  `cullthresh_domain.py`
-- now uses half-open containment, per the director's ruling, which wrote the
-- premise into the ruling itself: `queue.json:hero-39` = APPROVED-SCAN with
-- `add <= t < remove` MANDATORY (iterations/reports/director/
-- 20260906T222000Z.md section 2 -- "running it as-is reproduces #570's 2").
-- ⇒ This section flips direction: it used to pin that the scanner was still
-- CLOSED, it now pins that BOTH instruments are HALF-OPEN, so a revert is what
-- goes red.  The frozen rows and the 2-vs-0 arithmetic in section 6 are
-- UNCHANGED -- they are the historical record of the disagreement, and they
-- still reproduce it from the game's own numbers.  The 449-cast reading in the
-- request is still the request's, and is still owed a re-take under half-open;
-- that is the traversal the director approved as hero-39.
--
-- ⚠️ AND THE OLD FORM OF THIS TEST DID NOT SURVIVE THE FIX HONESTLY.  It read
-- `scanner:sub(from, from + 800)` and asserted the substring `a <= t <= b`.
-- Adding a docstring to `active_modifiers` that EXPLAINS the old closed form
-- pushed the real comparison past byte 800 and left the literal sitting in the
-- prose -- so on the fixed tree it still passed, green, while reading no
-- comparison at all.  A source-text tripwire anchored to a substring in a fixed
-- byte window is satisfied by prose ABOUT the thing it pins.  Both halves below
-- are now anchored to the COMPARISON STATEMENT (the `if` line inside the
-- function body), and each asserts the line it finds is unique, so a second
-- copy of the comparison cannot decide the reading either.

-- The `<lo> <= t <op> <hi>` comparison inside `def <fn>(` -- CODE only.  Two
-- things this has to get right, both of them learned the same afternoon:
--   * DOCSTRINGS ARE STRIPPED.  Prose about the boundary is exactly what made
--     the previous version of this test pass on a tree it was supposed to fail
--     on.  A sentence naming the old form must not be able to decide the
--     reading.
--   * THE BODY ENDS AT THE NEXT `def` AT ANY INDENTATION.  make_fixture's
--     active_modifiers is a NESTED function; cutting at a column-0 `def`
--     swallowed four more functions and picked up an unrelated `<= t)` from
--     one of them.
local function containment_cmp(path, fn)
    local src = read_file(path)
    local from = src:find('def ' .. fn .. '%(')
    assert(from, fn .. ' is gone from ' .. path)
    local rest = src:sub(from)
    local nextDef = rest:find('\n%s*def ')
    local body = nextDef and rest:sub(1, nextDef) or rest
    body = body:gsub('""".-"""', '')
    local found, n = nil, 0
    for cmp in body:gmatch('[%w_]+%s*<=%s*t%s*<=?%s*[%w_]+') do
        n = n + 1
        found = found or cmp
    end
    assert(n == 1, string.format(
        '%s / %s holds %d containment comparisons, not 1 -- this tripwire can '
        .. 'only speak about the boundary while exactly one comparison decides '
        .. 'it', path, fn, n))
    return found
end

tests['both containment instruments are HALF-OPEN, and a revert to closed is red'] = function()
    local scanner = containment_cmp(SCANNER, 'active_modifiers')
    assert(scanner:find('t < b', 1, true) and not scanner:find('t <= b', 1, true),
        string.format(
            '%s must use HALF-OPEN containment (`add <= t < remove`); its '
            .. 'comparison reads `%s`.  Closed containment is GH #570\'s '
            .. 'miscount: it cannot tell a cull fired INSIDE a live window from '
            .. 'the cull that ENDED it, and section 6 shows every hit on this '
            .. 'corpus is the second kind.  Directors ruling: queue.json:hero-39 '
            .. 'APPROVED-SCAN, half-open mandatory.', SCANNER, scanner))

    local gen = containment_cmp(GENERATOR, 'active_modifiers')
    assert(gen:find('t < e', 1, true) and not gen:find('t <= e', 1, true),
        string.format(
            '%s must use HALF-OPEN containment; its comparison reads `%s`.  The '
            .. 'frame under tests/frames/ was generated by the half-open '
            .. 'reconstruction, and section 8 reads `remaining` off it.',
            GENERATOR, gen))
end
