...
tests['[reverse] turbo is structural: both callers ask the situation first'] = function()
    ...
    -- [tpdeep 20260908] The eighth caller joins the same list for the same
    -- reason, and for one more that is specific to it: J.ShouldDeepSipNotTpRecover
    -- exists to work BELOW J.IsFieldRegenSituation's 0.18 floor (that floor stops
    -- 29 of its call site's 31 corpus trigger frames), so routing through the
