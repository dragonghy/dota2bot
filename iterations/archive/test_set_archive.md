# 测试集裁定档案(`test_set.md` 的历史节;owner P4.3 2026-09-06 拆出)

**这个文件不是备份,是 `test_set.md` 的一半。** 拆分之前 `test_set.md` 是
**1.47 MB**,其中 **1.42 MB 是本文件**;P4.3 买的正是每个 fresh session 的
上岗阅读量,而不是磁盘。

**引用照旧写 `test_set.md §XX`,不要改写成本文件名。**
`§XX` 的命名空间是**名字不是文件**:`citation_audit.py` 的 `SECTION_FALLBACKS`
让一条 `test_set.md §XX` 在 **`test_set.md` ∪ 本文件**上解析,AMBIGUOUS 也在并集上判
(同一个 `§XX` 在两边各有一个标题 = 真冲突,按第一个搜到的文件答就是把它藏起来)。
⛔ 因此**本文件里不许出现与 `test_set.md` 重名的 `§XX` 标题** —— 那会让那条引用变 AMBIGUOUS。
拆分当天实测:251 条不同的 `test_set.md §XX` 引用,168 条解析得到,拆分前后**逐条相同**。

**读它的还有 `pending_rulings.py`**:23 个「提议入集」小节**全部**在本文件里,
所以那个工具读的是 `test_set.md` **加**本文件(`read_test_set`)。
只拆文件不拆读者,`proposal sections scanned` 会从 23 掉到 0 而打印 `ORPHAN_PROPOSAL: none`
—— **检测器因为输入消失而变绿**,正是一个「让没裁的裁定变红」的工具最不能有的失效方向。

**新裁定写在哪:** 写进 `test_set.md`,像以前一样。等它长起来,由总监按同样的
边界(live 只留【当前 armed 串 + 下一波指令 + 未决裁定索引】)再往本文件**追加**。
⛔ 追加,不是插入 —— 本文件的节序是拆分当天的原序,保持它。
