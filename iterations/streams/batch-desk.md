...
- **2026-09-06T00:11–00:4xZ(收 W49:4 粒完整、**零抢占**、gpm **+11.70**。⛔⛔ 但本轮该被读的是账单 —— 上一轮预言的「headroom 被别人的项目吃掉」**已经落账**:`c7a` 从账单上没有这一行变成 **`$7.995`**,而本台自己那一行**逐位没动**。本轮 AWS 支出 **$0.01**,未发波)**
  ...
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),推送门 `ARM_HOOK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。两次 push 真码见报告第十节。
