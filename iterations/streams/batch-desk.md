...
- 2026-09-01T00:35Z:**纯收割轮,零 AWS 支出**(唯一支出是一次 `$0.01` 的 CE 复核)。
  ...
  **本台不自行改常数**。② **⭐⭐ 新开 `[harness]` —— 开工自检的 `trunk-red(python)` 腿会假红**:
  ...
  **它落在 GH #243 修好的分支旁边而不在里面**(#243 只把 runner 的 **exit 2** 改判 UNCERTIFIABLE,
  ...
  **本台不改 harness。** ③ **⭐⭐ 按需波成本存在一个读不到的窗口**:实例 <1.5h 老化出 API、CE 滞后 4.3–11.3h,
