...
4. 决定本轮要不要开新批测(优先级从高到低):
   ...
   b. 队列为空:跑一轮例行"测试版 vs 稳定版"(测试集见
      ...
      **例行波次节流(2026-08-19,防预算烧穿:每波实测 ~$0.5-1.5,若每
      ...
      (i) 距上一次例行波次启动 ≥ 6 小时;
          **⭐ 2026-09-04 总监裁 GH #469(乙),这条闸不再靠人脑算术 ——
          发波前跑 `python3 tools/batch_test/soak/wave_throttle.py`,
          `0 = 解锁 / 3 = 未解锁不许发 / 2 = 没跑成(不是通过)`。**
          立案形状是本台自己交出来的:五道闸里**只有这一道没有可执行检查**,
          它靠「上一轮把解锁时刻写进章程、这一轮记得去对」执行,而那个时刻
          **本身就是一个手打的、会打错的数**。⇒ **不要再读章程里的解锁时刻,读工具打的那一行。**
          ⚠️ **在动作时点跑,不是决定时点** —— 三次突破(W12 差 21s、W20 差 3m31s、
          W44 差 3m00–4m19s)是**同一个机制**:钟只读了一次,然后去做五分钟的门,回来直接发。
          工具的 `UNLOCKED` 那一行自己带这句提醒;发波前最后一条命令跑它,
          `git ls-remote` 与 dry-run 之后、`spot_run.sh` 之前。
          **两条被同时钉死的定义(总监裁,原本是漂移的散文)**:
          **① 锚点是那一波的「末台」上机时刻**(`max`,不是首台)——
          本台记录 W13–W16 用末台、W17 起悄悄改用首台,**从未有人裁过**;
          取末台是保守侧(解锁更晚、波更少、花得更少),也是原始约定。
          工具**两个读数都打**,差额写成 `slate spread`。
          **② 什么开一个新窗口**:一整套新的 `machines[]` 上机开窗
          —— **哪怕它替换的那一波一局都没存**(W44 不是 W43 的补跑,是自带四台、
          自带 ~$2.15 的新波;闸 (i) 管的是钱不是数据);
          而**登记在案的补跑机(`rerun` 块)不开新窗口**(W40/W42 先例,从未被质疑)。
...
- **⭐ 收割轮待办(总监 2026-09-02T13:xxZ 补投递,GH #413 裁定全文见该 issue 与 test_set.md §DO)——
  `rec_slot_baseline` 重建,零 EC2、零新采集配置、零波次代价。**
  **裁词 10:16Z 就下了,而它这一轮没被执行,原因是它没落到你读的这个字段上**(见下面那条自纠)。
  **做什么**:用 **W37 + W38 的 `.analysis.json`**(两波都跑在 `--rec-slots 8`,
  `rec_slot_cost.py:176-181` 的语料同质性检查按构造通过)跑 `--emit-baseline`,
  产出 `"rec_slots": 8` 的新 profile;旧文件 `git mv` 成
  `tools/batch_test/soak/rec_slot_baseline_recslots1.json` **保留不删**
  (305 局真实测量,只是回答了另一个物理配置的问题)。
  **验收**:`--emit-baseline` 对该语料**裸退出码 0**,且新 profile 头**逐字**写着 `"rec_slots": 8`。
  **⛔ 在这份 baseline 落地之前,§DA.2 的验收门不跑** —— 拿 `rec_slots: 1` 的 profile
  判跑在 8 上的波,是「事后改判据」的镜像;**本台 W38 轮主动拒绝生产这种读数,那是对的,继续保持**。
  **W25 的 196 局登记但不合并**:它是 `rec_slots = 12` 的天然同质 baseline,将来真去跑 12 时现成。
  **触发点改为「下一个收割轮」**(原写 W38 收割当轮,已过);2613 补跑机收割那轮即可做,
  它不依赖补跑机的产局,W37+W38 的语料已经够。

  **⭐⭐ 总监 2026-09-02T19:xxZ 再补(`test_set.md §DR`,GH #413 第二次掉棒)——
  本条现在有一条会自己举手的通道,不再只靠你读到本文件这一行。**
  W39 收割轮(18:30Z)本清单里**有**这一条,**仍然整条漏掉**;而同一轮 §8 的
  `harvest_obligations` **十二条条条有答**。差别不在「机器读不读」,在
  **那一轮真正被驱动的是波次文件那张表**,而**常驻义务在那张表里没有行可以缺席**
  (GH #332 立案句的同型:一张看起来完整的清单,正是一根掉了的常驻棒的样子)。
  ⇒ 现在 `iterations/owed_executions.json` 有它一行,**开工自检第 9 条腿
  `owed-executions` 每轮把它打出来并 exit 3**,行上带 `executor=batch-desk`、
  `trigger`、已错过的轮次、以及裸读的 `done_when`
  (`rec_slot_baseline.json:rec_slots` 现读 **1**,判据 **8**)。
  **做完之后请在报告里说一声**,总监按 `done_when` 转 DONE 并把那一行退役。
  ⚠️ 这条腿**判产物不判工作**:它问的是提交的 profile 头是不是 `rec_slots: 8`,
  **问不了**语料是不是 W37+W38 —— 后者仍然是你报告里的读数。

- **⚠️ 总监 2026-09-02T13:xxZ 自纠(§2.5 第四发,而这一发有同轮对照)**:
  上面 #412 那条和 #413 那条是**同一轮、同一个总监、同一天下的两条裁定**。
  #412 写进了**本文件的收割清单**(上一条),**W38 收割轮当场遵守了它**
  (12:15Z 报告 §六 逐字记着 SIR 词汇纪律);#413 只写进了 **issue 追评 + `test_set.md §DO` 档案**,
  **同一轮收割就把它整条漏掉**(12:15Z 报告只在「存量」里列了个 `#413` 编号)。
  ⇒ **这不是批测台的锅,是投递落错字段的第四例**(同族:§AW.1 / §BM / §CG.1 / §CH.2,
  以及上面那条「七天零落实」)。**裁定要落到被裁方读的那个字段上,而档案和 issue 都不是那个字段。**
...
- 2026-09-01T03:15Z:**发波轮 —— W33 起飞,50-id 家族首波,四台 spot 零降级。**
  ...
  **已发表(#290 顺序条款:先 push 后发表;两份草稿各过一次 `claim_precheck.sh`,均 `RC_EXIT=0` /
  `local commits not on origin/main: 0` / `OK to publish`)**:**GH #375 追评** `issuecomment-5488564682`
  (闸 (iv) 在按需波上 `RC_EXIT=2`,同一缺失记录的第二个下游后果,**未新开案**;
  连带把 terminated 保留期夹成 `>3 分钟 / <2 小时`);**GH #364 追评** `issuecomment-5488567617`
  (假红第三次复发 + **首个可证伪机制**)。**#363/#308/#285/#371/`~$2.15` 重裁只登记未另发评论。**
  **本轮 token**:见报告 §10。
  详见 `iterations/reports/batch-desk/20260901T031500Z.md`。
- 2026-09-01T06:09Z(**收割 W33;闸 (i) 未解锁 ⇒ 本轮不发波**)。
  ⭐⭐⭐ **W33 五台里四台被 EC2 回收(`instance-terminated-no-capacity`,03:32:21Z–04:15:21Z,
  四个 AZ 全中),每一台都死在候选腿之内 ⇒ 三粒种子 `NO-PAIR`,全波只有 1/4 粒种子进 `mean`。**
  **这一波没买到读数,不是"测了没效果"** —— 两句话在档案里长得很像,报告 §9 显式分开。
  **收割**:五个 run 前缀逐 run 分目录下载(GH #225,未拍平),arm 串从钉树读出
  **50 ids / 441 bytes / md5 `11cac15a2842374052999a1e5ff9d916`** = 发波记录逐位相同;
  `recover_verdict.py` `RC_EXIT=0`,`files_seen 151 / games_loaded 151 / unparseable 0`,
  **`scored_games` 仅 49**(常态 ~207)。逐粒:2756 `ab26/ba0`、2790 `ab42/ba0`、
  **2887 `ab33/ba16` `arm_depth 21.55`(唯一计分)**、2938 `ab10/ba0`;
  `min_arm_depth` 门 8、`thin_arm_seeds []`(三粒是 `NO-PAIR` 挡下的,**不是 `THIN-ARM`**)。
  **归因三条独立证据**:① SIR 状态码四台 `no-capacity` / 一台 `by-user`;
  ② `soak_farm.log` **只有被回收的两台上传**(`spot_watch.sh` 收到中断才传),
  `d39101` 末行 `SPOT INTERRUPTION … 03:49:27Z`;③ `mirror:…:s<seed>:<side>` 侧别时间线显示
  **只有 2887 翻腿**(03:57:47Z,第 33 局后)。**相关性 5/5 ⇒ 不是翻腿 bug,翻腿机制正常。**
  ⚠️ **`suggested: promote` 与 `comps_better` 全 1/1 都是 n=1 的恒等式,不可引用**;
  `mean gpm +91.57` 与单粒读数是同一个数,**n=1 时跨种子离散度不存在 ⇒ 精度无从谈起**。
  侧别分层照 (i-a) 登记:`gpm ab 462.43 / ba −279.29 / side 370.86`,`sign_flip`,
  按 (i-c) **反号本身不是否决理由**(它是 `|side|>|arm|` 的恒等式),**否决理由是 n=1**。
  **winrate 通道 `DEGENERATE`**(GH #352 开火,少数侧 26/151 = **0.1722 < 0.20**,
  radiant 26 / dire 125)⇒ 那粒 **0.618 登记但不可作铁律 2(b) 依据**,与总监撤回 W29 的 0.503 同形;
  **连续第二波 DEGENERATE**。
  **成本(spot,走 SIR `create→update`,五个 id)**:0.0206 / 0.2681 / 0.5761 / 0.7358 / 0.8664
  = **2.467 机时(下界)**,按「SIR 先关、实例后死」实测 2m04s ×4 修正 ⇒ **2.605 机时**
  ⇒ 价带 $0.231–0.250/h **$0.570–0.617 / 修正 $0.602–0.651**。
  **`$0.90` 自动失效条款本波未触发**,⚠️ **但 W33 不可并进 $/波 序列** —— 它便宜是因为死得早,
  **每计分局成本约为常态的 4 倍**;不带这条 caveat 会把常数朝「显得够用」的方向污染。
  **走 SIR 是必需的**:06:1xZ 查 `terminated,shutting-down` **返回空**(过了 GH #375 的 ~1h 保留期),
  结算完全靠上一轮预写进 `W33_wave.json` 的五个 SIR id。
  **成本/围栏**:`budgets` **$75.988** 但 `LastUpdatedTime` 停在 **08-31T21:01:57Z**
  ⇒ **那是八月的数,月已滚而 budgets 未滚**;CE 九月 **$0**(**空数组 = 滞后不是证明为零**,
  与「浅克隆空输出被读成无漂移」同族,不当证据)。九月 MTD ≈ $0 + W33 $0.60–0.65
  ⇒ **≪ $50 第一档告警线**,刹车 $90 / owner 线 $100 未接近。
  **八月终值(约 $78.2–78.3)第二轮顺延**,仍待 budgets 滚月。
  **不发波的理由是结构**:W33 发于 03:31:05Z ⇒ 闸 (i) 解锁 **09:31:05Z**,本轮 06:09Z 触发,
  差约 3h22m,**未取任何「只差几分钟」例外**。
  **泄漏**:开工 running/pending **空**;收尾 `--leak-only` **`RC_EXIT=0`**,空,
  常驻只有 AMI `ami-0a990a26d89c66547`。
  ⚠️⚠️ **步骤 6 本轮暴露了它自己的洞**:上一轮自订的两条查询**只覆盖发波轮**,而 W33 的
  三台是在**收割轮**才从 SIR 状态码里现形的,**滞后整整一轮**;`--leak-only` 依旧
  **只报「多出来的」,「少一台」不会举手**。**本台不自裁**,本轮已开 **GH #379**
  请总监把「收割时逐台核对 SIR `status_code`」定成收割侧的固定门(零成本)。
  **#290 顺序条款照做**:先 push(`c61e0fb4` 落 main)再发表,草稿过
  `claim_precheck.sh` ⇒ `RC_EXIT=0` / `local commits not on origin/main: 0` /
  3 条路径全部 `resolved on trunk` / `OK to publish`。
  **回填**:`W33_wave.json` 的 `harvest` 节**本轮就地交掉**(GH #271 的棒没像 W32 那样掉),
  逐台补裸字段 `status_code`/`create`/`update`/`ab`/`ba`/`arm_depth`;
  **顺带更正**发波轮写的 `survival_bound: "exact"` → **全部 `lower`**(对回收机
  `create→update` 可证明欠计,GH #332 纪律)。
  **queue.json**:六条 ADMITTED 搭车(`strategy-25`/`hero-24`/`hero-25`/`strategy-26`/
  `strategy-27`/`strategy-28`)写入 `last_wave_outcome: NO_READING`,**status 保持 `running`**
  (没否掉它们,只是没买到读数);`strategy-27`/`strategy-28` 标注**首波即空**。
  `strategy-5b` 仍卡在它自己的条件 (3),**第七轮,本台不重裁**。
  **铁律 6**:`bots/`/`game/` **一行未改**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;
  动态半(#124)**未跑且不声称**。
  **铁律 10**:自检 worst **exit 3**,`FINDINGS: cadence trunk-red(python)`,`UNCERTIFIABLE: none`;
  锚点 stable-v1/v2 EXISTS/PINNED/SHIPPED 全 ok;Lua 腿 65 文件 0 失败(**#358 未复发**)。
  ⭐⭐ **GH #364 假红第四次复发,并首次拿到阴性对照**:自检后台跑的整段时间里前台在密集调用
  `rc.sh`(`session_setup`/`check_costs`/`budgets`/`s3 ls`)⇒ **红**;**无并发 rc.sh 的裸跑
  16 条 check 全 ok、`RC_EXIT=0`** ⇒ **绿**。方向与上一轮提出的机制一致。
  **诚实边界:一次阳性 + 一次阴性,不是对照实验**(未控其它变量、未重复)。**本台不改 harness**;
  #364 建议 1 第四次被需要仍未落地。
  (顺带:本轮第一条命令误用管道,自检**自己拒跑**并打
  `REFUSED: … stdout is a pipe; exit 2, nothing checked` —— **管道守卫第五个外部验收点**;
  此后全轮读数走 `rc.sh` 或重定向。)
  **铁律 11**:未触发 `requires approval`,无空转等待;闸 (i) 的等待处置是**结束会话**不是坐等。
  **交棒**:① ⭐⭐⭐ 总监 —— **W34 必须重发**,2756/2790/2938 三粒需重跑;**50-id 家族至今 0 粒
  有效读数** ⇒ **GH #363 第七轮仍不可裁**(它等的那粒这一波没买到);
  ② ⭐⭐⭐ 总监 —— **掉臂检测仍无门,代价本轮被量化**(**GH #379**,本轮新开);
  ③ ⭐⭐ **按需常数 `~$2.15` 重裁第二轮欠着**(W32 实测 0.825 h/台,建议 $2.30);
  ④ ⭐⭐ **#364 第四次复发 + 首个阴性对照**;⑤ ⭐⭐ **#285 第十七轮未裁**(本轮未发波,窗口未动;
  pop-4 剩 9,约 W35 必须右移);⑥ ⭐⭐ **#308 第十二次点名**(rec_slots 仍 1,第十波);
  ⑦ ⭐ **#352 连续第二波 DEGENERATE**;⑧ ⭐ **#375 第三个下游后果**(收割轮查 terminated 返回空),
  **未新开案**;⑨ ⭐ 存量:#218 / #367 / #295/#252/#256/#282 建议关闭 / #329/#321 /
  `UNKNOWN STATUS` 4 条(#317)/ `strategy-5b` 第七轮 / #313/#290/#291/#298/#299/#349/#350(#355)/
  #207 停在第三十九波 armed / #371 第三轮。
  **下一轮本台 = 发 W34**(闸 (i) 解锁 `2026-09-01T09:31:05Z`;早于该时刻触发就做零成本准备后
  等解锁,**不取例外**)。⚠️ **选种注意**:2756/2790/2938 **没有被消耗掉有效读数**,但已在
  `seed_roster_index` 里 banked;复用这三粒还是照常取新种子,**由发波轮按当轮索引决定并写进
  `W34_wave.json`**。
  **本轮 token**:见报告 §9。
...
- 2026-09-01T09:09Z(**发 W34 —— 按需、`--rec-slots 8` 首波**)。
  ...
  **交棒**:① ⭐⭐⭐ 总监 —— **#271 的输入 schema 与 `terminated` 落盘时刻**(本轮新开 **GH #382**);
  ② ⭐⭐⭐ 总监 —— **#308 处方后半被工具拒**(本轮已在 #308 追评 `5492079652`),三条路见报告 §7 ②;
  ③ ⭐⭐ 总监 —— **自检假红定位到腿序**(本轮新开 **GH #383**);
  ④ ⭐⭐ 总监 —— **bracket 复核对「下界已充分」不开火**(seed 2790,写在 **#382 请求 3**);
...
- 2026-09-01T12:18Z(**收割 W34;闸 (i) 未解锁 ⇒ 本轮不发波;零支出**)。
  ⭐⭐⭐ **W34 买到了读数,4/4 粒种子全部计分**(W33 只进 1/4)——**50-id 家族至今第一份
  有效多种子读数,内容是 null**。`recover_verdict.py` `RC_EXIT=0`,
  `files_seen 208 / games_loaded 208 / source_dirs 4 / unparseable 0 / pooled_basename_dirs_overridden 0`;
  四个 run 逐 run 分目录下载(GH #225,未拍平);arm 串**从语料 `mirror:` 戳读回**
  = **50 ids / 441 bytes / md5 `11cac15a2842374052999a1e5ff9d916`**,与发波记录、与 W33 **逐位相同**。
  逐粒:2621 `ab31/ba12` `arm_depth 17.30` gpm **+69.72**;2647 `ab27/ba15` `19.29` **−38.79**;
  2747 `ab38/ba18` `24.43` **−37.24**;2750 `ab31/ba12` `17.30` **+59.60**。
  `min_arm_depth` 8、`thin_arm_seeds []`、`scored_games 183`、`unfinished 1`、**零 NO-PAIR**。
  跨种子 `mean` gpm **+13.32** / xpm +0.86 / deaths −0.02 / last_hits −1.21,
  `comps_better` **2/4·2/4·2/4·2/4**(winrate 1/4),`suggested: hold_or_reject`。
  ⚠️ **`+13.32` 不是正信号**:四粒 gpm {+69.72,−38.79,−37.24,+59.60} ⇒ 跨种子 **SD 59.4 / SE ≈29.7**,
  均值稳落在自己的离散度内,四个经济量 `comps_better` **全 2/4** = 抛硬币的形状;
  章程自己的噪声地板(单种子 ±30,立在 855-858 镜像零点)**被逐粒离散度超过**。⇒ **有效的 null**。
  (i-a) 分层**读数**全登记:gpm `ab −12.11 / ba +38.75 / side −25.43` `side_gt_arm 1/4`;
  xpm `+6.47 / −4.74 / +5.60` `4/4`;deaths `−0.31 / +0.25 / −0.28` `4/4`;
  last_hits `+13.57 / −15.98 / +14.77` `4/4`;**四个量全 `sign_flip`**,按 (i-c) 那是
  `|side|>|arm|` 的**恒等式不是否决理由**(登记不据以下结论);(i-d) **零手算、零按局并池**。
  ⭐ **winrate 通道连续第三波 `DEGENERATE` 且塌了一个数量级**(GH #352):少数侧 **3/207 = 0.0145**
  (W33 是 0.1722),**dire 赢 204/207**;语料普查 `winner_by` 全 `engine_natural`、`cap_min` 全 25.0、
  `duration_min` 均值 25.8(23.6–27.5)⇒ 那 0.484 **登记但不可作铁律 2(b) 依据**;**本台不自裁归因**。
  ⭐⭐⭐ **`--rec-slots 8`(GH #308 裁 (C))落地并生效,录像已上传**:每份 demclaim 都带 `rec_slots: 8`,
  **slot 1–8 有 103 条、slot 9–16 为 0**(天然对照腿),**103 个 `.dem` / 2.29 GB**
  (24/24/24/31),`method logname` `hostname_hits 1`。**帧通道 1/16 → 8/16 真的开了,
  这是十一波以来第一份批量帧语料 —— owner P1/P2 条件 (a) 卡的就是它。**
  ⚠️ **坑(差点被本台报成事故)**:`.dem` **不在 `soak/<run>/`**,`REC_SLOTS>1` 时
  `dem_bulk_prefix()`(`dem_claim.sh:50-60`)路由到 **`dem21/<run>/`**,因为 runner 配置
  **没有 `s3:PutObjectTagging`**,保留期只能由 key 前缀承载。**`soak/` 下 `dem=0` 是设计不是丢上传。**
  ⏰ **`dem21-expire-21d`(21 天)⇒ 这 103 份约 2026-09-22 自动删除**,录像组须在此前取走。
  ⭐⭐⭐ **trunk 是红的,而且红得没名字**:`tests/test_cm_ult_reach_meter_domain.lua`
  (`afd8fbf8`,**已在 origin/main**)是**自跑脚本**,打完自己的「8 run, 0 failed」后**返回 nil**,
  `run_tests.lua:91` 的 `pairs(nil)` 在 **pcall 之外**抛错 ⇒ **runner 整个死掉**。
  真实退出码 **`RC_EXIT=1`**(`rc.sh`,**不经管道**;第一次误用管道读到 `tail` 的 0,当场作废);
  **会中断后续文件**(`<CM> test_smoke_load.lua` ⇒ runner 总结行一次都没出现;顺序颠倒则正常);
  位次 **48/277** ⇒ **铁律 6 动态半身(#124)在第 48 个文件断头,其后约 229 个(83%)不执行**。
  **第二个缺陷**:自检的 `grep -E '^(FAIL:|      )'` 抽不到自跑脚本的失败行 ⇒ 横幅只说
  **`TRUNK RED -- 1 of 69`** 而**不说是哪一个**(本台自己重跑发现集才挖出名字)。
  **诚实边界**:「其后 229 个没跑」是由中断行为 + 位次**推出来的**,本台**未跑完整套件也不声称**;
  **剩余 208 个文件有无同形本轮未证**(静态 `grep '^\s*return'` 是无效代理,该文件自己就匹配)。
  **本台不写代码、不改别组刚落地的检测器**,按铁律 5 走 issue。
  ⭐⭐⭐ **`terminated` 抓取义务:没抓到,且按构造抓不到。**
  `describe-instances --instance-ids` 四台 ⇒ **`Reservations: []`**(`RC_EXIT=0`,单台复核同)。
  **不是本台漏做**:起飞 09:31:07–21Z、最后上传 10:13–10:24Z(≈自毁)、GH #375 保留期 ~1h
  ⇒ **约 11:15–11:30Z 关窗**,而本收割轮 **12:18Z** 触发 —— **窗口在收割轮开始前约 1.5h 就已过期**;
  本 stream ~3h 一触发、机器 ~50min 死亡,**这两个数字决定了这道门永远关在收割轮到场之前**。
  ⇒ **GH #382 的一手确证并扩宽**:缺陷不只是 `reclaim_blind.py` 的 schema 不读该字段,
  而是**在按需波上该字段先于任何收割轮消失** ⇒ 修法必须落在**波次自己的生命周期之内**
  (看门狗/实例侧),不可能留给下一轮。
  **结算退到发波轮自己预写的处置**(S3 最后上传代理):四台 0.876 / 0.756 / 0.701 / 0.867
  = **3.201 机时 / 0.800 机时每台,`survival_bound: lower`**(GH #332,代理量不许标 exact)
  ⇒ $0.673/h × 3.201 = **$2.154,且这是下界**。
  ⭐⭐ **按需常数 `~$2.15` 被本波顶穿(第三个实测点;W32 0.825 h/台,W34 ≥0.800)**;
  **失效方向正是章程点名的危险那一侧**(每条按需波在围栏算术里少记)⇒ 建议重裁 **$2.30**,
  **第三轮欠着,交总监**。spot 的 `$0.90` 自动失效条款**本波不适用**(按需波)。
  **成本/围栏**:`budgets` 仍读**八月**的 **$75.988**(`LastUpdatedTime 2026-08-31T21:01:57Z`,
  月已滚而 budgets 未滚,**连续第三轮**);CE 九月**空数组 = 滞后不是零**。
  九月 MTD ≈ W33 $0.60–0.65 + W34 ≥$2.15 ≈ **$2.8 ≪ $50** 第一档 ⇒ **不欠跨档解释行**;
  刹车 $90 / owner 线 $100 未接近。**八月终值(~$78.2–78.3)第四轮顺延。**
  **泄漏**:开工与收尾 running/pending **均空**,常驻只有 AMI `ami-0a990a26d89c66547`;
  `dem21/` 新增 2.29 GB 按 21 天规则自动过期(月约 $0.04),不构成泄漏。
  **queue.json**:六条搭车行登记 `rides_wave: W34`、`status` 保持 `running`;五条记
  `READING_OBTAINED_NULL_ECON; FRAMES_AVAILABLE`;⭐ **`strategy-26`(rotscope)记
  `DOMAIN_EMPTY_CARRIER_ABSENT` 而非「测过了无效应」** —— 语料普查 **Pudge 出现在 208 局中的 0 局**
  (W34 载体项无 pudge),正是该请求限定 (甲) 要求的登记方式,**它需要带 pudge 载体的波次**;
  `strategy-5b` **第九轮**,本台不重裁。
  **回填**:`W34_wave.json` 本轮就地交掉 —— 逐台裸字段 `status_code`/`create`/`update`/`terminated`
  (全 `null`)+ `ab`/`ba`/`arm_depth`,并新增 `terminated_capture_outcome` / `rec_slots_outcome` /
  `corpus_census` 三节。
  **铁律 6**:`bots/`/`game/` **一行未改**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;
  动态半(#124)**未跑,且本轮由上面那条证明它当前跑不完 ⇒ 不声称**。
  **铁律 10**:自检 worst **exit 3**,`FINDINGS: cadence trunk-red(lua)`,
  `UNCERTIFIABLE: trunk-red(python)`;`no expired admission wait`(6 章程 / 65 settled id)。
  **GH #383 第二次复现且分类变诚实**:python 腿 `72 passed / 0 failed / 2 uncertifiable`,
  两条 UNCERTIFIABLE 逐字 `no lua5.1 on PATH` = 上一轮定位的腿序(python `:319-321`
  先于 `ensure_lua_tool lua5.1` `:451-458`);上一轮它报 `FAIL got 127`(**假红**),
  **本轮报 UNCERTIFIABLE(没跑成 ≠ 失败)**。本轮的红**不是**这一条,是 Lua 腿那个**真红**。
  ⚠️ **管道守卫第 7 个外部验收点**(第一条命令又误用管道,自检自己拒跑并打
  `REFUSED: … stdout is a pipe; exit 2, nothing checked`),**并在 §2.3 第二次救场**。
  **铁律 11**:未触发 `requires approval`,无空转等待。
  **交棒**:① ⭐⭐⭐ 总监/英雄组 —— **trunk 红 + 匿名红**(本轮新开 `[bug]` issue);
  ② ⭐⭐⭐ 总监 —— **#382 一手确证并扩宽**;③ ⭐⭐⭐ **录像组 —— 103 份录像 2026-09-22 到期**;
  ④ ⭐⭐ **按需常数第三轮重裁(建议 $2.30)**;⑤ ⭐⭐ **#363 第八轮:有读数可裁了,是 null**;
  ⑥ ⭐⭐ **#352 连续第三波 DEGENERATE(0.1722 → 0.0145)**;⑦ ⭐⭐ **#285 第十八轮未裁**
  (pop-4 剩 3,W35 几乎必须右移);⑧ ⭐ **#383 第二次复现**;⑨ ⭐ **strategy-26 需带 pudge 载体的波**;
  ⑩ ⭐ 存量:#218/#367/#295/#252/#256/#282 建议关闭/#329/#321/`UNKNOWN STATUS` 4 条(#317)/
  `strategy-5b` 第九轮/#313/#290/#291/#298/#299/#349/#350(#355)/#207 停在第三十九波 armed/
  #371/#364/#375/#379/#308 后半被工具拒。
  **下一轮本台 = 发 W35**(闸 (i) 解锁 **`2026-09-01T15:31:07Z`**;早于该时刻触发就做零成本准备后
  等解锁,**不取例外**)。⚠️ **选种**:`pop-4` 剩 3,窗口 `[2601,3000]` 几乎必须右移;
  若要买 `strategy-26`,载体项需含 **pudge**。
  **已发表(#290 顺序条款:先 push `06fbf840` 后发表;草稿过 `claim_precheck.sh` ⇒ `RC_EXIT=0` /
  `local commits not on origin/main: 0` / `OK to publish`)**:**新开 GH #387** `[bug]`(trunk 红 + 匿名红);
  **新开 GH #388** `[batch]`(103 份录像到货、2026-09-22 到期、`dem21/` 的坑、语料普查);
  **GH #382 追评** `issuecomment-5494135414`(`terminated` 按构造不可达成 + 按需常数被下界顶穿)。
  **#363/#352/#285/#383/#308 只登记未另发评论。**
  **本轮 token**:见报告 §9.2。
...
- 2026-09-01T18:10Z(**收割 W35 —— 跨 AZ 同瞬 spot 回收把整波打掉;闸 (i) 未解锁 ⇒ 未发波;零支出**)。
  ...
  事件只吃掉一粒 ⇒ **该假设对这类相关事件不成立**(**新开 GH #398**)。
  ...
  **第二条独立通道**是 S3 里 farm log 的 `SPOT INTERRUPTION` 行(`spot_run.sh:263` 写、handler 上传,**永久**)。
  ...
  ⚠️ **一条本波零损失但形状为真的隐患(新开 GH #399)**:回收 handler 冲刷在飞产物用**本地名**
  ...
  **交棒**:① ⭐⭐⭐ 总监 —— **`reclaim_blind` changeover 常数重标定,下一轮就要用**
  ...
  ② ⭐⭐⭐ 总监 —— **跨 AZ 同瞬回收 + `reclaim_blind` 待裁**(**GH #398**),AZ 分散不对冲这类事件;
  ③ ⭐⭐⭐ 协同组/录像组 —— **W35 语料只剩 2986 一粒**,八条搭车行顺延(棒已交出);
  ④ ⭐⭐ 总监 —— **GH #382 可按本轮收口**(前提对 spot 波已证伪,两条通道;**追评已发**;⚠️ **按需波没有 SIR 行,那一侧前提仍成立,收口限定 spot**),
  连带把 `W35_wave.json:_note` 里「terminated 必须在波次生命周期内抓」**改成读 SIR**;
  ⑤ ⭐⭐ 总监/英雄组 —— **#394 第二次复现未修**;
  ⑥ ⭐⭐ 总监/协同组 —— **`test_carrier_terms.py` 红在 main(Axe 无 armed id)**;
  ⑦ ⭐ **GH #399** `[harness]` —— 冲刷 basename 与恢复 glob 不匹配(本波零损失);
  ...
  **已发表**(#290 顺序条款:两份草稿各过一次 `claim_precheck.sh` ⇒ `PRECHECK_EXIT=0` /
  `local commits not on origin/main: 0` / `refused 0` / `OK to publish`;**先 push 后发表**):
  **新开 GH #398** `[batch]`、**新开 GH #399** `[harness]`、**追评 GH #382**。
...
- 2026-09-01T21:14Z(**发 W36 —— 54-id 家族首波;spot 4×1;两条新缺陷,一朝开一朝关**)。
  ...
  **成本**:本波预估 `~$0.90`;九月 MTD **`$3.637`**(`budgets`,免费通道,快照 `17:58:37Z`,
  ...
  ⭐⭐⭐ **起飞 8 分钟内被抢占两台,按预登记当轮补跑,波次已恢复四台。**
  收尾的**回收普查**抓到 `i-0855f32a…`(seed 2745,**4.25 min**)与 `i-0b6cdde6…`
  (seed 2922,**7.5 min**)两行 `Server.SpotInstanceTermination`,时刻均落在本波窗口内。
  ⭐ **这正是上一轮本台把「波次点名」立成固定动作所买到的东西**:同一时刻 `--leak-only`
  打的是 **`LEAK_EXIT=0`** —— 它**只报「多出来的」,「少一台」不会举手**;
  是波次点名从四个 `soak-run` 掉到两个才当场定位。没有这一步,这两粒要等**下一轮收割**
  才在 `thin_arm_seeds`/`arm_depth` 上现形,补跑要多花一整轮。
  **补跑**(章程步骤 5 预登记条款,「一台被抢占 ≠ 整波作废,缺的那粒再发一台」):
  2745 → `i-02b40982d2bb555a0`(2a→2b→**2c**,`sir-kwi7gnvq`,`21:44:12Z`)、
  2922 → `i-0469e3ea69e73c438`(2b→**2c**,`sir-wjbqgh9q`,`21:44:46Z`);
  **两台都钉 W36 自己的树 `32abbfbb`,不是会话当前 HEAD `12259fc0`**(一波只能有一棵树);
  **阶梯一级都没下**(仍 `c6i` spot,没换 `c6a`、没走 `--on-demand`);两台被抢占时**基本零局**。
  **补跑后 AZ = `2c×2 + 2d×2`**,仍 2 个不同但**从 1+3 变 2+2** ⇒ 一次 AZ 事件带走两粒不是三粒。
  ⚠️ **连续第二波被 spot 回收重创**(W35 掉 3/4,#398),本波发波+补跑共吃 **9 次
  `InsufficientInstanceCapacity`** ⇒ `us-west-2` 的 `c6i.4xlarge` spot 容量**持续紧张**,
  而这是 owner 自己那条 **GH #158「spot 优先」**的输入。⛔ **本台不重裁 owner 的规则**,读数交 #403。
  **成本更正**:本波实为 **6 台**(4 + 2 补跑),但被抢占的两台各只活 4.25 / 7.5 分钟
  ⇒ 增量机时约 **0.20 h**,**单波成本仍远低于 `$0.90` 常数,自动失效条款不触发**;围栏 `~$4.8` ≪ `$80`。
  **泄漏**:收尾**固定两条**都做了,**而且本轮真的抓到了东西** —— (1) 波次点名:第一次四个
  `soak-run` 逐一对上,**收尾只剩两个 ⇒ 少了两台**(已补);补跑后四个重新对齐、无陌生实例;
  (2) 回收普查:**两行 `Server.SpotInstanceTermination`**,均在本波窗口内。
  常驻只有 AMI `ami-0a990a26d89c66547`。**无泄漏**(少的已补,多的一台都没有)。
...
### 2026-09-02T00:18–00:5xZ 批测台(**W36 收割;本轮不发波,闸 (i) 未解锁**)

⭐⭐⭐ **章程的回收处置按设计执行了,而且什么也没买到 —— 本轮立 GH #408。** W36 六台里**四台**被
`instance-terminated-no-capacity` 回收,其中**两台补跑机 `Status.UpdateTime` 精确到秒相同**
(`sir-kwi7gnvq` / `sir-wjbqgh9q`,**同为 2026-09-01T21:50:21Z**,**同在 us-west-2c**,活 5.63 / 5.38 分钟,
**产局为零**);两份 farm log 各以 `SPOT INTERRUPTION {"action":"terminate","time":"2026-09-01T21:52:21Z"}`
收尾 —— 同一次翻转的两分钟通知,与 SIR **两条独立第一手通道相互印证**。
⇒ **种子 2745 与 2922 各自「原机 + 补跑机」双双阵亡。** 往一个正在拒绝容量的窗口里重发是**重抽同一张必输的票**;
预登记条款**没有退避、没有市场升级、没有第二次补跑**,所以它**一次就终止,而那粒种子仍然是空的**。
⭐⭐⭐ **#403 的预测在六分钟内兑现,代价正是它点名的那两粒** —— 上一轮记下「补跑后 2+2 ⇒ 一次 AZ 事件带走两粒」,
六分钟后应验。**杀死补跑机的是集中,不是环内改投本身**(环确实走了,只是两台都落到当时唯一有容量的 2c)。
⛔ **本台不改市场/阶梯、不代数 #256 的重开计数、不重裁 owner 的 #158。**
**连续第三波**:W33 4/5 回收→1/4 粒、W35 3/4(同一瞬间)→1/4 粒、W36 **4/6**→**2/4 粒**;本波发波+补跑吃 **9 次
`InsufficientInstanceCapacity`** ⇒ **4 粒种子的 promote 门槛在当前市场下事实上够不着**,这是 #408 要裁的东西
(四条候选:退避 / 补跑打散 AZ / 只给补跑那台降阶梯 / 改判读门槛,**本台不代裁**)。
**收割**:`RV_EXIT=0`;**语料同一性先钉死** —— 交给工具的 54-id 串 md5 `74273b3ce5f58fcb50de769a700ab074` / 477 字节
与起飞记录**逐字节相同**;`files_seen 113 = games_loaded 113 = 64+49`,`source_dirs 2`,**R1/R2/R3 零拒答零 override**,
**#269 深度门零粒排除**(23.17 / 17.30 都带 margin)。
**读数(登记,不判读)**:mean gpm **−26.63** / xpm −13.28 / deaths +0.07 / last_hits −1.10,
`comps_better` **四项全 1/2**,`suggested: hold_or_reject`,101 局计分 / 0 局未完。
⛔ **n=2 且两粒在每一项经济量上符号相反**(2838 −64.22 / 2850 +10.95)⇒ **跨种子离散度大于均值**,
而按 4(i-c) 管精度的正是这个量;**54-id 家族只发过这一波且已被 §DH 解散 ⇒ 这个读数永远深不下去**。
**4(i-a) 分层读数**(取自工具自打的 `strata`,**未手算**,4(i-d)):gpm ab=114.47 ba=−167.74 side=141.11;
xpm ab=95.37 ba=−121.93 side=108.65;deaths ab=−1.75 ba=1.88 side=−1.81;last_hits ab=−1.82 ba=−0.39 side=−0.72。
**4(i-c)**:gpm/xpm/deaths `sign_flip` 为真(`side_gt_arm` 各 2/2)—— 已消侧偏的估计量,**反号是披露不是否决理由**。
⚠️ **胜负通道 DEGENERATE**(#352,少数侧 2/113 = **0.0177** ≪ 0.20,工具自打横幅)⇒ `winrate 0.464` 被语料限死,
**不得**作铁律 2(b) 依据;**连续第四波扫边**(W30 231:1 / W31 221:1 / W35 45:0 / W36 111:2,全 dire)。
⭐ **反向好消息交 #108**:`winner_by = {"engine_natural": 113}`、`winrate_independent_of_gold 113/113`
⇒ cap-25 后**自然结束率 1.00**,正是 #108 验收点名的量(**本台只给数据点,不代裁**)。
**两笔债**:§DC.3 (甲)(`illumove`×`illureal` 不正交)**无家可归** —— W35 n=1 还不上、W36 n=2 符号相反、家族已解散
⇒ **显式交回总监,债跟随这两条 id 下次入集的波**(铁律 9);§DF.6 已由 §DH 兑现(载体 ringmaster/brewmaster
**都不在 47 人池**,GH #402,**按构造域空,本轮不把任何读数当作对它们的证据**;**出集不是 reject**)。
**成本**:约 **2.2 机时**(两台 0.96h/0.90h + 四台合计 ~0.30h)⇒ **远低于 `$0.90/波` 常数,自动失效条款未触发**;
九月 MTD **`$3.637`**(`budgets` 免费通道,快照 `17:58:37Z`,**与上一轮同一张快照 ⇒ 本轮无新围栏信息**);
MTD `< $35` ⇒ **CE 复核未触发,那 `$0.01` 没花**。`forecast 150.867` 仍是开月外推,**不构成刹车信号**。
**泄漏:CLEAN** —— 收尾固定两条都做了:(1) 波次点名:`describe-instances` **不加状态过滤**返回**空**,
六个 `soak-run` 无一残留、**也无陌生实例**;(2) 回收普查:六个 SIR 全 `closed`、全在本波窗口内、
四 `no-capacity` + 两 `by-user`、**无第七个请求**;另 `describe-volumes` **空**。常驻仅 AMI `ami-0a990a26d89c66547`。
⚠️ **留档一条通道缺口**:被回收的机器上传了 `soak_farm.log`(中断处理器干的),而**两台正常自毁的机器
S3 前缀里根本没有 farm log** ⇒ **干净退出这条路上没有第二通道**,结算只剩 SIR。本波不受影响,交总监/harness 记档。
**本轮不发波**:闸 (i) 解锁 **2026-09-02T03:31:10Z**(W36 起飞 21:31:10Z + 6h),本轮 00:18Z 触发,差约 **3h13m**,
**一次付费调用都没发出**。闸 (iv) 输入**本轮已清**(收割完、wave 回填、queue 写回)。
**不为了「这轮得干点什么」去发一个闸没开的波。**
**queue.json**:**10 行**(`strategy-25/26/27/28/29/30/31/32` + `hero-24/25`)全部写回 W36 收割结果,
`rides_wave` 改记为「54-id 家族**唯一**一波,已收割」,**status 一律保持 `running`**;
`strategy-29/30` 另附 §DC.3 滚债、`strategy-31/32` 另附 §DF.6 域空。
**铁律 6**:`bots/`/`game/` **一行未改**(只动 `iterations/`);静态半由 `.githooks/pre-push` 在 push 时自跑;
**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。
**铁律 10**:⚠️ **本轮第一条命令又误用管道**,自检自己拒跑并打 `REFUSED … stdout is a pipe; exit 2, nothing checked`
(**管道守卫第 11 个外部验收点、同形状第七次**);改重定向重跑后 **`SELFCHECK_EXIT=124`**(600s 超时)
⇒ **本轮不存在自检总判决**,**连续第二轮**如此。日志已落地部分点名 `TRUNK RED`(python,**但日志自己写明
「main 是否也红这一行并未确立,须 `git stash` 后重跑」—— 本轮没跑那步,故不声称**)与
`test_incoming_damage_callsite_census.lua`(**GH #394 第四次复现未修**)。
**铁律 11**:未触发 `requires approval`;GitHub MCP 可用(#408 已开);**无空转等待**。
**交棒**:① ⭐⭐⭐ 总监 —— **GH #408**(补跑机制在容量紧张下无效,4 粒门槛够不着);
② ⭐⭐⭐ 总监 —— **#403 六分钟内兑现**;③ ⭐⭐ 总监 —— §DC.3 (甲) 的债无家可归;
④ ⭐⭐ 总监 —— **#108 数据点 `engine_natural 113/113`**;⑤ ⭐⭐ 总监 —— **#352 连续第四波扫边**;
⑥ ⭐ 总监/英雄组 —— **#394 第四次复现**;⑦ ⭐ 总监/harness —— 自检连续第二轮超时 + 管道守卫第 11 次;
⑧ ⭐ 总监/harness —— **farm-log 通道缺口**(干净自毁不上传);⑨ ⭐ 存量:**#388 W34 的 103 份 `.dem`
2026-09-22 到期(距今 20 天)** / #402 / #395 / 按需常数第五轮 / **#285 第二十一轮未裁** / #317 五条 /
`strategy-5b` 第十二轮 / #398 / #399 / #382 / #383 / #290 / #313 / #375 / #207 / #218 / #367 / #295 /
#252 / #256 / #282 / #329 / #321 / #271。
**下一轮本台 = 发 W37**(52-id 家族第一波),闸 (i) 解锁 **2026-09-02T03:31:10Z**。
⚠️ 发波前必读 **GH #408**:总监已裁则按裁定改补跑策略;**未裁则仍按预登记保守默认走 spot**
...
- 2026-09-02T06:15Z(**收割 W37;闸 (i) 未解锁 ⇒ 未发波;零 AWS 支出。本轮真正的产出是「并池判据被证伪」,不是那四个数**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**管道守卫第 13 个外部验收点、同形第九次**);
  改走重定向后 **`EXIT=124` = 400s 超时**,死在最后一条腿 `trunk health (python)` 上。
  ⛔ **`124` 不是通过也不是 `TRUNK RED`:总判决没拿到,python 那侧这轮没人看过**,据实登记不声称干净。
  已拿到的腿:`unlanded_commits` OK;`cadence` 2 findings(**本台自己 09-01T21:14Z→09-02T00:45Z 3.5h**);
  `stable-v1`/`v2` 锚点两个 OK;队列 57 open、`strategy-36` 仍欠路由裁定;`UNKNOWN STATUS` 5 条(#317)。
  **二、成本** 九月 MTD **`$3.637`**(`budgets` 免费通道,快照 `2026-09-01T17:58:37Z`,**与上一轮同一张 ⇒ 无新围栏信息**);
  MTD `< $35` ⇒ **CE 复核未触发,那 `$0.01` 没花**;**本轮零支出**;不跨任何 owner 可见告警档 ⇒ 不欠解释行。
  **W37 单波实测**:SIR 第一手机时 **2.4153 h** × $0.231–0.250 = **$0.558–$0.604** ⇒ **`$0.90` 自动失效条款不触发**。
  ⚠️ 但**便宜有一半是两台死得早**,不是效率变好,**不可当作 spot 单波价的新证据**。
  **三、⭐ SIR 抢在窗口内拿到,而 `describe-instances` 已经空了。** 四个 SIR **全读到**
  (`SIR_EXIT=0`);同一时刻 `terminated,shutting-down` **返回空**(最后一台 04:16Z 终止,本轮 06:1xZ 读,
  **已过 GH #375 的 ~1h 窗口**)⇒ **章程「这个字段必须在收割那一轮落盘」被实测再次证实**,SIR 是**唯一**还活着的来源。
  `ec2_state_reason_code` 四个全 `null`,**据实记 null,不拿代理量填**(代理量会**制造出并不存在的 `BRACKET VIOLATED`**)。
  **GH #412 词汇纪律已遵守**:`machines[].status_code` 写的是 SIR `Status.Code`。
  **四、局数** 2673 `ab31/ba11` depth 16.24 ✅(**41.9 min 被回收,但在 40.0 换腿点之后 ⇒ 仍配对**)/
  2735 `28/14` 18.67 ✅ / 2752 `29/13` 17.95 ✅ / **2874 `11/0` 0.0 ❌ `NO-PAIR`**(**15.6 min 就被回收**)。
  **ab/ba 高度不对称 88:38**。`files_seen 160 / games_loaded 160 / source_dirs 4 / unparseable 0 / 并池覆盖 0`;
  对账 **126 + 11 + 23 暖场 = 160 逐位对上**。**#269 深度门零粒排除**(2874 是 `NO-PAIR` 不是 `THIN-ARM`),
  **未传任何 override ⇒ 无「这是 SKIP 不是 pass」行**。
  `HARVEST run_id=spot_20260902_0331 games_total=160 games_effective=126 exclusions=warmup:23,no_pair:11`。
  ⭐ **`iterations/harvest_ledger.jsonl` 本轮建档并回填 W35/W36/W37 三行** —— 总监 08-30 提的那条**七天零落实**
  (交棒落错了字段),周日效率台账那格从本轮起**就是一次除法**,不再是 `NOT-COMPUTABLE`。
  **五、⚠️ 语料同一性:交给工具的串不是工作树里的那一条。** `test_set.md:2` 现已是 **54 id / md5 `80392b25…`**,
  而 W37 发的是 52 id ⇒ **照工作树取串会静默算错语料**。从 git 取回发波那棵树的那一行 ⇒
  **52 id / 459 字节 / md5 `c5af3b11…`,与 `arm_md5` 逐字节相同**。
  **暖场 `script_version` 断言(发波轮登记的收割义务)已履行**:23 局暖场**全部 `ada4717f`**(6/6/6/5,一致),
  非 `0a930645`、无第三个 SHA ⇒ 漏钉 `--ref` 本波无可测量代价,**但下次发波仍必须传 `--ref <40 位 sha>`**。
  **六、读数(登记不判读)** `RV_EXIT=0` 两次。**W37 单波 n=3**:gpm **+1.24** / xpm +3.77 / deaths −0.17 / lh −1.78,`comps 1/3·1/3·2/3·0/3`。
  **并池 W35 的 2986 ⇒ 52-id 家族 n=4 / 166 局**(每粒已 swap-average 再取**算术平均**,**未按局加权**,铁律 4(i-d)):
  gpm **−10.23**(`1/4`;逐粒 −2.32/−24.49/**+30.52**/−44.65,**sd 32.20 / se 16.10**)、xpm −1.80(1/4)、
  deaths −0.02(2/4)、lh −1.59(0/4);四粒 depth 16.24–18.67,**零排除**;`suggested hold_or_reject`。
  **4(i-a) 分层读数(工具自打,未手算)**:gpm ab `5.86` / ba `−26.33` / side `16.09`;xpm `2.88`/`−6.47`;
  deaths `0.44`/`−0.48`;lh `−8.20`/`5.02` —— **四项全反号**,但这是**侧偏已消除的估计量** ⇒
  **按 4(i-c) 反号不是否决理由**(它是 `|side|>|arm|` 的恒等式改写),**登记是 4(i-a) 的义务不是 4(i-b) 的判决**。
  ⛔ **胜负通道 `DEGENERATE`**(少数侧 **0.0683**,`mean winrate_headroom` **0.1071**,radiant 14 / dire 191)⇒
  **`winrate` 与 `comps_better winrate` 不得写成读数**,**铁律 2(b) 不能从胜负通道取证**;并池后**比单波更退化**。
  ✅ **`winrate_independent_of_gold` 打真值 `205/205`** ⇒ GH #108/#352 桶名 bug **确认已修**,08-29 立的逐波复发登记**可以停了**。
  **§DC.3 已登记**:`illumove`×`illureal` 同波 armed 且**同帧不正交**,**交集帧不能分摊归因**;
  本轮读数是**整包 52-id、无逐 id 主张** ⇒ 不违反,**该限定约束的是下一个做逐 id 归因的人**。
  ⛔ **本台不判读**:四个数、两粒反号、极差 75,promote/reject 归总监;**家族累计 4 粒 < 8,种子还没买齐**。
  **七、⭐⭐⭐ 并池判据被证伪(GH #418,本轮新立)。** 发波记录的理由是
  「落地的三个 id **都不在** armed 串里 ⇒ 行为等价」。按章程「**要判当前状态就去读当前树的源码**」逐行读 diff,**判据不成立**:
  **(甲) 真反例** —— `87bc6b33`(slotdust)夹带一行**闸外**改动:`jmz_func.lua:11520`
  `if dist < closestDist` → `if dist <= closestDist`,在循环体里、**不在 `if bSlotDust` 任何分支内** ⇒ **关闸照跑**;
  而其上方注释白纸黑字写 `-- Unarmed this is byte-for-byte the shipped function` —— **那句话是假的,
  而它正是下游据以判惰性的那句话**。影响面据实:只换**并列时的取胜方向**,两种写法都**恰好一个 bot 拿 true**
  (不是张开型),且现役 pid/slot 缺陷把域压到 dire 1 名 ⇒ **dire 侧结构上不可能并列**;**风险低但非零,且从没测过**。
  **(乙) 阴性一例,明写**(否则通过时它是隐形的)—— `0b468372`(slotarb)改的 `IsTheClosestOne`
  **正是 armed id `campsel` 也要走的那个包装器**;读源码确认 `bSlotArb` 假时 `nSlot = id`、归约回现役 ⇒ **确实惰性**,
  **但这个结论只能靠读代码拿到**。⇒ **判据应改写成「该 diff 在各闸关闭时逐行惰性,读源码作证」**;
  **失效方向是危险的那一侧**:现行判据把 (甲) 读成惰性,两波被当同一配置并池,**没有任何门会举手**。
  **处置**:并池**保留**但**登记为显式保留,不再声称逐字节等价**;`W37_wave.json:arm_note` 已就地更正。
  **八、市场:连续第四波被 no-capacity 咬**(W33 4/5、W35 3/4、W36 4/6、**W37 2/4**)。
  W37 起飞是四波里最干净的一次(4/4、四个 AZ 互不相同、零 `InsufficientInstanceCapacity`),**仍掉两台**。
  ⛔ 本台不重裁 owner 的 #158,读数交 **GH #408(仍零评论未裁)**。
  `reclaim_blind.py` **RB_EXIT=0** / `not blinded -- 3 paired seed(s)` / `NEXT WAVE: spot`。
  ⚠️ **第一次调用 `exit 2` 是我自己的调用错误**(位置参数,应为 `--wave-json`),**不是市场 UNDECIDABLE** ——
  点名登记,因为 GH #412 的立案句正是「把自造的拒答读成真拒答」。
  ⚠️ **载体薄的预登记风险兑现**:六项各恰好 2 粒,掉 2874 后 lion/OD/SK 三项**只剩 1 粒**;**归因是选种池不是市场**
  (popcount-4 层已空,**GH #285 第二十三轮未裁,下一波极可能直接搜出 0**,预登记右移 `[3001,3400]` 仍有效)。
  **九、发波决策:不发。** 闸 (i) **未满足** —— W37 起飞 `03:31:28Z`,解锁 **`2026-09-02T09:31:28Z`**,本轮 06:1xZ,**差约 3 小时**,
  无例外情形;(ii)(iii) 满足($3.637 + ~$0.6 ≈ $4.2 ≤ $80)。
  **十、泄漏:CLEAN** —— `--leak-only` **LEAK_EXIT=0**,**0 台 pending/running**;`describe-volumes` **返回空**(无孤儿);
  常驻仅 AMI `ami-0a990a26d89c66547`。本轮是收割轮不是发波轮,故不做发波轮那两条点名;
  **W37 的回收普查已在第三节以 SIR 通道做完**(且那是唯一还读得到的通道)。
  **十一、queue.json** 八行(`strategy-25/26/27/28/29/30` + `hero-24/25`)全部回填 W37 读数 + 并池读数 +
  **GH #418 保留**;`strategy-29/30` 另加 **§DC.3 交集限定**。`rides_wave` 标 `HARVESTED`,status 保持 `running`(4 粒 < 8)。
  **十二、铁律 6** `bots/`/`game/` **一行未改**;静态半 `luacheck_gate.sh` 见报告 §10;`arm_push_gate.sh` 已上膛;
  **未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。
  **铁律 11**:未触发 `requires approval`;GitHub MCP 可用(#418 已开);**无空转等待**。
  **交棒**:① ⭐⭐⭐ 总监/协同组 —— **GH #418**(新立;**连带要改并池判据本身**);
  ② ⭐⭐⭐ 总监 —— **GH #408 仍零评论未裁**,no-capacity **连续第四波**;
  ③ ⭐⭐ 总监 —— 52-id 家族 **4 粒 / 166 局**,gpm −10.23(1/4),胜负通道不可引用,**还差 4 粒到 8**;
  ④ ⭐⭐ 总监 —— **GH #285 第二十三轮未裁**,popcount-4 层已空;
  ⑤ ⭐ 本台下一轮 —— 闸 (i) `09:31:28Z` 解锁,**发波必须传 `--ref <40 位 sha>`**;
  ⑥ ⭐ 总监 —— **GH #413 / #412** 上一轮新立均未裁;
  ⑦ ⭐ 总监 —— **自检 python 腿本轮 400s 超时,那一侧这轮没人看过**;上一轮的 `illumove` python 红**未复核**;
  ⑧ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 20 天)** / #402 / #395 / #375 / 按需常数第七轮 /
  #317 五条 / `strategy-5b` 第十四轮 / #398 / #399 / #382 / #383 / #290 / #313 / #207 / #218 / #367 / #295 /
  #252 / #256 / #282 / #329 / #321 / #271 / `strategy-36` 路由未裁。
  **下一轮本台 = 判三条件,闸 (i) 于 `2026-09-02T09:31:28Z` 解锁**(满足则发 W38,52-id 家族第三波)。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260902T061500Z.md` 与 `iterations/reports/batch-desk/waves/W37_wave.json`。
- 2026-09-02T09:31Z(**发 W38 —— 54-id 家族第一波;闸 (ii) 走第一支,而那恰恰是「不能并池」的同一件事**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十次,每次都是当轮第一条命令**);
  ⚠️ **两个计数器对不上,据实并列**:工具自报 `it has recurred 5x`,报告序列记的是第 14 个外部验收点 / 第十次,
  **没有任何东西把这两个数对起来过**。改走重定向后拿到真判决 **`SELFCHECK_EXIT=3`**:
  `legs 8`,`FINDINGS = cadence + trunk-red(python) + trunk-red(lua)`,`UNCERTIFIABLE = none`。
  cadence 两处 GAP **与上一轮逐字相同**(本台 3.5h / strategy 5.0h,历史的洞);
  lua 红两个 —— `test_corpus_scale.lua`(根因是 **`tests/test_slotdust_dust_arbitration.lua:645` 的
  `nFixtures == 107` 等值断言**,GH #106/#127 同族)与 **GH #394 第六次复现**;
  python 红一个,**未 `git stash` 重跑故不声称 main 也红**;上一轮的 `illumove` python 红**未逐条复核,登记为未复核**。
  **二、⭐⭐ 本轮新立 GH #420(harness):自检 exit-sources 汇总块少一个桶。** 同一份输出里 python 腿打
  `73 passed, 1 failed, **2 uncertifiable**` 并**点名**了那两个文件,而汇总块打 **`UNCERTIFIABLE (exit 2): none`**。
  **不是打架是粒度**:汇总块按腿的 `note` 值分桶,该腿 `note 3` ⇒ 整条进 `FINDINGS`,**腿内没跑成的部分没有位置**。
  **失效方向危险**:只看汇总块的读者(那正是 GH #267 造它的用法)得到「没有东西没人看过」。
  更尖的是**没跑的两个是自检自己的自测**(`test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`),
  而本轮第一条命令又被那个管道守卫拒了一次。**与 #364 已划清**(#364 修好后这一格仍是空的)。
  发表前 `claim_precheck.sh` **exit 0 / `OK to publish`**(铁律 6 GH #290 已遵守)。
  **三、成本** 九月 MTD **`$3.637`**(`budgets` 免费通道,快照 `2026-09-01T17:58:37Z`,
  **与前两轮同一张 ⇒ 连续第三轮无新围栏信息**);MTD `< $35` ⇒ **CE 未触发,那 `$0.01` 没花**。
  围栏 = `3.637 + W37 实测 ~0.60 + W38 预估 spot 0.90` = **`$5.14`** ≤ **$80** ⇒ **闸 (iii) 过**;
  最坏全程降级按需 **`$6.39`** 同样过;**不跨任何 owner 可见告警档 ⇒ 不欠解释行**。
  **四、⭐⭐⭐ 闸 (ii) 走第一支,而它同时是「不能并池」的许可证反面。**
  `test_set.md` 于 09-02T04:xxZ 把 **`slotarb` + `slotdust` 同轮入集**(52 → 54,§DK,GH #406/#411,
  queue `strategy-34`/`strategy-35`)⇒ **armed 串真的变了**,第一支直接成立(第三支也成立:发波前 0 粒)。
  ⛔ **W38 不能与 W35/W37 并池** —— 那两波带 52-id 串(md5 `c5af3b11…`),**这两条 id 在那里不是 armed 的**。
  **52-id 家族就停在 4 粒 / 166 局**;54-id 家族**从本波起数**。并且按 **GH #418**,
  **并池判据本身正在返修**(「落地 id 不在 armed 串里 ⇒ 等价」已被 `87bc6b33` 的闸外一行证伪)⇒
  **今后并池必须逐行读关闸后的 diff**,不能靠 id 集合差集。**本轮已按新写法执行,判据措辞仍待总监改。**
  **五、四道门全绿** `tree_verified` = `git ls-remote origin main` = 本地 HEAD = **`7196468dad…`**(逐字节);
  `arm` = **54 ids / 476 字节 / md5 `80392b258fcd214cf351231be61d15a4`**(与 `test_set.md` 散文逐字对上);
  `carrier_terms.py --arm` **exit 0**(10 hero / 44 generic / 0 unresolved ⇒ 6 terms);
  `seed_draft.py --assert-carrier-from-arm` ⇒ **`CARRIER_GATE ids=10 seeds=4 exit=0`,六项全 `satisfied=2`**;
  `check_armed_wiring.py --cand` **exit 0**(`all 54 armed ids wired on HEAD`);
  `seed_roster_index.py --build` **exit 0**(**在选种之前**)。
  ⚠️ `check_armed_wiring.py` **第一次调用 `EXIT=2` 是本台自己的调用错误**(位置参数,应为 `--cand`),
  **不是工具对这棵树的拒答** —— 点名登记,因为 **GH #412 的立案句正是「把自造的拒答读成真拒答」**;
  顺带澄清:上一轮记的 `soak/` 路径 bug **这次没有咬**,第一次调用用的就是真实路径。**`exit 2` 不是通过。**
  **六、⭐⭐ `--ref` 钉 SHA 这一轮当场兑现,而 W37 恰好没钉。** 铁律 6 / GH #290 要求 push 排在
  发表引用之前,于是本轮报告提交 **`eea24818` 于 `09:26Z` 上了 main —— 正卡在门检与 `09:31:28Z` 起飞之间**,
  而**实例是 launch 时 clone `origin/main` 的**。**本波不受影响的承重理由是钉了 SHA**
  (实例 checkout `7196468dad…`,不是 boot 时刻的 tip)。另有一条独立理由**但它不承重**:
  `git diff 7196468dad..eea24818 -- bots/ game/` **为空**(只动 `iterations/`)——
  **写下来是为了不让人把巧合当成保证**。⇒ **推论**:本台每轮都在发波前后 push 报告,
  所以**「门检时 tip 对得上」与「起飞时 tip 对得上」从来不是同一句话**;W37 那次三次核对一致,
  是因为**那轮的 push 发生在起飞之后**,不是结构上安全。**`--ref <40 位 sha>` 从本波起当硬约束。**
  **七、选种:窗口 `[2601,3000]` 保留,右移没有触发。**
  `24 banked-or-drawn / 376 unused`,`popcount {0:50, 1:140, 2:141, 3:45}`,36 个 mask,**`BEST slots = 12`,20 个组合**。
  **预登记的右移条件是「搜出 `0`」,本轮搜出 `12` ⇒ 不触发,不提前右移、不放松 `≥2/term`**(第二十四轮未裁)。
  但**周期性机制仍在跑**:`popcount 4` 层**仍空**、`popcount 3` 从 **49 掉到 45** ⇒ **连续两波卡在下限**。
  排除集 = 索引 banked ∪ **W35/W36/W37 抽过的十二粒**(抽过就是抽过)。
  同层零成本次序偏好(pudge)⇒ 选 **2613 / 2665 / 2761 / 2840**,`--seed` 复核**四粒全 `no banked games`**。
  ⚠️ **载体冗余仍薄**:六项各恰好 2,**掉任何一台该项就到 1**;**pudge 只剩 1 粒**(W37 有 2)。
  **八、⭐ 起飞:四台全 `SPOT_RUN_EXIT=0`,4/4 请求即实得,四个 AZ 互不相同。**
  守卫 `09:31:32Z` 打 `GATE-I OPEN`(**先扣住过一次**并打 `GATE-I NOT OPEN: now=09:18:07Z unlock=09:31:28Z`)。
  `2613→2a` / `2665→2b` / `2761→2c` / `2840→2d`;**`InsufficientInstanceCapacity` 零次、
  `re-aiming` 一行都没有、`AZ RING EXHAUSTED` 没有、市场降级阶梯一级都没下**。
  `InstanceLifecycle` 四台全 **`spot`**、全 `running`、四个 SIR 互不相同(**这才是 owner DoD 要的证据,
  `run_id` 的 `spot_` 前缀不是**);四个 `RUN_TOKEN` 互不相同。
  **GH #252 / #256 验收严格通过,而两个 issue 均已 CLOSED ⇒ 不欠追评**(查过再说的)。
  ⚠️ **这是关于此刻容量的读数,不是对 GH #408 的反驳** —— 连续四波被咬 3/3/4/2 台,一次干净起飞注销不了那条。
  **九、泄漏:CLEAN**(发波轮两条点名都做了)—— (1) `soak-run` **恰好四个值**,与 `machines[].run_id` 一一对上;
  ...
  **十、queue.json** 12 行更新:八行(`strategy-25/26/27/28/29/30` + `hero-24/25`)`rides_wave` 改记 W38
  并写明其 id 全在 54-id 串里;**`strategy-34`/`strategy-35` 首次搭车**,各带
  **§DK.3 负控 / §DK.1(v-b) / §DK.5 不许并池**三条收割轮必读限定(**写进 `rides_wave` 不是散文**,§AW.1 的教训);
  `strategy-31/32` **连续第二轮不搭任何波**,按铁律 9 **再次显式交回总监**。
  **十一、铁律 6** `bots/`/`game/` **一行未改**;静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**
  (冷启自装 `lua-check`,`bots game: 0 警告`),**两次 `git push` 各自又跑了一次钩子,均绿**;
  `arm_push_gate.sh` 已上膛;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。
  **铁律 11**:未触发 `requires approval`;GitHub MCP 可用(#420 已开、#252 已读);**无空转等待**
  (闸 (i) 前那 ~13 分钟全用在零成本准备上:成本、四道门、选种、wave 记录、立案、两次 push)。
  **交棒**:① ⭐⭐⭐ 全体 —— **W38 不可并池**,收割照 `W38_wave.json:harvest_obligations` 七条;
  ② ⭐⭐⭐ 总监 —— **GH #418 未裁,且连带要改并池判据措辞**;③ ⭐⭐ 总监 —— **GH #420(新立)**;
  ④ ⭐⭐ 总监 —— **GH #408 仍零评论未裁**,下一轮收割给第五个数据点;
  ⑤ ⭐⭐ 总监 —— **GH #285 第二十四轮未裁**,连续两波卡在 `BEST slots = 12` 下限;
  ⑥ ⭐ 总监/英雄组 —— `nFixtures == 107` 等值断言 + **#394 第六次复现**;
  ⑦ ⭐ 总监/协同组 —— `strategy-31/32` 连续第二轮不搭波;⑧ ⭐ 总监 —— `strategy-36` 路由第二轮欠着;
  ⑨ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 20 天)** / #413 / #412 / #402 / #395 /
  #375 / 按需常数第八轮 / #317 五条 / `strategy-5b` 第十五轮 / #398 / #399 / #382 / #383 / #290 /
  #313 / #207 / #218 / #367 / #295 / #282 / #329 / #321 / #271 / #364。
  **下一轮本台 = 收割 W38**(四台预计 `~10:1x-10:3xZ` 自毁);闸 (i) 对下一次发波解锁于 **`2026-09-02T15:31:32Z`**。
  **收割两件必做**:(a) `machines[].status_code` 走 **SIR** 通道、**SIR 词汇**(GH #412);
...
- 2026-09-02T15:15Z(**收 2613 补跑把 W38 补成 4 粒;发 W39 = 55-id 家族第一波。本轮真正的产出是「GH #408 的按需补跑路径把自己的证据窗口丢了」,不是那四个数**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十一次,每次都是当轮第一条命令**);
  ...
  改走重定向后 **`SELFCHECK_EXIT=3`**:`legs 8`,`FINDINGS = cadence + trunk-red(python) + trunk-red(lua)`,`UNCERTIFIABLE = none`。
  cadence 两处 GAP 与上一轮逐字相同;**但本轮有个它还看不到的新洞:本台 12:15Z → 15:2xZ 约 3h,14:xxZ 那次触发零产出**
  —— **那个洞花掉的正是第三节的读数**。python 红 `test_detector_source_constants.py`(未 stash 重跑,不声称 main 也红);
  lua 红 `test_incoming_damage_callsite_census.lua`(43→44,与上一轮同一条)。
  ⭐ **GH #420 原样复发**:python 腿自打 `2 uncertifiable` 并点名两个文件,汇总块却打 `UNCERTIFIABLE: none`。
  **二、成本** 九月 MTD **`$3.637`**(`budgets` 免费通道,快照 `2026-09-01T17:58:37Z`,**连续第四轮同一张 ⇒ 无新围栏信息**);
  MTD `< $35` ⇒ **CE 未触发,那 `$0.01` 没花**。围栏 = `3.637 + W38 spot ~0.60–0.64 + 2613 按需补跑 ~0.60–0.63 + W39 spot 0.90`
  = **`~$5.77`** ≤ **$80**;最坏全程降级 **`~$7.02`** 同样过;**不跨任何 owner 可见告警档 ⇒ 不欠解释行**。
  **`$0.90` 自动失效条款不触发**(W38 spot 单波 $0.60–0.64)。
  **三、⭐⭐⭐ GH #408 的第一个数据点永久丢了,而丢法是结构性的。** 上一轮把「给它第一个完整数据点」交给本轮;
  **拿不到了**:补跑机 `i-0075962af14069424` 12:18:43Z 起飞、13:10:53Z 交完最后一局随即自毁,本轮 15:2xZ 读,
  **两条普查都返回空**(各自 exit 0)⇒ `ec2_state_reason_code` **永久不可恢复**。
  记 **`UNRECOVERABLE`,不是「null 因为干净」而是「null 因为窗口关了」** —— 这两件事在 JSON 里长得一模一样。
  **结构性的那一半**:**按需机没有 SIR**,而 SIR 正是让 spot 机的码活过 describe-instances 窗口的通道
  ⇒ **GH #408 那条路的证据半衰期比它取代的 spot 路径严格更短**,`status_code` **必须在终止后 ~1h 内落盘**。
  **四、W38 补齐(4 粒 / 211 局)** `RV_EXIT=0`;`files 235 seen / 235 loaded / 0 unparseable / 4 dirs`;
  **语料同一性**:交给工具的串从**发波那棵树**取回(工作树已是 55 id),**54 id / 476 字节 / md5 `80392b25…` 与 `arm_md5` 逐字节相同**。
  补跑机三条断言全 PASS:6 局暖场全 `7196468d`(**承重** —— `origin/main` 已漂到 `1a13b9df…` 且 `bots/` 真的动了)、
  53 局计分全打 `mirror:<54-id>:s2613:<side>` 0 处不匹配、**采集配置按语料查证 slot 1..16**(与另三台相同)。
  **读数(登记不判读)**:gpm **+18.10** / xpm +0.11 / deaths +0.20 / lh −0.57;`comps 3/4 · 2/4 · 1/4 · 2/4`;
  逐粒 gpm 48.93(补跑)/ −6.41 / 12.91 / 16.97 ⇒ **sd 22.94 / se 11.47**;
  `arm_depth` 22.34 / 26.21 / 26.79 / 17.95,**零排除**、`thin_arm_seeds` 空、**未传任何 override ⇒ 无「SKIP 不是 pass」行**;
  `suggested hold_or_reject`。**4(i-a) 分层(工具自打)**:gpm ab `124.47` / ba `−88.27`;xpm `106.27`/`−106.04`;
  deaths `−1.63`/`+2.04`;lh `18.22`/`−19.37` —— **四量全反号、side_gt_arm 4/4(deaths 3/4)**,按 **4(i-c)** 是恒等式不是诊断。
  ⛔ **胜负通道仍 `DEGENERATE`**(少数侧 11/235,share 0.0468;headroom 0.0813)⇒ **winrate 不得写成读数**。
  ⚠️ **补跑那粒是四粒里最大的正值,且是唯一在不同时刻/不同市场类型/不同实例上跑的**;`--ref` 钉树,钉不住时刻与机器 —— **登记不解读**。
  ⭐ **新读法陷阱**:`reclaim_blind.py` 打 `3 paired seed(s)`,而它只数 `machines[]`、数不到住在 `rerun` 里的补跑机
  ⇒ **W38 的实际产出是 4 粒**。**失效方向是花钱那一侧**(拿 yield 决定补跑 ⇒ 重复补同一粒)。本轮开 **GH #429**(`[harness]`)。
  **五、W39 发波:四闸全过,GH #424 的阻塞已解。** (i) 守卫 `15:31:51Z` 打 `GATE-I OPEN`(先扣住 ~10 分钟);
  (ii) 走**第一支** —— `slotpush` 入集(54 → 55,§DN,GH #415),**GH #424 已于 13:21:27Z CLOSED**、落地 `d16227aa` 在 main(本轮核过);
  (iii) 见第二节;(iv) **本轮亲手收完 W38 最后一条臂才发波**,`reclaim_blind` exit 0 / `NEXT WAVE: spot`。
  四道门:`tree_verified` = `git ls-remote origin main` = 本地 HEAD = **`1a13b9df15f23bef704fecc3828ee7e3eb21fc67`**;
  `arm` **55 ids / 485 字节 / md5 `bfe60fcdfbe466440213541c8f37a254`**(与散文逐字对上);
  `carrier_terms.py` exit 0(10 hero / 45 generic / 0 unresolved ⇒ 6 terms);`check_armed_wiring.py --cand` exit 0(`all 55 armed ids wired`);
  `seed_roster_index.py --build` exit 0(**在选种之前**);`CARRIER_GATE ids=10 seeds=4 exit=0`,**六项全 `satisfied=2`**。
  ⚠️ **三处 exit 2 全是我自己的调用错误**(自检误用管道 + `carrier_terms.py` 路径写错 + `--arm` 漏值),**逐条点名**(GH #412 的立案句)。
  **采集配置一个字未动**(`--slots 16 --hours 2 --rec-slots 8 --games 12`,4×1)—— 本波改了 id 集,按章程不许同时改采集配置。
  **六、选种** 窗口 `[2601,3000]` 保留(右移条件是「搜出 0」,本轮搜出 12);`372 available / 28 banked-or-drawn`;
  `popcount {0:50, 1:140, 2:141, 3:41}`;34 个 mask;**`BEST slots = 12`,该层只剩 `5` 个组合(上一轮 20)**。
  **`popcount 4` 仍空、`popcount 3` 45 → 41、连续第三波卡在下限** ⇒ **削得比上一轮快**(GH #285 第二十五轮未裁)。
  选 **2740 / 2877 / 2896 / 2914**,四粒全 `no banked games`。
  ⚠️ ⭐ **零成本 pudge 偏好本轮拿到 `0`**(最优层 5 个组合的 mask 池里一个 pudge 载体都没有;W37 有 2、W38 有 1)
  ⇒ **`strategy-26` / `rotscope` 本波拿不到任何 pudge 证据**;已写进 queue,**收割时不许把它写成负向读数**。
  **七、⛔⛔ 起飞:四台全 `SPOT_RUN_EXIT=0`,但 AZ 分散度是 GH #252 落地以来最差的一次。**
  三次 `InsufficientInstanceCapacity`(2a 一次、2b 两次)⇒ 两行 `re-aiming inside the ring`,
  **四台只落到两个 AZ(`2c`×3 / `2d`×1)**。#252 的验收(≥2 不同)过,**#256 的严格验收(4 不同、无 re-aiming)不过**;
  零 `AZ RING EXHAUSTED`、零市场降级、四台全 `InstanceLifecycle=spot`、四个 SIR / 四个 RUN_TOKEN 互不相同。
  **⭐ 机制**:环走是**确定性单向**(a→b→c→d)⇒ 全区容量紧张时**每一次改投都收敛到同一个「第一个还有容量的 AZ」**,
  **#252 买来的放置去相关恰恰在它被买来对付的工况下被抵消**;#256 抓到的是一跳版本,**这是多跳版本,挤了 3/4**。
  **暴露面具体**:六项载体各恰好 2,而 `2c` 那三台之间带着**全部六项** ⇒ 一次 `2c` 事件把**五项打到 ≤1、`obsidian_destroyer` 打到 0**。
  本轮开 **GH #428**(`[harness]`),**本台不自己改 `spot_run.sh`**。
  ⚠️ **连续第六波被容量咬,且第一次咬在起飞而非飞行中**(W33/W35/W36/W37/W38 回收 4/3/4/2/1 台)⇒ **GH #408 第五、第六个数据点**。
  **八、局数(铁律 7)** (a) W38 最终 **4 粒 / 211 计分局**(2613 `37/16`、2665 `38/20`、2761 `37/21`、2840 `29/13`;
  **ab:ba = 141:70,高度不对称**)+ 24 局暖场;(b) W39 起飞时 **0 局**,预期 **~96 计分局**,预计 `~16:1x–16:3xZ` 自毁。
  **九、泄漏:CLEAN** —— 进场 `pending,running` 0 行、`terminated,shutting-down` 0 行;
  发波后两条点名都做了:`soak-run` **恰好四个值**与 `machines[].run_id` 一一对上、`terminated,shutting-down` **返回空**;
  `describe-volumes` 四卷全 `in-use` 无孤儿;`--leak-only` **`LEAK_EXIT=0`**。常驻仅 AMI `ami-0a990a26d89c66547`。
  **十、queue.json** 13 行:十行(`strategy-25/26/27/28/29/30/34/35` + `hero-24/25`)`result` 追加 **W38 全波 4 粒读数**
  并把 `rides_wave` 改记 **W39**;**`strategy-28` 另加 SS-DN.6 两条硬限定**(`outlatch` 的 (a) 两分层分开登记、
  **反号按 4(i-b) 读成噪声而不是 4(i-c)**;**不许跨 54/55 波比**);`strategy-26` 另加**本波零 pudge**;
  `strategy-36` `pending` → **`running`**(首次真正搭上波);**`strategy-31/32` 连续第三轮不搭波,按铁律 9 第三次交回总监**。
  **十一、铁律 6** `bots/`/`game/` **一行未改**;静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(`bots game: 0 warnings`),
  `arm_push_gate.sh` 已上膛,**两次 `git push` 各自又跑一次同一道门**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;
  动态半(#124)**未跑不声称**。另跑 `tests/test_wave_gate_keys.py` ⇒ **exit 0 / `151 checks, 0 failed`**
  —— **W39 的四道闸键在起飞当轮就是齐的**(W38 是收割轮补写的)。
  **铁律 11**:未触发 `requires approval`;GitHub MCP 可用(读了 #424 正文与追评);**无空转等待**(闸 (i) 前 ~17 分钟全用在零成本工作上)。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #427(新开)**:GH #408 按需路径的证据半衰期,第一个数据点**已永久丢失**;
  ② ⭐⭐⭐ 总监 —— **GH #428(新开)**:AZ 环走确定性单向,容量紧张时挤回同一个 AZ,W39 现在就有 3/4 在 `2c`;
  ③ ⭐⭐⭐ 全体 —— **W39 不可与 W38/W35/W37 并池**,收割照 `W39_wave.json:harvest_obligations` **十二条**,
  其中 **SS-DN.6 两条本波第一次真的咬到**;④ ⭐⭐ 总监 —— **GH #408 仍零评论未裁**,连续第六波被容量咬;
  ⑤ ⭐⭐ 总监 —— **GH #418 / #420 均未裁**;本轮另开 **#427 / #428 / #429**;⑥ ⭐⭐ 总监 —— **GH #285 第二十五轮未裁**,最优层组合数 20 → 5;
  ⑦ ⭐ 录像组 —— **GH #423** 仍挂着,`slotarb` 条件 (a) **再发波也买不到**;
  ⑧ ⭐ 总监/协同组 —— `strategy-31/32` 第三轮不搭波、`strategy-37`/`strategy-38` 路由未裁;
  ⑨ ⭐ 总监/英雄组 —— 43→44 那条 lua 红、`test_detector_source_constants.py` 红、两个 `UNCERTIFIABLE`(即 #420);
  ⑩ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 20 天)** / #413 / #412 / #402 / #395 / #375 /
  按需常数第十轮 / #317 五条 / `strategy-5b` 第十六轮 / #290 / #313 / #207 / #271 / #364 / #398 / #399 / #382 / #383 /
  #218 / #367 / #295 / #329 / #321。
  **下一轮本台 = 收割 W39**(四台预计 `~16:1x–16:3xZ` 自毁);闸 (i) 对下一次发波解锁于 **`2026-09-02T21:31:51Z`**。
  **收割三件必做**:(a) `machines[].status_code` 走 **SIR** 通道、**SIR 词汇**(GH #412);
  (b) 暖场 `script_version` 断言 = `1a13b9df…`;
  (c) **若本波需要按需补跑,它的 `StateReason.Code` 必须在那台机器终止后 ~1h 内落盘**(按需机没有 SIR,过期永久没有)。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260902T151500Z.md`、`iterations/reports/batch-desk/waves/W38_wave.json`(`rerun` + `harvest_full_wave` 两节本轮回填)
  与 `iterations/reports/batch-desk/waves/W39_wave.json`。

- 2026-09-02T18:30Z(**收割 W39。本轮真正的产出不是那四个数,是「被抢占 ≠ 丢臂」第一次被语料证伪 —— 只读 SIR 码会让下一轮去补跑一粒已经跑完的种子。本轮零发波、零支出**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十二次,每次都是当轮第一条命令**);工具自报仍是 `it has recurred 5x`,与报告序列的计数依旧对不上,照上一轮据实并列。
  改走重定向后 **`SELFCHECK_EXIT=3`**:`legs 8`,`FINDINGS = cadence + trunk-red(python)`。
  ⭐ **GH #420 第三轮原样复发**:python 腿自打 `74 passed, 1 failed, 2 uncertifiable` 并**点名** `test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`,汇总块却打 `UNCERTIFIABLE: none` —— **同一份日志里两处读数互相矛盾**。
  python 红 `test_detector_source_constants.py`(未 stash 重跑,不声称 main 也红)。⭐ **Lua 腿本轮 73 个 tagged 文件 0 failures ——上一轮那条 `test_incoming_damage_callsite_census.lua` 43→44 的红已经不在了**(英雄组三个 commit 落在中间)。
  **二、成本** 九月 MTD **`$3.637`**(`budgets` 免费通道,快照 `2026-09-01T17:58:37Z`,**连续第五轮同一张 ⇒ 无新围栏信息**,W38/W39 一分钱都还没进去);MTD `< $35` ⇒ **CE 未触发,那 `$0.01` 没花**;在跑实例**为空**。
  围栏(2 甲)= `3.637 + W38 spot ~0.60–0.64 + 2613 按需 ~0.60–0.63 + W39 spot 0.70–0.76` = **`~$5.6`** ≤ **$80**,**不跨任何 owner 可见告警档 ⇒ 不欠解释行**。
  **W39 机时反解 3.045 机时 ⇒ $0.703–$0.761,低于 $0.90 常数 ⇒ 自动失效条款不触发**(⚠️ **这是机时推算不是账单侧读数**,禁用的增量对账手法一次都没用)。
  **三、⭐⭐⭐ W39 收割:被抢占的那台把 48 局全交了。** `RV_EXIT=0`;`192 seen / 192 loaded / 0 unparseable / 4 dirs`;**scored 168**、`unfinished 0`。
  **语料同一性**:armed 串**从本波自己的记录取回**(不从当前 `test_set.md` 取)—— 55 ids / 485 字节 / md5 `bfe60fcd…` 与 `arm_md5` 逐字节相同(另核:`test_set.md` 第 2 行本轮仍是同一个 md5,没漂)。
  seed **2740**(`i-022ceff8ec8ab5827`,2c)带 SIR **`instance-terminated-no-capacity`**(16:18:35Z),**只读这个码,本轮该写「第七波被咬、丢一条臂」并考虑补跑一台**。
  **语料说反面**:它交了 **48 局**,与三台干净机(全 `instance-terminated-by-user`,16:15:18 / 16:16:22 / 16:23:18Z)**一模一样**,而**它的最后一局落在被抢占前 17 分 52 秒**(16:00:43Z)。
  四台跑的是同一个 48 局程序(6 暖场 + 42 计分),跑完后都在上传/收尾阶段待了约 15 分钟;三台从那里自毁,2740 是在那里被回收的。⇒ **不欠任何补跑、没有一粒缺臂,W39 = 4 粒 / 168 局**。
  ⭐ **一般结论**:**SIR 码回答的是「这台机器怎么死的」,不是「这台机器交没交货」** —— 两个问题**在这里第一次分了家**,而**只读 SIR 的失效方向是花钱那一侧**(去补跑一粒已经完整的种子),与 **GH #429** 给 `reclaim_blind.py` yield 记的失效方向**是同一个**。**买补跑之前先看语料。**
  **四、读数(登记不判读)** gpm **−12.58** / xpm **−5.78** / deaths **+0.04** / lh **−1.02**;`comps_better` **四量全 1/4**;`suggested hold_or_reject`。
  逐粒 gpm **−23.68 / +13.42 / −3.21 / −36.86**;`arm_depth` **10.29 / 18.67 / 17.95 / 17.14**,`min_arm_depth 8`、**`thin_arm_seeds` 空**、**未传任何 override ⇒ 无「SKIP 不是 pass」行**。
  **4(i-a) 分层(工具自打,未手算)**:gpm ab `+103.12` / ba `−128.30`;xpm `+72.00`/`−83.56`;deaths `−1.39`/`+1.46`;lh `+11.78`/`−13.83` —— **四量全 sign_flip、side_gt_arm 4/4**,按 **4(i-c) 是恒等式不是诊断**,登记不据此下结论;并池由工具**逐粒 swap-average 后取算术平均,未按局数加权**(4(i-d))。
  ⛔ **胜负通道仍 `DEGENERATE`**(少数侧 **8/192**,share 0.0417;`mean.winrate_headroom` **0.0625**;**2740/2896/2914 三粒 headroom 恰为 0.0**)⇒ 按 SS-CT **winrate 0.478 与 comps_better winrate 0/4 写成占位符,不得引作 (b)**。
  `winrate_independent_of_gold` **`192/192`**、`winner_by {engine_natural:192}` ⇒ 桶名 bug 修复**第二波复核通过**。
  ⚠️ **ab:ba = 123:45,2740 更是 36:6 —— 这不是被抢占造成的**:换边按**经过时间**触发不按局数,四台同一墙钟窗口各交 48 局,radiant 腿慢的那粒翻得晚。2740 的 `arm_depth 10.29` 过门故计分,但**它是本波最薄的臂,要怀疑先怀疑它**。
  **五、三条断言全 PASS**:暖场 `script_version` = `1a13b9df`(4×6 = 24 局全中);168 计分局全打 `mirror:<55-id>:s<seed>:<side>` **0 处不匹配**;采集配置**按语料查证** slot 1..16(四 run 相同,与 W38 逐项一致)。
  ⚠️ **`--ref` 本波正确但不承重,据实登记**:`origin/main` 确实漂了(`1a13b9df` → `864b7d6a`),但 **`git diff … -- bots/ game/` 是空的**,被测面没动;W38 那次它**是**承重的。**没赔付的保险和赔付过的保险不是同一个主张。**
  **六、⭐⭐ 只有被抢占那台产出 31 个重名副本(开 GH #432)。** `724bad` 多出 31 个 `analysis_<TS>_slot<N>.json`(与正常 `<TS>_slot<N>.analysis.json` **镜像对称的第二套命名**),另三台一个都没有。
  查证:**overlap 31/31**(全对得上已计数的 `(时间戳, slot)`)、**31 个逐字节相同** ⇒ **重复上传不是漏局**。
  **对本次收割零影响**(`recover_verdict.py` 的 glob 是**精确的** `*.analysis.json`,匹配不上第二套)。
  ⛔ **但隐患指向花钱那一侧**:任何用 `*analysis*.json`(**更松也更自然**)的消费者会把 2740 的 31 局**数两遍** —— 灌水的正是**本波最薄的那条臂**,方向是**让读数看起来比实际好**,而**两种情况下工具都打 `files_seen == games_loaded`,没有门会举手**。**本台不自己改上传脚本。**
  **七、本轮不发波,卡在闸 (i)**:W39 起飞 `15:31:51Z` ⇒ 解锁 **`2026-09-02T21:31:51Z`**,本轮约 `18:4xZ`,**还差约 2h45m**。
  (ii) `test_set.md` 仍 `bfe60fcd…`/55 ids、`bots/` 自 `1a13b9df` 未动 ⇒ 第一支不成立,**第三支(累计种子 < 8)成立**;(iii) 过。**卡在 (i),不是卡在钱也不是卡在没东西测 —— 不发波、不空转等待**(铁律 7/11)。
  **八、收割义务十二条逐条核对**(见报告 §8):4(i-a)/(i-d)/winrate/SS-DK.1/DK.3/DK.5/不并池/零 pudge/GH #412 全部照做;
  **SS-DN.6 本轮不适用** —— `outlatch` 的 (a) 是**帧级检测器读数,不在本语料里**(与 SS-DK.3 同因),经济四量按 4(i-c),**两者没混**;`rerun_evidence_window` **不适用,本波不欠补跑**。
  **九、泄漏:CLEAN** —— `--leak-only` **`LEAK_EXIT=0`**、`pending,running` 空、`describe-volumes` 空;常驻仅 AMI `ami-0a990a26d89c66547`。
  ⚠️ **`terminated,shutting-down` 也是空的,而那个空是 GH #375 窗口过期不是「没东西终止过」** ⇒ 四台 `ec2_state_reason_code` 记 **`UNRECOVERABLE`**(「null 因为窗口关了」,不是「null 因为干净」);**本波全 spot,SIR 保住了 `status_code`,换成按需就什么都不剩** —— **GH #427 的第二个数据点**。
  ⚠️ 再登记:`--leak-only` **只报「多出来的」,「少一台」永远不举手**。
  **十、queue.json** 11 行(`strategy-25/26/27/28/29/30/34/35/36` + `hero-24/25`)写入 W39 全波读数,status → `harvested_pending_verification`。
  ⚠️ **一次自造失误,当场抓住并撤回**:回填脚本按 `'W39' in rides_wave` 匹配,把 **`strategy-31/32` 误伤了** —— 它俩正文里的 "W38 没搭,**W39** 也没搭" 含这三个字符。已撤回读数、status 复原 `running`,并在行内写明经过。**把一波的读数写进没搭这波的行,正是「读数看起来合理、没有任何门会举手」的静默污染**;`strategy-31/32` **连续第四轮不搭波**,按铁律 9 第四次交回总监。
  **十一、铁律 6** `bots/`/`game/` **一行未改**(改动全在 `iterations/`);静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(`bots game: 0 warnings`,容器冷启由脚本自己装 luacheck),`arm_push_gate.sh` `ARM_EXIT=0` 已上膛,**两次 `git push` 各自又跑一次同一道门**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。另跑 `tests/test_wave_gate_keys.py` ⇒ **exit 0 / `151 checks, 0 failed`**(回填后复跑仍 0)。
  **铁律 11**:未触发 `requires approval`;GitHub MCP 可用(开了 #432);**无空转等待**。
  **交棒**:① ⭐⭐⭐ 全体/总监 —— **「被抢占」不等于「丢臂」**,买补跑前先查语料;② ⭐⭐⭐ 总监 —— **GH #432(新开)**;
  ③ ⭐⭐ 总监 —— **GH #427 第二个数据点**;④ ⭐⭐ 总监 —— **GH #408 连续第七波被容量咬,仍零评论未裁**;
  ⑤ ⭐⭐ 总监 —— **GH #428 本波已兑现风险的一半**(被回收的恰是挤在 `2c` 那三台里的一台);
  ⑥ ⭐⭐ 总监/英雄组 —— GH #420 第三轮复发、`test_detector_source_constants.py` 仍红(Lua 腿已转绿);
  ⑦ ⭐ 总监/协同组 —— `strategy-31/32` 第四轮不搭波;⑧ ⭐ 录像组 —— **GH #423** 仍挂着,`slotarb` 的 (a) 再发波也买不到;
  ⑨ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 20 天)** / #429 / #418 / #420 / #413 / #412 / #402 / #395 / #375 / 按需常数第十一轮 / #317 五条 / `strategy-5b` 第十七轮 / #290 / #313 / #207 / #271 / #364 / #398 / #399 / #382 / #383 / #218 / #367 / #295 / #329 / #321 / **#285 第二十六轮未裁**。
  **下一轮本台 = 发 W40**(闸 (i) 解锁于 **`2026-09-02T21:31:51Z`**;届时 55-id 家族累计 4 粒 ⇒ 闸 (ii) 走**第三支**,除非 `test_set.md`/`bots/` 先动)。
  **发波三件必做**:(a) 发波当轮**亲手跑** `test_wave_gate_keys.py` 与 `reclaim_blind.py --wave-json W39`(**别再留给收割轮补写**);
  (b) `seed_roster_index.py --build` **在选种之前**,且把 W39 那四粒(2740/2877/2896/2914)记进排除集;
  (c) `reclaim_blind.py` 读 W39 时**记得它数的是 `machines[]`**,而本波**没有 `rerun`,四粒全在 `machines[]` 里 ⇒ 这次它数得对**。
  **本轮 token**:见报告最后一节。
...
- 2026-09-02T21:20Z(**执行 GH #413 那根掉了两轮的棒 + 发 W40 = 55-id 家族第二波。本轮真正的产出是「重建的 baseline 落地当场就量出:把验收门跑在它自己的语料上会打出一张零信息的满分表」,不是那四台**)。
  ...
  **六之一、⭐⭐⭐ 本轮真正该升级的:载体冗余与 AZ 塌缩相乘(GH #435 已开)。** `CARRIER_GATE` 打 `六项全 satisfied=2`,**为真且有误导性**:削到最优层只剩 2 个组合后,那 2 个组合是**两对相同掩码** —— **A 对 2641+2710** 各扛 `crystal_maiden`/`obsidian_destroyer`/`skeleton_king`,**B 对 2921+2941** 各扛 `lion`/`spirit_breaker`/`zuus` ⇒ **每项的「两个载体」是同一对的两个成员,不是两次独立抽取**。而环走把 **A 对整对放进 `2c`** ⇒ **一次 `2c` 事件把那三项打到 `0` 而不是 1**(W39 六项散在四台、无此配对结构)。
  ...
  **十、泄漏:CLEAN,判据是「恰好那几台」不是「0 台」**:起飞后 `21:33Z` 第一次点名 —— `soak-run` **恰好四个值**与 `machines[].run_id` 一一对上,`terminated,shutting-down` 空,四卷全 `in-use` 无孤儿;⛔ `21:44Z` 第二次点名**只剩一台**(见六之二),而 **`LEAK_EXIT` 那时依然是 0** —— **举手的是固定动作不是 `--leak-only`**(六之五);`21:46Z` 补跑起飞后第三次点名 = **恰好三台**,与 `machines[]` ∪ `rerun.machines[]` 的存活集合一一对上,无多余无泄漏。常驻仅 AMI。
  **六之二、⛔⛔⛔ 起飞 7 分钟后:上一段预登记的暴露面兑现了,而 GH #435 是在事件之前 7 分钟开出的。**
  `21:44Z` 收尾点名时在跑只剩一台:**`us-west-2c` 那三台在同一秒被回收**
  (2641/2710/2921,EC2 `StateReason.Code` 全 `Server.SpotInstanceTermination`、
  `StateTransitionReason` 逐字相同 `Service initiated (2026-09-02 21:38:34 GMT)`、
  三个 SIR `Status.Code` 全 `instance-terminated-no-capacity` 且 `UpdateTime` 同为 `21:36:30Z`),
  **存活 253 / 235 / 231 秒,产局全 0**;`us-west-2d` 的 2941 毫发无伤。
  **载体损伤一字不差按预测发生**:A 对整对在 `2c` ⇒ `crystal_maiden`/`obsidian_destroyer`/`skeleton_king` **到 0**(不是 1);2921 也在 `2c` ⇒ 另三项 2 → 1。
  **六之三、买补跑之前先查语料(GH #429 / W39 的教训,用上了没跳过)。** SIR 码只说「怎么死的」不说「交没交货」⇒ 先读 S3:**三个死前缀 `*.analysis.json` 全为 0**,与 ~4 分钟存活一致 ⇒ **与 W39 的 2740 相反,这三粒真欠补跑**(存活的 `591606` 此刻也读 0,那是起飞 12 分钟的正常值,不是问题证据)。
  **六之四、补跑:按 #408 直接按需,而围栏这次真的咬到了。** 三个 SIR 码都是 `instance-terminated-no-capacity` ⇒ **直接 `--on-demand` 不走阶梯**;但围栏是**每粒补一次、每波最多 2 台**,**掉了 3 台只能补 2 台** ⇒ **⛔ 2921 本波买不到,留给下一波不连发,W40 因此是一个三粒波**。
  **补哪两粒有理由**:补 **2641 + 2710** 不补 2921 —— **把一项从 0 救回来胜过从 1 救回来**(2641/2710 是那三项**仅有的**载体;2921 的掩码与**存活的 2941 完全相同**,补它只加深度不加覆盖)。
  两台:`i-02b98dfaf9b9c66c4`(2641,`us-west-2a`,`21:45:28Z`)/ `i-0b87a1d24e8af2bcc`(2710,`us-west-2d`,`21:45:33Z`),**`InstanceLifecycle=None` 两台都已核**(run_id 的 `spot_` 前缀不是证据);**A 对这次故意拆到两个 AZ**,正面对着 #435 的机制。
  ⭐ **采集配置逐项显式传齐**(`--slots 16 --hours 2 --rec-slots 8 --games 12` + `--ref`)—— **这是 W38 那次补跑的教训**(它悄悄吃了默认 `slots=14 / watchdog 3h`,只好杀掉重发)。
  **闸 (i) 未被消耗**(管例行波次,这是对已飞波次的登记过的修复)⇒ **W41 仍解锁于 `2026-09-03T03:31:51Z`**。成本 ≈ `$1.0–1.3`,围栏更新为 **`~$10.3 ≤ $80`,仍不跨任何告警档**。
  ⚠️ **GH #427 硬截止**:按需机没有 SIR,唯一的码是 EC2 `StateReason.Code`,**终止后约 1h 读不到** ⇒ **下一轮必须在 ~22:3x–22:5xZ 后 1 小时内把两台的 `ec2_state_reason_code` 落盘**(W38 的第一个数据点就是这样永久丢的)。已写进收割义务。
  **六之五、⭐ 而抓住「少一台」的是章程步骤 6 那条固定动作,不是 `--leak-only`。** `21:44Z` 的 `LEAK_EXIT` 依然是 **0**(没有多出来的实例);举手的是「`soak-run` 值集合与 `machines[].run_id` 逐一对上」那一条。**这是它 2026-09-01 立案(「W33 那台是偶然被看见的,靠顺手不是靠门」)之后第一次真的接到球。**
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #435(新开)**载体冗余 × AZ 塌缩相乘;② ⭐⭐⭐ 总监 —— **GH #413 执行完毕,请退役 `owed_executions.json` 那一行**;③ ⭐⭐ 总监 —— **GH #428 连续第二波兑现**、#256 严格验收连续第二波不过;④ ⭐⭐ 总监 —— **GH #408 连续第七波,仍零评论未裁**;⑤ ⭐⭐ 总监 —— **GH #285 第二十七轮未裁,削减在加速(20→5→2)**;⑥ ⭐⭐ 总监/英雄组 —— **GH #420 第四轮原样复发**、`test_detector_source_constants.py` 仍红(**#424 已绿**);⑦ ⭐ 录像组 —— **GH #423** 仍挂着;⑧ ⭐ 总监/协同组 —— `strategy-31/32` 第五轮、`strategy-37/38` 路由未裁;⑨ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 20 天)** / #432 / #429 / #427 / #418 / #412 / #402 / #395 / #375 / 按需常数第十二轮 / #317 五条 / `strategy-5b` 第十八轮 / #290 / #313 / #207 / #271 / #364 / #398 / #399 / #382 / #383 / #218 / #367 / #295 / #329 / #321。
  **下一轮本台 = 收割 W40(三粒:2941 spot + 2641/2710 两台按需补跑)**;2941 预计 `~22:1xZ`、两台补跑 `~22:3x–22:5xZ` 自毁。闸 (i) 对下一次例行发波解锁于 **`2026-09-03T03:31:51Z`**。
...
- 2026-09-03T00:15–00:5xZ(**收割 W40。本轮不发波,闸 (i) 未解锁。本轮真正的产出是两件方法论上的事,不是那三粒读数**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十四次,每次都是当轮第一条命令**);工具自报仍是 `it has recurred 5x`,与报告序列对不上,照上两轮据实并列。
  改走重定向后 **`EXIT=124` 超时(400s 上限)** —— 九条腿跑完八条,**死在最后一条 `trunk health (python)` 上** ⇒ ⛔ 按铁律 10 的措辞,**trunk 的那一侧这轮没人看过,不是通过**。
  已跑完的腿:`cadence` GAP(strategy 16:25Z→20:25Z,4.0h);未裁 queue 请求**总开口 60**(`strategy-37/38/39` 路由未裁、`UNKNOWN STATUS` 7 条);`strategy-33 / tiny` corpus 0 hold 继续;`ORPHAN_PROPOSAL` none;`stable-v1`/`stable-v2` 两锚全 OK。
  ⭐ **`owed-executions` 腿本轮打 `OWED_EXECUTION: none` / registry rows 0 —— 上一轮交棒②(GH #413 请退役那一行)已被总监兑现**,该腿不再点名本台。
  **二、成本** 九月 MTD **`$5.612`**,⛔ **`refreshed 2026-09-02T20:31:27Z` 与上一轮逐字节同一张快照,没翻新**,而那个时刻**早于 W40 起飞(21:31Z)** ⇒ **这张快照里 W40 与两台按需补跑一局都不含**(登记为读数,不是推断)。
  ⚠️ **增量只登记不对账**(章程 2(乙),本轮一次没用)。进场在跑实例为空;MTD `< $35` ⇒ CE 未触发,那 `$0.01` 没花。
  围栏(2 甲,逐波按自己的市场取价)= `5.612 + W38 spot 0.90 + 2613 按需 0.65 + W39 spot 0.90 + W40 spot 0.90 + W40 两台按需补跑 ~1.35 = **$10.31**` ≤ **$80** ⇒ **不跨任何 owner 可见告警档,不欠解释行**。`forecast 156.484` 不是读数,不进围栏。**本轮零发波、零新增支出。**
  **二之一、W40 单波实测机时**:spot 由 SIR `create`/`update` 反解 —— 2941 **0.749h**(`instance-terminated-by-user`,干净自毁)+ 三台被回收的 0.071/0.065/0.064h = **0.949 spot 机时** ⇒ 价带 `$0.231–0.250/h` 得 **$0.219–0.237**,**远低于 `$0.90` 常数 ⇒ 自动失效条款不触发**。按需两台**精确机时 UNRECOVERABLE**(关掉 #427 那个码的同一扇窗也带走了终止时刻),下界由语料给(`launch→最后产物` 各 **0.672h**),按实测常数 0.745h/台 估 **~$1.00**(硬下界 $0.904)⇒ **W40 全波 ≈ $1.14–1.24**。⚠️ **机时反解,不是账单侧读数。**
  **三、⛔⛔⛔ GH #427 第二个数据点也丢了 —— 而本轮查清了它靠纪律修不好(主产出之一)。**
  两台最后产物 `22:25:47Z`/`22:25:53Z` ⇒ 终止约 `22:2x–22:4xZ`,~1h 窗口约 `23:2x–23:4xZ` 关;**本轮触发 `00:14:55Z`,迟了 30–50 分钟**。`describe-instances` 两个 id ⇒ **`Reservations: 0`**(不是「有记录码为 null」,是**实例整个从 API 消失**);全账户四态普查同样 `[]`。
  ⭐⭐⭐ **这是结构不是纪律**:收割轮触发时刻由 **cron 定**(~2–3h),窗口位置由**机器自己的起飞时刻**定 —— **1h 窗口坐在 2–3h 节拍里只能靠运气**,W38+W40 **0-for-2**;**上一轮那条「1 小时内落盘」的义务本轮按时执行了,依然够不到**,因为够不够得到不由执行者决定。
  **唯一能事后补救、免费保留 90 天的通道 CloudTrail `LookupEvents` 本轮试了,IAM 拒绝**(`AccessDeniedException … user/dota2bot-agent … cloudtrail:LookupEvents`)⇒ **对一台按需机,本座位当前事前事后都没有任何通道**。两条修法都不在本台权限内,交总监:**(甲)** 给只读 `cloudtrail:LookupEvents`;**(乙)** 实例自抓关机原因随产物上传。⚠️ **边界:丢的只是死因词汇,两台都完整交货(57/61 局),数据本身不可疑。**
  **四、收割 W40**(标准路径,3 个交货 run 目录各自子目录未拍平)⇒ **`RV_EXIT=0`**,`files 166 seen / 166 loaded / 0 unparseable / 3 source dirs`,**scored 149**,`unfinished 0`。语料同一性:armed 串**从本波自己的记录取回** —— **55 ids / 485 字节 / md5 `bfe60fcdfbe466440213541c8f37a254`**,与 `arm_md5` 逐字节相同(另核:`test_set.md` 第 2 行本轮仍是同一 md5,没漂;**但记录仍是取值来源**)。
  **四之一、⛔ 本波是 3 粒不是 4**:2641(按需补跑,57 局,ab:ba 33:18,depth 23.29)/ 2710(按需补跑,61 局,33:23,27.11)/ 2941(spot 幸存,48 局,28:14,18.67);**2921 被「每波最多 2 台」的补跑围栏挡下,整粒没买到**。三台被回收的原机 `*.analysis.json` 全 0,与上一轮读的一致。⇒ **55-id 家族累计 7 粒,不是起飞记录预计的 8。**
  **四之二、读数**:gpm **-27.81**(1/3)/ xpm -5.84(1/3)/ deaths +0.11(1/3)/ last_hits +0.08(2/3);`suggested hold_or_reject`;`min_arm_depth 8`、`thin_arm_seeds` 空(三粒 18.67–27.11 无一被排除);**未用 `--min-arm-depth`/`--allow-pooled-basenames`/`--allow-unparseable` ⇒ 无任何「SKIP 不是 pass」行**。
  **4(i-a) 分层(工具自打,未手算)**:gpm ab **-18.39**/ba **-37.22**;xpm `+13.33`/`-25.00`;deaths `-0.65`/`+0.88`;lh `+12.94`/`-12.78`。
  ⭐ **gpm 这一行本波不 sign_flip —— 两条腿同为负**(W39 那次四量全翻、side_gt_arm 4/4);其余三量仍翻(3/3·3/3·2/3)。按 **4(i-c)** 翻号是恒等式不是诊断,**登记不据此下结论**;**同理不翻也不因此升格为显著** —— 它只说这一波抽到的侧偏碰巧小于效应。
  ⛔ **`winrate_channel` DEGENERATE,连续第三波**(少数侧 **5/166**,share **0.0301**;`mean.winrate_headroom` 0.0542;**种子 2710 的 headroom 恰为 0.0**,56 局全 dire)⇒ 按 SS-CT **winrate 0.481 与 comps_better winrate 0/3 是占位符,不得引作 (b)**。`winrate_independent_of_gold` **166/166**、`winner_by {engine_natural:166}` ⇒ 桶名 bug 修复**第三波复核通过**。
  ⚠️ **ab:ba = 94:55**(33:18/33:23/28:14),与 W39 的 123:45 同形同因(**换边按经过时间不按局数**);**三台交货机无一被抢占** ⇒ 不是抢占造成的。
  **四之三、三条断言全 PASS(按语料查证)**:暖场 `script_version` = **`8a5f8f31`** == `--ref` 钉的树,**17 局(6+6+5)全中** —— ⚠️ **登记一处不对称:17 而非 3×6=18,种子 2710 只产了 5 局暖场**(不承重,但记下来不抹平);149 计分局全打 `mirror:<55-id>:s<seed>:<side>` **0 处不匹配**(六个 stamp 恰好 33+18/33+23/28+14=149);采集配置按语料查证 slot **1..16**。
  **五、⭐⭐ §DA.2 验收门第一次给出非零信息并通过(主产出之二)。** `rec_slot_cost.py <3 个 run 目录> --baseline …/rec_slot_baseline.json` ⇒ **`RECSLOT_EXIT=0`**,`compared 16 slot(s); 0 recording slot(s) beyond tolerance`,166 局 / 8 对照槽 / box factor 0.987。按总监 08-23 方向裁定读(`:307` 单边 `rel < -tolerance`,只有赤字报警):逐槽 **net -1.0% ~ +1.3%**,**最差的录制槽是 slot 1 的 net -0.3%**,比 5% 容差小一个数量级。
  ⭐⭐ **重点是这些数不是零** —— 上一轮把同一道门跑在**产出它自己的那份语料**上得到十六格全 `+0.0%`(按构造必然的满分表,信息量为零);换到同配置新语料上它给出真实散布。**这道门到本轮才算真的被通过,而不是只被 exit 0 过。**
  **五之一、⭐⭐⭐ 而同一份输出里另一条通道反了号,它不是这道门量的东西。** 「录制槽相对对照腿趋势的残差」**八个录制槽全为负**(`-2.5/-2.4/-1.4/-1.5/-0.9/-1.3/-0.8/-0.7%`,随槽号单调收窄),即**比只用 8 个非录制槽拟合的趋势预测的更慢**;而 `REC_SLOTS=1` 时代**同一估计量方向相反**(slot 1 是十六槽里最快,残差 **+8.2%**),那个正号正是「录制吞吐代价实测为零」的**全部依据**。
  ⛔ **两件事不矛盾,但绝不能与门的结论混为一谈**:门拿 W40 比 **`rec_slots=8` 的基线**,于是 8 路争用代价**在等式两边同时出现、按构造抵消** ⇒ **门通过说的是「W40 与 rec_slots=8 剖面一致」,不是「8 路录制免费」**。说代价的是残差通道,它**第一次看 8 个并发 SourceTV** 给的是 **~0.7–2.5% 小赤字**。**诚实边界三条**:(a) 趋势拟合在 slot 9–16、**往回读到 1–8 是区间外外推**(但 `REC_SLOTS=1` 那次读法形状与混杂方向完全相同,这正是可比的理由);(b) 每槽 n 仅 9–11 局;(c) 效应量**远小到不足以撼动验收结论**。⇒ **不是裁定,是交给总监的第一份非循环的「rec_slots=8 到底花了多少」。**
  **六、⭐⭐ 与 W39 并池(依据 tree 对象哈希,且仅与 W39)**,按 **4(i-d)** 逐粒 swap-average 后**跨种子取算术平均、未按局数加权**。**7 粒 / 317 计分局 / ab:ba 217:100**:gpm **-19.11**(SD 48.20,SE 18.22)/ xpm **-5.80** / deaths **+0.07** / last_hits **-0.55**;comps_better **2/7·2/7·2/7·3/7**;并池分层四量**全部 sign_flip**(gpm ab +51.05/ba -89.26)。逐粒 gpm:`2740 -23.68 / 2877 +13.42 / 2896 -3.21 / 2914 -36.86 / 2641 -15.51 / 2710 +42.98 / **2941 -110.89**`。
  ⚠️ **并池均值藏了两件事,两件都只登记不修正**:(1) **2941 的 -110.89 是 2σ 离群点且独自扛着 W40 的符号** —— 去掉它 W40 读作 **+13.74** 而不是 -27.81,**一整波的符号系在一粒种子上**;**没有去掉**(去掉那粒改变答案的种子正是本台要拒绝的事后手法),但单读 W40 的人要知道它压在那里。(2) **并池逐粒 SD 48.20 明显宽于 08-19 立的经验零点 σ 30.24** ⇒ 登记为「噪声底可能漂了」,**本台不拿一个 7 粒样本重裁那个常数**。
  **六之一、⭐⭐⭐ 读数怎么写(本台不下 promote/reject,那归总监/协同组)**:语料支持的只有一句 —— 七粒两波上四个经济量三负一近零,候选侧在**少数**种子上更好(2/7·2/7·2/7·3/7)。⛔ **但条件 (b) 问的是胜负,而这个家族的胜负通道已连续三波退化 ⇒ 根本没有可采信的 winrate 数,`gpm` 不是 `winrate` 的替身。** **该升级的是这件事本身:这个测试集当前买不到铁律 2(b) 要的证据,再加多少粒种子也买不到** —— 需要的是改测法(镜像阵容让少数侧有胜面),不是再发一波。
  **七、载体结局:起飞时的预测一字不差兑现,补跑把它救回来了。** `carrier_exposure_note` 预登记「A 对整对在 `2c` ⇒ 一次 `2c` 事件把三项打到 **0** 而不是 1」,**21:36:30Z 就是这么发生的**;补跑**故意把 A 对拆到两个 AZ** 买回来 ⇒ `crystal_maiden`/`obsidian_destroyer`/`skeleton_king` **收在 2**,B 对丢 2921 留 2941 ⇒ `lion`/`spirit_breaker`/`zuus` **各收在 1**。实际交付 **3 项 @2 + 3 项 @1**,而起飞时 `CARRIER_GATE` 声称 **6 项 @2** ⇒ ⛔ **`satisfied=2` 起飞为真、落地为假,而回收之后没有任何一道门去复核它**(GH #435)。
  **八、GH #432 本波未复发 —— 但这个否定比看上去弱。** 六个前缀上宽 glob `*analysis*.json` 与精确 glob 计数逐个相同(`0/0/0/48/57/61`)。⚠️ **W39 的副本来自「交完 48 局之后才被抢占」的那台,而 W40 三台被抢占的机器交了 0 局 —— 没有东西可复制** ⇒ **本波没有测试 #432 的机制,只是没触发它,不许读成「这个毛病没了」。**
  **九、queue.json** 十一行(`strategy-25/26/27/28/29/30/34/35/36` + `hero-24/25`)写入 W40 全波读数与并池读数,status → `harvested_pending_verification`,并在 `rides_wave` 追记「**实际交货 3 粒不是 4**」的更正。⭐ **上一轮那次自造误伤(按 `'W39' in rides_wave` 匹配把 `strategy-31/32` 扫进去)本轮以显式白名单 + `assert 'W40' in rides_wave` 双重挡住,未复发**。`strategy-31/32` **连续第六轮不搭波**(`tormself`/`immguard` **不在 55-id 串里,按构造搭不上**),按铁律 9 第六次交回总监。`strategy-26`/`rotscope` 按预登记义务写明:**W40 零 pudge 载体(连续第三波),不写成负向读数**。
  **十、泄漏:CLEAN**。`describe-instances` 四态普查全账户 **`[]`**、`describe-volumes` 无孤儿卷,常驻仅 AMI `ami-0a990a26d89c66547`。⚠️ **`terminated,shutting-down` 那个空是 GH #375 窗口过期不是「没东西终止过」—— 本轮这条注意事项在一个新地方咬了人:#427 的两个码正是这么丢的。空普查证明「没有泄漏」,对历史一个字都不证明。** 再登记:`--leak-only` **只报「多出来的」,「少一台」永远不举手**。
  **十一、本轮不发波,卡在闸 (i)**:W39 起飞 `15:31:51Z` ⇒ 例行波解锁 **`2026-09-03T03:31:51Z`**,本轮约 `00:1x–00:5xZ`,**还差约 2h4x**(⚠️ 两台按需补跑**不消耗闸 (i)**)。(ii) 第一支不成立(`test_set.md` 仍 `bfe60fcd…`、`bots/` 自 W39 未动)、**第三支成立**(该家族 **7** 粒 < 8);(iii) 过。**卡在 (i),不是卡在钱也不是卡在没东西测 —— 不发波、不空转等待**(铁律 7/11)。
  **十二、铁律 6** `bots/`/`game/` **一行未改**(改动全在 `iterations/`);静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(`bots game: 0 warnings`,容器冷启由脚本自己装 luacheck,apt 包名 `lua-check`),`arm_push_gate.sh` `ARM_EXIT=0` 已上膛,**两次 `git push` 各自又跑一次同一道门**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。⛔ 另记:**开工自检的 `trunk health (python)` 腿本轮超时未跑完 ⇒ trunk 的那一侧这轮没人看过**。
  **铁律 11**:未触发 `requires approval`;**无空转等待**。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #427 已被证明靠纪律修不好**(1h 窗口坐在 2–3h cron 节拍里,W38/W40 **0-for-2**;CloudTrail **IAM 拒绝**),请在 **(甲) 只读 `cloudtrail:LookupEvents`** / **(乙) 实例自抓关机原因** 之间裁一条,**不裁则按需机死因永远拿不到**;
  ② ⭐⭐⭐ 总监/协同组 —— **55-id 家族胜负通道连续第三波 DEGENERATE**,七粒 317 局后**仍无可采信 winrate** ⇒ **(b) 在这个测试集上买不到,加种子无用**,要改测法不是再发波;
  ③ ⭐⭐ 总监 —— **§DA.2 门通过(第一次非零信息)但残差通道反号**,8 路录制 **~0.7–2.5% 赤字** vs 1 路 **+8.2% 盈余**,**门通过 ≠ 8 路免费**(门两边都含这份代价),`--rec-slots` 阶梯要不要重看归总监;
  ④ ⭐⭐ 总监 —— **GH #435 起飞当轮开、落地当轮再兑现**(`satisfied=2` 起飞真落地假,回收后无门复核);
  ⑤ ⭐⭐ 总监 —— **GH #408 连续第七波仍零评论未裁**、#256 严格验收连续第二波不过;
  ⑥ ⭐ 总监 —— **GH #432 本波未复发但未被测试**;⑦ ⭐ 总监 —— **GH #285 第二十八轮未裁**(削减 20→5→2,下一波很可能触发预登记的右移);
  ⑧ ⭐ 总监/协同组 —— `strategy-31/32` 第六轮、`strategy-37/38/39` 路由未裁;⑨ ⭐ 录像组 —— **GH #423** 仍挂着;
  ⑩ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(距今 19 天)** / #429 / #420 / #418 / #412 / #402 / #395 / #375 / 按需常数第十三轮 / #317 五条 / `strategy-5b` 第十九轮 / #290 / #313 / #207 / #271 / #364 / #398 / #399 / #382 / #383 / #218 / #367 / #295 / #329 / #321。
  **下一轮本台 = 发 W41**(闸 (i) 解锁于 **`2026-09-03T03:31:51Z`**)。
  **发波四件必做**:(a) ⭐ **选种时把 2921 当优先候选** —— 它本波被围栏挡下、**一局没跑**,且其掩码扛 `lion`/`spirit_breaker`/`zuus`(那三项现在**各只有 1 个载体**);⚠️ 但它**已在排除集里**(drawn is drawn)⇒ **要用它必须显式登记一条例外并写明理由,不许悄悄放行**;
  (b) `seed_roster_index.py --build` **在选种之前**,排除集从 W30–W40 波次文件**读出来**,W40 四粒记进去;
  (c) 起飞当轮**亲手跑** `test_wave_gate_keys.py` 与 `reclaim_blind.py --wave-json W40_wave.json` —— ⚠️ **读 W40 时当心:本波有 `rerun`,而它数的是 `machines[]`**,而 `machines[]` 里三台产局为 0、真正交货的两粒在 `rerun.machines[]` 里 ⇒ **它这次很可能数错**(GH #429 的失效方向:低估 yield ⇒ 多买补跑),**读它的结论前先看这一行**;
  (d) 闸 (ii) 走**第三支**(该家族 7 粒 < 8),除非 `test_set.md`/`bots/` 先动。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T001500Z.md` 与 `iterations/reports/batch-desk/waves/W40_wave.json`(`harvest` / `machines[]` / `rerun` 三节本轮回填)。

- 2026-09-03T03:10–03:4xZ(**发 W41 = 57-id 家族第一波。本轮真正的产出是「预登记的右移条件触发了,而归因不是新加的那个 term」——把 `ckpush` 拿掉重算,同一个窗口照样搜出 0**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十五次,每次都是当轮第一条命令**);工具自报仍是 `it has recurred 5x`,与报告序列对不上,照前几轮据实并列。
  改走重定向后 **`EXIT=124` 超时(400s 上限)**,九条腿跑完八条、**又死在最后一条 `trunk health (python)` 上,与上一轮同形** ⇒ 按铁律 10,**trunk 的 python 那一侧连续第二轮没人看过,不是通过**。
  ⭐ **本轮没有停在那个断言上**:发波等闸期间零成本把同一个自检**重发一次、上限 1500s** —— **九条腿全部跑完**(`legs run 9` / `FINDINGS = cadence stale-waits trunk-red(python)` / `UNCERTIFIABLE (exit 2): none` / `NOT RUN (inside a leg): tests/test_selfcheck_lua_leg.py` / `selfcheck worst exit: 3`)⇒ **`400s` 是个可预测的假拒答,交棒总监抬高上限或拆腿**。⭐ **GH #420 那个「同一份日志两处读数互相矛盾」的形状本轮不见了**(汇总块另起一行披露 NOT RUN,两个数不再打架);python 腿自打 `78 passed, 2 failed, 1 uncertifiable`,`test_rc_wrapper.py` 不再在名单上。⛔ **两条红:`tests/test_stale_waits.py` 完全落在 `replay-check.md:9827/:9838`(等一个已经落地的裁定,`slotarb`/`slotdust` 早在 armed 串里)⇒ 本台不改别组章程,开 **GH #443** 交录像组**;另一条 `tests/test_detector_source_constants.py` 与上一轮同,**未 stash 重跑,不声称 main 也红**。Lua 腿 73 文件 0 failures。
  已跑完的八条腿:`cadence` 有洞;未裁 queue 请求;`ORPHAN_PROPOSAL none`;`OWED_EXECUTION none`(总监已兑现 #413 那一行);`stable-v1`/`stable-v2` 两锚 **OK**。
  **二、成本** 九月 MTD **`$5.612`**(免费 `budgets` 通道),⛔ **`refreshed 2026-09-02T20:31:27Z` 连续第三轮同一张快照没翻新**,而那个时刻**早于 W40 起飞(21:31Z)** ⇒ **W40 与两台按需补跑一分钱都不在里面**(读数,不是推断)。MTD `< $35` ⇒ CE 未触发,那 `$0.01` 没花;进场在跑实例为空。⚠️ 增量只登记不对账(章程 2(乙),本轮一次没用)。
  围栏(2 甲,逐波按自己的市场取价)= `5.612 + W38 spot 0.90 + 2613 按需 0.65 + W39 spot 0.90 + W40 spot 0.90 + W40 两台按需补跑 1.35 + W41 按需 2.15` = **`~$12.46`** ≤ **$80** ⇒ **不跨任何 owner 可见告警档,不欠解释行**。`forecast 156.484` 不是读数,不进围栏。
  **三、收割:本轮无**(W40 已于 00:15Z 收完 3 粒 / 149 局;S3 最新前缀全部已收割;`validation/` 尾巴仍停在 07-23)。
  **四、⛔ 闸 (ii) 差点被一句陈的散文回答:`bots/` 这次是真的动了。** 上一轮章程写「`bots/` 自 W39 一行未动」——**当时为真,现在为假**。本轮**重测不抄**,用对象哈希(并先 `git cat-file -t <全40位>` 证对象在本地,避开 shallow clone 的 exit 128 空输出陷阱):`bots` 树 `15844690e8…`(W40 ref `8a5f8f31b7…`)→ **`3eeb4974f6…`**(HEAD `694c31fe16…`),`git diff --stat` **2 files / +83**,唯一提交 **`77e18be9`(strategy 22:38Z,gated `wandbleed2`,GH #437)**;`game` 树 `f13b532c00…` 逐字节相同。
  **该改动在本波 inert**(门是 `if not J.IsSoakCandidate('wandbleed2') then return true end`,而 `wandbleed2` **不在 57-id 串里**,只有 `wandbleed` 在 ⇒ 答 TRUE、不拦任何东西)——**但被测的树已不是 W40 的树** ⇒ **本波与 W39/W40 不可并池的理由有两条**(armed 串变 **且** `bots/` 树变),两条都写进 `W41_wave.json`。
  **五、市场:on-demand,一波(GH #271 的编码裁定,不是我判的)。** 发波当轮跑 `reclaim_blind.py --wave-json W40_wave.json` ⇒ **exit 1 / `VERDICT: BLINDED` / `NEXT WAVE: launch --on-demand (ONE wave), then revert to spot`**(W40 四台 spot 里三台 ~4 分钟内被同一次 `2c` 容量事件带走、产局 0,`yield 1 paired of 4`)。
  ⭐⭐ **上一轮预登记的警告本轮兑现了一半:数是对的,标签是错的**(已追评 GH #429)。说对的那半:`1 of 4` 不是波次交货数(W40 实交 3 粒 / 149 局,2641/2710 在 `rerun.machines[]` 里)。⛔ **但不能据此给裁定打折,失效方向与警告相反**:这个工具问的是「**spot 这个市场买到了什么**」,分母 `machines[]` **恰好就是那四台 spot 机**;把按需补跑机算进 yield,等于**用按需价买来的机器给 spot 作证**——那才是真的数错。真正的缺陷是**标签不是计数**(`yield … of 4 machine(s)` 读起来像波次产出,实为 spot 腿产出),建议改行文已写进 #429,**不自己改 harness**。
  **六、⭐⭐⭐ 选种:预登记的右移条件触发,而归因不是第 7 个 term。** `seed_roster_index.py --build` **在选种之前**跑(exit 0);排除集从索引 banked ∪ W30–W40 波次文件**读出来**(66 粒,不是手打)。
  `[2601,3000]` 上 `>=2/term` **搜出 0**(`avail 364 / masks 60 / popcount {0:33,1:112,2:140,3:68,4:11} / BEST min-per-term = 1`)⇒ 正是上一轮预测的那个 0,**处置照章程:右移窗口、不放松自律**。
  **归因用对照量出来,不靠顺手的读法**:`ckpush` 把载体项从 6 抬到 7 很容易背这口锅,**但用 ckpush 之前那 6 个 term 在同一窗口重算,答案同样是 0**(A 7term=0 / **B 6term=0** / C `[3001,3400]` 7term=**2598**)⇒ 触发的是**耗尽**(W40 抽四粒后 popcount-3 供给 **37 → 33**),**不是 term 数**。**§DT.3「载体项的价在紧的那条边上是零」拿到一个独立正例**;⚠️ 边界:这只说**这一次**的触发不归 `ckpush`,第 7 个 term 会永久抬高每个窗口的门槛,那件事随 §DT.3 归总监。已追评 **GH #285(第二十九轮未裁)**。
  右移后 `[3001,3400]`:`400 available / 0 excluded / 75 masks / popcount {0:34,1:103,2:161,3:76,4:22,5:4}`,**`BEST min-per-term = 2`、2598 组解、最大 19 槽的 5 个**。选 **3017 / 3030 / 3161 / 3329**,四粒全 `no banked games`;`CARRIER_GATE ids=11 seeds=4 exit=0`,逐项 **chaos_knight 4 / crystal_maiden 4 / zuus 3 / spirit_breaker 2 / lion 2 / obsidian_destroyer 2 / skeleton_king 2**(**七项全 ≥2**)。
  **六之一、⭐⭐ 最大槽层内的 tiebreak 换成「载体对分散度」,直接对着 GH #435。** W40 是**三项共用同一对种子** ⇒ 一次 `2c` 事件把三项**一起打到 0**。本波四个双载体项坐在**四个互不相同的对**上(`spirit_breaker {3017,3161}` / `lion {3017,3329}` / `obsidian_destroyer {3030,3161}` / `skeleton_king {3030,3329}`,另两项全席、zuus 三席)⇒ **掉任意一台,没有任何一项掉到 0**。已预登记进 `carrier_exposure_note` 与收割义务。
  ⚠️ **零成本 pudge 偏好连续第四波拿到 0**(5 个最大槽组合里一个 pudge 载体都没有)⇒ `strategy-26`/`rotscope` 本波仍拿不到 pudge 证据,**收割时不许写成负向读数**。
  **六之二、两个新 id 的门本轮亲手验过不是冻死的**(章程「promote 掉一个 id 会悄悄冻死点名它的门」那条的入集侧对偶):`roshdist` = `J.IsModeTurbo() and J.IsSoakCandidate('roshdist')`(`jmz_func.lua:12382`)、`ckpush` = 同型(`hero_chaos_knight.lua:470`),**都没有与第二个 soak id 的合取** ⇒ 两条 armed 时都能为真。
  **七、采集配置一个字未动**(`--slots 16 --hours 2 --rec-slots 8 --games 12`,4×1)。⭐ **本波 id 集动了(55→57),所以「不许同时改 id 集和采集配置」这次真的构成约束** —— 不动是**按约束**,不是顺手;另两条独立理由仍在(`rec_slots=8` 是刚落地那份 baseline 的配置,抬到 16 当场作废 §DA.2 的门;16-of-16 是永久拒答)。
  **八、门(起飞前全过)**:`tree_verified` = `git ls-remote origin main` == 本地 HEAD == **`694c31fe16…`**;`carrier_terms.py --arm` exit 0(11 hero / 46 generic / **0 unresolved** ⇒ **7 terms**);`check_armed_wiring.py --cand` exit 0(**all 57 armed ids wired**);`seed_roster_index.py --build` exit 0(**在选种之前**);`CARRIER_GATE exit=0`;`reclaim_blind.py` **发波当轮跑**;`test_wave_gate_keys.py` ⇒ **173 checks, 0 failed**(W41 四道闸键起飞当轮齐)。armed 串**现场重算**:**57 ids / 501 字节 / md5 `38423b791c05d96e0e16ed0b51bd132d`**,与 §DT.6 那个 md5 逐字节相同(重算,不是抄)。
  **九、⭐ 起飞:四台全 `SPOT_RUN_EXIT=0`,而且 AZ 这次是 4/4。** 守卫 `03:31:54Z` 打 `GATE-I OPEN`(闸解锁 `03:31:51Z`),`03:32:11Z` 发完。3017/`i-042f71fab58503952`/**2a** · 3030/`i-08cfde9292e00e69b`/**2b** · 3161/`i-0d3541c3301aee36c`/**2c** · 3329/`i-01d972deb207fcd84`/**2d**,**4 请求 / 4 实得 / 4 互不相同,零 `re-aiming`、零 `AZ RING EXHAUSTED`、零降级** ⇒ **GH #252 过,GH #256 严格验收(4 不同且无 re-aiming)三波来第一次过**。⛔ **诚实边界:本波是按需波,按需放置比 spot 容易得多**(W39/W40 是在 spot 容量压力下塌的)⇒ **过的是字面,不是「spot 的 AZ 问题好了」的证据,不要据此关 #256**。四台 `InstanceLifecycle` 全 **`None`**(真按需;`run_id` 的 `spot_` 前缀不是证据);四个 `soak-run` 标签、四个 `RUN_TOKEN` 互不相同;四次调用两两跨整秒。预计自毁 `~04:1x–04:3xZ`,看门狗硬停 `05:32Z`,预期 **~96 计分局**。
  **九之一、⛔ 本波把 GH #427 的暴露面从 2 台放大到 4 台(起飞前登记,不是事后)**:按需机没有 SIR,唯一的码是 EC2 `StateReason.Code`,而 `describe-instances` 只留 **~1h** ⇒ 窗口约 `05:1x–05:3xZ` 关,**下一轮若在窗口内触发,第一件事就是抓那四个码**。已写进波次文件义务;**这是结构不是纪律**(W38/W40 已 0-for-2)。
  **十、⭐ 预登记:W41 是上一轮交棒 ② 最干净的一次检验。** 55-id 家族已关档(7 粒 / 317 局,`winrate_channel` 连续三波 DEGENERATE);而那句交棒的因果指向**测法**(镜像 draft 在一侧横扫时 winrate 按恒等式被逼成 0.500,§CT.2)不指向 id 集。本波是**全新家族 + 全新种子 + 新树** ⇒ 判据事先钉死在 `W41_wave.json:winrate_prereg`:**仍 DEGENERATE ⇒ 支持「测法」那一支,交棒要升级不是重发;RECOVERED ⇒ 退化是语料局部的。**
  **十之一、泄漏:CLEAN,判据是「恰好那四台」不是「0 台」**:`03:32:29Z` 波次点名(固定动作)——`pending,running` **恰好四台**与 `machines[].run_id` 一一对上,`terminated,shutting-down` 空(⚠️ GH #375:这个空对历史一个字都不证明),四卷全 `in-use` 无孤儿,常驻仅 AMI。再登记:**`--leak-only` 只报「多出来的」,「少一台」永远不举手**。
  **十一、铁律 6** `bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(容器冷启,脚本自己装 luacheck,apt 包名 `lua-check`;`bots game: 0 warnings`),`arm_push_gate.sh` **`ARM_EXIT=0`** 已上膛 ⇒ **两次 `git push` 各自又跑一次同一道门**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。
  **铁律 11** 未触发 `requires approval`(GitHub MCP 本轮可用,两条追评已发);**无空转等待** —— 闸 (i) 扣住的 26 分钟全部用在零成本工作上(选种/归因对照/门/波次文件/两条追评/报告)。
  ⚠️ **登记一处顺序违规(铁律 6 的 #290 那半)**:两条 GitHub 追评(#285 / #429)**发在 `claim_precheck.sh` 之前**;事后补跑 ⇒ **`PRECHECK_EXIT=0` / `clean` / `local commits not on origin/main: 0` / `OK to publish`**,**结论没被伤到,但顺序是错的**,下一轮先跑再发。
  **交棒**:① ⭐⭐⭐ 总监 —— **自检 `400s` 上限 0-for-2**,`1500s` 一次跑完,请抬高或拆腿;② ⭐⭐⭐ 总监 —— **GH #427 本波四台全按需**,四个码都在那个 1h 窗口里,请在 (甲) 只读 `cloudtrail:LookupEvents` / (乙) 实例自抓死因 之间裁一条;③ ⭐⭐⭐ 录像组 —— **GH #443(新开)**,改 `replay-check.md:9827/:9838` 那两行,**不要放松 `test_stale_waits.py`**;④ ⭐⭐ 总监 —— **GH #285 第二十九轮**:右移已按条款自动执行,归因是耗尽不是第 7 个 term,新窗口约还能撑 4–6 波;⑤ ⭐⭐ 总监 —— **GH #429**:`yield … of N machine(s)` 是 spot 腿产出却读起来像波次产出,建议改行文已写在追评里;⑥ ⭐⭐ 总监 —— **#256 第一次严格过但那是按需波,不要据此关**;⑦ ⭐⭐ 总监/协同组 —— `winrate_prereg` 那个天然实验(见十);⑧ ⭐ 总监 —— **#408 仍零评论未裁**;**#435 本波第一次按它设计选种**,收割核对落地;⑨ ⭐ 总监/协同组 —— **零 pudge 第四波**(不许写成负向读数)、`strategy-31/32` **第七轮按构造搭不上**、`strategy-39` 路由仍 `open`;⑩ ⭐ 存量:**#388 的 W34 103 份 `.dem` 2026-09-22 到期(19 天)** / #420(形状本轮不见了)/ #418 / #412 / #432 / #402 / #395 / #375 / 按需常数第十四轮 / #317 五条 / `strategy-5b` 第二十轮 / #290 / #313 / #207 / #271 / #364 / #398 / #399 / #382 / #383 / #218 / #367 / #295 / #329 / #321 / #423。
  **下一轮本台 = 收割 W41**(约 `04:1x–04:3xZ` 自毁)。**四件必做**:(a) ⭐ 若触发早于 `~05:1x–05:3xZ`,**第一件事抓四个 `StateReason.Code`**;(b) `recover_verdict.py` 全量重算,armed 串**从本波记录取回**(`arm_md5 38423b79…`)不从当时的 `test_set.md` 取;(c) `rec_slot_cost.py` 对 baseline 跑 §DA.2 门(同 `rec_slots=8`);(d) **市场回 spot**(#271 是「一波按需之后回 spot」),发波前跑 `reclaim_blind.py --wave-json W41_wave.json` —— ⚠️ **本波无 `rerun`,四粒全在 `machines[]` 里 ⇒ 这次它数得对**;四行按 #408 第二半都要带 `"market": "on-demand"` + `"status_code": <EC2 StateReason.Code>`。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T031000Z.md` 与 `iterations/reports/batch-desk/waves/W41_wave.json`。

- 2026-09-03T06:15–06:5xZ(**收割 W41 = 57-id 家族第一波。本轮真正的产出是「上一轮预登记的胜负通道判据触发了,而且比两个预登记分支都更硬」——三条轴全换仍 DEGENERATE 且恶化一个数量级。本轮零发波、零新增支出**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十六次,每次都是当轮第一条命令**);工具自报仍 `it has recurred 5x`,照前几轮据实并列。改走重定向后重跑,⭐ **本轮九条腿全部跑完,真退出码 `EXIT=3`**(`legs run 9` / `FINDINGS = cadence stale-waits trunk-red(python)` / **`UNCERTIFIABLE (exit 2): none`** / `NOT RUN (inside a leg): test_rc_wrapper.py test_selfcheck_lua_leg.py` / `worst exit 3`)⇒ **本轮没有「没人看过」的那一侧**,exit 3 是**有发现**不是没跑成。Lua 腿真跑了:`73 tagged detector file(s), 0 failures`,但工具自声明这是 **FAST SUBSET 非全套**,照此不夸大。
  ⭐⭐ **上一轮交棒 ①(「400s 上限 0-for-2」)本轮拿到一条零改动的现成绕路,并连带修正我自己写下的一个错判**:跑到 400s 时**没有被杀**,而是被 harness **移到后台继续跑**,约 **25 分钟**后正常收尾。⇒ **不存在「400s 0-for-3」这回事**;我在报告初稿里按前两轮的形状写了「仍在跑 / Lua 那侧没人看过 / 第三次交同一根棒」,**那是照抄上轮结论而不是读本轮读数,已在 push 前改正**。交棒 ① **降级但不撤销**(25 分钟对 2h 节拍仍偏长,但后台跑完可用)。八条腿其余读数:`cadence` 两洞(hero/strategy);`ORPHAN_PROPOSAL none`;`OWED_EXECUTION none`;`stable-v1` 锚 OK;`REFUSED refs 699/722`(同形,非新增)。⛔ **python 腿 TRUNK RED 两条**:`test_stale_waits.py`(= 上轮为它开的 **GH #443**,全落在 `replay-check.md:9827/:9838`,棒在录像组,**本轮不重复开**)、`test_detector_source_constants.py`(与上轮同,**未 stash 重跑 ⇒ 不声称 main 也红**)。`UNCERTIFIABLE`:`test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`。
  **二、成本** 九月 MTD **`$7.056`**(免费 `budgets`)。⭐ **连续三轮不翻新的那张快照本轮翻新了**:`refreshed 2026-09-03T04:41:39Z` **晚于** W40 起飞(09-02T21:31Z)**和** W41 起飞(03:32Z,约 04:2x 自毁)⇒ W40 + 两台按需补跑 + W41 绝大部分**都已进入这个数**,上一轮「W40 一分钱不在里面」的登记**到此解除**。`forecast 111.209` 不是读数,不进围栏。MTD `< $35` ⇒ CE 未触发,`$0.01` 没花。围栏(2 甲,保守:W41 即便已进 MTD 也再计一次)= `7.056 + W41 on-demand 2.15` = **`~$9.21`** ≤ **$80** ⇒ 不跨任何 owner 可见告警档,不欠解释行。**本轮零新增 AWS 支出**(仅 S3 GET + 免费 `budgets`/`describe-*`)。
  **三、⛔ GH #427 义务 (a):0-for-3,窗口已关。** 本轮**第一件 AWS 动作**就是抓码:`describe-instances --instance-ids <四台>` ⇒ **`EXIT=0`,输出 `[]`** —— 四台一台都不在了,四个 `StateReason.Code` **全丢**。W38(2)+W40(2)+W41(**4**)⇒ **累计丢 8 台死因码**。**这是结构不是纪律**:1h 证据窗口(~`05:1x–05:3xZ` 关)套在 2–3h cron 节拍里,本轮 `06:15Z` 触发差 **~45 分钟**,没有任何纪律救得回来。⇒ 交棒总监在 (甲) 只读 `cloudtrail:LookupEvents` / (乙) 关机钩子自抓死因上传 S3 之间裁一条;**本台倾向 (乙)(不受窗口约束),不自己改 harness**。
  **四、收割:四台全产局,零抢占,零缺臂。** 分 run 下载**不扁平化**(GH #225),`files_seen == games_loaded == 223` / `unparseable 0` / `pooled_basename_dirs_overridden 0`。armed 串**从本波记录取回不从当时的 `test_set.md` 取**,且**现场重算 md5 不抄**:`57 ids / 501 字节 / md5 38423b791c05d96e0e16ed0b51bd132d`,与波次文件逐字节相同。`RV_EXIT=0`、`contrast vs_stable`。
  局数:3017 `ab37/ba19` depth 25.11 / 56 局;3030 `31/17` 21.96 / 48;3161 `37/16` 22.34 / 53;3329 `29/13` 17.95 / 42。`scored_games` **199**、`unfinished 0`、`min_arm_depth 8` ⇒ **四粒全过 GH #269 深度门**,`thin_arm_seeds` 空,**无作废粒**。⚠️ `ab:ba` 又约 2:1(合计 **134:65**),与 W39 同形(换边按经过时间不按局数),**不是抢占**(本波零抢占)。
  **4.1 四量**:gpm **−19.84**、xpm **−15.31**、deaths **+0.28**、last_hits **−1.54**,`comps_better` **四量全 1/4**,`suggested hold_or_reject`。
  **4.2 分层(4(i-a),工具自打未手算)**:gpm ab `−59.49`/ba `+19.82`;xpm `−55.66`/`+25.03`;deaths `+0.67`/`−0.11`;lh `−10.95`/`+7.87` —— **四量全 sign_flip、side_gt_arm 4/4(lh 3/4)**,按 **4(i-c) 是恒等式不是诊断**,登记不据此下结论;并池由工具**逐粒 swap-average 后取算术平均,未按局数加权**(4(i-d),已核对)。⚠️ 逐粒 gpm **−7.18 / +75.21 / −32.60 / −114.77**,极差 **190** ≫ 08-19 校准的镜像零点 σ≈30 ⇒ **这四个数本身也谈不上一个干净的效应量,本台不判读**。
  **4.3 ⭐⭐⭐ 胜负通道:预登记判据触发,且比两个预登记分支都更硬。** `winrate_channel` **`DEGENERATE`**;`winrate_side_census {radiant: 1, dire: 222}`、`minority_side_share` **`0.0045`**(W40 是 `8/192 = 0.0417`,**恶化一个数量级**);`mean.winrate 0.504` / `headroom 0.0073` / `comps_better winrate 1/4` **全按占位符,不得引作条件 (b)**;逐粒 headroom `0.0/0.0294/0.0/0.0`;`winner_by {engine_natural: 223}`、`winrate_independent_of_gold 223/223`。工具告警逐字抄进报告。按 GH #352 **事先登记**的验收方式(少数侧 ≥20% 才算恢复)**未恢复**。
  ⭐ **为什么这一波重**:`W41_wave.json:winrate_prereg` 事先把判据钉成二分,而 W41 在**三条轴上同时全新**——① 全新 id 家族(57-id,55-id 家族已关档)② 全新种子(四粒发波前全 `no banked games`,窗口右移 `[3001,3400]`)③ 新树(`694c31fe`,`bots/` 相对 W40 真动过)。三条全换横扫**不但没恢复还恶化** ⇒ **「换 id 集 / 换种子 / 加种子数」三条路被本波按构造排除**,上一轮交棒那句「要改测法不是改种子数」拿到最干净的确证。
  ⭐ **但本台把那句收窄一格,因为语料形状比「测法」更硬**:「镜像 winrate 被恒等式逼成 0.500」只是**测法**那一半;**真正的病灶是物理侧横扫本身,而它不是测法造的**(`winner_by` **223/223 全 `engine_natural`** = 遗迹真倒,不是按金钱判)。**当胜者几乎是「侧别」的函数时,这份语料里不存在任何可归因给臂的胜负信号 —— 换估计量也救不回来,因为信息不在语料里。** ⇒ **换测法必要但不充分,必须同时修横扫本身**(#352 那 23 个 commit 的归因自 08-31 立案**未动**)。⚠️ 边界:这是**读数 + 一步算术**不是归因判决,**归因不是本台的活**。
  **4.4 SS-DT.4(`ckpush`)/ SS-DP.8(`roshdist`):UNINTERPRETABLE,不是「测了没效果」。** 逐字核查本波 `*.analysis.json` **顶层键**(`log/script_version/winner/winner_by/team_gold/econ_winner/duration_*/natural_end/cap_min/wall_s/effective_timescale/mode/avg_gpm/players/towers/anomaly_*`)——**没有任何帧级检测器字段**(`players[]` 只到 per-hero gpm/xpm/lh/deaths/damage)⇒ 六合取域的帧计数**本语料答不了**。按义务给载体分布:**`chaos_knight` 223/223 局、4/4 粒(满席)**,`crystal_maiden` 同 ⇒ **缺的是检测器不是载体**,`ckpush` 载体覆盖已是可能的最大值。**两者与 4.2 的经济四量没有混。**
  **4.5 ⚠️ 零 pudge 第五波**(223 局 0 次,四粒全无;发波时已预登记)⇒ `strategy-26`/`rotscope` 第五轮无证据,**按义务写成「无载体」不许写成负向读数**。
  **五、⭐ SS-DA.2 采集门 PASS,而且是重建后 baseline 的第一次非循环通过。** `rec_slot_cost.py <四 run> --baseline rec_slot_baseline.json` ⇒ **`RSC_EXIT=0`**,`compared 16 slot(s); 0 recording slot(s) beyond tolerance` / `OK: measured on 223 games, 8 control slots`;`box factor 0.987`(以下已扣除),逐槽净值 **−0.8% ~ +0.9%**(录制槽 1–8 为 −0.6% ~ +0.8%),第二通道同向(八个录制槽 12–15 局,相对对照槽均值 −2.38 ~ +0.62 局)。⭐ **为什么这次算数**:W40 那次是**把门跑在它自己的语料上**(零信息满分表);**本轮 baseline(395 局)与语料(223 局)是两份不同语料、`rec_slots=8` 同配置** ⇒ 第一次真正非循环。⚠️ **一处诚实登记**:本门第一次跑打 `RSC_EXIT=2`(`no .demclaim.json anywhere`),**归因是我下载不全**(首遍只 `--include "*.analysis.json"` 漏了 claim sidecar),补下 106 份 sidecar 后 exit 0;**exit 2 不是通过,但这个 exit 2 不记到 harness 头上**。配置 `--rec-slots 8` 不动(抬到 16 当场作废本门;16-of-16 是永久拒答)。
  **六、GH #435 选种改动如预登记落地(7/7)。** 从 223 局 `players[]` **数出来不抄波次文件**:`spirit_breaker {3017,3161}` / `lion {3017,3329}` / `obsidian_destroyer {3030,3161}` / `skeleton_king {3030,3329}` **四个互不相同的对全部对上**,`chaos_knight`+`crystal_maiden` 全四席、`zuus` 三席 {3030,3161,3329} 也对上 ⇒ ≥2/term 自律在**交货侧**成立。⛔ **诚实边界:本波零抢占四台全交货,#435 想防的那件事(一次 AZ 事件把三项一起打到 0)这一波根本没被演练** —— 过的是「选种按设计执行了」,不是「掉机时真的不掉项」,**不要据此关 #435**。
  **七、泄漏 CLEAN(判据是「恰好为空」)**:进场 + 离场两次 `state=pending,running` **都空**;`describe-volumes` **空,零孤儿卷**;常驻仅 AMI `ami-0a990a26d89c66547`。⚠️ 再登记:`--leak-only` 只报「多出来的」,「少一台」永远不举手(本轮用全量点名)。
  **七之一、GH #432 本波不复现**:逐 run 数 S3 对象,`analysis_` 重名副本 **15a2fe 0 / 0af9fd 0 / 1197cd 0 / d87d45 0**(对象总数 154/133/148/120)⇒ 与上轮「只有被抢占那台产出重名副本」的归因**一致**(本波零抢占)。**是正例不是结案**,下次抢占会原样复发,**#432 保持 open**。
  **八、本轮不发波,卡在闸 (i)**:W41 起飞 `03:31:51Z` ⇒ 解锁 **`2026-09-03T09:31:51Z`**,本轮约 `06:15–06:5xZ`,**还差约 2h40m**。(ii) `test_set.md` 仍 57 ids、`bots/` 自 `694c31fe` 未再动 ⇒ 第一支不成立,**第三支(累计种子 < 8)成立**(57-id 家族现 4 粒 / 199 计分局);(iii) 围栏 `~$9.21` ≤ $80 过。**卡在 (i),不是卡在钱也不是卡在没东西测 —— 不发波、不空转等待**(铁律 7/11)。
  **九、GitHub:本轮把 #290 的顺序做对了** —— 报告与章程**先 commit+push**(`PUSH1_EXIT=0` 分支 / `PUSH2_EXIT=0` `8cd94889..303457c6 HEAD -> main`),再跑 `claim_precheck.sh`,最后发产物;上一轮登记的顺序违规**未复发**。✅ **GH #352 追评已发表 `#issuecomment-5521530866`**(4.3 的升级 + #427 的 0-for-3 附在同一条,不另开);**#443 本轮仍红但无新信息,不重复追评**。
  **九之一、⭐⭐ 发评论前那道门自己出了问题,失效方向是「让人删掉正确的引用」——已开 GH #446。** 草稿引了 #352 正文点名的三个非门控修复,`claim_precheck.sh` 打 **`PRECHECK_EXIT=3` / `OFF-TRUNK 48ff29fe、c48dc11b、75a88fa3` / `DO NOT PUBLISH YET` / `Push, then re-run`**,表面读法是「#352 正文那句『五个被引 commit 均已核实为 origin/main 的祖先』写错了」——**本轮差一步就要把这个论断写进评论**。
  **没有据此下结论,去查了原因**:`git rev-parse --is-shallow-repository` ⇒ `true`,`git rev-list --count origin/main` ⇒ **51**(底板停在 2026-09-02),而被引 commit 是 **08-25/08-27** 的、**在底板以下** ⇒ `merge-base --is-ancestor` 按构造答 NO,又因别的会话分支 fetch 深度不同而被归成 "resolves on `origin/claude/...`"。`git fetch --deepen=600 origin main`(758 commit)后重查:**五个全部 `ancestor-of-main: YES`**,同一份草稿重跑 ⇒ **`PRECHECK_EXIT=0` / `clean` / `OK to publish`**。⇒ **#352 正文是对的,是门错了。**
  三点:(1) **不是偶发** —— Routine 容器每次都是全新 shallow clone,底板 ~51 commit ≈ 一天 ⇒ **任何引用一天前以上 commit 的草稿都会踩**,跨周归因讨论几乎必然;(2) **失效方向朝着「删掉正确的东西」** —— `Push, then re-run` 把人往「有东西没推」推,但推什么都没用(同一份输出里 `local commits not on origin/main: 0` 与 `OFF-TRUNK` **两个读数在打架**,GH #420 同形),推了没用之后最省事的「修法」就是删掉那几个 hash;(3) **工具自己知道自己 shallow**(`(shallow clone)` 是它打的)**却没把这事实接到判据上** —— 与 #171/#205 同族但方向相反:那两次是**门没开过**,这次是**门开了却指错方向**。**本台不自己改 harness**,三条建议(甲 自行 deepen 后复判/拿不到就降 `UNCERTIFIABLE` exit 2、乙 只改行文但 `Push, then re-run` 该分支下必须不出现、丙 两读数打架自己举手)已写进 #446。⚠️ **本轮没有因此改动任何已发表引用。**
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #352 升级(头号)**:三条轴全换仍 DEGENERATE 且恶化一个数量级 ⇒ 「加种子/换 id 集」**按构造排除**;横扫是 `engine_natural` 223/223 的真胜负,**换估计量救不回来**。请裁:先修横扫(那 23 个 commit 的归因,立案至今未动),还是接受「条件 (b) 在横扫修好前一律买不到」并据此改铁律 2 的可执行形式;② ⭐⭐⭐ 总监 —— **GH #427 在 (甲) CloudTrail /(乙) 关机钩子自抓死因 之间裁一条**(0-for-3,累计丢 8 台码,结构性);③ ⭐ 总监 —— 自检上限**降级交棒**:本轮 400s 时被 harness 移到后台并跑完九条腿(约 25 分钟,`EXIT=3`),**「400s 0-for-3」不成立(那是我照抄上轮结论的错判,已改正)**;抬高上限/拆腿仍值得做但不再是 ⭐⭐⭐;④ ⭐⭐ 录像组 —— `ckpush`/`roshdist` 的 (a) 取证,本波 `.dem`(四 run,`rec_slots=8`)是最好对象且 `chaos_knight` **223/223 满载**;⑤ ⭐⭐ 录像组 —— **GH #443** 第二轮仍红;⑥ ⭐ 总监 —— #435 **7/7 如预登记但零抢占没演练防掉项那一面,不要据此关**;⑦ ⭐ 总监/协同组 —— **零 pudge 第五波**(写「无载体」);⑧ ⭐⭐ 总监 —— **GH #446 新开**(`claim_precheck.sh` 在 shallow clone 上的假阳性,失效方向是让人删掉正确引用);⑨ ⭐ 存量:#432(不复现,保持 open)/ **#388 的 W34 103 份 `.dem` 2026-09-22 到期(19 天)** / #429 / #420 / #418 / #412 / #408 / #402 / #395 / #375 / #285 / #256 / #290 / #313 / #271 / #329。
  **下一轮本台 = 发 W42**(57-id 家族第二波,闸 (i) `09:31:51Z` 解锁)。**四件必做**:(a) **市场回 spot**(#271 是「一波按需之后回 spot」,W41 就是那一波),发波前跑 `reclaim_blind.py --wave-json W41_wave.json` —— ⚠️ 本波无 `rerun`、四粒全在 `machines[]` 里 ⇒ **这次它数得对**;(b) 种子从 `[3001,3400]` 继续按 `≥2/term` + 载体对分散度选(窗口约还能撑 4–6 波);(c) 采集配置**不动**(`--slots 16 --hours 2 --rec-slots 8 --games 12`,4×1);(d) 起飞前照常过 `carrier_terms.py --arm` / `check_armed_wiring.py --cand` / `seed_roster_index.py --build`(**在选种之前**)/ `CARRIER_GATE` / `test_wave_gate_keys.py` / `tree_verified`。
  **十一、铁律 6**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态半 `luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(容器冷启自己装 luacheck,apt 包名 `lua-check`;`bots game: 0 warnings`),`arm_push_gate.sh` **`ARM_EXIT=0`** 已上膛 ⇒ 两次 `git push` 各自又跑一次同一道门;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`(GitHub MCP 本轮可用,一条追评 + 一个新 issue 都发成了);**无空转等待**。
  **本轮 token**:`TOKENS total_in=5,876,318 out=50,450 turns=50`。
  ⭐ **新开**:GH **#446**(`[harness]`,见九之一)。
  详见 `iterations/reports/batch-desk/20260903T061500Z.md`。

- 2026-09-03T09:15–09:4xZ(**发 W42 = 57-id 家族第二波,spot 回归。本轮真正的产出是两件:GH #427 丢的窗口把 GH #271 的闸也一起关了(而且关的形状是 `exit 2`),以及「树对象哈希」当并池判据在一个纯注释 commit 上给出了错的答案**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十七次,每次都是当轮第一条命令**)。改重定向后 ⭐ **九条腿全跑完,真退出码 `EXIT=3`**(`legs run 9` / `FINDINGS = cadence trunk-red(python)` / **`UNCERTIFIABLE: none`** / `NOT RUN: test_rc_wrapper.py test_selfcheck_lua_leg.py`)⇒ **本轮没有「没人看过」的那一侧**。python 腿 `80 passed, 1 failed`:⛔ `test_detector_source_constants.py` **第五轮原样红**(#420;**未 stash ⇒ 不声称 main 也红**);⭐ **`test_stale_waits.py` 本轮转绿**(上轮为它开的 **GH #443** 那条红线不在了)——**但本台只看到「这一轮绿」,没查是谁修的、也没查域有没有改**,关 #443 前请各自核一眼。Lua 腿 `73 tagged detector file(s), 0 failures`(工具自声明 **FAST SUBSET**,不夸大)。上一轮那根「400s 上限」的棒**再次证实是误报**(400s 时被 harness 移到后台,25 分钟后正常收尾),维持降级。
  ⛔⛔ **同一个管道错本轮犯了第二次,而且是在自检当面拒过之后**:发波循环写的是 `spot_run.sh … | tail -12` 然后 `SPOT_RUN_EXIT[$s]=$?`,**那个 `$?` 是 `tail` 的**。自检那次**有工具举手**,发波这次**没有任何东西举手**(`tail` 几乎总成功 ⇒ 一台起飞失败也打 `=0`)。⇒ 本轮**不引用**那些 `SPOT_RUN_EXIT`,起飞成败一律以 `describe-instances` 为准。**登记的意义在形状**:这个错被抓了十七次,抓的都是**当轮第一条命令**,而它真正咬人的地方是**发波循环**——那里没有守卫,失效方向是「起飞失败被读成起飞成功」。
  **二、成本** 九月 MTD **`$7.056`**(免费 `budgets`,`refreshed 2026-09-03T04:41:39Z`,**晚于 W40 与 W41 两次起飞 ⇒ 两波都已进这个数**)。`forecast 111.209` 不是读数。MTD `< $35` ⇒ CE 未触发,**`$0.01` 没花**。围栏(2 甲,保守重计 W41)= `7.056 + 2.15 + 0.90` = **`~$10.11`** ≤ **$80**,不跨任何 owner 可见告警档。
  **三、⭐⭐⭐ GH #271 的闸本轮差一点静默失效,追评已发。** 发波前按义务跑 `reclaim_blind.py --wave-json W41_wave.json` ⇒ **`RB_EXIT=2`**(`UNDECIDABLE: seed 3017: needs survival_min, or both create and update`)。**归因不是工具**:W41 收割轮拿到了 `ab/ba/arm_depth` 却没写回波次文件,而精确存活读数早已随 **#427** 的 1h 窗口全丢。⇒ **那个窗口关掉的不止一个字段,是一道闸**,而闸失效的形状是 `exit 2` —— 「exit 2 不是通过」,**但它也不会自己变成一个决定**。按工具文件头为这种情况写好的 `survival_bound: "lower"`(立它的是 GH #332)recover:**S3 `soak/<run_id>/` 最后一个对象的 `LastModified` 减 `launched_at`**,是存活下界,**S3 的上传不过期**。四台实测 ≥52.4 / ≥52.4 / ≥52.5 / ≥41.6 min(全在 40 min 换腿点之上),连同逐粒 `ab/ba/arm_depth` 回填进 `W41_wave.json`(每行带 `survival_source`/`depth_source`;`status_code` 据实写 `LOST -- GH #427 window shut …`,**不编没读到的码**)⇒ **`RB_EXIT=0`**,`not blinded / 4 paired seeds / NEXT WAVE: spot`。⭐ **这要变成收割轮的固定动作,不是本轮的巧思**(已写进 W42 收割义务)。⚠️ 边界:下界只对「活过换腿点」这一侧健全,反向不健全(工具自己会出声跳过)。
  **四、⭐⭐ 树漂移:哈希变了,编译出来没变 —— 并池判据要重裁(**GH #449 已开**)。** W41 的 ref 之后 `bots/` 只动 `hero_chaos_knight.lua`(commit `9053d199`,英雄组 GH #447,**+18 行注释**),`game/` 树 `f13b532c` 未动。**四个判官,把打架的也登记**:`git diff` 过滤注释 ⇒ 非注释改动 **0 行**;仓库 `strip_lua_comments` 逐行比 ⇒ **395/395** 相同(原始 18 字节差是这个朴素剥离器留下的**行尾空白**);⭐ **`luac5.1 -l -l` 归一化指针地址与行号 ⇒ 差 0 行(决定性)**;⛔ **`luac5.1 -s` 的 md5 不同 —— 未采信但登记**。⇒ **一个「编译后哈希」式的朴素判官会说这是两棵不同的树**,而带列表的反汇编说指令与常量池逐条相同、唯一的差是行号。**后果是实的**:57-id 家族要 8 粒,W41 给 4 粒、W42 给 4 粒,**并得了就是一步到位**,而现行判据是树对象哈希。本台读数是**能并**,**决定权在总监**,已**预登记**在 `W42_wave.json:pooling_claim`(**四个数落地之前**写的,防事后拟合)。**闸 (ii) 因此走第三支不走第一支**(arm 串逐字节未变、树编译后未变;家族 4 粒 < 8)。
  **五、发波前六道门全过**:`carrier_terms.py --arm` `CT_EXIT=0`(11 hero / 46 generic / **0 unresolved** ⇒ 7 项);`check_armed_wiring.py --cand` `CAW_EXIT=0`(**57 个 armed id 全 wired**);`seed_roster_index.py --build` `SRI_EXIT=0`(**在选种之前**);`CARRIER_GATE ids=11 seeds=4 exit=0`;`test_wave_gate_keys.py` `WGK_EXIT=0`(**184 checks, 0 failed**,含新写的 W42);`git ls-remote origin main == local HEAD == 1baa449b…`。⚠️ **章程勘误**:上一轮「下一轮必做 (d)」把接线门写成 `tools/batch_test/soak/check_armed_wiring.py`,**该路径不存在**(照它跑 `EXIT=2` 打 `No such file or directory`,**exit 2 不是通过**);真实路径是 **`tools/batch_test/check_armed_wiring.py`**(无 `soak/`)。
  **六、选种 3040/3101/3117/3204** —— 窗口 `[3001,3400]`,`avail=396`、`excluded-in-window=4`(恰好 W41 那四粒),`masks=71`、popcount `{0:34,1:103,2:161,3:76,4:21,5:1}`,**BEST min-per-term = 2**(787 组合,最大 17 槽的 25 个),**自律未放松**;`--seed` 复核**四粒全 `no banked games`**。⭐ **五个 2-载体项坐在五对互不相同的种子上**(W41 是四对):`spirit_breaker{3040,3101}` / `chaos_knight{3101,3117}` / `lion{3040,3117}` / `obsidian_destroyer{3117,3204}` / `skeleton_king{3040,3204}`,`crystal_maiden` 三席、`zuus` 满席。**代码核过(不是眼看)**:四种单台掉机下每项最少载体数**都是 1,没有 0**。⚠️ **但只深一层**:6 种两台同掉里 **5 种**会把至少一项打到 0。⚠️ 另两条诚实边界:搜索**穷举了 mask 组合但每组只取一个种子指派** ⇒ 这是 25 个最大槽组合里的最好、**不是全局最优**;搜索脚本把 banked 索引找错了路径(`tools/batch_test/soak/` 而非 `iterations/data/`)⇒ 索引贡献 0 条排除,**事后核过无害**(索引 193 粒 banked 与本窗口的交集恰好是 W41 那四粒,早已由波次文件排除)。⚠️ **零 pudge 第六波** ⇒ `strategy-26`/`rotscope` 收割时写「无载体」,**不许写成负向读数**。
  **七、⭐ 起飞:四台全 spot、四个不同 AZ、`requested == actual` 四比四。** 守卫 `09:32:03Z` 打 `GATE-I OPEN`(09:19:14Z 那次调用被它按 **exit 9** 挡下,`remaining=757s`),`09:32:21Z` 发完,树钉 `--ref 1baa449b…`。3040/`i-08f6554de561a016b`/**2a** · 3101/`i-0371d22b2345adff0`/**2b** · 3117/`i-0d24250f341351b93`/**2c** · 3204/`i-036f2c0ccba66ba56`/**2d**,四台 `InstanceLifecycle` 全 **`spot`**(⇒ owner GH #158 的 spot-first 已恢复,#271 的一波按需由 W41 消耗完毕),**零容量降级**(四台 c6i spot 全在请求 AZ 一次填满,没退 `c6a`、没上 `--on-demand`)。⭐ **GH #256 严格验收本波再次通过,而且这次是在 spot 上** —— W41 那次本台自己标过「按需放置容易得多,过的是字面」,本波把那条边界补上,是 **W36 以来 spot 容量压力下第一次严格通过**;⛔ **仍是一波,不要据此关 #256**(W36–W40 连咬七波)。预计自毁 `~10:1x–10:3xZ`,看门狗 **11:32Z**,预期 **~96 计分局**。
  **八、泄漏 CLEAN(判据是「恰好等于意图的那四台」)**:`pending,running` 全量普查**恰好四台**且与 `machines[].run_id` 逐一对上(**没有多的也没有少的**);`describe-volumes` 四卷全 `in-use`、零孤儿;常驻仅 AMI `ami-0a990a26d89c66547`。⚠️ 再登记:`--leak-only` 只报「多出来的」,**「少一台」永远不举手**。
  **九、铁律 6**:`bots/`/`game/` **本会话一行未改**;`luacheck_gate.sh` ⇒ **`GATE_EXIT=0 CLEAN`**(`bots game: 0 warnings`),`arm_push_gate.sh` **`ARM_EXIT=0`**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。⭐ **GH #446 本轮主动绕开**:发评论前先 `git fetch --deepen=600 origin main`(`DEEPEN_EXIT=0`,底板 **50 → 766**),免得 `claim_precheck.sh` 在 shallow clone 上把正确引用报成 `OFF-TRUNK`。**这是绕路不是修复,#446 保持 open。** **铁律 11** 未触发 `requires approval`;**无空转等待**(闸 (i) 扣住的 ~13 分钟全用在零成本工作上)。
  **交棒**:① ⭐⭐⭐ 总监 —— **并池判据要裁,GH #449 已开**(见四);② ⭐⭐⭐ 总监 —— **GH #427 追评已发**(`#issuecomment-5523706619`),建议验收改成两条并行,新增那条**今天就能做、零成本**(见三);③ ⭐⭐⭐ 总监 —— **GH #352 仍头号**,W42 **故意一条轴都不换**,是「病灶是语料不是 id 集/种子数」的干净复制检验,两分支已预登记;④ ⭐⭐ 总监 —— #256 **在 spot 上严格通过**但**仍是一波**;⑤ ⭐⭐ 总监 —— **章程勘误**(接线门路径,见五);⑥ ⭐⭐ 录像组 —— `ckpush`/`roshdist` 的 (a) 取证走本波 `.dem`(⚠️ W41 已证 `analysis.json` **不带帧级检测器字段**);⑦ ⭐ 录像组/总监 —— **#443 本轮转绿**,关它之前请各自核一眼;⑧ ⭐ 总监/协同组 —— **零 pudge 第六波**;⑨ ⭐ 总监 —— **#435 拿到比 W41 更强的预登记(五对)但只深一层**;⑩ ⭐ 存量:#420 第五轮原样红 / #432 / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(19 天)** / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #285 / #290 / #313 / #271 / #329 / `strategy-5b` / `strategy-31/32/37/38`。
  **下一轮本台 = 收割 W42**(四台预计 `~10:1x–10:3xZ` 自毁)。**收割四件本波特有**:(a) ⭐ **spot 机有 SIR** ⇒ 死因码走 `describe-spot-instance-requests`(活过 `describe-instances` 的 1h 窗口),**并且必须把 `survival_min` 与 `ab/ba/arm_depth` 写回 `machines[]`**(不写,下一轮 #271 闸又答不了 —— 本轮的教训);(b) **两种读数都报**(W42 单独 + W41∪W42 并池,标清哪个是哪个,并池交总监裁,**一律先每粒 swap-average 再跨种子算术平均**,铁律 4(i-d));(c) `rec_slot_cost.py … --baseline rec_slot_baseline.json` 第三次非循环跑,⚠️ **连 `.demclaim.json` sidecar 一起下**(只下 `*.analysis.json` 会 `exit 2`,上一轮踩过);(d) 按 `winrate_prereg` 两分支报,**不许反向拟合**。闸 (i) 对下一次例行发波解锁于 **`2026-09-03T15:32:03Z`**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T091500Z.md` 与 `iterations/reports/batch-desk/waves/W42_wave.json`。
- 2026-09-03T12:15–12:5xZ(**收割 W42 + 按 GH #408 发一台按需补跑机。本轮真正的产出是三件,第一件是本台自己漏掉的活**)。
  **一、⛔ 06:15Z 的 W41 收割轮从未把读数写回 `queue.json`。** 十三条搭 W41 的请求到今天为止 `status` 还是 `running`、`result_wave` 还是 **W40**、`result` 还是 W40 的散文 —— **下游读到的是「一波之前的数字挂在当前波的名下」,而没有任何门会举手**(章程步骤 3 那句「结果写进报告 **+ 更新 queue.json**」是纪律不是门)。本轮把 W41、W42、并池三份一次补齐(13 条,显式白名单 + `assert 'W41' in rides_wave`;`strategy-31/32` **第八轮不搭波**,按构造搭不上,按铁律 9 交回总监)。
  **二、⭐⭐⭐ GH #435 的载体设计第一次被真事故考了,过了。** 3040 被 `instance-terminated-no-capacity` 回收,**七个载体项一个都没归零**(逐项:zuus 3 / cm 3 / ck 2 / od 2 / **sb 1 / lion 1 / sk 1**,**代码算的不是目测**)。对照 W40:一次 `2c` 事件把三项直接打到 **0**。⚠️ **保护恰好只深一层**(六种两台同掉里五种仍会归零至少一项),**一次成功不是「修好了」,不要据此关 #435**。
  **三、⭐⭐⭐ `winrate` 预登记分支 (a) 触发。** W42 是**只换种子**的干净复制检验(arm 串逐字节不变、树编译后逐条不变),通道**仍 `DEGENERATE`**(少数侧 **0.0243**,并池 **0.0140**;`mean.winrate_headroom` 0.0347 / 0.0191)⇒ **「加种子 / 换种子集」这条路按构造已死,病灶是语料**;分支 (b) 未触发 ⇒ 上一轮的 4.3 读数不需更正。GH #352 的「改测法 **且** 修横扫」现在**站在两个互相独立的种子集上**。⭐ 顺带确认 `winrate_independent_of_gold` 打的是真值 **`206/206`**(不是 W31 那个假的 `0/222`)⇒ 桶名 bug 已修,本台 08-29 立的逐波复发登记**就此停掉**。
  **四、⭐⭐ 死因码:SIR 这条路第一次在窗口已关之后真的用上。** `terminated,shutting-down` 普查**返回空**(终止 09:46–10:28Z,读的时刻 12:2xZ,GH #427 的 1h 窗口早关);而 **spot 机有 SIR**,`describe-spot-instance-requests` **四台全读到**:3040 `instance-terminated-no-capacity` @09:46:31Z 存活 **14.43 min**;3101/3117/3204 全 `instance-terminated-by-user`,存活 56.20 / 55.05 / 56.48 min。存活是 **EXACT**(SIR `Status.UpdateTime` − `launched_at`),**不是 W41 那种 S3 下界**。⇒ **GH #427 多一条零成本的 (丙):spot 波本来就不吃那个窗口;真正暴露的是按需机 ⇒ GH #271 每下一次「一波按需」,就把那一波的死因码押在 1 小时上。**
  **五、⭐⭐ GH #271 的闸本轮第一手可答:`RB_EXIT=0`**,`yield 3 paired of 4`、`attribution 1 machine reclaimed before the flip`、`VERDICT: not blinded`、**`NEXT WAVE: spot`** ⇒ W43 仍 spot。⛔ **过程里踩到一个键名坑要登记**:回填先写成 `ab_games`/`ba_games`,而工具读的是 **`ab`/`ba`** ⇒ `RB_EXIT=2` `UNDECIDABLE: seed 3040: missing leg count 'ab'` —— **与它本来要修的毛病同形**(闸答不出来不会红,只会 exit 2,而 exit 2 最容易被读成通过)。`machines[]` 现在**两套键名都写**。
  **六、收割读数**。`RV_EXIT=0`,`files_seen == games_loaded == 206`、`unparseable 0`、`source_dirs 4`;`scored_games` **172**、`unfinished 0`、`min_arm_depth 8`、`thin_arm_seeds` 空。逐粒:**3040 ab10/ba0 `arm_depth 0.00` `NO-PAIR` 出集(仍点名 —— 那 10 局是真花了钱的)**;3101 32/24(27.43)、3117 41/17(24.03)、3204 37/21(26.79)。**W42 单波**:gpm **−19.15** / xpm **−7.80** / deaths **+0.12** / last_hits **−0.60**;`comps_better` 1/3·1/3·1/3·2/3;`suggested=hold_or_reject`。分层(工具自打未手算)gpm ab −35.99 / ba −2.32 / side −16.84 **不反号**(W41 是四量全反号),其余三量反号 —— 按 4(i-c) **恒等式不是诊断,登记不据此下结论**。逐粒 gpm −69.45 / −8.42 / +20.41,极差 90 > σ≈30 ⇒ **本台不判读效应量**。
  **七、⭐⭐ 并池 W41∪W42:报数不裁定(GH #449 在总监手上)。** 八个 run 子目录交给 `recover_verdict.py` ⇒ **逐粒 swap-average 后跨种子算术平均,绝不按局数加权**(4(i-d));`RV_EXIT=0`、`games_loaded 429`、`source_dirs 8`。**并池(7 粒 / 371 计分局)**:gpm **−19.54** / xpm **−12.09** / deaths **+0.21** / last_hits **−1.14**;`comps_better` 2/7·2/7·2/7·3/7;逐粒 gpm −7.18 / **+75.21** / −69.45 / −8.42 / −32.60 / +20.41 / **−114.77**(极差 **190**)。⛔⛔ **即便总监批准并池也只有 7 粒不是 8** —— 第 8 粒正由本轮的补跑机去买。
  **八、⚠️⚠️ 交总监:围栏用的机时常数漂了 25%,失效方向是危险的那一侧。** 活下来的三台 **0.9319 h/台**(0.9367/0.9175/0.9414)对章程常数 **0.745**。本波总机时 3.0361 × 价带 $0.231–0.250 ⇒ **实测 `$0.701–0.759`**,`$0.90` 自动失效条款**按字面不触发**;⛔ **但它不触发的唯一原因是有一台 14 分钟就死了** —— 按活下来的费率算一整波四台 = `3.7274` 机时 = **`$0.861–0.932`,上端越过 `$0.90`**。常数低估机时 ⇒ 围栏**少记**每一波的钱,方向是「以为还有余量」,而**一次抢占就能把实测压回线下**。⚠️ 这是**机时侧**推算**不是账单侧读数**(MTD `$7.056 < $35`,CE 未触发,本台没为此花那 $0.01)。**本台不自行重裁,按条款点名 —— GH #454。**
  **九、补跑(GH #408)**:3040 的 SIR 码是 `instance-terminated-no-capacity` ⇒ **直接 `--on-demand`,不走阶梯**;围栏「每粒只补一次 / 每波最多 2 台」**两条都满足**(1 台)。`i-07d6358854ccda217`,请求/实得 **`us-west-2c`**(**故意不选刚拒绝容量的 2a**),**`InstanceLifecycle=None`** 核实,12:19:25Z 起飞,**`RERUN_EXIT=0` 不经管道读的**。采集配置**逐项显式传齐**(`--slots 16 --hours 2 --rec-slots 8`),树钉 **`--ref 1baa449b…`**(W42 的树,**故意不跟**已走到 `c47925b9` 的 `origin/main`;`git fetch --depth 1 origin <40 位>` exit 0 核过)。**闸 (i) 未被消耗** ⇒ W43 仍解锁于 **`2026-09-03T15:32:03Z`**。3040 已有 10 局 ab 存量,补跑局按种子戳自动并入。
  **十、其余门**:`rec_slot_cost.py` 第三次非循环跑 **`RSC_EXIT=0`**(206 局 / 8 对照槽 / 箱体因子 0.993 / **`0 recording slot(s) beyond tolerance`**;⭐ **本轮把 `.demclaim.json` sidecar 一起下了**,上一轮那个 exit 2 没再踩)。`tests/test_wave_gate_keys.py` **`WGK_EXIT=0`,184 checks 0 failed**。`ckpush`/`roshdist` 的 (a) **本轮重查过不是沿用**:206 份 `analysis.json` 顶层键集合里**没有任何帧级检测器字段** ⇒ **UNINTERPRETABLE(没有量具),不是「测了没效果」**。**零 pudge 第七波**(逐局读 `players`,206 局 0 次;7 个载体项全部在场)⇒ `strategy-26`/`rotscope` 写「无载体」。**GH #432 本波未复发但这个否定很弱**:3040 被抢占时只交了 16 份,而 #432 要「交完再被抢占」⇒ **本波没测试它,只是没触发它**。
  **十一、泄漏 CLEAN(判据是「恰好等于意图」)**:进场 `pending,running` **空**;收尾点名 `soak-run` **恰好一个值**且与 `rerun.machines[0].run_id` 逐字对上,**没有多的也没有少的**;`describe-volumes` **一卷 `in-use`** 零孤儿;常驻仅 AMI。⚠️ `terminated,shutting-down` 的**空是 #375 窗口过期,不是「没东西终止过」**(SIR 证明四台都终止了)——**空普查对历史一个字都不证明**。
  **十二、自检(铁律 10)**:⚠️ 开工第一条又误用管道,自检**当场自拒** `REFUSED: stdout is a pipe; exit 2, nothing checked`(**同形第十八次,又是当轮第一条命令**);改重定向重跑。⭐ **但本轮它没有第二次咬人** —— 发波那条用 `> file 2>&1; echo "EXIT=$?"`,**真码 `RERUN_EXIT=0`**(上一轮正是在这里把 `tail` 的码读成了 `spot_run.sh` 的)。⭐ 九条腿跑完,**真码 `EXIT=3`**:`FINDINGS = cadence trunk-red(python)`、**`UNCERTIFIABLE (exit 2): none`** ⇒ **有发现不是没跑成**。python 腿 `81 passed, 1 failed, 2 uncertifiable`,⛔ **`test_detector_source_constants.py` 第六轮原样红**(#420;**未 stash 重跑 ⇒ 不声称 main 也红**)。Lua 腿 **74 tagged detector file(s), 0 failures**,工具自声明是 **FAST SUBSET 非全套**。⚠️ **再登记一个读码陷阱**:harness 给这条后台命令报的是 `exit code 0` —— **那是命令串末尾那个 `echo` 的码不是自检的**,真码要读脚本自打的 `selfcheck worst exit: 3` 或显式的 `EXIT=3`;**与管道陷阱同族(外层包装的成功掩盖内层的码)**。
  **十三、铁律 6 / 11**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);`luacheck_gate.sh` 与 `arm_push_gate.sh` 读数见报告;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`;**无空转等待**(自检 Lua 腿在后台跑的整段时间用于收割/并池/门/发波)。
  **交棒**:① ⭐⭐⭐ 总监 —— **机时常数漂 25%,GH #454**(见八);② ⭐⭐⭐ 总监 —— **GH #449 并池待裁,现在有数了**,且**批了也只有 7 粒**(见七);③ ⭐⭐⭐ 总监 —— **GH #352 分支 (a) 触发,加种子这条路按构造已死**(见三);④ ⭐⭐ 总监 —— **#435 第一次被真事故考过了,但只深一层,不要据此关**(见二);⑤ ⭐⭐ 总监 —— **#427 多一条 (丙)**,真正暴露的是按需机(见四);⑥ ⭐⭐ **本台下一轮必做** —— 补跑机 13:1x–14:19Z 自毁、`StateReason.Code` **只能读 1 小时**,**开工先读它**,连同 `survival_min` + **`ab`/`ba`**/`arm_depth` 写回 `W42_wave.json:rerun.machines[]`;⑦ ⭐ 录像组 —— `ckpush`/`roshdist` 的 (a) 只能走 `.dem`(本轮重查确认无帧级字段);⑧ ⭐ 总监/协同组 —— **零 pudge 第七波**;⑨ ⭐ 存量:#420 **第六轮**原样红 / #432(本波**未测试**它)/ **#388 的 W34 103 份 `.dem` 2026-09-22 到期(19 天)** / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #290 / #313 / #271 / #329 / `strategy-5b` / **`strategy-31/32` 第八轮不搭波**。
  **十四、⭐⭐ 顺带把上一轮那条 recover 路线的误差第一次量出来了(零成本)**:上一轮用「S3 最后上传 − `launched_at`」当存活**下界**救了 GH #271 的闸,但**那条路线紧不紧从没人量过**(要量就得同时有下界和真值,而 W41 按需机没有 SIR)。W42 两样都有 ⇒ **正常自毁的三台下界偏短 `3.19 / 3.28 / 3.65` 分钟**(**紧,方向对** ⇒ 上一轮 W41 的四个下界真值约 55.6/55.6/55.7/45.3,**那次 recover 没把闸带偏**);⛔ **但被抢占的 3040 上它反号** —— 最后一次上传比 SIR `Status.UpdateTime` **晚 1.17 分钟**(SIR 记的是回收被**通告**的时刻,机器随后仍在关机流程里继续 flush)⇒ **对被抢占的机器它不是下界,是可能偏大的估计**,而偏大用在 `reclaim_blind` 的 40 min 换腿点上,**失效方向是把该作废的种子读成配对成功**。⚠️ n=3+1、**一波**,**本台不据此改任何常数或工具**,只登记给下一个走 recover 路线的人。⭐ 同时量到**上传节奏**:每 ~12 分钟一批,**第一个对象落在起飞后 ~14.5 分钟** —— 这正是「补跑机 14.1 分钟时 `OBJ_COUNT=0`」是**预期值不是症状**的依据(⛔ 本台一度把它误读成症状,**错在读钟不在读数据**)。
  **下一轮本台 = 收割补跑机(先读死因码,1h 窗口)+ 视情况发 W43**(闸 (i) 解锁 `2026-09-03T15:32:03Z`)。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T121500Z.md` 与 `iterations/reports/batch-desk/waves/W42_wave.json`。

- 2026-09-03T15:15–15:5xZ(**收 GH #408 补跑机 → W42 补成完整 4 粒 → 57-id 家族并池到 8 粒。本轮零发波、零新增支出。真正的产出是两件:#427 的 1h 窗口对按需补跑机是「按构造读不到」,以及闸 (ii) 三支被逐支测量后全灭**)。
  **一、自检** 第一条命令**又误用管道**,自检自拒(**同形第十九次,每次都是当轮第一条命令**);工具自报仍是 `it has recurred 5x`,与报告序列对不上,照上轮据实并列。⛔ 第一次重定向重跑用了 `timeout 300` ⇒ **`EXIT=124`**,被杀在 python 腿,**124 不是通过也不是发现**;改 `timeout 1500` 后台跑完。真码 **`SELFCHECK_EXIT=3`**(`selfcheck worst exit: 3`),`legs 9`、`FINDINGS = cadence trunk-red(python) trunk-red(lua)`、**`UNCERTIFIABLE: none`** ⇒ **有发现不是没跑成**。⚠️ **读码陷阱第二轮原样复发**:harness 给后台命令报 `exit code 0`,那是末尾 `echo` 的码不是自检的。⚠️ **本轮还自己读错了一次钟**:一度按「跑了 35 分钟」判断自检挂死,实测 `date -u` 距开工只有 **6 分钟** ——**错在读钟不在读数据**,与上一轮误读 `OBJ_COUNT=0` **同形第二次**。
  **二、成本** 九月 MTD **`$7.056`**(免费 `budgets`,快照 `refreshed 04:41:39Z` —— ⚠️ **早于 W42(09:32Z)与补跑机(12:19Z)起飞,两者都不在读数里**)。MTD `< $35` ⇒ **CE 未触发,没花那 `$0.01`**。进场在跑实例**为空**。围栏(2 甲,逐波按自己的市场取价)= `7.056 + W41 按需 2.15 + W42 spot 0.90 + 补跑机按需 0.65 = **$10.76**` ≤ **$80**。**本轮新增支出 $0**。
  **三、⭐⭐⭐ GH #427 升级:1h 窗口对「按需补跑机」不是容易错过,是按构造读不到。** 上一轮交棒点名「开工先读死因码」,本轮**开工第三条命令就去读了**:`describe-instances --instance-ids` **`[]`(exit 0)**、全状态普查 **`[]`**。**码永久没了,而且不是懈怠**:补跑机 12:19:25Z 起飞、S3 末次上传 13:11:40Z ⇒ 正常自毁 ~13:15Z ⇒ 窗口 ~14:15Z 关 ⇒ **本 Routine 下一次触发 15:15Z,比窗口晚整整一小时**。⇒ **2h cadence × 1h 窗口 = 周期中段自毁的机器永远读不到**,是算术不是纪律。⛔ **且按需机没有 SIR**(W42 四台 spot 都有,救过一次)⇒ **上一轮交棒第 ⑤ 条一轮之后就兑现,且兑现在最贵的那种机器上**。存活只能走下界:**52.25 min**(`survival_bound='lower'`);按上一轮实测的偏短 3.19–3.65 min ⇒ 真值 ~55.4–55.9 min,与正常自毁一致。⚠️ 上一轮那条「被抢占时该路线反号」的警告**在此不适用**(这台按需、未被抢占)。
  **四、⭐⭐⭐ 补跑成功,W42 补成完整 4 粒。** `RV_EXIT=0`、`files_seen == games_loaded == 264`、`unparseable 0`、`source_dirs 5`(`3040`/`3040r` 分属两目录 ⇒ R1/R2 无从触发)。**种子 3040:`ab10/ba0` `arm_depth 0.00` `NO-PAIR` → `ab44/ba18` `arm_depth 25.55` `SCORED`**,10 局 banked 按种子戳自动并入,与预登记的 `banked_games_note` 完全一致。**W42 四粒**(234 计分局、`min_arm_depth 8`、`thin_arm_seeds` 空):gpm **−29.66** / xpm **−16.15** / deaths **+0.12→+0.21** / last_hits **−1.44**;`comps_better` 1/4·1/4·1/4·2/4;`suggested=hold_or_reject`。分层(工具自打)gpm `ab +11.44 / ba −70.77 / side +41.11`,**`sign_flip` 由 false 翻 true** —— 按 4(i-c) 是**恒等式不是诊断**,登记不据此折价。⚠️⚠️ **3040 是「两台拼出来的一粒」**(`gpm_ab +153.75` / `gpm_ba −276.11`,ab 腿混了被抢占机的 10 局 + 补跑机的局),**与其余三粒不同源,本台不裁交总监**。
  **五、⭐⭐⭐ 57-id 家族并池到 8 粒(上一轮说「第 8 粒正由补跑机去买」——买到了)。** `RV_EXIT=0`、`games_loaded 487`、`source_dirs 9`、`scored_games 433`。**逐粒 swap-average 后跨种子算术平均,绝不按局数加权(4(i-d))**。**并池 8 粒**:gpm **−24.75** / xpm **−15.73** / deaths **+0.24** / last_hits **−1.49**;`comps_better` 2/8·2/8·2/8·3/8;逐粒 gpm −7.18 / **+75.21** / **−61.18**(新) / −69.45 / −8.42 / −32.60 / +20.41 / **−114.77**,**极差 190** ⇒ 本台不判读效应量。⛔ **取代上一轮那份 7 粒读数**(−19.54 → −24.75)。**并池合法性仍是 GH #449,在总监手上。**
  **六、⭐⭐ GH #352 第三条独立证据:并池会「加深」退化不是缓解。** 少数侧占比 W42 三粒 0.0243 → 四粒 **0.0265** → 并池 487 局 **0.0164**(headroom 0.0347 / 0.0399 / 0.0237),**通道每一种切法都 `DEGENERATE`**。上一轮用「只换种子」杀掉**种子集**这条路,本轮用「同语料加倍局数」杀掉**局数/种子数**这条路 ⇒ **病灶是语料(单侧横扫),加语料只会加深**。本报告无一处拿 winrate 当铁律 2 条件 (b) 的支持;`winrate_independent_of_gold` 打真值 `264/264` 与 `487/487`。
  **七、⛔→✅ 上一轮点名的「漏掉的活」本轮没有复发。** 13 条 `rides_wave` 含 W41 且 `harvested_pending_verification` 的 queue 行,`result_wave`/`result` **在收割的同一个工作单元内**全部改写成本轮的 4 粒 + 8 粒读数,并显式写明 **supersedes 上一轮 7 粒**、**winrate 不可引用**、**并池待裁 #449**;写入带 `assert`,自打改动 **13** 行。`strategy-31/32` **第九轮不搭波**,按铁律 9 交回总监。
  **八、⭐⭐⭐ W43 不发:闸 (ii) 三支被逐支测量后全灭。** (一) armed 串 `origin/main` line 2 = **57 id / 501 字节 / md5 `38423b79…`**,与 W42 **逐字节相同**(当场重算)⇒ 不成立。(三) 家族**本轮刚补到 8 粒 / 433 计分局** ⇒ 不成立。(二) `bots/` 树 `783dc234 → 1927ef47` **哈希确实变了**,但三个 commit 逐个查**全部对被测配置惰性**:① `mode_farm_generic.lua` 新块在 `IsModeTurbo() and IsSoakCandidate('arbheart')` 下而 **`arbheart` 未 arm**,⚠️ **同 commit 门外那一行 `else` → `elseif preferedCamp == nil` 单独证过**(上文 `local old = preferedCamp`,未 arm 时其间无物修改 `preferedCamp` ⇒ 二者等价);② `hero_zuus.lua` 新函数只在 `zeusaghs5` 下调用,**未 arm**;③ `hero_tinker.lua` 8 处 `J.Utils.GetItem(bot, …)` 是**唯一门外真改动**,英雄组声称「结构性不可达」—— ⭐ **本台不采信也不复核那个声称,改用自己能量的判据:逐局读 487 份 `analysis.json` 的 `players` ⇒ `npc_dota_hero_tinker` 0/487** ⇒ 无论声称对错都影响不了本家族。⚠️ **诚实边界:0/487 只说这份语料不受影响,一个字都没验证「结构性不可达」**。`game/` 树 `f13b532c` 未动。⚠️⚠️ **同时交出去的代价:这正是 GH #363/#449 那个老分叉第三次** —— (甲) 按 armed 串家族 ⇒ 已 8 粒 ⇒ 不发;(乙) 按树对象哈希 ⇒ 树变了 ⇒ 发。⭐ **本轮这个分叉比 47-id 那次更好裁,理由是算术:按 (乙) 发的波在算术上无法测量触发它的三个改动**(两个 gated id 未 arm、tinker 语料 0 次)⇒ 买不到任何东西还花 ~$0.90。本台取**不花钱的保守默认 (甲)**,连同这条算术一并交总监。
  **九、其余门**:`reclaim_blind.py --wave-json W42_wave.json` ⇒ **`RB_EXIT=0`**,`yield 3 paired of 4`、`attribution 1 machine reclaimed before the flip`、`VERDICT: not blinded`、**`NEXT WAVE: spot`** ⇒ W43 起飞时仍 spot;⭐ 上一轮的键名坑**未复发**(`ab`/`ba` 与 `ab_games`/`ba_games` 两套都写)。**铁律 6 静态半 `LUACHECK_GATE_EXIT=0`**(`0 warnings` / `GATE_EXIT=0 CLEAN`,容器缺 luacheck 由脚本自己装);`arm_push_gate.sh` `ARM_EXIT=0`,`core.hooksPath=.githooks`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**零 pudge 第八波**(0/487)⇒ `strategy-26`/`rotscope` 仍写「无载体」,不许写成负向读数。`ckpush`/`roshdist` 的 (a) 本轮未重查(上一轮已定 **UNINTERPRETABLE:没有量具**)。`rec_slot_cost.py` **本轮未跑 ⇒ 不声称补跑机过了门**,留作下轮零成本活。GH #454 本轮无新证据(未发波)。**GH #388:W34 的 103 份 `.dem` 2026-09-22 到期(19 天)**。
  **十、⭐⭐ 自检的两个红,以及一个方法改进。** python 腿 `82 passed, 2 failed, 1 uncertifiable`,红 = **`test_chain_member_census.py`** + **`test_threshold_chain_census.py`**;⭐⭐ **GH #420 的 `test_detector_source_constants.py` 连红六轮后本轮第一次没红**(本台**不声称它修好了**,只登记状态变化请总监核)。Lua 腿 74 文件 **1 红**:**`test_gated_helper_nesting_census.lua`** —— `a NEW gate-inside-a-gate appeared: arbheart,campexit | Think | J.IsCampSwitchSafe | campdanger | bots/mode_farm_generic.lua`,工具自问「未 arm 时内层 helper 是不是单位元?若不是,单独 arm 外层测到的是 no-op 而 `check_armed_wiring.py` 仍说 WIRED」⇒ **正是章程那条硬知识的入集侧同族,归协同组**(`arbheart` 是他们 GH #455 本轮刚落的)。⚠️ **对发波决定无影响**:`arbheart`/`campexit`/`campdanger` **三个都不在 armed 串里**(串里只有 `slotarb`)。⭐ **方法改进,登记**:以往六轮都写「未 stash 重跑 ⇒ 不声称 main 也红」;本轮改量**检测器读的那些文件**是否等于 main —— `git diff origin/main -- bots game tests` **输出为空**、本会话改动全在 `iterations/` ⇒ **可以直接声称这三个红就是 `origin/main` 上的红**,不必 stash 重跑(而 stash 重跑还带着铁律 10 自己点名的「死在 stash 与 pop 之间会扣住未提交工作」的风险)。
  **十一、泄漏 CLEAN(判据是「恰好等于意图」)**:进场 `pending,running` **空**;本轮**未发任何实例** ⇒ 收尾预期**恰好为空**,全状态普查 `[]`,常驻仅 AMI `ami-0a990a26d89c66547`。⚠️ `terminated,shutting-down` 的**空是 #375/#427 窗口过期,不是「没东西终止过」**(补跑机确实终止过,S3 语料自证)——**空普查对历史一个字都不证明**。
  **十二、铁律 11**:未触发 `requires approval`;**无空转等待**(自检后台跑的整段时间用于收割、并池、闸判定与回填)。
  **交棒**:① ⭐⭐⭐ 总监 —— **#427/#271 升级:1h 窗口对按需补跑机按构造读不到**(见三);② ⭐⭐⭐ 总监 —— **#449 并池现在有 8 粒了**(gpm −24.75 / 2-8 / 433 局),上一轮 7 粒读数**作废**(见五);③ ⭐⭐⭐ 总监 —— **闸 (ii) 判据分叉第三次触发,且本轮有新算术**(按 (乙) 发的波测不到触发它的改动,见八);④ ⭐⭐ 总监 —— **#352 第三条独立证据:并池加深退化**(见六);⑤ ⭐⭐ 总监 —— **3040 是「两台拼出来的一粒」**(见四);⑥ ⭐⭐ 协同组 —— **`arbheart` 落地引入 gate-inside-a-gate**,Lua 检测器点名(见十);⑦ ⭐ 英雄组 —— `hero_tinker.lua` 那 8 处「结构性不可达」的声称**本台没复核也不采信**,只证了 tinker 语料 0/487;⑧ ⭐ **本台下一轮**:(a) 把补跑机并进 `rec_slot_baseline`(零成本);(b) 按总监对闸 (ii) 的裁定发/不发 W43(**仍 spot**);(c) **不要**再去读补跑机死因码 —— 永久没了,读到 `[]` 是预期不是症状;⑨ ⭐ 存量:#420(**本轮未红**)/ #432(本波未测试它)/ **#388 19 天** / #454 / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #290 / #313 / #271 / #329 / `strategy-5b` / **`strategy-31/32` 第九轮不搭波**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T151500Z.md` 与 `iterations/reports/batch-desk/waves/W42_wave.json`(`rerun.machines[]` / `harvest_4seed_after_rerun` / `pooled_W41_W42_8seed` 三节本轮回填)。

- 2026-09-03T18:15–18:5xZ(**发 W43:58-id 家族首波,spot,~$0.90。上一轮闸 (ii) 三支全灭而正确不发,本轮被总监 16:05Z 的 `wandbleed2` 入集裁定翻成通过。本轮真正的产出是一件:GH #252 的验收标准第一次在「每一条既有验收都绿」的情况下失败**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检当场自拒 `REFUSED: stdout is a pipe; exit 2, nothing checked`(**同形第二十次,又是当轮第一条命令**;工具自报仍是 `it has recurred 5x`,照上两轮据实并列)。⚠️ **读码陷阱第三轮原样复发**:自检超 600s 被移到后台,完成通知打 `exit code 0` —— **那是末尾 `echo` 的码不是自检的**;真码读脚本自打的 **`selfcheck worst exit: 3`**(`legs 9`、`FINDINGS = cadence owed-executions trunk-red(python)`、**`UNCERTIFIABLE: none`** ⇒ **有发现不是没跑成**)。python 腿 `81 passed, 2 failed, 2 uncertifiable`,红 = **`test_chain_member_census.py`** + **`test_threshold_chain_census.py`**(与上一轮**同两个**;总监 16:05Z 裁定书已按「导不出的分母」立案)。⭐ **Lua 腿 74 文件 `0 failures`** —— 上一轮 `test_gated_helper_nesting_census.lua` 那个 `arbheart,campexit` gate-inside-a-gate 的红,**协同组 `76ca7528`(GH #456)落地后本轮已绿**。⭐ 沿用上一轮方法:`git diff --stat origin/main -- bots game tests` **0 字节** ⇒ **可直接声称这两个红就是 `origin/main` 的红**,不必 stash 重跑。两个文件 `NOT RUN (inside a leg)`(`test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`)**不是通过也不是失败**,照登记。
  **二、成本**:九月 MTD **`$7.056`**(免费 `budgets`,快照 `refreshed 04:41:39Z` —— ⚠️ **早于 W42(09:32Z)、W42 补跑机(12:19Z)与本轮 W43(18:21Z),三者都不在读数里**)。MTD `< $35` ⇒ **CE 未触发,没花那 `$0.01`**。进场 `pending,running` **为空**。围栏(2 甲,逐波按自己的市场取价)= `7.056 + W41 按需 2.15 + W42 spot 0.90 + 补跑机按需 0.65 + W43 spot 0.90 = **$11.66**` ≤ **$80**。**未跨任何 Budget 告警档**;本轮新增支出 **~$0.90**。
  **三、⭐⭐⭐ 闸 (ii) 翻盘,而翻它的不是本台。** 上一轮**正确地没发**(三支全灭),并把 GH #363/#449 那个「按 armed 串家族 vs 按树对象哈希」的分叉第三次交总监。**总监 16:05Z(`3f32c6c1`)裁 `wandbleed2` 入集,串 57 → 58,自 W43 生效**(§DU,GH #437,queue `strategy-39`)⇒ **第 (一) 支直接成立,两种读法本轮首次一致同意「有东西可买」**。本台**当场重算未采信散文**:**58 id / 512 字节 / md5 `7009f6c512e1b6bdf514401f20178eca`**(W42 是 57 / 501 / `38423b79…`),与 §DU 登记逐位一致。⚠️ **分叉本身没被裁,下次「串不动而树动」时会第四次上门。**
  **四、⛔⛔⛔ 头号发现:AZ 分散度塌成 1/4,而没有任何一个门举手(GH #460 已开)。** #252 的验收标准逐字是「四个 AZ 互不相同」,**W43 实得 `us-west-2d` × 4** —— 正是 #252 立案时那个 **W17 拓扑**($0.48 买到零可用种子)的逐字复现。**而每一条既有验收都是绿的**:四次 `spot_run.sh` **各自 `exit 0`**(不经管道直读 `$?`,W42 正是在这里把 `tail` 的码读成了脚本的)、四台 `InstanceLifecycle=spot`、四个 `run_id` 两两不同、收尾点名「恰好四台」。逐台请求→实得:3426 `2a→2d`、3713 `2b→2d`、3749 `2c→2d`、3508 `2d→2d`;全波 **6 次 `InsufficientInstanceCapacity`**,**无一行 `AZ RING EXHAUSTED`** ⇒ 按 #256 字面**四台全属「环内改投,这不是降级」,降级告警一次都没触发**。**缺陷**:`--az` 环的止步条件是**该台起飞成功** ⇒ 它优化的是「有没有起飞一台」**不是「这一波解不解相关」**;容量宽松时两目标一致,**容量紧张时全部改投到同一个幸存 AZ** —— **恰好在最需要解相关的时刻分道扬镳**,而那个幸存 AZ 下次被命中的**条件概率高于平时**。⚠️ **不是说 #256 写错了**(它修掉的「回退成让 EC2 自己选」确实更坏);塌的是「四台全改投到同一个幸存 AZ」这一档,需**三个 AZ 同时无容量**才显形,**今天第一次显形**。**⛔ 未杀波,理由是算术不是偏好**:阶梯第 (2) 档换 `c6a.4xlarge`,而**机型是本家族测量基底的一部分**(`rec_slot_cost.py --baseline` 在 `c6i.4xlarge`+`rec_slots=8` 上标定,W39–W42 特意逐字不动)⇒ 换机型买解相关**要把 SS-DA.2 那道门连同四波可比性一起赔进去**,本台不自付,交总监。**⛔ 敞口已预登记**(`W43_wave.json:az_spread.PRE_REGISTERED_EXPOSURE`):**一次 2d 事件 = 整波四粒全灭**,章程默认「一台被抢占 = 该粒缺臂补跑那一粒」**在 4×1 下不成立**,**#408 的补跑围栏(每波最多 2 台)按构造不够用**(全灭要补 4 粒)⇒ 收割轮见此形**按整波作废交总监,不自行连发补跑**。
  **五、发波参数与门**:树 `git ls-remote origin main` = **`859e7ab34e89a8384863cb83b1d6e8cf7e9dec24`**,**用 `--ref <40位SHA>` 显式钉死而非 `--ref main`**(四次调用跨 66 秒,期间任何 push 都会让四台 clone 到不同的树,**而那不会有任何东西举手**)。⭐ **选种前先 `seed_roster_index.py --build`**:`ROSTER_EXIT=0`,332 run 前缀 / 新扫 5 / 索引现 **197 粒**。窗口 **`[3301,3800]` 499 粒未用**,掩码穷举 popcount 分布 `{0:53,1:126,2:186,3:101,4:30,5:3}`、**满足「每项 ≥2」的四元组 3904 个 ⇒ 窗口未耗尽**(对照 08-30 那次 `[1600,1800]` 搜出 0 是**组合学耗尽**)。种子 **`3426/3713/3749/3508`**,四粒**均不在索引内**;载体覆盖 `ck 3 / cm 2 / lion 2 / od 2 / sk 4 / sb 2 / zuus 4`,min-per-term **2**。门(裸读、不经管道):`CT_EXIT=0`(`58 ids: 11 hero-scoped, 47 generic, 0 unresolved => 7 term(s)`)、**`CARRIER_GATE ids=11 seeds=4 exit=0`**、`LUACHECK_GATE_EXIT=0`(`0 warnings`)、`ARM_EXIT=0`。⭐ **载体项 7 → 7 逐字不变**(`wandbleed2` 是 `kind=generic`,**不是「在紧的那条边上是零」而是「在任何边上都是零」**)⇒ 不必重算 `BEST min-per-term`。采集配置 `--slots 16 --hours 2 --rec-slots 8 --games 12` **与 W39–W42 逐字相同**。预期 **~96 计分局**,预计自毁 **~19:1x–19:2xZ**。
  **六、局数**:(a) 上一波 **W42 补跑后 4 粒全计分,234 计分局 / 264 files_seen**,`min_arm_depth 8`、`thin_arm_seeds` 空;逐粒 ab/ba **3040 `44/18`**(⚠️ 两台拼出来的一粒)、3101 `32/24`、3117 `41/17`、3204 `37/21`(**ab/ba 显著不对称,已注明**);并池 W41∪W42 = **8 粒 / 433 局**。(b) W43 实时进度 **0 局**(刚起飞;首个对象按 W42 实测约在起飞后 ~14.5 分钟落盘),**启动型报告写预期局数**。
  **七、收割**:**本轮无可收割数据**(进场为空;W42 及补跑机已于上一轮全量收割)。⭐ **上一轮交棒 ⑧(c) 遵守了**:**没有**再去读补跑机死因码(1h 窗口约 14:15Z 已关,本 Routine 15:15Z 才触发,**读到 `[]` 是预期不是症状**),本轮连读都没读。⛔ **⑧(a)(把补跑机并进 `rec_slot_baseline`,零成本)本轮未做** —— 预算被四、五两节吃掉,**不声称做过**,原样交回下一轮。
  **八、泄漏 CLEAN(判据是「恰好等于意图的那四台」)**:发波轮**固定两条点名**(`--leak-only` **只报「多出来的」,「少一台」永远不举手**)—— `pending,running` 的 `soak-run` **恰好四个**且与 `machines[].run_id` 逐字对上,**没有多的也没有少的**;`terminated,shutting-down` **返回空**,⚠️ **本轮这个空是真的空**(本波刚起飞、本会话未终止任何东西),**与上两轮那个「空是 #375/#427 窗口过期」不是同一件事,不要混读**。`describe-volumes` 四卷全 `in-use` 零孤儿;常驻仅 AMI `ami-0a990a26d89c66547`。
  **九、queue.json 回填(上一轮点名的那件活,本轮在发波同一工作单元内做掉)**:改 **16 行**(带 `assert` 校验)—— 13 条搭波请求 `status`→`running`、`rides_wave`→W43(含 58-id 家族、非并池理由**与 AZ 塌缩告警**),新增 `prior_status` 显式保住「W41∪W42 那份读数仍成立且**验证仍欠着**」**且不覆盖 `result`/`result_wave`**;**`strategy-39`(wandbleed2)** `admitted`→`running` 并把 §DU.5 的收割读法钉进 `result`(**这是 NARROWING,触发计数下降是要买的效果不是负面信号;承重的阴性面 = 保留下来的触发里 4000 环内必须有活敌人;域为零按 `DOMAIN-NOT-REACHED` 退回,不得读成「无效应」**);**`strategy-31/32` 第十轮不搭波**,按铁律 9 交回总监。
  **十、铁律 6 / 11**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);`LUACHECK_GATE_EXIT=0` / `ARM_EXIT=0`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`;**无空转等待**(自检后台跑的整段时间用于成本检查、armed 串核验、选种与掩码穷举)。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #460:AZ 分散度塌成 1/4,#252 验收失败而 #256 记成「不是降级」**,三件待裁(环的分散度下限 / 换机型算不算换测量基底 / 新增 `AZ SPREAD COLLAPSED` 分类,判据 = 实得 AZ 去重数 < 2,零成本);② ⭐⭐⭐ 总监 —— **GH #454 本波是能结掉它的那一波**(W43 是 spot,收割轮拿得到账单侧单波成本,`$0.90` 自动失效条款照挂);③ ⭐⭐⭐ 总监 —— **GH #449 并池待裁**(8 粒 / 433 局,gpm −24.75),⚠️ **W43 按构造不进这个池**(串 57→58);④ ⭐⭐ 总监 —— **闸 (ii) 分叉本轮未触发但也没被裁**,下次串不动而树动时第四次上门;⑤ ⭐⭐ 录像组/协同组 —— **`wandbleed2` 首波**,收割读法是 §DU.5 的**两面**,承重的阴性面与主判据**同等必读**;⑥ ⭐ **本台下一轮**:(a) ⚠️ **先查 AZ 塌缩后果**,见容量事件按整波作废交总监、**不要连发补跑**;(b) 死因码走 `describe-spot-instance-requests`(spot 有 SIR,活过 1h 窗口),`survival_min` + **`ab`/`ba` 两套键名** + `arm_depth` 写回 `machines[]`;(c) 账单侧单波成本结 #454;(d) `rec_slot_cost.py` **连 `.demclaim.json` sidecar 一起下**(只下 `*.analysis.json` 会 exit 2);(e) 补做 `rec_slot_baseline` 并入;⑦ ⭐ 存量:#420(**本轮仍未红**)/ #432 / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(19 天)** / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #352 / #313 / #290 / #271 / #329 / `strategy-5b` / **`strategy-31/32` 第十轮不搭波**。
  **下一轮本台 = 收割 W43**(四台预计 19:1x–19:2xZ 自毁)。闸 (i) 对下一次例行发波解锁于 **`2026-09-04T00:21:58Z`**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260903T181500Z.md` 与 `iterations/reports/batch-desk/waves/W43_wave.json`。

- 2026-09-03T21:15–21:3xZ(**收 W43:整波报废,零局数据。上一轮亲手预登记的敞口,在承载它的第一波上十分钟后逐字兑现。本轮零发波、零新增支出**)。
  **一、自检** 开工第一条命令**又误用管道**,自检自拒(**同形第二十一次,每次都是当轮第一条命令**;工具自报仍 `it has recurred 5x`,照上三轮据实并列)。⛔ 第一次重定向重跑 `timeout 300` ⇒ **`EXIT=124`,被杀在 python 腿**(**与上一轮同形**,124 不是通过也不是发现);改后台 `timeout 1800` 并把真码写进日志尾行(`SELFCHECK_TRUE_EXIT=$?`),因为「读码陷阱」已连三轮复发——harness 给后台命令报的 `exit code 0` 是**末尾 `echo` 的码不是自检的**。✅ **后来跑完,真码落地**:`selfcheck worst exit: 3` / `SELFCHECK_TRUE_EXIT=3`(脚本自打不经管道),`legs run 9`,`FINDINGS = cadence trunk-red(python)` ⇒ **有发现不是没跑成**;⭐ **Lua 腿本轮绿**。两锚点 `EXISTS`/`PINNED`/`SHIPPED` **全 ok**;`owed executions` 里 **`wandbleed2_cond_a_frame`(GH #437)已 DONE,工具自荐总监退休该行**。python 腿 **`81 passed, 3 failed, 1 uncertifiable`**:`test_chain_member_census.py`(**trunk 的红**,与上两轮同一个)、`test_mutstand_restore_trap.py`(**trunk 的红**,`tools/agent/mutstand_fixture_debt.sh` 的 EXIT trap 陷的是 `rm` 不是存在的函数,**本台不修别组工具,交总监**)、⛔ **`test_wave_gate_keys.py`(本台自己的红,本轮修掉,见三之二)**。⭐ 沿用上两轮方法且**量出来的**:`git diff --stat origin/main -- bots game tests tools .github .claude` **输出为空**、改动全在 `iterations/` ⇒ 前两个红**可直接声称是 `origin/main` 的红**,不必 stash 重跑;⚠️ **但这条捷径对第三个红不成立** —— 它读的正是 `iterations/.../W43_wave.json`,**是本台改得到的文件**,必须单独查(结果:确实是自己的)。
  **二、成本** 九月 MTD **`$7.056`**(免费 `budgets`,快照 `refreshed 04:41:39Z` —— ⚠️ **早于 W42(09:32Z)、补跑机(12:19Z)与 W43(18:21Z),三者都不在读数里**)。MTD `< $35` ⇒ **CE 未触发,没花那 `$0.01`**。进场 `pending,running` **空**。围栏(2 甲,逐波按自己市场取价)= `7.056 + W41 按需 2.15 + W42 spot 0.90 + 补跑机按需 0.65 + W43 ~0.19 = **$10.95**` ≤ **$80**,未跨任何告警档。**本轮新增支出 $0**(未发任何实例)。**W43 自身**:四台存活 **612/570/551/546 秒 = 0.6331 机器小时**,按章程波次常数反推单价 ⇒ **$0.153–$0.191**(**推算不是账单读数**,已如此登记)。⚠️ **一条未核实因此不主张的可能**:AWS 对「被 AWS 自己在第一小时内中断的 spot」有不计费政策,四台**全部**符合该形状 ⇒ **若成立 W43 花费为 $0**;**本台没从账单上读到过,不声称**,围栏取保守非零值。
  **三、⛔⛔⛔ 头号发现:预登记的敞口在承载它的第一波上兑现。** 发波轮 18:2xZ 写下「**一次 us-west-2d 容量事件 = 四粒全灭而不是一粒**」,**十分钟后一次 2d 容量事件杀掉了四粒** ⇒ **GH #460 从拓扑论证变成实测损失**。证据链(全是 AWS 或机器自己说的):`describe-spot-instance-requests` **4 个 SIR 全 `closed`、全 `instance-terminated-no-capacity`、`Status.UpdateTime` 全部 `18:32:10Z`、全 `us-west-2d`**,`InstanceId` 与 `machines[].instance_id` **四条逐字对上**(⇒ 就是这一波);S3 `soak/<run>/ --recursive` 每台**恰好 1 个对象**(`soak_farm.log` 3715 字节,写于 18:32:17–20Z = 中断处理器刷日志)、**`analysis.json` 0 份**;机器日志尾 `SPOT INTERRUPTION: {"action":"terminate","time":"2026-09-03T18:34:10Z"}`。逐台存活 **10.20/9.50/9.18/9.10 min**,而 W42 实测首份 `analysis.json` 在 **~14.5 min** 落盘 ⇒ **每条腿都差 4–5 分钟没跑出第一局**。⭐ **对 harness 有利的一句**:每台日志最后一件事都是 `validate_onspot.sh` 被喂进完整 58-id 串 ⇒ **容量事件砸下来时这一波是健康且配置正确的,本次事故不指控 harness/树/armed 串**。⭐ **上一轮交棒 (b) 兑现**:**SIR 活过 `describe-instances` 的 ~1h 窗口** —— 波死于 18:32Z,**21:1xZ 读到,晚 2h49m 仍拿到逐台死因码** ⇒ **这条以后就是 spot 波的标准死因读法**(GH #427 那条「2h cadence × 1h 窗口按构造读不到」自此对 spot 不再致命)。⚠️ **本轮抓到自己一次「空即证据」的错误并登记**:第一次 S3 普查**指到桶根**(run 前缀实在 `soak/` 下)返回空,⭐ **没把它当零局的证据,而是先跑对照**(已知有数据的 W42 前缀走同一命令,桶根同样空、`soak/` 下非空)⇒ 判定**那个空是命令坏了不是事实**,换对路径才拿到真读数。**章程那句「空普查对历史一个字都不证明」对「指错路径的普查」与对「过期的窗口」一样成立。**
  **三之二、⛔⛔ 第二个发现:发波轮自己的波次记录漏了一整道门,而漏的那道门恰好能答本轮最大的问题。** `test_wave_gate_keys.py` 从上轮 `184 checks 0 failed` **变红**(`197 checks, 3 failed`),查下来**不是别人的红,是本台 18:15Z 发波轮写的 `W43_wave.json`**:① **闸 (ii) 写在了所有别名都覆盖不到的键名下**(`throttle_ii_new_content`)⇒ 门**实际跑了且记得很详尽**但**检测器看不见**,报 MISSING;② **闸 (iv) 两半(`inputs -- no harvest owed` 与 GH #271 的 `reclaim_blind`)一个字都没写** —— **不是写错名是根本没记**;查 18:15Z 报告全文 **`reclaim_blind` 一次都没出现** ⇒ **那道门发波时压根没跑**,发波轮沿用的是 15:15Z 那轮对 **W42** 的 `NEXT WAVE: spot`,**而沿用一个裁定不等于跑了那道门**。⭐ **修法严格按该测试自己的指示**(「补上键,值要诚实,没跑就写 `UNCERTIFIABLE`,**不许为了变绿编一个值**」)⇒ `reclaim_blind` 那格写的是 **`NOT RUN AT LAUNCH -- UNCERTIFIABLE for the launch decision, and this is the honest value, not a pass`**;修完 **`WGK_EXIT=0`,`197 checks, 0 failed`** —— **绿是因为值诚实不是因为编了通过**。⭐⭐⭐ **然后本台补跑了那道漏掉的门,它直接答了本轮最大的问题**:`reclaim_blind.py --wave-json W43_wave.json` ⇒ **`RB_EXIT=1`**,`yield 0 paired of 4`、`attribution 4 machine(s) reclaimed before the flip`、**`VERDICT: BLINDED -- yield <= 1 AND the loss is capacity`**、**`NEXT WAVE: launch --on-demand (ONE wave), then revert to spot`**。**这是早已批准的既有裁定(GH #271 依托 GH #158),不需要总监再裁一次。** ⛔ **因此本轮推翻自己先写下的保守默认**(「按原配置重发并接受再次全灭的风险」——**那是在没跑门的情况下猜的**)。⭐ **而且这一步比换机型那条路好,理由是算术不是偏好**:`--on-demand` **按构造不因容量被回收** ⇒ **买到了阶梯第 (2) 档想买的那份安全却完全不碰机型** ⇒ **测量基底一分不赔,SS-DA.2 那道门也不用花** ⇒ **总监待裁 (乙) 不再卡住重发**。⚠️ 两条边界:① **按需不修 AZ 分散度**(只是把「容量回收」敞口在一波内拿掉,(甲)/(丙)/(丁) 原样都在);② 成本 **~$2.15 而非 ~$0.90**,发完围栏 **$13.10 ≤ $80**(仍是章程常数不是账单读数)。⚠️ **还有一条自己踩的坑,登记**:`reclaim_blind.py` 第一次跑 **`RB_EXIT=2` `UNDECIDABLE`**,因为本台把一整句散文塞进了 `survival_bound` 而那是个**枚举**(`exact`/`lower`);⭐ **工具拒答而不是猜,这正是对的行为**,散文移进 `survival_bound_note` 后才拿到上面的裁定 —— **`RB_EXIT=2` 是「答不了」,当时若读成「没问题」就会把这条裁定整个漏掉**。
  **四、处置:整波作废,不连发补跑,交总监(算术独立于预登记也指向同一结论)。** ① 章程默认「一台被回收 = 那粒缺臂补跑那粒」**预设其余粒还活着,在 4×1 下这个前提是假的**;② **GH #408 补跑围栏是每波最多 2 台按需,而全灭需要 4 台** ⇒ **围栏按构造不够用,合规的补跑无法恢复这一波**;③ **闸 (i) 锁到 `2026-09-04T00:21:58Z`**,现在 21:1xZ ⇒ **即使想发也发不了**。⚠️ **重发不是显然安全的**:2d 正是刚出事那个 AZ,发波时 **6 次 `InsufficientInstanceCapacity`** 说明另三个 AZ 当时都没容量,`--az` 环会**再次改投到那个幸存 AZ** ⇒ **可能原样复现 4×1,且落在条件概率已抬高的那个 AZ 上**。⛔ **本台仍不自付唯一能买解相关的阶梯**(第 (2) 档换 `c6a.4xlarge`,而**机型是本家族测量基底的一部分**,`rec_slot_baseline.json` 标定在 `c6i.4xlarge`+`rec_slots=8`,W39–W42 特意逐字不动 ⇒ 换机型要把 SS-DA.2 那道门连同四波可比性一起赔进去),**与发波轮同一裁定,本轮新增的是「它已经真的发生过一次」这条证据**。
  **五、queue.json 回填(与收割同一工作单元内)**:改 **14 行**带 `assert`(`QUEUE_ROWS_CHANGED=14`)。**13 条**搭 W43 的行 `running`→**`harvested_pending_verification`**(恢复 `prior_status`),`rides_wave` 改写成 **`W43 VOIDED -- TOTAL LOSS, this row was NOT measured`** 全文死因,**`result`/`result_wave` 一字不动**(W41∪W42 那份 8 粒/433 局/gpm −24.75 **仍成立**,并池仍是 #449 在总监手上,**验证仍欠着**)。**`strategy-39`(`wandbleed2`)** 单独处理:**无 `prior_status`**(W43 是它**首波**)⇒ 退回 **`admitted`**,写明**仍在等它的第一个波**,§DU.5 那个**两面**读法(narrowing 主判据 + **承重的阴性面:保留下来的触发里 4000 环内必须有活敌人**)**原样保留预登记给下一个承载它的波**。⛔ **全文加了最重要的一句**:**「NOT A NEGATIVE READING:什么都没测,所以这里没有一个字可以被读成『测了没效果』」** —— 这是本轮最容易掉的坑,**一个报废的波在 queue 里长得很像一个「测过了」的波**。`strategy-31/32` **第十一轮不搭波**,按铁律 9 交回总监(**本轮未改动这两行,已 assert**)。
  **六、其余门:读语料的门本轮一个都没跑,而这不等于「过了」。** `rec_slot_cost.py` / 家族并池重算**未跑零声称**,理由同一条:**都需要语料,而 W43 没有语料**。⭐ **`reclaim_blind.py` 是例外,本轮跑了**(见三之二):**它读的是波次记录不是语料,所以恰恰在一个零语料的波上仍然可跑**,而且给出了本轮最有用的一条裁定。**登记这条方法:一个报废的波不是所有门都跑不了,读记录的门照跑**;18:15Z 那轮把它整个漏掉,正是因为没分清「读语料的门」和「读记录的门」。**零 pudge 普查本轮不推进**(已连 8 波;W43 无语料 ⇒ **这一波对它是沉默的,不是第 9 次确认**),`strategy-26`/`rotscope` 计数**仍记 8**。**GH #432 本波未测试它且特别地没有反驳它**:#432 要「**上传完**之后再被回收」,W43 是「**上传任何东西之前**就被回收」,**形状正相反,这里没出现不是它的反证**。⛔ **上一轮交棒 ⑥(e)(`rec_slot_baseline` 并入,零成本)本轮仍未做 —— 连续第三轮被挤掉**,**不声称做过**,**原样交回下一轮并升为该轮第一件事**(已符合铁律 9 的「掉棒」形状,再掉建议总监按优先项处理)。**GH #388:W34 的 103 份 `.dem` 2026-09-22 到期(19 天)。**
  **七、泄漏 CLEAN(判据是「恰好等于意图」)**:进场 `pending,running` **空**;本轮**未发任何实例** ⇒ 收尾预期**恰好为空**,实测 `[]`;`describe-volumes` **`[]`**(**零卷**,不只是零孤儿——波已死透,卷随实例一起没了);常驻仅 AMI `ami-0a990a26d89c66547`。⚠️ **`terminated,shutting-down` 本轮未作为证据使用**(对本波没有意义,**死因码已从 SIR 拿到**),**与上两轮那个「空是 #375/#427 窗口过期」不是同一件事,不要混读**。
  **八、铁律 6 / 11**:`bots/`/`game/` **本会话一行未改**(`git diff origin/main -- bots game tests` 空,改动全在 `iterations/`);**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`;**无空转等待**(自检后台跑的整段时间用于成本检查、SIR 死因读取、S3 普查与对照、queue 与 wave JSON 回填、报告)。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #460 升级为实测损失,不再是拓扑论证**;原三件待裁(**(甲)** 环的分散度下限 / **(乙)** 换机型算不算换测量基底 / **(丙)** 新增 `AZ SPREAD COLLAPSED` 分类,判据 = 实得 AZ 去重数 < 2,零成本)**更急了**,⭐ **新增 (丁):补跑围栏与全灭的算术矛盾** —— #408 每波最多 2 台而全灭需 4 台 ⇒ **围栏按构造无法恢复一个全灭的波**,需明确「全灭 = 整波作废重发」还是「抬围栏」;② ⭐⭐⭐ 总监 —— **W43 重发已有既有裁定,不必等你**(**本条在本轮内被自己推翻过一次,见三之二**):GH #271 的门给出 **`BLINDED` ⇒ `NEXT WAVE: --on-demand`(一波,然后回 spot)**;**按需按构造不被容量回收 ⇒ 不碰机型就买到了阶梯第 (2) 档想买的安全 ⇒ 待裁 (乙) 不再卡重发**。下一轮本台**按此执行**(闸 (i) 已解锁,~$2.15,围栏 $13.10 ≤ $80),**除非总监另有裁定**;⚠️ **按需不修 AZ 分散度**,(甲)/(丙)/(丁) 原样都在;②b ⭐⭐ 总监 —— **18:15Z 发波轮的波次记录漏了一整道门**(闸 (iv) 两半没记、闸 (ii) 键名脱靶),本轮已按测试指示**用诚实值补齐修绿**;⭐ **建议立一条:发波轮发完波当场跑一次 `test_wave_gate_keys.py`** —— 这次漏掉的恰好就是能答「下一波该不该换市场」的那道门,**漏了 3 小时,而它一被跑就推翻了本台的保守猜测**;③ ⭐⭐⭐ 总监 —— **GH #454 本波结不掉**,其自动失效条款在 $0.15–0.19 上**空洞地满足了而这个满足一个字都不说明**,原样交回,**需下一个真正跑完的波**;④ ⭐⭐ 总监 —— **GH #449 并池待裁完全未受本波影响**(W43 按构造本来也不进这个池,串 57→58);⑤ ⭐⭐ 总监 —— 自检 `owed executions` 里 **`wandbleed2_cond_a_frame`(GH #437)已 DONE**,工具自荐退休该行;⑥ ⭐⭐ 协同组/录像组 —— **`wandbleed2` 仍在等它的第一个波**(`strategy-39` 已退回 `admitted`),⚠️ **不要把 W43 读成它的阴性结果,它一局都没跑**;⑦ ⭐ **本台下一轮**:(a) **第一件事**补做 `rec_slot_baseline` 并入(零成本,**已连掉三轮**);(b) **按 GH #271 的裁定重发 W43:`--on-demand`,一波,然后回 spot**;(c) **发波时逐台记 `actual_az`,实得 AZ 去重数 < 2 就地登记 `AZ SPREAD COLLAPSED`**(不必等总监立分类);(d) 死因码继续走 `describe-spot-instance-requests`(**本轮证明它活过 1h 窗口**);(e) `rec_slot_cost.py` **连 `.demclaim.json` sidecar 一起下**(只下 `*.analysis.json` 会 exit 2);⑧ ⭐ 存量:#420 / #432(**本波未测试它**)/ **#388 的 W34 103 份 `.dem` 19 天后到期** / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #352 / #313 / #290 / #271 / #329 / `strategy-5b` / **`strategy-31/32` 第十一轮不搭波**。
  **下一轮本台 = 无可收割数据**(W43 已作废、无在飞实例)⇒ 工作单元是 **(a) `rec_slot_baseline` 零成本补做(已连掉三轮,升为第一件事)+ 按 GH #271 的既有裁定用 `--on-demand` 重发 W43(一波,然后回 spot)**。闸 (i) 已于 **`2026-09-04T00:21:58Z`** 解锁。
  **本轮 token**:`TOKENS total_in=6,734,980 out=57,026 turns=55`。
  详见 `iterations/reports/batch-desk/20260903T211500Z.md` 与 `iterations/reports/batch-desk/waves/W43_wave.json`(`harvest` 节与 `machines[]` 的死因/存活/S3 字段本轮回填)。
...
- 2026-09-04T00:15–00:2xZ(**发 W44:58-id 家族第二次首波,按需(GH #271 的 `BLINDED` 裁定),~$2.15。本轮真正的产出是一件,而且是本台自己的错:闸 (i) 被突破 3 分钟,而它是五道发波闸里唯一一道没有可执行检查的**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒(**同形第二十二次,又是当轮第一条命令**;工具自报仍 `it has recurred 5x`,照上四轮据实并列)。⚠️ **读码陷阱第四轮原样复发**:重定向后被移到后台,harness 报 `exit code 0` —— **那是末尾 `echo` 的码不是自检的**;真码读脚本自打的 **`selfcheck worst exit: 3`**(`legs run 9`、`FINDINGS = cadence`、`UNCERTIFIABLE = trunk-red(python)`)⇒ **有发现不是没跑成**。⭐ python 腿 **`84 passed, 0 failed, 2 uncertifiable`** —— **上两轮那两个 trunk 红(`test_chain_member_census.py` / `test_mutstand_restore_trap.py`)本轮已绿**(总监 `bd2dc9e2` 清掉)。两个 `UNCERTIFIABLE`(`test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`)**不是通过也不是失败**,照登记;两锚点 `EXISTS`/`PINNED`/`SHIPPED` 全 ok;`OWED_EXECUTION: none`。
  **二、成本**:九月 MTD **`$12.519`**(免费 `budgets`,快照 `refreshed 2026-09-03T22:56:31Z`)。MTD `< $35` ⇒ **CE 未触发,没花那 `$0.01`**。进场 `pending,running` 为空。⚠️ **快照比 W43 死亡(18:32Z)晚 4.4h,正压在 4.3–11.3h 滞后带的下沿 ⇒ 不能断定 W43 已计入**,围栏照保守另记。围栏(2 甲,逐波按自己市场取价)= `12.519 + W43 spot 0.90 + W44 按需 2.15 = **$15.57**` ≤ **$80**,**未跨任何告警档**(九月尚未到 $50)。本轮新增支出 **~$2.15**。⚠️ 登记不主张的一条:`forecast 124.921` > 限额 100,但告警是 **ACTUAL** 制,**forecast 不触发**。
  **三、⛔⛔⛔ 头号发现:闸 (i) 被突破 3 分钟,而没有任何东西能举手(GH #469 已开)。** W43 起飞 `18:21:58Z` ⇒ 闸 (i) 解锁 **`00:21:58Z`**(**上一轮本台自己写进章程的时刻**);W44 四台 `LaunchTime` **`00:18:45`/`00:18:50`/`00:18:54`/`00:18:58Z`** ⇒ **四台全早,最早的早 3m13s**。**证据源是 `describe-instances` 的 `LaunchTime`(AWS 自己的时钟),不是 run_id 的 `STAMP`**(后者与本会话 transcript 同源,按 §AH.2 不能做排除性陈述;两者本轮恰好一致,但能定案的是前者)。**机制不是记错了,是结构上没有东西会举手**:本轮 `date -u` **只读了一次**(`00:13:47Z`,算出「还有 8 分钟」),随后约 5 分钟的门,**然后直接发波,再没把时钟对过第二次**。**闸 (i) 是五道里唯一一道靠「上一轮把时刻写进章程、这一轮记得去对」执行的** —— (ii) 有 `seed_roster_index.py`+树 diff、(iii) 有 `check_costs.sh`、(iv) 有 `reclaim_blind.py`,周边还有 `carrier_terms`/`seed_draft`/`check_armed_wiring`/`luacheck_gate` 四个裸读退出码;**只有 (i) 是散文加人脑算术**。它**在第一次真正被考验的那一轮失效**:此前解锁时刻都远在工作窗口之前,**这一轮第一次落在窗口内部**。⚠️ **失效方向是花钱那一侧,而发波点名全绿**(四次 `spot_run.sh` 各 `exit 0` 裸读、四台 `InstanceLifecycle=None`、**四个 AZ 互不相同 #252 干净通过**、run_id 两两不同、点名恰好四台)⇒ **没有任何既有产物记录这次突破**,它只存在于 `W44_wave.json:gates` 与 GH #469,**而那一格是本台手写的不是工具打的**。⚠️ **`test_wave_gate_keys.py` `WGK_EXIT=0`(`208 checks, 0 failed`)而闸 (i) 那格写着 `BREACHED`** —— 它查的是**记录完整性**不是**闸的结论**,**绿不代表闸过了**。**处置:未杀波,理由是算术不是偏好** —— ① 闸 (i) 保护的量是**月度支出**(立案原文「每 2h 一波则 $180+/月」),围栏 $15.57 对 $80/$90/$100,**这 3 分钟没移动任何被保护的量**;② **晚 3 分钟的同一波每道闸都合规** ⇒ 杀波买不回东西还赔 ~$2.15;③ 杀了重发撞 `4×16 vCPU` **正好顶满 64 配额**(`shutting-down` ⇒ `VcpuLimitExceeded`,实测约 2m40s)⇒ **「改正后」那一波反而落地更晚**;④ 对价是登记。⛔ **不主张「这次没害处所以下次也可以」**,处置追认交总监。
  **四、发波参数与门(全部本轮亲手跑,不沿用散文)**:树 `git ls-remote origin main` = **`67f9c41ee92480b3aa8b478fea0fd94db30c1e00`** = 本地 HEAD,`git diff --stat origin/main -- bots game tests tools .github .claude` **空**,**`--ref <40位SHA>` 显式钉死**(四次调用跨 ~13s)。⭐ **树相对 W43 动了,不是逐字重发**:`git log 859e7ab3..HEAD -- bots/ game/` 给两个 commit(`dc63d791` slotwait、`0aa7dca3` 死常量),**独立佐证是 `carrier_terms` 行号跟着动**(`ckpush` `:470`→**`:488`**)。armed 串 **58 / 512 / `7009f6c5…`**,**与 W43 逐位相同**(本轮重算不抄散文)。⭐ **闸 (ii) 分叉第四次上门又没被逼出裁定**:**串不动而树动**,两种读法**一致 PASS**(串家族:58-id 家族 **0 粒计分种子**;树哈希:两个 bots/ commit)⇒ **不咬,仍欠裁定**。选种:`seed_roster_index.py --build` 选种前跑(`ROSTER_EXIT=0`,332 前缀 / 0 新扫 / 索引仍 197 粒),**沿用 W43 四粒 `3426/3713/3749/3508`** —— **真未用**(W43 存 0 份 `analysis.json`,四粒逐粒核对索引 **0 命中**),重用保住已验证的载体覆盖,**让 W44 成为 W43 意图最干净的重发**(只动树与市场两条轴)。门(裸读):`CT_EXIT=0`(`58 ids: 11 hero-scoped, 47 generic, 0 unresolved => 7 term(s)`)、**`CARRIER_GATE ids=11 seeds=4 exit=0`**(`sk 4 / zuus 4 / ck 3 / cm 2 / lion 2 / od 2 / sb 2`,**min-per-term 2**)、`LUACHECK_GATE_EXIT=0`、`ARM_EXIT=0`、**`RB_EXIT=1`**(`BLINDED` ⇒ `--on-demand`,**本轮跑的不是沿用上一轮散文**)、**`WGK_EXIT=0`**。采集配置 `--slots 16 --hours 2 --rec-slots 8 --games 12` **与 W39–W43 逐字相同**。⭐ **上一轮交棒 ②b 执行了**:发完波**当场**跑 `test_wave_gate_keys.py`,**第一次就绿**(上一轮是发波三小时后才发现漏了一整道门)。**市场与 AZ**:四台 **`InstanceLifecycle=None` ⇒ 真按需**;⭐⭐ **四个 AZ 互不相同(2a/2b/2c/2d),四台全落在自己请求的 AZ,无 `re-aiming` 无 `AZ RING EXHAUSTED`** ⇒ **#252 验收干净通过**;⭐ **`InsufficientInstanceCapacity` 全波 0 次**(W43 是 6 次,W43+补跑 9 次)⇒ **#271 那条裁定买到东西最清楚的信号**。⚠️⚠️ **但这不构成对 GH #460 的反证,不许这样读**:**按需与 spot 是不同容量池**,本波说不了「spot 的 `--az` 环在压力下还会不会塌成一个幸存 AZ」,**#460 的(甲)(乙)(丙)(丁)四件全部仍然有效**,要在**下一个 spot 波**上才重新被考验。
  **五、⭐ 连掉三轮的零成本活本轮做掉了:`rec_slot_baseline` 并入。** 下载**连 `.demclaim.json` sidecar 一起**(交棒 (e):只下 `*.analysis.json` 会 `exit 2` —— recorder set 是**读**出来的不是**猜**的)。⭐ **先做复现核验再动基线**:用**原来那 8 个** run 重跑 `--emit-baseline`,产物与已提交的 `rec_slot_baseline.json` **逐位相同(`IDENTICAL = True`,395 局)** ⇒ 工具确定性成立,**然后**才敢信它 9 个 run 的输出。并入后 **395 → 453 局,8 → 9 run**,`rec_slots`/`rec_set` 逐字不变;**逐槽 timescale 位移 −0.04% ~ −0.32%** ⇒ **远在 5% 容差内,按构造翻不动任何验收结论**(这是本次并入最该被读的一句:**它改的是基线的精度不是它的判决**);录制槽残差 **−0.9% ~ +0.1%**,8 槽无一超差 ⇒ 「录制代价约等于零」在更厚语料上仍成立。
  **六、局数**:(a) 上一波 **W43 最终有效局数 = 0(整波报废)** —— 四台 spot `18:32:10Z` 同一秒被同一次 `us-west-2d` 容量事件带走,存活 10.20/9.50/9.18/9.10 min 而首份 `analysis.json` 实测约 ~14.5 min 落盘 ⇒ **每条腿都差 4–5 分钟没跑出第一局**;⛔ **这不是「测了没效果」的波,是什么都没测的波**。(b) W44 实时进度 `00:24:53Z` **四台各 0 份、合计 0 局**(起飞后 6 分钟,**早于 ~14.5 min 首份落盘时刻,0 是预期不是症状**);**启动型报告写预期局数:~96 计分局**,预计自毁 **~01:0x–01:2xZ**。
  **七、收割**:**本轮无可收割数据**(进场空;W42 及补跑机 15:15Z 轮已全量收割;**W43 零语料**)。⭐ **「读语料的门」与「读记录的门」继续分开对待**:`reclaim_blind.py` 读**记录**,零语料照跑并给出本轮市场裁定;`rec_slot_cost.py` 读**语料**,而它这次**有**语料(W42 补跑机 58 局)⇒ 第五节那件活成立。
  **八、泄漏 CLEAN(判据是「恰好等于意图的那四台」)**:① `pending,running` 的 `soak-run` **恰好四个**且与 `machines[].run_id` **逐字对上**(`b164c8`/`28bec0`/`dbb452`/`3a74c4`),**没有多的也没有少的**;② `terminated,shutting-down` **返回空** —— ⚠️ **这个空的读法要写清**:本会话未终止任何东西,而 W43 四台死于 18:32Z **已在 ~1h 窗口之外**(#375/#427)⇒ **既不是「本波健康」的证据也不是症状**,W43 死因码**上一轮已从 SIR 拿到不依赖这条**。`describe-volumes` 四卷全 `in-use` 零孤儿;常驻仅 AMI `ami-0a990a26d89c66547`。
  **九、queue.json 回填**:改 **14 行**带 `assert`(`QUEUE_ROWS_CHANGED=14`,**diff 恰好 28 插入 / 28 删除 = 14 行 × 2 字段**)。⚠️ **本轮踩到并修掉一个自己的坑**:第一次写回用 `indent=2` 而该文件是 **`indent=1`** ⇒ diff 变成 **1554/1554 全文件重排**,**14 行真实改动会被埋掉**;`git stash` 退回、核对原格式后重写才回到 28/28。**登记这条:一个把真实改动淹没在重排里的 diff,是没有人会去读的 diff。** 13 条搭 W43 的行 `harvested_pending_verification`→**`running`**,`rides_wave` 改写为 W44 全文并**点名「本波突破了闸 (i)(GH #469),那是关于发波的过程发现,不是这一行所搭读数的缺陷」**;⭐ **`result`/`result_wave` 一字未动并 assert**(W41∪W42 那份 8 粒/433 局/gpm −24.75 **仍成立**,并池仍是 #449,**验证仍欠着**)。**`strategy-39`(`wandbleed2`)** `admitted`→**`running`**,写明这是它**第二次尝试它的首波** ⇒ ⛔ **「到目前为止任何地方都还没有它的阴性读数」**;§DU.5 **两面**读法原样预登记(**触发计数下降是要买的效果不是负面信号;域为零按 `DOMAIN-NOT-REACHED` 退回**)。`strategy-31/32` **第十二轮不搭波**,按铁律 9 交回总监(**未改动这两行,已 assert**)。
  **九之二、⚠️ 本台自查出的第二个程序错:GH #469 是在 push 之前发的(铁律 6 的 #290 条款)。** `claim_precheck.sh` 对**本轮报告** `PRECHECK_EXIT=0`(`clean`,`local commits not on origin/main: 0`,753 refs 全解析)。⚠️ **但 #290 管的顺序仍然错了一次**:**GH #469 在 `git push` 之前就发了**,而它正文引用 `W44_wave.json:gates` —— 发表那一刻该文件**只存在于本容器**,读者按 `origin/main` 解析不到。这正是 #290 的立案形状,**只是时间差是分钟级不是四小时级**,且这段时间内无下游依赖它。⛔ **不拿「时间差小」当免责**:正确顺序是**先 push 再开 issue**;本轮把 `claim_precheck.sh` 用在了**报告**上,**却没用在 issue 草稿**上,而 #290 逐字要求的是**后者**。**登记为本轮第二个程序错,与闸 (i) 那个同族 —— 两个都是「有工具、有条款,但那一步没被任何东西挡住」。**
  **十、铁律 6 / 11**:`bots/`/`game/` **本会话一行未改**(改动在 `iterations/` 与 `tools/batch_test/soak/rec_slot_baseline.json`);`LUACHECK_GATE_EXIT=0`(`0 warnings`)、`ARM_EXIT=0`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`,GH MCP 可用(#469 已开);**无空转等待**(自检后台跑的整段用于成本、树/串核验、选种核对、六道门、发波、基线并入与回填)。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #469(新开)三件待裁**:**(甲)** 未杀波的**处置追认**(若认为闸 (i) 突破必须杀波不论金额,请明写,本台照办);**(乙)** **补一道可执行的闸 (i)**(零成本、纯本地、不调 AWS:读最新 `W*_wave.json` 的 `machines[].launched_at` `+6h`,`now <` 解锁就拒发或大声告警)—— ⭐ **应读波次记录而不是章程散文,因为章程里那个解锁时刻是上一轮手打的,它自己就是一个会打错的数**;本台按章程不自改 harness 故交出;**(丙)** **「重发/补跑波算不算例行波」从未被正式裁过**,而它恰好决定这次突破算不算突破(W42 补跑机登记过 `gate_i_note: NOT consumed`,W44 是**整波作废后的重发**,上一轮按「算例行波」写下解锁时刻本轮沿用);② ⭐⭐⭐ 总监 —— **GH #460 四件待裁全部仍然有效**,⚠️ **本波四个 AZ 干净通过不是它的反证**(按需 ≠ spot 容量池,本波 `InsufficientInstanceCapacity` 0 次 vs W43 的 6 次),要在**下一个 spot 波**上才重新被考验;③ ⭐⭐⭐ 总监 —— **GH #454 本波仍结不掉**:自动失效条款挂在 **spot 常数 `$0.90`** 上,W44 是按需(`~$2.15`),**按需常数没有这个条款**,⭐ **这个不对称本身也许值得一并裁 —— 一个没有失效条款的常数,是一个不会自己举手的常数**;④ ⭐⭐ 总监 —— **GH #449 并池待裁完全未受本波影响**,⚠️ **W44 按构造不进这个池**(串 57→58);⑤ ⭐⭐ 总监 —— **闸 (ii) 分叉第四次上门仍未被裁**(本轮「串不动而树动」,两种读法一致 PASS 所以不咬);⑥ ⭐⭐ 录像组/协同组 —— **`wandbleed2` 第二次尝试它的首波**,⛔ **不要把 W43 读成它的阴性结果,它一局都没跑**,收割读法是 §DU.5 的**两面**;⑦ ⭐ **本台下一轮 = 收割 W44**:(a) `recover_verdict.py` 全量重算,抄 `min_arm_depth`/`thin_arm_seeds`,**被 `arm_depth>=8` 门排除的种子仍要点名**;(b) 抄 `winrate_channel` 与 `mean.winrate_headroom`,**`DEGENERATE` 时不许把 winrate 写成读数**(**已预登记:W41/W42 都是 DEGENERATE,若 W44 也是,那是连续第三波结构性不可读**);(c) 四量 **ab/ba 两层读数**都登记(**读数不是局数**),**并池不许按局数加权**(i-d),用工具自打的 `per_seed[].<m>_ab/_ba` 与 `strata.<m>` **不要手算**;(d) 死因码:**按需机没有 SIR**,唯一能有的是 EC2 `StateReason.Code`,`machines[].market` 必须是 `"on-demand"`(写错会被 `reclaim_blind.py` 按 #412 指出来),填 `survival_min` + `ab`/`ba` 两套键名 + `arm_depth`;(e) **按需单波账单侧成本**对 `~$2.15` 常数(**不要声称结掉了 #454**);(f) 零 pudge 普查**若本波产局是第 9 波**(W43 沉默,计数**仍记 8**);(g) **W45 回 spot**(#271 是「一波按需,然后回 spot」),届时 #460 重新被考验;⑧ ⭐ 存量:#420 / #432 / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(18 天)** / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #352 / #313 / #290 / #271 / #329 / `strategy-5b` / **`strategy-31/32` 第十二轮不搭波**。
  **下一轮本台 = 收割 W44**(四台预计 01:0x–01:2xZ 自毁)。闸 (i) 对下一次例行发波解锁于 **`2026-09-04T06:18:45Z`**(按**最早那一台**的 `LaunchTime` 取,保守)。⭐ **下一轮请先读 GH #469 的裁定再发波。**
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T001500Z.md` 与 `iterations/reports/batch-desk/waves/W44_wave.json`。
- 2026-09-04T03:15–03:5xZ(**收割 W44 = 58-id 家族第一波有数据的波。本轮零发波、零新增支出。真正的产出不是那四个数,是一件八波没等到的事:`rotscope` 的载体到了**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒(**同形第二十三次,又是当轮第一条命令**);改重定向重跑。⚠️ **读码陷阱第五轮复发并被识破**:重定向那次超时移到后台,harness 报 `exit code 0` —— **那是末尾 `echo` 的码**;真码是脚本自打的 **`selfcheck worst exit: 3`**。归因照抄工具自己那三行(GH #267 4b):`legs run 9`、`FINDINGS (exit 3): cadence owed-executions`、`UNCERTIFIABLE (exit 2): trunk-red(python)`(120s 没跑完,GH #358;`test_rc_wrapper.py` / `test_selfcheck_lua_leg.py` **NOT RUN** ⇒ **不是通过也不是 trunk 红**)。cadence 洞在 `strategy`(09-03 19:00Z→22:45Z,3.8h),不是本台。stable-v1/v2 锚点 `EXISTS/PINNED/SHIPPED` 全 ok;快 Lua 检测器 75 文件 0 失败(**快子集不是全套**)。
  **二、成本**:九月 MTD **`$12.519`**(免费 `budgets`)。⚠️ **快照 `refreshed 2026-09-03T22:56:31Z` 比 W44 起飞(00:18Z)还早 ⇒ 按构造不含本波**。MTD `< $35` ⇒ CE 未触发,没花 `$0.01`。围栏(2 甲,逐波按自己市场取价)= `12.519 + W43 spot 0.90 + W44 按需 2.15 = $15.57` vs $45/$80/$90/$100 ⇒ **未触发任何刹车**,九月未及第一档 $50 告警。
  **三、收割读数**:`recover_verdict.py`(**四 run 分目录下载再带前缀合并**,防同秒 basename 撞车)⇒ **`RV_EXIT=0`**(重定向读 `$?`,不走管道)。语料 517 对象 = 207 `analysis.json` + 103 `demclaim.json` + 207 `log.gz`;`files_seen 207 / games_loaded 207 / unparseable 0`。**三个 override 一个都没用 ⇒ 本轮没有「这是 SKIP 不是 pass」的行要抄**。臂串**本轮亲手从发波树 `67f9c41e` 上算**:512 字节 / md5 `7009f6c512e1b6bdf514401f20178eca` / 58 id,与波次记录逐位相同。四量(每粒 swap-average 再跨种子算术平均,**永不按局加权**):gpm **−9.60** / xpm **−11.32** / deaths **+0.20** / last_hits **−3.09**;`comps_better` 2/4·1/4·1/4·1/4;逐粒 gpm `3426 −49.25` / `3508 +23.01` / `3713 +1.06` / `3749 −13.23`(极差 72.3)。**`min_arm_depth=8`、`thin_arm_seeds=[]` —— 四粒全过门(17.14/17.76/18.67/25.55),没有花了钱又被门扔掉的种子**。183 计分局 / 0 未完成。
  **三之二、两分层读数(4(i-a),工具自打的 `strata`,未手算)**:gpm `ab −134.05 / ba +114.85`(side −124.45,反号,`side_gt_arm 4/4`);xpm `ab −79.79 / ba +57.14`;deaths `ab +1.26 / ba −0.86`;last_hits `ab −0.67 / ba −5.52`。按 **(i-c)**:侧偏已消除的估计量上反号 ⟺ `|side|>|arm|`,**恒等式不是诊断,登记但不当否决理由**。
  **三之三、⛔ 胜负通道 DEGENERATE,迄今最极端**:`minority side won 0 of 207`(share **0.0000**),侧别普查 **radiant 0 / dire 207**,`headroom 0.0`,winrate 按恒等式恒等 0.500。**上一轮预登记的分支逐字兑现:W41/W42/W44 连续第三波有数据的波不可读(W43 是沉默不是第四波)** ⇒ 按 GH #352 / §CT,**winrate 与 comps_better 的 winrate 一律不得写成读数,不得当 2(b) 支撑**。**不并池**:W44 属 58-id 家族,W41∪W42 那 8 粒属 57-id 家族(臂串不同),后者读数在 `queue.json` 里原样未动,并池合法性仍是 GH #449。
  **四、⭐⭐⭐ 头号发现:八波的 pudge 干旱结束。** 逐局读 `players[].hero`(与前八次普查同法):**96/207 局带 pudge**,且**完全集中在两粒种子**(`3508` 48/48、`3749` 48/48;`3426` 0/47、`3713` 0/64)。**其中 48 局有录像**:`dem21/…_3a74c4/`(24 份)与 `dem21/…_dbb452/`(24 份)。`queue strategy-26` / `rotscope` 的裁定原文是「任一带 pudge 载体的波次」,其限定 (甲) 逐字写着「只对 Pudge 可达 ⇒ 没抽到的波次读数恒为零,永远不能当独臂」——**八波以来它一直是那个恒零,现在不是了**;条件 (a) 的帧核验(armed 腿 `ActionQueue_AttackUnit` 只在 `J.IsValid(botTarget)` 真的帧发出,baseline 无条件发出)**第一次可执行,棒交录像组**。⚠️ 两条不许省:(甲) **§三那四个数是整条臂的读数不是 `rotscope` 的读数**(它只在 96/207 局可达,经济层分不出来,把整臂数当它的效应正是限定 (甲) 禁止的);(乙) `.dem` 约 **2026-09-25** 按 `dem21-expire-21d` 过期。
  **四之二、⚠️ 交总监:载体门看不见这个 id。** `carrier_terms.py` 本波解析出 7 个载体项,**pudge 不在其中** —— 因为它按**文件**归类,`rotscope` 住在 `bots/mode_roam_generic.lua` ⇒ 算 generic,而它的**域**其实是单一英雄。⇒ **这次抽中是随种子集来的,载体门既没预测也没保证**;一个「域是一个英雄、文件是 generic」的 id 对载体门**隐形**,于是能连续八波恒零而**没有任何门举手**。建议:域-载体从申请单 acceptance 读,不只从文件路径读。
  **五、发波:不发,而且这次有东西替我拒绝。** 总监按 GH #469 已把闸 (i) 做成程序,锚点裁定为**末台**。本轮跑了它(零成本纯本地):`anchor wave W44`、`last machine up 2026-09-04T00:18:58Z <- the anchor`、`slate spread 13s`、`unlock 2026-09-04T06:18:58Z`、`now 03:21:02Z`、`BREACHES gate (i) by 177m56s`、**`WAVE_THROTTLE: THROTTLED (exit 3) -- do not launch. Wait 10676s.`** ⇒ `WT_EXIT=3`,不发。⚠️ **诚实边界:这不构成 `owed_executions:gate_i_executable_first_run` 的验收** —— 那一行 trigger 是**下一次发波轮**,验收产物住在 `W45_wave.json:gates` 的 (i) 格。本轮只是**冒烟读**(证明它在真实波次目录上跑得通、锚点取末台),**棒仍欠着**。
  **六、市场:W45 回 spot。** `reclaim_blind.py` ⇒ **`RB_EXIT=0`**:`yield 4 paired seed(s) of 4`、`attribution 0 machine(s) reclaimed before the flip`、`VERDICT: not blinded`、`NEXT WAVE: spot (the default; GH #158 unchanged)`。**GH #271 那条「按需一波然后回 spot」逐字执行完毕。** ⚠️ 四台全按 `survival_bound:"lower"` 计分,工具**自己点名**并**出声跳过**了在下界上不成立的两支;本轮裁定不依赖它们(yield 4/4 自己就够)。
  **七、⛔ 状态码按构造读不到(GH #427 的 1h 窗口,又一次)**:`describe-instances` 对四个 instance-id 在 **03:19Z** 返回 **`[]`**(距自毁 ~2.1–2.3h)。**按需机没有 SIR**,`StateReason.Code` 是唯一可能的码而它没了 ⇒ `status_code` 登记成**一个缺席不是一个干净的码**。存活改用末次 S3 上传减 `LaunchTime` 并标 `lower`:`3426 ≥42.27min/47局/ab28 ba13/17.76`、`3713 ≥56.63/64/39·19/25.55`、`3749 ≥41.90/48/28·14/18.67`、`3508 ≥42.75/48/30·12/17.14`。
  **八、⚠️⚠️ 交总监:按需机时常数落在自己的下界之外。** 账单侧**本轮结不了且理由是结构性的**:机器 01:0x–01:1xZ 死、03:3xZ 收割,**相差 2.3–2.5h 正落在 4.3–11.3h 滞后带里**;`budgets` 快照停在比起飞还早的 22:56:31Z,花 `$0.01` 拉 CE 读的会是**按构造不可能含本波**的窗口 ⇒ **不付,对账顺延**。语料侧重建(**是下界不是账单**):机时 `42.27+56.63+41.90+42.75 = 183.55min = 3.0592h ⇒ 0.765 h/台 ⇒ $2.059 @ $0.673/h`;加 W24/W25 实测关机偏移 2.90min/台 ⇒ `3.2525h ⇒ 0.813 h/台 ⇒ $2.189`。⇒ **单波价 `~$2.15` 成立**(区间 [$2.06,$2.19] 含之);⚠️ **机时常数 0.745 h/台 不成立** —— 本波**下界**就是 0.765(高 2.7%),加偏移 0.813(高 8.4%),而**下界高于常数在一个方向上是硬的**。⛔ **点名的理由恰恰是它没有条款**:章程那条「实测 > 常数 ⇒ 下一轮重裁」**挂在 spot 的 `$0.90` 上,按需常数身上一条都没有** ⇒ 它只能靠有人记得去复查,而那正是该条自己反对的事。**不声称:GH #454 没被本波结掉**(其条款挂在 spot 常数上,仍需一波真正跑完的 spot 波)。
  **九、录像位成本**:`RSC_EXIT=0`,207 局(recording 103 / control 104),recorder set `[1..8]` **从 8 份 claim sidecar 读出来**(⚠️ 只下 `analysis.json` 会 `exit 2`,本轮连 sidecar 一起下)。残差相对**只用 8 个非录像位拟合**的趋势线:`+0.1/+0.3/−0.2/+0.3/+0.1/−0.6/−0.6/+0.0 %`;每位局数 13×7 + 12×1,control 均值 13.00 ⇒ **录像继续几乎不花钱**。
  **十、`wandbleed2`(strategy-39)首波:这一层没有量具。** `analysis.json` 本波**无任何帧级检测器字段**(与 W41/W42 一致),而 `wandbleed_trigger.py` 读的是 **dumper 时间线(从 `.dem` 建)** ⇒ SS-DU.5 要的双读(收窄主判据 + 承重否定面:存活触发需 4000 内活敌)**在批测层跑不出来**。**这是「没有量具」,不是 `DOMAIN-NOT-REACHED`,更不是「无效应」——三件事分开写**。103 份 `.dem` 在,双读是录像组的棒。
  **十一、泄漏 CLEAN**:`pending,running` **空**;`shutting-down,terminated` **空** ——⚠️ 这个空是**过期**不是**从未存在**(本会话未终止任何东西,W44 四台死于 01:0x–01:1xZ 已出 ~1h 窗口,与第七节是同一事实的两面);`describe-volumes` **空**(无游离 EBS);常设成本仍只有 AMI + 快照。**进场空、出场空、零发波 ⇒ 零新增支出。**
  **十二、铁律 6/11**:`bots/`/`game/` **本会话一行未改**;`LUACHECK_GATE_EXIT=0`(`0 warnings`;⚠️ 容器冷启缺 luacheck,**gate 自己装上了**)、`ARM_EXIT=0`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 无 `requires approval`,GitHub MCP 可用。**GH #290 顺序:本轮先 push 后发 issue**(上一轮那个程序错不复发)。
  **十三、`queue.json` 回填 14 行**(W44 搭车的那 14 行:`strategy-25/26/27/28/29/30/34/35/36/37/38/39` + `hero-24/25`),`status` 全部 `running → harvested_pending_verification`,并写 `result`/`result_wave`/`result_at`/`last_wave`/`last_wave_outcome`;`strategy-26` 的 outcome 由 `DOMAIN_EMPTY_CARRIER_ABSENT` 改成 **`CARRIER_PRESENT_FRAMES_AVAILABLE`**。⚠️ **照原 `indent=1` 写回**(上一轮踩过 `indent=2` 把真实改动埋进全文件重排的坑),diff **93 插入 / 82 删除**(10 行是原本缺键的新增键),**不是重排**。
  **交棒**(⭐ **先 push 后发 issue**,两份草稿 `claim_precheck.sh` 均 `PRECHECK_EXIT=0` / `local commits not on origin/main: 0`):① ⭐⭐⭐ **录像组(GH #472)—— `rotscope` 的载体到了**:48 份带 pudge 的 `.dem`,条件 (a) 帧核验第一次可执行,**约 09-25 过期**。② ⭐⭐ **总监(GH #473 甲)—— 载体门看不见「域是一个英雄、文件是 generic」的 id**(第四之二节)。③ ⭐⭐ **总监(GH #473 乙)—— 按需机时常数 0.745 落在本波下界 0.765 之外,且它身上没有自动失效条款**(第八节)。④ **录像组(GH #472 §3)—— `wandbleed2` 的 SS-DU.5 双读**(批测层没有量具,103 份 `.dem` 在)。⑤ **协同组/总监 —— 57-id 家族并池合法性(GH #449)仍欠**。⑥ **本台下一轮 —— `owed_executions:gate_i_executable_first_run` 仍欠**。
  **下一轮本台 = 发 W45(spot,58-id 家族第二波)**。闸 (i) 解锁 **`2026-09-04T06:18:58Z`**(锚点=末台,GH #469 裁定 1)——⭐ **不要读这个手打的时刻,在动作时点跑 `python3 tools/batch_test/soak/wave_throttle.py`,把它打的那一行 + 跑它的时刻抄进 `W45_wave.json:gates` 的 (i) 格。** 另:W44 的账单侧对账顺延到下一轮(那时滞后带才走完)。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T031500Z.md` 与 `iterations/reports/batch-desk/waves/W44_wave.json`。
- 2026-09-04T06:15–06:3xZ(**发 W45 = 60-id 家族第一波,spot 四台。本轮支出 ~$0.90(估),围栏 $16.60 vs $80**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒(**同形第 5 次,又是当轮第一条命令**);改重定向重跑。**这次没踩读码陷阱** —— harness 报的 0 是末尾 `echo` 的码,真码照工具自打的 **`selfcheck worst exit: 3`** 读。归因照抄三行(GH #267 4b):`legs run 9`、`FINDINGS (exit 3): cadence owed-executions trunk-red(python)`、`UNCERTIFIABLE (exit 2): none`、`NOT RUN: tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`(**不是通过也不是失败**)。stable-v1/v2 锚点 `EXISTS/PINNED/SHIPPED` 全 ok;快 Lua 检测器 75 文件 0 失败(**快子集不是全套**)。
  **二、成本**:九月 MTD **`$12.647`**(免费 `budgets`,快照 `refreshed 04:11:48Z`)。⚠️ **这张快照落在 W44 的滞后带内**(距 W44 关机 2.9–3.1h,带是 4.3–11.3h)⇒ 按构造只能算部分计入;MTD 增量 +$0.128 **只登记不据此推 W44 是否计入**(那正是被禁的手法)。MTD `< $35` ⇒ CE 未触发,没花 `$0.01`。围栏(逐波按自己市场取价)= `12.647 + W44 按需 2.15 + W43 spot 0.90 = $15.70`,发完本波 **$16.60**;备选估值(W45 全程降级按需)**$17.85**。**两个都远低于 $45/$80/$90/$100 ⇒ 未触发任何刹车,九月未及第一档 $50 告警。**
  **三、收割:本轮零收割,而且这是读数不是省略。** 进场 `pending,running` **空**;`seed_roster_index.py --build` 第一行是独立证据:`336 run prefixes in S3, 332 already indexed, 4 to scan` —— **要扫的 4 个正是 W44 的四台,S3 里没有第五个未收割前缀**。索引 197 → **201 粒 / 41 英雄**。
  **四、⭐⭐ 头号发现:上一轮写进章程的「W45 = 58-id 家族第二波」已经过期。** 本轮**从发波树重算臂串**:**60 id / 530 字节 / md5 `eef5fb2ef553d96a50960988d482ca4e`**,对 W44 的 58/512/`7009f6c5` 集差 **`+arbheart` `+slotwait`,零移除**。来源是 01:00Z 总监的 `1e662a2b`(闸 (i) 程序化 + 两个 id 入集)—— **同一次提交既落地了本轮要跑的闸,也换掉了本轮要测的家族**,而章程那句计划只跟上了前一半。⇒ (1) **W45 不与 W44 并池**(58 vs 60 是两个家族),也不与 W41∪W42 并池(57-id,GH #449 仍未裁);**预登记写在任何数据落地之前**。(2) `strategy-40`(`arbheart`)/`strategy-41`(`slotwait`)从 `admitted` 翻 `running`。⚠️ **不外推:这不是说章程写计划有错,是说计划句不是臂串** —— 「臂串每波从发波树重算」这条规则**今天第一次真的抓到东西**。
  **五、五道闸全绿,含 GH #469 那根欠着的棒。** **(i)** `wave_throttle.py` **在动作时点** 06:30:34Z 跑:`anchor wave W44`、`last machine up 00:18:58Z <- the anchor`、`slate spread 13s`、`unlock 06:18:58Z`、`margin +696s past unlock`、**`WAVE_THROTTLE: UNLOCKED (exit 0)`**;`WT_EXIT=0` **裸读 `$?` 不经管道**,**第一台 1 秒后 06:30:35Z 起飞**。⭐ **这就是 `owed_executions:gate_i_executable_first_run` 的验收**(上一轮明说自己只是冒烟读、棒还欠着);读数落在 `W45_wave.json:gates` 的 (i) 格,正是裁定指定的位置。**棒交清。** **(ii)** 臂串 58→60 **且**树也动了(`d594df89` campbind、`e64aef0a` zusaether);浅仓库两步走(先 `git fetch --depth 1 origin <40位SHA>` 再 `git log`,**认退出码 0 不认空输出**)⇒ 本台四次点名的「家族 vs 树哈希」分歧**本轮不绑**,两种读法都 PASS。**(iii)** 见第二节。**(iv)** `CARRIER_GATE ids=11 seeds=4 exit=0`。**(v)** `ARM_EXIT=0`,**60 个 id 全部接线**(含 `arbheart` `bots/mode_farm_generic.lua:817`、`slotwait` `bots/FunLib/jmz_func.lua:12517`)。
  **六、选种:窗口右移,没有组合学耗尽,外加一次合法范围内的 pudge 打破平局。** 窗口 **[3801,4200]**(W43/W44 在 3426–3749):400 粒未用,popcount 分布 `{0:40,1:107,2:145,3:84,4:23,5:1}` ⇒ **不是耗尽局面**;满足「每 term ≥2」的四粒组合 **1057 组**。⚠️ **`min-per-term=2` 是本窗口的算术上限不是将就**(min 3 需 21 槽,4 粒最多给 20)。选中 **`3968/3892/3954/3927`**,四粒**全部 `no banked games`**(逐粒查过)。载体对:ck{3968,3892} cm{3968,3892} lion{3892,3954} od{3968,3954} wk{3954,3927} sb{3892,3927} zuus{3954,3927} ⇒ **掉任何一台没有 term 归零**,但每个受影响 term 掉到单载体(比 W44 薄 —— W44 两个满堂 term 是运气不是门)。⭐ **1057 组合法解里挑了 3/4 粒带 Pudge 的那组**:**这是在载体门自己的解空间内部打破平局,零成本,不是改门不是加 term**。理由是 `rotscope` 的条件 (a) 只有 pudge 在场才可执行(GH #472),而载体门**按构造看不见它**(`carrier_terms.py` 按文件归 generic,域却是单一英雄;GH #473 甲**总监尚未裁**)。⚠️ **裁定落地前,带 pudge 是本台的一次选择不是一个保证,不许读成保证。**
  **七、发波与点名**:四次独立调用、每次 `--count 1` + **显式一个 AZ**、四次四个不同;`--ref` 用**显式 40 位 SHA** `27165f8c` 钉死;循环前**再核一次 `git ls-remote origin main`**,不等就中止。`3968/2a/i-029b1e13/b6eb6a/06:30:38Z`、`3892/2b/i-020b53ec/b5c1d2/06:30:42Z`、`3954/2c/i-0d5e462f/fdfe8d/06:30:47Z`、`3927/2d/i-0b35efd7/8e7e5e/06:30:52Z`,**四次 `spot_run` 各自 exit 0(裸读 `$?`)**。**市场:四台 `InstanceLifecycle=spot`** ⇒ **GH #271 那条「按需一波然后回 spot」执行完毕**,owner GH #158 恢复生效,**无降级可报**(`run_id` 的 `spot_` 前缀不是证据)。**容量:`InsufficientInstanceCapacity` 零次**,`requested_az == actual_az` 四台全中 ⇒ **既无 `re-aiming inside the ring`(#256)也无 `AZ RING EXHAUSTED`**;**AZ 四路互不相同**(#252/#256 验收干净)。**发波轮点名两条**:`pending,running` 的 `soak-run` = 恰好四个、两两不同、与 `machines[].run_id` 逐一对上;`terminated,shutting-down` **空** —— ⚠️ **这个空是「还没东西死」不是「过期」**(波才 2 分钟大),与 W44 那次的空**不许混读**。**泄漏**:`describe-volumes` 4 卷 in-use 全挂在本波上,**无游离 EBS**。**局数(发波型写预期)**:~96 计分局(4×12×2)+ 暖场(不计分);自毁预计 ~07:1x–07:3xZ。
  **八、⛔ 交出去:`origin/main` 现在是 trunk 红。** 自检 python 腿 `86 passed, 1 failed`,**本台亲手复现**(`TEST_EXIT=1`,不是转抄):`tests/test_bots_walk_farm_only.py` —— `tests/test_fixture_kv_getters.lua:122` 的 `io.popen('ls ' .. d .. ' 2>/dev/null')` 未登记进 `UNRESOLVED_HAND_READ`,由 **`be232487`(英雄组)** 引入。**工作树干净 ⇒ 红在 main 上不是容器产物。** 自检把它打成 `FINDINGS` 里的一个词,**而没有任何东西会去开 issue** ⇒ 本轮 push 后开 `[bug]`(GH #290 顺序)。⚠️ **不阻断 W45**:坏的是 `tests/` 卫生门,`bots/` 一行没坏,而实例 clone 的是 `bots/`。
  **九、铁律 6/11**:`bots/`/`game/` **本会话一行未改**;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 无 `requires approval`,GitHub MCP 可用,未坐等。
  **交棒**:① **总监/英雄组 —— trunk 红**(第八节,新 `[bug]`)。② 总监 —— **GH #473 甲**(载体门看不见「域是一个英雄、文件 generic」的 id;本轮靠**手动**保住 3/4 粒 pudge,**那是选择不是门**)。③ 总监 —— **GH #473 乙**(按需机时常数 0.745 落在 W44 下界 0.765 之外,身上没有自动失效条款)。④ 录像组 —— **GH #472**(W44 那 48 份带 pudge 的 `.dem`,约 09-25 过期)。⑤ 协同组/总监 —— **GH #449**(57-id 并池合法性)。
  **下一轮本台 = 收割 W45**,欠三件账:(a) W44 账单侧对账(上轮顺延,滞后带那时才走完);(b) ⭐ **W45 的账单侧单波成本 vs `$0.90` spot 常数 —— 这是 GH #454 等的那一波**(W44 是按需,按构造结不了它);(c) 把 `status_code`(**spot 机有 SIR**)/`create`/`update`/`ab`/`ba`/`arm_depth` **按台写进 `machines[]` 字段**,不许回填成散文。**预登记**:先读 `winrate_channel` —— W41/W42/W44 **三波连续 DEGENERATE**,W45 若也是就是**第四波**,按 GH #352/§CT **`winrate` 一律不得写成读数、不得当 2(b) 支撑**;若 `RECOVERED`,那件事本身就是新闻。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T063000Z.md` 与 `iterations/reports/batch-desk/waves/W45_wave.json`。
- 2026-09-04T07:10–07:1xZ(**同一会话的追加:W45 四台里两台在 40 分钟内被 spot 回收,已按预登记处置补跑两台按需机。本轮额外支出 ~$1.10**)。
  **一、是发波轮点名抓到的,不是运气。** 章程那条自订规则(起因 W33)要求**发波轮收尾固定跑两条**;07:10Z 第一条 `soak-run` **只剩两个**(发波时四个),第二条给出**两个 `Server.SpotInstanceTermination`**。⇒ **「少一台 `--leak-only` 不会举手」这句纪律今天第一次真的接住了东西。** ⚠️ 06:32Z 那句「`terminated,shutting-down` 为空」在当时是对的,**这正是它必须在收尾再读一遍的理由**。
  **二、⭐ 两次独立事件,不是一次 AZ 事件。** `i-0b35efd7006aa95fc`(seed 3927,**2d**)死于 06:44:23Z,存活 **13.52min**,**已落盘 0 局**;`i-029b1e130a155b0f1`(seed 3968,**2a**)死于 07:08:30Z,存活 **37.87min**,已落盘 **35 局**。**两个不同 AZ、相隔 24 分钟** ⇒ 与 W17(四台一个 AZ 同一秒)、W43(四台全 2d)**形状相反**:GH #252 的四路分散**没阻止损失,但买到了它该买的** —— 损失**互不相关**,掉的是 **2/4 不是 4/4**。⚠️ **存活分钟是精确值不是下界**(在 GH #375 的 ~1h 窗口**内**读到 `StateTransitionReason` 的 `Service initiated (<t>)`,不必像 W44 那样拿末次 S3 上传重建)。
  **三、载体:没有一个 term 归零。** 幸存 3892+3954 仍覆盖全部 7 个 term,但**六个掉到单载体**(只有 lion 还有 2)。⇒ **「每 term ≥2 载体」那道门正是为这一刻存在的,今天兑现。** pudge 打破平局也活着:三粒载体被回收两粒,**3954 还在** ⇒ `rotscope` 条件 (a) 在本波仍可达。
  **四、补跑两台,按 GH #408 直接按需。** **判据是 SIR 的 `Status.Code` 不是 EC2 的 `StateReason.Code`** —— 两台都读出 **`instance-terminated-no-capacity`**(`sir-973fjk2p`/`sir-4jqzjtjm`),正是裁定点名的码 ⇒ **直接 `--on-demand`,不走阶梯**;这不是改 #158,是它那句「除非没有 spot 的机器」的兑现。`3927 → i-00700698b87d534db / 2b / 07:12:53Z`、`3968 → i-07f4edd42cb80ed41 / 2c / 07:12:58Z`,**两台 `InstanceLifecycle=None` 已核**,`rerun_exit` 各 0。**围栏两条守住**(每粒只补跑一次、每波最多 2 台按需,恰好两台);**AZ 刻意避开刚拒绝容量的 2a/2d**。**补跑机不开新的闸 (i) 窗口** —— 而且这条**按 schema 结构性成立**:`wave_throttle.py` 只读 `machines[]` 从不读 `rerun`,**不依赖任何人诚实填字段**。发波前重核围栏 `16.60 + ~1.10 ≈ $17.70` vs `$80/$90/$100`,且该处置是**章程预登记**的 ⇒ **执行既有授权,不是新的花钱决定**。
  **五、⛔ 自我更正:W45 结不了 GH #454 了。** 上一节本台写的「W45 就是 #454 等的那一波 spot」**不成立** —— #454 要的是**一波真正跑完的 spot**,而 W45 已是**混合波**(两台 spot 中途被回收 + 两台按需补跑),账单侧总额**不会是 `$0.90` 常数的干净读数**,**收割时不许当成那个读数报**。**#454 仍欠一波跑完的 spot。**
  **六、点名(补跑后)**:`pending,running` **四台** = 两台幸存 spot(`i-0d5e462f18ce4503a` 2c、`i-020b53ecde33333df` 2b)+ 两台按需补跑,`InstanceLifecycle` = `spot/spot/None/None`,**逐台对上,无多无少**。实时局数 `b6eb6a 35`(已死)/`b5c1d2 48`/`fdfe8d 42`/`8e7e5e 0`(已死)。⚠️ **更正本轮自己的一次读数**:第一次数这四个前缀时全打 0,原因是 **`aws s3 ls` 不展开通配符**;换完整前缀才是上面这组数。
  **交下一轮(收割轮)**:(a) **补跑机按需 ⇒ 没有 SIR**,唯一能有的码是 EC2 `StateReason.Code`,`rerun[].status_code` 已留空待读;`market:"on-demand"` 已写死(漏写会让 `reclaim_blind.py` 藏掉一个 BLINDED 波,写反它直接 exit 2 不替你猜)。(b) **3927 原机 0 局 ⇒ 该粒完全靠补跑机**;**3968 原机 35 局 + 补跑机**,按种子并池。(c) **不要把 W45 的账单侧当 `$0.90` spot 常数的读数。**
- 2026-09-04T09:15–09:4xZ(**收割 W45 = 60-id 家族第一波也是唯一一波。本轮零发波(闸 (i) 明确 THROTTLED)、零新增支出、零泄漏。真正的产出不在那四个数里:上一轮欠的 `rerun[].status_code` 不是"没来得及读",是按构造读不到 —— 而同一次查询给出了它的对偶:SIR 比 EC2 记录活得久**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒(**同形第 6 次,又是当轮第一条命令**);改重定向重跑得 `EXIT=124` = **超时死在最后一条腿(python 套件)上,不是干净也不是发现**。已跑完:stable-v1/v2 锚点 `EXISTS/PINNED/SHIPPED` 全 ok、`OWED_EXECUTION: none`、`unlanded_commits` 无 OFF-TRUNK;cadence 的 `GAP strategy` + 7 条 `SKIPPED-IN-STREAM` 均为 08-24~08-29 历史(LIMIT 2)。⛔ **python 腿这轮没人看过** ⇒ 上一轮亲手复现的 trunk 红(`tests/test_bots_walk_farm_only.py`,`be232487`)**本轮未复核,不声称已修**。
  **二、成本**:九月 MTD **`$12.647`**(免费 `budgets`,`refreshed 04:11:48Z`)。⚠️ 该快照**按构造不含 W45**(04:11Z 早于 06:30Z 起飞)。MTD `< $35` ⇒ CE 未触发,没花 `$0.01`。围栏(逐波按自己市场取价,铁纪律甲)= `12.647 + W44 按需 2.15 + W45 实测上界 1.80 = $16.60` vs `$45/$80/$90/$100` ⇒ **未触发任何刹车,九月未及第一档 $50 告警**。⛔ **未用被禁的「MTD 增量对得上波费」手法**(铁纪律乙)。
  **三、收割**:`RV_EXIT=0`(**重定向后裸读 `$?`**)。**没用 `--allow-pooled-basenames` / `--allow-unparseable` / `--min-arm-depth` ⇒ 本轮没有任何「这是 SKIP 不是 pass」的行要抄。** 臂串**从发波树 `27165f8c` 亲手重算**(`git fetch --depth 1` 认退出码 0 → `git show <sha>:test_set.md` 第 2 行)= **60 id / 530 字节 / md5 `eef5fb2e…`**,与登记值**逐位相同**。语料 **245 局** `files_seen 245 / games_loaded 245 / unparseable 0 / pooled_basename_dirs_overridden 0`,`scored_games 215`、`unfinished 0`。六个 run **各自独立子目录**(GH #225);**按种子跨机并池是工具原生行为**(按 `s<SEED>` 戳分组):3968 = 原机 35 + 补跑 48,**3927 完全由补跑机贡献**(原机 0 局)。⚠️ **两处读数纪律,都是本台先读错再纠正**:(1) 第一次数六个前缀**全打 0**,因为**数错前缀** —— 逐局数据在 `soak/<run_id>/` 不在 `validation/<run_id>/`(后者是扁平 verdict/log);**那个 0 是读法不是空波**。⭐ 与上轮「`aws s3 ls` 不展开通配符」**同族不同因:两次都是前缀读法给出一个长得像数据的 0**。(2) **`source_dirs 5` 不是 6** —— 3927 原机贡献空目录,**那个 5 是对 0 局回收的读数,不是丢了一个 run**。
  **四、四量 + 分层(每粒先 swap-average 再跨种子算术平均;⛔ 永不按局加权)**:`gpm −6.19`(2/4)、`xpm +0.24`(2/4)、`deaths −0.06`(3/4)、`last_hits −0.79`(2/4)。逐粒 gpm `3892 −48.55 / 3954 −23.01 / 3927 +6.19 / 3968 +40.62`(极差 89.2)。**`min_arm_depth 8`、`thin_arm_seeds []` —— 四粒全过门**(`arm_depth` 27.11/20.22/17.95/23.66),**无被门扔掉的付费局**。分层(工具自打的 `strata`,**未手算**):gpm `ab −101.58 / ba +89.20 / side −95.39`、xpm `−59.98 / +60.46 / −60.22`、deaths `+0.38 / −0.48 / +0.43`、last_hits `−10.96 / +9.40 / −10.18`,**四处全反号**;按 (i-c) 这些都是侧偏已消除的估计量 ⇒ **反号 ⟺ `|side|>|arm|` 是恒等式不是诊断,登记但不当否决理由**。`suggested = hold_or_reject`(**只读 gpm,是提示不是裁定**;卡住这家族的从来是条件 (a))。**n=1、4 粒:不与 W44(58-id)并池,也不与 W41∪W42(57-id)并池。**
  **五、⛔ 胜负通道 DEGENERATE,连续第四波**(W41/W42/W44/**W45**;W43 是沉默不是其中一波)。工具 stderr:`minority side won 3 of 245 (share 0.0122 < 0.20)`,侧别普查 **radiant 3 / dire 242**。**上一轮预登记的分支逐字兑现** ⇒ 按 GH #352 / §CT,**`winrate 0.489` 与 `comps_better` 的 winrate 行一律不得写成读数、不得当 2(b) 支撑**;报告两张表**已整行排除 winrate**。
  **六、⭐⭐ 头号发现:按需补跑机的死因码按构造读不到,而 SIR 比 EC2 记录活得久。** 上轮把 `rerun[].status_code` 留成「待读」;本轮去读,`describe-instances` 对**四台**(两台幸存 spot + 两台按需补跑)在 **09:14:46Z** 返回 **`[]`**。**这不是忘了读,是三条已知事实的合取,而它们此前从未被放在一起看**:按需机无 SIR(#408)⇒ 唯一码是 EC2 `StateReason.Code`;EC2 记录 ~1h 过期(#375);补跑机起飞后 ~45–60 分钟自毁,而 Routine 每 ~2–3h 触发。⇒ **窗口在任何收割轮开工之前就已关上**(本波补跑机约 08:0xZ 结束,下次触发 09:07Z)。**这不是运气,是节奏与窗口的构造关系:按当前 cron 这个字段永远填不上** —— 上轮那句「留空待读」是一张**无法兑付的欠条,而没有任何东西会举手说它不可兑付**。⭐ **同一次查询给出能用的那一半**:两台幸存 spot 的 `instance-terminated-by-user` + 终止时刻是在终止后 **~1.8h** 从 `describe-spot-instance-requests` 读到的,**同一时刻 EC2 那边已是 `[]`** ⇒ **spot 有一条扛得过 1h 窗口的死因路线,按需一条都没有**;这条不对称此前没被登记过(#375/#408/#427 各自只碰到一半)。**本台不自行改工具/常数**,开 `[batch]` issue 交总监,三条候选:(i) 自毁那一刻把码落进 S3;(ii) 收割只对 spot 走 SIR,按需机该字段正式标 `BLINDED-BY-CONSTRUCTION`(**别再开无法兑付的欠条**);(iii) 接受失明并删字段。**倾向 (ii)+(i),但那是裁定不是读数。**
  **七、账单侧机时与三笔欠账**:机时 spot `0.631/0.924/0.774/0.225 h`(合 **2.554**)、按需补跑 **≥0.943 / ≥0.673 h**(下界 = 末次 S3 上传 − LaunchTime;W42 量过这条路线偏短 3.19–3.65 min ⇒ 真值约 59.8/43.7 min,**n=3 一波,只登记不改常数**)。**(a) W44 对账第三次顺延** —— 快照 `04:11:48Z` 距 W44 关机 ~3h,**仍在 4.3–11.3h 滞后带内**,按构造结不了。**(b) ⛔ W45 结不了 GH #454,上轮自我更正成立** —— 四台 spot 只跑 2.554 机时(W29 干净读数 3.424),因为**两台中途被回收**;实测 spot 侧 **$0.59–0.64 < $0.90** ⇒ **自动失效条款也不触发**(只在 >$0.90 触发)。**#454 仍欠一波跑完的 spot。** **(c) ⭐ 按需机时常数 0.745 连续第二波读低(GH #473 乙)**:W44 下界 0.765(已在常数外),W45 给 0.943/0.673、均值 **≥0.808**(修正 ~0.863)= **高 ~16%**;**失效方向是危险那一侧**(单波价记低 ⇒ 围栏读出比实际更多的余量,与「滞后让 MTD 偏低」同向叠加),**而它身上没有自动失效条款** —— 那个缺口就是 #473 乙。本波实测总额 **~$1.68–1.80 vs 登记 ~$2.00 ⇒ 登记值偏高,朝安全侧失效**。
  **八、发波:闸 (i) 明确 THROTTLED,本轮不发。** ⚠️ 先按 `aws/wave_throttle.py` 调用得 `WT_EXIT=2 / No such file` —— **那个 2 是「没跑成」不是「没锁」**;工具实际在 `tools/batch_test/soak/`,改对路径重跑。09:17:46Z:`anchor wave W45`、`last machine up 06:30:52Z <- the anchor`、`unlock 12:30:52Z`、`margin 11586s SHORT`、**`WAVE_THROTTLE: THROTTLED (exit 3)`**(`WT_EXIT=3` **裸读 `$?`**)。⭐ 锚点落在 `machines[]` 的 06:30:52Z 而**不是**补跑机的 07:12:58Z ⇒ **「补跑机不开新的闸 (i) 窗口」这条按 schema 结构性成立,本轮是它第一次被真的行使。**
  **九、泄漏:干净。** 全状态实例 `[]`、`describe-volumes` `[]`(**无游离 EBS**)、`open,active` spot 请求 `[]`;常驻成本只有 AMI。**W45 六台全部自毁清理完毕,零残留。**
  **十、队列**:W45 是**家族级读数不是逐 id 裁定**,`suggested` 只是提示 ⇒ **本轮不翻动任何 id 的 queue 状态**(含 `strategy-40` `arbheart` / `strategy-41` `slotwait`);promote/reject 走三条件,归总监。
  **十一、铁律 6/11**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`,未坐等。
  **交棒**:① ⭐⭐⭐ 总监 —— **新 `[batch]` issue:按需补跑机死因码按构造失明 + SIR 比 EC2 耐久**(六)。② ⭐⭐ 总监 —— **GH #473 乙 第二个数据点**(七 c)。③ ⭐⭐ 总监/英雄组 —— **trunk 红本轮未复核,不声称已修**(一)。④ ⭐ 总监 —— **#352/§CT 胜负通道连续第四波 DEGENERATE**,已不像单波偶然(五)。⑤ ⭐ **#454 仍欠一波跑完的 spot**(七 b)。⑥ ⭐ 存量:#449 / #473 甲 / #472(W44 那 48 份 pudge `.dem` 约 09-25 过期)/ #427 / #408 / #420 / #290 / `strategy-5b`。
  **下一轮本台 = 视闸 (i)(`2026-09-04T12:30:52Z` 解锁)发 W46**;解锁前触发则**只巡检不发波**。**预登记两条(写在任何数据落地之前)**:(a) **W46 臂串必须从发波树重算**,章程计划句不是臂串(W45 就是这么抓到 `+arbheart +slotwait` 的);(b) **若 W46 是一波跑完的纯 spot,它就是 GH #454 等的那一波**,账单侧单波成本要**当场对 `$0.90` 结账并触发/不触发自动失效条款**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T091500Z.md` 与 `iterations/reports/batch-desk/waves/W45_wave.json`。

- 2026-09-04T12:15–12:5xZ(**发 W46 = 62-id 家族第一波,spot 四台。本轮真正的产出不是那四台机器,是闸 (iv) 第一次拒绝跑 —— 而它拒绝的方式是拦住一次发波,因为写坏它输入的那一轮读上去是干净的**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒(**同形第 7 次,又是当轮第一条命令**);改重定向重跑得 `EXIT=3`(**裸读 `$?`**)= **有发现**。`exit sources`(GH #267 4b)四项:`cadence / owed-executions / trunk-red(python) / trunk-red(lua)`,`UNCERTIFIABLE: none`,**不写「全是 cadence」**。锚点 stable-v1/v2 `EXISTS/PINNED/SHIPPED` 全 ok;`unlanded_commits` 无 OFF-TRUNK;`OWED` 一行 `campgrade_wave_type_premise_reading`(GH #480/#241,等的是 `campgrade` 独占波,**此刻还不存在,本波不是它**)。⛔ **python 腿仍红**(`tests/test_bots_walk_farm_only.py`,第三轮点名,**不声称已修**)。
  **二、⭐⭐ trunk 红(Lua),而且红在本波刚 armed 的那个 id 上**:`test_coarmed_attribution_register.lua:407` 报 **`creepthink > campbind` / `pullcad > campbind` / `pullthink > campbind` 三条新的 co-armed 合取未登记**。`campbind` 正是总监 10:xxZ 入集的两个新 id 之一(臂串 60→62),也就是 W46 第一次花钱测的两个之一。调用形状本台亲手核过(不是从检测器输出推的):helper `J.GetCampPullPokeTarget` 在 `bots/FunLib/jmz_func.lua:8993`,被读在 `bots/mode_roam_generic.lua` 的 poke 现场,外层是 `pullthink`(:264/:412)、`creepthink`(:265)、`pullcad`(:343)三个 armed id 的门内 ⇒ **外层 id 的逐 id (a) 从此测的是 `outer AND inner`**。⚠️ **不是 `pullcad` 陷阱的重复**:`campbind` 作者明确避开了合取门(helper 头逐字写着 `Gated STANDALONE`),**这条红说的是嵌套不是合取,两个缺陷同名不同形**。已开 **GH #486** 交协同组/总监;**本台不改 `test_set.md` 入集节、不撤 id**。W46 是**家族级**读数不受影响,但已预登记:**任何拿 W46 说 `creepthink`/`pullcad`/`pullthink` 逐 id (a) 的话,先看这条。**
  **三、成本**:九月 MTD **`$14.981`**(免费 `budgets`,`refreshed 11:29:50Z`)。⚠️ 该快照距 W45 关机 ~3.5h,**仍在 4.3–11.3h 滞后带内 ⇒ 按构造不能假定含 W45**。围栏(铁纪律甲,逐波按自己市场取价)= `14.981 + W44 按需 2.15 + W45 spot 0.90 + W45 按需补跑 1.10 + W46 spot 0.90 = $20.03` vs `$45/$80/$90/$100` ⇒ **闸 (iii) 通过,九月未及第一档 $50 告警,无跨档义务**。⛔ **未用被禁的「MTD 增量对得上波费」手法**(乙)。**(a) W44 对账第四次顺延**(快照距 W44 关机 ~10.5h,仍在带内)。
  **四、⭐⭐ 头号发现:闸 (iv) 在 W45 上没跑成,而「没跑成」只在下一次发波时才现形。** `reclaim_blind.py --wave-json W45_wave.json` 打 `RB_EXIT=2 / UNDECIDABLE: seed 3968: leg count 'ab' is not an integer`。**`exit 2` 是「什么都没检查」不是「通过」。** 原因不在工具:上一轮收割把**两台被回收机器**(3968/3927)的 `machines[].ab/ba` 写成 `null`,把该粒**并池后**的腿数放到了 `rerun[]` 行上;而工具 `reclaim_blind.py:423` **只取 `machines`,根本不读 `rerun`**,`machines[]` 装的是**那一台自己的**腿数。**先例逐字对得上**:W42 同样「一台被抢占 + 一台补跑」却 `exit 0`,因为它的 3040 写的是自己的 `ab10/ba0`(`NO-PAIR`);本轮把 W42/W44 一并重跑**都是 0,只有 W45 是 2** ⇒ 不是工具漂移,是那一轮的登记。⭐ **立案句不是那两个 null**:上一轮读上去**完全干净**(四粒全过门、四量齐全、零泄漏),**没有任何东西举手**说这两个 null 会让下一道闸瞎掉 —— 闸 (iv) 是**输入侧**的闸,检查的是**上一波**,于是「上一波登记写坏了」**结构上只能由下一个真要发波的轮次发现,而发现方式是拦住一次发波**。与 GH #469 **同族但反向**(那条是**没有检查**,这条是**有检查、输入由上一轮写、上一轮自己不跑它**)。
  **四·1、回填是量出来的不是按常数假设的**:⛔ 不能写「37.87 min < 40.0 min ⇒ 没翻腿」——changeover 实测夹逼 `34.80 < x <= 40.63`,**37.87 落在区间内,单凭常数判不了**。改用 S3 原始事实:**3968 原机 35 个不同对局 tag,最后一局开始于 `06:57:27Z` = 起飞后 26.8 分钟,低于夹逼下界 34.80** ⇒ **区间内任何 changeover 值都不会把任何一局推过翻腿点** ⇒ `ab=35/ba=0` **是测出来的**(登记里写明 35 是**已落盘**不是**计分**局数);**3927 原机落盘零** ⇒ `ab=0/ba=0`。回填后 `RB_EXIT=0`:`yield 2 paired of 4 machines` / `2 reclaimed before the flip` / `VERDICT: not blinded` / **`NEXT WAVE: spot`**。⚠️ **一处不许含糊(已请总监裁死)**:`yield 2` 是**原机**层面;W45 最终四粒全配对是**补跑机**买回来的,工具按 #271 **只看原机**(它问「采集有没有被回收致盲」不是「最终有没有数据」)——**两个读数都对,问的不是同一个问题**。
  **五、闸 (i):在动作时点被真的行使了一次,而且拦住了本台。** 启动脚本第一次跑:`LS_REMOTE_TIP` 核对通过 → `wave_throttle.py` 打 `unlock 12:30:52Z / now 12:24:12Z / margin 400s SHORT / BREACHES gate (i) by 6m40s / THROTTLED (exit 3)`,脚本 `exit 8`,**一台没发**。⭐ **这次不是形式主义**:本台对墙钟的心算**偏了约 25 分钟**(以为已 12:5xZ,实际 12:24Z)——**正是 GH #469 立案时描述的那个机制**,只是偏的方向让本台**以为早就解锁了**。**闸 (i) 可执行化之后的第一次真实拦截**;章程里那个手打的解锁时刻本轮**没被读**,读的是工具那一行。⭐ 顺带工具对新写的 `W46_wave.json` 打 `skipped W46 (no machines[] -- opened no window)` ⇒ **「还没上机的波次不开窗」按 schema 结构性成立**。
  **六、发波:W46 = 62-id 家族第一波,spot 四台,~$0.90(估)。** 臂串**从发波树 `efa7ba70` 亲手重算**(不是抄章程计划句)= **62 id / 550 字节 / md5 `c7e1f92c739f8f8dc0ee0c9484288628`**,对 W45(`60/530/eef5fb2e…`)集合差 **`+campbind` `+zusboltdom`,无删除** ⇒ **上一轮预登记的 (a) 又一次兑现**(章程只说「发 W46」,重算把它钉成**新家族**);与总监 10:xxZ 入集裁定登记的 `62/550/c7e1f92c…` **逐位相同**。漂移两步走认退出码:`git fetch --depth 1 origin 27165f8c…` → `git log … -- bots/ game/` `LOG_EXIT=0` 两个 commit(`a2762804` gated `zusboltdom`、`dd19aa70` 文档锚);HEAD `efa7ba70` 只动 `tests/`+`iterations/`(`git show --stat` 核过)。**⇒ 不与 W45(60)/W44(58)/W41∪W42(57)任何一个并池。** 选种:`--build` **在选种之前**跑(`ROSTER_EXIT=0`,341 前缀、205 粒、21278 局),窗口**右移**到 `[4201,4600]`(W45 在 3892–3968),400 粒未用、popcount `{0:39,1:110,2:128,3:90,4:27,5:5,6:1}` ⇒ **无组合学耗尽**,8170 组合法四元组;选 **4209/4401/4470/4252**(pudge 平局判据同 W45,4 粒里 3 粒,**是解空间内部的免费判据不是门的改动**)。四道程序化门:`CT_EXIT=0`(62 id = 12 hero/50 generic/**0 unresolved** ⇒ **7 term,与 W43/W44/W45 同一个 term 集**)、`CARRIER_GATE_EXIT=0`(min-per-term **2**,7 个全满足)、`ARM_EXIT=0`(**62 个全 wired on HEAD**)、`LUACHECK_EXIT=0`(`0 warnings`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**)。队列(步骤 4a)复核:**没有任何请求要专波**,ADMITTED 的全是「搭车、零 AWS 增量」⇒ **例行全集波就是对的波**。
  **六·1、起飞**:`WT_EXIT=0`(`now 12:31:13Z / margin +21s past unlock`)后四次独立调用、**每次显式一个 `--az`、四次四个不同**:`4209 i-0430f3a3b2e759c31 2a→2a 12:31:15Z` / `4401 i-03fe1ddddfd201f80 2b→2b 12:31:20Z` / `4470 i-0642a8c11a626ce8d 2c→2c 12:31:25Z` / `4252 i-085a71f7c65b9acba 2d→2d 12:31:30Z`,四台 `SPOT_RUN_EXIT=0`。**`InstanceLifecycle = spot` 四台全核过**(`run_id` 的 `spot_` 前缀不是证据);**零 `InsufficientInstanceCapacity` ⇒ 降级阶梯一步没进**;**GH #252 验收干净**(四 AZ 互不相同、requested==actual);**GH #256 验收干净**(`re-aiming` 与 `AZ RING EXHAUSTED` 两行一行没出现)。预期 ~96 计分局;**14 分钟内 `OBJ_COUNT=0` 是预期值不是症状**(W42 量到首个对象落在起飞后 ~14.5 分钟)。
  **七、泄漏与点名:干净。** 点名两条都跑了:`pending,running` 的 `soak-run` 标签**恰好四个、两两不同、与 `machines[].run_id` 逐一对上**(不多=无泄漏,不少=无掉臂);`terminated,shutting-down` **空**——⚠️ 这个空是**「还没有任何一台死掉」**(波 ~1 分钟大)**不是「记录过期」**(GH #375)。四个卷 = 这四台的根卷(非游离),四个 SIR `active/fulfilled` 一一对应。常驻成本仍只有 AMI。
  **八、预登记两条(写在任何数据落地之前)**:(a) **胜负通道已连续四波 `DEGENERATE`**(W41/W42/W44/W45;W43 是沉默不算)——W46 若第五次,`winrate` 与 `comps_better` 的 winrate 行**一律不得写成读数**(GH #352/§CT);若 `RECOVERED`,**那本身就是发现**。(b) **GH #454**:W46 是**纯 spot** 波,**若跑完**,收割轮必须**当场**把账单侧单波成本对 `$0.90` 结账并明写自动失效条款**触发/不触发**(W45 结不了:两台被早回收,2.554 机时 vs W29 干净 3.424)。
  **九、队列**:本轮**不翻动任何 id 的 queue 状态**(W46 是家族级读数,promote/reject 走三条件、归总监)。
  **十、铁律 6/11**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态门 `LUACHECK_EXIT=0`;**未用 `RULE6_BYPASS`**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`,**无空转等待**(闸 (i) 的 6 分钟窗口用于写 `W46_wave.json` 与报告,不是干等)。
  **交棒**:① ⭐⭐⭐ 协同组/总监(**GH #486**)—— **co-armed 登记册红在 `campbind` 上,W46 已在飞**,逐 id (a) 的解释权卡在这条(二);② ⭐⭐⭐ 总监(**GH #487**)—— **闸 (iv) 的结构缺口 + `yield` 语义请定名**(四);③ ⭐⭐ **本台下一轮必做** —— 收割 W46:`machines[].ab/ba` 填**每台自己的整数**(落盘零填 `0/0`,**不许留 null、不许填并池值**)、臂串从 `efa7ba70` 重算对 `62/550/c7e1f92c…`、**GH #454 当场结账**、照抄 `winrate_channel` 与 `mean.winrate_headroom`;④ ⭐⭐ 总监/英雄组 —— **python 腿仍红**(`tests/test_bots_walk_farm_only.py`),**第三轮点名,不声称已修**;⑤ ⭐ 总监 —— **W44 对账第四次顺延**,按构造结不了;⑥ ⭐ 存量:#480/#241(`campgrade` 独占波尚不存在,本波不是它)/ #473 甲乙 / #472 / #427 / #408 / #420 / #449 / #290 / `strategy-5b` / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(18 天)**。
  **下一轮本台 = 收割 W46**(闸 (i) 解锁 `2026-09-04T18:31:30Z`,**读工具那一行,不要读这个手打的数**)。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T121500Z.md` 与 `iterations/reports/batch-desk/waves/W46_wave.json`。
- 2026-09-04T15:15–15:5xZ(**收割 W46 = 62-id 家族第一波。本轮零发波(闸 (i) 18:31Z 才解锁)、零新增支出、零泄漏。真正的产出是两件,而且两件都是「本台自己的登记出的问题,由本台自己发现」**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒;改重定向重跑得 **`EXIT=3`**(**裸读 `$?`**)。`exit sources`(GH #267 4b)九腿:`FINDINGS: cadence / owed-executions / trunk-red(python)`,`UNCERTIFIABLE: none`,`NOT RUN (inside a leg): test_rc_wrapper.py test_selfcheck_lua_leg.py`,**不写「全是 cadence」**。Lua 检测器腿**绿**(78 文件 0 失败,**但那是 FAST SUBSET**)。锚点 stable-v1/v2 全 ok;`unlanded_commits` 无 OFF-TRUNK;`OWED` 仍是 `campgrade_wave_type_premise_reading`(#480/#241,`campgrade` 独占波**仍不存在**)。⛔ **本台不给管道复发编号** —— 章程 00:15Z 写「第二十二次」、12:15Z 写「第 7 次」、脚本写「recurred 5x」,**三个数互不相容**,查清前不该往下传(已交总监定名)。
  **二、⭐⭐ 头号产出:GH #454 结账,自动失效条款触发。** W46 是 #454 从 08-30 起等的那个东西 —— **一波跑到完的纯 spot 波**(四台 `market=spot`,四个 SIR 全 `instance-terminated-by-user` 自毁,**零回收**;W45 结不了是因为四台里两台早回收)。方法与 §CO.5 给 W29 定 `$0.90` 时**同一个**:逐台运行时长 × 该 AZ spot 费率(**不是账单行,bill-line 对账要等滞后带外的 CE 读数,本轮没有也不声称**)。⚠️ 时长给的是**区间**:EC2 记录已过期(`Reservations: []`,GH #375),下界 = 该台最后一个 S3 对象,上界 = SIR 关闭(逐台差 2m40s–3m54s)⇒ 机时 **3.385–3.607**。费率(`describe-spot-price-history`,窗口内无变价):2a `0.2959` / 2b `0.2740` / 2c `0.2967` / 2d `0.2474`。⇒ EC2 **$0.9422–$1.0037** + IPv4 $0.017 = **单波 $0.9591–$1.0217 vs 常数 $0.90**。⭐ **结论不依赖区间取哪端:下界也超,只算 EC2 也超**;要落回 $0.90 以下,这波得比它自己最后一次 S3 上传所证明的再短跑 ~22 分钟。**根因是价格不是时长** —— 机时 3.385–3.607 **夹住** W29 干净的 3.424(波是同样大的),而章程价带 `$0.231–0.250/h` 被**四个 AZ 里的三个**顶穿。**失效方向是危险的那一侧**(常数偏低 ⇒ 每条 spot 波在围栏里少记 ~$0.10–0.12,**与 budgets 滞后同向叠加**),**但条款起作用了**:它触发在**第一波能检验它的波**上,不靠谁记得复查。量级要说清:**这是「请总监重裁常数」的发现,不是刹车事件**。⇒ **总监下一次触发必须重裁本常数**(建议连价带一起重取)。
  **三、成本**:九月 MTD **`$14.981`**(免费 `budgets`,`refreshed 11:29:50Z`)—— ⚠️ **与上一轮逐位相同的那一张快照**,比 W46 起飞还早 1h(**按构造不含 W46**),距 W45 关机 ~3.5h(**仍在 4.3–11.3h 带内,也不能假定含 W45**)。围栏(铁纪律甲)= `14.981 + W44 2.15 + W45 0.90 + W45 补跑 1.10 + W46 0.90 = $20.03` vs `$45/$80/$90/$100` ⇒ **过**,九月未及第一档 $50 告警。⛔ **未用被禁的「MTD 增量对得上波费」手法**(乙)。**(a) W44 对账第五次顺延**(按构造结不了)。
  **四、收割 W46:四粒全过门,`RV_EXIT=0`。** ⭐ **一处路径事实**:章程步骤 3 散文写的是扫 `validation/`,而**逐局数据在 `soak/`** —— 按 `validation/<run_id>/` 递归 ls **四个 run 全返回 0 对象**,**那个零是找错前缀的产物不是空波**;按「空输出不当事实」查 `soak/` 才拿到 230 个文件。语料 230 `*.analysis.json`(48/56/62/64),`files_seen 230 / games_loaded 230 / unparseable 0 / pooled_basename_dirs_overridden 0`,**三个 override 一个没用 ⇒ 无「这是 SKIP 不是 pass」的行要抄**;计分 **206**,差额 **24 = 每台 6 局热身**(无 seed/leg 戳)。臂串**从发波树 `efa7ba70` 亲手重算** = **62/550/`c7e1f92c…`**,与登记逐位相同 ⇒ **不与 W45(60)/W44(58)/W41∪W42(57)并池**。GH #269 门:`min_arm_depth 8`、`thin_arm_seeds []`,**四粒全计分**(arm_depth 17.14/21.76/23.17/26.71),**无排除、不欠点名**。四量(每粒先 swap-average 再跨种子算术平均,**永不按局加权**):gpm **−20.26** / xpm **−19.53** / deaths **+0.38** / lh **−1.73**;`comps_better` gpm 2/4、xpm 1/4、deaths 1/4、lh 2/4;逐粒 gpm `4209 +56.98`/`4252 +5.07`/`4401 −64.56`/`4470 −78.53`(极差 135.5)。`suggested hold_or_reject` **只是提示**,promote/reject 走三条件归总监,且**这是家族级读数不是逐 id 读数**。两分层读数(工具自己打的 `strata`,**未手算**):gpm ab `153.38`/ba `−193.90`、xpm `108.54`/`−147.60`、deaths `−1.57`/`+2.34`、lh `2.72`/`−6.18`,**四个量全反号,全登记**(那是 (i-a));**不是否决理由也不是精度主张** —— 侧偏已消除的估计量上反号 ⟺ `|side|>|arm|`,**恒等式不是诊断**((i-c))。**胜负通道 `DEGENERATE`,第五次**(少数侧 28/230,share `0.1217`;`mean.winrate_headroom 0.2188`)⇒ **上一轮预登记的 (a) 分支兑现**(W41/W42/W44/W45/W46,W43 是沉默不计),**`winrate` 与 `comps_better` 的 winrate 行一律不写成读数**(#352/§CT);逐粒侧别普查给出了原因:`4470` **0 radiant/56 dire**(headroom **0.0**)、`4252` **1/57**。**逐 id 解释权仍卡 GH #486**:拿 W46 说 `creepthink`/`pullcad`/`pullthink` 逐 id (a) 的话测的是 `outer AND campbind`。
  **五、⭐⭐ 第二个产出:闸 (iv) 在本台自己的登记上又红两次,而这次是本台自己发现的。** 上一轮开 GH #487 说「输入侧的闸只能由下一个发波轮发现,发现方式是拦住一次发波」;本轮把它**反过来用** —— 填完记录**当场对自己这一波跑闸**。它在给 0 之前给了**两次 `RB_EXIT=2`**:(1) `unknown SIR status_code None`(`machines[]` 没有 `status_code`);(2) `needs survival_min, or both create and update`(有 `launched_at`/`terminated_at`,**但不是工具读的键名**)。两处从 SIR 补齐后 **`RB_EXIT=0`**:`4 paired seed(s) of 4 machine(s) / 0 reclaimed before the flip / VERDICT: not blinded / NEXT WAVE: spot`。⭐ **立案句不是那两个字段**:上一轮的 `harvest_obligations` **只点名了 `ab`/`ba`**;若本轮只把交到手上的义务做完,记录会读上去**完整而诚实**,而**下一个发波轮照样吃一个 `RB_EXIT=2`**,红在两个谁都没想到要点名的字段上。⇒ **修法不是把义务清单写更长**(清单只能枚举已见过的失效),**是「写记录的那一轮必须自己把闸跑一遍」**。已追评 #487。
  **六、`tests/test_wave_gate_keys.py` 是本台自己的红,本轮修掉(`WGK_EXIT=0`,230 checks 0 failed)。** 三个 FAIL 全在本台波次记录上(W45 缺 `(iv) inputs` 与 `(iv) reclaim_blind`,W46 缺 `(iv) inputs`)。按该文件**自己的**修法逐字(「honest value — `UNCERTIFIABLE` if it did not run；**Do NOT invent a value to turn this green**」)填 **`UNCERTIFIABLE`**:W46/W45 的 inputs 半**都没跑**(发波轮的闸键里没有这一半);W45 的 `reclaim_blind` **在 W45 发波时没跑**(那轮 (iv) 是 carrier、(v) 是 wiring)。⚠️ **两处刀口写死在记录里**:(甲) W46 的 inputs 半「闸没跑」与「12:31Z 时 W45 确已在 09:15Z 收割完、条件成立」**是两句话,只坐实第二句**,且**明标为事后重建不是那道闸**;(乙) W45 的 `gate_iv_backfill_note` 说的是 **W45 作为被检查对象**(W46 轮对它跑闸并回填腿数),与「W45 自己那轮跑过闸」**是两句话,只坐实第一句**。⚠️ 顺带一句该说的:**这个检测器在 `UNCERTIFIABLE` 上转绿,结构上分不清「闸跑了且过了」与「闸没跑」** —— 它自称是**记录完整性**测试不是**闸跑没跑**测试,不是缺陷,但读它的绿时要知道绿的是什么。**本台不改该文件判据。**
  **七、`machines[].ab/ba` 已按义务填「每台自己的整数」**,且**两条独立路径互证**:(1) `recover_verdict.py` 的 `per_seed ab_games/ba_games`;(2) 直接从该 run 前缀下的落盘 analysis 文件重导(每个前缀**只有一粒种子**、每局恰好一条腿)。两者一致 ⇒ `84e984/4209 ab30 ba12`、`d21f34/4401 ab34 ba16`、`d7082b/4470 ab34 ba22`、`b77771/4252 ab42 ba16`。**未留 null、未填并池值。**
  **八、泄漏:干净。** `pending,running` **0**、卷 **0**(无游离根卷)、`open,active` SIR **0**(四个全 `closed`/`instance-terminated-by-user`)。常驻成本仍只有 AMI。⚠️ `describe-instances --instance-ids` 对四台返回 `Reservations: []` —— **EC2 记录已过期(GH #375)**,这是上一轮登记的对偶「SIR 比 EC2 记录活得久」**本轮被正着用了一次**:停机时刻与状态码全部取自 SIR。
  **九、本轮不发波,卡在闸 (i)**:W46 起飞 `12:31:15Z` ⇒ 解锁 **`2026-09-04T18:31:15Z`**,本轮 15:1x–15:5xZ,**还差约 3h**。**⚠️ 这个手打的数不是判据**,下一轮**读 `wave_throttle.py` 那一行**(上一轮正是心算偏 25 分钟被它真的拦下过)。**卡在 (i),不是卡在钱也不是卡在没东西测 —— 不发波、不空转等待**(铁律 7/11)。
  **十、铁律 6/11/队列**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态门 `LUACHECK_EXIT=0`(`luacheck bots game: 0 warnings`,`GATE_EXIT=0 CLEAN`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`,**无空转等待**。**队列**:本轮不翻动任何 id 的 queue 状态。
  **交棒**:① ⭐⭐⭐ 总监(**GH #454**)—— **自动失效条款已触发**,W46 账单侧 **$0.959–$1.022 > $0.90**,**区间两端都超**,根因是**价带陈旧**,**下一次触发必须重裁本常数**;② ⭐⭐⭐ 总监(**GH #487**)—— 闸 (iv) 缺口本轮**又复发两次、在两个新字段上**,请裁「**写记录的那一轮必须自己跑一遍闸**」是否升硬规则;③ ⭐⭐ 协同组/英雄组 —— **GH #490** —— `tests/test_detector_source_constants.py` **红在 trunk**:`J.ShouldTpSupportTowerFight -> /GetUnitToUnitDistance\(\s*bot,\s*building\s*\)\s*>\s*N/: expected exactly 1 match, found 0`(`source_constants.py:193`),**本台不改 `bots/`**;④ ⭐⭐ 总监(**GH #486**)—— co-armed 登记册仍红在 `campbind`,**W46 已是第一波为它花过钱的波**;⑤ ⭐ 总监/英雄组 —— `tests/test_bots_walk_farm_only.py` 本轮**不在 FAIL 名单里**,但**本台不声称它已修**,同时**两个新红上桌**;⑥ ⭐ 总监 —— **W44 对账第五次顺延**;⑦ ⭐ 章程行文 —— 步骤 3 的收割前缀 `validation/` 与实际的 `soak/` 不一致(四),**本轮只点名不擅改步骤节**;⑧ ⭐ 总监 —— 管道误用**计数器该定名**(三个数互不相容,见一);⑨ ⭐ 存量:#480/#241 / #473 甲乙 / #472 / #427 / #408 / #420 / #449 / #446 / #290 / `strategy-5b` / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(18 天)**。
  **下一轮本台 = 视闸 (i) 发 W47**(约 `18:31Z` 解锁,**读工具那一行,不要读这个手打的数**);若发波,臂串**从发波树重算**,并按(五)的教训**当场对自己跑一遍闸 (iv)**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260904T151500Z.md` 与 `iterations/reports/batch-desk/waves/W46_wave.json`(`harvest` / `cost_settlement_gh454` / `gate_iv_self_verification` 三节)。
- 2026-09-04T18:15–18:4xZ(**发 W47 = 62-id 家族第二波,spot 四台,把家族做到 8 粒。本轮真正的产出是三件闸上的边界,而三件都不是那四台机器**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,自检自拒;改重定向重跑得 **`EXIT=124`(裸读 `$?`)= 超时**,用尽在最后一条腿 `trunk health (python test suite)` 上 ⇒ **trunk 的 python 那一侧本轮没人看过,本台既不声称红也不声称绿**(按 GH #171:124 不是通过)。跑完的腿:锚点 stable-v1/v2 `EXISTS/PINNED/SHIPPED` 全 ok;`unlanded_commits` 无 OFF-TRUNK;`OWED` 仍是 `campgrade_wave_type_premise_reading`(#480/#241,**`campgrade` 独占波仍不存在,本波不是它**);`ORPHAN_PROPOSAL` none;`expired waits` 无。⛔ **管道复发仍不给编号**(三个数互不相容,上一轮已交总监,未裁)。
  **二、⭐⭐ 头号产出:载体门的 term 集自己从 7 变成 8,而 armed 串一字未动。** `CT_EXIT=0` 打 `13 hero-scoped / 49 generic => 8 term(s)`,多出来的是 **pudge**;而 armed 串**当场重算** = **62 / 550 / `c7e1f92c…`**,与 W46 **逐字节相同**,`mode_roam_generic.lua` 自 W46 一行未动。差在**工具**:**总监 16:1xZ 落地 GH #473 甲**(`6ea528a8`,`carrier_terms.py`),于是 **`rotscope` 从 generic 变成 `hero pudge`**(`mode_roam_generic.lua:1039`)。⭐ **两个后果都要登记**:① **W46 的 7-term 读数是旧工具下取的,与本波的 8-term 读数不能逐 term 对比** —— 两次「min-per-term=2」不是同一句话;② **选种解空间被量化地收紧**:8 term × ≥2 载体 ⇒ 4 粒牌需 ≥16 个 term 槽,而窗口 `[4601,5000]` 最好的四粒只供得起 19(popcount 分布 `{0:26,1:105,2:145,3:91,4:31,5:1,6:1}`);对 popcount≥3 的 124 粒穷举 **C(124,4)=950 万**四元组,**可行解只有 471 个**。**这不是耗尽(门过了),是下一个 term 出现时该盯的那个数。**
  **三、成本(闸 (iii))**:九月 MTD **`$14.981`**(免费 `budgets`,`refreshed 11:29:50Z`)—— ⚠️ **连续第三轮读到逐位相同的同一张快照**,比 W46 起飞早 1h(**按构造不含 W46**),距 W45 关机 ~6.5h(**仍在 4.3–11.3h 带内**)。⭐ **围栏改用 W46 账单侧实测上界 `$1.02` 给两条 spot 波计价,不用章程常数 `$0.90`** —— GH #454 的自动失效条款上一轮已触发而总监未重裁,**取高值的失效方向是安全那一侧**:`14.981 + W44 2.15 + W45 0.90 + W45 补跑 1.10 + W46 1.02 + W47 1.02 = $21.15` vs `$45/$80/$90/$100` ⇒ **过**(照旧用 $0.90 是 $20.93,两个都过)。⛔ 未用被禁的 (乙) 手法。**(a) W44 对账第六次顺延**。
  **四、收割:本轮零收割。** S3 `soak/` 自 W46 那四个 run 之后**无新对象**;W46 上一轮已收干净。上一波实测局数:**230** 份 `analysis.json`(48/56/62/64),**计分 206**,差额 24 = 每台 6 局暖场。
  **五、闸 (ii) 走第三支,而这一支不依赖那个未裁的分叉。** (一) 串不动 ⇒ 不成立;(三) 62-id 家族现 **4 粒 < 8** ⇒ **成立,决定性**;(二) **按实测不成立**:`git log efa7ba70..HEAD -- bots/ game/`(`LOG_EXIT=0`)只给**一个** commit `da79f6bc`(`midsupyield`/GH #489),`game/` 零 diff;整个 diff 是 `jmz_func.lua` 59 行,两处:(a) `J.HasAvailableSupportResponder` 加参数加合取项,**唯一调用点 `:8489` 坐在 `IsSoakCandidate('midsupyield')` 门内**(整棵 `bots/` grep:一个调用点 + 一处注释),而 **`midsupyield` 不在 62-id 串里**;(b) `ShouldTpSupportTowerFight` 的字面量 `3500` → `J.TP_RESPONSE_FAR_FLOOR`,该量在 `:8332` **加载期赋值 = 3500** ⇒ **门外唯一改动是保值替换** ⇒ **树漂移对 armed 配置惰性**。⭐ 顺带查清 **GH #490 那条 trunk 红的因果就是这次替换**(检测器正则要一个数字,那里现在是常量名),**本台不改 `bots/` 也不改判据**。⚠️ **GH #363/#449 分叉第五次上门**(串不动而树动)**但不咬** —— 第三支的成立与取哪种读法无关;**分叉仍欠裁定**。
  **六、选种**:`seed_roster_index.py --build` **选种前**跑(`ROSTER_EXIT=0`;345 前缀 / 新扫 4 个 = W46 自己那四个 / 索引 **209 粒 / 41 英雄**)——**不 build 就会把 W46 刚烧掉的四粒当「从未用过」献回来**。窗口右移 `[4601,5000]`(400 粒全未用),选 **4723 / 4678 / 4763 / 4950**;`pudge` 平局判据同 W45/W46(**4 粒里 3 粒**,维持 `rotscope` 条件 (a) 帧级通道;#472 / `strategy-26`)——**门自己解空间内的免费平局判据,不是门的改动**。
  **七、五道门(全部本轮亲手跑,裸读)**:`CT_EXIT=0`(8 term)、**`CARRIER_GATE ids=13 seeds=4 exit=0`**(cm 3 / pudge 3 / 其余各 2,**min-per-term 2,8 term 全满足**)、`ARM_EXIT=0`(62 个 armed id 全 wired)、**`RB_EXIT=0`**(对 **W46** 跑:`4 paired of 4` / `0 reclaimed` / `not blinded` / **`NEXT WAVE: spot`**,**重跑不是沿用散文**;⚠️ 四行全打 `[ab/ba only (no arm_depth given)]` ⇒ **depth-checked 路径未被走到**,W47 记录已把 `arm_depth` 预置为键并写进义务)、`LUACHECK_EXIT=0`(`0 warnings`,`GATE_EXIT=0 CLEAN`)。**步骤 4a 队列**:74 行 / 25 pending,**无任何 pending 行申请会被本波挤掉的专波**(逐行扫「独占/solo/dedicated/专波」,命中的要么已结要么自写「搭车不申请专波」);全集波本就是 owner 2026-08-22 的默认波型。**本轮不翻动任何 queue 状态。**
  **八、闸 (i) 被行使四次,拦了四次,第五次才开。** 章程上一轮手打的解锁时刻是 `18:31:15Z`,**本轮没读它,读的是工具那一行** —— 工具锚在**锚定波最后一台**上给 **`18:31:30Z`**,**比手打的数晚 15 秒 ⇒ 照手打的数发波会真的破闸**。四次全 `exit 3` 全裸读:`18:13:57Z`(短 1053s)、`18:26:47Z`(短 283s)、**`18:29:13Z`(短 137s,在启动脚本内部,脚本 `exit 8`,一台未发)**、脚本内 30s 轮询四次仍 3,**`18:31:45Z` → `UNLOCKED (exit 0)`**,第一台 `18:31:48Z` 出。⭐ **那 ~18 分钟没有空转**(铁律 7/11):自检、成本、收割核查、五道门、950 万四元组的选种穷举、波次记录、`test_wave_gate_keys.py` **全在窗口内做完,零成本**;真正的「等」只是脚本里对同一道闸的 30s 轮询,**不是绕过**。顺带工具对 `machines[]` 还空着的 `W47_wave.json` 打 `skipped W47 (no machines[] -- opened no window)` ⇒ **没上机的波不开窗,按 schema 结构成立**。
  **九、起飞**:四次独立调用,每次显式一个 `--az`,`--slots 16 --hours 2 --rec-slots 8 --ref 15b0d9fd…(40 位) --validate "<62-id 臂串> <种子> --games 12"`,采集配置与 W39–W46 **逐字相同**。`4723 i-067c5f462a7d2933d 2a→2a b8fa5b 18:31:48Z` / `4678 i-0f9b588e467660385 2b→2b 1bfa3d 18:31:50Z` / `4763 i-0f6fb96cd27e0b01f 2c→2c b30fe4 18:31:53Z` / `4950 i-008ac3bde84e299cb **2d→2b(re-aimed)** 9782b8 18:32:27Z`,四台 `SPOT_RUN_EXIT=0`,SIR `ivtqk2fq/97qfgvrm/zh3zjean/5fczhafq`。**市场**:`InstanceLifecycle=spot` **四台全核过**(`run_id` 的 `spot_` 前缀是硬写字符串,**不是证据**)。预期 ~96 计分局 + 暖场;**14 分钟内 `OBJ_COUNT=0` 是预期不是症状**。
  **十、⭐⭐ 第二个产出:GH #252 本波失败,而失败的形状正是 GH #460 点名要在 spot 波上重看的那个。** 日志逐字:`launch in us-west-2d failed -- InsufficientInstanceCapacity` → `re-aiming inside the ring -> us-west-2a` → **`us-west-2a` 也 `InsufficientInstanceCapacity`** → `-> us-west-2b` → `PLACEMENT MISMATCH requested=us-west-2d actual=us-west-2b re-aimed=yes`。⇒ **#252 验收失败**(实得 2a/2b/2b/2c = **3 个去重** + 一处 mismatch);**#256 的 `re-aiming` 触发两次,`AZ RING EXHAUSTED` 未出现**;**#460 (丙) 的 `AZ SPREAD COLLAPSED`(去重 < 2)未达成**(是 3)。⭐⭐ **该登记给 #460 的硬事实是这一条**:**`us-west-2a` 在 `18:31:48Z` 成功交付第 1 台,39 秒后对第 4 台「没有 spot 容量」** ⇒ **一波之内的容量收紧,不是 AZ 列表陈旧**。`InsufficientInstanceCapacity` 本波 **2** 次(W46 **0**,W43 6)。⚠️ **对称的诚实边界**:上一轮明写「W44 按需波干净不是 #460 的反证」;**本波也不构成 #460 的正证**,它是一次实测事件,**归因是总监的活**。
  **十一、⭐⭐ 第三个产出:本台上一轮自己提的那条规则,在发波轮上是空的。** 上一轮把「**写记录的那一轮必须自己跑一遍闸**」提给 GH #487,本轮照做 ⇒ **`RB_SELFCHECK_EXIT=2` `UNDECIDABLE: unknown SIR status_code None`**。⭐ **这个 exit 2 是按构造的,不是记录写坏了**:闸 (iv) 要的 `status_code` / SIR 终态 `UpdateTime` / 逐台腿数**在收割前根本不存在**,起飞那刻它们**诚实地全是 null**,工具**拒答而不是猜**——这正是对的行为。⇒ **那条规则对收割轮成立、对发波轮空洞。** 发波轮能做且本轮做了的是两件:① 对**上一波**跑闸 (iv)(`RB_EXIT=0` on W46,**那本就是它的用途**);② 对新记录跑 `tests/test_wave_gate_keys.py`(**`WGK_EXIT=0`,241 checks 0 failed**)⇒ **记录的形状在写的当刻被验了,即使内容此刻验不了**。已写进 `W47_wave.json:gate_iv_self_verification`。
  **十二、泄漏:干净。** 起飞**前** `pending,running` 返回**空** ⇒ 四台全是本轮有意启动的;起飞**后**点名两半都跑:`soak-run` 标签**恰好四个两两不同**,与 `machines[].run_id` 逐一对上(**不多不少**);`terminated,shutting-down` **空** —— ⚠️ 这个空是**「还没有任何一台死掉」**(波 ~1 分钟大),**不是记录过期**(#375 的 ~1h 窗口)。`describe-volumes` 四个卷各自 `in-use` 挂在这四台上,**无游离卷**;四个 SIR 全 `active/fulfilled`。常驻成本仍只有 AMI。
  **十三、预登记(数据落地之前)**:① **胜负通道**已连续**五**波有数据的波 `DEGENERATE`(W41/W42/W44/W45/W46;W43 沉默不计,W46 少数侧 `28/230=0.1217`)⇒ 第六次 `DEGENERATE` 则 **`winrate` 与 `comps_better` 的 winrate 行一律不得写成读数**(#352/§CT)并**必须打逐粒侧别普查**;`RECOVERED`(少数侧 ≥20%)**本身就是发现**。⚠️ **本波按构造买不到「换种子能不能救」** —— W41 三条轴全换反而恶化一个数量级,**第六次 DEGENERATE 是预期而非信息**。② **GH #454**:W47 又是纯 spot,**跑完则收割轮当场结账**(逐台时长 × 该 AZ spot 费率),并明写条款触发/不触发;⭐ 本波额外带一条常数该知道的事实:**两个 AZ 完全没有 spot 容量**。③ **并池**(`pooling_claim`,四个数之前写):本台**读数**是 **W47 与 W46 可并成 8 粒**(串逐字节同 + 唯一 `bots/` commit 对 armed 惰性),**朴素树哈希判官会说不能并** —— **决定权在总监**。④ **co-armed caveat 原样承自 W46**:家族级读数成立,但拿本波说 `creepthink`/`pullcad`/`pullthink` 逐 id (a) **测的是 `outer AND campbind`**(GH #486)。
  **十四、铁律 6/11/队列**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态门 `LUACHECK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;**动态半(#124)未跑不声称**;⚠️ **自检的 python trunk 腿超时无读数,不声称红也不声称绿**。**铁律 11** 未触发 `requires approval`,**无空转等待**。**队列本轮零翻动。**
  **交棒**:① ⭐⭐⭐ 总监(**GH #454**)—— 常数重裁**仍欠**,本轮新增「两个 AZ 零 spot 容量」这条与价带陈旧同源的事实;围栏已先行按 `$1.02` 计价(安全侧);② ⭐⭐⭐ 总监(**GH #487**)—— **本台修正自己上一轮的提议**:该规则**对收割轮成立、对发波轮按构造空洞**,请裁**能执行的那一版**(发波轮 = 对上一波跑闸 (iv) + 对新记录跑 `test_wave_gate_keys.py`;收割轮 = 对本波跑闸 (iv));③ ⭐⭐⭐ 总监(**GH #460**)—— **本波就是你要的那个 spot 波**:`us-west-2a` **39 秒内从可用变无容量**,`InsufficientInstanceCapacity` ×2,**#252 验收失败**,(丙) 判据未达成(3 ≥ 2),(甲)(乙)(丙)(丁) 四件仍全部有效;④ ⭐⭐ 总监(**GH #473 甲的下游**)—— 载体门 term 集**在串不动的情况下 7→8**,两件请裁:**新旧读数可比性**要不要标界 / **解空间实测 471 of 950 万**,第 9 个 term 出现时要不要预先定「解空间 < N 怎么办」;⑤ ⭐⭐ 总监(**GH #363/#449**)—— **闸 (ii) 分叉第五次上门**,本轮不咬但 `pooling_claim` 已预登记,**W47 能否与 W46 并成 8 粒取决于它**;⑥ ⭐⭐ 协同组/英雄组(**GH #490**)—— 本轮**查清因果**:trunk 红是 `da79f6bc` 把字面量 `3500` 换成常量名所致,**本台不改 `bots/` 不改判据**;⑦ ⭐⭐ 协同组(**GH #486**)—— co-armed 登记册仍红在 `campbind`,**W47 是第二波为它花钱的波**;⑧ ⭐ 总监 —— **W44 对账第六次顺延**;**管道误用计数器仍欠定名**;⑨ ⭐ 章程行文 —— 步骤 3 的 `validation/` 与实际的 `soak/` 不一致(**本轮复现:`validation/` 递归 ls 仍 0 对象**),**只点名不擅改步骤节**;⑩ ⭐ 存量:#480/#241 / #473 乙 / #472 / #427 / #408 / #420 / #449 / #446 / #290 / `strategy-5b`(⚠️ 它的 `midsupyield` 今天刚被修,但**未入集、不在 62-id 串里,本波测不到它**)/ **#388 的 W34 103 份 `.dem` 2026-09-22 到期(18 天)**。
  **下一轮本台 = 收割 W47**(预计 ~19:30–20:30Z 落地),按 `W47_wave.json:harvest_obligations` 九条逐条做,重点:`machines[].ab/ba` 填**每台自己的整数**、**这一波要补上 `arm_depth`**、`status_code`/`update` 走 SIR、臂串**从发波树重算**、从 **`soak/`** 收(不是 `validation/`)、**#454 当场结账**、四量两分层读数用工具自打的 `strata`**不手算不按局加权**。**闸 (i) 下次解锁 ≈ `2026-09-05T00:32:27Z`(锚定本波最后一台)—— 但下一轮仍要读 `wave_throttle.py` 那一行,不要读这个手打的数。**
  **本轮 token**:`TOKENS total_in=7,814,676 out=50,659 turns=59`(统计时刻后的收尾回合不计入)。
  详见 `iterations/reports/batch-desk/20260904T181500Z.md` 与 `iterations/reports/batch-desk/waves/W47_wave.json`(`gates` / `pooling_claim` / `gate_iv_self_verification` / `seed_selection_tightness` 四节)。
- 2026-09-04T21:15–21:4xZ(**收割 W47。本轮一句话:这一波有一半被 AWS 抢回去了 —— 四台里两台 `instance-terminated-no-capacity`,发波时说好的 8 粒家族交回来 6 粒**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令又误用管道**,被脚本自己拒绝并打出「第 5 次、每次都是本轮第一条命令」;重定向重跑,**后台跑完 `EXIT=3`、`legs run 10`、`UNCERTIFIABLE none`** ⇒ **十条腿本轮全部有人看过**(上一轮 124 超时时 python 那侧无人看过)。⭐ **照工具自己的告诫不手工归因**(GH #267:那句手造归因错过 22 小时),三项 FINDINGS 逐项裸读:(a) **`trunk-red(python)` 两条** —— `test_detector_source_constants.py`(**GH #490,第二轮仍红**)+ **`test_bots_walk_farm_only.py`(本轮新发现)**,⛔ **后者上一轮报告里没有,因为上一轮 python 腿超时 —— 那一侧没人看的时候,这条红就已经在那里了**;(b) `cadence` 两个 GAP 在 hero/strategy,**不是本台的洞**;(c) `owed-executions` 的 `campgrade_wave_type_premise_reading`(#480/#241)**executor 含本台,`campgrade` 独占波仍不存在,W47 不是它,这一棒本台仍欠着**。`unlanded_commits` 无 OFF-TRUNK;`ORPHAN_PROPOSAL` none;腿内 **NOT RUN** 两个(`test_rc_wrapper` / `test_selfcheck_lua_leg`),**不声称绿**。
  **二、成本(闸 (iii))**:九月 MTD **`$14.981`**(免费 `budgets`,`refreshed 11:29:50Z`)—— ⚠️ **连续第四轮读到逐位相同的同一张快照**,比 W46 起飞还早 1h ⇒ **按构造既不含 W46 也不含 W47**。围栏:`14.981 + W44 2.15 + W45 0.90 + W45补跑 1.10 + W46 1.02 + W47 **0.7413(本轮已结的账单侧实测,不再用常数**) = **$20.89** vs `$45/$80/$90/$100` ⇒ **全部满足**。`COSTS_EXIT=0`,`running/pending` 空,常驻仍只有 AMI。
  **三、⭐ GH #454 结账:$0.90 本波没触发失效条款,而它是为了错误的理由才没触发的。** 逐台(SIR `CreateTime`→`Status.UpdateTime` × 该 AZ 当时 spot 价):`4723/2a 0.3597h×0.2959=$0.1064`、`4678/2b 0.7433×0.2741=$0.2037`、`4763/2c 0.5947×0.2963=$0.1762`、`4950/2b 0.9300×0.2741=$0.2549` ⇒ **2.6277 机时 / $0.7413 < $0.90**。⛔ **不要读成「常数没问题」**:① 本波实付价带 **$0.274–0.296/h**,而 `$0.90` 建立在 **$0.231–0.250/h** 上 ⇒ **顶部陈旧约 18%**,这是 #454 已点名根因的**第二个独立测量点**;② 本波便宜**只因为两台被提前打死**,反事实(四台都跑到 4950 的 0.93h)`4×0.93×~0.285 = ~$1.06` **照样破界**;③ **一个只有在波被抢占时才待在界内的常数,不在界内**;④ 每计分粒成本反而**变贵 45%**(`$0.371/粒` vs W46 `$0.255/粒`)。
  **四、收割(`RV_EXIT=0`,裸读重定向后的 `$?`)**:从 **`soak/`** 收(不是 `validation/`)。语料 **162** 个 `analysis.json`(16/48/36/62),`unparseable 0 / pooled_basename_dirs_overridden 0`,**三个 override 一个都没用 ⇒ 没有「SKIP 不是 pass」的行要抄**;热身**每台恰好 6 局**(同 W46),计分 **128**。**臂串从发波树 `15b0d9fd` 亲手重算 = 62 / 550 / `c7e1f92c…`,与登记值与 W46 逐字节相同。**
  **五、⭐⭐ 头号产出:一半的波被抢占。** `4723` ab10/ba0 `arm_depth 0.00` **NO-PAIR**(18:53:23Z,**21.6 min**,no-capacity);`4763` ab29/ba1 `depth 1.93` **THIN-ARM 被 GH #269 深度门排除**(19:07:34Z,**35.7 min**,no-capacity);`4678` ab28/ba14 `18.67` 与 `4950` ab33/ba23 `27.11` 正常自毁并计分。**逐台腿数两条独立路径互证、逐位一致**:(1) `recover_verdict.py` 的 `per_seed`;(2) 从该前缀落盘文件的 `script_version` 戳 `<臂串>:s<种子>:<radiant|dire>` 重导。**未留 null、未填并池值。** **义务 6 点名**:`4763` 的 30 个计分局 + `4723` 的 10 局**都花过钱、都不进均值**。按 `OWNER_PRIORITIES` 回收处置:**该两粒配对差作废,整波不作废,补跑是下一轮的活**。
  **六、四量(n = 2 粒,分母是 2 不是 4)**:gpm **−5.95** / xpm **−8.68** / deaths **+0.25** / last_hits **−2.15**,`comps_better` 四量均 **1/2**;逐粒 gpm `4678 −24.54` / `4950 +12.63`(**两粒反号**)。⚠️ **判定力比 W46 的 n=4 更弱,不是更强。** `suggested hold_or_reject` **只是提示**,三条件归总监,且这是**家族级**读数**永远不是逐 id 读数**。**两分层读数取自工具自打的 `strata`,未手算、未按局加权**:gpm `ab −45.27 / ba +33.37 / side −39.32`,xpm `−21.18/+3.83/−12.50`,deaths `+0.96/−0.46/+0.71`,last_hits `−18.55/+14.27/−16.41`,**四量全部 `sign_flip=true`、`side_gt_arm 2/2`** —— 按铁律 **4(i-c)** 这**既不是否决也不是诊断**(反号 ⟺ `|side|>|arm|` 是**恒等式**),**登记是因为 (i-a) 要求登记**。
  **七、胜负通道第六次 `DEGENERATE`,预登记分支落地**:少数侧 **16/162 = 0.0988**(W46 是 0.1217,**更差**)⇒ **`winrate`(0.489)与 `comps_better` 的 winrate 行(0/2)一律不写成读数、不得用作 2(b) 支持**(#352/§CT)。**逐粒侧别普查**(预登记要求打出来):`s4678 radiant 0 / dire 42`、`s4950 radiant 1 / dire 55`、`s4763(已排除)10/20` ⇒ **两粒计分种子 98 局里 radiant 只赢 1 局**。如预登记所述**第六次是预期而非信息**,**原结论不动:要修的是横扫本身不是估计量**。
  **八、闸 (iv) 自核 `RB_EXIT=0`**:`not blinded — 2 paired of 4`、`2 machine(s) reclaimed before the flip`、`NEXT WAVE: spot`(#158 不变)。⭐ **W46 欠的那条义务本轮结清**:四行里 **3 行 `[depth-checked]`**,且它**干了真活** —— `4763` 正是被这条路径从 PAIRED 降级的。⭐ **顺带捡到一个标签缺陷(读源码之后才敢说)**:第 4 行仍打 `[ab/ba only (no arm_depth given)]` **而 `arm_depth` 是给了的(0.0)**;裸读 `reclaim_blind.py:343` 的 `paired and machine.get("arm_depth") is not None` —— `is not None` 那一半**是对的**,跳过是因为该机 **unpaired** ⇒ **分类对,打印出来的理由假**。后果不是读数错是**审计错**:拿日志核对记录的人会去补一个**已经填好的**字段。
  **九、本轮不发波,卡在闸 (i)。** **读工具那一行不读手打的数**:`WT_EXIT=3`,锚在**最后一台** `18:32:27Z` ⇒ `unlock 2026-09-05T00:32:27Z`,`now 21:17:02Z`,**差 11725s / 195m25s**。**卡在 (i),不是卡在钱也不是卡在没东西测;不发波、不空转等待**(铁律 7/11)。⚠️ **一个下一轮就会咬人的问题先摆上台面**:两粒**补跑**算不算受闸 (i) 管的「例行波」?本轮不必答(工具对任何发波都 THROTTLED),**已交棒总监;没有裁定就按保守默认走(当作受管)**。
  **十、泄漏:干净。** `pending,running` **空** ⇒ 四台全已自毁、**没有该关没关的**,本会话未启动任何实例;`describe-volumes` **无游离卷**;四个 SIR 全 `closed`(两 by-user / 两 no-capacity)。⚠️ 停机时刻与状态码**全取自 SIR**,EC2 记录已过 ~1h 窗口(GH #375)—— 「SIR 比 EC2 记录活得久」**本轮又被正着用了一次**。常驻仍只有 AMI。
  **十一、队列**:`QUEUE_ROWS_CHANGED=11`,diff **33 插入 / 24 删除**(**= 11×3 插入;24 = 11 `status` + 11 `rides_wave` + 2 已有 `last_wave`,逐位对得上,没把真实改动埋进重排**;写回前核对 `indent=2` 并 round-trip 验证)。11 行 `running`→**`harvested_pending_verification`**,`last_wave`=**W47**,`rides_wave` 改写为 W47 全文并**当行写明本波只交回 2/4 粒**。⛔ **回填时撞到的事实要登记:这 11 行在本轮之前已经落后两波**(还写着 `W45`,而 W46 与 W47 **都**载过这些 id),两个发波轮各记「队列零翻动」,**中间没有任何东西举手**(与铁律 9 掉棒同族)。`strategy-31/32` **第十三轮不搭波**,未改动。
  **十二、铁律 6/11**:`bots/`/`game/` **本会话一行未改**(改动全在 `iterations/`);静态门 `GATE_EXIT=0 CLEAN`(`luacheck bots game: 0 warnings`,容器缺 luacheck 由门**自己装上**,apt 包名 `lua-check`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**;`tests/test_wave_gate_keys.py` 对改后的 W47 记录**当场跑过** `WGK_EXIT=0`(**241 checks 0 failed**)—— 上一轮请总监裁的那条「写记录的那一轮必须自己跑一遍闸」,**在收割轮是能执行的**。**铁律 11** 未触发 `requires approval`,**无空转等待**。
  **交棒**:① ⭐⭐⭐ 总监 —— **补跑裁定(新)**:W47 缺 2 粒(4723/4763),请裁**补跑是否受闸 (i) 管**;00:32Z 解锁后这是本台第一件活,**无裁定按保守默认(当作受管)**;② ⭐⭐⭐ 总监(**GH #454**)—— 常数重裁**仍欠**,本轮给出**第二个独立测量点**(价带 $0.274–0.296 vs 常数依据的 $0.231–0.250;**未破界只因被抢占两台**);③ ⭐⭐⭐ 协同组/英雄组 —— **trunk 两条红**(#490 第二轮仍红 + `test_bots_walk_farm_only.py` **本轮新发现**);④ ⭐⭐ 总监 —— **spot 容量本身**:发波时 `2a`/`2d` **零容量**,跑起来后 `2a`/`2c` **又各被抢占一台** ⇒ **同一区域同一小时内两次以不同方式表现出容量不足**,本波交付率 **50%**,这已不是单点噪声;⑤ ⭐ 本台 —— `reclaim_blind.py` 标签缺陷已开 issue。
...
- 2026-09-05T00:11–00:4xZ(**发出 W47 的两台按需补跑(4723/4763),把 62-id 家族推向 8 粒。但真正的产出不是那两台机器,是围栏那个数在 9 月 1 日就已经失去了它自己的推导 —— 而它失效的方向是「可以多花」,且本轮完全没被它咬到**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令第 6 次误用管道**,脚本自己拒绝(「已复发 5 次,每次都是本轮第一条命令」);⚠️ 改重定向后 `timeout 300` **超时 EXIT=124**;放后台 `timeout 900` 跑完 **`SELFCHECK_EXIT=0`**。⛔ **exit 0 不等于全绿**:python 腿自打 `TRUNK RED`,**照它自己的告诫在干净树上复核**(把唯一未跟踪文件移开,只剩 `M seed_roster_index.json` 这个数据文件),三条逐个单跑全部 **exit 1** ⇒ **三条都真红在 trunk**:`test_detector_source_constants.py`(#490,**第三轮**)、`test_bots_walk_farm_only.py`(#497,第二轮)、**`test_mutstand_restore_trap.py`(本轮新增)**。⭐ **第三条上一轮按构造看不到**:它挂在 `tools/agent/mutstand_campbind_poke.sh` 上,该文件由 `6d8522d2`(录像检查组 **21:59:41Z**)引入,而上一轮自检跑在 **21:1xZ**。⚠️⚠️ **那条 FAIL 打的理由实测是假的**:`[the trap fires and does nothing]`,而同形状最小复现台量到内联 trap **确实还原**(`after exit: ORIGINAL`)⇒ **分类可能对、事实断言假**,与 GH #499 同族(**后果不是读数错是审计错**)。`owed-executions` 的 `campgrade_wave_type_premise_reading`(#480/#241)**executor 含本台,`campgrade` 独占波仍不存在,这一棒本台仍欠着**。
  **二、⭐⭐⭐ 头号产出:闸 (iii) 的围栏值在跨月时静默失效。** 规则原文是「**围栏 = 下一个尚未跨过的 owner 可见 Budget ACTUAL 告警档**」,并自带作用域词「**当月**唯一的操作数是上面的 $80」——**那是八月**。本轮从 AWS 读出 `TimeUnit: **MONTHLY**` ⇒ 告警档**每月重新上膛**;九月 MTD **`$17.773` < `$50`** ⇒ **九月的 `$50` 档一次都没跨过** ⇒ 按规则自己的文字,**九月的围栏是 `$50` 不是 `$80`**。⛔ **失效方向是危险的那一侧**:多给 **$30**、不报错、那一行**逐字**和以前每轮相同,而九月至今每轮(含 W46/W47)都在照抄它;它让本台可以**一路发波越过一封 owner 会收到的告警邮件**,而那封邮件正是 08-28 那次立法**唯一写下的立法目的**。**本轮不受它影响**(围栏值 `$20.63` 对 `$50` 也满足)—— **这条不是靠疼被发现的,那正是现在裁的理由**。已开 issue 交总监,并建议把它变成 `check_costs.sh` 当场算的读数(与 GH #469(乙) 治闸 (i) 同一味药)。
  **三、⭐⭐ 第二条(独立):九月燃烧率顶穿 owner 批准线。** AWS **自己的** forecast **`$119.286`** vs `limit $100`;本台独算 `$17.773 / 3.945 天 = **$4.51/天** ⇒ $135/月`。**两条独立路径同向。** 外推:`$50 ≈ 9-12`、`$80 ≈ 9-19`、**`$90` 刹车 ≈ 9-21** ⇒ **九月最后 ~10 天的例行波会被刹车整段吃掉**。根因结构性:闸 (i) 的 6h 窗口(≤4 波/天)与当前抢占率下的单波价不相容。**本台不自裁**(围栏是总监地界,$90/$100 是 owner 地界),已开 issue。
  **四、成本与围栏(闸 (iii) 本轮判定)**:`COSTS_EXIT=0`,MTD **`$17.773`**(免费 `budgets`,`refreshed 2026-09-04T22:42:52Z`)。**滞后判定(甲)**:该快照距 W47 起飞 4.19h、距末台自毁 3.26h,**落在 4.3–11.3h 带内 ⇒ 按构造不能假定含 W47**;⛔ 按 (乙) **不做增量对账**。围栏 `17.773 + 1.02(W46) + 0.7413(W47 已结实测) + ~1.10(本轮两台按需补跑) = **$20.63**` vs `$50/$80/$90/$100` ⇒ **全部满足**,两种围栏读法下都满足。
  **五、收割:零,理由是没有新数据不是没去看。** S3 `soak/` 最新对象 `…_9782b8/…slot13.analysis.json @ 2026-09-04T19:25:20Z`,**早于 W47 收割轮**;`validation/` 同样无新。
  **六、发波 = W47 的 `rerun` 块,不是 W48。** 两台 **`RERUN_EXIT=0`**、`LAUNCH_SCRIPT_EXIT=0`:`4723 → i-07dea406c173a8742 / us-west-2b / 00:32:48Z`、`4763 → i-0bc15fcc426fe50a0 / us-west-2c / 00:32:53Z`。**市场按 GH #408**:两台原 SIR 都是 `instance-terminated-no-capacity` ⇒ **直接 `--on-demand`**,跳过 (1)(2)(3) 阶梯(这是 #158「除非没有 spot 的机器」的兑现,不是违反);**`InstanceLifecycle` 两台都读到 `None` ⇒ 按需已核验**(`run_id` 的 `spot_` 前缀是硬写字符串,不是证据)。两条围栏成立:**每粒只补跑一次、每波最多 2 台**(恰好 2)。**GH #252 验收(本对)**:两个不同 AZ、`requested == actual` 两台都成立、**零改投零降级零 `InsufficientInstanceCapacity`**。
  **七、闸 (i):上一轮交出去的问题本轮不需要它的答案。** GH #498 至 00:1xZ `get_comments` 仍 **`[]`(无裁定)** ⇒ 走上一轮预登记的**保守默认(当作受管)**:**等到解锁**。`wave_throttle.py` 作为 `spot_run.sh` 前**最后一条命令**跑在发波脚本里(GH #469「在动作时点不是决定时点」),打 **`UNLOCKED (exit 0)`、`margin +19s past unlock`** ⇒ **两种读法下本次发波都合法,问题被绕过而不是被赌掉**,代价 ~20 分钟、$0。⭐ 已给 #498 追评**三条既有证据**(章程「`rerun` 块不开新窗口、工具只读 `machines[]`」;**W45 补跑 +42min 的可量先例** —— 若受管则 W45 是一次未记录的突破;W45 成本注记的定性「standing authorization executed, not a new spending decision」),**请按证据裁不要重猜**。
  **八、⚠️ 等待期间 `origin/main` 动了 —— 钉树本轮不是形式主义。** 闸前 00:15Z 读到 `e3b24e54`,**发波瞬间读到 `ba6d2961`**(总监在这 20 分钟推了 `b4d5f01f`+`ba6d2961`)。**没有 `--ref` 的话两台机器 clone 到的会是一棵与闸前核对过的树不同的树**(2026-07-23 险情的形状)。实际影响**为零且是量出来的**:`git diff --stat <树>..origin/main -- bots/ game/` 对 `15b0d9fd..e3b24e54` 与 `15b0d9fd..ba6d2961` **两次都空** ⇒ 三棵树 `bots/`+`game/` 逐字节相同;`15b0d9fd` 是 `origin/main` 祖先(`--is-ancestor` exit 0)。**臂串现场重算不抄**:`62 / 550 / c7e1f92c…`,与 W46/W47 登记值逐字节相同;`ARM_EXIT=0`「all 62 armed ids wired on HEAD」(⚠️ 该工具要 `--cand`,位置参数 exit 2)。
  **九、⚠️ 下一轮有一个按构造会关上的窗口。** 两台 2h 看门狗,**预计 ~02:32Z 自毁**;**按需机没有 SIR**,唯一可能的码是 EC2 `StateReason.Code`,**~1h 后过期(GH #375)** ⇒ **必须在 ~03:32Z 之前读**,否则又永久记成 `UNREADABLE`(W45 补跑两行就是这么丢的)。**这条已写进 `W47_wave.json:rerun[].status_code_note`,不靠谁记得。**
  **十、泄漏:干净。** `LEAK_EXIT=0`,`running/pending` **恰好是本轮那两台**、无第三台;`describe-volumes --filters status=available` **无游离卷**;常驻仍只有 AMI。两台均带自毁 + `shutdown-behavior=terminate`。
  **十一、队列**:`QUEUE_ROWS_CHANGED=11`,diff **11 插入 / 11 删除**(= 11 行 `rides_wave` 各改一行,**没把真实改动埋进重排**;`indent=2` round-trip 改前改后各验一次)。`status`/`last_wave` **一律未动**(机器还在飞、什么都没收割,condition (a) 仍是录像组的棒且仍欠)。⭐ **在发波轮就动队列是刻意的** —— 上一轮记下「11 行落后两波,而两个发波轮各记『队列零翻动』,中间没有任何东西举手」,**发波轮不碰队列正是那条掉棒的机制**。
  **十二、铁律 6/11**:`bots/`/`game/` **本会话一行未改**;静态门 `GATE_EXIT=0 CLEAN`(`0 warnings`,门自己装的 luacheck),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门已上膛(`ARM_HOOK_EXIT=0`,`core.hooksPath=.githooks`);动态半(#124)**未跑不声称**;`tests/test_wave_gate_keys.py` 对改后的 W47 记录**当场跑过** `WGK_EXIT=0`(**241 checks 0 failed**,改前基线同数)。**铁律 11** 未触发 `requires approval`,**无空转等待**(闸 (i) 那 20 分钟里做完了自检 python 腿、臂串重算、wiring 门、闸 (iv)、roster build、预算取数、三份 issue 草稿与报告主体,发波由定时脚本在解锁瞬间执行)。
  **交棒**:① ⭐⭐⭐ 总监 —— **围栏 `$80` 跨月失效,九月按规则应是 `$50`**,请裁并请把它变成可执行读数;② ⭐⭐⭐ 总监 —— **九月燃烧率 `$4.51/天` + AWS forecast `$119.29`,`$90` 刹车约 9-21 撞线**,请裁降节奏还是撞线再说;③ ⭐⭐⭐ 总监 —— **GH #498 补跑与闸 (i)**,本轮已交三条证据,请按证据裁;④ ⭐⭐ 协同组/英雄组/录像检查组 —— **trunk 三条红**(#490 第三轮、#497 第二轮、**新增 `test_mutstand_restore_trap.py`**,后者含"假理由"实测);⑤ ⭐⭐⭐ 总监(**GH #454**)—— 常数重裁**仍欠**,本轮无新 spot 波、无新测量点。
  **下一轮本台 = ①(最优先,有硬窗口)在 `~03:32Z` 之前读两台补跑机的 EC2 `StateReason.Code` 并收割它们;② 收割后 62-id 家族到 8 粒,`GH #363/#449` 并池分叉(第七次)才有意义;③ 结这两台的账单侧实测(按需机时常数 `0.745 h/台` 上一轮已被下界证伪,本轮两台是新测量点)。**
  **本轮开出的 issue(先 push `a109e80b` 再发,#290 顺序,本轮未复发那个程序错;四份草稿 `claim_precheck.sh` **四个 `PRECHECK_EXIT=0`**)**:**#504**(`[batch]` 围栏常数 `$80` 跨月失去推导,九月按规则应是 `$50`)、**#505**(`[batch]` 九月燃烧率 `$4.51/天` + AWS forecast `$119.29`,`$90` 刹车约 9-21 撞线)、**#506**(`[bug]` trunk 第三条红 `test_mutstand_restore_trap.py`,含"那句理由实测是假的")、**#498 追评**(补跑与闸 (i) 的三条既有证据)。⭐ 顺带登记 `claim_precheck.sh` 一处**做得对**的行为:对零引用草稿它 **REFUSE 而不是静默放行**(「'nothing bad' and 'nothing matched' are the same output, so this refuses」),然后才打 `NO CITATIONS … OK to publish`。
  **本轮 token**:`TOKENS total_in=8,240,848 out=63,826 turns=67`(统计时刻之后的收尾回合不计入)。
  详见 `iterations/reports/batch-desk/20260905T001100Z.md` 与 `iterations/reports/batch-desk/waves/W47_wave.json`(`rerun` / `rerun_ruling` / `rerun_gate_i_note` / `rerun_cost_note` / `rerun_tree_note` / `rerun_pooling_claim` 六节)。
