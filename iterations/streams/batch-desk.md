...
- 2026-09-03T09:15–09:4xZ(**发 W42 = 57-id 家族第二波,spot 回归。本轮真正的产出是两件:GH #427 丢的窗口把 GH #271 的闸也一起关了(而且关的形状是 `exit 2`),以及「树对象哈希」当并池判据在一个纯注释 commit 上给出了错的答案**)。
  ...
  **四、⭐⭐ 树漂移:哈希变了,编译出来没变 —— 并池判据要重裁(新开 issue)。** W41 的 ref 之后 `bots/` 只动 `hero_chaos_knight.lua`(commit `9053d199`,英雄组 GH #447,**+18 行注释**),`game/` 树 `f13b532c` 未动。**四个判官,把打架的也登记**:`git diff` 过滤注释 ⇒ 非注释改动 **0 行**;仓库 `strip_lua_comments` 逐行比 ⇒ **395/395** 相同(原始 18 字节差是这个朴素剥离器留下的**行尾空白**);⭐ **`luac5.1 -l -l` 归一化指针地址与行号 ⇒ 差 0 行(决定性)**;⛔ **`luac5.1 -s` 的 md5 不同 —— 未采信但登记**。⇒ **一个「编译后哈希」式的朴素判官会说这是两棵不同的树**,而带列表的反汇编说指令与常量池逐条相同、唯一的差是行号。**后果是实的**:57-id 家族要 8 粒,W41 给 4 粒、W42 给 4 粒,**并得了就是一步到位**,而现行判据是树对象哈希。本台读数是**能并**,**决定权在总监**,已**预登记**在 `W42_wave.json:pooling_claim`(**四个数落地之前**写的,防事后拟合)。**闸 (ii) 因此走第三支不走第一支**(arm 串逐字节未变、树编译后未变;家族 4 粒 < 8)。
