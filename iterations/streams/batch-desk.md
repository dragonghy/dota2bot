...
2. `bash tools/batch_test/aws/check_costs.sh` — 查 MTD 花费和在跑资源。
   **(2026-08-21T08:xxZ 起)MTD 默认走免费的 `budgets describe-budgets`,不再每次
   ...
   - **(2026-08-24T15:xxZ 起,两条铁纪律)`budgets` 的 MTD 对 EC2 有 4.3–11.3 小时以上的滞后。**
     ...
     - **(甲) 围栏算术不用裸 MTD。** 一律用 `MTD + Σ(近 12h 内已发波次 × 该波的单波价)`。
       **⚠️ (2026-08-25T13:xxZ 总监)这一行原文写死的是 `× $1.48`,而 §BE 的界线之后
       那不再是一个常数**:界前波 $1.48,界后波按下面那条基准外推 **~$3.05**。
   ...
   - **单波成本基准(2026-08-24 账单侧钉死,替代口口相传的「~$1.4/波」)**:
     4 台 × **0.550 h**(= `--hours 2` 看门狗的 27.5%,完成即关)× **$0.673/台·小时** = **$1.480/波**;
     单台 $0.370。**$0.673/h = us-west-2 `c6i.4xlarge` 的按需牌价** —— 波次走的是 `--on-demand`,
     不是 spot(spot 同期 $0.231–0.250,**按需贵 2.80×,占波次账单 63.5%**)。
     **方法学 vs 钱的取舍已交总监(GH #158),在裁定前保守默认维持 `--on-demand`。**
     顺带:run_id 的 `spot_` 前缀是 `spot_run.sh:185` 硬写的字符串,**与市场类型无关**,不可当证据。
   - **⚠️ (2026-08-25T09:xxZ 总监,GH #108)这条基准跨界了。** owner 的 cap 10 → 25 已落地
     ...
     **围栏算术请直接用新单价**;不够就缩 `--games` 或缩种子,**不要缩 cap**(缩 cap 等于把
...
4. 决定本轮要不要开新批测(优先级从高到低):
   ...
   b. 队列为空:跑一轮例行"测试版 vs 稳定版"(测试集见
      ...
      **例行波次节流(2026-08-19,防预算烧穿:每波实测 ~$0.5-1.5,若每
      ...
      (iii) 当月已花 + 本波预估 ≤ **$60**(总监 2026-08-25T18:5xZ 从 $45 抬高,
            档案 `test_set.md` §BH.3;**刹车线仍是 $90,owner 批准线另算,见铁律**)。
            **为什么抬:$45 这个数的原始理由写的是「给 $50 的 AWS Budget 留余量」,
            而 $50 是 Budget 的第一档 ACTUAL **告警**(一封邮件),不是限额也不是冻结
            (AGENTS.md 已记:冻结动作从本账号不可验证);owner 的批准档**当前批到 $100**,
            ⇒ $50–$100 这一段的钱**已经被批过了**,把一封告警邮件当成硬停会让例行波次
            在**已获批的预算里**静默停摆(本轮的算术:围栏值 $41.60,再发一波界后波
            ~$2.05–2.15 就到 $43.7,**第二波起 (iii) 必然不满足**)。
            **⚠️ 配套义务(抬高的对价,必须照办)**:跨过 **$50** 的那一轮,批测台要在
            **当轮报告里显式写一行**「本轮跨过 $50,owner 会收到 Budget 告警邮件,
            原因是 X 波 × Y 元」——**让 owner 收到的告警邮件永远有一份对得上的书面解释**,
            这是这条豁免存在的全部理由。跨 $60 仍然停,并按原规则报总监。
