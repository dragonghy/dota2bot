...
- **2026-09-07T03:05–03:3xZ(收割轮:W52 收割 —— ⭐⭐⭐ 头号产出是一个五量 0/4 的负读数,以及「这个负号不止一种机制」这半句)**
  ...
  **⭐ 追记(03:5xZ,后台自检跑完之后)**:`SELFCHECK_EXIT=3`,工具自己的 `exit sources` 节逐字 `FINDINGS (exit 3) : cadence owed-executions trunk-red(python) trunk-red(lua)` / `UNCERTIFIABLE (exit 2): none` / `legs run 10` ⇒ 第一节那句「Lua 腿不声称」**现在有读数了:trunk Lua 也红**。`RED test_corpus_scale.lua` ⇒ `FAIL: [detector] no test pins the live corpus size with an equality`,点名**协同组 01:20Z 刚落地**的 `tests/test_wkreinctr_untrained.lua:238` 的 `assert(C.fixtures == 109, ...)`;检测器自己写着立案句「**landing the next fixture turns them red without anything they measure having changed**」(GH #106/#127)并给出替代品(`tests/corpus_scale.lua` 的 `ratchet()`/`universal()`/`corpus()`)。⭐ **与第二节的 `chaos_knight`(GH #584)是同一缺陷族的两个实例,同一天** —— 一个写死载体词项被 promote 证伪,一个写死语料规模会被下一个 fixture 证伪;**区别只在于这一个有专职检测器在抓**。⚠️ **这条比第二节能说得更强**:`git status` 干净、`HEAD == origin/main`(`e94d6930`)、`git diff origin/main -- tests/ bots/ game/` **为空** ⇒ **两条红都在 main 上不是工作树伪影**(不需要 `git stash`,那三棵树本会话一行未改);Lua 那条来源提交 `e6de91a6`(协同组),**不是本会话**。⇒ 已开 **GH #585 `[bug]`** 并在 **GH #584** 追评点明同族。
