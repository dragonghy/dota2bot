...
- 2026-09-05T06:17–06:3xZ(**三道发波闸形式上全过,仍不发波 —— 因为发波树相对 W47 那棵树的全部 `bots/` 变更是 59 行加 / 0 行删,其中非注释行恰好 1 行,而那一行按 Lua 短路在本波臂串下不可达。本轮零发波、零新增支出、零泄漏**)。
  ...
  **七之二、⭐⭐⭐ 收尾复检抓到的第二件事(不在原工作单元计划内):发波围栏读的是账号级的数,而账号上有别的项目在跑。** ① `check_costs.sh:67-68` 的「空」**带过滤器**(`tag:Name,Values=dota2bot-*`)⇒ 它答的是「有没有 dota2bot 的机器在跑」,**对本台的泄漏检查这是对的过滤器**,但按构造看不见别的项目。② 不带过滤器再查,region 里恰好一台在跑而**不是我们的**:`i-0ce7ddf08ea24cc7f c7a.16xlarge running 2026-09-05T05:02:24Z spot ami-0bea529386a62a2ad`,`Name=fft-cache-v1-attempt3` / `Project=final-table-trainer` —— **机型、AMI、标签三处都对不上本台**(本台是 `c6i.4xlarge` / `ami-0a990a26d89c66547`)。⛔ **未做任何处置**:带着别人的项目标签,终止它既越权又不可逆。③ ⭐ **而 `dota2bot-batch` 是账号级预算,`CostFilters = null`(实测)** ⇒ `wave_fence.py` 读的 `ActualSpend`、三档告警、AWS forecast **统计整个账号** ⇒ **本台能不能发波取决于一个它不控制的数**。④ **九月至今那 `$17.773` 是本台的,这也是量出来的**:花 `$0.01` 做一次 CE 调用按 `INSTANCE_TYPE` 分组(章程允许的情形)⇒ `c6i.4xlarge $15.061` + `NoInstanceType $2.712` = `$17.773`,**`c7a.16xlarge` 一分钱还没进来**(05:02Z 才起,CE 滞后)⇒ **GH #505「燃烧率 $4.51/天 是本台的」那部分未被推翻。**⑤ ⭐ **但往前看方向反了**:`c7a.16xlarge` spot 实测 **`$1.008–1.210/hr`** ⇒ 持续运行约 **`$24–29/天`**,是本台日燃烧率的 **~6 倍**,且落进**同一个** `$100` 限额 / **同一组**告警 / **同一个** fence。⇒ **(甲) 本台的发波围栏会被别人的算力关上**(headroom `$31.227`,按此速率约一天多吃光,而本台发一波只要 ~$0.9–2.15;届时 `THROTTLED` **读起来像批测超支而实际不是**);**(乙) owner 的 `$50` 告警会先响,而它长得像批测的账**。⑥ **建议(交 owner/总监,本台不自裁)**:给 `dota2bot-batch` 加 cost filter(按 `Project` 标签或机型),或给 `final-table-trainer` 单开预算;**两条都不改本台任何工具**。⚠️ **不主张那台机器不该跑** —— 该不该跑本台不知道也不该猜;只主张**它和批测的闸共用一个数,而那两件事应该分开**。⑦ 顺带**订正措辞不订正结论**:第七节今后读作「**dota2bot 侧**泄漏干净」—— 本台历轮的泄漏检查一直是这个范围,**读数没错,范围写宽了**。
  ...
  **本轮开出的 issue(先 push `3489a82a`+`94e7556e` 再发,#290 顺序;草稿 `PRECHECK_EXIT=0`,`local commits not on origin/main: 0`、`refused 0`)**:**GH #513**(`[batch]` 闸 (ii) 没有可执行检查,带判据 (甲)(乙)(丙)(丁))、**GH #514**(`[bug]` trunk python 5 红,3 条第一次点名,附「自检要跑满 ~19 分钟、短 `timeout` 会周期性地把 trunk 一整侧变成没人看过」)。
