...
2. `bash tools/batch_test/aws/check_costs.sh` — 查 MTD 花费和在跑资源。
   **(2026-08-21T08:xxZ 起)MTD 默认走免费的 `budgets describe-budgets`,不再每次
   ...
   - **单波成本基准(2026-08-24 账单侧钉死按需价;2026-08-26 owner 改路径)**:
     ...
     **Spot**(同期 $0.231–0.250/h)界前 ~**$0.54/波**,界后 **~$1.10/波**
     (**2026-09-06T07:xxZ 总监按 GH #454 重裁,从 `~$0.90` 上调**;全文 `test_set.md` §FG。
     历史:GH #219 路 4 立 `~$0.8`,2026-08-30T22:0xZ 上调 `~$0.90`,`test_set.md` §CO.5)。
     围栏算术**按该波实际市场类型取值**:W10–W14 是按需,用 ~$2.15;W15 起默认 spot,用 **~$1.10**
     (容量降级成按需的那一波仍用按需价)。
     ...
     **⭐⭐ 2026-09-06T07:xxZ 总监重裁(GH #454;`$0.90` 的自动失效条款已于 W46 触发,
     本台连续四轮点名「总监仍未重裁」——本轮结清)。两件事一起改:常数,和触发它的那个判据。**

     **(甲) 常数 `$0.90` → `$1.10`。** 方法**照抄 08-30 那次**(取覆盖**全部账单侧实测点**的
     整数档,**不取两个从未同时出现的极大值之积**):实测点现在是
     $0.747 / $0.795 / $0.799 / $0.79–0.86 (W29) / $0.824–0.892 (W30) / $0.794–0.859 (W31) /
     **$0.959–1.022 (W46)** / $0.7413 (W47) / $0.776–0.840 (W48) / $0.70–0.76 (W49)
     ⇒ 上确界 **$1.022**(W46,现货紧张:两个 AZ 无容量、四个 AZ 里三个越过 $0.231–0.250 价带)
     ⇒ 覆盖档 **$1.10**(余量 +7.6%,与上次 $0.86→$0.90 的 +4.7% 同量级)。
     ⚠️ **本台自 W47 起登记的 `~$1.02` 不是常数,是 W46 的实测上确界本身** ——
     「登记实测最大值」在下一次市场再紧一点时**恰好不够**,这正是 $0.90 出事的方式。
     对 $50 围栏 ≈ 每波比 `$1.02` 多记 $0.08,失效方向朝安全侧。

     **(乙) ⭐ 触发判据从「钱」改成「机时」,因为按钱写的那条判据与它要抓的东西负相关。**
     #454 的立案句(总监采纳,并认为它比常数本身重要):旧条款要「实测单波成本 > $0.90」,
     而**一次容量抢占就把总机时压下去、把实测成本压回线下** ⇒ **抢占越频繁,常数越不会被复查**,
     而抢占频繁正是市场紧张、单台费率上涨的时候。**失效条件与失效原因负相关**,
     与 `pullcad`「promote 一个 id 会静默冻死引用它的 gate」同族:门在,但被结构性绕开。
     ⇒ **两条腿,任一条成立即触发**(本台在该轮报告里点名,总监下一次触发必须重裁):
       - **(乙1)** 任一波**账单侧实测单波成本 > `$1.10`**;
       - **(乙2)** 任一波**「跑完自终」的机器(SIR `Status.Code=instance-terminated-by-user`)
         的机时均值 > `1.00 h/台`**。被容量回收的台**不进这个均值**——它们正是要被绕开的那部分。
     **(乙2) 的阈值不取 #454 建议的 `0.80 h`。** 理由是算术:自 W18 起**每一个**实测点
     (W18 0.89 / 0.90 / 0.909 / 0.920;W42 0.9175 / 0.9367 / 0.9414)**都在 0.80 之上**,
     ⇒ 那条判据会**每一波都触发**,而**每一波都触发的判据与没有判据等价**(只是更吵)。
     取覆盖全部实测点(上确界 **0.9414**)的整数档 **1.00 h/台**(余量 +6.2%)。

     **(丙) 顺带:章程别处写的机时常数 `0.745 h/台` 同轮作废,改 `0.93 h/台`。**
     `0.745` 是**含被回收台**的均值,而被回收的台跑了 14 分钟就没了 ⇒ 它系统性**低估**机时,
     低估方向是「以为围栏还有余量」,**与 MTD 对 EC2 的 4.3–11.3h 滞后同向叠加**。
     `0.93` = 覆盖上面 7 个「跑完自终」实测点(0.89–0.9414)的档。
     ⚠️ **诚实边界**:`0.93 h/台 × 4 × $0.28/h = $1.04`,与 (甲) 的 $1.10 **不是同一条推导**,
     也**不该**用来复核它 —— (甲) 走的是账单侧实测点的覆盖档,(丙) 走的是机时侧;
     把两者相乘去校对,就是本段上面刚否掉的那种「极大值之积」。**两条各自独立登记。**

     **旧条款留档(2026-08-30T22:0xZ 立,已被本轮 (乙) 取代)**:
     ~~任一波账单侧实测单波成本 > $0.90 ⇒ 本台在该轮报告里点名,总监下一次触发必须重裁本常数。~~
     **这个常数不许靠人记得去复查。**(这一句不变,而且本轮证明了它是对的:
     常数确实没人记得复查 —— 靠的是本台连续四轮点名,不是靠谁想起来。)
   ...
   - **⚠️ (2026-08-25T09:xxZ 总监,GH #108)这条基准跨界了。** owner 的 cap 10 → 25 已落地
     ...
     **围栏算术一律用上面那张实测表**(按需 ~$2.15 / spot **~$1.10**,**2026-09-06T07:xxZ 起**,
     GH #454 重裁;08-30T22:0xZ–09-06 之间是 ~$0.90),不用本段的外推值。
      ...
      **例行波次节流(2026-08-19,防预算烧穿:每波实测 ~$0.5-1.5,若每
      ...
      (i) 距上一次例行波次启动 ≥ 6 小时;
          ...
          **② 什么开一个新窗口**:一整套新的 `machines[]` 上机开窗
          ...
          所以它不依赖任何人诚实地填一个字段。
          **③ ⭐ 2026-09-05 总监裁 GH #498:补跑机本身也不受这 6h 窗口约束
          ——「不开新窗口」与「不必等窗口」两条都成立**(全文 `test_set.md §EM.6`)。
          **理由是闸 (i) 管的是新的花钱决定的节奏**,而补跑按定义是**买回一粒
          已经在那一波预算里付过钱的种子**,是一条**预先登记的处置被执行**,
          不是一个新的花钱决定。**节流它的是它自己那两道围栏**(每粒种子只补跑一次、
          每波最多 2 台按需补跑),两条都有界 ⇒ 豁免不打开任何无界的花钱通路;
          **闸 (iii) 照常适用**(补跑的钱进围栏算术)。
          ⚠️ **裁的依据是本台交上来的三条既有证据,不是新推理**:章程条文本身、
          **W45 可量的先例**(slate `06:30:52Z`,两台补跑机 `07:12:53Z`/`07:12:58Z`
          = **+42min**;若补跑受管,W45 那一轮就是一次未被记录的突破,而当轮
          记录把它写成合规)、W45 成本注记的定性。
          **从此不必再走「等解锁」的保守默认**(它在裁定落地前是对的)。
      (ii) 有新东西可测:bots/ 或 test_set.md 自上一波测过的 commit 之后
           有变更,或当前 tree+测试集的累计种子数 < 8;
      (iii) **跑 `python3 tools/batch_test/soak/wave_fence.py --planned <本波预估>
            [--pending <已发但可能未计入 MTD 的波>]`,读它打的那一行:
            `WAVE_FENCE: CLEAR (exit 0)` / `THROTTLED (exit 3)` /
            `UNCERTIFIABLE (exit 2)`。0 才可发;2 和 3 都不是通过。**
            ⭐⭐ **2026-09-06T09:xxZ 总监裁定(交棒 ②,全文档案 `test_set.md §FI.1`):
            本台 09:12Z 推翻自己的绿工具、不发波 —— **自裁确认,你是对的**。
            并把那一步搬进工具:`--pending` 现在是一个**被检查的声明**,不再默认 `$0.000`
            (RULING 4)。行为变化只有一条,请照读:**账户里有实例在跑而你没传 `--pending`
            时,工具 `exit 2` 并逐台列出在跑的是什么**;什么都没跑的安静轮次照旧 `exit 0`,
            并把那个零标成 **CERTIFIED**(是读数,不是默认值)。
            估一个数传 `--pending`(工具**故意不给任意机型定价** —— 在那里猜一个 markup
            常数就是新的自由参数),或 `--no-accrual-check` 跳过并把它打出的
            **「SKIPPED, NOT CERTIFIED」**那一行抄进当轮报告。枚举失败也是 `exit 2`,
            **不是「没东西在跑」**。⚠️ 枚举**账户级、不按 tag 过滤**是故意的:预算也没过滤;
            owner 哪天加上 `Project` 过滤器(`DECISIONS_NEEDED.md` 第 15 条)之后它会**过保守**,
            而那是会自己被人喊出来的失效方向。**本轮活的 AWS 路径没有被跑过**(总监不花 AWS 的钱)
            ⇒ **首次实跑是你下一轮**;权限不足会打 `accrual read : FAILED` 后 `exit 2`,
            不会静默放行。
            ⭐ **2026-09-05 总监按 GH #504 改成读工具,全文档案 `test_set.md §EM`。**
            **本条以下不再有任何可以抄进报告的围栏数字 —— 那正是被修的缺陷本身。**
            08-28 那次重写的立法目的是**取消这个数的自由度**(见下方存档),
            它做到了规则那一半,却把规则的**答案**(当时求得的 `$80`)写成了字面量;
            而 `dota2bot-batch` 是 `TimeUnit: MONTHLY`,**9-01 ActualSpend 归零,
            「已跨过的档」跟着归零**,规则的输出变回 `$50`,**却没有任何东西重算它**
            (九月至今每一轮报告都照抄 `$80`,**多给 $30**,批测台 09-05T00:11Z 立案)。
            工具把两个操作数**每次现读**(都免费):`describe-budget` 取
            ActualSpend/BudgetLimit/TimeUnit,`describe-notifications-for-budget`
            取告警档本身 —— **连 `50/80/100%` 都不再是写死的常数**
            (该权限本账号可读,2026-09-05 实测 exit 0;`AGENTS.md` 记的那道
            `DescribeBudgetActionsForBudget` 缺口不覆盖 notifications)。
            **刹车线仍是 $90,owner 批准线仍是 $100** —— 工具只**应用**刹车不**推导**它,
            `operative ceiling = min(fence, brake)`;撞刹车打的是「停下并报 owner」
            而不是「等」。**配套义务不变**:跨过任一告警档的那一轮,报告里显式写一行
            「本轮跨过 $X,owner 会收到 Budget 告警邮件,原因是 Y 波 × Z 元」,
            且跨档需要总监当轮明确裁定。
            ---- 以下是 **2026-08-28 那次改写($60→「下一个未跨档」)的原文,存档,不再是操作数** ----
            (**它的 `$80` 是 8 月的求值结果,不是规则;读它是为了理解上面那条工具在替代什么。**)
            > 当月已花 + 本波预估 ≤ **$80**(总监 2026-08-28T00:5xZ 从 $60 抬高;
...
- **2026-09-05T15:06–15:2xZ(收 W48 干净 4/4 + ⭐⭐⭐ 认领并发出 P4.1 upstream 标尺波 Y1 —— 而挡了它三轮的那个理由是假的。本轮支出 ~$1.38 最坏 / ~$0.70 预期)**
  **一、自检(铁律 10)**:⭐ **开工第一条命令没有再误用管道**(连二十五轮的那个形状本轮**中断**):直接重定向 + 放后台给足时间(上一轮交棒逐字要求,照办)。⭐ **收尾前跑完了**(⚠️ **真码读脚本自打那行不读 harness 报的**):**`selfcheck worst exit: 3` / `SELFCHECK_TRUE_EXIT=3`**,`legs run 10`,**`UNCERTIFIABLE (exit 2): none`**,`FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(python)` ⇒ **3 = 有发现不是没跑成**;`NOT RUN (inside a leg)`: `test_rc_wrapper.py` / `test_selfcheck_lua_leg.py`。⭐ **Lua 腿绿**:`84 tagged detector file(s), 0 failures`,⚠️ 边界照抄 `FAST SUBSET, not the full suite` ⇒ **GH #508 答不了**,仍挂着。`queue-rulings`/`owed-executions`/`cadence` 三项**非本台本轮引入**,交回总监。已跑完的腿:**python `95 passed, 5 failed, 2 uncertifiable`**;`stable-v1`/`v2` `EXISTS`/`PINNED`/`SHIPPED` 全 ok;`FROZEN none`(145 活 gate / 13 带 PROMOTED 注),atom `field_hold_needs_magnitude` 三者仍全 GATED;`no expired admission wait`;两个 `UNCERTIFIABLE`(`test_rc_wrapper.py`/`test_selfcheck_lua_leg.py`)**不是通过也不是失败**。⭐⭐ **五个红的归属全部查实,本轮引入零条,而且是量出来的**:`git diff --stat origin/main -- bots game tests .github .claude` **空** ⇒ 四个红读的文件与 main 逐字节相同;⚠️ **但本轮动了 `tools/`,这条捷径不够** ⇒ 对其中三个**单独做变异归属**(把 `aws_run.sh` **先存副本**再还原 HEAD 版重跑 ⇒ **照旧全红**,随即还原并核对 25 insertions 仍在)。⭐ 用「存副本」而非 `git stash`,正是铁律 10 自己点名的「死在 stash 与 pop 之间会扣住未提交工作」那个风险。⛔ 第五个 **`test_wave_gate_keys.py` 是本台自己的红,本轮修掉**(见三之二)。
  **二、成本** 九月 MTD **`$19.597`**(免费 `budgets`,快照 `refreshed 11:21:28Z` —— ⚠️ **早于 W48(12:24Z)与本轮两台标尺机,三者都不在读数里**)。`< $35` ⇒ **CE 未触发,没花那 `$0.01`**。进场 `running/pending` **空**。闸 (iii) 走工具不抄常数:`wave_fence.py --planned 1.35` **`FENCE_EXIT=0`**,projected `$20.947`,**operative ceiling `$50.00`**,headroom **`$29.053`**,三档 ACTUAL 告警 `$50/$80/$100` **全部未跨** ⇒ **owner 不会收到 Budget 邮件**。围栏(2 甲)= `19.597 + W48 spot ~0.84 + 标尺#1 ~0.03 + 标尺#2 ~1.35 = **$21.82** ≤ $50`。⭐ **`$0.90` 自动失效条款本轮不触发,但请读清条件**:W48 四台总机时 **3.3602** × 价带 $0.231–0.250 ⇒ **`$0.776–$0.840`**,⚠️ **这是机时侧推算不是账单侧读数**,而条款写的是账单侧 ⇒ **按字面不触发,不等于「常数没问题」**;该条款自 W46 实测 `$0.959–$1.022` 越线以来**连轮点名仍未重裁**,原样交回。
  **三、⭐ 收 W48:4/4 粒,零抢占,这一波是干净的。** `RV_EXIT=0`、`files_seen == games_loaded == 208`、`unparseable 0`、`source_dirs 4`(四个 run **各自子目录不拍平**,08:08Z 那个跨 run 撞车的坑没踩)。⭐ **armed 串在收割时重算不抄散文**:`git show 54b839d1:test_set.md` 第 2 行 = **63 id / 559 字节 / md5 `4aefc887…`**,与发波记录**逐字节相同**。**四粒全计分**(`min_arm_depth 8`、`thin_arm_seeds` **空**、**无一粒排除**),184 计分局:gpm **+27.25** / xpm **+11.33** / deaths **+0.06** / last_hits **+1.14**;`comps_better` 3/4·3/4·2/4·3/4;逐粒 gpm +49.95 / +37.58 / −23.92 / +45.39;`suggested=hold_or_reject`。**分层(4(i-a) 两腿读数都登记)** gpm `ab +127.24 / ba −72.75 / side +100.00`,四个指标 `sign_flip` **全 true** ⇒ 按 **4(i-c)** 是**恒等式不是诊断**,登记不折价。⛔ **`winrate` 不许引用**:`DEGENERATE`,少数侧 14/208(**0.0673**),边界 `0.5±0.1292` —— ⭐⭐ **第七次**(W41/42/44/45/46/47/48 全中),**发波轮预登记的「第七次就不再逐波复议」条件now满足**,据此交棒。⛔ **这四个数不是对 `stayattr` 的裁定**(本波测整个 63-id bundle;条件 (a) 是**帧问题**,判据**「留没留下」不是「活没活下来」**,且明令禁止拿胜负/存活率背书)。**死因**:四个 SIR 全 `closed`、**全 `instance-terminated-by-user`** ⇒ **正常自毁,零抢占**(对照 W47 丢 2/4、W43 丢 4/4);⭐ **SIR 活过 1h 窗口**第三次兑现;⭐ **AZ 分散度「死的时候」也是干净 4/4**(2a/2b/2c/2d)。**`machines[]` 的 null 全部填掉**(`ab`/`ba`/`arm_depth`/`status_code`/`az`/`launched_at`/`survival_min`,带 `assert`)—— 上一轮交棒逐字点名「**不许留 null**」,照办。
  **三之二、⛔ 闸 (iv) 的一半在 W48 记录里又漏了,是 W43 那次的一轮之隔复发。** `test_wave_gate_keys.py` 红(`251 checks, 1 failed`):W48 的 `gates` 只有 `(iv) reclaim-blind`,缺 `(iv) inputs -- no harvest owed`。⭐ **归属查实不是猜的**:`git show HEAD:W48_wave.json` 的 `gates` 键集合与现在**完全相同** ⇒ **不是本轮回填弄坏的,是 12:18Z 发波轮自己漏的**。⛔⛔ 而 **2026-09-03T21:15Z 那轮在 W43 上写过同一个缺陷**(那次两半都没记),当时建议逐字是「**发波轮发完波当场跑一次 `test_wave_gate_keys.py`**」——**未被采纳,一个波之后同一道门的一半又没记**。**修法严按测试自己的指示**(「值要诚实,**不许为了变绿编一个值**」):补上的值原文写明**实质在发波轮确实建立过**(其报告 §3:进场无本台实例、S3 无未收割 run、W47 已 4/4 收完)**但键是本轮补写的** ⇒ **`WGK_EXIT=0`,`252 checks, 0 failed`,绿是因为值诚实不是因为编了通过**。
  **四、⭐⭐⭐ 头号产出:P4.1 标尺波认领了,而挡了三轮的理由是假的。** 主会话 14:5xZ 已下最后通牒(06:17Z 与 12:1xZ 两轮未认领,「**下一轮必须发标尺波,排在任何 arm 波之前**,再空一轮写进 DECISIONS_NEEDED」)。**(1) GH #33 的 blocker 不存在了**:根因确实在(`aws_run.sh` 把 `--old` 硬编成 **`origin/$OLD_REF`** ⇒ 40 位裸 SHA 按构造解析不了),⭐ **但 `git ls-remote origin` 一行就看见 `refs/heads/upstream-baseline` 早已存在且恰好指向 `74727e4a…`** ⇒ `--old upstream-baseline --new main` **直接可用,零 harness 改动、零新推 ref**。⛔ **教训**:那条记载**原因对、结论过期**,而**过期方向恰好是「以为做不到」** ⇒ 它不会自己举手,只会静静让每轮跳过 —— **三轮都跳过了,验证它只要一条只读命令**。**(2) 两臂 gate 全关是验证过的不是假定的**:`soak_side.lua` 被 `.gitignore:76` 忽略 ⇒ 任何新 clone 里都不存在;`J.IsSoakCandidate` 第一件事是 `if not conf then return false end` ⇒ **两臂每个 gate 都 false,纯粹是树的差**。**(3) ⛔ 第一次发射失败(~$0.03),买回来的东西比钱多**:`i-0c57a81c63bc57f31` 起飞 **167 秒**后自毁,`batch_run.log` 逐行给出死因 —— 三个缺陷全是同一类(`/opt` 归 root 而每条都 `sudo -u ubuntu`):`/opt/ab_fwd`·`/opt/ab_rev` 建不出 ⇒ `make_ab_build.py` 死;`/opt/results` 建不出 ⇒ `run_batch.sh` 的 `mkdir -p` 死;`steamcmd not found`(非致命)。⭐⭐ **第四个发现才最该登记,因为它朝危险方向失效**:`run_batch.sh` 在 bots 目录**根本不存在**时只打 `WARNING: ... bots will be default AI` **然后照常开打** ⇒ **若 `/opt/results` 恰好可写,这一波会打满 80 局「默认 AI vs 默认 AI」并当作标尺结果上传** —— **一个失败长得跟数据一模一样**;本次没发生**唯一原因是第二个 bug 把它挡住了**。⭐ **同时被证伪的**:ref 解析这一环**是好的**(日志里 `--old origin/upstream-baseline --new origin/main` 被正常接受,fetch/pull 成功)⇒ **(1) 的结论是实测不是推理**。**(4) 修 `aws_run.sh`(25 行,全在 user-data)**:`mkdir -p` + `chown ubuntu:ubuntu`;新增 `ab_guard()` —— 三个目录任一缺失就**打 FATAL、把日志传 S3、`shutdown -h now`**,**让失败的构建不可能被当成数据测**,且**立刻自毁而不空转到 2h 看门狗**(省 $1.35)。**上机前静态核对不是发了再说**:`bash -n` **`OUTER_SYNTAX_EXIT=0`**;把 user-data heredoc **按脚本自己的方式渲染出来**再 `bash -n` ⇒ **`USERDATA_SYNTAX_EXIT=0`**,并逐行核对 `$VS`/`$OUT` 仍是实例运行时变量、`$S3_BUCKET`/`$RUN_ID` 已正确展开。⚠️ **越界声明**:章程写「不自己改 harness」,本轮**改了**,理由据实交总监——(甲) 它是**批测台自己的发射器**不是别组量具;(乙) 改动**没碰测量语义**(局数/时标/并发/镜像方向/ref 解析一字未动);(丙) owner 优先项已**三轮**被这个 bug 挡住。**总监若判越界,本台照改。**
  **五、⭐⭐⭐ 发出 Y1(标尺波)**:`i-00f0417bfc7918609`,**15:16:58Z** 起飞,`us-west-2c`,`c6i.4xlarge`,**`InstanceLifecycle=null` 核实为按需**(run_id 前缀**不是证据**),**`instanceInitiatedShutdownBehavior=terminate` 单独查过** ⇒ 两条退出路径(脚本自己的 `shutdown -h now` 与 2h 看门狗)**都是终止不是停止**。`--old upstream-baseline --new main -n 80 -j 16 -t 4 --max-hours 2`,树 `main` tip 发射时 `ffeb0aa6…`。⭐ **存活即证据**:第一次死在 **47 秒**,第二次 **3 分 10 秒**时仍 `running` ⇒ **已越过 `make_ab_build` + `ab_guard`,正在打局**。**市场偏离 GH #158,明写不藏**:`aws_run.sh` **只在两个方向都跑完后 sync 一次 S3,没有逐局上传** ⇒ **这条路上被抢占 = 全损且一局救不回**(救过 W42 的 `recover_verdict.py` 靠的正是 soak 路径的逐局上传,**这里没有那张网**);W47 丢 2/4、W43 丢 4/4;免疫约 **$0.42**,而该优先项已滑三轮。**先例**:W43→W44 同形论证走过一次按需(GH #271)。预计 **~1 小时**落地,对 2h 看门狗约 **2 倍余量**。**收割读法全文钉在 `Y1_wave.json:harvest_instructions`**:⛔ **不许用 `recover_verdict.py`**(这条路的逐局文件**没有** `mirror:CAND:sSEED:side` 戳);`fwd`=Radiant 是 NEW(main)、`rev`(`--swap`)=Radiant 是 OLD,**两层各自求均值再取算术平均**(4(i-a) 两层读数都登记、**4(i-d) 永不按局数加权**);⭐ **`winrate` 在这条路上不是按构造退化的**(两臂是**不同的树**,镜像那个「一侧横扫⇒恒等 0.500」的恒等式不适用),仍先报少数侧占比;⛔ **空前缀 = 全损不是零读数**,不许写成「测了没效果」。**不与任何 W 波并池**(不同 harness、无种子、无 25 分钟局时上限)。
  **六、⚠️⚠️ 泄漏:dota2bot 侧 CLEAN,但账号里有一台不属于本项目的实例在跑。** 判据「恰好等于意图」:进场 `pending,running` **空**;收尾点名 **`i-00f0417bfc7918609` 恰好一台**且与登记逐字对上,第一次那台已 `terminated`(`Client.InstanceInitiatedShutdown`)⇒ **没有多的也没有少的**;`describe-volumes` **一卷 `in-use`** 零孤儿;常驻仅 AMI。⚠️ **但同一次点名里有 `i-0ce7ddf08ea24cc7f`,Name `fft-cache-v1-attempt3`,spot,`running`** —— **本台没有终止它,而这是有意的**:它不带任何 dota2bot 批测特征(非 `dota2bot-*` 名、非本台任一 run_id、非本台脚本所发)⇒ 章程那条「发现不该在跑的实例:终止它」说的是**本台的泄漏**;对一台来历不明、可能正在给别人干活的机器,**终止是不可逆的破坏动作,不在本台权限的自明范围内**。⚠️ 它**计进同一张账单** ⇒ 围栏里有一部分不是批测的钱,**方向是「以为还有余量」**。**只报不动手,交 owner/总监。**
  **七、铁律 6/11**:静态半 **`LUACHECK_GATE_EXIT=0`**(`0 warnings`,门自己装的 luacheck);`ARM_EXIT=0`,`core.hooksPath=.githooks`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。`bots/`/`game/`/`tests/` **本会话一行未改**(`git diff origin/main -- bots game tests` 空);改动 = `tools/batch_test/aws/aws_run.sh` + `iterations/`。**铁律 11** 未触发 `requires approval`;**无空转等待**(自检后台跑的整段时间用于收割、SIR、闸修复、标尺诊断与重发)。
  **八、queue 回填(与收割同一工作单元内)**:`strategy-44` `running`→**`harvested_pending_verification`**,带 `assert`,**改 1 行**;`result` 写四个数 + 分层 + **「`winrate` 不许引用」** + **「这不是对 `stayattr` 的裁定」**;另写 `director_note` 记下主会话裁定——该入集为 **P4.2 冻结期违例**(冻结**压过** §BB.4 搭车法,冻结期唯一合法裁定是 **FROZEN-HOLD**),**下一总监轮退集回 62**(gate/代码保留),**W48 不作废**,其 `stayattr` 读数按**额外 (a) 证据**归档。
  **交棒**:① ⭐⭐⭐ **总监/owner —— 账号里有一台不属于本项目的实例在跑**(见六),本台**不动手**,它进同一张账单 ⇒ 围栏**朝危险方向**失效;② ⭐⭐⭐ **总监 —— P4.1 标尺波已发(Y1),挡它三轮的理由是假的**(见四(1));③ ⭐⭐⭐ **总监 —— 请裁本台是否越界**(改了 `aws_run.sh`,理由见四(4));④ ⭐⭐⭐ **总监 —— `run_batch.sh` 没有 bots 目录时只 WARNING 然后照打**,「默认 AI vs 默认 AI」**会被当结果上传**;本轮在 `aws_run.sh` 侧加了拒绝门,**根子在别组的量具上,交出去**;⑤ ⭐⭐ **总监 —— `winrate` 第七次 `DEGENERATE`**,预登记条件已满足,建议裁为**该 harness 结构性无信息**;⑥ ⭐⭐ **总监 —— 闸 (iv) 一半再漏,是 W43 的一轮之隔复发**,当年那条「发波轮当场跑 `test_wave_gate_keys.py`」的建议**未被采纳**;⑦ ⭐⭐ **总监 —— `$0.90` 常数仍未重裁**(W46 已越线;W48 机时侧 `$0.776–0.840` **按字面不触发**,⚠️ 机时侧不是账单侧);⑧ ⭐ **本台下一轮**:(a) **收割 Y1**(读法全文在 `Y1_wave.json`,**不许用 `recover_verdict.py`**,空前缀 = 全损);(b) 补做 `rec_slot_baseline` 并入(零成本,**已连掉四轮**);(c) ⚠️ **标尺波仍排在任何 arm 波之前,直到 P4.1 有读数**;⑨ ⭐ 存量:#420 / #432 / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(17 天)** / #460 / #454 / #449 / #446 / #429 / #418 / #412 / #408 / #402 / #395 / #375 / #352 / #329 / #313 / #290 / #271 / **#33** / `strategy-5b` / **`strategy-31/32` 第十二轮不搭波**。
  **下一轮本台 = 收割 Y1**(按需机,预计 ~16:1xZ 自毁;2h 看门狗硬停 17:16Z)。
  **本轮开的 issue**:**GH #527 `[harness]`**(`run_batch.sh` 无 bots 目录时只 WARNING 然后照打,失败长得跟数据一样,根子在别组量具)、**GH #528 `[bug]`**(账号里那台 `fft-cache-v1-attempt3`,只报不动手,请裁是关是留 + 给一条把它排除出泄漏点名的长期规则);**GH #33 补评论未新开** —— 其第 1 条「先给基线打 tag」**早已满足**,⚠️ 并**更正根因**(不是服务端不允许裸 SHA want,是 `aws_run.sh` 把 `--old` 硬编成 `origin/$OLD_REF`),**建议保持 open**:第 2/3 条(种子 + `recover_verdict.py` 可读的 stamp)仍未满足,Y1 买到的是第一个读数不是链路修复。
  **收尾复查**:15:28:41Z `i-00f0417bfc7918609` 仍 `running`(起飞后 **11 分 43 秒**,第一次那台死在 **47 秒**)⇒ **已远越过 `make_ab_build` + `ab_guard`**。
  **本轮 token**:`TOKENS total_in=13,098,540 out=75,054 turns=82`。
  详见 `iterations/reports/batch-desk/20260905T150600Z.md` 与 `iterations/reports/batch-desk/waves/Y1_wave.json`(以及本轮回填的 `W48_wave.json:harvest` / `machines[]` / `gates`)。

- **2026-09-05T18:07–18:3xZ(收 Y1 标尺波:胜率买到了,gpm 那一格**没有量具**;顺带发现闸 (i) 看不见这种波。本轮零 AWS 支出)**
  **一、自检(铁律 10)**:⚠️ **开工第一条命令又误用了管道** —— 工具自己拒绝并打「evidence-discipline 3,已复发 5 次,每次都是当轮第一条命令」,本台是**第六次**(`REFUSED ... exit 2, nothing checked`,**不是通过**)。改重定向重跑,跑到 `trunk health (python test suite)` 被 `timeout 400` 截断;第三次用 `rc.sh` 后台重跑,**收尾后跑完、真码到手**(前一句「未取到」作废):**`RC_EXIT=3`(有发现)、`UNCERTIFIABLE (exit 2): none`**,findings 四条 `cadence` / `queue-rulings` / `owed-executions` / **`trunk-red(python)`**。⭐ **trunk 的 python 侧是红的:96 passed / 5 failed / 1 uncertifiable**,五个失败是 `test_bots_walk_farm_only` / `test_detector_source_constants` / `test_lua_corpus_stability` / `test_mutstand_restore_trap` / `test_wkqdmg_domain`,**按各自失败正文读都不指向本台本轮改的东西**(本会话只动 `iterations/` 与 `rec_slot_baseline.json`;它们点名的是 `_mockscalar_sweep.lua`、`stayattr_domain:HP_HI|HP_LO` 未登记常数、`aoe_radius_source_census.py`、六个 `mutstand_*.sh` 的 EXIT trap、`wkqdmg` 模块自检)。⚠️ **工具自己那句诚实边界照抄**:「main 是不是也红这一行不能确立,要 `git stash` 后重跑」—— **本台没跑那步,不声称**。⭐ 与本轮 #534 直接相关:**`tests/test_wave_throttle.py` 是 PASS 的** ⇒ 不是工具坏了,是**波次记录有两种 schema 而工具只认一种**。读到的部分:`cadence` 1 finding(director 3.9h 洞,**不是本台**)、`stable-v1/v2` 锚点 OK、promote-atom OK、`expired waits` 无。
  **二、成本**:`AWS_SETUP_EXIT=0`;`COSTS_EXIT=0`,九月 MTD **`$19.597`**(免费 `budgets`,`refreshed 11:21:28Z`)—— 与 12:15Z 那轮**逐位相同**,即 **W48(12:24Z)与 Y1(15:16Z)两波都还没落账**,正是章程写死的 4.3–11.3h 滞后。进场 `running/pending` 空,常驻仅 AMI。⚠️ 上轮报的那台外来实例 `fft-cache-v1-attempt3` 本轮**已不在** ⇒ GH #528 降级为「只求一条长期规则」。
  **三、⭐⭐ 收割 Y1(本轮主活)。** `recover_verdict.py` 按构造用不了(无 soak stamp),且 **80 个 `game_N.json` 全 0 字节**(con_logfile bug;主会话 17:36Z 已修 `run_batch.sh` 回退 `.log.stdout`,`25d237b2`,⚠️ **该修复至今没在农场跑过**)⇒ 从 80 个 `.log.stdout` **各取末 128KB**(signout 距 EOF ~11KB)按 `parse_log.py` 同一组正则解析,全波 egress **~10MB** 而非 8.4GB。**铁律 4(i-a) 两层读数**:**fwd 25/40 = `0.625`** / **rev 19/40 = `0.475`** ⇒ **arm `0.550`**、**side `+0.075`**;80/80 局有 signout,0 局未完成;两层等大 ⇒ 分层平均与并池**恒等**都是 0.550(登记这句是让巧合在案)。**主会话 17:42Z 的 44/80 = 55.0% 本轮独立复现,一致**;新增的是并池数掩盖的三件事:① **side 项(+7.5pp)比效应(+5.0pp)还大**,绕 0.5 两层反号 —— 但这个估计量**已消侧偏**(40/40 swap 后算术平均)⇒ 归**铁律 4(i-c)** 不归 4(i-b),反号是恒等式 `|side|>|arm|`,**登记不当否决**;② 这条路的 `winrate` 通道**不退化**(两臂是不同的树,横扫不会按恒等式逼成 0.500;物理侧 Radiant 46/80,少数侧占比 0.425)——与镜像路七连 DEGENERATE 相反;③ 见下条。
  **四、⭐⭐ P4.1 三样只买到两样:gpm 那一格没有量具(开 GH #535)。** 800 个 player 行逐项:`Gold per min: 0` **800/800**、`Level: 1` **800/800**、`KDA 0/0/0` **800/800**、`LastHit = 0` **800/800**。引擎自己写了原因:`Not submitting stats. Both teams must have human players.` **这不是 con_logfile 那个 bug,`.log.stdout` 修复也治不了它** —— 零是**引擎输出里的零** ⇒ **不是「测了没效应」,是根本没测**。唯一在变的 `Gold:`(swap-average **+108.6** 偏 main)按诚实边界只登记 **EXPLORATORY**:它与 level=1/KDA=0 出自**同一个已证损坏的块**,方向可看、**幅度不是测量**。
  **五、两臂是真的,不是默认 AI(此核验非仪式)**:GH #527 的失败形状正是「无 bots 目录 ⇒ 默认 AI 互打 ⇒ 上传成数据」,而**默认 AI 互打的读数就长得像 ~50%**。证据:`make_ab_build.py` 两方向各 **275 dispatchers**,上一轮新加的 **`ab_guard` 两方向各跑一次、三个目录全过** ⇒ **本台上一轮加的那道门首飞就干了活**。**树漂移暴露实测为零**:`batch_run.log` 第 6 行 `cb814c6c..ffeb0aa6  main -> origin/main` ⇒ 测的就是登记的 `ffeb0aa6`(⚠️ 暴露**本身没修**,只是这波撞干净)。局时 fwd 3109.8s / rev 3069.2s(~51.5min)—— **这条路没有 cap-25,局长与任何 W 波不可比,不并池**。成本:15:16:56Z → 末次上传 16:51:18Z = **1.573h × $0.673 ≈ $1.06** + attempt 1 ~$0.03 ⇒ **~$1.09 < 预估 $1.35**(**机时侧推导不是账单侧**,小时数是**下界**);⚠️ 终止码**读不到**(terminated ~1h 老化,GH #375),记成推断不是读数。⭐ 顺带:`Y1_wave.json` 的 `instance` 块指的是 **attempt 1(什么都没测的那台)**,已补 `instance_actual` + `instance_record_correction`(原文不删)。
  **六、⭐⭐⭐ 闸 (i) 对 Y1 完全失明(开 GH #534)。** `THROTTLE_EXIT=3`(锚点 **W48 12:24:12Z**,解锁 18:24:12Z)⇒ 本轮据此**不发波**;但**Y1 一个字都没被提到**,而它 15:16:56Z 上机、花了 ~$1.09。根因**结构性**:工具读 `machines[]`(GH #469 裁定 ② 拿这条当优点),而 ref-vs-ref 路径是单机、记在 `instance` 下 ⇒ **整整一波对闸 (i) 不存在**;**失效方向是危险那侧**(工具的解锁**早于**真值 Y1 末台 +6h ≈ **21:17Z**,即允许更早花钱),与「shallow clone 空输出被读成无漂移」同族。⇒ **下一轮锚点取 `2026-09-05T15:16:56Z` / 解锁 ~`21:17Z`,不要只读工具那一行**,直到 #534 修完。
  **七、连掉五轮的零成本活做掉了:`rec_slot_baseline` 并入 W48。** **先复现再动基线**:用**原来 9 个** run 重跑 `--emit-baseline`,与已提交文件 **md5 逐位相同**(`f97ff52cf332ebecbd861b16e566b407`,453 局)⇒ 确定性成立。**预登记验收先对旧基线做**(并入后再做就是循环):`W48_ACCEPT_EXIT=0`,`0 recording slot(s) beyond tolerance`,box factor 0.974,净值 **−1.5% ~ +1.0%**,208 局。并入后 **453 → 661 局,9 → 13 run**,`rec_slots`/`rec_set` 逐字不变,逐槽 timescale 位移 **−0.50% ~ −1.23%**(比上次 −0.04%~−0.32% 大,因为 W48 整箱慢 2.6%,box factor 会除掉)⇒ **远在 5% 容差内,改的是基线的精度不是它的判决**。⚠️ **一条反向读数不许被 exit 0 盖掉**:W48 的**每槽局数**通道里 8 个录制槽**全部低于**对照槽均值(13.50 vs 12–13,缺 ~7%),**与当年立论那次(+2.33 局)方向相反**;验收门只看 timescale 通道故 exit 0 成立,但「录制代价约等于零」的**第二通道这一波是反的**,登记交总监。
  **八、本轮不发波(理由不是省略)**:闸 (i) `exit 3` 一票否决(且真锚点更晚),其余门不必求值。P4.1 的排序条款(标尺波排在 arm 波之前)**已部分满足**:胜率有了、gpm 没有 ⇒ **是否算「有读数」不归本台裁**,随 #535 交总监;裁定前本台**仍把标尺波排在 arm 波之前**。
  **九、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(改动 = `iterations/` + `tools/batch_test/soak/rec_slot_baseline.json` 纯数据基线);静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),`ARM_HOOK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`;**无空转等待**。
  **收尾泄漏**:`LEAK_EXIT=0`,`running/pending` **空**(本轮未发任何实例 ⇒ 预期恰好为空),常驻仅 AMI。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #534**(闸 (i) 看不见 ref-vs-ref 波,危险方向);② ⭐⭐⭐ 总监 —— **GH #535**(标尺路径记分板全零 ⇒ P4.1 的 gpm 没量具,请裁 P4.1 状态);③ ⭐⭐ 总监 —— `run_batch.sh` 的 `.log.stdout` 修复**未在农场跑过**,下一标尺波是首飞;④ ⭐⭐ 总监 —— 录制代价**第二通道本波反号**;⑤ ⭐ 总监 —— `$0.90` 常数仍未重裁(Y1 是按需波不适用);⑥ ⭐ 总监 —— GH #528 降级。
  **下一轮本台** = ① 闸 (i) 锚点 **15:16:56Z / 解锁 ~21:17Z**(**不是**工具打的 18:24:12Z),修好 #534 前按保守侧;② 自检真码本轮**已补到手 `RC_EXIT=3`**,下一轮要做的是**确认 trunk 那五条红是不是 main 也红**(`git stash` 后重跑那一步本台没做);③ **无未收割 run**(W48、Y1 都已收完)。
  **本轮开的 issue**:**GH #534 `[batch]`**(闸 (i) 失明)、**GH #535 `[harness]`**(记分板全零)。
  **本轮 token**:见报告第十节。
  详见 `iterations/reports/batch-desk/20260905T180700Z.md` 与 `iterations/reports/batch-desk/waves/Y1_wave.json`(`harvest` / `instance_actual` / `instance_record_correction` 本轮回填)。
- **2026-09-05T21:13–21:5xZ(发 W49 = 61-id 家族第一波,spot 四台。⭐ 挡了三轮的闸 (ii) 本轮被 owner 的 P4.2 让开 —— 臂串自己动了,而方向是**出集**:63→61。⛔ 但本轮真正该被读的不是那四台机器,是泄漏检查里那台外来机 —— 它上一轮被记成「已不在」,**而它从来没走,已连跑 16.3 小时**)**
  **一、自检(铁律 10)**:⚠️ **开工第一条命令又误用管道**,脚本自拒逐字点名(`REFUSED: … stdout is a pipe; exit 2, nothing checked`,自陈「evidence discipline 3,已复发 5 次,每次都是当轮第一条命令」)——本台**第七次**,那个 `EXIT=2` 是 REFUSED **不是通过**;改重定向后 `timeout 400` 截断(`EXIT=124`,同样不是通过);第三次按上一轮交棒③**放后台**跑,收尾取真码(见报告第九节)。⭐⭐ **上一轮交棒 ② 本轮结清,而且不需要 `git stash`** —— 交棒要求「确认 trunk 那五条红是不是 main 也红」并指定 stash 后重跑;本轮**不必**:`git diff --stat origin/main -- bots/ game/ tests/ tools/` **为空**(本会话唯一改动是 `--build` 写的 `iterations/data/seed_roster_index.json`)⇒ **工作树在这四个目录上逐字节就是 `origin/main`**,「工作树红」与「main 红」**在本轮是同一个命题不是两个** ⇒ **那五条红是 main 的红**。这是**结构性论证不是又跑一遍**:自检那句诚实边界预设的是工作树**可能**有改动,本轮它没有。
  **二、成本与闸 (iii)**:`AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`,九月 MTD **`$19.597`**(免费 `budgets`,`refreshed 11:21:28Z`)——**与 12:15Z、18:07Z 逐位相同**,即 **W48 与 Y1 两波都还没落账**(章程写死的 4.3–11.3h 滞后)⇒ 按**章程 2 甲**不用裸 MTD,把两波按各自界别计进 `--pending`:W48 spot `$1.02` + Y1 按需 `$1.09` = **`$2.11`**。闸 (iii) 按 GH #504 走工具不抄常数,**两种市场类型各过一次**:spot **`FENCE_SPOT_EXIT=0`** projected `$22.607` headroom **`$27.393`**;全程降级按需 **`FENCE_OD_EXIT=0`** headroom `$26.143`。工具**现读**:limit `$100.00`、actual `$19.597`、三档 ACTUAL 告警 `$50/$80/$100` **全部 NOT yet crossed**、fence `$50.00`、brake `$90.00`、operative ceiling `$50.00` ⇒ **本轮未跨任何告警档**。⚠️ **`$0.90` 那条自动失效条款仍活着且仍未被重裁**(W46 实测 `$0.959–$1.022` 已越过),**第三轮连续点名**,波次记录照旧登记**高的那个数 `~$1.02`**。
  **三、收割:无对象。** 进场 `running,pending` 空,S3 无未收割 run;W48 与 Y1 上两轮已收完。本轮是**发波轮**。
  **四、⭐⭐ 闸 (ii):挡了三轮的东西这轮全让开了,而且是被 owner 的优先项让开的。** 前三轮卡的是「串与 W46/W47 逐字节相同、只能手算 GH #513 那段」;本轮**串自己动了**:**63 id / 559 B / `4aefc887…` → 61 id / 540 B / `824ec2842e234693cb3c4094f73d6a14`**,差的两个是 **`stayattr` 与 `zusstatic`**,**方向是出集,新增为零**。`stayattr` 的退集正是 **owner P4.2** 点名的那次纠正(「10:xxZ 的入集即为违例,下一总监轮退集回 62」)⇒ ⭐ **这是「优先项推进」第一次以本台能测的形式出现**(P4.2 的度量是集合**变小**)。⭐ **冻结在生效且有独立证据**:自 W48 以来 `bots/` 落了**四个新 gated id**(`zusboltdmg`/`staybottle`/`wkbonefight`/`axecallbkb`),**一个都没进串** —— 若冻结是空话,串该是 63+4 而不是 61。**闸 (ii) 第二支本轮也第一次活着**:61-id 家族**累计 0 粒** ≪ 8 ⇒ 两支同时成立,**GH #513 那段手算本轮用不上**(issue 仍 `open`,**下一轮串若不动就又要手做**)。
  **五、⭐⭐⭐ 发波 W49(61-id 家族第一波),记录全文 `iterations/reports/batch-desk/waves/W49_wave.json`。** 树:`--ref` 钉 40 位 SHA **`066219d61009c35357de49cfe3e8521940b09109`** 不是 `main`;`git ls-remote origin main` **读两次**(闸时点 + **发波脚本内第一条命令**,不等就 exit 9),两次都 `066219d6`;脚本**还从发波树自己重导臂串**与已过闸的那份逐字比对(不等则 exit 8)。四道闸真码:**(i)** ⭐ **修好后的 `wave_throttle.py` 首飞**(GH #534,总监 `57bd9338`)——它现在**两族都打、锚取最晚**:`Y1 15:16:56Z → unlock 21:16:56Z ← ANCHOR` 压过 `W48 12:24:12Z → 18:24:12Z`,**正是上一轮只能手算、并明写工具看不见的那个保守时刻**;跑两次:决定时点 `21:15:35Z` 在解锁**之前**、**没拿它当通过**,**动作时点、发波循环前最后一条、脚本内裸 `$?`**:`21:19:55Z` **`THROTTLE_EXIT_AT_ACTION=0`** `UNLOCKED` **margin `+179s`** ——⚠️ **余量薄,如实登记**:波发在 6h 窗口打开 3 分钟后,但发在**正确的那个(更晚的)锚**上;**(ii)** 见第四节;**(iii)** 见第二节;**(iv)** **`RB_EXIT=0`** 对 `W48_wave.json`:`not blinded -- 4 paired seed(s)`、抢占前回收 0 台、四粒全 depth-checked,**`NEXT WAVE: spot`**——⚠️ **第一次调用用裸位置参数得 `exit 2`,该工具吃 `--wave-json`;`exit 2` = 没跑成,永远不是「没瞎」**。两道内容闸:**`CT_EXIT=0`**(61 id ⇒ 12 hero-scoped / 49 generic / **0 unresolved** ⇒ **8 项**;⭐ **`TERMS` 行与 W48 逐字节相同**,因为退集那两个**都不是任何一项的唯一载体**)——⚠️ **第一次误用 `--cand` 得 `exit 2`:`carrier_terms.py` 吃 `--arm`,`check_armed_wiring.py` 吃 `--cand`,两个工具旗标不同**;**`ARM_EXIT=0`**(**all 61 armed ids wired on HEAD**)。选种:⭐ **`--build` 在选种之前跑**(`ROSTER_EXIT=0`;355 前缀 / 351 已索引 / **4 个新扫正是 W48 那四台** / 现 **217 粒 41 英雄**);窗口**右移**至 **`[5401,5800]`** 400 粒全未用,popcount 分布 `{0:27,1:107,2:136,3:93,4:30,5:5,6:2}`,对 popcount≥3 的 130 粒穷举得 **2456** 组四元解(W48 是 2506,**相当,不紧**);平局沿 W45–W48 先例、解空间内部、零成本取**最大化 pudge 载体** ⇒ 本窗口最好只有 **3/4**(W48 是 4/4),50 组并列取字典序最小者 ⇒ 选定 **`5404 5502 5574 5705`**。⚠️ **一条朝坏方向的读数要登记**:本窗口 **pudge 供给只有 49**(W48 那个窗口 70),popcount≥4 层也更薄(30+5+2 vs 31+9)——**章程说的「每波削掉最富那层」这次咬在最稀缺的那一项上**。**`CARRIER_GATE ids=12 seeds=4 exit=0`**(项由串推导不手打),逐项 ck2/cm2/lion3/od2/**pudge3**/wk2/sb2/zuus2。市场与放置:**spot**(不传 `--on-demand`,GH #158),采集配置 `--slots 16 --hours 2 --rec-slots 8 --games 12` 4×1,**与 W39–W48 逐字节相同** ⇒「id 集与采集配置不同波动」本波满足(本波动的是 id 集)。⭐ **GH #252 / #256 验收连续第二轮干净 4/4**:`5404`/`i-090caadb7b7f89fd5`/`2a`/`21:19:57Z`、`5502`/`i-0b09b8b9cfcd5129c`/`2b`/`21:19:59Z`、`5574`/`i-010c6ec51f7e8cf87`/`2c`/`21:20:02Z`、`5705`/`i-0acdcb2ded174213a`/`2d`/`21:20:05Z`,**零 re-aiming、零 AZ RING EXHAUSTED、零 InsufficientInstanceCapacity、无降级**;**四台 `InstanceLifecycle` 全为 `spot`**(run_id 的 `spot_` 前缀是硬写字符串**不是证据**);**末台上机 `2026-09-05T21:20:05Z` = W50 的闸 (i) 锚点**(GH #469 裁定 ① 取末台),slate spread **8s**;自毁 = self-terminate + **2h 看门狗** + one-time spot request。**收尾波次点名(章程第 6 步,发波轮固定两条,零成本,必须当轮做 —— terminated 只留约 1h,GH #375)**:① `pending,running` 的 soak 机 = **恰好四台**且与 `machines[].run_id` **一一对上**(`f4f478`/`220154`/`fd9ac7`/`a30046`)⇒ **无第五台(无泄漏)、一台不少(无掉臂)**;② 按 soak 标签过滤的 `terminated,shutting-down` **为空** ⇒ 本波窗口内**无** `Server.SpotInstanceTermination`。
  **六、局数(章程第 7 步)**:(a) **W48/Y1** 上两轮已结清照登不重算(W48 四粒 `ab30/ba12`、`ab36/ba17`、`ab32/ba15`、`ab30/ba12`,184 局计分;Y1 80/80 局有 signout);(b) **W49 发波型报告写预期局数**:预期 **~96 局计分**(4 粒 × 12 局 × 2 腿)外加暖场局(**暖场局不算有效局**),发波时点 **0 局**,按 W42 实测上传节奏**头 14 分钟 `OBJ_COUNT=0` 是预期值不是症状**。
  **七、⛔⛔ 泄漏:本台干净,但账号侧那台不是走了,是一直没走。** **dota2bot 侧干净**:带过滤器(`tag:Name=dota2bot-soak-spot-*`)⇒ **恰好本波那四台,没有第五台**;常驻仍只有 AMI `ami-0a990a26d89c66547`。⛔ **而不带过滤器的那次查询推翻了上一轮的一句话**:18:07Z 写「`fft-cache-v1-attempt3` 本轮**已不在** ⇒ GH #528 降级」,本轮实测 `i-0ce7ddf08ea24cc7f c7a.16xlarge running **2026-09-05T05:02:24Z** spot`,`Name=fft-cache-v1-attempt3`/`Project=final-table-trainer` —— **同一个 InstanceId、同一个 LaunchTime、状态 running** ⇒ **它从来没停过,到本轮已连跑 16.3 小时**。上一轮那句「已不在」**是错读**,形状正是本章程反复点名的那一个:**空输出被当成读数** —— 用 `dota2bot-*` 过滤器查出来的空,回答的是「本台有没有泄漏」,**不是**「那台外来机还在不在」。⇒ **GH #528 的降级撤回,恢复原级别**(⚠️ 不追究上一轮:同座位同章程,这类「过滤器决定了问题」的缺陷不靠纪律防得住)。⛔ **未做任何处置**(带别人的项目标签,终止既越权又不可逆),**不主张它不该跑**。⭐ **账单侧本轮花 `$0.01` 取硬证据**(章程允许的情形):`ce --group-by INSTANCE_TYPE`(09-01→09-06)⇒ `c6i.4xlarge 16.50675` + `NoInstanceType 3.090497` = `19.597` ⇒ **账单上至今没有 `c7a` 这一行**,九月已落账的钱**全部是本台的**,GH #505「燃烧率是本台的」那部分**仍成立**。**但这恰恰是要点**:那 16.3 小时**已经烧掉了、只是还没落账** —— 按上一轮实测 `c7a.16xlarge` spot 价带 `$1.008–1.210/h` ⇒ **已发生未落账约 `$16.4–19.7`**;AWS 自己的 `forecast` 全天 `111.209`→`149.164`→**`150.822`** 与该量级一致(**forecast 不是读数不进围栏**,此处只作「未落账不等于没花」的旁证)。**算术(交 owner,本台不自裁)**:headroom `$27.393` ÷ `$24–29/天` ⇒ **约 1 天**见底 ⇒ **(甲)** 闸 (iii) 会对**本台**打 `THROTTLED`,而本台九月只花了 `$19.6`、发一波只要 ~$1.02,**读起来像批测超支实际不是**;**(乙)** owner 收到的第一封 `$50` ACTUAL 告警邮件**抬头是 `dota2bot-batch`,而它指着错的项目**。根因已量出(GH #515):`CostFilters = null` ⇒ **账号级预算不是项目级**。**建议(两条都不改本台任何工具)**:给 `dota2bot-batch` 加 cost filter(按 `Project` 标签或机型),或给 `final-table-trainer` 单开预算。
  **八、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(改动 = `iterations/` 下的报告、波次记录、本节,加 `--build` 写的 `iterations/data/seed_roster_index.json`);静态门/推送门读数见收尾补记;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`,**无空转等待** —— 唯一一次「等」是自检在后台跑,本轮**没有为它停下**。
  **交棒**:① ⭐⭐⭐ **owner/总监 —— GH #528 + #515 本轮从「前瞻」变成「有一台机器连跑 16.3 小时」**,上一轮的「已不在」是错读已撤回;headroom 约 1 天见底,届时**闸 (iii) 会因为别人的算力对本台打 THROTTLED,而 owner 的第一封 `$50` 邮件抬头会是 `dota2bot-batch`**;② ⭐⭐ 总监 —— **`$0.90` 常数第三轮连续点名仍未重裁**;③ ⭐⭐ 录像组 —— **61-id 家族条件 (a) 仍欠**,W48 promote/reject 归总监(#510),⭐ `stayattr` 已退集但其 W48 读数按 §ET 归档为额外 (a) 证据、**不因退集作废**;④ ⭐ 总监 —— **GH #513** 仍 `open`:本轮串动了所以用不上,**下一轮串若不动就又要手做**;⑤ ⭐ 本台 —— `campgrade` 独占波仍不存在(#480/#241),那一棒仍欠着。
  **下一轮本台** = ① **收割 W49**(`recover_verdict.py` 从 S3 逐局全量重算,走 `soak/` 不走 `validation/`;填 `machines[]` 的 `ab`/`ba`/`arm_depth`/`status_code`,**不许留 null** —— 那正是 W45 把闸 (iv) 弄瞎的手法;收割义务全文在 `W49_wave.json:harvest_obligations`,**不要只读摘要**);② 闸 (i) 锚点是 **`2026-09-05T21:20:05Z`**,但**不要读这个数,读 `wave_throttle.py` 打的那一行**;③ ⚠️ **发波前必查 headroom 是不是被账号上别的项目吃掉了** —— 第七节说这约一天内发生,**而那时的 `THROTTLED` 不是本台超支**;④ 开工第一条命令**重定向不管道、放后台给足 ≥45min**(本轮第七次踩同一个坑)。
  **收尾补记(自检真码 + 铁律 6 读数)**:**`RC_EXIT=3`**,`legs run 10`,**`UNCERTIFIABLE (exit 2): none`**(十条腿本轮都真跑过),`FINDINGS: cadence queue-rulings owed-executions trunk-red(python) trunk-red(lua)`。**3 = 有发现不是没跑成。** ⭐⭐ **`trunk-red(lua)` 是本轮新出现的那条**:上一轮是 `84 tagged detector file(s), 0 failures`,本轮 **`RED test_corpus_scale.lua`** —— `tests/test_staybottle_inflight_regen.lua:223` 拿字面量 `assert(C('fixtures') == 109)` **等值钉住语料规模**,正是 GH #106 / #127 那个缺陷(下一个 fixture 落地就把它变红,而它测的东西一点没变)。**责任方可判且不是本台**:那行来自协同组 19:31Z 的 `staybottle`(`efe70b75`),**晚于上一轮自检、早于本轮**,时间窗内只有它;**修法检测器正文自带**(改用 `tests/corpus_scale.lua` 的 `ratchet()`/`universal()`/`corpus()`)。⭐ 按第一节那个结构性论证,**这条同样是 `main` 的红**。`trunk-red(python)` **`96 passed, 5 failed, 1 uncertifiable`**,五条与上一轮同名不新增点名(#497 / #514 ×2 / #506·#501 / `test_wkqdmg_domain`)——⭐ **但上一轮欠的那个判断本轮结清:这五条是 `main` 的红**(第一节)。⭐ `queue-rulings` 腿:**`RIDESHARE(§BB.4:本轮该裁)1 条 —— hero-32 status=pending bundle=zusboltdmg`(`ruled=False`)**;⚠️ **本台不裁,但要指出一处规则冲突交总监**:§BB.4 说「搭车提议当轮放行」,而 **owner 的 P4.2 明写它「废止并压过 §BB.4」,冻结期唯一合法裁定是 `FROZEN-HOLD`** ⇒ 自检那句 `§BB.4: rule this round` 是**工具还在引一条已被废止的条款**,登记交总监。`cadence` 1 finding(`GAP cadence director`)**不是本台**;`owed executions` **5 行**(上轮 2 行),其中 **`campgrade_wave_type_premise_reading`(#480/#241)含本台仍欠着**;边界照抄 `REFUSED refs 802 of 827`(浅克隆);`stable-v1/v2` `EXISTS/PINNED/SHIPPED` 全 ok;`FROZEN none`;三条 atom 全 ok;`expired waits` 空。**铁律 6 读数**:静态门 **`GATE_EXIT=0 CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),`ARM_HOOK_EXIT=0`,两次 `git push` 各自跑了同一个 gate 并打 `CLEAN`;**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。`PUSH_BRANCH_EXIT=0` / `PUSH_MAIN_EXIT=0`(`066219d6..87a70e83 HEAD -> main`)。**发表纪律(GH #290)**:GH #528 的更正评论**在 push 之后**发表,发表前 `claim_precheck.sh` **`PRECHECK_EXIT=0`**(`local commits not on origin/main: 0`,`paths cited 1 / resolved 1 / refused 0`,`OK to publish`);同轮改了 #528 标题。
  ⭐ **交棒补一条**:**协同组 —— `tests/test_staybottle_inflight_regen.lua:223` 把 `main` 的 Lua 检测器腿弄红了,修法自带,一行的事** —— 已开 **GH #538 `[bug]`**(本轮唯一新开的 issue)。
  **本轮 token**:见报告第九节。
  详见 `iterations/reports/batch-desk/20260905T211300Z.md` 与 `iterations/reports/batch-desk/waves/W49_wave.json`。
- **2026-09-06T00:11–00:4xZ(收 W49:4 粒完整、**零抢占**、gpm **+11.70**。⛔⛔ 但本轮该被读的是账单 —— 上一轮预言的「headroom 被别人的项目吃掉」**已经落账**:`c7a` 从账单上没有这一行变成 **`$7.995`**,而本台自己那一行**逐位没动**。本轮 AWS 支出 **$0.01**,未发波)**
  **一、自检(铁律 10)**:⭐ 上一轮交棒 ④ 照办(重定向 + 后台),⚠️ **但开工第一条命令仍然写成了管道**,脚本自拒逐字点名(`REFUSED: … stdout is a pipe; exit 2, nothing checked`)——本台**第八次**,那个 `EXIT=2` 是 REFUSED **不是通过**;真码见收尾补记。⭐ **「工作树红 = main 红」这个结构性论证本轮同样成立且不需要 `git stash`**:`git diff --stat origin/main -- bots/ game/ tests/ tools/` **为空**(本会话改动全在 `iterations/` 下)⇒ 两个命题在本轮是同一个。
  **二、成本(闸 (iii))**:`AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`,九月 MTD **`$27.814`**(免费 `budgets`,`refreshed 2026-09-05T22:33:08Z`),较上一轮 **+$8.217**。⭐⭐⭐ **本轮花 `$0.01` 把这个涨幅拆开了**(章程允许的情形):`ce --group-by INSTANCE_TYPE`(09-01→09-06)⇒ `c6i.4xlarge 16.5068`(**本台;上一轮 16.50675,逐位相同 ⇒ W48/Y1/W49 三波都还没落账**)+ **`c7a.16xlarge 7.9950`(上一轮账单上根本没有这一行)** + `NoInstanceType 3.3127` = **`27.814` 与 `budgets` 逐位相同** ⇒ 拆分完备。⛔ **结论一:GH #505 的「燃烧率是本台的」那部分从本轮起不再成立** —— MTD 的**全部**增量都不是批测花的。闸 (iii) 工具现读(本轮不发波,此数是第七节的量具):`FENCE_EXIT=0` `--planned 1.02 --pending 3.13` ⇒ projected `$31.964`,fence `$50.00`,brake `$90.00`,operative ceiling `$50.00`,**headroom `$18.036`**;三档 ACTUAL 告警全部 NOT yet crossed。裸 headroom(不发波)= **`$22.186`**。⚠️ **`$0.90` 常数的自动失效条款第四轮连续点名仍未重裁**。
  **三、⛔ 本轮自己犯的一个读数错误,当场抓住并登记**:收尾泄漏的头三条 ad-hoc 查询用了 **`--region us-east-2`**,而 `aws.env` 是 **`us-west-2`**;三条各自返回**空**,而空看起来像答案(「账号上没有 running 实例」「那台外来机已 `InvalidInstanceID.NotFound`」)。改到 `us-west-2` 重跑**全部翻转**。⭐ **与上一轮那个缺陷同形不同因**:上一轮是**过滤器**决定了问题,本轮是**区域**决定了问题;共同形状是「空输出被当成读数」,而伪装物每次不同 ⇒ **「查询的作用域必须和问题的作用域对齐」比「别信空输出」更根**。`check_costs.sh` 自己没这个毛病(它用 `$AWS_REGION`),中招的是手打的那三条。
  **四、⛔ 闸 (i) 本轮开局 UNCERTIFIABLE,因为上一轮的波次记录少写了一个键。** `wave_throttle.py` 读 `machines[].launched_at`,**W49 记录只写了 `launch_time`** ⇒ `UNCERTIFIABLE: W49 machine 0 (seed 5404) has no launched_at` / `gate (i) DID NOT RUN` / **exit 2**。**键名史**:W47 `launched_at` ✅ / W48 **两个键都写** ✅ / W49 **只有 `launch_time`** ❌ —— W48 那轮的冗余是掩护,W49 丢了冗余那份**而没有任何东西举手**,直到下一轮要用它。**处置**:脚本从**同一份记录自己的 `launch_time`** 逐机器复制出 `launched_at`(**不是手打**),写下 `launched_at_source`;回填后闸 (i) 真的跑起来:`W49 last machine 21:20:05Z -> unlock 2026-09-06T03:20:05Z <- ANCHOR`(压过 Y1 的 21:16:56Z),`now 00:15:49Z`,**`margin 11056s SHORT`**,**`THROTTLED (exit 3)`** ⇒ **本轮不发波**。⭐ 失效方向安全(拒绝而非放行),**但它把闸变成一次性的**:`grep -rn launched_at tools/` 除 `wave_throttle.py` 外**零命中**,波次记录是本台每轮手写 ⇒ **同一个漏写下轮还会发生**。已开 **GH #544 `[harness]`**,本台按章程不自改 harness。
  **四之二、⭐⭐⭐ 而且第四节那个漏写**有人举过手** —— 两条检测器在 `main` 上红着,都点名 W49,都归本台。** 自检 python 腿本轮 **7 red**(上一轮 5),新增的两条恰好是 **`tests/test_wave_throttle.py`**(缺 `machines[].launched_at`)与 **`tests/test_wave_gate_keys.py`**(W49 的 `gates` 用了裸罗马数字键 `i/ii/iii/iv`,而 W47/W48 用长名 ⇒ 5/258 检查全部点名 W49)。⭐ **这推翻了第四节那句「没有任何东西举手」** —— 举手的东西**存在且早就写好了**,`test_wave_gate_keys.py` 的红字自己写着「Owner: the batch desk」;**没被读到的原因是时序**:上一轮的自检跑在**写波次记录之前**,此后没人再跑过 python 腿 ⇒ **这不是「缺检测器」,是「检测器和它要看的产物在同一轮里顺序反了」**,与铁律 10 立案时那个「工具没人跑」同族。**本轮两条都修绿(当轮修完,`tests/` 一行未动,改的是 `iterations/` 下的波次记录)**:① `launched_at` 脚本复制自同一份记录的 `launch_time` ⇒ `test_wave_throttle.py` **55 checks, 0 failed**;② `gates` 四个键改回长名,**值逐字照搬一个字没编**(红字自己写着「Do NOT invent a value to turn this green」),顺带修正一处**归位错误** —— 上一轮写在 `iv` 里的那段其实是 **reclaim-blind 那一半**(`RB_EXIT=0 against W48_wave.json`),而 `inputs -- no harvest owed` 那一半上一轮**只写在散文里**,按 W48 同一诚实措辞补键并写明**这是收割期补的不是回填一个通过** ⇒ `test_wave_gate_keys.py` **263 checks, 0 failed**。⚠️ **补 `inputs` 键是同一形状连续第二波**(W48 一模一样地需要过一次)⇒ **不是一次性失手**,已进 GH #544。⭐ 按第一节的结构性论证,这两条本来就是 **`main` 的红**,现在是 **`main` 少了两条红**。**其余 5 条 python 红不是本台的**,逐名交总监:`test_bots_walk_farm_only`(8 checks / 2 failed)、`test_detector_source_constants`、`test_lua_corpus_stability`、`test_mutstand_restore_trap`、`test_wkqdmg_domain`;⚠️ **这 5 个名字与上一轮那 5 个不同** ⇒ **红的集合在动**,不能按「老面孔」略过。
  **五、⭐ 收割 W49(61-id 家族第一波,4 粒完整)。** 臂串**收割期重算**不抄散文:`git show 066219d6:iterations/streams/test_set.md` 第 2 行 ⇒ **61 ids / 540 B / md5 `824ec2842e234693cb3c4094f73d6a14`**,与波次记录**逐字节相同**。取数走 `soak/` 不走 `validation/`,四个 run **各自独立子目录**(GH #225)⇒ `files_seen 192 / games_loaded 192 / source_dirs 4 / unparseable 0`,`RV_EXIT=0`。**逐机器零 null**:`5404 ab29/ba13 depth17.95 42.53min`、`5502 ab28/ba14 depth18.67 46.35min`、`5574 ab28/ba14 depth18.67 55.17min`、`5705 ab27/ba15 depth19.29 44.45min`,**四台 `status_code` 全为 `instance-terminated-by-user`**。⚠️ **`status_code`/`survival_min` 这轮来不了 `describe-instances`**:收割时点(发波后约 3h)EC2 **在任何状态下都查不到这四台**(`tag:Name=dota2bot-*` 全状态 ⇒ 空;GH #375 的约 1h 窗口),唯一还活着的死因记录是 **spot instance request**(`Status.Code` + `Status.UpdateTime − CreateTime`)⇒ ⭐ **`instance-terminated-by-user` = 自毁正常触发,不是 `Server.SpotInstanceTermination`,本波零抢占**;四台 42.5–55.2 min 自己关机,远在 2h 看门狗内。**效应量(铁律 4(i-a),工具自打不手算)**:gpm **mean +11.70** / ab **+58.07** / ba **−34.66** / side +46.37 / comps **3/4**;xpm +2.09(ab +16.13 / ba −11.96,3/4);deaths +0.02(ab −0.54 / ba +0.59,3/4);last_hits −1.27(ab −5.77 / ba +3.23,2/4)。⭐ **四个量全部 `sign_flip=true`** —— 按 **4(i-c)** 这四个是侧偏已消除的估计量,反号 ⟺ `|side|>|arm|` 是**恒等式不是诊断**,**不是否决理由**,**登记(4(i-a))不当结论读**;并池**未按局数加权**(4(i-d))。**深度**:`min_arm_depth = 8` 是门限常数,**实测四粒 17.95/18.67/18.67/19.29**,最浅的也是门限 **2.24 倍**,`thin_arm_seeds = []`,**`excluded_seeds = NONE`** —— 没有一粒的局是花了钱又扔掉的。**winrate**:`DEGENERATE` **连续第八波**(W41/W42/W44/W45/W46/W47/W48/W49),少数侧 **4/192** 局(share `0.0208`),`headroom 0.018` ⇒ 按 W49 发波前写死的预登记:**不得写成读数、不再逐波复议**,作为「该通道对本 harness 结构性无信息」交总监。**局数 vs 预期**:`scored_games 168 / unfinished 0`,预登记预期 ~96 ⇒ **多交付 75%**(每粒 42 局而非 24)——⭐ **登记为读数不解释**,但它意味着 `expected_games` 这个预登记量在过去若干波一直偏低,**若哪一波真的少交付,拿它当分母读不出来**(交总监)。`suggested = hold_or_reject` ——⚠️ **这是 gpm-only 提示、门限 `>5`,而 mean gpm 是 +11.70** ⇒ 提示**不由效应量驱动**;促/退归总监,条件 (a) 录像确认**仍欠**。**并池**:W49 **自成一族 4 粒**,与发波前预登记逐字一致。
  **六、闸 (iv)(给 W50 的输入)**:`RB_EXIT=0` —— `4 paired seed(s) of 4 machine(s)`、`0 machine(s) reclaimed before the flip`、四粒全 `PAIRED [depth-checked]`、**`NEXT WAVE: spot`**。⚠️ **第一次调用因缺 `survival_min` 得 `exit 2` = UNDECIDABLE**;`exit 2` 是没跑成,**永远不是「没瞎」**。
  **七、⛔⛔ 泄漏:本台干净;外来机没有走,而且已经落账。** **dota2bot 侧干净**:`tag:Name=dota2bot-*` 在 `us-west-2` **全状态**查询 ⇒ **空**(四台已自毁且过了保留窗口),`check_costs.sh` 的 running/pending 表 ⇒ 空,常驻仍只有 AMI `ami-0a990a26d89c66547`。⛔ **不带过滤器、在正确区域**的查询:`i-0ce7ddf08ea24cc7f c7a.16xlarge running 2026-09-05T05:02:24Z spot`,`Name=fft-cache-v1-attempt3`/`Project=final-table-trainer`,spot request `sir-ra3qg27m` 仍 `active/fulfilled` ⇒ **同一台、仍在跑、已连跑约 19.2 小时**(上一轮 16.3h)。⭐ **上一轮只能说「已发生未落账约 $16.4–19.7」;本轮它落账了,是 `$7.995`** —— 两者不矛盾:CE 窗口本身也有滞后,按 `$1.008–1.210/h × 19.2h` ⇒ 已发生总额约 **`$19.4–23.2`**,**还有约 `$11–15` 在路上**。**算术(交 owner,本台不自裁)**:裸 headroom `$22.186` ÷ `$24–29/天` ⇒ **约 0.6–0.9 天见底**,且**在路上那 `$11–15` 落账时会一次性吃掉一半以上** ⇒ **(甲)** 届时闸 (iii) 对**本台**打 `THROTTLED`,而本台九月自己只花了 `$16.5`、发一波只要 `~$1.02`,**读起来像批测超支实际不是**;**(乙)** owner 第一封 `$50` ACTUAL 告警邮件**抬头是 `dota2bot-batch` 而它指着错的项目**。根因已量出(GH #515):`CostFilters = null` ⇒ 账号级不是项目级。**建议(不动本台任何工具)**:给 `dota2bot-batch` 加 cost filter(按 `Project` 标签或机型),或给 `final-table-trainer` 单开预算。⛔ **本台未做任何处置**(带别人的项目标签,终止既越权又不可逆),**不主张它不该跑**。
  **八、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(改动 = `iterations/` 下的报告与波次记录);铁律 11 未触发 `requires approval`,**无空转等待** —— 唯一的「等」是自检在后台跑,本轮没有为它停下。
  **交棒**:① ⭐⭐⭐ **owner/总监 —— GH #528 + #515 本轮从「前瞻」变成「账单上的一行」**(`$7.995` 已落账 + 约 `$11–15` 在路上 + headroom 0.6–0.9 天见底),**本台无法自解**;② ⭐⭐⭐ **总监 —— GH #544**:`launched_at` 漏写会让闸 (i) 下轮再次 UNCERTIFIABLE,最便宜的修法是让 `wave_throttle.py` 接受 `launch_time` 作别名(或加波次记录 schema 检查);③ ⭐⭐ 总监 —— **`$0.90` 常数第四轮连续点名**;④ ⭐⭐ 录像组 —— **61-id 家族条件 (a) 仍欠**(W49 四粒的 `.dem` 已在 S3);⑤ ⭐⭐ 总监 —— **W49 促/退**(gpm +11.70 / comps 3/4 / 四粒深度全 ≥17.95 / 零抢占 / 零排除种子;winrate 第八波 DEGENERATE 请按预登记作结构性结论收掉);⑥ ⭐ 总监 —— **`expected_games` 预登记量偏低约 75%**,现在不能当「少交付」的分母;⑦ ⭐ 总监 —— **GH #513** 仍 `open`(本轮串未动故未用上);⑧ ⭐ 本台 —— **`campgrade` 独占波仍不存在**(#480/#241)。
  **下一轮本台** = ① **发波 W50**(闸 (i) 解锁于 **`2026-09-06T03:20:05Z`**,但**不要读这个数,读 `wave_throttle.py` 打的那一行**);② ⚠️ **发波前先看第七节** —— headroom 可能已被外来机吃到 `THROTTLED`,**而那个 `THROTTLED` 不是本台超支**,若真被挡,把算术贴给 owner,**不要自行放行**;③ 闸 (iv) 已备好 `NEXT WAVE: spot`;④ 开工第一条命令**重定向不管道、放后台**(本轮**第八次**踩管道,已当场被脚本拒绝)。
  **收尾补记(自检真码)**:**`EXIT=3`**(重定向不管道、后台跑满),`legs run 10`,**`UNCERTIFIABLE (exit 2): none`**(十条腿都真跑过),`FINDINGS: queue-rulings owed-executions trunk-red(python)`。**3 = 有发现不是没跑成。** ⭐ **`trunk-red(lua)` 本轮消失**:上一轮是 `RED test_corpus_scale.lua`(本台上一轮开的 **GH #538**),本轮 **`84 tagged detector file(s), 0 failures`** ⇒ **#538 已被协同组修掉,上一轮那一棒落地了**。`trunk-red(python)` **`93 passed, 7 failed, 2 uncertifiable`**(2 条本台的已修绿,见四之二)。`queue-rulings`:**`RIDESHARE (§BB.4: rule this round): 2`** ——⚠️ 本台不裁,但**连续第二轮**指出 owner 的 **P4.2 已废止并压过 §BB.4**(冻结期唯一合法裁定是 `FROZEN-HOLD`)⇒ **工具还在引一条已被废止的条款**,交总监。`owed executions` 含本台仍欠的 `campgrade_wave_type_premise_reading`(#480/#241);另有 `p2_pair_arm_stayattr_staysrc`(GH #532 / P2)——**否决权不是待办**,本轮不排 P2 波故登记不动作。`cadence` 本轮**无 finding**(上一轮有 `GAP cadence director`)。`unlanded commits` / `expired waits` / `stable-v1/v2` / 三条 promote atom **全 ok**;边界 `REFUSED refs 802 of 827`(浅克隆),`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。**铁律 6 读数**见报告第十节收尾行;**动态半(#124)未跑不声称**。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),推送门 `ARM_HOOK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态半(#124)**未跑不声称**。`PUSH_BRANCH_EXIT=0` / `PUSH_MAIN_EXIT=0`(`1b453201..4bccd559 HEAD -> main`,未被拒无需 rebase;回填编号那次 `4bccd559..4c0df743`)。**收尾泄漏复检**:`LEAK_EXIT=0`,us-west-2 不带过滤器唯一 running 的是外来机 `i-0ce7ddf08ea24cc7f`,**dota2bot 侧零实例**,常驻仍只有 AMI;本轮未发波故无「恰好在跑本波那四台」这一半。**发表纪律(GH #290)**:两处发表都在 push 之后,GH #528 升级评论发表前 `PRECHECK_EXIT=0`(`local commits not on origin/main: 0`,`OK to publish`);新开 issue 实际编号是 **#544**(初稿的 #545 是占位,已全文回填)。
  **本轮 token**:`TOKENS total_in=8,172,747 out=51,300 turns=70`(到统计时刻为止)。
  详见 `iterations/reports/batch-desk/20260906T001100Z.md` 与 `iterations/reports/batch-desk/waves/W49_wave.json`。
- **2026-09-06T03:11–03:4xZ(发波轮:W50 = 59-id 家族第一波。⭐ 本轮真正的产出不是那四台机器,是 `pudge` 载体那个数换掉了一种解释 —— 上一轮把 49/400 记成「削最富那层」的侵蚀,而本轮在一个**从未被碰过**的新窗口里拿到**同一个 49**,侵蚀按构造解释不了它)**
  **一、自检(铁律 10)**:⚠️ **开工第一条命令又误用管道,脚本自拒**(`REFUSED: routine_selfcheck.sh stdout is a pipe; exit 2, nothing checked`)——**同形第二十六次,又是当轮第一条命令**,那个 `EXIT=2` 是 REFUSED **不是通过**;改重定向 + 后台后跑完(上一轮交棒照办)。**真码读脚本自打的那一行**:**`selfcheck worst exit: 3`**,`legs run 10`,**`FINDINGS (exit 3): cadence queue-rulings owed-executions`**,**`UNCERTIFIABLE (exit 2): trunk-red(python)`**,`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。⭐ **python 语料 `100 passed, 0 failed, 2 uncertifiable`** —— **对比上一轮的 7 red,python 侧的红本轮全部不在了**,而**归因不是本台**:总监 01:19Z `a2caace8`「main 五条红全部修绿」。⚠️ **但不许把它读成「python 腿通过了」**:腿自己的横幅是 `UNCERTIFIABLE -- a python test did NOT run`,**那一行既不是通过也不是「trunk 是红的」的证据**(它自己写着这句)。本台上一轮自己修绿的两条独立复跑仍绿(`WT_EXIT=0` 55 checks / `WGK_EXIT=0` 263 checks)。Lua 快检测器腿**本轮跑完**:`84 tagged detector file(s), 0 failures`(横幅自写 **FAST SUBSET**)。两条 `UNCERTIFIABLE` 的理由行是 `no lua5.1 on PATH`,而 `which lua5.1` 在稍后答 `/usr/bin/lua5.1` ⇒ **是时序产物**(自检在更后面的腿里才装解释器),与 GH #171 同族,登记不自改 harness。
  **二、成本与闸 (iii)**:`AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`,九月 MTD **`$27.814`**(免费 `budgets`,`refreshed 2026-09-05T22:33:08Z`),**与上一轮逐位相同** ⇒ ⛔ **本轮不再花那 `$0.01` 复核**(上一轮已用 CE 拆开:增量全部来自账号上别人的 `c7a.16xlarge`,本台 `c6i.4xlarge 16.5068` 逐位没动;且 `$27.814 < $35` 的 `COST_CONFIRM_AT` 门)。闸 (iii) 按 GH #504 走工具不抄常数,两种市场类型各过一次:spot(`--planned 1.02`)**`FENCE_SPOT_EXIT=0`** projected `$28.834` headroom **`$21.166`**;全程降级按需(`--planned 2.15`)**`FENCE_OD_EXIT=0`** projected `$29.964` headroom `$20.036`。工具**现读**:limit `$100.00`、actual `$27.814`、三档 ACTUAL 告警 `$50`·`$80`·`$100` **全部 NOT yet crossed(AWS state=OK)**、fence `$50.00`、brake `$90.00`、operative ceiling `$50.00`。**本轮未跨任何告警档 ⇒ 不产生 owner 可见的 Budget 告警邮件。** ⚠️ **`$0.90` 常数的自动失效条款第五轮连续点名仍未重裁**,本波按上一轮先例登记 **`~$1.02`**(实测上界,失效方向朝安全侧)。
  **三、收割:无对象。** 发波前 `running,pending` 无本台实例,`soak/` 最新四个前缀就是 W49 那四台、上一轮已 4/4 粒收完(gpm `+11.70`,促/退归总监 GH #510)。⭐ **闸 (iv) 的「no harvest owed」那一半本轮在发波轮当轮写进 `gates` 键** —— W48/W49 **连续两波**都是收割轮补写的(GH #544),本轮按那条建议**在发波轮就跑了 `test_wave_gate_keys.py`**。
  **四、闸 (ii):第二轮走 (一) 支,方向仍是出集(61 → 59)。** 臂串现读现算不抄散文:W49(树 `066219d6`)**61 / 540 / `824ec284…`** → W50(树 `b73358bc`)**59 / 524 / `572b6075…`**;差集**用 `diff` 算不用眼看** ⇒ **只减不加,减掉 `tpdying` 与 `tpreach`**,出处是总监 commit `7335dcd3 P4.2 two rulings closed (tpdying/tpreach returned, 61 -> 59)`。⇒ **串直接扛闸,GH #513 那段手算连续两轮不需要**(#513 仍 `open`;但那是运气不是机制)。**并池预登记(数据落地前写死)**:W50 **自成一族**,与 W49(61)/ W48(63)/ W46+W47(62)**四个家族两两不逐字节相同** ⇒ **谁也不并**。
  **五、⭐⭐⭐ 发波 W50,记录全文 `iterations/reports/batch-desk/waves/W50_wave.json`。** 树:`--ref` 钉 40 位 SHA **`b73358bc0f764fd4a22171da2ae4556a0d5d38f7`** 不是 `main`;`git ls-remote origin main` **读两次**(闸时点 + 发波脚本内第一条命令,不等就 exit 9 什么都不发),两次都 `b73358bc…` 且等于本地 HEAD。⭐⭐ **闸 (i) 本轮不是形式通过的,是真的挡过三次**:`wave_throttle.py` 跑四次 —— `03:12:58Z` **THROTTLED(exit 3)`margin 427s SHORT`**、`03:15:56Z`(249s)、`03:16:09Z`(236s)**发波脚本两次都拒绝发、一台没起**,直到**动作时点、循环前最后一条、脚本内裸 `$?`**:`03:20:19Z` **`UNLOCKED (exit 0)`,`margin +14s past unlock`**,`THROTTLE_EXIT_AT_ACTION=0`(锚点 W49 末台 `09-05T21:20:05Z`,解锁 `09-06T03:20:05Z`)。**那 14 秒值得单独登记**:章程记过的三次突破(W12 21s、W20 3m31s、W44 3m00–4m19s)是**同一个机制** —— 钟只读一次、去做五分钟的门、回来直接发;本轮把钟读在**动作上**,于是它三次说不。两道内容闸:**`CT_EXIT=0`**(59 id:12 hero-scoped / 47 generic / **0 unresolved** ⇒ **8 项**,`TERMS` 行与 W48/W49 **逐字节相同** —— 退回的两个 id **对载体项零贡献**,**串动了而载体要求没动**,与 W48 `stayattr` 入集同形、方向相反);**`ARM_EXIT=0`**(**all 59 armed ids wired on HEAD**)。⚠️ **两个工具本轮各误用一次,都得 `exit 2`**:`carrier_terms.py` 吃 `--arm` 不是 `--cand`;`check_armed_wiring.py` 在 **`tools/batch_test/`** 不在 `soak/` 下(`No such file or directory`)。**`exit 2` = 没跑成,永远不是「没有载体项」/「没接线」。** 选种:`--build` **在选种之前**跑(`ROSTER_EXIT=0`,索引现 **221 粒**),窗口**右移**至 **`[5801,6200]`**(W49 在 `5401–5800`)**400 粒全未用**,popcount 分布 `{0:26,1:99,2:153,3:99,4:19,5:4}`;对 popcount≥3 的 **122 粒穷举**得 **142** 组解 ⇒ 选定 **`5813 5830 6072 6128`**,`CARRIER_GATE ids=12 seeds=4 exit=0`,逐项 ck3/cm2/lion3/od2/**pudge2**/wk2/sb2/zuus2。市场与放置:**spot**(不传 `--on-demand`,GH #158),`--slots 16 --hours 2 --rec-slots 8 --games 12` 4×1 **与 W39–W49 逐字节相同**(本波动的是 id 集);⭐ **GH #252 / #256 验收连续第二波干净 4/4** —— 四台四个互不相同的 AZ 且**每台都拿到它请求的那个**(`5813`/`i-01c5afea8732762b3`/`2a`/`03:20:21Z`、`5830`/`i-0c93e460fa74ae9be`/`2b`/`03:20:25Z`、`6072`/`i-04428cac86ab59e53`/`2c`/`03:20:29Z`、`6128`/`i-0201d8d5fad9ccb3b`/`2d`/`03:20:34Z`),**零 `re-aiming inside the ring` 行、零 `AZ RING EXHAUSTED` 行、零 `InsufficientInstanceCapacity`,无降级**;**四台 `InstanceLifecycle` 全为 `spot`**(run_id 的 `spot_` 前缀是硬写字符串**不是证据**);末台上机 `2026-09-06T03:20:34Z` = W51 的闸 (i) 锚点,slate spread **13s**;自毁 = self-terminate + **2h 看门狗** + one-time spot request;`--dry-run` 先过(`DRY_EXIT=0`,#108 的波次预算闸未拒发),四次 `SPOT_RUN_EXIT=0`。⭐ **`launched_at` 本轮在发波轮就写齐(GH #544)**,四行同时带 `launch_time` 与 `launched_at`(源 `describe-instances LaunchTime`,`launched_at_source` 写明);并**在写完记录之后**复跑两条检测器 —— **`WGK_EXIT=0`(274 checks, 0 failed)**、**`WT_EXIT=0`(55 checks, 0 failed)**,⭐ **这正是上一轮「检测器和它要看的产物顺序反了」那条的修法**。
  **五之二、⭐⭐⭐ 本轮头号产出:`pudge` 载体供给是结构性的,不是侵蚀。** 三个数据点:W48 窗口 `[5001,5400]` 供给 **70/400**、解 2506、实得 **4/4**;W49 `[5401,5800]` 供给 **49/400**、解 2456、实得 **3/4**;**W50 `[5801,6200]` 供给 49/400、解 142、实得 2/4**。**2/4 不是本台挑的,是 142 组解里的最大值**(穷举自己给的上界)。⭐ **上一轮把 49 登记成章程说的「每波削掉最富那层」的侵蚀 —— 本轮否掉了那个解释**:`[5801,6200]` 是**从未被任何波碰过**的新窗口,侵蚀在它身上按构造不成立,而供给**还是 49**,同窗口其余七项 65–158。⇒ 更省的读法:**`pudge` 本来就是八项里最稀缺的**(49/400 ≈ 12%,其余七项 **16–40%**:od 65、sb 69 紧随其后),**载体要求会一直先卡在它身上,换窗口无用**;⚠️ **边界**:主张的是「pudge 最先成为瓶颈」不是「只有 pudge 稀缺」,与 od/sb 只差 16–20 粒。**交总监 / 协同组:骑在 pudge 上的帧通道(`rotscope` 条件 (a),GH #472 / queue `strategy-26`)不能按「4 粒全有」排期**;本波它的域是 **2 粒** —— 收割时那是**域的大小**,**不许把更小的计数读成更小的效应**(已写进 `harvest_obligations`)。
  **六、局数(章程第 7 步)**:(a) **W49** 上轮已结清照登不重算 —— 四粒完整、零抢占(`5404 ab29/ba13`、`5502 ab28/ba14`、`5574 ab28/ba14`、`5705 ab27/ba15`),`scored 168 / unfinished 0`,`arm_depth 17.95–19.29`,`thin_arm_seeds` 空;(b) **W50 为发波型 ⇒ 写预期局数**:预期 **~96 局计分**(4 粒 × 12 局 × 2 腿)外加暖场局(**暖场局不算有效局**),发波时点 **0 局**,头 14 分钟 `OBJ_COUNT=0` 是预期值不是症状。⚠️ 上一轮已交总监:`expected_games` 这个预登记量**偏低约 75%**,**现在不能当「少交付」的分母**。
  **七、发波轮收尾波次点名(两条,零成本,当轮必做)**:`RC1_EXIT=0` / `RC2_EXIT=0`。① `pending,running` 里本台的 `soak-run` **恰好四个**且与 `machines[].run_id` **一一对上**(`295740`/`e56ba0`/`f2182d`/`eff51c`)⇒ **无第五台(无泄漏)、一台不少(无掉臂)**;② `terminated,shutting-down` **为空** ⇒ 本波窗口内**无** `Server.SpotInstanceTermination`(W33 那个形状未出现)。⚠️ **发波轮收尾要的不是「0 台在跑」而是「恰好在跑本波登记的那四台」**。**账号侧第五台不是本台的**:`i-0a7d4c7fbeb3a6712` **`r7a.16xlarge`** `us-west-2d` `02:10:36Z` 起,`Name=fft-cache-v1-repair`/`Project=final-table-trainer`,机型/标签对不上 ⇒ **不是本台的泄漏**;⛔ **未做任何处置**(带别人的项目标签,终止既越权又不可逆)。⚠️ **它是新的一台**(上一轮点名的是 `i-0ce7ddf08ea24cc7f` `c7a.16xlarge` `fft-cache-v1-attempt3`)⇒ **GH #515 / #528 不是一次性事件,是一条在续跑的作业**。
  **八、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只动 `iterations/` 下的报告、波次记录、本节)。**铁律 11** 未触发任何 `requires approval`;唯一一次「等」是**闸 (i) 自己的 4 分钟未解锁**,那是**门在拒绝**不是空转等待,且等待期间本台在做选种、写波次记录与报告。
  **交棒**:① ⭐⭐⭐ **owner/总监 —— GH #515/#528 不是一次性事件**(账号上换了一台新的 16xlarge 在跑;headroom `$21.166` 是共用的,约一天内会被吃光,那时的 `THROTTLED` **不是本台超支**),本台无法自解;② ⭐⭐⭐ **总监 —— `$0.90` 常数第五轮连续点名仍未重裁**(条款原文写着「这个常数不许靠人记得去复查」,而它现在正靠这个);③ ⭐⭐ **总监/协同组 —— `pudge` 载体是结构性稀缺**(见五之二),**帧通道不能按 4 粒排期**;④ ⭐⭐ **录像组 —— 59-id 家族条件 (a) 从本波起才买得到;61-id 家族(W49)条件 (a) 仍欠**(`.dem` 已在 S3);⚠️ `tpdying`/`tpreach` 已出集,**W50 对它们一个字都没说**;⑤ ⭐ 总监 —— **GH #513** 仍 `open`(连续两轮 (一) 支自己扛住,是运气不是机制);⑥ ⭐ 总监 —— **GH #544 的另一半**(`wave_throttle.py` 接受 `launch_time` 作别名)仍未落地,本轮的不复发靠的是「本台记得做」;⑦ ⭐ 本台 —— **`campgrade` 独占波仍不存在**(#480/#241,`owed_executions` 本轮仍报它)。
  **下一轮本台** = ① **收割 W50**(`recover_verdict.py` 从 S3 逐局全量重算,走 `soak/` 不走 `validation/`;填 `machines[]` 的 `ab`/`ba`/`arm_depth`/`status_code`,**不许留 null**;**收割义务全文在 `W50_wave.json:harvest_obligations`,不要只读摘要**);② 闸 (i) 锚点是 W50 末台,**但不要读这个数,读 `wave_throttle.py` 打的那一行**(本轮已复核工具确实改锚到 W50:`unlock 2026-09-06T09:20:34Z`);③ 开工第一条命令**重定向不管道、放后台给足 ≥45min**(本轮第八次踩同一个坑)。
  **本轮开出的 issue(#290 顺序:先 push `b73bd068` 再发;`PRECHECK_EXIT=0`,`local commits not on origin/main: 0`,`paths cited 5 / resolved on trunk 4 / refused 0`)**:**GH #552** `[batch]`(pudge 载体稀缺是结构性的,帧通道不能按 4 粒/波排期);**GH #454 追评**(`$0.90` 常数第五轮点名,**不新开** —— #454 已 `open` 且正是这条)。⚠️ **第一版草稿 `PRECHECK_EXIT=0` 却 `paths cited 0`**,工具自打 `REFUSE zero citations extracted ... 'nothing bad' and 'nothing matched' are the same output`;把 `W50_wave.json:…` 这类**裸文件名**改成**仓库相对全路径**后才解析出 5 条 ⇒ **登记:引用要写全路径,否则审计器看不见它们。**
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck:`installed luacheck (it was missing; apt package lua-check, bounded)`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门 `ARM_HOOK_EXIT=0`;**`PUSH1_EXIT=0`**(`claude/great-sagan-7rmq3e`,新分支)/ **`PUSH2_EXIT=0`**(`b73358bc..b73bd068  HEAD -> main`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。
  **收尾泄漏复检**:`LEAK_EXIT=0` —— `running/pending` = **恰好本波那四台**(全 `c6i.4xlarge` `dota2bot-soak-spot-1`),**没有第五台本台实例**;常驻仍只有 AMI `ami-0a990a26d89c66547`。账号侧那台 `r7a.16xlarge` 见第七节。
  **本轮 token**:`TOKENS total_in=14,148,870 out=61,921 turns=92`(到统计时刻为止)。
  详见 `iterations/reports/batch-desk/20260906T031100Z.md` 与 `iterations/reports/batch-desk/waves/W50_wave.json`。

- **2026-09-07T00:14–00:4xZ(发波轮:W52 = 52-id 成员串家族第一波,spot 四台,⭐ 四道闸**全部在起飞前**跑完。⭐⭐⭐ 头号产出不是那四台机器,是起飞前那道闸的第一声:`reclaim_blind.py` 对 W51 打了 `UNDECIDABLE (exit 2)` —— 而 W51 那一轮什么都没做错,门是被一个**键名**打败的)**
  **一、自检(铁律 10)**:⚠️ **第一条命令连续第十轮踩同一个坑**(`| tail -40` ⇒ 工具自拒 `REFUSED: stdout is a pipe; exit 2, nothing checked`,harness 报的 `SELFCHECK_EXIT=0` 是 `tail` 的);改重定向重跑第一次 `timeout 400` ⇒ **`EXIT=124`**(上一轮交棒 ③ 点名的坑),第三次放后台 `timeout 900` **仍是 `SELFCHECK_EXIT=124`** —— 但这次死在**最后一条腿**(Lua 检测器)上而不是 python 腿 ⇒ ⭐ **可登记的读数:整份自检在本容器上现在需要 >900s**(上一轮 400s 死在 python 腿,本轮 900s 走完 python 又死在 Lua 腿),而这两条腿名字里都写着 fast。**下一轮直接给 ≥45min,不要再试探。**⭐ **本轮把这条第十次的复发写成交棒交给总监**(要独有退出码或 wrapper,十轮靠人记得不是机制)。逐腿(照抄工具自己的行):推送门**已上膛**;未落地工作 `OK: no unlanded work in the certifiable window`(26 可核验 / 848 uncertifiable,shallow);报告节奏 **2 findings**(协同组两个 3.6h 洞);未裁 queue 行 RIDESHARE/OTHER **均 none**(77 open);孤儿提案 **none**;owed registry **10 行**(3 DONE 待退休 / 7 OWED);过期等待 **无**;稳定版锚点 **4/4 OK**(**`stable-v4` = `odbuild`+`illumove`,`0d3f857fb683`,自检里首见**);promote 约束 **`161 live gate id(s)` / 16 PROMOTED / `FROZEN none`**,**4 个 co-promote 原子全 ok**;⏳ **trunk Lua 腿本轮被 900s 超时砍死、一次都没跑,不声称干净也不声称红**(上一轮那条 `test_gated_helper_nesting_census.lua` RED **本轮未复核**)。
  **二、⛔⛔ trunk python RED,而红的是登记表的形状不是数据。** `tests/test_stable_anchors.py` `SA_EXIT=1`,唯一 FAIL 逐字:`stable-v4: state_json_key odbuild_PROMOTE_20260906 + illumove_PROMOTE_20260906 not in state.json`。**这句话字面为真而它想说的事是假的**:两个键**都在** `state.json` 里(逐个 grep 确认)。红出在 `iterations/stable_anchors.json:102` 把两个键名塞进一个 `state_json_key` 字符串,而 `test_stable_anchors.py:109-110` 做的是 `a["state_json_key"] in state` —— **一次字面单键查表**,那个 ` + ` 串按构造永远查不到。⭐ **立案的不是这次红是它的形状**:`stable-v4` 是本项目**第一个双 id 锚点**,写的人与读的人**各自都对**,红出在两者之间那个从没写下来的约定上 ⇒ **下一次任何双 id promote 必然复发**。⚠️ 诚实边界:**未跑 `git stash` 复核**,「main 也红」是推断(依据是本轮改的文件与该断言无关),自检自己那行也这么说。另有 **9 条 `UNCERTIFIABLE`**(`test_selfcheck_lua_leg.py` 120s 预算腿,本容器 84 文件 120.1s)—— **没跑成不是通过**。
  **三、成本:MTD 逐位没动,而这正是本轮 `--pending` 的推导依据。** `AWS_SETUP_EXIT=0`;`COST_EXIT=0`;MTD **`$50.804`**(免费 `budgets`,**`refreshed 2026-09-06T15:19:34Z`**),≥$35 ⇒ 自动 `$0.01` CE 复核 **`50.8036495163`**,两源一致。⭐ **本轮读数与 18:13Z / 21:13Z 两轮逐位相同,连 `refreshed` 戳都相同** ⇒ 自 15:19:34Z 起 **~9.1h 零入账** ⇒ (甲) 15:20Z 量到的 `$10.926` 外来 `r7a` 燃烧**仍未入账**、(乙) W51 自己结算的 `$0.9774` 同样在 MTD 之外 ⇒ `pending = 10.926 + 0.977 = $11.91`,**重估的不是抄的**;⚠️ 是**下界**且出处不可复算(外来 SIR 已老化,GH #565);⛔ 未用被禁的「MTD 增量对得上波费」手法。闸 (iii):`wave_fence.py --planned 1.10 --pending 11.91` **`FENCE_EXIT=0 CLEAR`** —— `actual $50.804` / `pending $11.910` / `planned $1.100` ⇒ `projected $63.814`;**`$50.00 crossed (AWS state=ALARM)`**、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、operative ceiling **`$80.00`**、**headroom `$16.186`**。Ruling 4 accrual 行逐字:`accrual check    : CERTIFIED ZERO (0 accruing instances account-wide), but --pending $11.910 was given and is used as the larger claim.`(**读数不是默认值**;**未用 `--no-accrual-check` ⇒ 无「SKIPPED, NOT CERTIFIED」行**)。**本轮未新跨任何告警档**,故不写「本轮跨过 $X」那一行。
  **四、收割:无新数据(查过的,不是默认的)。** `soak/` 最新四前缀就是 W51 那四台,**21:13Z 已 4/4 收完**;`validation/` 最新仍是 **07-23** 历史物 —— 这同时是闸 (iv) 第一半的读数。
  **五、⭐⭐⭐ 闸 (iv) 起飞前跑,于是它出了声。** 第一次 **`RECLAIM_BLIND_EXIT=2`** 逐字 `UNDECIDABLE: seed 6421: needs survival_min, or both create and update`(**exit 2 = 什么都没检查**)。诊断:工具只认 `survival_min` 或 `create`+**`update`**;而 W51 收割轮**确实**在 SIR 窗口内第一手读下四台终止时刻、**确实**写进了记录、字段的 `_source` 甚至**用文字写明**它就是 `Status.UpdateTime` —— **它写在 `terminated_at` 上,工具读 `update`**。⇒ 值**逐字**复制进 `update`(原键保留 + provenance 注),**在取 pin 之前**推上 main(静态门 `GATE_EXIT=0 CLEAN`)。第二次 **`RECLAIM_BLIND_EXIT=0`**:`4 paired seed(s) of 4 machine(s)` / `0 machine(s) reclaimed before the flip` / `not blinded` / 收尾行 **`NEXT WAVE: spot`**。⭐ **要记的不是这次修好了,是这次为什么能被看见** —— 上一轮的 (iv) 是**起飞后**被测试打红才补跑的(那一轮自己写成交棒 ④),本轮照办把它排进发波脚本、**排在 (i) 之前**,于是 exit 2 **在花钱之前**到达。**它与 `pullcad`、`--cand`/`--arm` 两个 flag 陷阱、apt 的 `lua-check`、以及本轮第二节的 `stable-v4` 复合键同族:门在、接线在、数据在,被一个名字结构性绕开,而失效时打的是很容易被读成「没事」的 exit 2。**
  **六、W52 发波**:树 **`7a49dc7154262c907440b33deb3b176f7a453371`**(`--ref` 40 位 SHA;`git ls-remote origin main` **读两次**,发波脚本第一条命令**第三次**读并对 pin 做 `exit 9` 守卫,三次全等);armed **52 id / 467 bytes / md5 `445ea52100428d4e6c9aab31f3556245`**(现算,脚本内**第二次**重算)⇒ **新家族**(与 W50 `59/524/572b6075`、W49 `61/540/824ec284`、W48 `63/559/4aefc887`、W46+W47 `62/550/c7e1f92c` **逐字皆不同**,累计种子 **0 < 8**);相对 W50 **−7 id 零新增**(`illumove`/`midtp`/`odbuild`/`roshdist`/`slotwait`/`suptp`/`towerfear`,`diff` 出来的不是眼看的;**其中三个是 promote 出集不是被拒** —— `slotwait`(v3)、`odbuild`+`illumove`(v4);`towerfear` 由 owner P4.2 冻结按住,GH #558);载体项 `CT_EXIT=0` **11 hero / 41 generic / 0 unresolved ⇒ 8 项**,TERMS 行与 W48/W49/W50 **逐字相同**(少 7 个 id、少 1 个 hero-scoped,**载体要求一项没动** —— `odbuild` 的 `obsidian_destroyer` 仍由 `odaoe` 载着);载体门 `CARRIER_GATE ids=11 seeds=4 exit=0`,逐项 `ck3/cm3/lion2/od3/pudge2/wk2/sb2/zuus3` **每项 ≥2**;接线门 **`ARM_EXIT=0` 52/52 wired**(⚠️ 首次调用误传 `--arm`(它要 `--cand`)得 exit 2 —— **exit 2 是「没跑成」永远不是「没接线」**,与 W50 记的 PATH 陷阱、W50 记的 `carrier_terms.py` flag 陷阱**三轮三种方式**同一个工具打 exit 2);种子 **6743/6858/6877/6907**(窗口 `[6701,7100]` 全新;`seed_roster_index.py --build` `ROSTER_EXIT=0`,索引 229 粒;**另按手工排除上一轮起飞 70 秒被杀那波烧掉的 6211/6277/6343/6409** —— 它们没上传过任何东西,**索引按构造排不掉它们**);4 台 × 1 粒 `c6i.4xlarge`,**`--slots 16 --rec-slots 8 --hours 2 --games 12`**(与 W39–W51 逐字相同,`--rec-slots 8` **显式传**);**spot**,`InstanceLifecycle=spot` 逐台核实,**四个 AZ 全部拿到所请求的那个**,零 re-aiming / 零 `RING EXHAUSTED` / 零容量错误;实例 `i-0d3c9cf26be9c9ac3`(2a)/`i-04e6ba47de2c8293a`(2b)/`i-0bbc48e26b83a8a4b`(2c)/`i-010d948664a594efa`(2d);SIR `vw9fg23p`/`m6xfjain`/`3287jw4q`/`6iszkj3m` **四条全 `active/fulfilled`,发波当轮第一手读下**(GH #556);末台 **`00:21:45Z`**,slate spread **7s**;预估 **`~$1.10`**(GH #454 重裁常数,**不是**作废的 `$0.90`,**也不是** W46 上确界 `$1.022`)。⭐ **顺带结清两条早该上车的入集**:`campbind`(`strategy-42`/GH #475)与 `zusboltdom`(`hero-29`/GH #477),裁定都写「W46 或其后第一波」——**本轮 grep 逐个确认在串里,不是假设**。闸 (i) 两读:决策 `00:14:54Z` **`THROTTLE_EXIT=3` THROTTLED `margin 288s SHORT`**;**动作时点**(脚本内倒数第一条,裸 `$?`)`00:21:36Z` **`THROTTLE_EXIT_AT_ACTION=0` UNLOCKED `+114s`**。发波后复跑锚点**已自动变 W52**(末台 `00:21:45Z` ⇒ 下次解锁 `06:21:45Z`,**结构性的,工具只读 `machines[]`**);`test_wave_gate_keys.py` **296/0**、`test_wave_throttle.py` **55/0**,均在写完 `W52_wave.json` 之后跑。
  **七、局数(章程第 7 步)**:发波型 ⇒ 写**预期** `expected_games ~96`(4 粒 × 12 局 × 2 腿)。⚠️ 该预登记量**系统性偏低约 75%**(W49 实得 168、W51 实得 214),**两个方向都不许拿它当分母**。
  **八、泄漏**:`LEAK_EXIT=0`,全账号(**不按 tag 过滤**)`running,pending` = **恰好本波那四台** ⇒ **无第五台、一台不少**;常驻仍只有 AMI `ami-0a990a26d89c66547`。⭐ **外来负载仍停着**(`r7a`/`c7a` 本轮 region 内 0 条)。⛔ **GH #515/#528 立案理由一字未变**:工具自己第一行 `budget filters   : none  <- so every instance in the account does land in this budget, whoever owns it`,本台围栏**仍读着一个它不控制的账号级数字**。
  **九、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只动 `iterations/` 下的报告、`W51_wave.json`(加 `update`)、`W52_wave.json`、本节)。静态门 **`GATE_EXIT=0 CLEAN`**,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;动态那半(GH #124)**不在钩子里,本轮也不声称跑过**。**铁律 11** 未触发任何 `requires approval`;唯一的「等」是闸 (i) 自己那 288 秒,**那是门在拒绝**,且那段时间全部用于跑闸 (ii)/(iii)/(iv)、载体门、接线门与选种,**没有一秒是空转**。
  **交棒**:① ⭐⭐⭐ **总监 —— trunk 红在登记表的形状不是数据**(二):`stable-v4` 是第一个双 id 锚点,`state_json_key` 只装得下一个键,校验器只会字面单键查表 ⇒ **下一次任何双 id promote 必然复发**;两个键都真在 `state.json` 里。② ⭐⭐⭐ **总监 —— 闸 (iv) 被一个键名打败了一整轮**(五):修复已落地,但**要修的是约定不是这一份记录**(合并 `terminated_at`/`update`,或让 `reclaim_blind.py` 也认前者);`W52_wave.json` 已把 `update: null` **预置**在 `machines[]` 里并附注,让同型缺陷下轮不能静默复发。③ ⭐⭐ **总监 —— 退休两行 owed-executions**:`wave_fence_ruling4_first_live_run`(18:13Z + 本轮,活路径两次实跑)、`campgrade_wave_type_premise_reading`(其 trigger 原文「此刻还不存在」的那一波已经发过,119 `.dem` 在手,现在挂着的是录像组的读数不是波)。④ ⭐⭐ **录像组 —— `campgrade` 条件 (a) 欠到第八轮,语料仍在手上**(判读规则全文 `W51_wave.json:harvest_obligations[0]`,本轮一字未改:尺子已存在不要另发明;**域为零 ⇒ `DOMAIN-NOT-REACHED`,不许读成「测过了」**;归类取不到 ⇒ 回总监重裁)。⑤ ⭐⭐ **总监 —— 开工第一条命令的管道坑第十轮**,请给机制(独有退出码或 wrapper);它每轮吃掉的是**自检本身**。⑥ ⭐ **总监 —— `$1.10` 下一次会死在价格不是机时上**(W51:机时 `0.8610` 稳在 `1.00` 下,而现货报价 `$0.2521–$0.2972` 已出 `$0.231–$0.250` 带 **9–19%**,乙1 余量 **~13%**,**两条腿都不直接看价格**);本波四 AZ 同时拿到容量,报价将在收割轮逐台读下 = 第三个测量点。⑦ ⭐ **owner —— `$50` 仍越线**(`$50.804`,`AWS state=ALARM`),诚实下界仍 **≥`$61.73`**、**约 61% 不是批测的**;外来负载已停 ⇒ `forecast $185.828` 不再成立,但**已花的不退**,且预算过滤器仍是 `null`。
  **下一轮本台** = ① **收割 W52**(`recover_verdict.py` 从 S3 逐局全量重算,走 `soak/` 不走 `validation/`;**先按 run 分目录下载再带前缀合并**,GH #225;`machines[]` 的 `ab`/`ba`/`arm_depth`/`status_code` **不许留 null**);② ⚠️ **收割轮第一件事(有时限)**:读四条 SIR 的 `Status.Code` + `Status.UpdateTime`(窗口 ≲1–3h),**把 UpdateTime 同时写进 `update` 和 `terminated_at`** —— `W52_wave.json` 的 `update_note` 已把这句钉在每台机器上;③ **当场结清 `harvest_obligations` 四条**(全文在 `W52_wave.json`,不要只读摘要);④ **读数照铁律 4(i-a)** 取工具自己的 `strata.<m>` 与 `per_seed[].<m>_ab/_ba`,**不手算不按局加权**,`winrate` 先看 `winrate_channel` 与 `mean.winrate_headroom`;⑤ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`**(本轮第十次)。
  **本轮开出的 issue(#290 顺序:三条产物先 push 上 main `60c56069`,之后才发表;两份草稿各跑一次 `claim_precheck.sh`,均 `PRECHECK_EXIT=0` `clean`、`local commits not on origin/main: 0`)**:**GH #579 `[bug]`**(trunk 红在登记表形状不是数据:`stable_anchors.json` 单键 `state_json_key` 装不下 `stable-v4` 的双 id promote,两个键都真在 `state.json` 里;`paths cited 8 / resolved 5 / refused 0`);**GH #580 `[harness]`**(闸 (iv) 被键名打败:`reclaim_blind.py` 读 `update`、收割轮写 `terminated_at`,要的是把约定变成机制而不是这一份记录的绷带;`paths cited 5 / resolved 4 / refused 0`)。
  详见 `iterations/reports/batch-desk/20260907T001400Z.md` 与 `iterations/reports/batch-desk/waves/W52_wave.json`。
  **本轮 token**:`TOKENS total_in=10,652,555 out=52,662 turns=77`(到统计时刻为止)。

- **2026-09-07T03:05–03:3xZ(收割轮:W52 收割 —— ⭐⭐⭐ 头号产出是一个五量 0/4 的负读数,以及「这个负号不止一种机制」这半句)**
  **一、自检(铁律 10)**:⚠️ **第一条命令第十一轮踩同一个坑**(`| tail -40` ⇒ 工具自拒 `REFUSED: stdout is a pipe; exit 2, nothing checked`);改重定向 + 后台 + **不设短 `timeout`** 后跑到 python 腿与锚点腿出结果,**Lua 腿写报告时仍未跑完 ⇒ 既不声称干净也不声称红**(与上一轮一致:整份自检在本容器 >900s)。逐腿照抄:稳定版锚点 **5 个全 OK**(⭐ **`stable-v5` = `ckpush`,`3fe17f9dd08b`,本轮首见**);promote 约束 `161 live gate id(s)` / **17** PROMOTED / **`FROZEN none`**,4 个 co-promote 原子全 ok;**2 条 `UNCERTIFIABLE`**(`rc.sh` 两格,逐字 `no lua5.1 on PATH`)—— **没跑成不是通过**。
  **二、⛔ trunk python RED,而红的又是登记表的形状不是数据(本周第三例)。** `107 passed, 1 failed, 2 uncertifiable`,唯一 FAIL `tests/test_carrier_hero_guard.py`,逐字 `FAIL pre-existing term chaos_knight survives the change`。诊断:该断言(`:112-114`)读 `iterations/streams/test_set.md` 第 2 行的**活体 arm 串**再硬编码要求 7 个载体词项各自仍在;本轮 `ckpush` promote 成 stable-v5(`armed 52 -> 51`,现算确认 `ckpush in ids? False`),而它正是 `chaos_knight` 词项的来源 ⇒ **断言按构造转红**。⭐ **立案的是形状**:测试的立案句是「GH #473 这次改动**不丢词项**」不是「`chaos_knight` 永远是词项」⇒ **任何一次 promote 都能打红一个与 promote 无关的地方**,与 `pullcad`、GH #579(`stable-v4` 双 id 塞单键)**同族**。⚠️ 诚实边界:**本会话 `bots/`/`game/`/`tests/` 一行未改**,「main 也红」是**推断未跑 `git stash` 复核**;上一轮那条 `test_stable_anchors.py`(#579)**本轮 PASS**。
  **三、成本**:`AWS_SETUP_EXIT=0`;`COST_EXIT=0`;MTD **`$62.042`**(免费 `budgets`,`refreshed 2026-09-07T02:04:33Z`),≥$35 ⇒ 自动 `$0.01` CE 复核 **`62.0418196925`**,两源一致;`forecast $205.267`。⚠️ 比上一轮 `$50.804` 高 **`$11.238`**,而上一轮申报 pending 是 `$11.910` —— ⛔ **本台不把这个吻合当对账**(章程步骤 2 (乙) 明令禁用该手法);能说的只有「上一轮的 pending 大部分已入账,是哪几笔本账号分不出来」。围栏:`wave_fence.py --planned 1.10 --pending 0.99` **`FENCE_EXIT=0 CLEAR`** ⇒ `projected $64.132`;**`$50.00 crossed (AWS state=ALARM)`**、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、ceiling **`$80.00`**、**headroom `$15.868`**;accrual 行 `CERTIFIED ZERO (0 accruing instances account-wide), but --pending $0.990 was given and is used as the larger claim.`(**未用 `--no-accrual-check`**)。**本轮未新跨任何告警档。**
  **四、⭐ 有时限的那件事排第一位**:`SIR_EXIT=0`,四条 SIR **全部还在**(终止后 ~1.8h 读到),**四台全 `instance-terminated-by-user` ⇒ 完工自毁、零抢占**:6743 `01:16:27Z`/0.9136h、6858 `01:16:22Z`/0.9117h、6877 `01:07:21Z`/0.7608h、6907 `01:16:45Z`/0.9167h。⭐ `UpdateTime` **同时写进 `update`(`reclaim_blind.py` 读的键)与 `terminated_at`** ⇒ **GH #580 那个缺陷本轮没有静默复发,因为记录自己把要求写在每台机器上**。⚠️ SIR 的 `SpotPrice` 四台全读 `0.680000` —— **那是出价上限不是成交价**,不许拿它算钱。
  **五、⭐⭐⭐ W52 收割(本轮主体)**:`RV_EXIT=0` 无拒答;语料走 `soak/` 前缀,四 run **各自子目录**(GH #225 不压平)`212ea5`/`7e6283`/`af2874`/`0e6ccf` = **64/61/48/56** 文件;`files_seen 229 / games_loaded 229 / source_dirs 4 / unparseable 0 / pooled_basename_dirs_overridden 0`。逐机(`machines[]` **零 null**):`6743 ab42/ba16 depth23.17`、`6858 ab41/ba14 depth20.87`、`6877 ab28/ba14 depth18.67`、`6907 ab36/ba14 depth20.16`;`scored 203 / unfinished 2 / seeds_scored 4/4 / min_arm_depth 8 / thin_arm_seeds []`。**读数(铁律 4(i-a),取工具自己的 `strata.<m>` 与 `per_seed[].<m>_ab/_ba`,未手算未按局加权)**:gpm **−64.72**(ab +22.21 / ba −151.65 / side 86.93 / 反号 / side>arm 2/4)、xpm **−44.28**(+6.93 / −95.48)、deaths **+0.67**(−0.46 / +1.79)、last_hits **−4.03**(−1.50 / −6.57,**不反号**);**`comps_better` 五量全 `0/4`**;逐粒 gpm **−18.34 / −60.21 / −68.45 / −111.87 四粒同号**;`suggested=hold_or_reject`。**4(i-c)**:三量反号**不是否决理由**(恒等式 `|side|>|arm|`),⭐ **本波读数不靠反号承载,承载它的是 arm 四粒同号为负**。⛔ **winrate 通道 DEGENERATE(工具自拒)**:少数侧只赢 227 局里的 **5** 局(share 0.0220 < 0.20),界定在 `0.5 ± 0.044`,侧别普查 radiant 5 / dire 222 ⇒ **`0.47` 与 `0/4` 两个方向都不许当铁律 2(b) 证据**。
  **五之二、⭐⭐⭐ 本轮头号产出:负读数是真的,而「成员串变差了」不是它唯一支持的解释。** 背景(**不是对照**):W46 −20.26(2/4)、W47 −5.95(1/2)、W48 **+27.25**(3/4)、W49 +11.70(3/4)、W50 +13.76(2/4)、**W52 −64.72(0/4)** ⇒ W52 是最负的一次,也是**唯一一次五量全 0/4**。⚠️ **可证伪的另一半(算术)**:工具量的是「成员串腿 **减** 稳定版腿」,而 **W50→W52 之间稳定版自己动了三次** —— `slotwait`(v3 `0d0a7201b8f2`)、`odbuild`+`illumove`(v4 `0d3f857fb683`)**从「候选腿 armed」变成两条腿都默认开** ⇒ 同一个负号**至少两种机制**:(甲) 成员串变差;(乙) **参照腿变强**。**本台区分不了,也不判**(章程:本台不做判断分析)⇒ 交总监 + 协同组。⚠️ 另一处**不许读成结论**:`ckpush` 在 W52 串里 armed 而在 W52 **落地之后**才 promote(`git merge-base --is-ancestor` 确认 `7a49dc71` 是 `3fe17f9d` 的祖先)⇒ **W52 量到的是 gated 状态的它,读数有效**;但**家族级读数永远不是 per-id 读数**,它不是本波负号的解释。
  **六、GH #454 两条腿(无论是否触发都报)**:逐机按**各自 AZ** 的现货报价计价 + `机时 × $0.005/h` IPv4(**不用单一账号级费率**):2a 0.9136h×$0.2880、2b 0.9117h×$0.2728、2c 0.7608h×$0.2955、2d 0.9167h×$0.2534 ⇒ EC2 `$0.9689` + IPv4 `$0.0175` = **单波实测 `$0.9865`**。**(乙1)** `$0.9865 > $1.10`?**否**(余量 10.3%);**(乙2)** 自终机器机时均值 **`0.8757 h/台`** `> 1.00`?**否**(余量 12.4%),**四台全自终 ⇒ 没有任何一台被排除在均值外**。⭐ **第三个测量点,形状与上一轮交棒 ⑥ 的预测一致**:机时稳稳压住,而**四个 AZ 里三个报价越过 `$0.231–0.250` 价带**(2c `$0.2955` 高 **18%**),**成本没破线只因为机时短**;**两条腿都不直接看价格** —— 这是 (乙1) 的分辨率问题,**登记不改常数**。
  **七、发波:本轮不发,门自己说的。** `wave_throttle.py` **`THROTTLE_EXIT=3`**,逐字 `W52 last machine 2026-09-07T00:21:45Z -> unlock 2026-09-07T06:21:45Z <- ANCHOR`、`now 03:13:45Z`、`margin 11280s SHORT`、`A launch at this instant BREACHES gate (i) by 188m00s.` ⇒ **零发波、零支出**(除 `$0.01` CE 复核)。锚点**已自动是 W52**,**读的是工具打的那一行不是自己算的数**。
  **八、收割义务四条当场结清**:`sir_window_read_first` DONE(排在任何 S3 工作之前)、`gh454_constant_legs` DONE(两条腿都报,都不触发)、`recover_verdict_per_run_download` DONE(四子目录、`pooled_basename_dirs_overridden 0`、`229=229`)、`campbind_zusboltdom_admission_discharge` DONE(`queue.json` 的 `strategy-42`/`hero-29` 从 `admitted` 改 `harvested_pending_verification` 并附 W52 结果行;两 id **本轮 grep 逐字确认在串里**;⚠️ 两者**条件 (a) 仍欠录像组**,语料已在 S3)。
  **九、泄漏**:`LEAK_EXIT=0` —— 全账号(**不按 tag 过滤**)`running,pending` = **`[]` 空** ⇒ 四台全自毁、**无第五台**;常驻仍只有 AMI `ami-0a990a26d89c66547`。⭐ **外来 `r7a`/`c7a` 本轮 region 内 0 条**。⛔ **GH #515/#528 立案理由一字未变**:工具第一行仍是 `budget filters : none <- so every instance in the account does land in this budget, whoever owns it`。
  **十、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只动 `iterations/` 下的报告、`W52_wave.json`、`queue.json` 两行、本节)。**铁律 11** 未触发任何 `requires approval`;**本轮没有任何等待** —— 闸 (i) 的 188 分钟是**门直接拒绝发波**,本台按章程转成纯收割轮。
  **交棒**:① ⭐⭐⭐ **总监 + 协同组 —— W52 五量 0/4 为负,而参照腿在 W50→W52 之间动过三次** ⇒ 负号至少两种机制,本台区分不了;② ⭐⭐⭐ **总监 —— trunk 红在登记表形状不是数据,本周第三例**(`test_carrier_hero_guard.py` 把 7 个载体词项硬编码成「必须仍在活体串里」,`ckpush` 一 promote 它就红);③ ⭐⭐ **总监 —— `$1.10` 的第三个测量点已到**,机时余量 12% 而三个 AZ 报价越带(2c 高 18%),**两条腿都不直接看价格**;④ ⭐⭐ **录像组 —— 三笔条件 (a) 欠着,语料都在手**(`campgrade` 第九轮、61-id 家族(W49)、**新增 `campbind`/`zusboltdom`(W52)**);⑤ ⭐ 总监 —— **开工第一条命令的管道坑第十一轮**,仍靠人记得;⑥ ⭐ owner —— **`$50` 仍越线**(`$62.042`,`AWS state=ALARM`,`forecast $205.267`),外来负载已停但**已花的不退**,预算过滤器仍 `null`。
  **下一轮本台** = ① **发波轮**(闸 (i) `06:21:45Z` 后解锁,**读工具打的那一行不要自己算**),下一波是 **51-id 成员串**(`ckpush` 已出集);② 四道闸**全部起飞前跑完**,(iv) 排在 (i) 之前(`reclaim_blind.py` 的输入 `update` 键本轮已填齐,应直接 `exit 0`);③ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`**(第十一次);④ **载体门按 7 项跑**(`chaos_knight` 随 `ckpush` promote 出集 —— **那是 promote 的正确后果不是波的缺陷**),项数变化**要在报告里点名**。
  **本轮开出的 issue(#290 顺序:三件产物先 push 上 main `903ac4f6`,之后才发表;两份草稿各跑一次 `claim_precheck.sh`,均 `PRECHECK_EXIT=0` `clean`、`local commits not on origin/main: 0`)**:**GH #583 `[batch]`**(W52 五量 0/4 为负,而参照腿动过三次;`paths cited 2 / resolved 2 / refused 0`);**GH #584 `[bug]`**(trunk 红在登记表形状不是数据,`test_carrier_hero_guard.py` 硬编码载体词项被 `ckpush` 的 promote 证伪;`paths cited 7 / resolved 5 / refused 0`)。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck:`installed luacheck (it was missing; apt package lua-check, bounded)`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门 `ARM_HOOK_EXIT=0`;**`PUSH1_EXIT=0`**(`claude/great-sagan-frqxkw`,新分支)/ **`PUSH2_EXIT=0`**(`fe131d06..903ac4f6  HEAD -> main`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。
  **本轮 token**:`TOKENS total_in=6,621,622 out=43,928 turns=59`(到统计时刻为止)。
  **⭐ 追记(03:5xZ,后台自检跑完之后)**:`SELFCHECK_EXIT=3`,工具自己的 `exit sources` 节逐字 `FINDINGS (exit 3) : cadence owed-executions trunk-red(python) trunk-red(lua)` / `UNCERTIFIABLE (exit 2): none` / `legs run 10` ⇒ 第一节那句「Lua 腿不声称」**现在有读数了:trunk Lua 也红**。`RED test_corpus_scale.lua` ⇒ `FAIL: [detector] no test pins the live corpus size with an equality`,点名**协同组 01:20Z 刚落地**的 `tests/test_wkreinctr_untrained.lua:238` 的 `assert(C.fixtures == 109, ...)`;检测器自己写着立案句「**landing the next fixture turns them red without anything they measure having changed**」(GH #106/#127)并给出替代品(`tests/corpus_scale.lua` 的 `ratchet()`/`universal()`/`corpus()`)。⭐ **与第二节的 `chaos_knight`(GH #584)是同一缺陷族的两个实例,同一天** —— 一个写死载体词项被 promote 证伪,一个写死语料规模会被下一个 fixture 证伪;**区别只在于这一个有专职检测器在抓**。⚠️ **这条比第二节能说得更强**:`git status` 干净、`HEAD == origin/main`(`e94d6930`)、`git diff origin/main -- tests/ bots/ game/` **为空** ⇒ **两条红都在 main 上不是工作树伪影**(不需要 `git stash`,那三棵树本会话一行未改);Lua 那条来源提交 `e6de91a6`(协同组),**不是本会话**。⇒ 已开 **GH #585 `[bug]`** 并在 **GH #584** 追评点明同族(追评顺带把 #584 里那句「未跑 stash ⇒ 推断」升级成「两条红都在 main 上」)。⇒ **交棒补一条:总监/协同组 —— trunk 现在两条红,一条 python 一条 Lua,来源都不是本台。**
  详见 `iterations/reports/batch-desk/20260907T030500Z.md` 与 `iterations/reports/batch-desk/waves/W52_wave.json`。
...
- 2026-09-05T03:13–03:5xZ(**收割 W47 的两粒补跑,W47 第一次完整(4/4 粒)—— 而补完这个波把它头号指标的符号翻了过来(gpm −5.95 → +9.89)。但真正的产出是另一件:上一轮那个「硬窗口」按构造根本抓不到,而上一轮什么都没做错。本轮零发波、零新增支出**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令第 7 次误用管道**,脚本自拒并逐字点名;改重定向后 `timeout 300` **超时 `EXIT=124`**,截断在 `trunk health (python)` 那一腿 ⇒ **不是通过也不是 trunk 红**(GH #358 形状),**本轮不声称 trunk 干净**,上一轮三条红(#490 第三轮 / #497 第二轮 / `test_mutstand_restore_trap.py`)**没人看过、仍挂着**。跑出读数的腿:`unlanded work` **OK**(certifiable 30 / REFUSED 767 浅克隆);cadence **1 finding = `director` 09-04T19:10Z→23:06Z 3.9h,不是本台**;stable-v1/v2 `EXISTS/PINNED/SHIPPED` 全 ok;`promote-time constraints` **`FROZEN none`**(141 个活 gate,`pullcad` 陷阱未复发)+ `field_hold_needs_magnitude` 三 id 全 GATED。`owed-executions` 的 `campgrade_wave_type_premise_reading`(#480/#241)**本台仍欠着**。
  **二、⭐⭐⭐ 头号产出:那个硬窗口按构造抓不到,上一轮的缓解措施被逐字执行了、码还是丢了。** 上一轮把「~03:32Z 前必须读 EC2 `StateReason.Code`」写进 `W47_wave.json` 并写明「不靠谁记得」——本轮它**被读到、被排第一、AWS 是第一件实质动作**,**码仍然丢了**。实测 `03:13–03:14Z`:两个 id `DESC_EXIT=0` 但 `Reservations: []`(**是空表,不是 `InvalidInstanceID.NotFound` ⇒ 空表本身是证据**),region 内**总实例数 0**。⭐ **算术**:起飞 `00:32:48/53Z`,看门狗上限 2h ⇒ `02:32Z`,上一轮据此推 `~03:32Z`;**但末次 S3 上传实测 `01:14:14Z`/`01:15:45Z` ⇒ 干完活只用了 ~41–43 min,机器不跑到上限就自毁 ⇒ 真实截止 `~02:15–02:20Z`,比那条注记早约 1h15m**。而**上一轮结束 ~00:4xZ、本轮触发 03:13Z ⇒ 机器出生/干活/死亡/记录过期全在两次触发之间的空档里**。⇒ **这不是记性问题是算术问题**:~2h 触发节奏 + ~45min 完成即自毁的按需机,退出码**按构造读不到,任何排班都抓不到**(W45 两行同因,本轮是第二例、第一次钉住算术)。⭐ **耐久性缺口(新查明)**:S3 **无任何终态 sidecar**(4723 run 的对象类型 `analysis.json 48 / demclaim.json 24 / log.gz 48`,全是逐局产物)⇒ **码只活在会衰减的 API 里**。两行 `status_code` 如实填 **`UNREADABLE`**,连算术+实测命令+「上一轮没做错」一起写进 `status_code_outcome`,**不写成推测的码**。
  **三、收割**:臂串**本轮亲手从发波树 `15b0d9fd` 重算** ⇒ **62 id / 550 字节 / md5 `c7e1f92c739f8f8dc0ee0c9484288628`**,与 W46/W47 登记值逐字节相同。**两 run 分目录下载再带 `<run_id>__<tag>` 前缀合并**(防 2026-08-19 那次静默丢 15.9% 局的同秒 basename 撞车),`RV_EXIT=0`(重定向读 `$?`)。**三个 override 一个都没用 ⇒ 无「这是 SKIP 不是 pass」的行要抄。** 补跑两粒 96 局 / 0 unparseable:`4723` ab28/ba15 **depth 19.53** gpm +5.79、`4763` ab30/ba12 **depth 17.14** gpm +45.68 ⇒ **两粒都过 GH #269 深度门,`thin_arm_seeds` 空,W47 被抢占的两粒全部收回**。
  **四、⭐⭐ 补完一个波把它的符号翻了过来。** **构造先说清**:W47 四粒 = 干净原机 4678/4950 + **补跑** 4723/4763,**同树同臂串**;被抢占的原始残局**保持作废不并进来**(章程事先登记的回收处置)⇒ ⛔ **这是波内补完,不是 GH #363/#449 的跨波并池分叉**。`RVFULL_EXIT=0`,206 局 loaded / **183 计分** / 0 unparseable,四粒 depth **18.67/19.53/17.14/27.11**,`thin_arm_seeds` 空。四量(每粒 swap-average 再跨粒算术平均,**永不按局加权**):**半波 n=2 `gpm −5.95`/xpm −8.68/deaths +0.25/lh −2.15 ⇒ 完整波 n=4 `gpm +9.89`/xpm −4.63/deaths +0.24/lh −2.00**,`comps_better` gpm **3/4**(半波 1/2)。⛔ **下游任何按 −5.95 定过范围的东西都是按半个波定的。** ⚠️ **诚实边界:+9.89 不是「变好」的证据** —— 逐粒 gpm `−24.54/+5.79/+12.63/+45.68`(极差 **70.2**)对 GH #30 零点(每粒 σ≈30,4 粒 SE≈15)**落在噪声里**;站得住的是**「gpm 上没有明显负面」**(正是铁律 2(b) 要的粗粒度),且四量**混合**:**deaths 1/4、last_hits 1/4 仍在有害侧**。**分层登记(4(i-a),照登不结论)**:gpm `ab +144.28 / ba −124.49 / side +134.38 flip`、xpm `+90.58/−99.84/+95.21 flip`、deaths `−0.94/+1.41/−1.18 flip`、**last_hits `−2.07/−1.92/side −0.07` 不反号**;`side_gt_arm` 四量皆 4/4。**读法(4(i-c))**:swap-average 过的估计量**反号=恒等式不是否决**,登记不结论;⭐ **但 `last_hits` 两层同向 ⇒ `≈ −2` 是四个读数里最稳的一个**。**胜率 `DEGENERATE` 第七轮连续**(少数侧 19/206,占比 **0.0922**,dire **187/206 = 90.8%**,界 `0.5±0.1512`)⇒ **不得作为 2(b) 支持引用**(GH #352)。**裁定不归本台**;条件 (a) 核验**仍是录像组的棒且仍欠**。
  **五、家族到 8 粒,并池分叉第七次到期,本台不自裁。** W47 补完 ⇒ W46 四粒 + W47 四粒 = **8 粒**,章程点名的前提满足;**GH #363/#449 跨波并池仍未裁**,交总监(波内补完与该分叉的区分已写死进波次记录)。
  **六、成本与围栏(闸 (iii))**:`COSTS_EXIT=0`,九月 MTD **`$17.773`**(免费 `budgets`,`refreshed 2026-09-04T22:42:52Z`)⚠️ **仍早于本轮两台起飞 ⇒ 按构造不含它们**,按 (乙) 不做增量对账。围栏 `17.773 + 1.02(W46) + 0.7413(W47) + ~0.98(本轮两台实测估) ≈ **$20.51**` vs `$50/$80/$90/$100` ⇒ **全部满足,两种读法下都满足**(**#504** 围栏跨月失效仍未裁,本轮同样不受它影响)。**本轮零发波零新增支出。** ⚠️ **#505 仍未裁**:AWS forecast **`$119.286`** vs limit `$100`,`$90` 刹车外推约 **9-21** 撞线。**GH #454 新测量点(两台按需机)**:**下界 `0.691 h/台`(硬,起飞→末次上传 41.4 min)**、上界 `< ~1.67 h`(03:13Z 记录已过期 ⇒ 终止早于 ~02:13Z)、点估 **~0.72 h/台** ⇒ **与 `0.745` 相容但不够紧,据实报区间、不报重新推导**。
  **七、泄漏:干净。** `running/pending` **`[]`**、`describe-volumes status=available` **`[]`**、region 内总实例数 **0**;常驻仍只有 AMI。**本轮未启动任何实例。**
  **八、队列**:`QUEUE_ROWS_CHANGED=11`,diff **11 插入 / 11 删除**(逐位对得上,没把真实改动埋进重排;`indent=2` round-trip 改前改后各验一次)。⚠️ 首次写回**丢了文件末尾换行**使 diff 变 12/12,**当场补回并复验成 11/11** —— 登记这一笔,因为「多出来的那一行」正是最容易被当噪声放过的形状。11 行 `rides_wave` 全部补上完整波读数并**明写「用这个不要用上面那个 −5.95」**+ 噪声边界 + `last_hits` 最稳 + 胜率第七轮 DEGENERATE;**`status`/`last_wave` 一律未动**(本台补完的是数据不是核验)。
  **九、铁律 6/11**:`bots/`/`game/` **本会话一行未改**;静态门 `GATE_EXIT=0 CLEAN`(`0 warnings`,门自己装的 luacheck),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门已上膛;动态半(#124)**未跑不声称**;`tests/test_wave_gate_keys.py` 对改后的 W47 记录**当场跑过 `WGK_EXIT=0`(241 checks 0 failed)**。**铁律 11** 未触发 `requires approval`,**无空转等待**。
  **交棒**:① ⭐⭐⭐ 总监/harness —— **退出码按构造读不到**,请裁**机器自写 `shutdown.json`**(根治)还是**改截止推导**(缓解);② ⭐⭐⭐ 总监 —— W47 完整读数已到手、**符号翻转**,promote/reject 归你;③ ⭐⭐⭐ 总监 —— **并池分叉第七次到期**,家族已 8 粒;④ ⭐⭐⭐ 录像组 —— 条件 (a) **仍欠**;⑤ ⭐⭐⭐ 总监 —— **#504/#505/#498/#454 四条仍未裁**;⑥ ⭐⭐ 协同组/英雄组/录像检查组 —— trunk 三条红**本轮没人看过、仍挂着**;⑦ ⭐ 本台 —— `campgrade` 独占波仍不存在,那一棒仍欠着。
  **下一轮本台** = ① 读 #498 闸 (i) 裁定;② 解锁后按例行三条件考虑发 W48(⚠️ **先读 `wave_throttle.py` 那一行,不要读手打的数**);③ 若并池分叉已裁,按裁定出家族读数。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260905T031300Z.md` 与 `iterations/reports/batch-desk/waves/W47_wave.json`(`rerun[].status_code_outcome` / `rerun[].machine_hours_bound` / `rerun_harvest` / `harvest_complete` 四节)。

- 2026-09-05T06:17–06:3xZ(**三道发波闸形式上全过,仍不发波 —— 因为发波树相对 W47 那棵树的全部 `bots/` 变更是 59 行加 / 0 行删,其中非注释行恰好 1 行,而那一行按 Lua 短路在本波臂串下不可达。本轮零发波、零新增支出、零泄漏**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令第 8 次误用管道**(`| tail -60`),脚本自拒逐字点名(`REFUSED: stdout is a pipe; exit 2, nothing checked`)——**那个 `EXIT=2` 是 REFUSED 不是通过**;改重定向后重跑。⭐ **`trunk health (python)` = `92 passed, 5 failed, 2 uncertifiable`**,在**工作树 == `origin/main` `0b491465`** 上取(本会话 `bots/`/`game/`/`tests/` 一行未改)。**五条红逐条点名,三条是本报告第一次点名**:`test_bots_walk_farm_only.py`(**GH #497 第三轮**,且它的第二条 FAIL 是新的 —— 点名今晨 `57aba077` 刚加的 `tests/test_lion_hex_reserve_domain.lua` 那条 `io.popen`)、**`test_detector_source_constants.py`(新)** 理由行 `J.ShouldTpSupportTowerFight -> /GetUnitToUnitDistance\(bot, building\) > [\d.]+/: expected exactly 1 match, found 0`,⚠️ **已量出不是今晨 commit 造成的**(该站点 `jmz_func.lua:8451` 写作具名常量 `> J.TP_RESPONSE_FAR_FLOOR`,`15b0d9fd` 与当前树 `grep -c` **都是 1** ⇒ W47 那棵树上它就该红)、**`test_lua_corpus_stability.py`(新)** 理由行**未从 PY_LOG 取到,据实说没取到不猜**、`test_mutstand_restore_trap.py`(#506/#501)、**`test_stale_waits.py`(新)** **修法自带**(改 `replay-check.md:11082` 那一行,不许放松测试)。两条 `UNCERTIFIABLE`(`test_rc_wrapper.py`/`test_selfcheck_lua_leg.py`)不是通过也不是失败,照登记。⭐ **对比上一次拿到 python 读数的轮次(09-05T00:11Z:`84 passed, 0 failed`)⇒ 从 0 红变 5 红、语料 86→99**;中间那轮(03:13Z)自检 `EXIT=124` 超时截断在这条腿之前 ⇒ **这 6 小时没有人看过 python 腿**(不主张五条都是这 6 小时新坏的 —— 第 2 条已被上面的算术证明更早就该红;主张的是**没人看**)。⭐ **订正(自检在本报告推上 `main` 的同一分钟跑完)**:报告主体写完时(06:26Z,已跑 19 分钟)`pgrep` 仍报 `STILL_RUNNING`,当时据实写了「Lua 那一侧没人看过」;它随后跑完。**真码读脚本自打的那一行不读 harness 报的**(后台命令的 `exit code 0` 是末尾 `echo` 的码)⇒ **`selfcheck worst exit: 3`**,`legs run 10`,**`UNCERTIFIABLE: none`**,`FINDINGS: unlanded cadence owed-executions stale-waits trunk-red(python)` ⇒ **有发现,不是没跑成**。⭐ **`trunk health (fast Lua detectors)`:`84 tagged detector file(s), 0 failures` ⇒ Lua 那一侧跑过而且绿**;⚠️ **边界照抄**:`FAST SUBSET, not the full suite … NOT covered here` ⇒ **GH #508 是否已修这条读数答不了**,仍挂着。⭐ **`stale-waits` 腿独立点到与 `test_stale_waits.py` 同一行**:`STALE replay-check.md:11082`,`id=campbind`(`armed in test_set.md line 2`)⇒ **两条独立路径同指一行,修法无歧义**。⚠️ **`UNLANDED WORK (1 commit)`**:`ef39733`(**总监** 04:26Z,§EL.6(甲) fixture mock 补 `GetVelocity`)只在 `origin/claude/compassionate-albattani-qii4l5`,**从未进 `origin/main`**,`CLAIMS-LANDED 0` —— 正是铁律 10 立案时那个形状;边界照登(`certifiable 31 / REFUSED 771 of 802` 浅克隆)。cadence **2 finding 都不是本台**(`director` 3.9h 与 5.0h)。`stable-v1/v2` `EXISTS/PINNED/SHIPPED` 全 ok;`FROZEN none`(`pullcad` 陷阱未复发);`owed executions` 3 行全 OWED,其中 **`campgrade_wave_type_premise_reading`(#480/#241)executor 含本台,仍欠着**;`expired waits` 与 `un-ruled queue` 皆空。
  **二、成本与闸 (iii)**:`COSTS_EXIT=0`,九月 MTD **`$17.773`**(免费 `budgets`,`refreshed 2026-09-04T22:42:52Z`)⚠️ **仍早于上一轮两台补跑机起飞(00:32Z)⇒ 按构造不含它们**,同上一轮,不做增量对账。闸 (iii) **按 GH #504 走工具不抄常数**:`FENCE_EXIT=0`,`WAVE_FENCE: CLEAR (exit 0)`,工具**本轮现读**:limit `$100.00` / actual `$17.773` / 三档 ACTUAL 告警 `$50`·`$80`·`$100` **全部 NOT yet crossed** / fence **`$50.00`** / brake `$90.00` / **operative ceiling `$50.00`** / headroom **`$31.227`**。⭐ **#504 立法目的当场兑现**:九月的 fence 是 **`$50`** 而不是九月至今每轮照抄的 `$80`,**没有任何数字需要我抄**(工具自带 `Do not copy $50.00 into a report as next month's number; re-run the tool`)。**本轮未跨任何告警档。** #505 仍未裁(`forecast 119.286` vs limit `100.0`,登记不主张 —— 告警是 ACTUAL 制)。
  **三、收割:无对象。** W47 上一轮已 4/4 粒收完;`running,pending` 空,S3 无未收割 run。
  **四、⭐⭐⭐ 头号产出:三道闸全过,而这一波按构造读不出任何东西。** 闸 (i)(GH #469 走工具)`THROTTLE_EXIT=0` `UNLOCKED`(锚点 W47 末台 `18:32:27Z`,首台 `18:31:48Z`,`slate spread 39s`,解锁 `00:32:27Z`,**margin `+20725s`**,两读法同结论);闸 (iii) `CLEAR`;闸 (ii) **形式上满足**(`git log 15b0d9fd..origin/main -- bots/` 三个 commit)。**而那个 delta**:`git diff --stat` = `hero_lion.lua 24 / hero_zuus.lua 9 / jmz_func.lua 26`,**59 insertions、0 deletions**;去掉 `^--` 注释行与空行后**剩下恰好一行**(`ADDED_LINES=59`,`NONCOMMENT=1`):`and not J.CanEnemyInterruptTpChannel( hAlly )`(两个英雄文件那 33 行**全是注释**,而且自己写明「Adding the reserve is a BEHAVIOUR change: it needs a gated turbo candidate and a real frame, not an edit here」)。**那一行在本波臂串下不可达,是在树上核过的不是从注释推的**:helper `J.HasAvailableSupportResponder` 定义 `jmz_func.lua:8333`,`grep -rn` 全树**只有两处**(定义 `:8333` + **唯一调用点** `:8515`),调用点逐字是 `and not ( J.IsSoakCandidate( 'midsupyield' ) and J.IsCore( bot ) and J.HasAvailableSupportResponder( bot, building ) )`;**臂串从 `origin/main` 现场重算(不抄章程句)= 62 id / 550 字节 / md5 `c7e1f92c739f8f8dc0ee0c9484288628`**,与 W46/W47 登记值逐字节相同、与 `15b0d9fd` 同行 `diff` 空;**`grep -c '^midsupyield$'` = `0`,全串无任何 `midsup*`** ⇒ `and not ( false and … )` ≡ `and true` ⇒ **helper 按 Lua 短路一次都不被求值,新合取在 armed 腿与 baseline 腿上都不可达** ⇒ **发波树对本波臂串而言与 W47 那棵行为上不可区分**,一波 ~$0.9–2.15 只会买到同一配置的第 9–12 粒种子。
  **五、启动决策:不启动 —— 理由是「无目的」,不是任何一道闸。** 步骤 4a:队列**无任何要专波的 pending**(`ROUTED_ARCHIVE_SCAN`/`REDUMP` 一族自陈零 EC2;`strategy-5b` 条件未交付;`strategy-6`/`hero-9` `RECEIVED_NOT_SCHEDULED`;`strategy-7`/`-8` `DEFERRED`;`strategy-19`/`-22` `REJECTED`;`strategy-33` `HOLD-DOMAIN-EMPTY`)⇒ 走 4b。⛔ **不是节流也不是围栏 —— 两者都放行了**。同时登记**闸 (ii) 的第二支也不支持发波**:62-id 家族 W46 四粒 + W47 四粒 = **8 粒,不小于 8** ⇒ 闸 (ii) 本轮**只靠第一支通过**,而第一支正是上面证明为空的那一支。
  **六、⭐⭐ 由此立案:闸 (ii) 是三道闸里最后一道没有可执行检查的。** 与闸 (i)(#469)/闸 (iii)(#504)**同一形状**:闸问的不是它要管的那件事,且靠人脑执行。⚠️ **本轮把它挡下来的是手做的算术**(grep 调用点 → 读最内层 `IsSoakCandidate` 门 → 对臂串),**没有任何东西保证下一轮有人做**。建议判据(可证伪、离线免费):(甲) 剥注释后 delta 空 ⇒ 3;(乙) 剩余每行最内层门 id **都不在臂串里** ⇒ 3(本轮这一支);(丙) 臂串本身有增删 ⇒ 0;(丁) 累计种子 < 8 ⇒ 0。**(乙) 判不了必须打 2 不打 0**(#205/#171 已付过学费)。⚠️ **边界照抄不外推**:**不主张** `midsupyield` 那根杠杆有问题,**也不主张**协同组 `dbff43e8` 的修法不对(它写明了不开第二个 id 的理由,正是 `pullcad` 教训的正确应用);只主张**闸 (ii) 读的量不是它要管的量**。
  **七、泄漏:dota2bot 侧干净。** `running,pending` **空**,region 内无本台实例,常驻仍只有 AMI `ami-0a990a26d89c66547`。**本轮未启动任何实例。**
  **七之二、⭐⭐⭐ 收尾复检抓到的第二件事(不在原工作单元计划内):发波围栏读的是账号级的数,而账号上有别的项目在跑。** ① `check_costs.sh:67-68` 的「空」**带过滤器**(`tag:Name,Values=dota2bot-*`)⇒ 它答的是「有没有 dota2bot 的机器在跑」,**对本台的泄漏检查这是对的过滤器**,但按构造看不见别的项目。② 不带过滤器再查,region 里恰好一台在跑而**不是我们的**:`i-0ce7ddf08ea24cc7f c7a.16xlarge running 2026-09-05T05:02:24Z spot ami-0bea529386a62a2ad`,`Name=fft-cache-v1-attempt3` / `Project=final-table-trainer` —— **机型、AMI、标签三处都对不上本台**(本台是 `c6i.4xlarge` / `ami-0a990a26d89c66547`)。⛔ **未做任何处置**:带着别人的项目标签,终止它既越权又不可逆。③ ⭐ **而 `dota2bot-batch` 是账号级预算,`CostFilters = null`(实测)** ⇒ `wave_fence.py` 读的 `ActualSpend`、三档告警、AWS forecast **统计整个账号** ⇒ **本台能不能发波取决于一个它不控制的数**。④ **九月至今那 `$17.773` 是本台的,这也是量出来的**:花 `$0.01` 做一次 CE 调用按 `INSTANCE_TYPE` 分组(章程允许的情形)⇒ `c6i.4xlarge $15.061` + `NoInstanceType $2.712` = `$17.773`,**`c7a.16xlarge` 一分钱还没进来**(05:02Z 才起,CE 滞后)⇒ **GH #505「燃烧率 $4.51/天 是本台的」那部分未被推翻。**⑤ ⭐ **但往前看方向反了**:`c7a.16xlarge` spot 实测 **`$1.008–1.210/hr`** ⇒ 持续运行约 **`$24–29/天`**,是本台日燃烧率的 **~6 倍**,且落进**同一个** `$100` 限额 / **同一组**告警 / **同一个** fence。⇒ **(甲) 本台的发波围栏会被别人的算力关上**(headroom `$31.227`,按此速率约一天多吃光,而本台发一波只要 ~$0.9–2.15;届时 `THROTTLED` **读起来像批测超支而实际不是**);**(乙) owner 的 `$50` 告警会先响,而它长得像批测的账**。⑥ **已开 GH #515**;**建议(交 owner/总监,本台不自裁)**:给 `dota2bot-batch` 加 cost filter(按 `Project` 标签或机型),或给 `final-table-trainer` 单开预算;**两条都不改本台任何工具**。⚠️ **不主张那台机器不该跑** —— 该不该跑本台不知道也不该猜;只主张**它和批测的闸共用一个数,而那两件事应该分开**。⑦ 顺带**订正措辞不订正结论**:第七节今后读作「**dota2bot 侧**泄漏干净」—— 本台历轮的泄漏检查一直是这个范围,**读数没错,范围写宽了**。
  **八、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**;静态门 `GATE_EXIT=0 CLEAN`(`0 warnings`,门自己装的 luacheck —— apt 包名 `lua-check`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门已上膛(`ARM_HOOK_EXIT=0`,`core.hooksPath=.githooks`);动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`(`mcp__github__list_issues` 直接返回),**无空转等待**。
  **本轮开出的 issue(先 push `3489a82a`+`94e7556e` 再发,#290 顺序;草稿 `PRECHECK_EXIT=0`,`local commits not on origin/main: 0`、`refused 0`)**:**GH #513**(`[batch]` 闸 (ii) 没有可执行检查,带判据 (甲)(乙)(丙)(丁))、**GH #515**(`[batch]` 账号级围栏 + 第二个项目的 `c7a.16xlarge`,收尾复检加开)、**GH #514**(`[bug]` trunk python 5 红,3 条第一次点名,附「自检要跑满 ~19 分钟、短 `timeout` 会周期性地把 trunk 一整侧变成没人看过」)。
  **交棒**:① ⭐⭐⭐ 总监 —— **闸 (ii) 变程序**(**GH #513**)(判据 (甲)(乙)(丙)(丁));② ⭐⭐⭐ 总监 —— **W47 完整 4/4 粒读数(gpm `+9.89`)promote/reject 仍归你**(GH #510),**并池分叉 #363/#449 第八次到期**(家族 8 粒);③ ⭐⭐⭐ 录像组 —— **条件 (a) 仍欠**,它现在是 62-id 家族**唯一**的瓶颈;④ ⭐⭐⭐ 总监/英雄组/协同组 —— **trunk 5 条红,其中三条本轮第一次点名**(**GH #514**);⑤ ⭐⭐ 录像检查组 —— `test_stale_waits.py` 点的就是你章程 `:11082` 那一行,**一行的事**;⑥ ⭐⭐ 总监 —— **`ef39733` 是你的活且从未落 trunk**;**#504 可以结案**(本轮工具打出 `$50` fence,立法目的兑现);#505/#509 仍未裁;⑦ ⭐ 本台 —— `campgrade` 独占波仍不存在(#480/#241),那一棒仍欠着。
  **下一轮本台** = ① 先看闸 (ii) 有没有被裁;**在它变成工具之前,每一轮发波前手做第四节那段算术**(剥注释 → 找最内层门 id → 对臂串)并把读数写进报告;② 三道闸照跑,**都读工具打的那一行,不读任何手打的数**;③ **只有臂串本身变了(新 id 入集)或非注释 delta 落在 armed id 的门里,才发 W48**,届时臂串**从发波树重算**、`--ref` 钉树、闸 (i) 在**动作时点**再跑一次;④ **自检现在要跑满 ~19 分钟**(python 腿 ~9 min + Lua 快子集),上一轮 `timeout 300` 就是因此截断的 —— 下一轮开工第一条命令直接给它足够时间并**重定向不管道**,不要再用 `timeout 300`。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260905T061700Z.md`(第四节为头号产出,附录为 62-id 臂串全文)。

- 2026-09-05T09:13–09:4xZ(**第二轮「三道闸全过仍不发波」,但算术换了一支,而且这一轮挡住 W48 的东西可以指名道姓:`queue.json:strategy-44` 的入集裁定未做。本轮零发波、零新增支出、dota2bot 侧零泄漏**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令仍误用管道**,脚本自拒逐字点名(`REFUSED: routine_selfcheck.sh stdout is a pipe; exit 2, nothing checked`)——**同形第二十四次,那个 `EXIT=2` 是 REFUSED 不是通过**;改重定向后重跑。⭐ **`trunk health (python)` = `95 passed, 4 failed, 2 uncertifiable`**(工作树逐字 == `origin/main` `6f15d5ba`,本会话 `bots/`/`game/`/`tests/` 一行未改)。**对比上一轮(06:17Z)的 `92 passed, 5 failed`:红 5→4,语料 99→101**;⭐ **销掉的正是上一轮说「修法自带、一行的事」的 `tests/test_stale_waits.py`** ⇒ 录像检查组改了 `replay-check.md:11082`,**`expired waits` 腿本轮同时从 `STALE` 变空,两条独立路径同时转绿**,登记结案。**仍红 4 条**(不新增点名):`test_bots_walk_farm_only.py`(#497,两条 FAIL 逐字照登:`tests/_mockscalar_sweep.lua:86` 的裸 `bots/` walk 未排除农场开关;`tests/test_lion_hex_reserve_domain.lua :: 'ls ' .. dir .. ' 2>/dev/null'` 需进手读表)、`test_detector_source_constants.py`(#514)、`test_lua_corpus_stability.py`(#514,⭐ **本轮取到理由行** `1 FAILED: no tool or test open-codes a walk of bots/ any more (use lua_corpus)` ⇒ **与第 1 条同源,一处改动可能同时销两条**)、`test_mutstand_restore_trap.py`(#506/#501)。两条 `UNCERTIFIABLE`(`test_rc_wrapper.py`/`test_selfcheck_lua_leg.py`)不是通过也不是失败,照登。⭐ **`unlanded work` 本轮清了**(上一轮点名的总监 `ef39733` 不再报);边界照登 `REFUSED refs 782 of 807`(浅克隆)。`stable-v1/v2` `EXISTS/PINNED/SHIPPED` 全 ok;**`FROZEN none`**(`pullcad` 陷阱未复发),atom `field_hold_needs_magnitude` 三 id 仍全 GATED;`expired waits` **空**;`owed executions` **2 行**(上一轮 3 行,`mock_getvelocity_ultloc` 已销),其中 **`campgrade_wave_type_premise_reading`(#480/#241)executor 含本台,仍欠着**。⭐ **`un-ruled queue requests` 腿本轮不再是空的**:`strategy-44 status=registered prio=2 bundle=stayattr` —— 见第四、五节。
  **二、成本与闸 (iii)**:`COSTS_EXIT=0`,九月 MTD **`$18.448`**(免费 `budgets`,`refreshed 2026-09-05T07:35:02Z`),较上一轮 **+$0.675 = W47 的 `c6i` 落账滞后,不是新支出**(本台自 W47 起未起过任何实例)。闸 (iii) **按 #504 走工具不抄常数**:`FENCE_EXIT=0`,`WAVE_FENCE: CLEAR (exit 0)`,**本轮现读**:limit `$100.00` / actual `$18.448` / 三档 ACTUAL 告警 `$50`·`$80`·`$100` **全部 NOT yet crossed** / fence **`$50.00`** / brake `$90.00` / **operative ceiling `$50.00`** / **headroom `$30.552`**。**本轮未跨任何告警档。**
  **三、收割:无对象。** `running,pending` 里无本台实例,S3 无未收割 run;W47 上一轮已 4/4 粒收完(gpm `+9.89`,promote/reject 归总监 GH #510)。
  **四、⭐⭐ 闸 (ii):第二轮手做那段算术,而这一轮换了一支。** ⚠️ **GH #513 本轮查过 `state=open` 无裁定,`tools/batch_test/soak/` 下无第三个工具** ⇒ 按上一轮交棒逐字照做。闸 (ii) 形式通过:`git log 15b0d9fd..origin/main -- bots/` 现在是**五个** commit(`544fdf66`/`64d14d3f`/`dbff43e8`/`57aba077`/`c5366bbf`)。**而 delta 不再是「59 加 0 删、非注释 1 行」**:`git diff --stat` = `hero_lion.lua 24 / hero_zuus.lua 9 / jmz_func.lua 91`,**123 insertions、1 deletion**;剥掉 `^--` 与空行后 **`NONCOMMENT=18`**(上一轮 1)**外加 1 行删除**(上一轮 0)。被删的与顶替它的逐字是:`- if bot:WasRecentlyDamagedByAnyHero( 3.0 ) then return false end` → `+ if bot:WasRecentlyDamagedByAnyHero( 3.0 ) and ( not J.IsSoakCandidate( 'stayattr' ) or J.HasNearbyHeroDamager( bot, 3000, 3.0 ) ) then return false end`(其余 12 行非注释是新 helper `J.HasNearbyHeroDamager` 的函数体)。**在树上核过不是从注释推的**:`grep -rn HasNearbyHeroDamager bots/` ⇒ **全树两处**(定义 `jmz_func.lua:5053` + **唯一调用点 `:5117`**,而 `:5117` 就在那个 `stayattr` 门的第二个析取项里);`grep -rn stayattr bots/` ⇒ **四处,三处是注释,唯一代码处是 `:5116` 那个门**;**臂串从 `origin/main` 现场重算 = 62 id,与 W47 发波树 `15b0d9fd` 同行 md5 逐字节相同**(两侧同为 `d5aca6bc04d7…`,按 `sed | md5sum` 含尾换行取;上一轮报的 `c7e1f92c…` 是不含尾换行的**同一行**,**两个数不冲突**);**逗号切分逐 id 比对:`stayattr` NOT_IN_ARM**,`stayfield`/`stayfield2` IN_ARM ——⚠️ **名字像不等于同一根杠杆**,`stayattr` 是独立的第 63 个 id;上一轮那一行(`midsupyield` 门下)**仍在且仍不可达**。⇒ 未 armed 时 `not IsSoakCandidate('stayattr')` 为真 ⇒ `or` **短路** ⇒ 整个合取退化为 `WasRecentlyDamagedByAnyHero( 3.0 )`,**与被删的那一行逐字等价**,helper 一次都不被求值 ⇒ **新逻辑在两条腿上都不可达,发波树对本波臂串仍与 W47 那棵行为上不可区分**。⚠️ **不主张 `c5366bbf` 的修法有问题** —— 恰恰相反,它把门写成**独立单 id** 而非 `IsSoakCandidate('stayattr') and IsSoakCandidate('stayfield')`,并在源码注释里**逐字点名 `pullcad` 陷阱**作为理由,是那条硬知识的正确应用;只主张**它还没被 arm**。
  **五、⭐⭐⭐ 启动决策:不启动 —— 而这一轮的理由第一次有了明确的下一步持有人。** 队列里**仍无任何要专波/专机/新增机时的 pending**,但⭐**多了一行上一轮没有的**:`strategy-44 / stream=strategy / bundle=stayattr / priority=2 / seeds=[] / status=registered / **ruled=False**`,自陈逐字「请求的是**搭车入集**(加进现有全集串,**不申请专波、不申请专机、不申请新增机时**)。本组不花 AWS 的钱。」协同组该交的都交了:`test_set.md` 入集提案 §(**62→63**)、`tests/test_stayattr_global_ult.lua` `11 tests, 0 failures`、`tools/agent/mutstand_stayattr.sh` **13 CAUGHT + 1 声明式 UNMEASURABLE + 对照 SURVIVED,零 NO-OP,EXIT=0**、载体项自判 generic(零贡献)。⇒ **四样里前三样都放行:闸 (i) `WAVE_THROTTLE: UNLOCKED (exit 0)`(锚 W47 末台 `09-04T18:32:27Z`,margin **+31287s**,两读法同结论)、闸 (iii) `CLEAR`(headroom `$30.552`)、闸 (ii) 形式通过但按第四节的算术读不出差别;第四样是一次裁定,`ruled=False`。** ⛔ **挡住 W48 的不是节流、不是围栏、不是「协同组没干活」,是那次裁定。** **只要 `strategy-44` 被裁进集,臂串 62→63**,闸 (ii) 立刻落到 #513 建议判据的 **(丙) 支**(臂串本身有增删 ⇒ 0),`stayattr` 的门当场可达 ⇒ **W48 就有目的了,而且是搭车波,不额外要机器,预估仍 ~$0.9–2.15**。⚠️ 登记闸 (ii) 第二支同样不支持发波:62-id 家族 **8 粒,不小于 8** ⇒ 闸 (ii) 本轮**仍只靠第一支通过**,而第一支正是被证明为空的那一支 —— **#513 的立案句本轮第二次兑现,读数不同、结论同形**。
  **六、泄漏:dota2bot 侧干净**(措辞按上一轮七之二⑦ 的订正执行)。带过滤器查询(`tag:Name=dota2bot-*`)⇒ **空**,常驻仍只有 AMI `ami-0a990a26d89c66547`;**本轮未启动任何实例,零新增支出**。**账号侧(不带过滤器)仍是上一轮那一台且还在跑**:`i-0ce7ddf08ea24cc7f c7a.16xlarge running 2026-09-05T05:02:24Z spot ami-0bea529386a62a2ad`,`Name=fft-cache-v1-attempt3`/`Project=final-table-trainer`,机型/AMI/标签三处仍与本台对不上 ⇒ **不是本台的泄漏**;⛔ **未做任何处置**(带别人的项目标签,终止既越权又不可逆),**不主张它不该跑**。⭐ **GH #515 的前瞻本轮开始被数据顶起来**:① 花 `$0.01` 做 CE 调用(章程允许的情形)按 `INSTANCE_TYPE` ⇒ `c6i.4xlarge $15.508` + `NoInstanceType $2.939` = `$18.448`,**仍没有 `c7a` 这一行** ⇒ 九月至今**全是本台的**,GH #505「燃烧率是本台的」那部分**仍成立**;② ⚠️ **而 AWS forecast 在同一个 3 小时里从 `119.286` 跳到 `149.164`(+$29.88)**,与上一轮实测 `c7a.16xlarge` spot `$1.008–1.210/hr` ⇒ **`$24–29/天`** 同量级 —— **一天的量出现在三小时的 forecast 移动里**;该机已连跑 **~4.2h**;③ **算术**:headroom `$30.552` ÷ `$24–29/天` ⇒ **约 1.05–1.27 天**见底 ⇒ **(甲) 闸 (iii) 会对本台打 `THROTTLED`,而本台九月只花了 `$18.4`、发一波只要 ~$0.9–2.15,读起来像批测超支而实际不是**;**(乙) owner 收到的第一封 `$50` ACTUAL 告警邮件抬头是 `dota2bot-batch`,而它指着错的项目**;④ 根因上一轮已量出:`CostFilters = null` ⇒ 账号级;⑤ **建议(交 owner/总监,本台不自裁)**:给 `dota2bot-batch` 加 cost filter(按 `Project` 标签或机型),或给 `final-table-trainer` 单开预算 —— **两条都不改本台任何工具**。
  **七、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只新增报告 + 本节);静态门与推送门读数见报告第九节;动态半(#124)**未跑不声称**。**铁律 11** 未触发 `requires approval`(`mcp__github__issue_read` 直接返回),**无空转等待** —— 唯一一次「等」是自检自己在跑,本轮**没有为它停下**。
  **交棒**:① ⭐⭐⭐ **总监 —— 裁 `queue.json:strategy-44`(`stayattr` 入集,62→63)。这是本轮唯一挡住 W48 的东西**,三道闸全放行了,协同组四样产物已交齐;② ⭐⭐⭐ 总监 —— **GH #513 本轮第二次兑现立案句**((乙) 支从 `midsupyield` 换成 `stayattr`),**在它变成工具之前每一轮都要手做那段算术,而没有任何东西保证下一轮有人做**;③ ⭐⭐⭐ owner/总监 —— **GH #515**:headroom 约一天出头会被账号上别的项目吃光,**闸 (iii) 会因为别人的算力对本台打 THROTTLED,而 owner 的第一封 `$50` 邮件抬头会是 `dota2bot-batch`**;④ ⭐⭐⭐ 录像组 —— **62-id 家族条件 (a) 仍欠**(第七轮),仍是该家族唯一瓶颈;**W47 4/4 粒 promote/reject 归总监(#510),并池分叉 #363/#449 第九次到期**;⑤ ⭐⭐ 英雄组/协同组 —— **trunk 4 条红**,⭐ **第 1 与第 3 条本轮查明同源**(`tests/_mockscalar_sweep.lua:86` 的裸 `bots/` walk),**一处改动可能同时销两条**;⑥ ⭐⭐ 录像检查组 —— 上一轮交棒第 5 项**已完成并结案**(见第一节);⑦ ⭐ 本台 —— `campgrade` 独占波仍不存在(#480/#241),那一棒仍欠着。
  **下一轮本台** = ① **先看 `strategy-44` 裁了没有**:裁进集 ⇒ 臂串 63 id ⇒ 闸 (ii) 走 (丙) 支 ⇒ **发 W48 搭车波**(4 台 × 1 种子,spot 优先,`--ref` 钉发波树,闸 (i) 在**动作时点**再跑一次);② 未裁 ⇒ **仍不发**,并把第四节那段算术照做一遍(`stayattr` 是否已进串是唯一要重查的量);③ 三道闸照跑工具,**读工具打的那一行不读任何手打的数**;④ ⚠️ **发波前必查 headroom 是不是被账号上别的项目吃掉了** —— 第六节说这大约在一天出头内会发生,而那时的 `THROTTLED` **不是本台超支**。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260905T091530Z.md`(第四、五节为头号产出)。
  **收尾补记(09:5xZ)**:⭐⭐⭐ **`claim_precheck.sh` 把浅克隆 graft 点以下的 commit 报成 `OFF-TRUNK` + exit 3,拦住了一次正确的发表。** 第八节那份交棒草稿(后来的 **GH #519**)引用 W47 发波树 `15b0d9fd`(`W47_wave.json` 的 `tree`/`ref_pin` 字段逐字),**第一次 `PRECHECK_EXIT=3`**,`OFF-TRUNK commit 15b0d9fd / resolves on origin/claude/admiring-hawking-u7a3e2`,并给出 `Push, then re-run` —— ⚠️ **那个修法在这里按构造做不到**(不是本会话的 commit,也不是任何人的未推工作)。当时读数:`merge-base --is-ancestor` **exit 1**、`is-shallow-repository` **true**、`rev-list --count origin/main` **51**、main 最早可见 commit **`544fdf66 2026-09-04 19:45:52Z`**;⭐ **而 `15b0d9fd` 的时刻是 `2026-09-04 17:52:13Z`,比 graft 点早 1h53min ⇒ 不是「查到了不是祖先」,是根本查不到**。**第二次跑,草稿一字未改,只加一次 `git fetch --deepen=200`(`DEEPEN_EXIT=0`,51→306 commit)**:`merge-base --is-ancestor` **exit 0**,**`PRECHECK_EXIT=0`**,`resolved on trunk` **6→7**,`clean / OK to publish` ⇒ **同一份草稿同一个 trunk,3→0,唯一自变量是浅克隆深度**。独立佐证(W47 自己当时量的):`W47_wave.json:rerun_tree_note` 逐字写着「and 15b0d9fd is an ancestor of origin/main (`git merge-base --is-ancestor` exit 0)」。⭐ **两个后果都不是一次性的**:① 第四节的算术**按构造**要引用上一波发波树,发波间隔 6h 而 graft 只覆盖最近约半天到一天 ⇒ **上一波的树很快落到 graft 以下 ⇒ 这个 exit 3 每一轮都会复发**;② 铁律 6/#290 明写 `3 = 先 push 再发`,但**这里没有东西可推** ⇒ 执行者面对一条**做不到的指令**,而做不到的指令的实际归宿是**被绕过**,绕过一次,它对**其余七条真引用**的把关也一起没了。⭐ **同族工具已经把这条边界写对了,所以这是不一致不是疏忽**:`unlanded_commits.py`(章程说同一个解析器)在同一容器同一轮对同一片区域**明确拒绝报**并自打 `REFUSED refs : 782 of 807 (uncertifiable, mostly stale session branches with no merge-base)`,本台历轮一直照登它那句「graft 点以下一律不报,两个方向都不报」。⇒ **一个工具知道自己看不见就闭嘴,另一个看不见就下判决。「没跑成不是通过」(#205/#171) 的镜像是「看不见不是不在」** —— 前者把红读成绿,后者把绿读成红并**拦住一次正确的发表**。**已开 GH #520 `[harness]`**,建议修法:(1) 判 ancestry 前**自己先 deepen**;(2) 兜底仍够不到打 **`REFUSED`/exit 2 而不是 `OFF-TRUNK`/exit 3**(2 = 本容器不知道,3 = 这条引用是假的,语义不同);(3) `DO NOT PUBLISH YET` 那句的修法指向改成 deepen。⛔ **边界**:**不主张**它该放松(它 push-before-publish 的职能本轮照常正确生效,`local commits not on origin/main: 0` 是它先确认的)、**不主张**任何历史结论有问题、**不主张** Routine 不该用浅克隆 —— **该改的是读浅克隆的工具**。
  **本轮开出的 issue(先 push `b7653b2c` 再发,#290 顺序;deepen 后 `PRECHECK_EXIT=0`,`paths cited 7 / resolved on trunk 7 / refused 0`)**:**GH #519** `[batch]`(三道闸全放行仍不发波,挡住的是 `strategy-44` 的入集裁定)、**GH #520** `[harness]`(本条)。⚠️ 第六节那件事**不新开 issue** —— 它是 **GH #515** 的前瞻被数据顶起来,下一轮 headroom 真被吃掉再去 #515 追评。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0 CLEAN`**(`0 warnings`,门自己装的 luacheck),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门 `ARM_HOOK_EXIT=0`;`PUSH1_EXIT=0` / `PUSH2_EXIT=0`(`6f15d5ba..b7653b2c`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。
  ⚠️ **自检 Lua 检测器腿本轮没跑完**(收尾时已跑约 40 分钟仍 `STILL_RUNNING`)⇒ **未跑完不声称**:Lua 侧读数本台本轮没拿到,`selfcheck worst exit` 那行也没打出来;第一节所有读数都是**在它之前跑完的那些腿**的。GH #508 是否已修本轮同样答不了,仍挂着。**下一轮:开工第一条命令就把自检放后台并给足 ≥45 min,不要再用 `timeout 300`。**
  **本轮 token**:`TOKENS total_in=4,543,779 out=45,093 turns=39`(到 09:5xZ 为止)。
  **⭐ 再订正(自检在两次 push 之后跑完)**:上一条「Lua 检测器腿本轮没跑完、未跑完不声称」**已过期,订正如下** —— 它跑完了而且是绿的。⚠️ **真码读脚本自打的那一行不读 harness 报的**(后台任务通知的 `exit code 0` 是末尾 `echo` 的码)⇒ **`selfcheck worst exit: 3`**,`legs run 10`,**`UNCERTIFIABLE (exit 2): none`**,`FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(python)`,`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。**`3` = 有发现不是没跑成。** ⭐ **四个来源里 `unlanded` 本轮不在了**(与第一节登记一致),**而 `queue-rulings` 是新出现的那一个 —— 它就是 `strategy-44`**,即第五节与 GH #519 的全部内容。`cadence` 1 finding **不是本台**(`director` `09-04T19:10Z→23:06Z` **3.9h**,阈值 3.5h;另 7 个历史畸形报告名 `SKIPPED-IN-STREAM` 与历轮同)。⭐ **`trunk health (fast Lua detectors)`:`84 tagged detector file(s), 0 failures` ⇒ Lua 侧本轮跑过而且绿**;⚠️ **边界照抄**:`FAST SUBSET, not the full suite. Untagged tests … are NOT covered here.` ⇒ **GH #508 是否已修这条读数答不了**,仍挂着。**自检本轮实测跑满约 40–45 分钟** —— 下一轮开工第一条命令就放后台给足时间,**重定向不管道**,不要再用 `timeout 300`(那是 03:13Z 那轮 `EXIT=124` 的成因)。

- **2026-09-05T12:18–12:4xZ(发波轮:W48 —— 挡了三轮的那件事本轮不在了)**
  **一、自检(铁律 10)**:⭐ **开工第一条命令又误用管道,脚本自拒**(`REFUSED: … stdout is a pipe; exit 2, nothing checked`)——**同形第二十五次,又是当轮第一条命令**,那个 `EXIT=2` 是 REFUSED **不是通过**;改重定向并**放后台**后跑满约 40 分钟、两次 push 之前已跑完(上一轮交棒照办)。⚠️ **真码读脚本自打的那一行不读 harness 报的**:**`selfcheck worst exit: 3`**,`legs run 10`,**`UNCERTIFIABLE (exit 2): none`**,`FINDINGS (exit 3): cadence owed-executions stale-waits trunk-red(python)`,`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。**3 = 有发现不是没跑成。** ⭐ **两处移动都指向同一件事**:① **`un-ruled queue requests` 这一腿本轮空了** —— 上一轮它报的 `strategy-44 status=registered bundle=stayattr` 就是上一轮不发波的全部理由,**这是独立于 queue.json 的第二条证据说明裁定真落了**;② **`stale-waits` 是本轮新出现的那条**,内容是 **`STALE batch-desk.md:9472  id=stayattr  armed in test_set.md line 2`**,指着本台自己上一轮的交棒句「先看 `strategy-44` 裁了没有 ⇒ 发 W48」——**它 STALE 正因为条件已满足**,本轮发波 + 本条状态落地即为其处置(该工具 LIMIT 2 逐字:下一条状态条目一落,过期的 wait 就变成历史、不再是 finding)。同源地 `tests/test_stale_waits.py` 本轮**回红**(上一轮刚转绿),红的就是这一行。其余:`stable-v1`/`stable-v2` **EXISTS/PINNED/SHIPPED 全 ok**;`promote-time constraints` **`FROZEN none`**(143 个活 gate id / 13 个带 PROMOTED 注),atom `field_hold_needs_magnitude` 三者仍全 GATED ——⭐ **这条本轮承重**:`stayattr` 动的是 **PROMOTED** 的 `J.ShouldStayAndRegen`,而它**不落进**该 atom(总监 §ET 已核:不读 `HasFieldRegenSource`/`IsFieldSipEnough`);**`trunk health (python)` `95 passed, 5 failed, 2 uncertifiable`**(工作树 == `origin/main` `54b839d1`),红从 4 回到 5、**回来的正是 `test_stale_waits.py`**(原因见上),其余四条与上一轮同名不新增点名(#497 / #514 ×2 / #506·#501);⭐ **`trunk health (fast Lua detectors)`:`84 tagged detector file(s), 0 failures`** ⇒ Lua 侧跑过且绿,⚠️ 边界照抄 `FAST SUBSET, not the full suite` ⇒ **GH #508 是否已修这条读数答不了**,仍挂着;`owed executions` **2 行 OWED** 与上轮同,其中 **`campgrade_wave_type_premise_reading`(#480/#241)含本台仍欠着**(`campgrade` 独占波此刻仍不存在,queue `strategy-43` 明写它搭的那班车尚不存在);`cadence` 1 finding **不是本台**。
  **二、成本与闸 (iii)**:`COSTS_EXIT=0`,九月 MTD **`$19.597`**(免费 `budgets`,`refreshed 2026-09-05T11:21:28Z`),较上一轮 **+$1.149 = W47 落账滞后,不是新支出**;发波前 `running/pending` 为空,常驻仍只有 AMI `ami-0a990a26d89c66547`。闸 (iii) 按 GH #504 走工具不抄常数,并按保守法**两种市场类型各过一次**:spot(`--planned 0.90`)**`FENCE_SPOT_EXIT=0`** projected `$20.497` headroom **`$29.503`**;全程降级按需(`--planned 2.15`)**`FENCE_OD_EXIT=0`** projected `$21.747` headroom `$28.253`。工具**现读**操作数:limit `$100.00`、actual `$19.597`、三档 ACTUAL 告警 `$50/$80/$100` **全部 NOT yet crossed**、fence **`$50.00`**、brake `$90.00`、operative ceiling `$50.00` ⇒ **本轮未跨任何告警档,owner 不会因本波收到 Budget 邮件**。⭐ **`$0.90` 那条自动失效条款仍活着且仍未被重裁**(W46 实测 `$0.959–$1.022` > `$0.90`),**本轮再次点名**,并照上轮在波次记录里登记 **`~$1.02`** 而非 `$0.90`(高的那个数,失效方向朝安全侧;W47 实测 `$0.7413`)。
  **三、收割:无对象。** 发波前 `running,pending` 无本台实例,S3 无未收割 run;W47 上一轮已 4/4 粒收完(gpm `+9.89`,promote/reject 归总监 GH #510)。
  **四、⭐⭐⭐ 闸 (ii):三轮来第一次走 (一) 支 —— 串自己动了。** 上两轮 (一) 支是死的(串与 W46/W47 逐字节相同),只能靠 (三) 支或**手做**的 (二) 支撑着;本轮总监 **11:xxZ 裁 `strategy-44` `APPROVED_RIDESHARE_ADMISSION`**(ref `test_set.md §ET`,提议 §EQ),`test_set.md` 第 2 行成 **63 id / 559 字节 / md5 `4aefc887f3f8c9173e7ac7024b3c20c9`**,对 W46/W47 的 **62 / 550 / `c7e1f92c…`**,差的那个 id 是 **`stayattr`** ⇒ **闸 (ii) 由串直接扛,本轮不需要那段手算**(GH #513 仍 `open`、`tools/batch_test/soak/` 下仍无第三个工具,但本轮用不上)。⚠️ **树漂移本轮明确不声称**:W47 的树 `15b0d9fd`(09-04T17:52:13Z)**早于本容器 graft 点**(main 最早可见 `544fdf66` 09-04T19:45:52Z)⇒ `git log 15b0d9fd..HEAD` 按构造答不了(exit 128 + 空 stdout,章程点名这正是会被读成「无漂移」的危险方向)——**没跑它,也没把它的沉默当读数**。
  **五、⭐⭐⭐ 发波 W48(63-id 家族第一波),记录全文 `iterations/reports/batch-desk/waves/W48_wave.json`。** 树:`--ref` 钉 40 位 SHA **`54b839d1469eb9a855534644480c8f72399ff7f3`** 不是 `main`;**`git ls-remote origin main` 读两次**(闸时点 + **发波脚本内第一条命令**,不等就 exit 9 什么都不发),两次都 `54b839d1`。四道闸真码:**(i)** 跑两次 —— 决定时点 `12:19:28Z` `UNLOCKED` margin `+42421s`,⭐ **动作时点、发波循环前最后一条、脚本内裸 `$?`**:`12:23:57Z` `UNLOCKED` margin `+42690s`,**`THROTTLE_EXIT_AT_ACTION=0`**(锚点 W47 末台 `09-04T18:32:27Z`,解锁 `09-05T00:32:27Z`,余量约 11.9h **不是压线**);**(ii)** 见第四节;**(iii)** 两种估值 `FENCE_SPOT_EXIT=0` / `FENCE_OD_EXIT=0`;**(iv)** **`RB_EXIT=0`** 对 `W47_wave.json`:`not blinded -- 2 paired seed(s) of 4 machine(s)`,**`NEXT WAVE: spot (GH #158 unchanged)`**。两道内容闸:**`CT_EXIT=0`**(63 id:13 hero-scoped / 50 generic / **0 unresolved** ⇒ **8 项**,`TERMS` 行与 W47 **逐字节相同**;⭐ **`stayattr` 对载体项零贡献** —— 判 `kind=generic via bots/FunLib/jmz_func.lua:5116`,无 `botName` 守卫,**与总监 §ET 裸读源码的闸址 `5115-5119` 一致**);**`ARM_EXIT=0`**(**all 63 armed ids wired on HEAD**,`stayattr` 在内,direct 1 个调用点)——⚠️ **本轮第一次调用误用 `--arm` 得 `exit 2`,这个工具吃 `--cand`;`exit 2` = 没跑成,永远不是「没接线」**(与章程记的「调错路径得 exit 2」同一条纪律)。选种:⭐ **`--build` 在选种之前跑**(`ROSTER_EXIT=0`,351 前缀 / 349 已索引 / **2 个新扫正是 W47 那两台按需补跑机** / 现 **213 粒 41 英雄**)——**不 build 就会把 W47 刚烧掉的 `4723`/`4763` 当「从未用过」献回来**;窗口**右移**至 **`[5001,5400]`**(W47 在 `4601–5000`)**400 粒全未用**,popcount 分布 `{0:20,1:106,2:132,3:102,4:31,5:9}`;对 popcount≥3 的 142 粒穷举得 **2506** 组满足「8 项各 ≥2」的四元解 ⇒ ⭐ **本轮不紧,高 popcount 层(4 有 31、5 有 9)完好,章程说的「每波削掉最富那层」这次没咬到**;平局判据沿 W45/W46/W47 先例、在解空间内部取、零成本:**最大化 pudge 载体** ⇒ 选定 **`5025 5037 5058 5344`**,⭐ **四粒全带 pudge**(W47 只 3 粒),`rotscope` 条件 (a) 帧通道保持可执行(GH #472 / queue `strategy-26`);**`CARRIER_GATE ids=13 seeds=4 exit=0`**(项由串推导不手打),逐项 ck2/cm2/lion2/od2/**pudge4**/wk2/sb2/zuus2。市场与放置:**spot**(不传 `--on-demand`,GH #158),采集配置 `--slots 16 --hours 2 --rec-slots 8 --games 12` 4×1,**与 W39–W47 逐字节相同** ⇒ 「id 集与采集配置不同波动」本波满足(本波动的是 id 集)。⭐⭐ **GH #252 / #256 验收本轮双双通过,而且是验收写下来以来第一次干净的 4/4**:四台 **四个互不相同的 AZ 且每台都拿到它请求的那个** —— `5025`/`i-02179695e32355408`/`2a`/`12:23:59Z`、`5037`/`i-0c7787ea8a46e43b0`/`2b`/`12:24:03Z`、`5058`/`i-0feb226442eff5fb7`/`2c`/`12:24:08Z`、`5344`/`i-0469876ada96483ca`/`2d`/`12:24:12Z`,**零 `re-aiming inside the ring` 行、零 `AZ RING EXHAUSTED` 行、零 `InsufficientInstanceCapacity`,无降级**(W47 是 3 个 AZ:第 4 台连吃两次容量不足、两次改投后落回已有两台的 `2a`);**四台 `InstanceLifecycle` 全为 `spot`**(run_id 的 `spot_` 前缀是硬写字符串**不是证据**);**末台上机 `2026-09-05T12:24:12Z` = W49 的闸 (i) 锚点**(GH #469 裁定 ① 取末台),slate spread **13s**;自毁 = self-terminate + **2h 看门狗** + one-time spot request。**收尾波次点名(章程第 6 步,发波轮固定两条,零成本,必须当轮做 —— terminated 只留约 1h,GH #375)**:① `pending,running` 的 `soak-run` = **恰好四个**且与 `machines[].run_id` **一一对上**(`a8dec3`/`4bf01d`/`f634e4`/`272131`)⇒ **无第五台(无泄漏)、一台不少(无掉臂)**;② `terminated,shutting-down` **为空** ⇒ 本波窗口内**无** `Server.SpotInstanceTermination`(W33 那个形状本轮未出现)。
  **六、局数(章程第 7 步)**:(a) **W47** 上轮已结清照登不重算 —— 四粒完整(原始 `4678` `ab28/ba14`、`4950` `ab33/ba23` + 按需补跑 `4723`/`4763`),被回收的两台原始局**不并入**(预登记处置),62-id 家族(W46+W47)达 8 粒;(b) **W48 发波型报告写预期局数**:预期 **~96 局计分**(4 粒 × 12 局 × 2 腿)外加暖场局(**暖场局不算有效局**),发波时点 **0 局**,按 W42 实测上传节奏**头 14 分钟 `OBJ_COUNT=0` 是预期值不是症状**。
  **七、`stayattr` 条件 (a) 的那一格已按总监 §ET 的指定写进 `W48_wave.json:harvest_obligations`**(总监明写它「属于一个 id 和一波,不是常驻义务,所以不进 `owed_executions.json`」)。判据逐字钉住:**「留没留下」不是「活没活下来」**;**⛔ 禁止拿胜负/存活率给它背书**(诚实边界 4);铁律 4(i-a) ab/ba 两层**读数**分别登记,计数类且侧偏未消除 ⇒ 4(i-b) 反号读噪声、4(ii) 不报中位数;域为空按 **DOMAIN-NOT-REACHED** 退回,**不许**读成「测过了无效应」。⭐ 离线域价钱已知(109 fixture / 1012 帧:95 帧触发否决,其中 **88 帧**伤害确实来自 3000 内)⇒ **杠杆按构造在约 93% 的域里惰性,波上域小是预期形状不是缺陷**。同记录另带三条随波限定:`campbind` 的共 armed 未登记合取(trunk 仍红)、`pulldrag` 与 `campbind` **不正交**(不许与 W45 及更早并池)、`zusboltdom` 的**反向刹车**((1) 归零必须同时给击杀落地数;(1) 零 + (2) 明显下降 = REJECT)。**并池主张预登记(反拟合,数据落地前写死)**:**W48 不与 W46/W47 并池** —— 臂串非逐字节相同(63 vs 62,md5 不同),按本台一贯为 W46+W47 并池背书的**同一条**臂串读法,这是新家族,W48 自成 4 粒。`winrate` 预登记:已连续**六**波有数据的波次打 `DEGENERATE`(W41/W42/W44/W45/W46/W47),**若 W48 第七次**,建议不再逐波复议而作为「该通道对本 harness 结构性无信息」交总监。
...
- **2026-09-06T06:14–06:5xZ(收割轮:W50 收完 4/4;而本轮头号产出是一笔不是本台花的钱)**
  **一、自检(铁律 10)**:⭐ **开工第一条命令又误用管道,脚本自拒**(`REFUSED: … stdout is a pipe; exit 2, nothing checked`,脚本自报「**同形已复发 5 次,每次都是当轮第一条命令**」)——那个 `EXIT=2` 是 REFUSED **不是通过**;改重定向 + 放后台后跑满约 40 分钟,**两次 push 之前跑完**。⚠️ **真码读脚本自打的那一行不读 harness 报的**(后台通知的 `exit code 0` 是末尾 `echo` 的码):**`selfcheck worst exit: 3`**,`legs run 10`,`FINDINGS (exit 3): cadence queue-rulings owed-executions`,**`UNCERTIFIABLE (exit 2): trunk-red(python)`**,`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。**3 = 有发现不是没跑成。** `cadence` 1 条 `GAP cadence strategy` **不是本台**;`owed-executions` 点名本台的仍是 `campgrade_wave_type_premise_reading`(#480/#241)**连续第三轮欠着**;⛔ **`trunk health (python)` 本轮 UNCERTIFIABLE ⇒「没跑成」不是「绿」,python 侧 trunk 红不红本台本轮答不了**(工具自己的修法:`re-run on a quiet tree`,归因大概率是本轮同时在写 `iterations/`);⭐ `trunk health (fast Lua detectors)` **`84 tagged detector file(s), 0 failures`** ⇒ Lua 侧绿,⚠️ 边界照抄 `FAST SUBSET, not the full suite`。
  **二、⭐⭐⭐ 成本:MTD 一夜跳 $19.7,而跳的那部分不是 dota2bot 的。** `COSTS_EXIT=0`,九月 MTD **`$39.328`**(免费 `budgets`,`refreshed 06:02:46Z`;因 ≥$35 自动付一次 CE 确认,逐位复现 `39.3278311289`),较上一轮(03:11Z)的 `$19.597` **+$19.7**,而这段时间本台只发了 W50 一波(登记 ~$1.02)。闸 (iii) 走工具不抄常数(`wave_fence.py --planned 1.02`,**`FENCE_EXIT=0`**):limit `$100.00`、actual `$39.328`、projected `$40.348`、三档 ACTUAL 告警 `$50/$80/$100` **全部 NOT yet crossed**、fence **`$50.00`**、brake `$90.00`、operative ceiling `$50.00`、**headroom `$9.652`**。⭐ **归因由两次付费 CE 读数钉死**:(a) DAILY×SERVICE —— 09-01~04 每日 `$4.1–5.8`,**09-05 `$20.742`(EC2 `$20.106`)**;(b) 09-05×USAGE_TYPE 拆开那 $20.1 —— **`USW2-SpotUsage:c7a.16xlarge $17.109 / 14.99 机时`**、`BoxUsage:c6i.4xlarge $2.081`、`SpotUsage:c6i.4xlarge $0.917`。**本台机型是 `c6i.4xlarge`(备选 `c6a.4xlarge`)从来不是 `c7a.16xlarge`** ⇒ 09-05 本台三波(W48/Y1/W49)= 那两条 c6i 行合计 **$2.998**,**`c7a` 那 $17.109 与本台无关**。⛔ **全账号实例普查(不只查本台 tag)读到此刻仍在跑的一台**:`i-0a7d4c7fbeb3a6712  r7a.16xlarge  spot  running  LaunchTime 2026-09-06T02:10:36Z`,tags **`Name=fft-cache-v1-repair` / `Project=final-table-trainer`**;现货报价(06:00Z)**$1.82–2.79/hr**,已跑约 4.2h。**算术推论**:headroom `$9.652` ÷ ~$2/hr ≈ **4.8h ⇒ 约 11:30Z 撞 $50**,而 $50 是最低那档 ACTUAL 告警 ⇒ **owner 的第一封预算邮件抬头会是 `dota2bot-batch`,而花钱的是别的项目**;`budgets` 自报 **forecast `$179.873` vs limit `$100`**。**这正是 GH #515 的预判,本轮它从预判变成读数,已追评。** ⛔ **本台没有动那台机器也不打算动**:它带别的项目的 tag、不在本台自毁体系里,**终止它是破坏性且越权的**,归 owner;章程「还在跑就该终止」那条的对象是**批测实例**不是账号上任何一台机器——**这条边界本轮特意写下来,免得下一轮照字面去关**。**已推手机通知 owner。**
  **三、闸 (i):本轮不该发波,而这是工具说的。** `wave_throttle.py` 自打:`WAVE_THROTTLE: THROTTLED (exit 3) -- do not launch. Wait 11045s.`,锚点 W50 末台 `2026-09-06T03:20:34Z`,解锁 **`2026-09-06T09:20:34Z`**。⚠️ **真码是 3**:第一次调用误用 `| tail` 得 `THROTTLE_EXIT=0` —— **那是 tail 的码不是工具的码**(evidence-discipline 3,**本轮第二次踩同一个坑**,第一次是自检的管道自拒);去掉管道重跑得 **`THROTTLE_EXIT_TRUE=3`**。⇒ 本轮是**收割轮**。P4.1 标尺波无冲突:Y1 已于 09-05 15:16Z 发出、18:0xZ 收割,那一棒已交。
  **四、⭐ 收割 W50(4/4 干净),记录全文 `W50_wave.json:harvest`。** 下机状态:四台 SIR `Status.Code` **全为 `instance-terminated-by-user`**(GH #408 判据是 SIR 的 `Status.Code` 不是 EC2 的 `StateReason.Code`)⇒ **自毁,零抢占**。输入普查(GH #225):逐 run 下载进**四个独立子目录**不 flatten,并把 S3 侧 `s3 ls --recursive | grep -c analysis.json` **逐 run** 与本地对读 —— **48/48、48/48、51/51、64/64 ⇒ 211 对象 / 211 文件,零丢失**;`RV_EXIT=0` 无 R1/R2/R3 拒绝;`input={files_seen 211, games_loaded 211, source_dirs 4, unparseable 0}`;每 run 恰好一粒种子(295740→5813、e56ba0→5830、f2182d→6072、eff51c→6128,与 `machines[]` 对上),每 run 6 局暖场(共 24)**不计分**。臂串复核**不抄 prose**:`git show b73358bc:…test_set.md | sed -n 2p | tr -d '\n'` ⇒ **59 / 524 / `572b60753699bc68b2ad5ec9020f2512`** 与记录逐字节相同;⭐ **发波树 `b73358bc` 不 deepen 就可见(`TREE_PRESENT`)⇒ GH #520 的浅克隆坑本轮没咬到**。**读数**:gpm **+13.76**(comps **2/4**)、xpm +2.46(1/4)、deaths +0.07(1/4)、last_hits +1.13(2/4);`scored_games` **187**、`unfinished` **0**、`min_arm_depth` 8、**`thin_arm_seeds` 空**、无排除种子(最浅 arm_depth **17.14**,门槛两倍多);逐粒 arm_depth 17.14/19.29/18.49/23.17。⚠️ **离散度是头号保留意见**:逐粒 gpm **−39.01 / −26.67 / +1.08 / +119.64** ⇒ **+13.76 由单粒(6072)扛着,去掉它剩三粒均值 −21.53**;按 4(i-c) 说精度的是 arm 自己的跨种子离散度,**本台登记均值但不为它背书**。`suggested=hold_or_reject`(工具只读 gpm:+13.76 过了 >5 那一半,**comps 2/4 没过「多数」那一半**)——**读数不是裁定**,promote/reject 归总监。**铁律 4(i-a) 两个分层的读数**(全取自工具字段,**未手算、未按局加权**):gpm `ab −131.61 / ba +159.12 / side −145.37`、xpm `−85.48 / +90.41 / −87.94`、deaths `+1.03 / −0.90 / +0.96`、last_hits `−3.85 / +6.12 / −4.99`,**四个量全 `sign_flip=true`**;按 4(i-c) 这四个都是已消侧偏的估计量 ⇒ 反号是恒等式 `|side|>|arm|`,**登记不当判决读**。⭐ **交总监**:side 项**巨大且相对 W49 反了向**(W49 gpm side **+46.37** → W50 **−145.37**),而本波 ab:ba = **131:56** ⇒ 按局加权会把 `0.40 × (−145.37)` 直接灌进答案,**这是 4(i-d) 的现场演示**。**winrate 第九次 DEGENERATE**:`channel DEGENERATE`、`headroom 0.0096`、`side_census {radiant 1, dire 210}`、`minority_share 0.0047`(**至今最差**,W49 是 0.0208)⇒ 按数据落地前的预登记**不写成读数、不逐波复议**;⭐ **但有新东西**:`winner_by = {engine_natural: 211}`、`winrate_independent_of_gold = 211/211 games` ⇒ **本波每局都真的推掉了遗迹,econ_winner 改写一次都没触发,通道完全独立于金钱而仍然没有信号** —— **这是 W49 交给总监那条「结构性无信息」主张的最强形态**。
  **五、⭐ 闸 (iv) 先报 UNDECIDABLE,根因是发波轮没记 survival 通道。** `reclaim_blind.py --wave-json W50_wave.json` 第一次 **`RB_EXIT=2`**:`UNDECIDABLE: seed 5813: needs survival_min, or both create and update` —— **W50 的 `machines[]` 那四个字段全 null,发波轮也没写 `survival_min`**(W48/W49 写了 `survival_min`,W47 写的是 `sir`+`create`)。处置:**第一手**从 SIR 行读回 `CreateTime`/`Status.UpdateTime`(工具第 103 行文档的形状)补进 `machines[].sir/create/update`,重跑 ⇒ **`RB_EXIT=0`**:`VERDICT: not blinded -- the wave delivered 4 paired seed(s). NEXT WAVE: spot (GH #158 unchanged).` ⭐ **顺带买到一个没量过的数**:四台**都没跑满 2h 看门狗**,而是干完活自毁 —— 存活 **44.98 / 47.12 / 55.15 / 56.73 分钟** ⇒ 真实机时约 **3.4** 而非 8,账单侧 09-05 `c6i` spot `$0.917 / 3.27 机时` 正是同一形状,**本波落在 ~$1.02 常数的便宜一侧**。⚠️ **这是会复发的洞不是一次性失误**:闸 (iv) 要的 `update` **只有终止后才存在**而 SIR 行不会永远留着,`sir`+`create` 却**发波当轮就能写**(W47 写过)——一旦某轮收割赶在 SIR 行消失之后,闸 (iv) **永久 UNDECIDABLE**,而它决定的是**下一波的市场类型** ⇒ 把市场决策弄瞎。**与 GH #544 把 `launched_at` 挪到发波轮写死是同一缺陷族**,已开 issue。
  **六、局数(章程第 7 步)**:W50 结清照登不重算 —— 四粒完整(`ab/ba` = 30/12、27/15、32/13、42/16),**187 局计分 / 0 未完成 / 24 暖场不计**。**59-id 家族现有 4 粒**,按数据落地前的预登记 **W50 与 W46+W47、W48、W49 都不并池**(四个 md5 各不相同)。预期局数照上一轮告诫处理:登记 ~96、**实交 187** ⇒ **继续印证「~96 偏低约一倍,不得用作『交付不足』的分母」**(W49 实交 168)。
  **交棒**:① ⭐⭐⭐ **owner —— `i-0a7d4c7fbeb3a6712`(`final-table-trainer`)要不要继续跑,`dota2bot-batch` 预算要不要按项目拆开**;不拆的话本台会**在别人的花费上**被闸 (iii) 挡住发波,而 owner 的告警邮件指向错误的项目。**本台不动它。** ② ⭐⭐⭐ **总监 —— 59-id 家族 W50 的 promote/reject**:gpm +13.76 / comps 2/4,**且均值由单粒 6072 扛着**(去掉后 −21.53),**请连离散度一起裁不要只读均值**。③ ⭐⭐ **总监 —— winrate 通道第九次 DEGENERATE 而本波是 `engine_natural 211/211`** ⇒「结构性无信息」现在有了**独立于金钱**的证据,该结案。④ ⭐⭐ **录像组 —— W50 的 `rotscope` 条件 (a) 只有 5813/5830 两粒带 pudge(W48 的一半),不许把更小的计数读成更小的效应**;`zusboltdom` 反向刹车(zusult 触发数**必须**与击杀落地数同时给)**经济裁定答不了**。⑤ ⭐ **本台 —— `campgrade` 独占波仍不存在(#480/#241),连续第三轮欠着**(自检 `owed-executions` 本轮再次点名)。⑥ ⭐ **总监 —— `$0.90` 单波成本常数的自动失效条款仍未重裁**;本轮新数据点:W50 真实机时 3.4,落在常数便宜一侧。
  **下一轮本台** = ① 闸 (i) 解锁 **`2026-09-06T09:20:34Z`**,但**不要读这个数,读 `wave_throttle.py` 打的那一行**;② ⛔ **发波前必查 headroom 是不是已被 `final-table-trainer` 吃掉** —— 按本轮算术它约 11:30Z 撞 $50,**那时的 `THROTTLED`/围栏拒绝不是本台超支**,不许当成本台的账;③ **发波当轮就把 `machines[].sir` 与 `create` 写进波次记录**(第五节的洞);④ 开工第一条命令**重定向不管道、放后台给足 ≥45min** —— 管道那一坑本轮又踩了两次。
  **本轮 token**:见报告最后一节。
  详见 `iterations/reports/batch-desk/20260906T061400Z.md` 与 `iterations/reports/batch-desk/waves/W50_wave.json`。
  **收尾补记(06:5xZ)**:**本轮开出/追评** —— **GH #515 追评**(comment `5557449660`,第二节全文:两次付费 CE 读数、机型对照、仍在跑的 `i-0a7d4c7fbeb3a6712`)、**GH #556 新开 `[batch]`**(第五节:闸 (iv) 依赖一个会过期的 SIR 窗口,`sir`+`create` 必须发波当轮写死,与 #544 同族;验收 = W51 发波轮就带这两个字段且收割轮闸 (iv) 第一次调用即 `RB_EXIT=0`)。**GH #290 顺序照做**:两份草稿都**先 push 再发表**,发表前各跑一次 `claim_precheck.sh` —— #515 `PRECHECK515_EXIT=0`(`NO CITATIONS -- nothing to certify, nothing to strand`),#556 `PRECHECK_SIR_EXIT=0`(`paths cited 1 / resolved on trunk 1 / refused 0`,`clean`)。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门 `ARM_HOOK_EXIT=0`;**`PUSH1_EXIT=0`**(`claude/great-sagan-vjj2dm`,新分支)/ **`PUSH2_EXIT=0`**(`ef0ea24c..69ec1670  HEAD -> main`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。`bots/`/`game/`/`tests/` **本会话一行未改**。
  **收尾泄漏检查**:**`LEAK_EXIT=0`** —— `Name=soak-run*` 且 `running,pending` **为空**,与四台 SIR 全 `instance-terminated-by-user` 一致 ⇒ **dota2bot 侧无泄漏、无掉臂**;常驻仍只有 AMI `ami-0a990a26d89c66547`。⚠️ **全账号仍有一台在跑** `i-0a7d4c7fbeb3a6712`(`Project=final-table-trainer`),**不是本台的,本台未动**——收割轮要的是「本台零台在跑」,而这一台不在「本台」的量程里,**这条区分本轮第一次真的承重**。
  **本轮 token**:`TOKENS total_in=6,724,760 out=51,288 turns=53`(到统计时刻为止)。
- **2026-09-06T09:12–09:4xZ(不发波轮:闸 (iii) 打 CLEAR,而诚实重建说 $50 已经越线)**
  **一、自检(铁律 10)**:⭐ **开工第一条命令第 6 次误用管道**,脚本自拒(`REFUSED: … stdout is a pipe; exit 2, nothing checked`),脚本自己点名「同形已复发 5 次,每次都是当轮第一条命令」——本轮是第 6 次。**那个 `EXIT=2` 是 REFUSED 不是通过。** 改重定向重跑,⛔ **然后被本台自己的 `timeout 600` 砍断:`EXIT=124`** ⇒ **本轮自检没跑完,没有 `selfcheck worst exit` 那一行,「没跑完」不是「通过」**。⚠️ 上一轮交棒写的是「重定向不管道、给足 ≥45min」,**本轮两半都犯了**(管道 + 只给 10min)——下一轮两半都要照办。**跑完并读到的腿**:admission-wait 无过期;`stable-v1/v2` 两个锚点 OK;**`pullcad` 陷阱 clean**(157 个活 gate、13 个带 PROMOTED 注记,`FROZEN none`,3 个 promote-atom 全 ok);**python trunk `102 passed, 0 failed, 2 uncertifiable`** —— ⚠️ 两个 UNCERTIFIABLE 照抄边界:`tests/test_rc_wrapper.py` / `tests/test_selfcheck_lua_leg.py` **没跑**,原因逐字都是 **`no lua5.1 on PATH`**(GH #171 同族);`cadence` **1 条 `GAP cadence strategy`,不是本台**;`owed-executions` 点名本台的仍是 `campgrade_wave_type_premise_reading`(#480/#241)**第四轮**;`5a0` 腿 `84 file(s) ... in 120.1s (budget 120s)` 超预算 ⇒ 连带 4 条 `UNC`。⛔ **没跑到的腿:`trunk health (fast Lua detectors)` 与汇总横幅** ⇒ **Lua 侧 trunk 本轮红不红,本台答不了。**
  **二、⭐⭐⭐ 本轮头号产出:闸 (iii) 的 `CLEAR` 是假的,而假在一个工具按构造读不到的量上。** `wave_fence.py --planned 1.10` 打 `FENCE_EXIT=0 / CLEAR / headroom $9.572`,输入是 `budgets` 的 **`$39.328`**(`refreshed 2026-09-06T06:02:46Z`,三小时前)。⭐ **CE 逐日 × SERVICE 实测:`2026-09-06` 整日无任何分组行** ⇒ `$39.328` 实质是「截至 09-05 收盘」。把今天补回去(**实测机时 × 实测同 AZ 现货价**,两台都在 us-west-2d,价格 09-05T20:00Z 起全窗口平 `$1.8209/h`):`i-0a7d4c7fbeb3a6712` 02:10:36Z→**08:23:29Z**(`User initiated`)= **6.2147h** = `$11.316`;`i-0114b249d00ad956e` **08:24:02Z**→仍在跑 = 0.7567h = `$1.378`(**上一台被终止 33 秒后就起了下一台 —— 修复重试循环,不是一次性作业**);本台 W50 章程常数 `$1.10`。⇒ 未入账 **`$13.794`**,**真实 MTD ≈ `$53.12`** ⇒ **`$50` 档已越线约 `$3.12`,而越线的那 `$12.69` 没有一分是批测的**(本台占未入账的 8.0%)。⇒ **按闸 (iii) 自己的算术,诚实输入下本轮是 THROTTLED。** 往前看:外部那台 `$1.8209/h` = **`$43.7/天`**,即使本台一波不发,账号约一天多撞 `$100`;`budgets` forecast **`$179.873` vs limit `$100`** 与此一致。
  **三、⇒ 本轮不发波,决定性理由是闸 (iii) 不是闸 (i)。** `wave_throttle.py` 也打 `THROTTLE_EXIT=3`(`margin 668s SHORT`,解锁 `2026-09-06T09:20:34Z`,anchor = W50 末台 `03:20:34Z`,slate spread 13s)。⚠️ **登记清楚:等 11 分钟不是本轮的结论** —— 09:20:34Z 之后闸 (i) 会解锁,而那时闸 (iii) 只会更红(外部那台又多跑 11 分钟)。
  **四、收割:本轮无新数据。** S3 `validation/` 最新对象仍是 W50 自己那四份(`l1trade+59ids-bfd72fe79ac8_20260906_0321*`,落盘 04:02–04:14Z),**已由 06:14Z 那轮全量收割完毕**(4/4 干净、187 局计分、gpm +13.76、comps 2/4)。⇒ 零新 verdict,不重复收割。
  **五、泄漏检查:dota2bot 侧干净(范围按 GH #515 订正后的写法)。** `check_costs.sh` 实例段空 —— ⚠️ 那是**带 `tag:Name=dota2bot-*` 过滤器**的空;**不带过滤器的全账号普查**(本台自 #515 起每轮做):region 内 `running/pending` **恰好一台**,`i-0114b249d00ad956e` / `Project=final-table-trainer`,**不是本台的**。⛔ **未做任何处置也不打算做**(终止别人项目的算力破坏性且越权,归 owner)。本台本轮一台未起,W50 四台已于 04:14Z 前全部自终。
  **交棒**:① ⭐⭐⭐ **owner(从「预判」升级为「现在」)** —— GH #515 的两个后果本轮双双兑现:(甲) 批测发波闸被别人的算力关上(**本轮就是**);(乙) `$50` 告警即将以 `dota2bot-batch` 抬头发出,而其中约 **92%** 不是批测的。二选一、都不改本台工具:给 `dota2bot-batch` 加 cost filter(⚠️ 走 `Project` 标签路线**必须先给批测机器补 `Project` 标签**,否则 fence 恒读 0 —— **一个永远打 CLEAR 的闸比现在更坏**),或给 `final-table-trainer` 单开预算。② ⭐⭐ **总监** —— 闸 (iii) 该不该带「今天未入账机时」补正项:本轮工具打 `CLEAR` 而正确答案是 `THROTTLED`。本台按铁纪律(甲)的**理由**(字面只覆盖「本台自己近 12h 的波」)外推并据此不发波,**属自裁,请确认或推翻**;若确认,建议 `wave_fence.py` 把「全账号 running 实例 × 同 AZ 实测现货价 × 已跑机时」计入 `pending waves`(它现在恒 `$0.000`,**本轮就是它该非零的那一轮**)。③ ⭐ **本台连续第四轮欠 `campgrade` 独占波(#480/#241)**,但**本轮欠的理由第一次不是遗忘是钱**。④ ⭐⭐ **总监(承上轮未结)** —— W50 的 promote/reject(gpm +13.76 / comps 2/4,**均值由单粒 6072 扛着,去掉后 −21.53**);winrate 第九次 DEGENERATE 的结案。⑤ ⭐ **总监** —— `$1.10` 常数(GH #454)新数据点:W50 实测 3.4 机时 ⇒ 账单侧约 `$0.82`,**落在便宜一侧**;(乙2) 机时判据本轮**未触发**(0.85 h/台 < 1.00)。
  **下一轮本台** = ① **先跑 `check_costs.sh` 看 `budget refreshed` 有没有翻过 09-06** —— 翻过了就**用工具的数不用本轮的重建**;没翻过就**照本轮的方法重建一遍再判**(全账号 `describe-instances` + `describe-spot-price-history` 同 AZ)。② 重算后仍在 `$50` 内、或 §交棒① 落地(预算拆开)⇒ **第一件事发 `campgrade` 独占波**(#480/#241,已欠四轮)。③ 仍越线 ⇒ **继续不发波**,只做收割/泄漏/交棒,不要因为「工具说 CLEAR」就发。④ 发波前最后一条命令仍是 `wave_throttle.py`(**动作时点跑,不是决定时点**)。
  **铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只写 `iterations/` 下的报告与本节);动态半(#124)**未跑不声称**;**铁律 11** 无 `requires approval`、无空转等待。CE 付费调用一次(`$0.01`,章程允许的「确认贴近围栏的读数」),**而且正是它证明了 09-06 无数据**。
  **收尾读数**:`PUSH1_EXIT=0` / `PUSH2_EXIT=0`(`ac10e95a..fb2bb3ef HEAD -> main`),静态门两次都自打 **`GATE_EXIT=0 CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;发表前置 `PRECHECK_EXIT=0`(`local commits not on origin/main: 0`,`paths cited 1 / resolved 1 / refused 0`,`OK to publish`),**顺序照办:先 push `fb2bb3ef` 再发评论**;**本轮追评 GH #515 comment `5558273147`**,**未新开 issue**(本轮的事就是 #515 预判的那件事本身,另开会把同一条线拆散)。
  **本轮 token**:`TOKENS total_in=4,520,087 out=31,548 turns=43`(到统计时刻为止)。
  详见 `iterations/reports/batch-desk/20260906T091200Z.md`。
- **2026-09-06T12:12–12:4xZ(不发波轮:外来负载停了,而 `$50` 已经越线。⭐ 头号发现是闸 (iii) 的 `CLEAR` 这次被 `CERTIFIED` 背书了,而背书它的机制正是它的盲区)**
  **一、自检(铁律 10)**:⚠️ **开工的自检命令第 9 次误用管道**,脚本逐字自拒(`REFUSED: routine_selfcheck.sh stdout is a pipe; exit 2, nothing checked`,自陈「evidence discipline 3,已复发 5 次,每次都是当轮第一条命令」)——**那个 `EXIT=2` 是 REFUSED 不是通过**;⭐ 上一轮交棒 ④ 的**另一半照办了**(重定向 + 后台,**不设 `timeout`**,不重演上一轮的 `EXIT=124`),真码见收尾补记。⭐ **「工作树红 = main 红」的结构性论证本轮同样成立**:`git diff --stat origin/main -- bots/ game/ tests/ tools/` 为空(改动全在 `iterations/` 下)⇒ 不需要 `git stash`。
  **二、⭐⭐⭐ 成本:`$50` 已经越线,而越线的钱大头不是批测的。** `AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`,九月 MTD **`$39.328`**(免费 `budgets`,**`refreshed 2026-09-06T06:02:46Z`** —— ⚠️ **与上一轮逐位相同、刷新时点也相同** ⇒ 上一轮交棒 ① 落在「没翻过 09-06」那一侧,故按它指定的方法重建)。⭐⭐⭐ **头号发现**:裸跑 `wave_fence.py --planned 1.10` **`FENCE_TOOL_EXIT=0` `CLEAR` headroom `$9.572`**,且它把 `pending $0.000` 标成 **`accrual check : CERTIFIED (0 accruing instances account-wide, read this run) -- pending $0.000 is a reading, not a default.`** —— **这一行为真而结论为假**:Ruling 4 证「零」的办法是读**此刻还在 accruing 的实例**,而今天已花未落账的 `$15.345` **全部来自已经 `terminated` 的机器**(钱先发生再落账,实例先消失)⇒ **「零台在跑」与「零元未落账」是两个命题,工具把前者当后者的证据**。⛔ **这直接反驳上一轮交给总监的修法建议**(09:12Z 交棒 ②「把全账号 running 实例 × 现货价计进 pending」)——那条**在上一轮成立、在本轮失效**,因为本轮该计进去的钱**没有一分是 running 的**;正确的量是**本账期已发生但未落账的机时**,证据在 **SIR** 不在 `describe-instances`(⚠️ 诚实边界:SIR 行也会过期,与 GH #556 同族)。**重建(实测机时 × 实测同 AZ 现货价)**:全账号 SIR 恰 2 条、**都已 `closed`** —— `sir-rmaqhkvp`/`i-0a7d4c7fbeb3a6712` `02:10:35Z→08:25:24Z` **6.2469 h**、`sir-ctifj6bp`/`i-0114b249d00ad956e` `08:24:02Z→09:58:36Z` **1.5761 h**,两条 `Status.Code` 全为 `instance-terminated-by-user`;`r7a.16xlarge`/`us-west-2d` 现货价 `02:00Z`与`06:00Z` 两档均 **`$1.8209/h`**(`10:00Z` 起 `$1.8140`,**两台都在 09:58Z 前结束故全程用 `$1.8209`**)⇒ 外来 **`$11.3751 + $2.8699 = $14.2450`**,加本台 W50 常数 `$1.10` ⇒ **pending `$15.345`** ⇒ **诚实 MTD `$54.673`**。喂回同一个工具(**不手判,让闸自己说**):**`FENCE_HONEST_EXIT=3`** `projected $55.773` / `operative ceiling $50.00` / **`THROTTLED -- Headroom was $-4.673`**。**归因(付费 CE `$0.01`,MONTHLY × `INSTANCE_TYPE`)**:`c6i.4xlarge $18.5054`(**本台**)+ `c7a.16xlarge $17.1086`(**外来**)+ `NoInstanceType $3.7138` = **`$39.3278` 与 `budgets` 逐位相同 ⇒ 拆分完备**;加未落账的 09-06 ⇒ **本台 ≈ `$19.61`(36%)/ 外来 `final-table-trainer` ≈ `$31.35`(57%)/ 未拆 `$3.71`(7%)**。工具自打 `budget filters : none` ⇒ owner 第一封 `$50` 告警邮件**抬头仍是 `dota2bot-batch`**(根因 GH #515)。⭐ **好消息也登记:外来那条作业本轮停了** —— 两台 `r7a` 都 `closed`(末台 `09:58:36Z`),账号内**零实例** ⇒ 外来燃烧率现在是 **`$0/h`**,上一轮「一天多撞 `$100`」的前提不再成立;⚠️ **但 `$50` 是累计门槛不是速率门槛,已越的 `$4.67` 不会退回来**,且 09-06 那两台是「上一台终止 33 秒后起下一台」的重试循环,**本轮只是没有第三台**。
  **三、闸 (i):UNLOCKED ⇒ 本轮的阻塞确实只是钱。** `wave_throttle.py`(**不带管道**)**`THROTTLE_EXIT=0`**:锚点 W50 末台 `2026-09-06T03:20:34Z` ⇒ `unlock 09:20:34Z`,`now 12:16:26Z`,**`margin +10552s past unlock`**,`UNLOCKED (exit 0)`(Y1 那条候选也打了,按 ruling 3 取最晚者)。⇒ ⭐ **与上两轮不同,节奏闸本轮不挡了**:不发波**只因为闸 (iii)**,即 **`campgrade` 独占波(#480/#241)连续第五轮欠着,而欠的理由连续第二轮是钱不是遗忘**。
  **四、收割:无新数据(查过的,不是默认的)。** `soak/` 最新四个前缀就是 W50 那四台(`spot_20260906_0320*_b73358bc…_{295740,e56ba0,f2182d,eff51c}`),**已由 06:14Z 那轮 4/4 收完**(187 局计分、gpm `+13.76`、comps 2/4);`validation/` 最新对象是 07-23 的历史物。本轮零实例、零新 run ⇒ **无收割义务**。
  **五、泄漏:本台干净,而本轮的证据强度比往轮高一档。** 全账号、正确区域、**不带过滤器**普查(GH #515 订正后的写法):`running,pending` ⇒ 空;⭐ **再放宽到全状态** ⇒ **`Reservations` 0 条,账号内一台实例都没有**。⚠️ **空输出的处置**:先用「全状态」这条**更宽的查询**验证空不是查询坏了,再采信(防 09-05「过滤器决定了问题」与 09-06「区域决定了问题」两次同族缺陷)。常驻仍只有 AMI `ami-0a990a26d89c66547`。⇒ 本台无泄漏、W50 四台早已自终;⇒ **外来那台也确实不在了** —— ⚠️ 这一次的「已不在」有 SIR 的 `closed` + `Status.UpdateTime` 撑着,**不是** 09-05 那次靠错过滤器读出来的。⛔ 本台对外来资源**未做任何处置**(它们已自行终止;即便还在跑,终止别项目的算力破坏性且越权,归 owner)。
  **六、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(只写 `iterations/` 下的报告与本节);**铁律 11** 无 `requires approval`、**无空转等待**(唯一的「等」是自检在后台跑,本轮没为它停下);CE 付费调用 **2 次(`$0.02`)** —— 一次是 `check_costs.sh` 因 ≥`$35` 自动复核,一次是按机型归因,**而 owner 的决策正压在归因上**;动态半(#124)**未跑不声称**。
  **交棒**:① ⭐⭐⭐ **owner —— `$50` 已越线(诚实 `$54.673`),57% 不是批测的**;两条路都不改本台工具:给 `dota2bot-batch` 加 cost filter(⚠️ 走 `Project` 标签路线**必须先给批测机器补 `Project` 标签**,否则 fence 恒读 0,**一个永远打 CLEAR 的闸比现在更坏**),或给 `final-table-trainer` 单开预算;⭐ 新事实:外来负载**已停** ⇒ 决策不再紧急,**但已越的线不会自己退回**。**已推手机通知。** ② ⭐⭐⭐ **总监 —— `wave_fence.py` 的 accrual check 有结构性盲区,且它上一轮拿到的修法建议堵不住**(见第二节),本轮开 issue。③ ⭐⭐ **总监(承前两轮未结)** —— W50 促/退(gpm `+13.76` / comps 2/4,**均值由单粒 `6072` 扛着,去掉后 `−21.53`**,请连离散度一起裁);winrate 第九次 `DEGENERATE`(`engine_natural 211/211`,**独立于金钱**)该结案。④ ⭐⭐ **本台 —— `campgrade` 独占波连续第五轮欠着**,⭐ 本轮闸 (i) 已 UNLOCKED ⇒ 唯一阻塞是闸 (iii)。⑤ ⭐ **总监 —— `$1.10` 常数(GH #454)本轮无新数据点**(未发波),(乙2) 机时判据未触发。
  **下一轮本台** = ① **先看 `budget refreshed` 有没有翻过 09-06** —— 翻过用工具的数,没翻过照第 2.3 节重建,⚠️ **重建查的是 SIR 不是 running 实例**;② 仍 > `$50` 且 owner 未拆预算/未批准 ⇒ **继续不发波,不要因为工具打 `CLEAR` 就发**(本轮它就打了 `CLEAR`);③ 落回 `$50` 内或交棒 ① 落地 ⇒ **第一件事发 `campgrade` 独占波**,发波当轮写齐 `machines[].sir`/`create`/`launched_at`(GH #556/#544)并在写完记录**之后**复跑两条检测器;④ 发波前**最后一条命令**仍是 `wave_throttle.py`(**动作时点**);⑤ ⚠️ 开工第一条命令**重定向不管道、放后台给足 ≥45min**(管道本轮第 9 次)。
  **本轮 token**:见报告第九节。
  详见 `iterations/reports/batch-desk/20260906T121200Z.md`。
  **收尾补记(自检真码)**:⚠️ **后台完成通知说 `exit code 0`,那是末尾 `echo` 的码不是自检的码**;脚本自打 **`selfcheck worst exit: 3`**,`legs run 10`,**`FINDINGS (exit 3): cadence queue-rulings owed-executions`**,**`UNCERTIFIABLE (exit 2): trunk-red(python)`**,`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。**3 = 有发现不是没跑成**,十条腿都真跑过。⛔ **`trunk-red(python)` UNCERTIFIABLE ⇒ python 侧 trunk 红不红本台本轮答不了**(腿自陈 `NOT a pass, and NOT evidence that trunk is red`;根因 `5a0` 腿 `84 file(s) in 120.1s (budget 120s)` 超预算连带 4 条 `UNC` 空过,工具自己写明「墙钟分不开集合变大与容器变慢,要跨轮读 NOTE 行」)。⭐ **Lua 侧绿**:`84 tagged detector file(s), 0 failures`,⚠️ 边界照抄 **`FAST SUBSET, not the full suite`**。两条 `NOT RUN` 的理由逐字是 `no lua5.1 on PATH`(GH #171 同族,时序产物)。`cadence` **2 条 `GAP cadence strategy` 不是本台**;`owed-executions` registry 6 行,点名本台的仍是 **`campgrade_wave_type_premise_reading`(#480/#241)第五轮**;`queue-rulings` **`RIDESHARE (§BB.4: rule this round): 1`** ——⚠️ 本台不裁,但**连续第三轮**指出 owner 的 **P4.2 已废止并压过 §BB.4**(冻结期唯一合法裁定是 `FROZEN-HOLD`)⇒ **工具还在引一条已废止的条款**,交总监。⭐ 按第一节的结构性论证,这些「工作树」读数**本轮就是 `main` 的读数**。
  **铁律 6 读数**:两次 `git push` **各自**跑同一个 gate 并各自自打 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),`ARM_HOOK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;**`PUSH1_EXIT=0`**(`claude/great-sagan-awlwnp`,新分支)/ **`PUSH2_EXIT=0`**(`0244fe68..25bc149f  HEAD -> main`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。
  **发表(GH #290 顺序:先 push 再发)**:两处都在 `25bc149f` push 之后,发表前 **`PRECHECK_EXIT=0`**(`local commits not on origin/main: 0`,`paths cited 2 / resolved on trunk 2 / refused 0`,`OK to publish`)。**GH #565 新开 `[harness]`**(accrual check 把「零台在跑」证成「零元未落账」,**并说明上一轮交给总监的修法建议本轮堵不住**;验收 = 下轮 `budget refreshed` 未翻天时工具应自己计入当天已终止实例的机时,取不到证据应打 `UNCERTIFIABLE` 而不是 `CERTIFIED`)、**GH #515 追评**(comment `5559180518`)。
  **收尾泄漏复检**:**`LEAK_EXIT=0`** —— 本轮未发波,`describe-instances` **全状态 0 条 Reservation**(既无本台泄漏,也无外来机在跑),常驻仍只有 AMI `ami-0a990a26d89c66547`。⚠️ **这是 09-05 以来第一次连账号侧也是零。**
  **本轮 token**:`TOKENS total_in=5,565,112 out=36,026 turns=45`(到统计时刻为止)。
- **2026-09-06T15:20–15:5xZ(不发波轮:`$50` 由**实测**越线。⭐⭐⭐ 头号发现是同一份滞后把**围栏冻在一档太低** —— 同一个 `projected total $55.773` 上,陈操作数打 `THROTTLED`、CE 实测操作数打 `CLEAR`,翻转全在「哪一档算已跨」)**
  **一、自检(铁律 10)**:⚠️ **管道那一坑第 10 次踩**(开工首条命令仍带 `| tail`,脚本逐字自拒 `REFUSED: ... stdout is a pipe; exit 2, nothing checked`);⛔ **改重定向重跑那次也没交出判决** —— 上一轮交棒 ⑤ 写的是「后台给足 ≥45min」,本台给了 `timeout 900`,日志断在 `=== trunk health (fast Lua detectors) ===`,**没有 `selfcheck worst exit` 行 ⇒ 那一跑 UNCERTIFIABLE 不是通过**(harness 通知的 `exit code 0` 是末尾 `echo` 的码)。已 `nohup` 无超时重跑,真码见收尾补记。断点前完成的腿照登:`cadence` 2 条 **`GAP cadence strategy`(不是本台)**;`owed-executions` 9 行,点名本台的仍是 **`campgrade_wave_type_premise_reading`(GH #480/#241)—— 连续第六轮欠着**,欠的理由连续第三轮是**钱**;⭐ `queue-rulings` 的 `RIDESHARE` 行**已改成引 owner 的 P4.2**,本台连点三轮的规则冲突**已修,停止上报**;`5a0` 的 `84 file(s)` 与上一轮**逐位相同 ⇒ 动的不是集合**。
  **二、⭐⭐⭐ 成本:`$50` 不再是重建出来的,是量到的。** `AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`。两个来源**本轮第一次分岔**:`budgets` **`$39.328`**(`refreshed 2026-09-06T06:02:46Z`,**与 09:12Z / 12:12Z 三轮逐位相同** ⇒ 滞后量到 **≥9.3h**),而 ≥`$35` 自动付费复核的 CE 打 **`$50.8036495163`**(上一轮两者还是逐位相同的 `$39.3278`)⇒ **`$50` 档已由账单侧读数越线**。按机型归因(付费 CE 一次):`c6i $20.0961`(本台)/ `c7a $23.2805` + `r7a $3.3186`(外来 `final-table-trainer`)/ `NoInstanceType $4.1084`,**合计逐位等于 `$50.8036` ⇒ 拆分完备**;⭐ **没有 `c6a` 行 ⇒ 整个九月没有一波走到过容量降级阶梯第 (2) 步**。⭐⭐ **上一轮的 SIR 重建法:方向被证实,数值被证明偏低** —— 它宣称 `$15.345` 未落账,CE 本轮正好多出 **`$11.476`**(**第一次被更晚的 CE 复核**);但它把 `c7a $17.109` 当已结清,而那一行已长到 **`$23.281`(+$6.172)**,`r7a` 重建 `$14.245` 对 CE 的 `$3.319` **仍差 `$10.926`** ⇒ **诚实 MTD ≥ `$61.73`(下界,不是读数)**,上一轮那个 `$54.673` **至少偏低 `$7`**;诚实归因 **外来 ≈ `$37.53`(61%)/ 本台 ≈ `$20.10`(33%)**。
  **三、⭐⭐⭐ 头号发现:同一个滞后把围栏冻在一档太低,判决因此翻转。** 活跑 `FENCE_LIVE_EXIT=3`:`actual $39.328` / `pending $15.345` / `planned $1.100` ⇒ `projected $55.773`,`$50.00 NOT yet crossed`,`fence $50.00`,`operative ceiling $50.00`,**`THROTTLED (exit 3) -- Headroom was $-4.673`**;把 CE 实测喂进同一个工具(离线)`FENCE_CE_EXIT=0`:`actual $50.804` / `pending $3.869` ⇒ **`projected $55.773` 逐位相同**,但 `$50.00 crossed`,`fence $80.00`,**`CLEAR (exit 0)`,`headroom $24.227`**。⭐ **立案句:翻转 100% 在「哪一档算已跨」上,不在投影上** —— `wave_fence.py` 的立案故事(GH #504)是「导出值被写成字面量」,本轮它每轮都重导了,**但导出所用的输入落后现实 ≥9.3h**。⭐⭐ **两个失效方向相反**:陈 `actual` 让投影偏低 ⇒ **偏放行**(= GH #565 那条盲区),让已跨集合偏小 ⇒ **偏拦截**;**本轮第二个压过第一个**。⚠️ **有界不是死锁**:九月已花的钱不会变少,`budgets` 一刷新过 `$50` 围栏自动变 `$80`,本条自解 —— 但时长由一次没人控制的 AWS 刷新决定,已跨三轮 ≥9.3h。⛔ **决定:不发波,两条独立理由** —— ① 活工具 `exit 3`(「**0 才可发**」,工具自己也写 `a launch must use the AWS read`,离线那次**是审计不是许可**;并已照抄它自打的 `SKIPPED, NOT CERTIFIED` 与 `OFFLINE` 两行);② 即便按 CLEAR 读,**跨 owner 可见告警档需要总监当轮明确裁定**,本轮没有。⭐ **不援引上一轮的先例**:上一轮推翻的是**绿**工具(往少花钱的方向,总监事后裁「你是对的」),本轮推翻**红**工具是往花钱的方向,**不是同一种动作**。
  **四、闸 (i):UNLOCKED ⇒ 本轮阻塞确实只是钱。** `THROTTLE_EXIT=0`:锚点 W50 末台 `03:20:34Z` ⇒ `unlock 09:20:34Z`,`now 15:20:09Z`,**`margin +21575s past unlock`**,`slate spread 13s`。
  **五、收割:无新数据(查过的,不是默认的)。** `soak/` 最新四个前缀就是 W50 那四台,**已由 06:14Z 那轮 4/4 收完**;`validation/` 最新仍是 **07-23** 的历史物。**队列 76 条 open 里,需要新波次且带可发裁定的 0 条**(绝大多数是零 EC2 的 `ROUTED_ARCHIVE_SCAN`/`APPROVED-SCAN`,`strategy-31/32` 是搭车)⇒ **本轮无被钱耽误的显式波次请求**。
  **六、泄漏 + ⭐ 一条新窗口读数。** 全账号、正确区域、不带过滤器:`describe-instances` **全状态 `Reservations` 0 条**,`soak-run` 标签空(`RC1_EXIT=0`),常驻仍只有 AMI `ami-0a990a26d89c66547` ⇒ **本台无泄漏,外来机也确实不在**。⭐⭐ **上一轮那两条 SIR 本轮返回 0 行 ⇒ SIR 窗口 ≲3h**(与 GH #375 记的「terminated 实例约留 1h」不是同一张表)。⛔ **后果是可证性不是不便**:上一轮那个 `$14.245` **已无法从原始证据复算**,只能靠报告里抄下的数字活着,**而第二节刚证明那份重建偏低** ⇒ **GH #565 建议的「按 SIR 补回已终止机时」必须在花钱那一轮当场跑**,取不到时应打 `UNCERTIFIABLE` 而非 `CERTIFIED`。
  **七、局数(第 7 步)**:(a) **W50** 上轮已结清照登不重算 —— 四粒全配对零抢占(`5813 ab30/ba12`、`5830 ab27/ba15`、`6072 ab32/ba13`、`6128 ab42/ba16`),`scored 187`、gpm **`+13.76`**、comps 2/4,⚠️ 均值由单粒 `6072` 扛着(去掉后 `−21.53`),winrate 第九次 `DEGENERATE`(`engine_natural 211/211`)按 §CT **不写成读数**;(b) 本轮**无在跑波次**,零实例零新局。
  **八、⭐ `$1.10` 常数(GH #454):一个自上而下的新校验点,两条腿都没触发。** 九月 `c6i` 账单 **`$20.0961`** ÷(**18 个四机波 W33–W50 + 4 台登记补跑机 + Y1 单机**)⇒ **≥`$1.058`/波**。⭐ 这条**不依赖任何逐波重建**(既有方法是每波「机时 × 现货价」),两条路给出的数互相靠得很近,而 `$1.058` 是**下界**(尾巴还在落账)。**(乙1) 未触发**:⛔ 本台**拒绝**把 `c6i` 增量 `$18.505→$20.096`(+`$1.591`)读成「W50 实测 `$1.591` > `$1.10`」—— 那里面**混着仍在落账的 W49 尾巴**,月度聚合分不开两波,**没有可用的单波账单读数就不点名**。**(乙2) 未触发**(未发波)。
  **九、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(只写 `iterations/` 下的报告与本节);**铁律 11** 无 `requires approval`、**无空转等待**(自检在后台跑,本台没为它停下);CE 付费 **2 次(`$0.02`)**,其中自动复核那次**正是本轮头号发现的来源**;动态半(GH #124)**未跑不声称**。
  **交棒**:① ⭐⭐⭐ **owner —— `$50` 由实测越线,诚实 MTD ≥ `$61.73`,~61% 不是批测的**(上一轮的「已越 `$4.67`」修正为**至少 `$11.7`**);外来负载仍是停的 ⇒ 不紧急,但已花的不退;两条路不变(加 cost filter,⚠️ 走 `Project` 标签**必须先给批测机器补标签**否则 fence 恒读 0、**一个永远 CLEAR 的闸比现在更坏**;或给 `final-table-trainer` 单开预算)。**已推手机通知。** ② ⭐⭐⭐ **总监 —— 请裁围栏「已跨档」判据读滞后操作数**(第三节;与 GH #565 是**同一份滞后的另一个方向**);**本台不自行取用 CLEAR 那一读**。③ ⭐⭐ **总监 —— `campgrade` 独占波连续第六轮欠着**,闸 (i) 已 UNLOCKED,唯一阻塞是闸 (iii)。④ ⭐⭐ **总监(承上轮)** —— W50 的 promote/reject 与 `DEGENERATE` 结案。⑤ ⭐ **总监 —— `$1.10` 常数**新校验点 `≥$1.058/波`,两条腿未触发。⑥ ⭐ **总监 —— SIR 窗口 ≲3h**,GH #565 的修法必须当轮跑。
  **下一轮本台** = ① 先 `check_costs.sh` 盯 `budget refreshed` 有没有翻过 `06:02:46Z` —— 翻过且 `actual ≥ $50` ⇒ **围栏自动变 `$80`,活工具自己打 CLEAR ⇒ 第一件事发 `campgrade` 独占波**;没翻过 ⇒ **继续不发波,不许用离线那一读发波**;② ⚠️ **`--pending` 要重估不要抄 `$15.345`**(CE 已吃掉 `$11.476`),正确的量 = 诚实总额 − 工具当轮读到的 `actual`;③ 发波当轮写 `machines[].sir`/`create`/`launched_at`(GH #556/#544),写完**之后**复跑 `test_wave_gate_keys.py` 与 `test_wave_throttle.py`,发波前最后一条命令仍是 `wave_throttle.py`;④ ⚠️ 开工首条命令**不管道、重定向、`nohup`、不设短 `timeout`**(本轮两半各犯一次)。
  **本轮 token**:见报告第十二节。
  详见 `iterations/reports/batch-desk/20260906T152000Z.md`。
  **收尾补记(自检真码)**:`nohup` 无超时重跑,**约 12 分钟**跑完(这正是 `timeout 900` 砍在半路的原因)。脚本自打 **`selfcheck worst exit: 3`**、`legs run 10`、**`FINDINGS (exit 3): cadence queue-rulings owed-executions`**、**`UNCERTIFIABLE (exit 2): trunk-red(python)`**、`NOT RUN (inside a leg): tests/test_selfcheck_lua_leg.py`。**`3` = 有发现不是没跑成,十条腿全部真跑过。** ⭐ `trunk health (fast Lua detectors)` **`84 tagged detector file(s), 0 failures`**(⚠️ 边界照抄 **`FAST SUBSET, not the full suite`**);⛔ python 侧仍 `UNCERTIFIABLE`(`5a0` 超预算连带 9 条未核验,照抄边界 `This line is NOT a pass, and it is NOT evidence that trunk is red`);⭐ **`NOT RUN` 从上一轮的两个减到一个** —— `tests/test_rc_wrapper.py` 本轮跑成了,因为更靠前的腿已把 `lua5.1` 装上(GH #171 修复的时序效果);promote-time 约束 `160 live gate id(s)` / **`FROZEN none`** ⇒ **`pullcad` 陷阱无复发**,三个 co-promote 原子全 `ok`。
  **铁律 6 读数**:两次 `git push` **各自**跑同一个 gate 并各自自打 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck),`ARM_HOOK_EXIT=0`,**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;**`PUSH1_EXIT=0`**(新分支 `claude/great-sagan-c8z6fp`)/ **`PUSH2_EXIT=0`**(`a13cbb4d..52bf26eb  HEAD -> main`,未被拒)。动态半(GH #124)**未跑不声称**。
  **发表(GH #290 顺序:先 push 再发)**:在 `52bf26eb` push 之后,发表前 **`PRECHECK_EXIT=0`**(`local commits not on origin/main: 0`,`paths cited 1 / resolved on trunk 1 / refused 0`,`OK to publish`)。**新开 `[harness] GH #569`** —— 同一投影上的两个判决、两个相反的失效方向、三选一候选与本台倾向 (c),以及**本台不自行取用 CLEAR** 的声明。
  **收尾泄漏复检**:**`LEAK_EXIT=0`** —— 未发波,`running/pending` 空,常驻仍只有 AMI `ami-0a990a26d89c66547`。
  **本轮 token**:`TOKENS total_in=3,650,330 out=38,975 turns=33`(到统计时刻为止)。

- **2026-09-06T18:13–18:5xZ(发波轮:欠了六轮的 `campgrade` 独占入集波终于发出 = W51。⭐⭐⭐ 头号产出是**另一波** —— 起飞 70 秒后被本台亲手杀掉的那四台:它每一道闸都是绿的,却买不到自己唯一存在的理由)**
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**又误用管道**,脚本自拒(**同形第 7 次,又是当轮第一条命令**);改重定向重跑得 **`EXIT=124`** = **`timeout 400` 砍死在 python 腿上,不是干净也不是发现**(上一轮交棒 ④「不设短 `timeout`」**本轮没照做**)。已读到:stable-v1/v2/**v3**(`slotwait`,`0d0a7201b8f2`)锚点全 ok;promote-time **`160 live gate id(s)` / `FROZEN none`** ⇒ **`pullcad` 陷阱无复发**,三个 co-promote 原子全 `ok`;registry 9 行,`campgrade_wave_type_premise_reading` **仍 OWED** —— 那正是本轮要买的。⛔ **python 腿这轮没人看过**,不声称干净也不声称红。
  **二、⭐⭐ 成本:上一轮那条「有界不是死锁」的预言本轮兑现,一个字都不用改。** `AWS_SETUP_EXIT=0`,`COSTS_EXIT=0`。九月 MTD **`$50.804`**(免费 `budgets`,**`refreshed 2026-09-06T15:19:34Z`** ⇒ ⭐ **翻过了** `06:02:46Z`,那个数曾在 09:12Z/12:12Z/15:20Z **三轮逐位不动**);≥`$35` 自动付费 CE 复核 **`50.8036495163`**,两源本轮**重新合流**。15:20Z 那轮拒绝自行取用离线的 `CLEAR`、并写下「`budgets` 一刷新过 `$50`,围栏自动变 `$80`,本条自解」——**本轮它自解了,没有任何人推翻任何工具**。闸 (iii) 走工具:`wave_fence.py --planned 1.10 --pending 10.93` **`FENCE_EXIT=0 CLEAR`**:`actual $50.804` / `pending $10.930` / `planned $1.100` ⇒ `projected $62.834`;**`$50.00 crossed (AWS state=ALARM)`**、`$80/$100` 未跨 ⇒ **fence `$80.00`**、brake `$90.00`、**operative ceiling `$80.00`**、**headroom `$17.166`**。⚠️ **`--pending` 是重估的不是抄的**(上一轮交棒 ② 点名警告):CE 本轮与 15:20Z **逐位相同** ⇒ 这 ~3h **零入账** ⇒ 那 `$10.926` 外来 `r7a` 仍未入账;`pending = 诚实总额 − 当轮 actual = $61.73 − $50.804`。⚠️ **是下界,且出处已不可复算**:两条外来 SIR 本轮返回 **0 行**(窗口 ≲3h)⇒ **GH #565 的修法必须在花钱那一轮当场跑**,再次坐实。⭐ 外来负载确实停了:发波前全账号 `describe-instances`(全状态)与 SIR **均 0 条**,燃烧率 `$0/h`。⛔ **未用被禁的「MTD 增量对得上波费」手法**。
  **三、闸:三道 pre-launch 全绿,⛔ 第四道漏跑在起飞前。** (i) `wave_throttle.py`**不带管道** `THROTTLE_EXIT=0`(锚点 W50 末台 `03:20:34Z` ⇒ unlock `09:20:34Z`,`margin +32003s`),**并在发波脚本里作为最后一条命令再跑一次**:**`THROTTLE_EXIT_AT_LAUNCH=0`**(`+32264s`)。(ii) armed 串是 **1 个 id 的独占串**,与任何一波都不逐字相同 ⇒ 最强分支通过。(iii) 见第二节。⛔ **(iv) 是起飞后被 `test_wave_gate_keys.py` 打红才补跑的**:`reclaim_blind.py --wave-json W50_wave.json` **`RECLAIM_BLIND_EXIT=0`**,`not blinded -- 4 paired seed(s)`,四台全 `instance-terminated-by-user`、0 台 flip 前回收,工具收尾行 **`NEXT WAVE: spot`** 反过来给本波市场选择背书。两半皆绿 ⇒ 发波决定不变,但**「碰巧是绿的」不等于「事先查过」**,已按原样写进波次记录,未抹平。
  **四、⭐⭐⭐ 头号产出:一波闸全绿、却买不到自己存在理由的波,起飞 70 秒后被杀。** 18:18:19–18:18:30Z 先发出的四台 `SPOT_RUN_EXIT` 全 0、四 AZ 全中、零容量错误 —— **从每一个门看它都是好波**。它错在**没有任何门会看的地方**:未传 `--rec-slots 8`,吃了默认 **`REC_SLOTS=1`**。**两条独立理由**:(1) 它**在同一波里同时挪动了「被测 id 集」与「采集配置」**,而 id 集挪动幅度恰是最大的一种(55-id 成员串 → 1-id 独占串);`--slots 16 --hours 2 --rec-slots 8 --games 12` 自 **W39–W50 逐字未变**。(2) **更要命:它买不到本波唯一存在的理由** —— 读数跑在 **behav-dump** 上,dump 来自**被录制的对局**,`rec_slots=1`(16 槽录 1)⇒ 语料约标准的 **1/8**,最可能结局是 **`DOMAIN-NOT-REACHED`**,而裁定**明文禁止**把它读成「测过了,`.type` 不是字符串」。⇒ `terminate-instances` **`KILL_EXIT=0`**,四台 `running → shutting-down`,存活 ~70s,代价 **≈`$0.06`**(低于本轮每个围栏读数的分辨率)。⚠️ **重发换了四粒新种子**(6211/6277/6343/6409 → 6421/6487/6553/6619):70 秒几乎不可能出局,但**「几乎不可能」是个假设**,而 `recover_verdict.py` **按 `s<SEED>` 戳跨 run 并池** ⇒ 死前缀漏出一局就会并进本波读数,**没有门会举手**;换种子是把假设**去掉**。⇒ 那四粒**当作已花掉**。
  **五、W51 登记**:armed **`campgrade`** 单 id(**9 字节**,md5 **`185481d3d036c8dcf44313a4ad2b249a`**,现算);树 **`44329f419173e5f081a139e345eceecabf8a4348`**(`--ref` 用 40 位 SHA;`git ls-remote` **读两次**,发波脚本第一条命令再核一次,tip 移动则 exit 9 一台不发);**4 台 × 1 粒**,`c6i.4xlarge`/16 槽/**`--rec-slots 8`**/`--hours 2`/`--games 12`;种子 **6421/6487/6553/6619**(选种前 `seed_roster_index.py --build` `ROSTER_EXIT=0`,四粒逐个验未用);**spot**,由 `InstanceLifecycle=spot` **四台逐台核实**(`spot_` 前缀不是证据),降级阶梯一步未进;**四个不同 AZ 全部拿到所请求的那个**,零 re-aiming / 零 `RING EXHAUSTED` / 零 `InsufficientInstanceCapacity`;`check_armed_wiring.py --cand campgrade` **`WIRING_EXIT=0`** `1/1 WIRED` direct `mode_farm_generic.lua:304`;载体门**不适用**(总监原话「独占波,载体门对它是 no-op(按等级不按英雄)」)⇒ 故意没跑 `--assert-carrier`。⛔ **排波前置就是本波合法性所在**:`campfarm` 与 `campgrade` **不得同腿 arm**,而 `campfarm` **就在当前 55-id 成员串里**(实测) ⇒ **独占波是唯一满足它的形状**;`campgrade` **不在**成员串(`grep -c`=0),是**待入集候选**不是成员。**四条 SIR 全在发波当轮读下**(`sir-6j67gkip`/`sir-2mcfj4gn`/`sir-7c9zjdbm`/`sir-rmn7jrbq`,全 `active/fulfilled`),`create`+`launch_time` 一并写进 `machines[]`(GH #544:收割时不必回头修)。末台 `18:19:42Z`,slate spread **10s**。波次记录 `waves/W51_wave.json`;写完复跑 **`test_wave_gate_keys.py` 285/0**(先红 3 条)与 **`test_wave_throttle.py` 55/0**。
  **六、收割:无新数据(查过的,不是默认的)。** `soak/` 最新四个前缀就是 W50 那四台,**06:14Z 那轮已 4/4 收完**;`validation/` 最新仍是 **07-23** 历史物 —— 这也正是闸 (iv) 第一半成立的读数。
  **交棒**:① ⭐⭐⭐ **录像组 —— 欠了六轮的读数现在买得到了**(W51 语料 ~20:20Z 前落地):本方 10..11 带远古 engagement,armed vs baseline;**尺子已存在不要另发明**(`ancient_camp_domain.py` + `campgrade_ladder.py`);**降 ⇒ `.type` 是字符串前提结案,不降 ⇒ 不确定不是通过**;反向护栏:总数**不许塌到 0**;铁律 4(i-a) 两层各登记**读数**;⛔ 域为零 ⇒ `DOMAIN-NOT-REACHED` 退回,归类取不到 ⇒ **回总监重裁**。全文已转载进 `W51_wave.json:harvest_obligations[0]`。② ⭐⭐ **总监 —— 那条常驻行的 trigger 本轮已触发**(它自己写的就是「独占入集波……此刻还不存在」),读数落地后可退休;**`wave_fence_ruling4_first_live_run`(GH #515)本轮已结清**(第二节活跑读数 + 逐字抄下的 accrual 行),可一并退休。③ ⭐⭐ **总监 —— `strategy-4` 的 `status=harvested` 是陈账**:W24–W50+Y1 共 29 张波次记录 `grep -rl campgrade` **零命中**,09-04 裁定原文也说这波「还不存在」⇒ **`campgrade` 从未被任何一波 arm 过**;本轮**没有据它做任何决定**(依据取的是 `APPROVED_CONDITIONAL` 与「名额是集合增长的闸门,不是发波的闸门」)。**请修正,否则下一个读它的人会以为跑过了。** ④ ⭐ **总监 —— 闸 (iv) 漏在起飞前**,立案点不是这次的结果,是**发波脚本里没有 (iv) 这一条**,靠的是事后碰巧有个测试会红。⑤ ⭐ **总监 —— GH #454 两条腿**发波时不可评估,欠在 W51 收割轮(已写进 `harvest_obligations`)。⑥ ⭐ **owner —— `$50` 仍越线**,诚实 MTD **≥`$61.73`**(下界),**约 61% 不是批测的**;外来负载**已停**(`$0/h`)⇒ 不紧急,但已花的不退,`budgets` forecast **`$185.828` vs limit `$100`**。
  **下一轮本台** = ① **先收割 W51**(`recover_verdict.py` 标准路径;**先按 run 分目录下载再带前缀合并** —— 同秒 basename 撞车曾静默丢 15.9% 的局);② 收割当轮**当场**结清三条 `harvest_obligations`(`.type` 读数 / GH #454 两条腿 / **按 SIR 补机时,窗口 ≲3h 过了不可复算**);③ ⚠️ 开工第一条命令**重定向 + `nohup`,不管道、不设短 `timeout`**(本轮两坑各踩一次);④ **把闸 (iv) 排进发波脚本**,与 (i)-(iii) 并列。
  详见 `iterations/reports/batch-desk/20260906T181300Z.md`。
  **本轮 token**:`TOKENS total_in=7,611,651 out=55,596 turns=57`(到统计时刻为止)。
- **2026-09-06T21:13–21:4xZ(收割轮:W51 全量收割。⭐⭐⭐ 两件产出:① `campgrade` 独占波的 2(b) 读数干净落地而 2(a) 一个字没买到 —— 后者是录像组的活,本轮把 119 个 `.dem` 交出去;② GH #454 的 `$1.10` 两条腿结清,**都没触发**,但它是**在机时上守住的,价格已经跑出拟合带 9–19%**)**
  **零、闸 (i)**:`THROTTLE_EXIT=3` `THROTTLED`,`margin 10933s SHORT`,解锁 `2026-09-07T00:19:42Z`(锚点 W51 末台 `18:19:42Z`,slate spread 10s)⇒ **本轮按构造不发波,不存在预算问题。**
  **一、开工自检**:⚠️ **第一条命令连续第九轮踩同一个坑** —— `| tail -40` ⇒ 工具**自己拒答**打 `REFUSED: stdout is a pipe; exit 2, nothing checked`,**而 harness 报的 `SELFCHECK_EXIT=0` 是 `tail` 的**。改重定向重跑,跑完 **`SELFCHECK_EXIT=3`**(`legs run 10`);⭐ 归因**照抄工具自己打的那一行不手编**(GH #267:手编归因曾错 22 小时):`FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(lua)` / `UNCERTIFIABLE (exit 2): trunk-red(python)` / `NOT RUN: tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。逐腿:推送门**已上膛**(`core.hooksPath=.githooks`);未落地工作 **OK**(25 可核验 / 843 uncertifiable,shallow);报告节奏 **2 findings**(协同组两个 3.6h 洞);稳定版锚点 **3/3 OK**(**`stable-v3` `slotwait` 13:2xZ 新增**);promote 约束 **OK**(161 活 gate / 14 PROMOTED / `FROZEN none` / 3 原子全绿);trunk python **UNCERTIFIABLE**(`106 passed / 0 failed / 2 未跑成`,无 `lua5.1` + 120s 超时 ⇒ **那一侧本轮没人看过,不是绿的**)。⛔ **Lua 检测器 RED `test_gated_helper_nesting_census.lua`:新的 gate-inside-a-gate**(`stayattr,staybag,staybottle,staysrc,staytower,stayurn` → `J.ShouldStayAndRegen` → `J.HasFieldRegenSource`/`bagsalve`,`mode_retreat_generic.lua` + `jmz_func.lua`),时间上对应 `6c089cf` 协同组 19:46Z。⚠️ **工具自己的边界照抄**:`failing ON THE WORKING TREE. Whether main is red too is NOT established by this line: re-run after git stash` —— 本台**没跑那次 stash 复核,所以不主张 main 是红的**;能诚实说的只有**本会话 `bots/` 一行未改 ⇒ 这条红不是本会话引入的**。**交协同组+总监,本台不改 `bots/` 也不替它判**;本台的利害是:内层若非单位元 ⇒ **单 arm 外层测到的是 no-op 而 `check_armed_wiring.py` 仍说 WIRED** ⇒ **本台花钱发的局买不到读数**。
  **二、成本**:`AWS_SETUP_EXIT=0`(`dota2bot-agent`);`COST_EXIT=0`,**MTD `$50.804`**(`budgets` 免费读数,`refreshed 15:19:34Z`;≥$35 ⇒ 脚本自动花 `$0.01` CE 复核 **`50.8036495163`**,一致);`forecast $185.828` vs `limit $100.0`。刹车 $90 / 围栏 $80 / 批准线 $100 **均未触发**;**`$50` owner 告警仍越线**(`AWS state=ALARM`)。⭐ 本轮对它有利的新事实:**外来负载已停**(见六)⇒ 那条 forecast 是**按它还在跑外推的,不再成立**;⛔ 但**已花的不退**,上一轮量到的诚实下界仍是 **≈61% 不是批测的**。
  **三、⭐⭐⭐ W51 收割**:**`RV_EXIT=0`,无拒答**。`soak/` 前缀,四 run **各自子目录**(GH #225 不压平:`3af107/3c2be6/fff95f/73aa9d` = **64/48/64/62**);`files_seen 238 / games_loaded 238 / source_dirs 4 / unparseable 0 / pooled_basename_dirs_overridden 0` ⇒ **同秒 basename 撞车一局没丢**。逐机(`machines[]` **零 null**):`6421/2a ab42 ba16 depth23.17`、`6487/2b ab30 ba12 depth17.14`、`6553/2c ab40 ba18 depth24.83`、`6619/2d ab39 ba17 depth23.68`;**四台全 `instance-terminated-by-user`**(`user-initiated termination`)⇒ **完工自毁、零抢占、四粒完整无缺臂**。⭐ **SIR 窗口抢在关闭前读下**(GH #375 仅存 ~1h;四台 `19:03–19:14Z` 终止,`21:1xZ` 读到)—— **这是本轮唯一有时限的事,所以排第一件做**;**GH #556 要制度化的正是它,本轮仍靠「本台记得」不靠机制**。`scored 214 / unfinished 0 / 4-4 粒`,`arm_depth 17.14–24.83`,`min_arm_depth 8`,`thin_arm_seeds` 空。**读数(铁律 4(i-a) 两层读数,非局数,取自工具自己的 `strata.<m>`,未手算未按局加权)**:gpm **−2.19**(3/4;ab +87.22 / ba −91.60 / side 89.41)、xpm **−3.00**(2/4;+53.49 / −59.50 / 56.50)、deaths **+0.07 略差**(1/4;−0.50 / +0.64 / −0.57)、last_hits **−0.48**(3/4;+13.80 / −14.75 / 14.28)。**四量全反号 ⇒ 按 4(i-c) 不是否决理由**(侧偏已消除的估计量,反号 ⟺ `|side|>|arm|`,**恒等式不是诊断**),登记因 4(i-a) 要求,**不按 4(i-b) 读成噪声**。⛔ **胜率通道 `DEGENERATE`(工具自拒)**:少数侧 238 局只赢 13(share 0.0546 < 0.20),四粒里**三粒 headroom 0.0**(侧别普查 0/58、0/42、0/58,仅 6619 为 12/44) ⇒ **`winrate 0.507` 与 `1/4` 本报告不作读数、不许被引为 2(b) 支持**。**判读**:四量全远在镜像波噪声底之内 ⇒ **2(b) 没有明显负面**;⛔ **但这不是入集** —— 本波唯一的存在理由是买 **2(a)**(GH #241 `.type` 前提的否定式读数),**执行人是录像组,四个经济数字对它一个字都没说**;⛔ **不与任何波并池**(1-id 独占串 9 字节 md5 `185481d3…`,对手腿挂着 55–62 个 id)。
  **四、⭐⭐ GH #454 两条腿结清,都未触发,`$1.10` 站住**:乙2 自毁机**平均机时 `0.8610 h/台`**(3.4442/4)< 1.00 —— 排除条款本波为空(无一台被抢占);乙1 **单波账单 `$0.9774`** < $1.10,逐台按**它自己 AZ** 在 `18:19–19:14Z` 的现货报价(`2a 0.8997h×$0.2914`、`2b 0.7317h×$0.2732`、`2c 0.9114h×$0.2972`、`2d 0.9014h×$0.2521` = EC2 `$0.9602`,+ `4×0.861h×$0.005` IPv4 `$0.0172`)。⚠️ **交总监,这才是本节的产出:它是在机时上守住的,不是在价格上。** 机时 0.861 落在拟合带 0.745–0.9 内**很正常**;而**现货报价 `$0.2521–$0.2972` 已跑出此前每一个结算点所在的 `$0.231–$0.250` 带 9–19%**,`$0.9774` **落在 W46 区间 `$0.959–1.022` 之内、高于其余每一个结算点**(其余 ≤ `$0.892`)⚠️(**订正本条初稿的「迄今最高」——错的,W46 上沿 `$1.022` 更高**;正确说法是**同处最高档并把该档从一个孤点变成两个点**,结论不变反而更强)⇒ **乙1 只剩约 13% 价格余量,而两条腿都不直接点名价格**(乙2 只看时长,乙1 只看乘积)⇒ **下次顶掉 `$1.10` 的大概率是价格移动,现有判据只能等它先穿过一个乘积。本台不自裁。**
  **五、收割义务三条逐条结清**:`GH_454_constant_legs` ✅ **本轮**(见四);`wave_fence_ruling4_first_live_run` ✅ **18:13Z 发波轮已结清**(`gates.gate_iii_fence` 逐字载着活路径读数)⇒ **请总监退休该 registry 行**;`campgrade_wave_type_premise_reading` ⏳ **OPEN 且不归本台关**(本台那一半发波当轮已履行,读数属录像组)—— ⭐ **本轮新增的是:语料现在存在了,119 个 `.dem` 在四个 run 前缀下(32/24/32/31)**;该行 trigger 原文是「独占波……此刻**还不存在**」,**它现在存在了,这条不能再以"没有波"挂着**。
  **六、泄漏**:`LEAK_EXIT=0`,`running,pending` **全账号为空**。① **本台侧干净**(W51 四台全部自毁、无第五台、无留存;常驻仍只有 AMI `ami-0a990a26d89c66547`);② ⭐ **外来负载停了** —— 前两轮点名的 `final-table-trainer`(`i-0ce7ddf08ea24cc7f` `c7a.16xlarge` → `i-0a7d4c7fbeb3a6712` `r7a.16xlarge`)本轮 region 内不在跑 ⇒ 上一轮「headroom 约一天内吃光」**暂停计时**;⛔ **但 GH #515/#528 立案理由不变**:`dota2bot-batch` 的 `CostFilters=null`,**本台发波围栏仍读着一个它不控制的账号级数字**,那台机器随时可再起。
  **七、局数(章程第 7 步)**:收割型 ⇒ **报实得** `scored 214 / unfinished 0`,四粒 `58/42/58/56`。预登记 `expected_games` 是 ~96,实得 214;⚠️ 该预登记量**偏低约 75%**,**两个方向都不许拿它当分母**(既不是"超交付"也不是"少交付")。
  **八、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**(只动 `iterations/` 下的报告、`W51_wave.json`、本节)。**铁律 11** 未触发任何 `requires approval`,**无空转等待** —— 本轮唯一的"等"是闸 (i) 自己 `THROTTLED`,那是**门在拒绝**不是空转。
  **交棒**:① ⭐⭐⭐ **录像组 —— 欠了七轮的 `campgrade` 条件 (a),语料现在真在手上**(119 `.dem`):尺子**已存在不要另发明**(`ancient_camp_domain.py` + `campgrade_ladder.py`);**降 ⇒ `.type` 是字符串前提结案,不降 ⇒ UNCERTAIN 不是通过**;反向护栏总数**不许塌到 0**;铁律 4(i-a) 两层各登记**读数**;⛔ 域为零 ⇒ `DOMAIN-NOT-REACHED`,**不许读成「测过了、`.type` 不是字符串」**;归类取不到 ⇒ **回总监重裁,执行者不得自行套用后果条款**(全文 `iterations/reports/batch-desk/waves/W51_wave.json:harvest_obligations[0]`)。② ⭐⭐ **总监 —— `$1.10` 是在机时上守住的,价格已出带 9–19%,乙1 余量 ~13%,而两条腿都不看价格**(四)。③ ⭐⭐ **协同组+总监 —— 自检 RED 的新 gate-inside-a-gate**(一)。④ ⭐⭐ **总监 —— 退休 `wave_fence_ruling4_first_live_run` 行**。⑤ ⭐ **总监 —— GH #556 本轮再次靠「本台记得先做」成立**(SIR 距终止仅 ~2h,再晚一轮就没了),**这不是机制**。⑥ ⭐ **owner —— `$50` 仍越线**(MTD `$50.804`);好消息是外来负载已停、`forecast $185.828` 不再成立,坏消息是已花的不退且 `CostFilters=null` 意味着**它再起来时关上的是批测的闸**。
  **下一轮本台** = ① 闸 (i) 解锁 `2026-09-07T00:19:42Z` —— ⛔ **不要读这个数,读 `wave_throttle.py` 打的那一行**,且**把钟读在动作上**(循环前最后一条 + 脚本内裸 `$?`);② 解锁后若发波:默认**全成员串**(现 55 id)、**spot 优先**(不传 `--on-demand`)、`--ref` 钉 40 位 SHA + `git ls-remote origin main` 读两次,**闸 (iii) 必须活跑**(MTD 已在 `$50` 之上,`$80` 围栏是当前操作天花板);③ **把闸 (iv) 排进发波脚本**与 (i)–(iii) 并列(上一轮 ④,本轮仍欠);④ ⚠️ **开工第一条命令重定向、不管道、后台给足**(本轮第九次)。
  详见 `iterations/reports/batch-desk/20260906T211300Z.md` 与 `iterations/reports/batch-desk/waves/W51_wave.json`。
  **本轮开出/追评的 issue(#290 顺序:三份草稿都在 `eb4e8b28` 推上 main 之后才发表,发表前各跑 `claim_precheck.sh`)**:**GH #576 新开 `[harness]`**(`stayattr` 一族 6 个 id 嵌进 `J.ShouldStayAndRegen`,#542/#484 那族的第三形态;`PRECHECK_NEST_EXIT=0`,`paths cited 6 / resolved 6 / refused 0`);**GH #454 追评**(两条腿结清 + 价格才是下一个失效方向,**不新开**;`PRECHECK_454B_EXIT=0`,`paths cited 2 / resolved 2 / refused 0`);**GH #480 追评**(§5 预登记买法的语料 119 `.dem` 现已在手,判读规则原样转抄,交录像组)。⚠️ **本轮订正了本台自己的一句话**:§4 初稿的「`$0.9774` 是迄今最高单波结算值」**是错的**(W46 上沿 `$1.022` 更高);正确说法是**落在 W46 区间之内、高于其余每一个结算点**(其余 ≤ `$0.892`)。⭐ **订正不削弱结论反而加强它**(高档从一个孤点变两个点,`$0.95+` 不再能当异常打发);三处写过它的地方已在 `eb4e8b28` 一并订正,且**订正先于三条 issue 发表**,否则 #454 会带着错的最高值出去。
  **本轮 token**:`TOKENS total_in=4,333,251 out=36,906 turns=38`(到统计时刻为止)。
- 2026-09-07T06:12–06:3xZ(**发波轮:W53 = 51-id 成员串家族第一波,spot 四台一 AZ 一台,四道闸全部在起飞前跑完。⭐⭐⭐ 头号产出不是那四台机器,是闸 (i) 在同一轮里赢了两次——一次赢了本台自己手算的容器钟,一次赢在 `exit 9` 真的挡下了第一次发波尝试**)。
  **一、自检(铁律 10)**:⚠️ 开工第一条命令**第十二轮**误用管道,工具自拒(`REFUSED: stdout is a pipe; exit 2, nothing checked`);改重定向 + 后台重跑,**`SELFCHECK_EXIT=3`**(真退出码,不经管道)。`exit sources` 逐字:`legs run 10`、`FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(python)`、**`UNCERTIFIABLE (exit 2): none`**、`NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`(**腿内两条没跑成,不是通过**)。锚点 **5 个全 OK**(`stable-v5 = ckpush` `3fe17f9dd08b`);`162 live gate id / 17 PROMOTED / FROZEN none`;4 个 co-promote 原子全 ok;`unlanded_commits` 无 OFF-TRUNK;`ORPHAN_PROPOSAL` none;`expired waits` 无;`CADENCE` 2 处 GAP **都在协同组**(3.6h / 3.7h);`RIDESHARE` 1(`hero-40` `liondrainmi`,未 armed)。⭐ **trunk Lua 腿本轮跑完且干净**(`84 tagged detector file(s), 0 failures`,快子集)⇒ **上一轮的 GH #585 不复现**。⛔ **本轮值得记的不是又踩了管道坑,是「已知坑仍复发」——「靠人记得」这条路被本台自己证伪(第三次请总监给独有退出码或 wrapper)。**
  **二、⛔ trunk python RED,而这一次门是对的**:`108 passed, 1 failed`,唯一 FAIL `tests/test_detector_source_constants.py`,单跑复核 `DSC_EXIT=1`,逐字两条 `zusultstrand_domain:HP_GATE / :ULTCASH_HP_GATE is classified (UNREGISTERED) -- a new module-level HP constant appeared and no one said what it is.` 溯源:**录像组 `6eaebe38`(2026-09-07T04:27:42Z)**落地的 `tools/batch_test/behavioral/zusultstrand_domain.py` 带进两个模块级 HP 常数,`HP_CENSUS` 无对应行。⭐ **本周第四例「红在登记表的形状」,但方向与前三例相反**:#579 / #584 / #585 是**门有缺陷**(硬编码被一次正当 promote 或下一个 fixture 证伪),**本例是门抓到了一个真的漏做的步骤** ⇒ **修法是补 `HP_CENSUS` 行(MIRROR/PROXY/INDEPENDENT 三选一),不是改门;把它当第四个"门缺陷"去修,恰好是这条门存在理由的反面。** 诚实边界(比上一轮更强):`HEAD == origin/main == 523f21baf14a` 且 `git diff origin/main -- tests/ bots/ game/ tools/` **为空** ⇒ **红就在 main 上,不是工作树伪影**,不需要 `git stash`。两条好消息:**GH #584 的 `test_carrier_hero_guard.py` 本轮 PASS**;**GH #490 点名的 `J.ShouldTpSupportTowerFight` 断言已挪进测试文件且本轮 ok**(本台只声称"那条现在解析得到且 ok",**关不关归总监**)。
  **三、成本**:`AWS_SETUP_EXIT=0`(容器无 AWS CLI,脚本自己装上);`COST_EXIT=0`;MTD **`$62.042`**(免费 `budgets`,`refreshed 2026-09-07T02:04:33Z`),≥$35 ⇒ 自动 `$0.01` CE 复核 **`62.0418196925`**,两源一致;`forecast $205.267`。⚠️ **与 03:05Z 那轮逐位相同且 `refreshed` 戳相同 ⇒ 同一张快照**,不是「三小时没花钱」的读数;⛔ 本台**不**从中推断 W52 是否已入账(步骤 2 乙 明令禁用)。常驻成本仍只有 AMI `ami-0a990a26d89c66547`。
  **四、发波 W53:四台 spot 全部起飞,零容量降级。** `7144/i-0d10b2db261c57b13/2a/06:22:02Z/sir-99r7gyxm`、`7154/i-00a6e66d54b1a9fb3/2b/06:22:05Z/sir-e8azhk8m`、`7156/i-0fbfedf2a0cc4de69/2c/06:22:08Z/sir-wcmzk25n`、`7162/i-02bad63ba3c9b03af/2d/06:22:11Z/sir-rsk7g9mm`,四行全 `spot / active / fulfilled`(**逐台由 `InstanceLifecycle` 核实,不读 run_id 的 `spot_` 前缀**;**SIR 在发波轮就读了**,GH #556)。臂串 **51 id / 460 字节 / md5 `e61700f44778dc1ab67166922d22b0bc`**,从**发波树**重算;`--ref` 钉 40 位 SHA `523f21baf14a03cadfb5df7002ba7124d70375c2`,`git ls-remote origin main` **读三次**(选型两次 + 脚本内第三次带 `exit 9` 漂移检查)全一致;采购参数 `--slots 16 --hours 2 --rec-slots 8 --games 12` 与 W39–W52 逐字节相同,`--az` 逐台轮转;发波前先跑 `--dry-run`。
  **⭐⭐⭐ 五、闸 (i) 在同一轮里赢了两次(交总监,这才是本轮的产出)。** **第一次赢的是本台自己手算的钟**:决策时刻 `06:14:27Z` 工具打 `THROTTLED (exit 3) / margin 438s SHORT / unlock 06:21:45Z`;本台随后**凭感觉认定"应该已过 06:21"**并组织了一次发波,**发波脚本里的闸 (i) 复读打出 `now 06:17:15Z / margin 270s SHORT`,`exit 9` 挡下,零台起飞**——**容器钟比本台的估计晚约 4 分钟,而本台错在朝越闸方向。** ⚠️ **这是一条结构不是一次疏忽**:闸 (ii)(iii)(iv) 读的都是文件或 API,谁都伪造不了;**只有闸 (i) 的输入是「现在几点」,而这是唯一一个执行者能在脑子里替工具算出来的量** ⇒ **四道闸里只有它手算就能绕过,且绕过不留痕迹**(报告仍可诚实地写「闸 (i) 已跑」,那是真的——06:14 那次)。与 `pullcad` 同族:门在、两次读数各自都对,**错的是「哪一次读数绑定动作」这个约定没被机器执行**。**第二次赢的是它自己的措辞**:工具在 `UNLOCKED` 行末逐字写着 `RE-RUN IT AT THE LAUNCH INSTANT: it is the action, not the decision, that the gate binds.` ⇒ 合规发波是脚本内轮询到 `exit 0` 后立刻发,`THROTTLE_AT_ACTION_EXIT=0`、`now 06:21:59Z`、`margin +14s past unlock`。**⇒ 请总监把「闸 (i) 必须在发波脚本内、动作处复读,非零即 `exit 9`」升成硬规则:本轮它靠本台自觉才挡住,而自觉那一侧本轮真的失败过一次。**
  **六、四道闸((iv)→(i)→(ii)→(iii),(i) 在动作处复读)**:**(iv)** `reclaim_blind.py --wave-json W52_wave.json` **`RECLAIM_EXIT=0`**,逐字 `VERDICT: not blinded -- the wave delivered 4 paired seed(s).`,四台全 `instance-terminated-by-user`(45.6–55.0 min),`0 reclaimed before the flip` ⇒ ⭐ **这是 `update` 键修好后第一次干净的 exit 0**(W51 那次因收割写进 `terminated_at` 而 `UNDECIDABLE`)——**修复成立**;前置无欠收割(S3 最新四前缀是 W52,03:05Z 已 4/4 全收)。**(ii)** 臂串动了(52→51,`ckpush` 出集),md5 与此前每一波都不同 ⇒ 本树+本集 **0 粒累计**,PASS。**(iii)** `wave_fence.py --planned 1.10 --pending 0.99` **`FENCE_EXIT=0 CLEAR`** ⇒ `projected $64.132`;`$50.00 crossed (AWS state=ALARM)`、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、ceiling **`$80.00`**、**headroom `$15.868`**;accrual 行逐字 `CERTIFIED ZERO (0 accruing instances account-wide), but --pending $0.990 was given and is used as the larger claim.`(**未传 `--no-accrual-check`**)。`--planned 1.10` = GH #454 常数(**不是已作废的 `$0.90`,也不是 W46 实测上确界 `$1.02`**);`--pending 0.99` = W52 结算价 `$0.9865`,**仍当 pending 的理由是本轮 MTD 与上一轮同快照、W52 约 01:1xZ 结束落在 4.3–11.3h 滞后内,而「MTD 增量对得上波费 ⇒ 已计入」是步骤 2(乙)禁用的手法** ⇒ 可能重复计一次,**失效方向是安全那一侧**。**本轮未新跨任何告警档。**
  **七、载体门项数变了,而且有正当理由(上一轮交棒 ④ 结清)**:`CT_EXIT=0`,51 id ⇒ **10 hero-scoped / 41 generic / 0 unresolved ⇒ 7 词项**;⭐ **TERMS 行不再与 W48–W52 逐字节相同:`chaos_knight` 没了**,这是 **promote `ckpush` 的正确后果**(唯一 chaos_knight-scoped 的 armed id),**不是本波的缺陷**,也正是上一轮打红 `test_carrier_hero_guard.py` 的同一个事实。`check_armed_wiring.py --cand` **`ARM_EXIT=0`**(`all 51 armed ids wired on HEAD`;旗标是 `--cand` 不是 `--arm`,上一轮留的注**本轮省了一次重试**)。`CARRIER_GATE_EXIT=0` `ids=10 seeds=4`,逐词项 `cm 2 / lion 2 / od 2 / pudge 2 / wk 4 / sb 2 / zuus 4`,**每项 ≥2**。种子:`ROSTER_EXIT=0`(233 粒),⭐ **多做一步核对——W52 那四粒确实在册,这才是「build 吃进了最新一波」的证据**;窗口右移 **[7101,7500]**,**窗口 ∩ 索引 = 空集**(集合求交,非目视);popcount 分布 `{0:39,1:124,2:126,3:91,4:18,5:2}`;穷举 4-子集**最大化逐词项载体数的最小值** ⇒ **7144/7154/7156/7162**(最小 2,总数 18)。⚠️ **本轮订正本台自己的一个数**:第一次用 `tr ',' '\n' | wc -l` 数出 **50**,与 `test_set.md` 第 4 行自称的 51 冲突;根因是 **`wc -l` 数换行不数行**而该串被 `tr -d '\n'` 剥了尾换行 ⇒ **系统性少一**;改 `split(',')` 得 **51**,与 460 字节、md5、第 4 行散文三方一致。⭐ **失效方向才是要点**:若当时"信自己的 50、疑文件的 51",就会**把一次正确的 promote 记成一次串损坏**。**口径从此是 `split(',')`,不是 `wc -l`。**
  **八、泄漏**:`LEAK_EXIT=0`,全账号 `running,pending` **恰好是本波这四台**;无第五台、无留存、无降级成按需的台;⭐ **外来负载 `final-table-trainer` 连续第二轮不在**;⛔ **但 GH #515/#528 立案理由不变**:`CostFilters=null`,**本台围栏仍读着一个它不控制的账号级数字**。
  **九、收割义务**:`wave_fence_ruling4_first_live_run` ✅ **本轮再次活跑**(第二次请总监退休该行);`campgrade_wave_type_premise_reading` ⏳ OPEN **不归本台关**(读数属录像组,**第十轮**),且该行 trigger 原文「独占波此刻还不存在」**自 W51 起已失效**,上一轮点名、本轮仍未改;`p2_pair_arm_stayattr_staysrc` 本波未触碰 P2,不触发。W53 自己的四条义务写进 `waves/W53_wave.json:harvest_obligations`。
  **十、局数(第 7 步)**:发波型 ⇒ 报**预登记** `expected_games ≈ 96`;⚠️ 该量已连续偏低约 75%(W49 168 / W51 214 / W52 214),**两个方向都不许拿它当分母**。
  **十一、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(只动 `iterations/` 下的报告、`W53_wave.json`、本节,加上 `--build` 写的 `iterations/data/seed_roster_index.json`)。**铁律 11** 无任何 `requires approval`、**无空转等待**;⚠️ 有一次**有界等待**(闸 (i) 差 270s ⇒ 脚本内 20×30s 有界轮询,实等约 4.5 分钟)——**那是门在拒绝,不是空转**,且写在脚本里、有上界、开门即发,登记在此供总监复核。
  **交棒**:① ⭐⭐⭐ **总监 —— 请把「闸 (i) 必须在发波脚本内动作处复读、非零即 `exit 9`」升成硬规则**(立案句见五:四道闸里只有它的输入是"现在几点",也只有它手算就能绕过且不留痕迹);② ⭐⭐⭐ **录像组 —— `zusultstrand_domain.py`(你们 `6eaebe38`)把 trunk python 打红**,补 `HP_CENSUS` 两行,**不是改门**(与本周前三例不同族);③ ⭐⭐ **总监 —— GH #454 第四个测量点下轮到位**,本波四 AZ 全 spot 零降级是价格侧又一观测机会(上一轮已报:`$1.10` 守在**机时**上,价格已出带 9–19%,乙1 余量 ~13%,**两条腿都不看价格**);④ ⭐⭐ **总监 —— 退休 `wave_fence_ruling4_first_live_run` 行**(第二次请求);⑤ ⭐⭐ **录像组 —— 三笔条件 (a) 仍欠**(`campgrade` 第十轮 / 61-id 家族 W49 / `campbind`+`zusboltdom` W52),且 `campgrade` 那行 trigger 原文已失效请总监改词;⑥ ⭐ **总监 —— 管道坑第十二轮且是「已知坑仍复发」**,请给独有退出码或 wrapper(第三次请求);⑦ ⭐ **总监 —— 协同组 cadence 两处 GAP**(3.6h/3.7h),本台只登记不解读;⑧ ⭐ **owner —— `$50` 仍越线**(`$62.042`,`AWS state=ALARM`,`forecast $205.267`),外来负载已停两轮但**已花的不退**且 `CostFilters=null`;⑨ ⭐ 存量:#480/#241 / #490(点名断言已 ok,关不关归总监)/ #584(已修好)/ #585(本轮不复现)/ #576 / #486 / #472 / #427 / #408 / #420 / #449 / #446 / #290 / `strategy-5b` / **#388 的 W34 103 份 `.dem` 2026-09-22 到期(15 天)**。
  **下一轮本台** = **收割轮**:① `recover_verdict.py` 从 S3 逐局全量重算(`soak/` 前缀**不是 `validation/`**,四个 run 下到四个分开的子目录);② **先填 `machines[].update`**(SIR `Status.UpdateTime`,~1–3h 就老化,**第一时间读**)**再**跑闸 (iv);③ 闸 (i) 下次解锁 `2026-09-07T12:22:11Z` —— ⛔ **不要读这个数,读 `wave_throttle.py` 打的那一行,且把钟读在动作上;本轮就是这条规则第一次真的救了一波**;④ 结算本波成本并对 GH #454 **两条腿**逐条判定(乙1 `>$1.10`/波;乙2 自终止台机时均值 `>1.00 h/台`);⑤ ⚠️ **开工第一条命令重定向、不管道、后台给足**(第十二次)。
  详见 `iterations/reports/batch-desk/20260907T061200Z.md` 与 `iterations/reports/batch-desk/waves/W53_wave.json`。
  **本轮开出的 issue(#290 顺序:三份产物先在 `09b14e32` 推上 main,之后才发表;两份草稿各跑一次 `claim_precheck.sh`,均 `PRECHECK_EXIT=0` `clean`、`paths cited 4 / resolved on trunk 4 / refused 0`)**:**GH #588 `[harness]`**(闸 (i) 是四道闸里唯一手算就能绕过且不留痕迹的一道,请升硬规则;含本轮被 `exit 9` 挡下的那次越闸尝试)、**GH #589 `[bug]`**(`zusultstrand_domain` 两个 HP 常数未登记进 `HP_CENSUS`,**门是对的,修法是补登记不是改门**)。
  **收尾复核**:发完 issue 后全账号 `running,pending` 仍**恰好是本波四台**,`FINAL_LEAK_EXIT=0`。
- **2026-09-07T09:14–09:4xZ(收割轮:W53 收割 —— ⭐⭐⭐ 头号产出不是那个 −8.40,是「这一波量不出任何东西」,以及量不出来的那个理由自己是有形状的:四台里**三台被容量回收**,而回收**只会砍 ba 腿**)**
  **一、自检(铁律 10)**:⚠️ **第一条命令第十三轮踩同一个坑**(`| tail -40` ⇒ 工具自拒 `REFUSED: routine_selfcheck.sh stdout is a pipe; exit 2, nothing checked`,harness 报回来的 `SELFCHECK_EXIT=0` **是 `tail` 的**);改重定向 + 后台 + **不设短 `timeout`** 后跑到 python 腿/锚点腿/promote 腿出结果,**Lua 腿写报告时仍未跑完 ⇒ 既不声称干净也不声称红**(整份自检在本容器 >900s,连续第三轮)。逐腿照抄:推送门已上膛;未落地工作 `OK: no unlanded work in the certifiable window`;报告节奏 **2 findings**(协同组两个洞 3.6h/3.7h);未裁 queue 行 `RIDESHARE 1`(`hero-40`,P4.2 冻结期内不入集)/ `OTHER none`,`total open requests: 77`;孤儿提案 none;owed registry **10 行**(`campgrade_wave_type_premise_reading` 仍点名本台);过期等待 无;稳定版锚点 **5/5 OK**;promote 约束 `164 distinct live gate id(s)` / **17** PROMOTED / **`FROZEN none`** / `promote-atom constraints: OK`。⭐ **trunk python 本轮 `109 passed, 0 failed, 2 uncertifiable`** —— **上一轮那两条红(GH #579 `test_stable_anchors.py`、GH #584 `test_carrier_hero_guard.py`)双双 PASS**;⚠️ **诚实边界:本台只读到「现在是绿的」,没去核实是谁修的怎么修的,归因不是本台的读数**。两条 `UNCERTIFIABLE`(`test_rc_wrapper.py`/`test_selfcheck_lua_leg.py`,逐字 `no lua5.1 on PATH`)—— **没跑成不是通过**。
  **二、成本**:`AWS_SETUP_EXIT=0`;`COST_EXIT=0`;MTD **`$63.197`**(免费 `budgets`,`refreshed 2026-09-07T08:05:04Z`),≥$35 ⇒ 自动 `$0.01` CE 复核 **`63.1965021733`**,两源一致;`forecast $206.77`。⭐ **pending 是推导的不是抄的**:W53 四台在 `06:53:14Z–07:10:38Z` 之间全部消失,而刷新戳 `08:05:04Z` 只晚约 **1 小时** ⇒ **落在章程点名的 4.3–11.3h EC2 滞后窗口内**,故本波**不假设已入账**,按 `$0.77` 申报 pending;⛔ **未用被步骤 2 (乙) 禁用的「MTD 增量对得上波费 ⇒ 已计入」手法**。闸 (iii):`wave_fence.py --planned 1.10 --pending 0.77` **`FENCE_EXIT=0 CLEAR`** ⇒ `actual $63.197` / `pending $0.770` / `planned $1.100` / `projected $65.067`;**`$50.00 crossed (AWS state=ALARM)`**、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、ceiling **`$80.00`**、**headroom `$14.933`**;accrual 行逐字 `CERTIFIED ZERO (0 accruing instances account-wide), but --pending $0.770 was given and is used as the larger claim.`(**未用 `--no-accrual-check`**)。**本轮未新跨任何告警档**;实际支出 **$0.01**,**零发波**。
  **三、⭐ 有时限的那件事排第一位**:`SIR_EXIT=0`,四条 SIR **全部还在**(末台终止后约 2h 读到),`UpdateTime` **同时写进 `update`(闸 (iv) 读的键)与 `terminated_at`** ⇒ **GH #580 本轮没有静默复发**。⛔⛔ **四台里三台是 `instance-terminated-no-capacity`**:`7144/2a 06:53:14Z/0.5200h`、`7156/2c 07:01:46Z/0.6606h`、`7162/2d 07:10:38Z/0.8075h`;唯一自终的是 `7154/2b 07:06:25Z/0.7389h`。**这是 spot 时代自 W29 以来第一次容量回收,而且一次三台。**⚠️ SIR 的 `SpotPrice` 四台全读 `0.680000` —— **那是出价上限不是成交价**。
  **四、⭐⭐⭐ W53 收割:一波量不出东西的波。** `RV_EXIT=0` 无拒答;语料走 `soak/`,四 run **各自子目录**(GH #225);`files_seen 179 / games_loaded 179 / source_dirs 4 / unparseable 0 / pooled_basename_dirs_overridden 0`。逐机(`machines[]` 零 null):`7144 ab26/ba0 depth0.00 **NO-PAIR**`、`7154 ab29/ba13 depth17.95 计分`、`7156 ab37/ba5 depth**8.81** 计分`、`7162 ab42/ba3 depth5.60 **THIN-ARM**`;`scored 129 / unfinished 0 / seeds_scored **2 of 4** / min_arm_depth 8`。**读数(铁律 4(i-a),取工具自己的 `strata.<m>` 与 `per_seed[].<m>_ab/_ba`,未手算未按局加权)**:gpm **−8.40**(ab −90.61 / ba +73.81 / side −82.21 / 反号 / side>arm 2/2)、xpm **−12.55**(−33.47 / +8.37)、deaths **+0.26**(+0.30 / +0.21,不反号)、last_hits **−2.08**(+8.62 / −12.78);`comps_better` gpm **1/2**、xpm 0/2、deaths 0/2、last_hits 1/2、winrate 0/2;逐粒 gpm **7154 −32.77 / 7156 +15.97 —— 仅有的两粒而且反号**;`suggested=hold_or_reject`。⛔ **winrate 通道 DEGENERATE 到极限**(工具自拒):少数侧赢 **0** 局 / 179(share `0.0000`),侧别普查 **radiant 0 / dire 179**,界定 `0.5 ± 0.0` ⇒ **两个方向都不许当铁律 2(b) 证据**(W52 已是 5/227,本波是它的极限)。⛔ **本台不把它当家族读数交出去**:铁律 4(i-c) 的反号条款在这里**用不上** —— **否决它的不是反号是深度**,两粒计分种子的 ab 腿都比自己的 ba 腿深 2–7 倍,而唯一结构完整的那粒(7154)是**唯一没被回收的那台**。
  **五、⭐⭐⭐ 本轮头号产出:回收的伤害是有方向的,不是随机的。** `mirror_ab.sh` **先跑 ab 腿、跑完再翻面跑 ba 腿** ⇒ **任何一次回收都只砍 ba 腿**。三台被回收的机器 ab/ba 分别是 **26/0、37/5、42/3 —— 三台同一个形状**;唯一自终的是 **29/13**。**三条推论**:(甲) **每粒种子内部的 swap-average 修不了一条几乎没有局的腿** —— 4(i-c) 的恒等式 `arm=(ab+ba)/2` 默认两腿可比,`ba=0` 时它连定义都没有;**抓住这一波的不是 strata 算术,是 GH #269 的深度门**,而这是深度门第一次在**真实抢占**下起作用(此前都是局数分配不均)。(乙) **章程「回收 ⇒ 作废那粒补跑」现在有了机制依据不只是保守做法**:被回收的种子不是"少了点局",是**系统性地少在同一条腿上** ⇒ 并池**会朝一个固定方向偏,不是加噪声**。(丙) ⚠️ **诚实边界:本台主张的是伤害的方向(总是 ba 腿),不是那个偏倚的符号**(那取决于哪条腿是 armed 腿,是判读不是收割)。
  **六、⭐⭐ 闸 (iv) 在收割侧出了声,而它拒答的理由比它能给的答案值钱。** `reclaim_blind.py --wave-json W53_wave.json` ⇒ **`RECLAIM_BLIND_EXIT=2` `UNDECIDABLE`**(**exit 2 = 什么都没检查,不是通过也不是 BLINDED**)。工具两行逐字:`BRACKET VIOLATED -- the changeover constant no longer separates this input; re-derive it before trusting any verdict:` / `seed 7156: paired after only 39.6 min (BEFORE the 40.0 min flip) -- the flip point moved earlier` / `seed 7162: reclaimed at 48.5 min (PAST the 40.0 min flip) yet unpaired -- the flip point moved later`。⭐ **立案的不是这次拒答是那个常数的形状**:`changeover 40.0 min` 是**墙钟常数**,而它代表的事件(ab 腿的局跑干时翻面)是**局数事件** ⇒ 同一波数据**在两个方向上同时**证伪它。**它从 W29 撑到 W52 只因为那 24 波一次回收都没有 —— 从来没有人问过它,而回收正是唯一会问它的场合。**与 GH #580 **同族**:门在、接线在、数据在,**失效在一个从没被写下来的前提上**,且失效时打的是**很容易被读成「没事」的 exit 2**。⇒ 已开 **GH #591**。
  **七、GH #454 两条腿(无论是否触发都报)**:逐机按**各自 AZ** 的现货报价分段计价 + `机时 × $0.005/h` IPv4 作**独立**交叉核对:`2a 0.5200h×$0.2880=$0.1498`、`2b 0.7389h×$0.2728→$0.2687=$0.2011`、`2c 0.6606h×$0.2950→$0.2915=$0.1948`、`2d 0.8075h×$0.2554=$0.2062` ⇒ EC2 **$0.7519** + IPv4 **$0.0136** = **单波实测 `$0.7655`**;交叉核对 `$0.0136/$0.005 = 2.727` 机时与 SIR 推出的总和**逐位相同**。**(乙1)** `$0.7655 > $1.10`?**否**(常数的 **46%**)。**(乙2)** 自终机器机时均值 **`0.7389 h/台`**(**n=1**,只有 7154)`> 1.00`?**否**;⚠️ 另外三台**是被该条款自己排除的**。⭐⭐ **这正是 #454 立案句自己在演**:「一次容量抢占就把总机时压下去、把实测成本压回线下 ⇒ 抢占越频繁常数越不会被复查,而抢占频繁正是市场紧张的时候」—— 三次回收把总机时压到 **2.727h**,于是波费 **$0.77**、**两条腿一条都没响**,而同时**四个 AZ 里三个越过 `$0.231–0.250` 价带**(2c `$0.2950` **+18%**、2a +15%、2b +9%)**且容量自 W29 以来第一次真的咬人**。⚠️ **两条腿按裁定在正常工作**(它们本就设计成不直接看价格);本台登记的是**第四个连续测量点**,四个点讲的是同一件事:**在动的是价格信号,而两条腿都不读价格**。**这是分辨率问题,登记不改常数。**
  **八、发波:本轮不发,门自己说的。** `wave_throttle.py` **`THROTTLE_EXIT=3`**,逐字 `anchor wave : W53 (4 machine(s))` / `last machine up : 2026-09-07T06:22:11.000Z <- the anchor` / `unlock : 2026-09-07T12:22:11Z` / `now : 2026-09-07T09:14:46Z` / `margin : 11245s SHORT of unlock` / `A launch at this instant BREACHES gate (i) by 187m25s.` ⇒ **零发波、零支出**(除 `$0.01` CE)。锚点已自动是 W53,**读的是工具打的那一行**(上一轮交棒 ① 照做)。
  **九、收割义务四条当场结清**:`sir_window_read_first` **DONE**(排在任何 S3 工作之前,四条 SIR 都还在,`update`+`terminated_at` 双写);`gh454_constant_legs` **DONE**(两条腿都报,都不触发);`recover_verdict_per_run_download` **DONE**(四子目录、`pooled_basename_dirs_overridden 0`、`179=179`);`winrate_channel_registered` **DONE**(**DEGENERATE 到极限 0/179**,两个方向都未引用)。
  **十、局数(第 7 步)**:收割型 ⇒ **报实得** `scored 129 / unfinished 0`,四粒 `26+0 / 29+13 / 37+5 / 42+3`(语料 179 局、计分 129 局,差额是被排除的两粒)。预登记 `expected_games ≈ 96`;⚠️ 该量已连续偏低约 75%(W49 168 / W51 214 / W52 214),**本波 129 低于前三波不是「少交付」——那是三次回收**,**两个方向都不许拿它当分母**。
  **十一、泄漏**:`LEAK_EXIT=0` —— 全账号(**不按 tag 过滤**)`running,pending` = **`[]` 空** ⇒ 四台全消失(一台自毁 + 三台被回收)、**无第五台、无留存、无降级成按需的台**;常驻仍只有 AMI `ami-0a990a26d89c66547`。⭐ **外来 `r7a`/`c7a` 连续第三轮 region 内 0 条**。⛔ **GH #515/#528 立案理由一字未变**:工具第一行仍是 `budget filters : none <- so every instance in the account does land in this budget, whoever owns it`。
  **十二、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(只动 `iterations/` 下的报告、`W53_wave.json`、本节)。**铁律 11** 未触发任何 `requires approval`;**本轮没有任何等待** —— 闸 (i) 的 187 分钟是**门直接拒绝发波**,本台按章程转成纯收割轮。
  **交棒**:① ⭐⭐⭐ **总监/协同组 —— W53 没有交出家族读数,需要补跑**:四台三台被回收 ⇒ 只剩 2 粒计分,其中 7156 离深度地板只有 **0.81 局/腿**,而两粒 gpm **反号**(−32.77 / +15.97);按章程「回收 ⇒ 作废那粒补跑,不作废整波」,**7144(NO-PAIR)与 7162(THIN-ARM)已作废,欠一次同串同树的补跑**;⚠️ **7156 是总监的判断不是本台的**(它过了深度门,但它的机器是被回收的,ba 腿是被容量截断的不是按设计跑完的)。② ⭐⭐⭐ **总监 —— 闸 (iv) 的 `changeover 40.0 min` 是墙钟常数,代表的却是局数事件**(六;**GH #591**):同一波数据在**两个方向上**同时证伪它,它撑了 24 波只因为那 24 波零回收,**而回收正是唯一会查询它的场合** ⇒ **补跑波之前重新推导它**,否则下一波只要再被回收闸 (iv) 还会 exit 2,而 **exit 2 不许当通过放行**。③ ⭐⭐ **总监 —— 回收的伤害有方向,这条应进章程**(五):`mirror_ab.sh` 先 ab 后 ba ⇒ 回收**只砍 ba 腿**,三台同一个形状;「作废补跑」由此有了机制依据。④ ⭐⭐ **总监 —— `$1.10` 的第四个测量点到了,而这次是 #454 立案句自己在演**(七);**登记不改常数**。⑤ ⭐⭐ **总监 —— spot 容量在 us-west-2 收紧了**:章程步骤 5 的容量降级阶梯处理的是**发波时拿不到容量**,**没有处理拿到之后跑到一半被收回**;补跑波要不要走按需(~$2.15,headroom `$14.933` 吃得下)是**owner/总监的钱的决定**,**本台不自行升级市场类型**。⑥ ⭐ **录像组 —— 三笔条件 (a) 仍欠**(`campgrade` 第十轮、61-id 家族(W49)、`campbind`/`zusboltdom`(W52));W53 的 `.dem` 也已在 S3,但**它不是一个可判读的波**,不要拿它做条件 (a)。⑦ ⭐ **总监 —— 开工第一条命令的管道坑第十三轮**:工具自己已经会拒答了,**但它拒答之后 harness 报回来的仍然是 `tail` 的 0**,请给独有退出码或 wrapper。⑧ ⭐ **owner —— `$50` 仍越线**(`$63.197`,`AWS state=ALARM`,`forecast $206.77`),外来负载已连续三轮不在但**已花的不退**,预算过滤器仍 `null`。
  **下一轮本台** = **发波轮**(闸 (i) `12:22:11Z` 后解锁 —— ⛔ **不要读这个数,读 `wave_throttle.py` 在发波那一刻打的那一行**,并在发波脚本内**动作处**复读、非零即 `exit 9`):① **本波 = W53 的补跑波**,同 51-id 串、同树,**至少补 7144 / 7162 两粒**(7156 按总监裁定);串没变 ⇒ **与 W53 同家族可并池**,md5 现算复核;② ⚠️ **闸 (iv) 会再次 exit 2**,除非 `changeover` 常数先被重新推导(GH #591),**exit 2 不许当通过放行**;③ 四道闸**全部起飞前跑完**,(iv) 排在 (i) 之前;④ **载体门按 7 项跑**,项数变化要在报告里点名;⑤ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`**(第十四次)。
  详见 `iterations/reports/batch-desk/20260907T091400Z.md` 与 `iterations/reports/batch-desk/waves/W53_wave.json`。
  **⭐ 自检追记(后台跑完之后)**:**`SELFCHECK_EXIT=3`**,工具自己的 `exit sources` 节逐字 `legs run 10` / `FINDINGS (exit 3) : cadence queue-rulings owed-executions` / `UNCERTIFIABLE (exit 2): trunk-red(python)` / `NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py` / `selfcheck worst exit: 3` ⇒ **exit 3 的三个来源全是登记表类的(节奏 / queue 裁定 / owed executions),没有一条是 trunk 红**。第一节那句「Lua 腿不声称」**现在有读数了**:`84 tagged detector file(s), 0 failures` ⚠️ 工具自己写着这是**快子集不是全套**,未打标签的测试不在覆盖内。`UNCERTIFIABLE (exit 2): trunk-red(python)` 指的是**那条腿有两个测试没跑成**(`no lua5.1 on PATH`),**不是 trunk 红** ⇒ **本轮 trunk 两侧都不红**,上一轮的 GH #579 / #584 两条红都不在了。
  **本轮开出的 issue(#290 顺序:三件产物先在 `021c8e97` 推上 main,之后才发表;两份草稿各跑一次 `claim_precheck.sh`,均 `PRECHECK_EXIT=0` `clean`、`local commits not on origin/main: 0`)**:**GH #591 `[harness]`**(闸 (iv) 的 `changeover 40.0 min` 是墙钟常数、代表的却是局数事件,被 W53 一波数据在两个方向同时证伪;`paths cited 5 / resolved 3 / refused 0`);**GH #592 `[batch]`**(W53 三台被回收、没有交出家族读数,欠 7144/7162 补跑;回收的伤害有方向,只砍 ba 腿;`paths cited 3 / resolved 3 / refused 0`)。⚠️ **未新开 GH #454 追评**:本轮两条腿**都不触发**,章程「点名 ⇒ 总监必须重裁」条款按字面未被触发,第四个测量点已登记在 #592 第四节与报告第七节。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0  CLEAN`**(`luacheck bots game: 0 warnings`,门自己装的 luacheck:`installed luacheck (it was missing; apt package lua-check, bounded)`),**未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行**;推送门 `ARM_HOOK_EXIT=0`;**`PUSH1_EXIT=0`**(`claude/great-sagan-8goxp6`,新分支)/ **`PUSH2_EXIT=0`**(`03627372..021c8e97  HEAD -> main`,**未被拒无需 rebase**);动态半(#124)**未跑不声称**。
  **收尾泄漏复检**:发完 issue 后全账号 `running,pending` 仍为 **`[]`**,**`FINAL_LEAK_EXIT=0`**。
  **本轮 token**:`TOKENS total_in=6,075,378 out=54,514 turns=49`(到统计时刻为止)。
- **2026-09-07T12:08–12:3xZ(发波轮:W53 补跑波两台按需起飞 —— ⭐⭐⭐ 头号产出是「放行它的不是闸 (iv),是闸 (iv) 已经没有自由度」,而这句话本身有一个必须一起带走的边界)**
  **一、自检(铁律 10)**:⚠️ **第一条命令第十四轮踩管道坑**(`| tail -40` ⇒ 工具自拒 `REFUSED: ... exit 2, nothing checked`,harness 报回来的 `SELFCHECK_EXIT=0` **是 `tail` 的**);改重定向 + 后台后跑完(本容器约 10 分钟)。**`SELFCHECK_EXIT=3`**,`exit sources` 逐字 `legs run 10` / `FINDINGS (exit 3) : cadence owed-executions trunk-red(python)` / `UNCERTIFIABLE (exit 2): none` / `NOT RUN (inside a leg): tests/test_rc_wrapper.py tests/test_selfcheck_lua_leg.py`。⛔ **trunk python 本轮红,是一条新的红**:`108 passed, 1 failed`,`FAIL tests/test_bots_walk_farm_only.py`,失败行点名英雄组 `17ed802` 带进来的 `tests/test_lion_considere_earlyreturn_domain.lua` 里一条 `io.popen` 未登记进 `UNRESOLVED_HAND_READ` ⇒ **本周第四例同族,门是对的、修法是登记不是改门**(GH #589 逐字同族),本台不写别组代码,已开 issue。锚点 **6/6 OK**(新增 `stable-v6 slotpush f0474dbbcb44`);`163 live gate id` / 18 PROMOTED / **`FROZEN none`** / `promote-atom constraints: OK`;快 Lua 子集 `84 files, 0 failures`(**快子集不是全套**)。
  **二、成本**:`AWS_SETUP_EXIT=0`;`COST_EXIT=0`;MTD **`$64.409`**(免费 `budgets`,`refreshed 12:04:15Z`),≥$35 ⇒ 自动 `$0.01` CE 复核 **`64.4088949308`**,两源逐位一致;`forecast $209.016`。⚠️ 较上一轮 `$63.197` 高 `$1.212`,而上一轮 pending 报 `$0.770` —— ⛔ **不当对账**(步骤 2(乙)禁用该手法);快照 12:04:15Z 距 W53 末台终止 **4.9h**,落在 4.3–11.3h 滞后带内 ⇒ **W53 仍按 `$0.770` 申报 pending**,失效方向朝安全侧。
  **三、⭐⭐⭐ 闸 (iv) 照旧 `exit 2`,而本台没有引用它的任何答案。** `reclaim_blind.py --wave-json W53_wave.json` ⇒ **`RECLAIM_BLIND_EXIT=2`** `BRACKET VIOLATED` / `UNDECIDABLE`;**GH #591 `get_comments` 返回 `[]`**(总监未裁,常数未重推;本台**不自己改那个常数**,上一轮 `aws_run.sh` 越界未裁不叠第二次)。放行本波的是**闸外**的一条规则:这道闸唯一喂养的决定是它文件头写的 `whether the NEXT wave escalates off spot`,而本波机器**已经是按需的** —— 依据 **GH #408**(被回收那台的 SIR 码是 `instance-terminated-no-capacity` ⇒ 补跑机直接 `--on-demand`),**而 #408 的判据是那台机器自己的 SIR 码,一个字都不读 `changeover` 常数** ⇒ 被拒答的裁定**两个分支落到同一个市场**,取的还是**升级后(更贵、数据侧更保守)**的那一支。⚠️ **必须一起带走的边界**:这**不是**「`exit 2` 可以放行」的通则,它成立只因为本波市场被一条**不经过该常数**的独立规则钉死;**下一波只要市场取决于这道闸,常数就必须先被重新推导**。而它现在的形状是**连续两轮拒答却不阻止任何事** —— 与 #454「失效条件与失效原因负相关」同族,**最容易被忘掉**。
  **四、发波**:`7144 i-0e7ed4a3bf4135ea0 / 2a / 12:22:18Z` 与 `7162 i-04aaed93e47e369a7 / 2d / 12:22:23Z`,**逐台 `InstanceLifecycle=None` 核实为按需**(不读 run_id 的 `spot_` 前缀),`instanceInitiatedShutdownBehavior=terminate` 逐台单独查过,2h 看门狗。围栏两条守住:**每粒只补跑一次** + **每波最多 2 台按需补跑(本波额度用尽)**。⭐ **`--ref` 钉 40 位 SHA `523f21ba…` 而不是 `main`,本波是承重的**:`slotpush` 今天 ~10:xxZ 被 promote 成 `stable-v6`,**而它就在这 51 个 armed id 里** ⇒ 用 `main` 发就会把一个「已 promote、不在任何 live gate 里」的 id armed(`pullcad` 坑,静默冻结),并立刻毁掉与 7154 并池的前提;**核实不是假定**:钉住 SHA 的 detached worktree 里 `check_armed_wiring.py --cand` ⇒ **`ARM_WIRING_EXIT=0`**,`all 51 armed ids wired on HEAD`,`slotpush` 在 `bots/FunLib/jmz_func.lua:13439`。臂串 md5 **现算** `e61700f44778dc1ab67166922d22b0bc`(51 id / 460 字节,与 W53 逐位相同 ⇒ 与 7154 同家族可并池)。采购参数 `--slots 16 --hours 2 --rec-slots 8 --games 12` 与 W39–W53 逐字节相同;`--dry-run` 先跑过;AZ `actual == requested` 逐台相同 ⇒ **无 `re-aiming`、无 `AZ RING EXHAUSTED`、零降级**。
  **五、四道闸**:**(iv)** exit 2,处置见三;前置无欠收割是**读**出来的(`s3 ls soak/` 最新四前缀仍是 W53,上一轮 4/4 已收)。**(i)** `wave_throttle.py` 不走管道、**在发波脚本内动作处**跑:决策时 `THROTTLED (exit 3) / margin 623s SHORT`,脚本内有界轮询(20×30s)后**发波那一刻**逐字 `now 12:22:15Z` / `margin +4s past unlock` / **`UNLOCKED (exit 0)`**;⭐ **登记一句诚实的**:补跑机按 **GH #498** 本不受 6h 窗口约束,**本可直接发**,等那 10 分钟是为了让闸自己打 `exit 0`,不让合规性挂在豁免上;脚本对 exit 2 会 `exit 9` ABORT。**(ii)** 补跑买回的是 W53 预算里已付过钱的两粒,臂串与树按构造与 W53 逐位相同。**(iii)** `wave_fence.py --planned 1.35 --pending 0.77` ⇒ **`FENCE_EXIT=0 CLEAR`**,`projected $66.529`;**`$50.00 crossed (AWS state=ALARM)`**、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、ceiling `$80.00`、**headroom `$13.471`**;accrual 行逐字 `CERTIFIED ZERO (0 accruing instances account-wide), but --pending $0.770 was given and is used as the larger claim.`(未用 `--no-accrual-check`)。**本轮未新跨任何告警档。**
  **六、7156 不补跑**:它过了深度门(8.81)但机器是被回收的,**作不作废是总监的判断**(GH #592);且按需补跑额度本波已用尽 ⇒ 即便判作废也落在**下一波**。
  **七、GH #454 两条腿**:(乙1) 无实测(还在跑,下轮结算);(乙2) ⚠️ **按需机没有 SIR ⇒ 本波对这条腿贡献 0 台样本** —— ⭐ **这是立案句换个形状再演**:抢占把补跑推上按需,于是补跑机**连进入这条腿的资格都没有**,**市场越紧张这条腿越没样本**。**第五个连续测量点,登记不改常数。**
  **八、局数**:(a) W53 语料 179 / 计分 **129**(`26+0 / 29+13 / 37+5 / 42+3`,`seeds_scored 2 of 4`);(b) 本波**预登记 `expected_games ≈ 48`**(2 粒 × 12 局 × 2 腿),⚠️ 该量连续偏低约 75%,**两个方向都不许当分母**;首个 S3 对象约在起飞后 ~14.5 分钟出现,**十几分钟内 `OBJ_COUNT=0` 是预期值不是症状**。
  **九、泄漏与点名(发波轮两条)**:`running,pending` 的 `soak-run` 值 **恰好两个**且与 `rerun.machines[].run_id` 逐一对上;`terminated,shutting-down` **返回空**(W53 四台已过 ~1h API 保留期,GH #375 —— 这正是终止码必须在发波轮读的原因,上一轮读过了);常驻仍只有 AMI `ami-0a990a26d89c66547`;⭐ 外来 `r7a`/`c7a` **连续第四轮 0 条**;⛔ 预算过滤器仍 `budget filters : none`(#515/#528 立案理由一字未变)。
  **十、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(只动 `iterations/` 下的报告、`W53_wave.json` 的 `rerun` 块、本节)。**铁律 11** 无任何 `requires approval`、**无空转等待**;⚠️ 有一次**有界等待**(闸 (i) 差 623s ⇒ 脚本内 20×30s,实等约 10 分钟),**那是门在拒绝,不是空转**,且**本可按 GH #498 豁免**,登记在此供总监复核。
  **下一轮本台** = **收割轮**:① ⭐ **第一条命令去读终止码** —— 按需机**没有 SIR**,唯一的码是 EC2 `StateReason.Code`,**约 1 小时后就读不到**(GH #375),按 GH #408 第二半写成 `"market": "on-demand"` + `"status_code": "<EC2 StateReason.Code>"`(**写成 SIR 词汇会被 `reclaim_blind.py` 按 #412 指着骂**);② `recover_verdict.py` 从 S3 逐局全量重算,**两个 run 两个子目录**(`soak/` 不是 `validation/`);③ **并池只跟 7154**,md5 现算复核,**7156 等总监裁**;④ GH #408 验收三样逐台登记(`InstanceLifecycle` 已 `None`、存活分钟、增量账单);⑤ GH #454 两条腿逐条判定;⑥ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`**(第十五次)。
  详见 `iterations/reports/batch-desk/20260907T120800Z.md` 与 `iterations/reports/batch-desk/waves/W53_wave.json:rerun`。
  **本轮开出的 issue(#290 顺序:三件产物先在 `6c8c4f67` 推上 main,之后才发表;草稿各跑一次 `claim_precheck.sh`,均 `PRECHECK_EXIT=0` `clean`)**:**GH #596 `[bug]`**(trunk python 红:`test_bots_walk_farm_only.py` 被英雄组 `17ed802` 带进来的一条未登记 `io.popen` 打红,**门是对的、修法是登记不是改门**,本周第四例同族);**GH #592 追评**(补跑波已发,市场问题由 GH #408 覆盖不需新裁,7156 仍欠裁但不再挡路);**GH #591 追评**(闸 (iv) 第二轮拒答且**没有阻止任何事**,建议重推常数或给它「连续 N 轮 UNDECIDABLE ⇒ 硬阻断」)。
  **铁律 6 读数**:静态门 **`GATE_EXIT=0 CLEAN`**(0 warnings,门自己装的 luacheck),**未用 `RULE6_BYPASS`**;`ARM_HOOK_EXIT=0`;`PUSH1_EXIT=0` / `PUSH2_EXIT=1` ⇒ `REBASE_EXIT=0` ⇒ **`PUSH2B_EXIT=0`**(`5d6c9f1b..6c8c4f67 HEAD -> main`);动态半(#124)**未跑不声称**。**收尾泄漏复检 `FINAL_LEAK_EXIT=0`**,全账号 `running,pending` = **恰好本波两台**,`soak-run` 值与 `rerun.machines[].run_id` 逐一对上。
  **本轮 token**:`TOKENS total_in=5,158,507 out=25,120 turns=40`(到统计时刻为止)。

- 2026-09-07T15:14–15:5xZ(**收割 W53 的两粒补跑,波补到 3/4 粒,头号 gpm 从 −8.40 走到 −6.80。但真正的产出是收割途中量出来的三件事:GH #591 卡了三轮的常数第一次被直接测出、`stayfield2` 在今天的 main 上已是死 gate 而 wiring 门报它 WIRED、ab 腿在从未被抢占的机器上系统性是 ba 的两倍。本轮零发波、零新增 EC2 支出**)。
  **一、自检(铁律 10)**:⚠️ **开工第一条命令第十五轮误用管道**,脚本自拒(harness 报回的 `EXIT=2` 是 `tail` 的)。改重定向 + 后台跑完:**`selfcheck worst exit: 3`**,`legs run 10`,`FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(lua)`,`UNCERTIFIABLE (exit 2): trunk-red(python)`。**trunk python `109 passed, 0 failed, 2 uncertifiable`** ⇒ 上一轮开的 **GH #596 那条红本轮不在了**(⚠️ 只读到「现在是绿的」,**归因不是本台的读数**)。⛔ **trunk Lua 红:`4 of 84`**(`test_gated_helper_nesting_census` / `test_stayfield2_marginal_domain` / `test_stayfield_callsite_domain` / `test_stayfield_hp_window_reach`),前两轮都是 0。⭐ **一次自己抓住的方法错误,登记它因为失效方向是「假通过」**:复核时先打了 `lua5.1 tests/test_X.lua` ⇒ **四个全 `EXIT=0`、零输出**,差点写成「自检误报」;真相是那样跑什么都没跑 —— 自检那条腿跑的是 **`lua5.1 tests/run_tests.lua <basename>`**(`routine_selfcheck.sh:536`),改用同一条命令 ⇒ **四条 `RUNNER_EXIT=1`,都真红在 trunk**(`git rev-parse HEAD origin/main` 同为 `0e0dd45f`,`bots/`/`tests/`/`game/` 一行未改)。`owed-executions` 的 `campgrade_wave_type_premise_reading`(#480/#241)**executor 仍含本台,仍欠着**。
  **二、时限第一件事:退出码第三次 `UNREADABLE`,而这次连「窗口」都不成立。** 上一轮把它写成交棒第 1 条,本轮**作为第一条 AWS 命令执行**:`describe-instances --instance-ids i-0e7ed4a3bf4135ea0 i-04aaed93e47e369a7` ⇒ `DESC2_EXIT=0` + **`Reservations: []`**(空表不是 `NotFound` ⇒ 空表本身是证据),全账号过滤同样 `[]`。**算术**:起飞 12:22:18/12:22:23Z,末次 S3 上传 13:03:28/13:15:31Z ⇒ 两台在 **14:14Z 前**已死透,GH #375 的 ~1h 窗口在本轮 15:14Z 触发前就关了。⭐ **上一轮钉的是「~2h 节奏抓不到」;本轮触发间隔 ~3h,更抓不到** ⇒ **没有任何排班能修它**,只有「机器自写 `shutdown.json`」能修,那件事 09-05 交总监至今未裁。两行如实填 `UNREADABLE` 并把算术+命令写进 `W53_wave.json:rerun.status_code_outcome`,**不填推测的码**。
  **三、收割**:臂串**亲手从钉住的树 `523f21ba` 重算**(`git show <pin>:test_set.md | sed -n 2p`)⇒ **51 id / 460 bytes / md5 `e61700f44778dc1ab67166922d22b0bc`**,与发波登记值逐字节相同。两 run **各下到自己的目录**(R2 防同秒 basename 撞车);**三个 override 一个都没用 ⇒ 没有「这是 SKIP 不是 pass」的行要抄**。`7144` **RV_EXIT=0** 48 局 ab27/ba15 **depth 19.29** gpm −18.88;`7162` **RV_EXIT=0** 64 局 ab40/ba18 **depth 24.83** gpm +31.24 ⇒ **两粒都过 GH #269 深度门,`thin_arm_seeds` 空**,被容量抢走的两粒种子带着深度回来了。
  **四、W53 完整读数(3/4 粒)**:池 = 两粒补跑 + 干净存活的 7154,**同树同臂串** ⇒ ⛔ **波内补完,不是 GH #363/#449 的跨波并池分叉**;**7156 不在池里**(GH #592,归总监)。`RVPOOL_EXIT=0`,160 loaded / **142 计分** / 0 unparseable / source_dirs 3。四量(每粒 swap-average 再跨粒算术平均,**永不按局加权**):**gpm −6.80 / xpm −3.04 / deaths +0.07 / last_hits −1.17**,`comps_better` **四量皆 1/3**。⛔ **下游任何按上一轮 `−8.40` 定过范围的东西都是按「两粒 ba 腿被截断的半个波」定的。** **分层登记(4(i-a),照登不结论)**:gpm `ab −67.30 / ba +53.69 / side −60.50` flip `side>arm 3/3`;xpm `−27.71/+21.63/−24.67` flip 2/3;deaths `+0.80/−0.66/+0.73` flip 3/3;last_hits `+11.50/−13.85/+12.68` flip 2/3 —— 按 4(i-c) **反号是恒等式不是否决**。⚠️ **诚实边界**:`−6.80` **不是变好的证据**,逐粒 `−18.88/−32.77/+31.24`(极差 **64.1**)对 GH #30 零点(σ≈30,n=3 ⇒ SE≈17)落在噪声里;站得住的只有 2(b) 那句**「gpm 上没有明显负面」**,而 deaths 与 last_hits **仍在有害侧**。**胜率 `DEGENERATE` 且是极限值**:少数侧 **0/160**,普查 **radiant 0 / dire 160** ⇒ 不得作 2(b) 支持(GH #352);连同 W53 自己的 179 局,**339 局连续 finished 局 radiant 一局没赢**。**条件 (a) 仍是录像组的棒且仍欠。**
  **五、⭐⭐⭐ GH #591 的常数,本轮被直接量出来了(本台不改常数,只交测量)。** `RECLAIM_BLIND_EXIT=2` 照旧 `UNDECIDABLE`,#591 仍零评论。**方法**:读本波自己的 `script_version` 戳分 ab/ba 腿,翻面 = 首个 ba **开局** − 起飞,墙钟 = `wall_s` 中位数,预测首个 ba 结果 = 两者之和,与**实际**首个 ba 的 S3 上传时刻对。四台:翻面 **26.98 / 27.10 / 27.75 / 27.80**(**27.4 ± 0.4**,极差 0.82),墙钟 **12.18**(四台**两位小数逐位相同**),预测 39.17/39.28/39.93/39.98 vs 实际 **39.18/39.07/39.70/39.52**。⇒ **常数不是一个数,是两个可测项之和 = 39.6 ± 0.4**,写死的 `40.0` **坐在带的上沿**,那正是源源不断产出「paired BEFORE the flip」的形状。**`BRACKET VIOLATED` 两条腿各有解释、一条都不用动常数**:腿1(7156「39.6 就配对」)—— 它 5 局 ba 上传在 **39.70/39.90/40.02/40.10/40.60**,**全部晚于它自己的 39.70**,落在抢占通知后的 **~2 min 宽限窗**里,**模型里没有宽限窗这一项**;腿2(7162「48.5 已过翻面却未配对」)—— **它并没有未配对**,自己那行写着 `ab42/ba3`,`recover_verdict` 标它 **THIN-ARM**(depth 5.60<8)而 `reclaim_blind` 标 **NO-PAIR**,**两个工具对同一份输入打架**,PAIRED 列把 `ba==0` 与 `ba>0 但不到门` 混成一格。**建议总监**:不要把 40.0 当自由参数重拟合,改成每波从该波语料自推 + PAIRED 列拆 **NO-PAIR / THIN-ARM / PAIRED** 三档。
  **六、⭐⭐⭐ `stayfield2` 在今天的 main 上是死 gate,而 wiring 门报它 WIRED。** 逐源码两棵树对比(`grep -rn ShouldRegenNotWalkHome bots/`):钉住的 `523f21ba` 上定义 `jmz_func.lua:5830` + **真实调用点 `mode_retreat_generic.lua:236`(1 个)**;今天的 `origin/main` `0e0dd45` 上 **调用点 0 个**,只剩两处注释。那一行被换成了 `if J.ShouldLetTpChannelFinish(bot) then`(另一个候选 `pgchannel`),**而它上面那段注释原文未改,仍写着「GATED on soak candidate 'stayfield2'」**。⇒ **它在本波的 51-id 串里,而它的谓词没人调用。** 而 `check_armed_wiring.py --cand '<51-id 串>'` 对它报 **`stayfield2 direct 1 jmz_func.lua:5831`** —— 它数的那个 "site" **就是 helper 自己体内的 `IsSoakCandidate('stayfield2')` 那一行** ⇒ **一个没人调用的 helper 和一个活的 helper 在这个工具眼里一模一样**。这是章程 `pullcad` 那一课的**新形态**(且更进一步:连"调用点"数的都是 gate 表达式自己)。⚠️ **两条独立路径同指此处**:trunk 上红着的 `test_gated_helper_nesting_census.lua` 的 FAIL 文本**预先写好了这句话**(「arming the outer id ALONE measures a no-op and check_armed_wiring.py will still call it WIRED」,点名 `pgchannel`/`tpwatch` 这条新 gate 套 gate),`test_stayfield_callsite_domain.lua` 的红逐字是「the stayfield2 call left GetDesireHelper」;**两条都红着、都没人分诊**。⭐ 门确实抓到了另一个:`ARM_WIRING_MAIN_EXIT=1`,**`slotpush: UNWIRED … DO NOT LAUNCH`**(今晨 promote 成 `stable-v6`,预期内)⇒ **门抓得住「gate 表达式消失」,抓不住「被 gate 的 helper 失去最后一个调用者」**。本台**未改任何 bots/ 代码**。
  **七、⭐⭐ ab 腿系统性是 ba 的两倍,而这不是抢占造成的。** 上一轮把 ab 偏重归给抢占截断;极端值(ba 0/3/5)确实是,**但基线本身已经歪了**:**从未被抢占**的三台 —— `7154`(spot 跑完自终)ab29/ba13、`7144`(按需跑完)ab27/ba15、`7162`(按需跑完)ab40/ba18,比 **2.2 / 1.8 / 2.2**。**机制**(`mirror_ab.sh:collect()`):它在 `n >= TARGET` 那一刻翻面,**但 16 个 slot 在飞** —— ab 开腿时 16 个同时起跑,单第一批就超过 `TARGET=12`,60s 轮询注意到之前第二批已起跑 ⇒ ab 溢出到 2–3 倍;ba 开腿时 slot 已错开,只轻微溢出。**代价可算**:`arm_depth` 是调和平均由细腿决定 —— 7154 的 42 局买到 17.95,同样 42 局若 21/21 买到 **21.0(+17%)**;7162 的 58 局买到 24.83,29/29 买到 **29.0(+17%)** ⇒ **约每七局丢一局**。**二阶效应**:翻面落在 ~27 min 而机器一生 ~40–53 min ⇒ **ba 腿只占最后 15–25%**,**第 30 分钟之后的任何抢占按构造只砍 ba 腿**(W53 抢占在 31.2/39.6/48.5 min ⇒ ba 0/5/3)。本台**未改 harness**(`aws_run.sh` 越界未裁,不叠第二次)。
  **八、成本与闸 (iii)**:`COST_EXIT=0`,九月 MTD **`$64.409`**,≥$35 ⇒ 自动 `$0.01` CE 复核 **`64.4088949308`** 两源逐位一致,`forecast $209.016`。⚠️ **三个数与上一轮逐位相同,`budgets refreshed` 也逐字相同(`2026-09-07T12:04:15Z`)** ⇒ 两源都还停在上一轮那一刻,12:22Z 起飞的两台按构造不在里面,按 (乙) **不做增量对账**。闸 (iii) 走工具:`FENCE_EXIT=0` `CLEAR`,limit `$100` / actual `$64.409` / **`$50` 档 crossed(state=ALARM)** / `$80`·`$100` 未跨 / fence **`$80`** / brake `$90` / ceiling **`$80`**;带诚实 pending(`--pending 2.02` = W53 的 `$0.770` + 两台按需 `$1.25`)⇒ projected **`$66.429`**,headroom **`$13.571`**,仍 CLEAR。⚠️ **顺带查明**:不带 `--pending` 时工具打 `CERTIFIED … pending $0.000 is a reading, not a default`,**而那个 reading 只覆盖"此刻仍在计费状态里的实例"** ⇒ **一波在两次 budget 刷新之间起飞并结束的机器,既不在 MTD 也不在 pending**,正是本轮与上一轮的现场;两轮都是**靠人手**把它抬进来的,**失效方向是「可以多花」**。⭐ **花 `$0.01` 拉的逐日 CE 值这个钱**:09-01→09-07 = `4.471 / 4.198 / 4.116 / 5.800 / **26.398** / **18.411** / 1.014`,EC2-Compute 同形 ⇒ **九月 `$64.4` 里 `$44.8` 烧在 09-05 与 09-06 两天**,而本台的波每波 `$0.75–2.15`(09-07 一整天 EC2 才 `$0.948`,还含 W53 整波)⇒ **`$50` 那道 owner 可见告警主要不是本台的波跨过去的**;预算过滤器仍 `null`,**本台的围栏替别人的花销挨刹车**。
  **九、发波:不发,而且是有理由的判断。** 闸 (i) `THROTTLE_EXIT=0` **UNLOCKED**(锚点 W53 末台 `06:22:11Z` ⇒ 解锁 `12:22:11Z`,`margin +10839s`,两读法同结论);闸 (iii) `CLEAR`;**闸 (iv) `exit 2` UNDECIDABLE** ⇒ **两过一拒答,不发**。理由是**上一轮自己写下的那句**本轮正好命中:「`exit 2` 可以放行**不是通则**,它成立只因为那波市场被 GH #408 从闸外钉死;**下一波只要市场取决于这道闸,常数就必须先重推**」—— 而 W54 正是那一波(补跑额度已尽,唯一悬着的 7156 归 #592)。**本台把重推它所需的测量交上去了(第五节),裁定归总监。** ⭐ 另**即便闸 (iv) 现在放行也不该照抄那条臂串**:`slotpush` 已 UNWIRED(工具报 DO NOT LAUNCH)、`stayfield2` 是死 gate 但工具不报 ⇒ **修臂串是发波前置,不是发波后的注记**。
  **十、泄漏:干净。** `LEAK1_EXIT=0` 全账号(**不按 tag 过滤**)`running,pending` **空**;`LEAK2_EXIT=0` `describe-volumes status=available` **空**;常驻仍只有 AMI `ami-0a990a26d89c66547`。**本轮未启动任何实例。**
  **十一、队列**:`QUEUE_ROWS_CHANGED=27`。⭐ **格式先钉死再改**:改前先验 `json.dumps(indent=1) == 原文` ⇒ **`ROUND_TRIP_IDENTICAL: True`**(第一次用 `indent=2` 写出来是 **2000 插入 / 1998 删除**的整档重排,**当场丢弃重做**)。改后 **29 插入 / 27 删除**,并做**结构化审计**(解析后逐字段比,不靠肉眼看 diff):`rows with any field changed: 27` / **`rows where something OTHER than result changed: 0`** / `done-list untouched: True`。27 行 = bundle 与 51-id 串有交集且 status 非 `done`/`rejected` 的全部行,每行 `result` 追加完整波读数并**明写「用 −6.80 不要用 −8.40」**+ 噪声边界 + `comps_better` 全 1/3 + 胜率 DEGENERATE 不得引用 + **条件 (a) 仍欠**;**`status`/`director`/`bundle`/`seeds` 一律未动**。
  **十二、铁律 6/11**:`bots/`/`game/`/`tests/` **本会话一行未改**;静态门与推送读数见报告文末追记;动态半(#124)**未跑不声称**;`tests/test_wave_gate_keys.py` 对改后的 W53 记录**当场跑过 `WGK_EXIT=0`(307 checks, 0 failed)**。**铁律 11** 未触发 `requires approval`,**无空转等待**(唯一等待是自检后台跑完,期间做完收割、并池、常数测量、wiring 核对、成本与三道闸、队列与报告主体)。
  **交棒**:① ⭐⭐⭐ 总监 —— **GH #591 本轮拿到了它要的测量,请裁**(常数 = 27.4 + 12.18 = 39.6±0.4;两条腿各有解释,一条都不用动常数);② ⭐⭐⭐ 协同组/总监 —— **`stayfield2` 在 main 上是死 gate 而 wiring 门报它 WIRED**,它在当前臂串里;③ ⭐⭐⭐ 总监/harness —— **trunk Lua 四条红**,附带「`lua5.1 tests/test_X.lua` 对红文件静默 exit 0 零输出」这个假通过陷阱;④ ⭐⭐ 总监/harness —— **`mirror_ab.sh` 的 ab 腿溢出到 ba 的两倍**(未被抢占的三台实测 1.8–2.2),且让抢占伤害**按构造有方向**;⑤ ⭐⭐ 总监 —— **`wave_fence.py` 的 pending 漏掉「两次刷新之间起飞并结束」的整波**,失效方向是「可以多花」;⑥ ⭐⭐ owner/总监 —— **九月 `$64.4` 里 `$44.8` 烧在 09-05/09-06 两天,不是本台的波**,预算过滤器仍 `null`;⑦ ⭐⭐ 总监 —— **退出码第三次 `UNREADABLE`,没有排班能修**,`shutdown.json` 09-05 交出至今未裁;⑧ ⭐ 总监 —— #592 / GH #454(按需机对乙2 腿贡献仍为 0,登记不改常数)/ `aws_run.sh` 越界仍未裁;⑨ ⭐ 录像组 —— **条件 (a) 仍欠**(27 行队列请求已补上语料读数);⑩ ⭐ 本台 —— `campgrade` 独占波仍不存在,那一棒仍欠着。
  **下一轮本台** = ① 读 #591 裁定;② **发 W54 前先修臂串**(`slotpush` 移出、`stayfield2` 核实调用点)—— **这是前置不是注记**;③ 读 #592,若入池则出 4/4 粒家族读数;④ 结两台按需机的账单侧实测;⑤ ⚠️ 开工第一条命令重定向 + 后台(第十六次)。
  **本轮开出/追评**(push 后发表,#290 顺序;四份草稿 `claim_precheck.sh` 全 `PRECHECK_EXIT=0` `clean`):**#591 追评**(`changeover` 常数的直接测量 + 两条腿的解释)、**#600 `[bug]`**(`stayfield2` 死 gate,wiring 门报它 WIRED,它在当前臂串里)、**#601 `[bug]`**(trunk Lua 四条红 + `lua5.1 tests/test_X.lua` 静默 exit 0 的假通过陷阱)、**#602 `[harness]`**(ab 腿溢出到 ba 的两倍)、**#603 `[batch]`**(`wave_fence.py` 的 pending 洞 + 九月逐日 CE 归因)。**推送**:`PUSH1_EXIT=0`、`PUSH2_EXIT=0`(`0e0dd45f..8c85062d HEAD -> main`),推送钩子当场跑了静态门并放行。
  **本轮 token**:`TOKENS total_in=9,522,670 out=73,351 turns=71`(到统计时刻为止;此后的收尾回合不计入)。
  详见 `iterations/reports/batch-desk/20260907T151400Z.md` 与 `iterations/reports/batch-desk/waves/W53_wave.json`(`rerun.status_code_outcome` / `rerun.harvest` / `rerun.pooled_reading` / `rerun.changeover_measurement` / `rerun.ab_leg_overshoot` / `rerun.cost` 六节)。
- 2026-09-07T18:13–18:5xZ(**不发波轮:W54 的每一条前置本轮全部结清,于是闸 (iv) 第一次成为唯一的承重墙 —— 它已连续三轮拒答、连续两轮真的挡住一波、四轮零总监回应,而它要的测量上一轮已经交了。本轮零发波、零新增 EC2 支出(唯一支出:`$0.01` CE 复核)**)
  **一、自检(铁律 10)**:⚠️ **开工第一条命令第十六轮踩管道坑**(`2>&1 | tail -60` ⇒ 逐字 `REFUSED: ... stdout is a pipe; exit 2, nothing checked`,harness 报的 `SELFCHECK_EXIT=2` **是 `tail` 的**)。上一轮交棒逐字写的是「重定向 + 后台 + 不设短 `timeout`」,**三件里犯了第一件**;改重定向 + 后台跑完,真码见文末追记。⭐ **本轮登记一句方法论**:这个坑**连续十六轮**在同一位置发生,而**每一轮的交棒都写了「下轮别踩」⇒ 提醒的成功率是 0/16,提醒不是修法**;修法是让它不可能被打错(开工那条命令只以 `rc.sh` 形式出现,或包一层不接受输出重定向的 wrapper)。**不是本台能单方面改的量具,交总监。**
  **二、成本**:`AWS_SETUP_EXIT=0`;`COST_EXIT=0`;MTD **`$64.409`**(免费 `budgets`,`refreshed 12:04:15Z`),≥$35 ⇒ 自动 `$0.01` CE 复核 **`64.4088949308`**,两源逐位一致。⛔ **三个数与快照时刻均与上一轮逐位相同 ⇒ 账单侧滞后已达 6.2h,本轮零新信息。** ⭐ **由此上一轮交棒第 4 条(结两台按需机的账单侧实测)本轮仍买不到,而理由是结构性的、可提前算出**:快照 **12:04:15Z** 比那两台起飞 **12:22:18Z/12:22:23Z** 还**早 18 分钟** ⇒ 按构造不可能出现在这个 MTD 里,**不是"再等等",是必须等下一次刷新**。`pending` 维持 **`$2.02`**。⚠️ **不当对账的诱惑再次出现并被拒绝**:MTD 没动**不等于**那两台没花钱,失效方向朝少报,故 `pending` 只增不减。
  **三、按需机终止码:第四次 `UNREADABLE`,本轮起永久买不到。** `describe-instances --instance-ids i-0e7ed4a3bf4135ea0 i-04aaed93e47e369a7` ⇒ **`[]`**;两台 12:22Z 起飞、2h 看门狗 ⇒ 约 14:22Z 终止,本轮读它 18:14Z,**滞后 3.9h** 远超 ~1h 保留期 ⇒ **`[]` 是"记录已过期"不是"没有码"**(GH #375 立案句:一个 `[]` 长得像好消息)。⭐ **掉棒的不是疏忽是排班**:12:08Z 轮把它写成"下一轮第一条命令",而下一轮(15:14Z)距终止已 ~53min 贴着窗口边,再下一轮彻底出界 ⇒ **~1h 窗口 / 2h 机器 / ~3h 触发间隔,没有排班能修**;正解是机器关机前把码写进 S3(`shutdown.json`,09-05 交出至今未裁)。
  **四、⭐⭐⭐ W54 发波前置本轮全部结清(上一轮交棒第 2 条:「这是前置不是注记」)。** (1) **臂串换代 51 → 48**:`git show origin/main:test_set.md | sed -n 2p | tr -d '\n'` 现算 ⇒ **48 id / 433 bytes / md5 `51eb44347f25676ec5bf767383eed244`**(id 数用 `split(',')` 数**不用 `wc -l`**,无尾换行会少一个);对 W53 的 51-id 串做**集合差**(非肉眼):**移出 3**(`slotpush` promote 成 `stable-v6`;`teambrain`/`capmono` 是总监 §FV 16:28Z 两条**退回出集**,⛔ 都不是 reject,gate 与代码逐字保留)、**新增 0**(P4.2 冻结仍在执行) ⇒ **W54 与 W53 不同家族不可并池,现在就登记免得收割轮再判**。(2) **`slotpush` 的 `DO NOT LAUNCH` 已消失**(总监 §FV 顺手结清,归属登记清楚,不是本台做的)。(3) ⭐ **`stayfield2` 死 gate 已在 main 上修好**:`git grep -n ShouldRegenNotWalkHome origin/main -- bots/` ⇒ **`bots/mode_retreat_generic.lua:236` 非注释调用点回来了**(总监 `7c44d7ca` 16:28Z);协同组在 #600 回评定性为**误伤不是退役**,根因是 `8b25217e` 用**替换**写法落 `pgchannel`、两条否决函数体逐字相同 ⇒ **一行 diff 同时新增与删除一条否决**。(4) `check_armed_wiring.py --cand '<48-id>'` ⇒ **`ARM_WIRING_EXIT=0`**,`all 48 armed ids wired on HEAD`。⚠️ **诚实边界**:这个 `WIRED` **不比上一轮的 `WIRED` 更值得信**(同一工具对 `stayfield2` 那次也打 `direct 1 WIRED`);本轮敢说它活着靠的是 (3) 的 `git grep` **直读调用点**,不是这一行。**#600 第 2 条的 python 半仍欠**,Lua 半协同组已落地 `tests/test_gated_helper_liveness.lua`(66 个带闸 helper / 恰好 2 条豁免 / 变异台 7/7 CAUGHT)。
  **五、⭐⭐⭐ 交回协同组的终审:14:01:25Z–16:28Z 窗口内零发波,`stayfield2` 的删除作废 0 粒种子。** 协同组在 #600 明确把它交回本台终审。四条**独立**证据:① S3 `soak/` 最新前缀是 `spot_20260907_122216_…`/`…122221_…`(那两台按需补跑机;`spot_` 只是 run_id 前缀,市场以 `InstanceLifecycle` 为准),**之后再无前缀**;② `wave_throttle.py` 家族普查只列 `W53`(末台 06:22:11Z)与 `Y1`(09-05),**无家族末台晚于 12:22Z**;③ 全账号 `running,pending` **空**,闸 (iii) 独立打 `CERTIFIED ZERO`;④ 15:14Z 报告与章程均记为零发波。⭐ **而让这条终审防弹的不是时间线是那个 pin**:即便窗口内真有波起飞也不受损 —— 在飞两台的 `--ref` 钉的是 40 位 SHA **`523f21baf14a03cadfb5df7002ba7124d70375c2`**,而 #600 自己的两棵树对照表逐字写着该树 `mode_retreat_generic.lua:236` **有**调用点 ⇒ **删除只发生在 `main`,没有任何在飞的波从 `main` 取树**。「钉 40 位 SHA 不钉 `main`」这条纪律**第二次承重**(第一次是 12:08Z 挡住 `slotpush` 的 `pullcad` 坑)。**这一棒到此结清。**
  **六、⭐⭐⭐ 闸 (iv):第三轮拒答,第二轮真的挡住一波,四轮零总监回应。** `RECLAIM_BLIND_EXIT=2` / `BRACKET VIOLATED` / `UNDECIDABLE` **与前两轮逐字相同**;GH #591 `get_comments` 返回 **2 条,两条都是本台自己写的**(12:31:41Z / 15:38:50Z),GH #592 返回 **1 条也是本台的** ⇒ **总监四轮零回应**,而 #591 要的常数**上一轮已量出并交出**(翻面 27.4±0.4 + 墙钟 12.18 = 39.6±0.4,`40.0` 坐在带的上沿;两条腿各有解释、**一条都不用动常数**)。**不发 W54**:W54 是全新 slate,其市场取决于这道闸,而 12:08Z 预登记的边界逐字是「下一波只要市场取决于这道闸,常数就必须先被重新推导」;W53 补跑额度已尽,**GH #408 那条闸外规则不再覆盖**。⛔ 本台**不自己改常数**(归 #591),**不叠第二次越界**(09-05 `aws_run.sh` 越界至今未裁)。⭐ **本轮的新事实不是"它又拒答了"而是它的形状变了**:12:08Z 拒答**但没挡住任何事**(市场被 #408 从闸外钉死)→ 15:14Z 拒答**第一次真的挡住一波** → **本轮拒答挡住第二波,而此前与它并列的每一条理由本轮全被拆掉** ⇒ **它现在是唯一承重墙**,不再有"反正还有别的原因不发波"这层遮蔽。**本台已连续两轮无法发波。**
  **七、闸 (i)/(iii) 读数,及一处登记而不修改的张力**:**(i)** `THROTTLE_EXIT=0` **UNLOCKED**,锚 W53 原 slate 末台 `06:22:11Z` ⇒ 解锁 `12:22:11Z`,`now 18:16:24Z`,`margin +21253s`。⚠️ **该锚不含那两台补跑机**——那是该文件 **RULING 2** 的**故意**排除(`rerun` vs `machines[]`,理由「a repair machine is not a new wave with its own ~$2.15」)。⭐ **本台先怀疑它是 bug,读源码 docstring 后自己撤回了怀疑 —— 登记这一步,因为差一点多开一个假 issue。** 但张力是真的且**今天可测**:按**最后一台真实计费机器**(补跑机 `12:22:23Z`)解锁应为 `18:22:23Z`,而本台跑它是 `18:16:24Z` ⇒ **THROTTLED 还差 ~6 分钟**,**两种读法恰好骑在决策边界两侧**;而 **RULING 3**(总监 09-05,GH #534)自己立的原则逐字是「gate (i) bounds MONEY, so **anything that put a billed machine up opens a window**」,那两台**是计费的**(`$1.25`),按这句应当开窗,RULING 3 却同时写「`rerun` is still excluded: ruling 2 is untouched」⇒ **两条裁定在这一点上互相拉扯**,失效方向是**可以更早发波 = 更早花钱**(与 #534 立案时那个 3h 缺口同向,量级 3h → 6min)。本台**不改这个量具**,已在 #534 追评。今天实际代价 **6 分钟且本轮无论如何不发波 ⇒ 零后果**。**(iii)** `wave_fence.py --planned 0.85 --pending 2.02` ⇒ **`FENCE_EXIT=0 CLEAR`**,`limit $100 / actual $64.409 / pending $2.020 / planned $0.850 / projected $67.279`;**`$50` crossed (AWS state=ALARM)**、`$80`/`$100` 未跨 ⇒ fence **`$80.00`**、brake `$90.00`、ceiling **`$80.00`**、headroom **`$12.721`**;accrual 行逐字 `CERTIFIED ZERO (0 accruing instances account-wide), but --pending $2.020 was given and is used as the larger claim.`。**本轮未新跨任何告警档。** ⛔ `budget filters : none` **逐字未变**(#515/#528/#603 立案理由一字未变)。
  **八、泄漏:干净。** 全账号 `running,pending` **返回空**(不只"本台零台",是**全账号零台**);闸 (iii) 的 accrual 腿**独立**打 `CERTIFIED ZERO` ⇒ **两条独立路径一致**;⭐ 上一轮点名的外来 `Project=final-table-trainer`(`i-0a7d4c7fbeb3a6712`)**本轮已不在**,`r7a`/`c7a` 连续第五轮 0 条;常驻仍只有 AMI `ami-0a990a26d89c66547`。**`LEAK_EXIT=0`,无泄漏无掉臂。**
  **九、本轮不做的事(登记免得被读成遗漏)**:不发 W54(判断不是没轮到)、不改 `reclaim_blind.py` 常数(归 #591 且不叠越界)、不改 `wave_throttle.py`(裁定间冲突归总监)、不写 `bots/`(`stayfield2` 已由总监修好本台只核实)、不重开 `shutdown.json` 的 issue(09-05 已交只再点名);⭐ **`campgrade_wave_type_premise_reading`(#480/#241)连续第八轮欠着**,理由仍是钱与闸 (iv),**本台自己的欠账,照登不辩解**。
  **十、铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**(改动只在 `iterations/`);**铁律 11** 全程无 `requires approval`、无权限对话框、**无空转等待**(自检后台跑的整段时间用于成本、终止码、臂串核对、四道闸与泄漏检查)。静态门与推送读数见报告文末追记;动态半(#124)**未跑不声称**。
  **下一轮本台** = ① ⭐⭐⭐ **先读 GH #591**:已裁 ⇒ **闸 (iv) 恢复自由度,W54 当轮就发**(前置已结清,但**臂串必须当轮现算 md5 复核,不许抄本条**);仍未裁 ⇒ 第三轮被同一条挡住,**按铁律 9 写进 DECISIONS_NEEDED 向 owner 说明**;② 结两台按需机的账单侧实测(快照仍是 `12:04:15Z` 则 `pending` 继续按 `$2.02`,**不许因 MTD 没动就写成"没花钱"**);③ 读 #592(7156 入不入池),入池则出 4/4 粒家族读数;④ 读 #534 追评有无裁定 —— 若裁 `rerun` 开窗,**闸 (i) 的锚会后移,发波时刻要重算**;⑤ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`(第十七次)**。
  详见 `iterations/reports/batch-desk/20260907T181300Z.md`。

- **2026-09-07T20:5x–21:3xZ(发波轮:**W54 四台 spot 起飞,连续三轮的停摆结束** —— 解开它的不是裁定,是一次测量:闸 (iv) 拒答的那个常数**对本波的市场没有任何自由度**)**
  **闸 (iv) 本轮第四次逐字 `RECLAIM_BLIND_EXIT=2`,#591 第五轮零总监回应**(3 条评论全是本台自己写的)。本轮**没有停在"再报一次"**,而是把「这道闸的拒答到底锁住了什么」从推理换成测量,得到三条读数:
  ⛔ **① 那个常数在 W53 上不可满足** —— 真工具、未改一字,`--changeover-min` 从 **10 扫到 120 分钟共 17 个取值,17 次全部 `exit 2`**。两端约束由工具自己打出:`c=39.6` 打 `seed 7162: reclaimed at 48.5 min (PAST the flip) yet unpaired` ⇒ **c ≥ 48.5**;`c=48.5` 打 `seed 7156: paired after only 39.6 min (BEFORE the flip)` ⇒ **c ≤ 39.6**。`48.5 > 39.6` ⇒ **可行集为空** ⇒ **#591 四轮来要的「重推常数」,推出任何数都清不开这道闸**。
  ⭐⭐⭐ **② 它的裁决在整个定义域上恒为 NOT BLINDED** —— 变异台(**从文件拷贝建台,只删 bracket 短路;出货工具零 diff**)同区间全部 `not blinded -- the wave delivered 2 paired seed(s)`。机制**读源码**:`classify_machine()` **收了 `changeover_min` 形参但函数体从不读它** ⇒ `paired_n` 与常数无关;常数唯一的消费者是归因子句,**在 `paired_n=2` 时不可达**(归因计数在扫描里真的从 0 动到 3,始终没进那条分支)。
  ⭐ **③ 两项分解一旦落地,bracket 立刻可满足** —— 真工具 `--min-arm-depth 5.0`(**探针,不是降低深度门的提议**;深度门是 GH #269 的 promote 侧硬门)⇒ 可行区间 **`[31.3, 39.6]`**,15:14Z 量出的 **`39.6±0.4` 落在带内**,写死的 **`40.0` 在带外**。
  ⇒ **本波市场不取决于这道闸**(其裁决函数在常数全域都选 spot,而 spot 也是 owner GH #158 常设默认)。⛔ **本台没有把 `exit 2` 当通过**:`W54_wave.json:gates.gate_iv_reclaim_blind` 逐字写 `THIS WAVE DOES NOT CLAIM THE GATE PASSED`;**结论输入相关,不随波次继承** —— 下一波收割时重跑重测,`paired_n ≤ 1` 则常数立刻恢复承重。
  **W54 发波**:臂串**本轮现算** **46 id / 417 字节 / md5 `cd3b46d4e3ca4a446272b330a09a2751`**(`split(',')` 数,不用 `wc -l`),与 W53 的 51-id 串 **−5/+0**(`slotpush` promote 成 `stable-v6`;`teambrain`+`capmono` §FV 退集;`l1trade`+`l5combo` §FW 19:xxZ 退集,**四条都不是 reject**)⇒ **新家族,不可与 W52/W53 并池**;P4.2 冻结继续成立(**新增 0**)。term 由臂串导出 `7 term(s)`,与 W53 逐字相同(**退出的两个 id 都是 generic,动不了 hero term**)。`seed_roster_index --build` 先跑(`ROSTER_EXIT=0`,237 粒),窗口右移 **`[7501,7900]`**,**与索引交集是空集**,选 **7527/7529/7719/7789**,`CARRIER_GATE ids=10 seeds=4 exit=0`,每 term 载体 ≥2。四道闸:(i) `THROTTLE_EXIT=0` `UNLOCKED` `+31956s`(**跑在 `spot_run.sh` 前最后一条**);(ii) 新东西可测 = main `523f21ba→0d62930` + 臂串两次换代 + 本家族累计种子 0;(ii') `ARM_WIRING_EXIT=0` `all 46 armed ids wired`,**无 `DO NOT LAUNCH`**;(iii) `WAVE_FENCE: CLEAR (exit 0)`,`--pending 2.02` **ASSERTED**、`--planned 0.85`,围栏 **`$80.00` 现算**,**余量 `$12.721`**;(iv) 见上。`git ls-remote` 读两次(脚本对漂移写死 `exit 9`),**`--ref` 传 40 位 SHA `0d62930936c6d5415efa4fa20f54e4c5a002f646`**。**四台 `InstanceLifecycle=spot`,零按需**(证据是 `InstanceLifecycle` 不是 run_id 前缀),SIR 四行**发波轮第一手读到**(GH #556)。
  ⚠️ **AZ 拓扑 `2a/2c/2c/2d` —— 三个 AZ 不是四个**:`us-west-2b` 报 `InsufficientInstanceCapacity`,#256 的环改投 2c(**环内改投不是市场降级**,机型/市场都没动),但 **GH #252 的验收标准逐字是「四个互不相同」** ⇒ **GH #460 那个形状,轻一档**。**预登记后果**(发波时登记不留到收割):**一次 2c 容量事件带走两粒不是一粒**,而既有补跑围栏(每粒一次 / 每波最多 2 台)仍够用 ⇒ **未杀波,理由是算术:杀波赔 `$0.85` + 一轮,买回的只是一个已被围栏覆盖的尾部**。处置追认交总监。
  **成本**:MTD **`$64.409`**(免费 budgets;CE 复核 `64.4088949308` 逐位一致),⛔ **快照 `refreshed 2026-09-07T12:04:15Z` 与前两轮逐位相同 ⇒ 账单侧滞后 9.2h,本轮无新信息**;两台按需补跑机**按构造**不可能在这张快照里(快照比它们起飞早 18 分钟)⇒ `pending` 维持 **`$2.02`**,**不许因 MTD 没动就写成没花钱**。本波预估 **`$0.85`**。**泄漏:发波前 running/pending 空;发波后账户内恰好四台全属 W54**;常驻仍只有 AMI `ami-0a990a26d89c66547`。
  **铁律 9**:#591 是**同一条裁定请求的第五轮零回应**且此前真的挡住两波,已按新题目追评;**本轮不写进 DECISIONS_NEEDED**(它本轮已不再阻塞产出,写进去等于把一个已消失的停摆报给 owner),**若下一轮仍零回应则下一轮写**。
  **铁律 6/11**:`bots/`/`game/`/`tests/`/`tools/` **本会话一行未改**;全程无 `requires approval`、**无空转等待**。⚠️ **开工第一条命令第十七轮踩同族坑,而本轮是"改对了一半"**:没用 `| tail -60`,用了 `2>&1 | tail -40`,自检**当场自拒** `REFUSED: stdout is a pipe; exit 2, nothing checked` ⇒ **十七轮提醒零成功率,提醒不是修法**(第二次交总监/harness)。
  **下一轮本台** = ① ⭐⭐⭐ **第一条命令读 W54 四行 SIR 的 `Status.UpdateTime`** 填进 `machines[].update`(**闸 (iv) 读的就是这个键**;SIR ~1–3h 过期,过期就永久买不到,GH #375/#556);② ⭐⭐⭐ **在 W54 自己的记录上重跑闸 (iv) 并重测**(不变性**输入相关不继承**);③ 收割走 `recover_verdict.py` 从 S3 全量重算,照抄 `winrate_channel` 与 `mean.winrate_headroom`;④ 2c 双押的兑现检查;⑤ 账单侧:快照若仍停在 `12:04:15Z`,`pending` 记 `$2.02 + $0.85`;⑥ ⚠️ **开工第一条命令重定向 + 后台 + 不设短 `timeout`,`2>&1 | tail` 也算管道(第十八次)**。
  详见 `iterations/reports/batch-desk/20260907T211500Z.md`。
  **收尾追记(泄漏复检抓到的)**:账户级计数打回 **3 不是 4** ⇒ 逐台核对,**seed 7527 那台(`i-0f54672e00323252c`,us-west-2a)起飞 96 秒后被抢占**(`21:14:49Z → 21:16:25Z`,SIR `closed` / `instance-terminated-no-capacity`,EC2 `Server.SpotInstanceTermination`),**零局落盘、不可能翻面 ⇒ 该粒 spot 臂配对差作废**。⭐ **SIR 码在发波轮第一手读到(事件后 ~19 分钟),~1–3h 过期窗这次根本没派上用场 —— 能读到它靠的是当轮复检不是排班。** **当轮补跑已发**(章程步骤 5 / `OWNER_PRIORITIES` 回收处置是**事先登记**的处置):`i-041a2044ddd3741b7`,us-west-2a,`21:35:27Z`,**`InstanceLifecycle=null` ⇒ 按需**(证据是这个字段,run_id 的 `spot_` 前缀是硬写字符串不是证据);**依据是 GH #408 的输出不是本台的选择**(被抢那台的 SIR 码是容量码 ⇒ 补跑直接按需,不走阶梯);`--ref` 与 slate **同一个 40 位 SHA 不是 `main`**(按 `main` 发会毁掉并池前提,W53 在 #592 登记过这个形状)。**围栏**:每粒只补跑一次(7527 用掉)、每波最多 2 台按需补跑(**已用 1**)、闸 (i) 不开新窗口也不受窗口约束(GH #498 ③)、闸 (iii) **自己过了一遍** `WAVE_FENCE: CLEAR (exit 0)`(`--planned 0.63 --pending 2.87`,投影 `$67.909`,**余量 `$12.091`**)。⚠️ **同时是 GH #460/#252 的新观测点**:发波那 15 秒里 `2b` 直接拒容量、`2a` 给了机器又 96 秒收回,而环早把这一波的解相关**花在了唯一活下来的 2c 上** ⇒ **真正在跑 spot 臂的拓扑是 `2c/2c/2d`,两个 AZ 不是四个**。**收尾泄漏复检:账户内 4 台全属 W54(3 spot + 1 按需),无孤儿无外来负载。** **⇒ 下一轮第一条命令升级**:读四行 SIR 的 `UpdateTime`,**外加按需补跑机的 EC2 `StateReason.Code`**(2h 看门狗 ⇒ 约 `23:35Z` 自终,**约 `00:35Z` 后永久读不到**,GH #375,连续第四波这条读数处在危险里);**7527 那台的码已读到写进记录,不必再读**。
