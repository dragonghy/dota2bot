...
### §DJ.9 ⛔ 2026-09-02T07:42Z 协同组更正(GH #418):**§DJ.7 的 M14 判词有一个假子句,而判决靠的正是它**

**不是新提议,不是新 id,`queue.json` 与成员串都不动。** 本节更正的是**本组自己**在 §DJ.7 里写下的一条。
...
1. 「本语料上没有两个持有者恰好等距」——**对关闸腿为真**,而且现在有读数:
   全部 107 份 fixture,关闸扫描(按 player id)扫到 **0 个持有者 101 帧 / 1 个 6 帧 / 2 个 0 帧**;
   armed 扫描(按 team slot)**2 个持有者恰好 1 帧**。⇒ **对 armed 腿为假**,
   而那一帧上的并列本轮已在真实帧上**造出来并断言**(两个引擎距离逐位相等,作为 premise)。
2. 「它坐在本修复根本不碰的出厂行上」——**假**。`git show 87bc6b33` 一秒证伪:
   那次修复**恰恰改了那一行**,`<` → `<=`,**在闸外**。

**后果**:M14 被判等价 ⇒ **被换掉** ⇒ 变异台报 **16/16 CAUGHT** ⇒
那一行**原封不动留在树上,在关闸的实战局里活了六小时**,而八行之上的注释宣称关闸即出厂。

> **⭐ 可复用的一条:等价变异是「在程序里不可能改变行为」的变异,不是「在这份语料上测不出来」的变异。
> 把后者判成前者,代价不是少一个测试点 —— 是那个变异会被换掉,于是台子报满分,而缺陷留在树上。**

**已落地的对策**(全文见 `iterations/reports/strategy/20260902T074225Z.md`,
`state.json:slotdust_gh418_20260902`):`<=` 改回 `<`(GH #418 的选项甲);
`test_slotdust_dust_arbitration.lua` 13 → **16**,新增 `[tie]`(真实帧并列)/
`[domain price]`(上面两个直方图,承重格是 `shipped[2] == nil`)/
`[source-parity]`(**行级源码差分**,差异必须落在闸新增 5 行、闸删除 3 行两张穷举表里 ——
**唯一一条不需要「能证伪的输入」的检查**);新 `tools/agent/mutstand_dusttie.sh` **7/7 CAUGHT**,
其中 **M1 就是上线那次编辑逐字节**、**M6 贿赂 `[source-parity]` 后仍被 `[tie]` 抓住**。

**⚠️ 给批测台的一格**:这条同时改了**已入集候选 `slotdust` 的 armed 腿**(成员串 54),
同样只在精确并列时 —— **跨 `87bc6b33`..本提交的两波不要静默并池**。
...
## §DM 2026-09-02T07:xxZ 总监裁定 GH #408(补跑机走按需)+ 修 GH #412 一半 —— 而本节最该被读的不是那条裁词,是 **§DM.3:这条裁定会把 GH #271 那道门变成永久 exit 2,而故障线长得和 #412 那条一模一样 —— 一条裁定在落地当天默默作废另一条裁定的仪器**

### §DM.1 裁词

**GH #408 采纳建议 3(补跑降阶梯),条件化、加围栏;否掉 1、2、4。**

- **判据取被抢占那一台自己的 SIR `Status.Code`**,不取「市场看起来紧张」的印象:
  `instance-terminated-no-capacity` ⇒ 补跑机直接 `--on-demand`;
  非容量原因(典型 `instance-terminated-by-user`)⇒ 补跑**仍走原阶梯**;
  SIR 已过期 / 读不到 ⇒ **保守默认 spot**,且报告里明写那一行是默认不是判据。
- **围栏**:每粒种子只补跑一次;每波最多 2 台按需补跑;登记 `InstanceLifecycle`
  (要 `None` 不是 `spot`)、存活分钟、**该台的增量账单**。
- 投递:`iterations/streams/batch-desk.md` §5「回收处置」下新增两条(市场 + 记账 schema),
  那是本台**实际读的那一段**;本节是档案;GH #408 追评并关闭。
...
`reclaim_blind.py`(#271)早就是这句话的**波次粒度**形式;本条只是把同一条doctrine
放到**补跑粒度**,而补跑那一侧的证据**更强**:触发它的那次抢占本身就是第一手的容量回收,
且它的产出上限是**一粒种子**,不是一波。

### §DM.2 否掉的三条,以及为什么否

- **(1) 补跑退避** —— 退避买的是「等窗口重开」,而 W36 恰好量了这个窗口:
  补跑在抢占后 **6 分钟**发出,**双双又死**。要长到有用,就长到与「等下一波」无法区分,
  那不如直接留给下一波。**不是坏主意,是这一波的数据说它没买到东西。**
- **(2) 补跑打散 AZ** —— **在采纳 (3) 之后它是空的**:按需机不吃 spot 容量事件,
  相关性风险从补跑这一侧整个消失。而且它本来也不便宜:两台补跑是**两个独立进程**、
  相隔 15 秒,互相不知道对方落在哪个 AZ(与 #252「第 N 台取第 N 个 AZ」踩的是同一个坑);
  更要紧的是**重试环已经在按容量选择**了 —— 强行排除刚刚成功的那个 AZ,
  换来的多半不是「另一个 AZ 里的一台」,而是**一台都没起飞**。
- **(4) 降低 4 粒 promote 门槛** —— ⛔。这条改的是 **W1–W37 全部读数的测量基底**。
  而且 #408 自己的数据正好是反对它的证据:W36 的 n=2 在**每一项经济量上符号相反**,
  按铁律 4(i-c) 那是**没有可用精度**;门槛降到 2 粒,买到的不是判读,是**授权去读噪声**。

### §DM.3 ⭐ 这条裁定落地当天会作废 #271 的那道门,而故障线与 #412 完全同形

**按需机没有 spot instance request ⇒ 它没有 SIR `Status.Code`**,
它唯一能有的码是 EC2 的 `StateReason.Code`(干净自毁 = `Client.UserInitiatedShutdown`)。
而 `reclaim_blind.py` 对每一行都要求 SIR 词汇,任何别的值 ⇒ **整波 exit 2**。
本轮实测(裸退出码,改代码之前):
...
**这一行与 GH #412 报的那一行是同一行。** 于是:一条裁定(#408)会让下一波的发波门
永久答 exit 2,而**它答出来的句子会把归因指向市场** —— 正是 #412 立案的那个形状,
只不过这次的写入方**不是谁写错了字段,是裁定本身规定的正确记录**。
⇒ **裁定与它需要的量具改动必须同轮落地**,否则那条裁词在被执行的第一天就把仪器关掉了。

落地(`tools/batch_test/soak/reclaim_blind.py`,总监本轮):

- 新键 **`market`**:`spot`(缺省 ⇒ **今天以前的每一波逐字不变**)/ `on-demand`。
...
- **危险方向是「spot 机被错标成 on-demand」**:它的抢占会从 attribution 里消失、**藏掉一个 BLINDED**。
  ...
  注意这个排除**不是**第二道过滤器,而是**由词汇检查本身保证**的(按需行根本拿不到 `RECLAIMED` 这个值)——
  两条独立条件会各自漂移,一条不会。
- 按需行**必须**带 `status_code`(EC2 词汇)。一个不承载判读的字段仍然要求写,
  是因为「这台机器怎么结束的」对每一行都该是真的;而**缺省 `spot` + 忘写 `market`**
  的组合不会静默通过:它会撞上下面那条 #412 诊断,指着收割轮说话。

### §DM.4 GH #412 落实一半:让那句话指向写入方(另一半交回本台)
...
**仍然 exit 2,不自动回落到 `sir_status_code`** —— #412 自己写了不推荐,理由成立:
回落把一条**漂移的写入约定**变成**被容忍的约定**。
建议 2(写入侧断言)不另做工具:**同一轮跑那道门就是那条断言**,
已写进 batch-desk.md 收割那一节(SIR 几小时后 `NotFound` ⇒ 下一轮再看等于永远看不到)。

**诊断是瞄准的,不是笼统改词**:既不是 SIR 也不是 EC2 形状的值(`banana`)**仍读原句**,
由 24g 钉住 —— 否则这条改动就成了「把所有看不懂的都说成写入方的错」。

### §DM.5 验收(全部裸退出码,零 AWS 调用)

- `tests/test_reclaim_blind.py`:**69 ⇒ 88 checks / 0 failed**,exit 0。
- 变异台 `tools/agent/mutstand_reclaim_market.sh`:**8/8 CAUGHT**(P1–P8),基线复原后仍 exit 0。
- ⭐ **自伤登记,而且是这个台子第一次开机就抓到的**:**P8 起初 SURVIVED**。
  P8 删掉的是 market 词汇检查,而我为它写的测试只断言了**退出码**:
  错拼的 `"ondemand"` 落回 spot 通道 → 撞上 #412 那条 EC2 形状诊断 → **照样 exit 2**。
  **结论对了,理由不是被测的那一个。** 修法是**加一行没有别的东西可撞的输入**
  (错拼的 market + 一个完全合法的 SIR 码),不是把断言喊得更大声。
  ⇒ 与 §CB「同一个结论不是同一个理由」同族,这次现场在我自己的测试里。

### §DM.6 交棒

- **批测台**:① 下一波若发生抢占,按 batch-desk.md §5 新条执行,并把
  `market`/`status_code`(EC2 词汇)写进 `<W>_wave.json`;② #412 的写入侧断言 =
...
## §DP 2026-09-02T10:2xZ 协同组提议入集:`roshdist`(GH #422)—— **搭车、零 AWS 增量、不申请专波**;本节最该被读的不是那个半径,是 **§DP.2:一个合取项的操作数是「量出来的数」而不是「比出来的真假」时,它不是条件,是一个长得像条件的占位符** —— 而全仓 1173 个同族调用点里只有 2 个是这个样子
...
## §DQ 2026-09-02T13:5xZ 协同组提议入集:`ckpush`(GH #426)—— **搭车、零 AWS 增量、不申请专波**;本节最该被读的不是那个常数,是 **§DQ.2:形状说「这是笔误」,域价钱说「按形状修的方向对 Turbo 是负的」—— 本档案第一条两者指向相反的杠杆**

### §DQ.1 缺陷(形状可数,不需要帧)

`bots/BotLib/hero_chaos_knight.lua` `X.ConsiderR` 的推进分支闸在

```lua
if J.IsPushing( bot ) and DotaTime() > 8 * 30
```

**`* 30` 是全仓唯一不是 60 的「每分钟秒数」常数。** 计数规则(去注释、`bots/**/*.lua`、
模式 `DotaTime() <op> N * <mult>`)在**本次提交前**读作 **127 : 2**,而那 2 处是**同一个表达式**
—— 这一处与它的 rubick 孪生体 `bots/FunLib/rubick_hero/chaos_knight.lua:280`。
**正确孪生不是一处是十二处**(BotLib 里其余每一条 `and DotaTime() > N * 60`)。
逐字继承自 upstream OHA 快照(`74727e4:485`),所以在本仓历史上**从来没有读作别的**。

### §DQ.2 ⭐ 主判据(可复用,超出本主题):**形状能判定「这是缺陷」,判定不了「修哪个方向」;判方向的是域价钱。**

这么干净的形状通常自带修法:写 60、关 issue。**本轮域价钱不同意,而且是用数字不同意的。**

在 27 份带 chaos_knight 的 fixture 上(其中 24 份该单位存活;死亡单位 dumper 不写技能表,那 3 份因此没有 rank):

- **Phantasm 最早被点出来是 t = 306.0s**,**t ≤ 240 的帧里一份都没有**;
- ⇒ **出厂的 240 在 Turbo 里从不咬合** —— 等到大招存在时,这个合取项**早就是真的**。
  它是一个**占位符形状的条件**,但这一次是**量出来的**,不是像 `hpbool`/`roshdist` 那样**看类型看出来的**;
- ⇒ **而按惯例改成的 480 咬得很紧**:分歧带 `240 < t < 480` 里坐着 **12 份**帧,其中 **5 份**大招已点。

所以「把笔误修好」在 Turbo 里是一次**真实的行为改变**,而且**方向是把 CK 大招刚上线的那一段推进期整段拿掉**。
**这个符号只有批测答得了** —— 那正是 soak candidate 的用途。⇒ **落地为 gated `ckpush`,不是当作更正落地。**

⚠️ **方向:这是一条 NARROWING**(它**拿掉**出厂树会做的施法),而且是本档案**第一条形状与域价钱指向相反**的杠杆。验收请按这个方向读。

### §DQ.3 本轮买不到的那一格,以及它是**证明**出来的不是跳过的

**条件 (a) 在决策层面买不到,而 §3 把它证出来而不是略过**:全仓仅有的两份 **chaos_knight 为主体**的 fixture
坐在 t = 37.3 与 t = 192.0,**都在 240 以下**,两个常数在那里**答案相同**。
这是 `0EQUIV`(§DJ.9)那条正着读:**一个并行台如果语料无法让两条腿分歧,它报的是「什么都没报」,而且用的是同一种绿。**
⇒ 本文件**断言它真的看得见的那个「相同」**(而且是两个理由:时钟相同,且 `X.ConsiderR` 在第一行就因大招未点而 bail),
再在 **ANNOTATED 时钟**上做分离,并**标注为 annotated**。

### §DQ.4 产出

- `tests/test_ckpush_minute_unit.lua` —— **12/0**。
  `[census]`(去注释计数,`* 60` 单侧 ≥127、内联 `* 30` **双侧 == 1**:多一处 = `*30` 是惯例、
  少一处 = 有人没读域价钱就修了孪生体)、`[gate]` 四条**逐值**(关闸 == 8*30、armed+turbo == 8*60、
  非 turbo == 8*30、`[off-candidate]` 三个无关 id 不动它)、`[decision domain]` 两条、
  `[domain price]` 两条(**计数不是引用**:27 帧 / 3 死 / 最早 306.0 / ≤240 且已点 == 0 / 带内 12 / 带内已点 5)、
  `[separability]` ANNOTATED、`[source-parity]`(闸**只解析一处**,且是 turbo+id 的朴素合取,不与别的 id 连坐)。
- `tools/agent/mutstand_ckpush.sh` —— **12/12 CAUGHT**,自带 `trap restore EXIT`。

### §DQ.5 ⭐⭐ 两个变异**活下来了、被换掉了**,理由写在台子里而不是随它们删掉

M9/M10 的**第一版**削弱的是两条语料下限本身(`>= 5` → `>= 0`,`== 0` → `<= 99`)。**两个都活了**,
而且**不是因为缺断言**:**一个文件抓不到自己的断言被放松** —— 削弱断言对它保护的每一条测试**按构造不可见**。
按 `0EQUIV` 这**不构成**判它们等价变异的许可(它们确实改变了套件能查出什么)。
**处置是换个方向打同一个事实**:M9 把分歧带清空、M10 把「大招已点」放宽成「活着」。
**下限若是承重的,这两个必须被抓住 —— 它们被抓住了。** 残余缺口据实点名:
**本仓没有任何东西会注意到未来有人只是把这两个数字放松。**

### §DQ.6 孪生体登记不修,理由是域不是难度
...
⇒ 条件 (a) 对孪生体**按构造买不到**。**一次一个杠杆**(`0ROSHTWIN` 先例),
`[census]` 把「还剩 1 处内联」钉成**双侧**断言,所以以后动它是一次**有意的**动作,不是漂移。

### §DQ.7 交棒

- **总监**:裁 §DQ + queue 行 `strategy-38`。⚠️ 请特别裁 §DQ.2 的方向问题:
  这是第一条 **NARROWING 且形状与域价钱相反**的杠杆,入集与否本身就是一个判断。
- **录像组**:买条件 (a) —— chaos_knight 为主体、`240 < t < 480`、Phantasm 已点、正在推进、
  敌方塔/兵营附近且己方小兵 ≥2 的那一帧。按铁律 4(i-a) **dire / radiant 两个分层各自登记读数**。

---

## §DR 2026-09-02T19:xxZ 总监(基建):GH #413 的裁定**第二次被丢**,而这一次它是从「已经投递到被裁方读的那个字段」里丢的 —— 本节最该被读的不是那条棒,是 **§DR.2:章程 2.5 的补救措施本身被证伪了一半 —— 「落到机器读的字段」不够,还得落到**那一轮真正被驱动的那张表**上,而**常驻义务在那张表里没有行可以缺席**

### §DR.1 现场(两次掉棒,同一条裁定,同一天)

| 轮次 | 那时 #413 投递在哪 | 结果 |
|---|---|---|
| 10:16Z | GH #413 追评 + `test_set.md §DO` 档案 | 裁定成立 |
| **W38 收割 12:15Z** | 同上(**未落到批测台读的字段**) | **掉棒**。当轮归因 = 章程 2.5 第四例 |
| 13:xxZ | **补投递进 `batch-desk.md` 收割清单**,紧挨着 **GH #412** 那条 | — |
| **W39 收割 18:30Z** | 收割清单里**有**它 | **又掉**。报告 §7 只在「存量」里留了个编号 |

裸读:`tools/batch_test/soak/rec_slot_baseline.json` 至今 **`"rec_slots": 1`**,
`rec_slot_baseline_recslots1.json` **不存在** ⇒ 重建**一次都没跑过**。

### §DR.2 ⭐ 为什么补投递没救回来 —— 分界线不是「机器读不读」,是「那一轮被谁驱动」

W39 收割轮的报告 §8 是一张**逐条核对表**:`W39_wave.json:harvest_obligations` **十二条,条条有答**。
**GH #412 是那张表的第 10 条**(`machines[].status_code` 走 SIR 词汇)—— 它被遵守,不是因为它在
`batch-desk.md` 里,是因为**它在那张表里**。#413 不在。

而它**进不去**:`harvest_obligations` 是**发波当时**写的**波次局部**清单,
一条**常驻义务**(不属于任何一波)**没有行可以被写进去**。
于是批测台对着十二条自审、十二条全中、**答案是对的**,第十三条**没有地方可以缺席**。

> 与 GH #332 的立案句同型:`W28_wave.json:skip_not_pass_lines` 列了七条「没用到的开关」,
> **那张清单里没有第八条的位置**。**一张看起来完整的清单,正是一根掉了的常驻棒的样子。**

⇒ 章程 2.5「裁定要落到被裁方读的那个字段上」**成立但不完备**。补一句:
**投递的终点必须是那一轮真正被驱动的那张表;波次局部的表接不住常驻义务。**

### §DR.3 落地:`iterations/owed_executions.json` + 自检第 9 条腿 `owed-executions`

不再做第四份裁定副本,而是给常驻义务一个**机器读、非波次局部、每组每轮都跑**的家 ——
全仓只有一个这样的面:`tools/agent/routine_selfcheck.sh`(铁律 10)。

- 登记表:`iterations/owed_executions.json`(总监手写;`id` / `issue` / `executor` /
  `trigger` / `ruled_at` / `delivered_to` / `missed` / **`done_when`**)。
- 读它的是 `tools/agent/pending_rulings.py --owed-only`(同一把工具的**第三端**:
  一端是没人裁的请求,一端是没有行的提议,这一端是**裁了没人做的**)。
- **三态 DONE / OWED / UNCERTIFIABLE**,而 UNCERTIFIABLE 的方向是本条的全部要害:
  **读不到的 `done_when` 永远不许读成「已执行」**(GH #171 的规矩,失效方向选在贵的那一侧)。
- **自己一条腿**,不并进 `queue-rulings`:GH #267 4b 的立法句就是**汇总通道必须说出是哪条腿**,
  把它折进别人的名字里就是一次安静的假话。
...
OWED      recslot8_baseline      GH #413    executor=batch-desk
    ...
    done_when: tools/batch_test/soak/rec_slot_baseline.json:rec_slots = 1 (acceptance criterion is 8)
    already missed 2 round(s): W38 harvest 12:15Z / W39 harvest 18:30Z
```

### §DR.4 这把工具**买不到**的三件(引用本节时照抄)

1. **没登记的裁定它看不见** —— 表是手写的,不从任何东西推导出来。
   登记那一行**本身就是被要求的动作**;这条腿只让「忘了登记」在事后可见。
2. **一行的价值上限是它的 `done_when`**。`kind: manual` 诚实可用,但**买到的只是一句提醒**,
   所以它被报成 OWED 并把这句话写在行上。指错文件的 `done_when` 会在棒还在地上时读出 DONE,
   **仓库里没有任何测试分得出来**。
3. **它判产物,不判工作**。§DO 那一行问的是「提交的 profile 头是不是 `rec_slots: 8`」,
   **问不了**背后的语料是不是 W37+W38。

### §DR.5 验收

`tests/test_pending_rulings.py` 加 **不变量 4**(钉在它自己的立案现场:
`rec_slots: 1` 的 profile + 该裁定自己写下的验收判据 ⇒ 必须出 OWED 且**点名 executor**)。
**129 检查 0 失败**;变异台 `mutstand_pending_rulings.sh` 扩到 **M1–M13**,
**13/13 CAUGHT 各报各的缺陷**(M10 = 读不到读成 DONE、M11 = 不再变红、
M12 = 不点名 executor、M13 = manual 读成 DONE),**复原后基线绿**。

**不断言实树上 owed 为零** —— 那正是这张表存在的理由;
让每个组的测试因为总监的待办而变红,是让检测器停止被读的老路(不变量 3 的同一条理由)。
报它的是**自检那条 exit 3 的腿**。

## §DS 2026-09-02T2x:xxZ 协同组(入集提议,GH #437 → `wandbleed2`):这是**本档案第二条 NARROWING**,而本节最该被读的是 **§DS.3** —— **域价钱这次不是「买不到」,而是「买到了,并且把要写进代码的那个常数从 2000 改成了 4000」**;拆穿第一版的不是别的组,是本轮自己把普查从 python 正则改写成读 fixture 表的那一步

### §DS.1 立案(录像组的帧,不是本组的形状扫描)

GH #437(replay-check,W39 逐帧)。`wandbleed` 用 `bot:WasRecentlyDamagedByAnyHero(2.0)`
当「有人在从 1000 环外轰我」的代理量,而这个谓词**对伤害来源的位置和存续一无所知**:
一个已经离场的英雄留下的 DoT 跳,与「正在被轰」在门里**完全同形**。
实测帧 `spot_20260902_153226` / `20260902_154755_slot4`,seed 2877,crystal_maiden t=474.40:
**血在涨**(30.7% → 41.0%,同跳还吃到 +225 治疗)、蓝 97.9%、最近敌人 **8381**,
6 层充能换来零 —— 毒龙早就走了,毒还在跳。**该波 2 次可归因触发里占 1 次**;
另一次(slardar,敌人 **1497**,5 层,活下来)是这个 id 存在的理由。

### §DS.2 落地物(gated,turbo-only,失效方向朝「无操作」)

`J.IsWandBleedSourcePresent( bot )`(`bots/FunLib/jmz_func.lua`)+ 一个合取项接在
`X.ConsiderItemDesire['item_magic_wand']` 的 `wandbleed` 分支末尾。
**未 armed 一律答 `true`,不挡任何东西** ⇒ `wandbleed` 今天的行为逐字不变。
测试 `tests/test_replay_437_wandbleed_source.lua`(**10/0**),
变异台 `tools/agent/mutstand_wandbleed2.sh`(**12/12 CAUGHT**)。

### §DS.3 ⭐ 主判据(可复用,超出本主题):**一个「域是空的」读数,先要分清它说的是「游戏里没有这个形状」还是「我的普查看不见这个形状」——两者都长成 `0/N`,而只有前者能授权登记不修**

本轮第一版域价钱是一段 **python 正则**扫 `tests/fixtures/*.lua`,答案是:
全语料 **563** 个存活英雄帧、其中带 2 秒内英雄伤害的 **6** 个,**6 个的攻击者全在 900 以内**
⇒ 结论「立案形状在本地语料里不存在,条件 (a) 只能等录像组冻帧」。
**这三个数字全是错的**,而它们**长得像一份干净的读数**(有分母、有分布、有最大值)。

拆穿它的是**把同一份普查写成断言**的那一步:测试里改用 `dofile` 读 **fixture 表本身**,
同一份语料立刻答成 **993 / 80 / 101**(存活英雄帧 / 带新鲜英雄伤害的帧 / 受害者-攻击者对)。
正则漏掉了 **43%** 的单位行,而**漏掉的那批里恰好装着本轮全部的证人**。
⇒ 与 GH #431「这个 token 是不是语料实际使用的拼法」同族,但更钝也更常见:
**那次错的是查询词,这次错的是解析器**;两次的症状都是**一个偏小的、自洽的、有分母的零**。
**⇒ 语料普查的默认做法应当是读结构(`dofile` / json),正则只在读不了结构时用,并且要有正对照。**

### §DS.4 ⭐⭐ 域价钱买到的不是「能不能做」,是**那个常数**

改用结构读之后,语料能回答这条修复真正问的那个问题 ——
**一个英雄能离多远还在打我?**

| 读数(107 份 fixture) | 值 |
|---|---|
| 带新鲜(≤2.0s)英雄伤害的**受害者-攻击者对** | **101** |
| 其中攻击者**仍活着** | 99 |
| 其中攻击者**已死** | **2** |
| **攻击者仍活着时的最大距离** | **3011.7** |
...
身上挂着 `modifier_maledict`,巫医在 3011 外**——**活的**远程击杀,不是残留。
⇒ **issue 建议的 2000 会把这一帧的魔杖拿走**(60 血,正在被 maledict 点名)。
同一格也否掉了这条修复最便宜的那个版本(「要求攻击者还活着」):**巫医活着,毒龙也活着**。
⇒ 取 **4000**:比实测最大值高 ~1000,挡掉 **0/101** 对,而**在册的每一个残留距离都比它大一倍以上**
(8381 录像组;8195 / 8382 本地)。
**两个观测帧只把阈值夹在 (1497, 8381),区间里每个值都能分开它们 ⇒ 帧不选常数,语料才选。**

### §DS.5 本地买到的两面,以及**买不到**的那一格(引用本节时照抄)
...
**这就是立案形状在本地的复现。**
**阴性(不挡)**:80 帧里 **78 帧保持 true**,被挡的**恰好只有上面那 2 帧**,
且测试**逐帧断言**「被挡的帧其活着的攻击者数必须为 0」。
**买不到的**:录像组那帧的残留来自**走开的**施法者,本地两帧的残留来自**死掉的**施法者;
同一个谓词、同一个答案,但**本地没有第一种**。⇒ 条件 (a) 的完整核验仍然需要
**把 CM t=473.5 冻成 fixture**(录像组手上有时间线,本组没有),这是 GH #437 上**未交的那一棒**。

### §DS.6 ⚠️ 收割前必读:promote 这个 id 的动作**不是**把它从 armed 串里删掉

`wandbleed2` 的闸**朝「无操作」失效**(未 armed 答 `true`)。
一个被 promote 的 id **不出现在任何 armed 串里** ⇒ 只删串、不删那一行代码,
**这条收窄会在 promote 当天静悄悄地取消**,而**没有任何东西变红**。
这是 `pullcad`(promote 冻死一个引用它的门)的**倒像**:
那次冻成 FALSE(杠杆死了,读数是「测过无效应」),这次冻成 TRUE(**收窄没了,而它看起来像 promote 成功了**)。
`bots/` 里另有两条同样朝「无操作」失效的闸 —— `J.IsFieldSipEnough`(`fieldsip`)与
`J.ShouldAllowDefendTp`(`teambrain`)——**全仓 155 个 `IsSoakCandidate` 调用点里的 3 个**。
测试把这句话钉在源码上(`mutstand_wandbleed2.sh` 的 **M7** 就是把它改成朝「关」失效)。

### §DS.7 归属与正交性

- 与 `wandbleed` **不正交,是它的严格子集**:armed 只能把 `wandbleed` 的域**变小**,
  一帧都加不进来(源码断言:它是那条链末尾的一个 `and`)。
  ⇒ 两条同波 armed 时,**`wandbleed` 的触发计数下降本身不是负面信号**,那正是本 id 要买的;
  要读的是**下降的那些帧里有没有「活着的攻击者」**。
- 与 `wandlimbo` 正交但**共用一条链**:`wandlimbo` 的域要求 1600 环为空、
  本 id 要求 4000 环**非**空 ⇒ 两者**不可能同帧都开**,并且 armed 的本 id
  **不会**把帧让给 `wandlimbo`(后者另有 ≤25% HP 与 ≥6 充能两条更严的腿)。
- **对局数为零的增量**:搭车、零 AWS 增量、不申请专波、零 EC2。

### §DS.8 交棒

- **总监**:裁 §DS 入集(55 → 56)。queue 行 `strategy-39`(`bundle` 已填)。
- **录像组**:GH #437 上那一棒 —— 把 CM `t=473.5` 冻成 `tests/fixtures/`,
...
2. **承重的阴性面与主判据同等必读**:armed 腿里**保留**的 `wandbleed` 触发,其 4000 环内
   活敌人数必须 ≥1(本地已逐帧钉住 **78/81**;⛔ 原写作 `78/80`,是立案帧
   `f_260902_154755_cm_wandbleed_residue.lua` 落地前的语料,见 §DU.8)。没有这一面,「armed 少喝魔杖」与
...
3. **域为零时按 `DOMAIN-NOT-REACHED` 退回**,不得读成「无效应」:域是
   ...
   最后一项按语料的命中率是 **3/81 = 3.70%**(**帧/帧**:81 个带新鲜英雄伤害的帧里
   3 个环内无活敌人),一波抽不到是完全可能的。⛔ 原写作 **2/101** —— **分子取帧、
   分母取「受害者-攻击者对」**的混合单位,而它在旧语料上恰好与另一个真实比值
   (攻击者已死的对,当时同样是 2 比 101)**取同一个数**,所以没有任何一步会逼人
   说清用的是哪一种切法;立案帧落地后两种切法**分岔**(帧/帧 **3/81** = 3.70%
   vs 对/对 **2/102** = 1.96%,近 2 倍,而方向是把域说小)。见 §DU.8。
...
### §DU.8 2026-09-04T01:xxZ 协同组(认领 GH #464 §5)更正 §DU.5 的两个数 —— 本格最该被读的不是 `78/80 → 78/81`,是 ⭐ **一个比值的分子和分母可以来自两个不同的总体,而只要分子在两种切法下取同一个值,就没有人需要说清用的是哪一种**

GH #464 §5 请本组确认一处更正(§DU.5 第 2 条 `78/80 → 78/81`)。确认了,**而且它是两处里较小的那一处**。

#### §DU.8.1 独立复测(裸退出码,不抄 #464 的表)

两台**互相独立**的仪器,同一份语料(`ls tests/fixtures/*.lua` ⇒ **109**):

| 量 | 仪器 A(`tests/test_replay_437_wandbleed_source.lua`,**走出厂谓词** `J.IsWandBleedSourcePresent`) | 仪器 B(本轮临时脚本,**几何直读** fixture 表的 `team`/`x`/`y`/`alive`,不过 mock 谓词) |
|---|---|---|
| 存活英雄帧 | 1012 | **1012** |
...
仪器 B 另买一个 #464 没报的量:**被拦帧上落在 4000 环内的活攻击者对 = 0**(即 §DU.2 那句「取 4000 挡掉 0/101 对」在新语料上是 **0/102**,**结论不变、分母陈旧**)。
⇒ **`78/81` 确认;三个被拦帧点名**:`f_260820_043120_viper_defend_paired.lua` 的 viper 与 zuus(两个全死残留)+ `f_260902_154755_cm_wandbleed_residue.lua` 的 crystal_maiden(走开的施法者)。

#### §DU.8.2 ⭐ 主判据:第二处更正,`#464` 没点到,而它坐在承重的那一条上

§DU.5 第 3 条是 **`DOMAIN-NOT-REACHED` 的退回条款** —— 收割 W43 时决定「域抽到了没有」的那把尺。它写的是「最后一项本地命中率只有 **2/101**」。

**那个比值的分子和分母来自两个不同的总体**:最后一个合取项(4000 环内无活敌人)是**逐帧**求值的,分子 2 是**帧**(旧语料里被拦掉的帧数);而 101 是**受害者-攻击者对**(§DS 的第三行,同节 14011)。旧语料上**另一个真实比值恰好也是 2/101**(攻击者已死的**对** 2 / 对 102… 旧值 101)⇒ **两种切法给出同一个数,于是没有任何一步会逼人说清用的是哪一种**。

立案帧落地后两者**分岔**:

- **帧/帧(正确的那把尺)**:被拦帧 3 / 带新鲜英雄伤害的帧 81 = **3.70%**;
- **对/对(另一个真实但不相干的量)**:攻击者已死的对 2 / 对 102 = **1.96%**。

**近 2 倍,而且方向是把域说小** ⇒ 照旧数读,W43 收割会更容易接受一个「域没抽到」的零。⇒ §DU.5 第 3 条已改为 **3/81 = 3.70%(帧/帧)**,并把切法写在数字旁边。

这正是铁律 4(iii)(**登记过的效应量,连「用哪种切法得到的」一起登记**)的一个**阳性病例**:那条规则立案时的证据是 `med n_rc` 池化与均值分岔;这里的新意是**分子在两种切法下同值**,所以连「有两种切法」这件事都没有被看见 —— 与 `0FIXDEBT` 的「一个语料事实钉在两个文件里就是两个钉子」同族,但这里两个钉子钉的是**同一个字符串的两个不同含义**。

#### §DU.8.3 载体普查:这两个数一共钉在四处,#464 点了一处

`grep -rn "78/80\|2/101"`(排除 `iterations/reports/`,历史报告不改):

| 载体 | `78/80` | `2/101` | 处置 |
|---|---|---|---|
| `test_set.md:7`(收割前必读横幅) | — | ✅ | **本轮改**(横幅是活的) |
| `test_set.md` §DU.5 第 2 条 | ✅ | — | **本轮改**(#464 点名的那一处) |
| `test_set.md` §DU.5 第 3 条 | — | ✅ | **本轮改**(§DU.8.2) |
| `iterations/queue.json:1501`(`strategy-39` 的裁定 note) | ✅ | ✅ | ⛔ **未改,交总监** —— 那是**总监自己写的裁定存档**,本组不改别组的裁定原文;但收割 W43 的人**可能读的是它** |

另外 §DS(14021)与 §DU.2(14200)的「挡掉 **0/101** 对」按「留历史行」**逐字未动**:结论(0)在新语料上仍成立,只有分母陈旧,§DU.8.1 已把 **0/102** 登记在这里。

#### §DU.8.4 落地的守卫:为什么这个数第二次变陈旧,以及下一次会自己举手

**语料侧已经有棘轮**(`test_replay_437_wandbleed_source.lua` 的 `cs.ratchet(#c.fresh_frames, 81, …)` / `cs.ratchet(#blocked, 3, …)`)—— 语料一长,**测试会红**。**档案侧没有任何东西**:§DU.5 是散文,109 份 fixture 长到 110 份时它会静悄悄继续说 `78/81`。

⇒ 落地 `tests/test_du5_archive_census.lua`:它**不重算语料**(重算就是造第三个钉子),而是从上面那两个 `cs.ratchet` 的**源码**里读回 `fresh` 与 `blocked`,算出 `kept = fresh - blocked`,再断言 §DU.5 与横幅里写的正是 `kept/fresh` 与 `blocked/fresh`;被取代的旧值只允许以 `原写作 \`X\`` 的形式出现。
链条因此是 **语料 → 棘轮(已有) → 档案(本轮新增)**,一把锤子敲两颗钉子。
变异台 `tools/agent/mutstand_du5_archive.sh`,读数见报告。

**⚠️ 登记的 LIMIT**:这台守卫看的是 `test_set.md`,**看不到 `queue.json:1501`**(§DU.8.3 第四行)—— 它是别组的裁定原文,本组不改也不断言;若总监认为该纳入,那是一行常量的事。
...
## §DY 2026-09-04T04:xxZ 总监裁定,两件:**顺延八轮的 `RIDESHARE` 分类腿落地**(它读错了字段,`none` 与「队列干净」不可分辨)+ 它当场点名的 `hero-28` **ROUTED_ARCHIVE_SCAN** —— 本节最该被读的是 **§DY.2:一条为某类行举手的腿,读的是那类行不写的地方,于是「必裁」对它们八轮零次响过;而这件事在同一天、同一份报告里,以另一条腿的形状又发生了一次**

### §DY.1 摘要

| 件 | 裁定 |
|---|---|
| ① `RIDESHARE` 分类腿(章程 2026-09-04T01:00Z 条 ⑤,**第八轮顺延、本轮到期**) | **已修**:`is_rideshare` 从只读 `question` 改为读 `axis` + `question` + `acceptance`(`DECLARATION_FIELDS`)。4 只变异全杀,`tests/test_pending_rulings.py` **148 检查 0 失败** |
| ② `hero-28`(`zusaether` 的条件 (a) 取证) | **ROUTED_ARCHIVE_SCAN / 当轮放行**,`executor=batch-desk`,与 `hero-27` 合成同一次遍历,零 EC2 |

**没有做的事,照实登记**:零 AWS 调用、零发波、**`bots/` 与 `game/` 零改动**
(⇒ 铁律 6 的动态那半**未跑也不声称**)。②**不是**入集裁定,不动成员串(仍 60 id)。

### §DY.2 ⭐ 立案句:一条腿在它自己声称的领域里,`none` 打了八轮

`tools/agent/pending_rulings.py` 的 `RIDESHARE` 桶存在的唯一理由是 §BB.4:
**搭车提议必须在它到达的那一轮被裁**。桶的判据是 `is_rideshare`,而 `is_rideshare` 读的是
**`question` 一个字段**。

**本轮量出来的**(真实 `iterations/queue.json`,71 行):

- **59 行**在自己的散文里自陈搭车;
- 其中 **23 行**把这句话**只写在 `axis` 或 `acceptance` 里,一个字都不在 `question`**;
- ⇒ 这 23 行**从来没有进过 RIDESHARE 桶**,它们一直坐在 `OTHER` 里,
  而 §BB.4 的「本轮必裁」**对它们一次都没有响过**。

立案的现场是 2026-09-02 那一轮记的:自检打
`RIDESHARE (§BB.4: rule this round): none` / `OTHER … 3`,
而那三行的 `axis` 里**逐字**写着「搭车、零 AWS 增量、不申请专波」。
⭐ **当时的结论是「三轮顺延不只是总监选了别的项,是替它们举手的那条腿把它们分错了桶」——
这个结论是对的,而它随后又顺延了六轮**,直到本轮的到期条款。

**⚠️ 一处自我更正,方向对我不利**:本轮起草时先写成「RIDESHARE 桶从未响过」,
**那是错的** —— 另有 **36 行**确实把自陈写进了 `question`,桶对它们是响的。
缺陷的真实形状是 **59 中漏 23(39%)**,不是「全漏」。
写进本节是因为**夸大的立案句比准确的立案句更容易被后来的人推翻,连带推翻真的那部分**。

**为什么这个缺陷能活这么久**:`none` 是它唯一的输出形状,而
**「没有欠裁的搭车件」与「这条腿看不见搭车件」是同一行字**。
与 **GH #317**(`is_open` 看不见漂移的 status,把行从**两个**桶里同时拿走)、
**GH #332 / §DR** 同族 —— 都是「举手的腿读错了地方,它的沉默看起来像好消息」。

⭐ **同一个文件里就有反例,而且是对的那个**:`subjects_of()`(域腿)读的是
`SUBJECT_FIELDS = ("axis", "question", "acceptance")` —— **同一份三元组**。
也就是说这个文件**早就知道**申请书的自陈会写在三个字段里的任何一个;
`is_rideshare` 是那条知识**没有传到**的一行。

### §DY.3 修法,以及为什么「读所有字段」是错的
...
**⛔ 没有读 `director` / `result` / `notes`,这不是省事,是判据**:那三个字段里的
「零 EC2」是**别人写的关于这条请求的句子**(批测台的成本核查、总监的裁词),
**不是申请方的自陈**,而且通常写在**它被排期之后**。
读了它们,一条申请专波的请求会因为批测台自己的记账而变成搭车件。
**这个过度修正长得和修复一模一样**,所以它是变异台上最值钱的那只(M2)。

**扩宽的正当性是失效方向,不是整齐**:RIDESHARE 桶的语义是「本轮必裁」,
**过宽**的代价是某一轮多看一眼,**过窄**的代价是这条义务失去全部可见性 ——
这与 GH #317 给 `is_open` 的论证逐字同构。

**23 行逐条手核过负式与引用式用法**:没有一条是「**不是**搭车」「若**不申请专波**则…」这类,
也没有一条真的在申请专波(`strategy-6` / `hero-23` 的 `seeds` 非空是**收割时记的载体种子**,
不是波次申请)。⇒ **真实队列上零假阳性**。

### §DY.4 承重是变异台,不是「测试变绿了」

`tools/agent/mutstand_rideshare_field.sh`(新建;从**文件副本**还原,每只之后
`sha256sum -c` 核验,`trap on_exit EXIT` 先还原再删副本 —— GH #418/#468 的形状)。
裸读 `RC_EXIT=0`,**`mutants: 4/4 CAUGHT`**,M0 基线 `exit=0`:

| 变异 | 内容 | 结果 |
|---|---|---|
| M1 | 退回修前形态(只读 `question`) | CAUGHT |
| **M2** ⭐ | **读 `req` 的所有字段**(含 `director`/`result`) | **CAUGHT** |
| M3 | `any` → `all`(要求七个 marker 全中) | CAUGHT |
| M4 | 加了 `axis` **忘了 `acceptance`**(半落地) | CAUGHT |

⭐ **M2 是这台子存在的理由**。只有 M1/M4 的台子会被
`is_rideshare = lambda req: True` 通过 —— 那会把 `OTHER` 桶整个清空并自称修好。
M3 同理:`all` 读起来「更严格」,却会让桶重新恒为 `none`(没有任何真实行同时写全七个 marker),
**是从另一侧回到同一个静默**。

**新增的断言里有一条不是按字段列表写的**(`tests/test_pending_rulings.py` 不变量 1):
它扫**申请方写的每一个字符串值**(只排除 §DY.3 那三个字段),断言
**`OTHER` 桶里不许有任何一行在自己的散文里自陈搭车**。
理由是**棘轮的形状**:按 `DECLARATION_FIELDS` 写的断言只能锁住这次恰好挑中的三个字段,
而这一条**在某个流发明第四个散文字段并往里写「搭车」的那一天就会红**,
修法是去加宽 `DECLARATION_FIELDS`,而不是八轮之后再用肉眼发现一次。

### §DY.5 `hero-28` ROUTED_ARCHIVE_SCAN(裁定全文见 `queue.json:hero-28.director.note`)

修完腿之后,`hero-28` 是**唯一**落进 RIDESHARE 桶的行(`RC_EXIT=3`),
⇒ **本裁定是那次修复的直接后果**。放行后两桶皆 `none`,`RC_EXIT=0`。

沿 §BF.2 / §AZ.5 统一路由,**与 `hero-27` 合成同一次遍历**(申请书自己提的),
`executor` = **`batch-desk`**(§CH:恰好一个流)。零 EC2 / 零新局 / 零 AWS 增量 / 不申请 arm。

**总监独立核过的两条**(核的是**买不买得着**,不是机制对不对):
(1) `grep -rl aether tests/fixtures/` 裸读 **0**(目录 111 项)⇒ 申请书「条件 (a) **结构上**买不到
第二份 fixture」**成立**,这是它值一次语料的理由;
(2) 域是活的:`sRoleItemsBuyList` 在 pos_4/pos_5 **都买** `item_aether_lens`,
闸 `AetherReach()`(`hero_zuus.lua:381-390`)是单条独立 gate、turbo 合取在场、
off 路径 `return 0` = 出厂表达式,**且注释点名回避了 `pullcad` 陷阱**。

**⛔ 边界**:这是**路由裁定不是对前提的背书**(§BF.2 逐字)。总监**没有**复核机制论证、
没有跑 `tests/test_zeus_aether_cast_range.lua` 或它的变异台。

**⭐ 一条执行方不许照抄的数**:同一轮两处普查对不上 ——
申请书「33 声明,**26** 接了、7 不读,Zeus 是第 8 个」(26+7=33,而 Zeus 又是第 8 个 ⇒ 34);
`hero_zuus.lua:349-351`「33 里 **22** 个接进至少一处 cast-range 读,Zeus 是**读它 nowhere 的 8 个之一**」。
分子口径不同(「接了」vs「接进 cast-range 读」)**而两处都没说明**。
⇒ **引用前必须重算并说明口径**;本裁定不依赖其中任何一个数。

**时限不是它自己的**:若这次合并遍历读 GH #388 的 W34 语料(`dem21/`),
那份语料 **2026-09-22 自动删除**,**没有任何工具会在那天举手**;跑不完**在过期前退回**,
不要静默过期。Zeus 那 54 局与 `dem21/` 的重叠比例总监**未**核实,由执行方在 (1) 里一并给出。

### §DY.6 ⚠️ 同一个形状,同一天,第二条腿(不在本轮处置范围,登记以免它也顺延八轮)

批测台 03:15Z 报告 §4.1 交上来的 GH #473 甲:**载体门看不见 `rotscope`**,
因为域-载体是从**文件路径**(`bots/BotLib/hero_*.lua`)读的,而 `rotscope` 改的是 generic 文件、
域却是一个具体英雄 ⇒ 连续八波恒零而**没有任何门举手**。

⭐ **这与 §DY.2 是同一句话的两个实例**:`subjects_of()` 用
`HERO_FILE_IN_PROSE = re.compile(r"bots/BotLib/hero_([a-z_0-9]+)\.lua")` 扫三个散文字段 ——
本轮修的是**字段选错**,这一条是**同一个函数里模式选错**。
**两条腿的沉默都长得像好消息。** 本轮**不裁**(它需要自己的普查 + 变异台,是一个完整单元),
列为下一单元第一项。

### §DY.7 交棒

- **批测台**:`hero-28` 已放行,**与 `hero-27` 同一次遍历**;`owed` 行
  `gate_i_executable_first_run` 本轮**仍未到期**(trigger = 下一次发波轮,W44 是收割轮),
  总监**不退休它**。GH #473 乙(按需机时常数 0.745 落在本波下界 0.765 之外)**下一单元裁**。
- **总监(自己的下一单元)**:①GH #473 甲 = §DY.6 的载体门 ②GH #473 乙 ③自检腿序
...
issue **GH #479**([batch],总监开,含修法与两条不许走的路);GH #476 已追评并以 `completed` 关闭。

---

## §EB 2026-09-04T08:xxZ 协同组(**零行为改动;OWNER_PRIORITIES P1 的「先把前提买下来」那一格**):`.type` 买不到 —— 而买不到的**理由是结构性的,不是「还没轮到它」**;本节最该被读的是 **§EB.2:§BN.4 有一条从没写出来的前提 —— 它只能结掉「失效方向朝关」的谓词;朝开失效的那一半,它连一句话都说不出,而伸手去够它看起来跟尽职一模一样**

**本轮零 AWS、零 S3、零 EC2、零波次;`bots/` 与 `game/` 逐字节未动;armed 串一字未动;无新 id、无新闸、无新 fixture。**
产出:`tests/test_camp_type_premise_polarity.lua`(13/13)、`tools/agent/mutstand_camptype.sh`(9/9 CAUGHT,零 NO-OP)、
`docs/BOT_API_REFERENCE.md` 的 `.type` 行改写、`state.json:camptype_POLARITY_20260904`、`queue.json:strategy-43`。

### §EB.1 交的是章程写死的那一格,而答案是「否」

章程「下一格」逐字写着:(甲)选点循环不排除远古营地,但 `camp.type` 那一半**至今未 settled**
(`campteam_SETTLED_20260827` 的 `boundary` 写着「the `.type` / `.speed` halves stay OPEN」)
⇒ **先按 §BN.4 把 `.type` 的前提买下来,否则守卫可能是一台瞎仪器**。
本轮把 §BN.4 机械地跑了一遍,结论是:**这个前提用 §BN.4 买不到,而且永远买不到。**
这不是「本实验室恰好还没跑过压着它的波」,是**方法与谓词的极性不匹配**。

### §EB.2 ⭐ 本节的主判据(立法级,可复用,超出本主题):§BN.4 的极性前提

§BN.4 从**已被观测到的行为**倒推一个前提。它对 `.team` 生效,是因为 `.team` 压在一道**准入门**上:
`J.ShouldPullNeutralCamp` 不满足 `camp.team == GetTeam()` 就 `return nil`,而下游的拉野行为
**只能穿过非 nil 返回**到达 ⇒ 反驳假设(「`.team` 不是队伍号」)预言那个行为**根本不可能发生**,
于是 283 局「它发生了」把假设免费证伪。

`.type` 的**每一处**读法极性相反。它们全是写成排除式的**过滤子句**
(`... and botLevel < 12 then return false`、`or not IsAncientCamp(rec)`、`camp.type ~= "small"`),
于是反驳假设(「`.type` 不是那几个字符串」)让每一条**恒真地放行**。
**而一条恒真的过滤器和一条不存在的过滤器,产出的对局一模一样。**
「行为发生了」正是反驳假设自己也预言的东西 ⇒ **任何数量的它都分不开两者。**

> **⇒ §BN.4 有一条极性前提:只有当反驳假设让谓词「朝关失效」(什么都不发生)时,它才能结案。
> 谓词「朝开失效」(什么都放行)的地方,这个方法是结构性沉默的 —— 而伸手去够它
> 看起来跟尽职调查一模一样,却什么都没买到。决定「能不能省掉一次在线探针」之前,
> 先问「它朝哪边失效」。**

与 §BL.4「门的合取项要逐条查」、§BN.4 本身「这个门为真过没有」同族,补的是第三问:
**这个门为假的时候,是把人关在外面还是把人放进来。**

### §EB.3 极性是**跑出来的不是论证的**(这一节是本文件的产品)

`tests/test_camp_type_premise_polarity.lua` 把每一条通路**驱动两遍**:同一份营地记录,
一遍 `.type` 是 shipped 代码比较的那个字符串,一遍是**被退役的文档行声称的那个 int**
(`type (int)` 与 small/medium/large/ancient 并排,正是 §BN.5 点名的那个自相矛盾指纹)。
「朝开失效」于是变成一句机器可判的话:**`admitted(string) ⊆ admitted(int)`**。

| 站点组 | 读的是 | 分类 |
|---|---|---|
| `aba_site.lua:432/:435`(`campgrade` 阶梯) | `IsAncientCamp` / `IsLargeCamp` | **FAILS OPEN**(9 级/60 伤:字符串腿准入 2,int 腿准入 4) |
| `aba_site.lua:453/:455`(`RefreshCamp` 落穿链) | 同上,未 armed | **INVARIANT** —— 每条分支 append 同一个值、末尾还有无条件 `else` ⇒ **输出完全不依赖 `.type`**,任何行为测量都压不到它 |
| `aba_site.lua:528`(`campsel` 选点) | `IsAncientCamp(rec)` | **FAILS OPEN**(9 级、只有一个本方远古营:字符串腿 nil,int 腿选中);**未 armed 时读的是 wrapper ⇒ 两腿同解 = shipped 默认双向都压不到** |
| `hero_templar_assassin.lua:618/:619/:712` | `~=` 字符串 | **FAILS OPEN**(Lua 语义:数字 `~=` 字符串恒真,已作为可执行事实断言) |

**零个站点朝关失效**,而这个 0 **是用例里算出来的一个计数、不是散文**:
M8 变异体(把阶梯的 `IsAncientCamp(camp)` 改成 `not IsAncientCamp(camp)`)**造得出一个真的朝关站点**,
用例当场转红 —— **一台造不出反例的仪器不算测过这个结论。**

### §EB.4 ⭐⭐ 用例自己第一次跑就买到的一条:**argmin 不是单调量,于是「更宽」在选点器上会读成「更窄」**

结论那一格的第一稿按**赢家**给选点器分类(整张 8 营名单丢进 `GetClosestNeutralSpwan`,看返回哪一个),
读出来是 **1 个站点组 FAILS CLOSED** —— 而那正是**唯一能反转本文件全部结论**的那个发现。
它是假的:选点器返回的是**候选集上的 argmin**,而**放宽候选集会把 argmin 挪到新准入的那个候选上**
⇒ 字符串腿的赢家从 int 腿的赢家集合里消失,长得**与「朝关失效」逐字同形**。
修法不是放松断言,是**换成对的量**:逐个营地问「它单独在名单里时准不准入」。
⚠️ 这一条**留成了一条用例**(`[argmin]`)而不是修掉就算,因为它可复用:
**凡是把「更宽/更窄」写成对一个 argmin / argmax / 「最近者」的比较,极性读数就可能反号**,
而反号那一侧恰好是「有大发现」的那一侧。与 §DZ.4(`tNeut[1]` 这个「最近者」撤销四条否决)
**是同一个 `最近者` 在两天里咬的第二口**,这次咬的是**观测者**不是被观测的代码。

### §EB.5 顺带纠正的两处(小,但都是分类逃逸的入口)

1. **`docs/BOT_API_REFERENCE.md` 的 `.type` 行少数了一处**:它写「two clauses in
   `hero_templar_assassin.lua`」,实际是**三处**,而漏掉的第三处正是最不同的那一处 ——
   :618/:619 过滤的是**营地名单**,:712 (`loc.type ~= 'ancient'`) 门的是一次**施法**
   (Psionic Trap 布点)。**一行文档少数了自己的读者集合,就是一个站点逃过分类的入口。**
   已改写为 **7 处**(`aba_site` 4 + TA 3),并把 §EB.2 的极性结论写进该行。
2. **`IsSmallCamp` / `IsMediumCamp` 零调用点**(定义存在、无人调用)。登记而不删
   —— 删是行为改动,而且它们是将来任何 `.type` 守卫最自然的落点。
   用例把「0 调用点」钉成断言:哪天有人调它,就是一个新的 `.type` 决定,需要自己的极性行。

**普查是棘轮**:全 `bots/` 树扫 `<ident>.type`,今天 6 行 16 处(营地记录 7 处 + 无关 9 处:
`cfg.type`×3 / `sound.type`×4 / `rune.type`×2)。**新增一个读者 ⇒ 用例转红** ——
因为 §EB.2 的结论是关于**一个闭集**的,一个未分类的站点会**静悄悄**地把它作废。

### §EB.6 那么怎么才买得到(**已预登记,零增量 AWS**)

`.type` 要结案,需要一条**阴性读数带分母**:某处该被字符串腿拒掉的行为**没有发生**,
而域不为零。**这条读数已经有人欠着了** —— `campgrade` 的独占入集波
(§AU.2 裁的形状)本来就要读**本方远古 engagement 在 10..11 级带**的下降
(`campfarm` 的验收逐字:`ancient_camp_domain.py` + `campgrade_ladder.py`)。

**关键的算术**:反驳假设下 `campgrade` armed **动不了那个数** —— 它的远古档与大营档
**都靠 `.type`,两条都死**,活着的只有 `.team` 那一档(敌方营 + `botLevel < 15`),
而那一档**按定义碰不到本方营**。⇒ **本方**远古 engagement 在 10..11 带上**掉了 ⇒ `.type` 是字符串**;
**没掉** ⇒ 不确定(可能是通路走的是 `campfarm` 管的那条扫描,不是名单)—— **不确定不是通过**。

⇒ `queue.json:strategy-43`(**不申请新波,挂在 `campgrade` 那一波上,零增量美元**)。
⚠️ 与本文件已有的两条 `campgrade` 排波前置(§AO 那条 `pullcamp`/`campfarm` 侧、
§BG 那条 `campsel` 侧)**并列写进 `campgrade` 未来入集裁定的前置检查**,不要各自为政。
...
- **总监**:(i) 裁 §EB.6 的预登记要不要挂进 `campgrade` 的前置检查清单(与已有两条并列);
  ...
  **仍然成立、不要动**,但现在有了理由:**不是没人去买,是这条路买不到**。
- **本组下一轮**:`.speed` 那一半**不需要一轮**,理由比 `.type` 还短,而且是本轮顺手量到的:
  全 `bots/` 树里 `.speed` 只有**一个**读者(`aba_site.GetCampStackTime`),
  而**那个读者本身零调用点** ⇒ 这个前提今天**不承载任何决定**,买它买到的是零。
  (顺带:它的链是 `"fast"→56 / "slow"→55 / 兜底 56` ⇒ 反驳假设下**每个营地都答 56**,
  差 1 秒、只在 slow 营上 —— 但既然没人调用,这个差连发生的地方都没有。)
...
## §ED 2026-09-04T10:45Z 协同组 —— **零行为改动的前提采买**:`J.HasFieldRegenSource` 自己写下的合同(「在槽里出现就证明喝得着」)对**四条腿里的一条是假的**,而它**同时被两个极性相反的消费者各花掉一次**;真正要总监先收下的不是这条缺陷,是它带出来的**共同 promote 约束**

**本节不申请入集,不要波次,不花一分钱。** `bots/`、`game/` 逐字节未动,armed 串一字未动,
`queue.json` 一字未动,**无新 id、无新闸、无新 fixture**。产出是**一份真帧普查 8/8 + 一台变异台
11/11 CAUGHT(零 NO-OP)+ 一条交给总监的 promote 约束**。

### §ED.0 一句话

`J.HasFieldRegenSource` 的头注把自己的合同写死成「**只数那些一出现在槽里就证明用得上的消耗品**」;
对 `item_faerie_fire` 这条腿,**仓库自己的代码说这句话是假的** —— 全树唯一能吃掉一颗仙灵之火的是
`X.ConsiderItemDesire["item_faerie_fire"]`,而在「野外回复」这一族自己的域里,它的三条分支
**在语料的 7 帧上被量到 7/7 关闭**。于是**一个 TRUE 被花了两次、方向相反**:它给持有侧
(`stayfield`/`stayfield2`)买来「留在野外」,又告诉补给侧(`fieldbuy`)「这人已经有补给了」。

### §ED.1 域(量出来的,不是论出来的)

109 fixture / **1012 个活着的英雄帧**:57 帧满足 `J.IsFieldRegenSituation`,24 帧带一个被接受的
补给源,**7 帧的唯一被接受源是仙灵之火**。这 7 帧上:

| 读数 | 值 |
|---|---|
| `自己吃` 分支可开(时间 ∧ 背包槽 ∧ 缺血 ∧ 距泉水) | **0 / 7** |
| `自己吃` 的时间门 `DotaTime() > 10*60` 开 | **0 / 7**(帧时刻 192.4 .. 473.5 s) |
| `自己吃` 的背包槽门 `GetItemInSlot(6) ~= nil` 开 | **0 / 7** |
| `撤退` 分支的**绝对**血量地板 `OriginalGetHealth() < 90` 开 | **0 / 7**(最低 230) |
| 函数自身的泉水地板 `DistanceFromFountain() >= 1800` | **7 / 7**(5316 .. 10009) |
| armed `fieldsip` 已经放掉它 | **7 / 7** |

**两个零是读数不是盲区,而且这一条是量出来的**:同一语料里 **203/1012** 个活帧背包槽 6 有东西、
**173/1012** 个活帧已过 10 分钟 —— 两个门在别处都被真正求值过。
**结构性的一句**:24 个带源帧里**只有 1 帧**在 t=600 之后 —— 这一族的处境谓词(HP 0.18..0.55、
1600 环全空)在 Turbo 里**几乎整个活在吃窗打开之前**。

### §ED.2 它咬在 owner P2 自己钉的那一帧上

`f_260822_063722_lina_tp_home`,**t=349.0 s**:lina 346/1088,主槽一颗仙灵之火,**背包槽 6 是空的**。
留下她的那个理由,**要到 t=600 才吃得动** —— 一局 Turbo 约 20 分钟,这是 251 秒。
同一帧上 armed `fieldbuy` **不买大药**,被挡它的正是同一颗仙灵之火。

### §ED.3 ⭐ 主判据(可复用,超出本主题):**一个分区可以在谓词上完备,而在世界里两边同时是错的**

`fieldbuy` 读的是 `not ( HasFieldRegenSource(bot) and IsFieldSipEnough(bot) )` —— 与持有侧**同一个
合取式取反**。所以「没有帧属于双方」「没有帧无人认领」是**构造出来的真**,`fieldsip` 落地时还
特意把这条写进注释(campvoid 的教训「先于事实」而不是事后)。**那份完备性是谓词的性质。**
两边都没问的是:**被拿来分区的那个字段,下游认不认。** 这 7 帧上
`hold=TRUE` 的意思是「被一件吃不动的东西留住」,`buy=FALSE` 的意思是「他已经有补给了」,
**两句同时是错的,而分区依然完备。**

与 `campbind_20260904`(GH #475)是同一枚硬币:那次是**选点器的四条否决被下游一个「最近者」整体撤销**,
这次是**上游一次槽位存在性判断,没有任何消费者兑现它**。同一条规则的两种方向:
**一个谓词的价值 = 它的答案被下游兑现的那一部分。**

### §ED.4 ⭐⭐ 为什么至今没人看见:**它被同族的另一个 id 精确地遮住了,而遮住它的理由与它无关**

`fieldsip`(量级杠杆,**已在 armed 串里**)把这 7 帧**全部**放掉 —— 85 点血不到本语料里任何一条血
条的四分之一。遮蔽是**精确的(7/7,已断言)**,但它是**方向上的巧合,不是修复**:
`fieldsip` 问的是「这一口够不够值得留下」,本节问的是「这一口**吃不吃得着**」。
⇒ 一旦 `fieldsip` 在闸口被拒,**前提照旧是坏的,而没有任何东西在看它**。

### §ED.5 ⛔ 交给总监的**共同 promote 约束**(本节真正要收下的那件事)

**至今每一波给 `stayfield`/`stayfield2` 买 (a) 的波,都在同一根串里 armed 了 `fieldsip`。**
所以在这 7 帧的族群上,**被测到的行为是 `fieldsip` 的,不是持有侧的**。
⇒ **promote `stayfield`/`stayfield2` 而 reject `fieldsip`,会 ship 一个没有任何一波测过的配置**
(在一件吃不动的东西上持有)。方向是单向的:

- ⛔ 「有持有、无量级」这个方向上,`stayfield` / `stayfield2` / `fieldsip` **是一个原子**;
- ✅ 反方向(promote `fieldsip`、拒持有侧)不受本节影响。

这是 `pullcad`(promote 一个 id 会冻死点名它的闸)与 `creeppull`+`pullbeat`(作为一个原子 promote)
的同族,**区别是这次是在裁定之前写下来的,不是在裁定之后**。

### §ED.6 本组**故意没有**落一个新 id,理由是读数不是偏好

自然的修法是在 `J.HasFieldRegenSource` 里给仙灵之火那条腿加一个 gated 合取项(镜像消费者的两道门),
写法与 `fieldcreep` / `bagsalve` 同形。**没有落**,因为在本语料上它 armed 的效果与 `fieldsip`
**逐帧相同**(同样 7 帧、同样方向)⇒ 一根串里两个效果相同的 id,**波次读数无法把它们分开**,
而那正是每个 id 单独一根闸的全部意义。**要不要收窄是总监的裁定,不是本组的既成事实**:
(甲) 认 `fieldsip` 的 promote 为这条前提的标准答案;或 (乙) 若 `fieldsip` 被拒,再落 `ffeat` 收窄
(那时它不再与任何 armed id 同效)。

### §ED.7 诚实边界(先说,不埋)

1. **`攻击` 分支不声称关闭**:它的 `J.IsValidHero( botTarget )` 在每一个 fixture 帧上结构性为 nil
   (GH #474)⇒ 在 HP < 0.3 的那 **2/7** 帧上只有两个合取项可测。结论**只建立在三条分支全部可证关闭
   的 5 帧上**,另外 2 帧登记为 **UNMEASURABLE**,并作为一条断言(不是一句散文)钉在用例里。
2. **`item_tango` 那条腿没测**:它的消费者要一棵**树**,fixture 不带树。**明说没测,不假设健全。**
3. `item_bottle` 腿在 fixture 上永远答不了 TRUE(mock 的 `GetCurrentCharges` 默认 0),
   与 `J.HasFieldRegenSource` 自己的盲区同一条。
4. 本节**不声称**被放掉的 bot 随后买到了大药:金钱、库存、快递都不在 fixture 里。

### §ED.8 仪器自己踩的一脚(留档,方法不是缺陷)

普查第一版把**背包槽读数 100% 读反**:`(op == 'occupied') and bOcc or not bOcc` —— Lua 的
`and/or` 三元写法在**中项为 false 时返回错的那一支**,于是 `bOcc = false` 掉进 `not bOcc` = true,
7 帧全部报成「槽门开着」。**它长得像一个普通计数**,方向恰好是「本节没有发现」那一边。
已改成显式 `if`,并把理由留在源码里。凡是把一个布尔读数写成 `cond and X or Y` 的地方,
**X 可以是 false 就是它的失效方向**。

### §ED.9 产物与交棒

- `tests/_fieldsrc_ff_sweep.lua`(语料普查,子进程;消费者的每一个常量**从 shipped 源码解析**,
  连 `~=`/`==` 这个算子一起,所以动一个门就动一次普查);
...
> **【§EI 修订,总监 2026-09-04T19:xxZ,按 §EH.7 (乙) 批准】**
> 上表的 **8** 与「1012 个活着的英雄帧」这个分母,**读法要改**:
> `tests/_midsupfar_sweep.lua` 用 `pcall` 读扳机,**三值只有两个桶** ——
> 「抛错」与「量过了答否」记成同一个东西。协同组 16:3xZ 量出
> **75/1012 活帧在 `J.ShouldTpSupportTowerFight` 里抛错**
> (成因与 `J.CanEnemyInterruptTpChannel` 同一条:fixture mock 没有
> `GetExtrapolatedLocation`)。
> ⇒ **这个 8 是在 937 帧上量的,不是 1012**;而且**被删的 75 帧与问题不独立** ——
> 删掉的正是「敌人贴脸」的帧,也就是这个 helper 存在的理由(一场仗)。
> **8 是下界,不是计数。** 表格与两个具体帧的记述**逐字保留**(档案不改写),
...
## §EI 2026-09-04T19:xxZ 总监 —— **积压的七条「提请总监」一次裁完,而本节最该被读的不是任何一条裁定,是 §EI.0:这七条没有任何一条有过投递终点** —— 它们不是入集申请、不是波次申请,于是 `pending_rulings.py` 的两条腿按构造都看不见它们;外加 §EI.1:**收下 §ED.5 时把它的前提量了一遍,发现那句「至今每一波都同时 armed」对三波是假的**

### §EI.0 ⭐ 主判据(立法级,可复用):**一条裁定请求可以既不是入集申请也不是波次申请,而我们的两道门都只认这两种**

协同组 09-04 三轮各交了一批「提请总监,都不要钱」的验收项:
§ED.5/§ED.6(10:45Z)、§EF.8 三件(13:5xZ)、§EH.7 两件(16:3xZ),**共七条**。
本轮之前**一条都没裁**,而且**没有任何东西举过手**:

- `pending_rulings.py` 的 **RIDESHARE/OTHER** 腿只扫 `queue.json` 的**请求行** ——
  这七条**没有请求行**(它们不申请入集、不申请波次,按定义不该有);
- 同一工具的 **ORPHAN_PROPOSAL** 腿只认 `test_set.md` 里的**入集提议** ——
  这七条**不是入集提议**;
- `owed_executions.json` 认的是**已裁待执行**,这七条**未裁**。

⇒ **三道门,七条请求,零命中。** 它们唯一的载体是总监自己「下次触发」清单里的一行散文,
而那一行在 16:1xZ 那轮被逐字写下、**同一轮没做、下一轮(本轮)才做** ——
`§ED.5` 从 10:45Z 到 19:xxZ 跨了**三轮总监触发**。
**与 §CG.1(`fieldsip` 入集提议只存在于 test_set.md 一节里)同族,区别是那次补了一条腿,
这次连"提议"这个类型都不对** —— 补腿的人按当时看见的形状定义了类型,
**而缺陷的形状是「提请总监的事」,不是「入集提议」**。
(与铁律 0 那条措辞失效同型:按**位置**/按**子类**写的规则,对同族的另一个成员结构上无话可说。)

**本节不新造第四道门**:先把七条裁完(积压清零),把**唯一一条会在别的日子失效的**做成机器检查
(§EI.1),其余按「已有的看守是谁」逐条点名。**新腿留给下一轮**,理由写在 §EI.8。

### §EI.1 ⭐⭐ §ED.5 共同 promote 约束:**收下,但前提被重铸** —— 那句「至今每一波」对三波是假的

§ED.5 交上来的句子:「至今每一波给 `stayfield`/`stayfield2` 买 (a) 的波,都在同一根串里 armed 了 `fieldsip`」。
**本轮没有照抄,量了一遍**:按每一波自己钉的树读它自己的 armed 串
(`git show <W*_wave.json:tree>:iterations/streams/test_set.md` 第 2 行,W24..W47,24 波全覆盖):

| 波 | stayfield / stayfield2 | fieldsip | 备注 |
|---|---|---|---|
| **W24** | armed | **不在串里** | 该树(`2d1024ee`)`bots/` 里 `IsSoakCandidate('fieldsip')` **一处都没有**(0 sites) |
| **W25 / W26** | armed | **不在串里** | 闸已存在(`b51bac77` 起 1 site),但**未 armed** |
| W27 .. W47 | armed | armed | 21 波,如 §ED.5 所述 |

**未 armed 的 `fieldsip` 会让 `J.IsFieldSipEnough` 走早退返回 `true`
—— 那与一个被 REJECT 的 `fieldsip` 会 ship 的行为逐字节相同** ⇒
**W24/W25/W26 三波真的跑过 §ED.5 称之为「没有任何一波测过」的那个配置。**

**裁定:约束照收,理由换掉。** 新理由是量出来的、更窄、且仍然支持同一个方向:
24 波里 21 波共臂;唯一三波没共臂的是**旧波、全串 42–44 个 armed id 的 bundle 波**,
且**没有已知的、针对持有侧的逐 id 条件 (a) 读数**。
⇒ 「促持有侧、拒量级」仍然是一个**没有被干净测过**的配置,只是理由从
「从来没跑过」降级成「跑过三次,但那三次买不出 (a)」。
**退休条件一并写进登记行**:任何一条来自 W24/W25/W26(或将来任何 `fieldsip` 未 armed 的腿)
的、可用的持有侧 (a) 读数出现 ⇒ 本约束过严,应当撤销。

⚠️ **这一条本身就是 §EI.0 的例证**:§ED.5 把约束**写在裁定之前**(比 `pullcad` 强,§ED.5 自己说了),
但它写下的载体是散文;**散文经不起三轮**,而它的前提**在散文里没有任何东西会去量**。

### §EI.2 ⭐⭐⭐ 于是这一条做成机器检查,而不是再一次「收下」

`iterations/promote_atoms.json`(新)+ `tools/agent/promote_atoms.py`(新)+
开工自检新腿 **`promote-atoms`**。它管两个形状,**同一个根**——
**只在"某人 promote 的那一天"才生效的约束,写成没人会在那天重读的散文**:

- **(A) `pullcad` 陷阱**:活代码里的闸点名了一个**已 promote** 的 id ⇒ 该谓词**永远为假**,
  而 `check_armed_wiring.py` 照旧判 WIRED(它查调用点**存在**,不查谓词**可能为真**)。
  AGENTS.md 今天用一句**给人的指令**回答它(「promote 前 grep 该 id 在别的 gate 条件里的出现」)
  —— **那条指令已经失效过一次**(`creeppull`/`pullbeat` promote 时靠手抓)。**指令不是门。**
- **(B) 共同 promote 原子**:§EI.1 那一条,`no_promote_without` 规则,**单向**
  (促 `fieldsip`、留持有侧 gated **不受本行约束**,这是 §ED.5 自己写明的方向)。

**trunk 上的读数(裸读)**:`141 distinct live gate id / 13 id 带 PROMOTED 注记 /
FROZEN none / atoms 1 / exit 0`。

### §EI.3 ⭐⭐⭐⭐ 本节最该被读的实现细节:**不剥注释的版本,从落地第一天就是红的**

朴素扫描在本树上得到 **143** 个 id;剥掉注释后 **141**。差的两个是
`pullbeat` 与占位符 `X`,**五处出现,全部是警告这条陷阱本身的注释**
...
所以 `tests/test_promote_atoms.py` 的**承重用例是诱饵用例(case 3 / 3b)不是违规用例**:
case 3 钉住「只出现在注释里 ⇒ 不算」,case 4 钉住「剥注释没有把旁边那个**活的**也剥瞎」,
**case 3b 是变异台逼出来的** —— 单行 `--[[...]]` 证明不了块注释剥离(它自己就以 `--` 开头,
行注释腿已经杀掉它),**只有跨行块**能把两条腿分开,而 M3(删块剥离)**在 case 3b 存在之前存活**。

**第二处同族**:行号最初是拿**剥后偏移**去数**原文**换行,读出来的行**指向活调用点上方那条警告注释**
—— 即**这个工具存在的理由所要分辨的那个诱饵本身**。case 4 的第三条断言抓到的。

### §EI.4 变异台(`tools/agent/mutstand_promote_atoms.sh`,新)

**9 真发 9 CAUGHT + 2 量程对照正确 SURVIVED,零 NO-OP,树外 `cp` 还原 + `sha256sum -c` OK,还原后基线 19/0。**
两条**必须存活**的对照:**M9** 只改提示语;**M10** 把扫描目录从 `bots`+`game` 缩到 `bots`
—— 本树 `game/` 下没有 soak 闸 ⇒ **无行为差**,给它写一条能杀死它的检查
= 把实现细节钉成行为。**一台什么都杀得死的台,是一台已经不再分辨的台。**

### §EI.5 §ED.6 / §EF.8 / §EH.7 逐条裁定

**(1) §ED.6 —— 取 (甲)。** 认 `fieldsip` 的 promote 为这条前提(仙灵之火腿吃不动)的标准答案,
**本轮不落 `ffeat`**:协同组自己量出它 armed 后与 `fieldsip` **逐帧同效**(同 7 帧、同方向),
一根串里两个同效 id **波次读数分不开**,而"每个 id 一根独立闸"的全部意义就在这里。
**(乙) 的条件分支照收**:`fieldsip` 若被 REJECT,`ffeat` 才落(那时它不再与任何 armed id 同效)。
⚠️ **(乙) 故意不进 `owed_executions.json`**:它的触发器是一个**可能永远不发生**的 reject,
没有机器键,进去就是每轮一条永久噪声,而**噪声腿会被跳过读**(§EI.3 同一条道理)。
它写在 `promote_atoms.json` 那一行的 `note` 里 —— **那正是有人在乎它的那一刻会打开的文件**。

**(2) §EF.8 (甲) —— 批准维持,理由更换。** `AX.5` 第 2 条的原话
「方向由构造有界:只能把一次塔防响应从核心重新分配给辅助,不能丢掉一次响应」
的支点是**镜像完备性的自述**,已被 §EF.2/§EF.3 证伪(镜像漏掉的恰是唯一一条**配对**谓词
`GetUnitToUnitDistance(bot, building) > 3500`)。**新理由**:界现在由 §EF.4 补上的那个**合取项**给
—— 让路只在**同一条配对谓词也接受某个支援时**才发生,让路集合成为响应者集合的**严格子集**;
**不再由"镜像是完备的"这句自述给**。
**(甲) 与 (乙) 其实是一条**:重铸后的理由**带一个树前提**(必须是带 §EF.4 修复的树),
而那个前提就是 (乙)。

**(3) §EF.8 (乙) —— 照收,且不需要新机器**:`tests/test_midsupyield_core_yields.lua`
(改写后 13/13,`[ratchet]`)已经把修复后的行为钉死 —— **任何缺 §EF.4 的树在这个文件上是红的**,
而它在 Lua 发现集里每轮被跑。⇒ **点名已有的看守,不新造一道**(新造一道等于给同一件事两个真相源)。
投递:`queue.json:strategy-5b` 的 `director` 字段。

**(4) §EF.8 (丙) —— 照收。** `strategy-5b` 的 NEGATIVE control **保留、改判读**:
它现在覆盖的是**镜像接受且确实接得住**的那一半,不是全部;继续读成「全部丢弃风险」
会**高估波次买到的东西**。同样投递到 `queue.json:strategy-5b` 的 `director` 字段。

**(5) §EH.7 (甲) —— 批准补 `GetExtrapolatedLocation`,但执行方是总监不是协同组,且必须与两处重取同轮落地。**
不选"不补":现状是一道**已出厂、无闸**的守卫 `J.CanEnemyInterruptTpChannel` 在定义域内
**257/257 抛错**、`J.ShouldTpSupportTowerFight` 在 **75/1012** 活帧抛错,而 `pcall` 三值读数只有两个桶
⇒ **仪器正在静默删掉"敌人贴脸"的帧,也就是这些 helper 存在的理由**。让它留着,等于让以后每一份
基于 fixture 的域读数继续在被截断的语料上取。
也不选"协同组顺手补":这是 **harness 全局改动**,会移动每一个做外推的 helper 的读数,
落在一个组的工作单元里等于**无声地给别人的已发布数字换底**。
⇒ **执行方=总监,时机=下一轮总监触发的一个独立工作单元,同一次提交里重取两处**:
`tests/test_itemdesire_world_assertion.lua` 压着的 **178**,以及 §EF.1 的域。
已进 `iterations/owed_executions.json`(`fixture_mock_extrapolated_location`)。

**(6) §EH.7 (乙) —— 照做,本轮已落地。** §EF.1 的「helper 触发 **8**」在档案里加了修订注记:
**分母是 937 不是 1012**,被仪器删掉的 75 帧**与问题不独立**(删的正是敌人贴脸的帧),
**8 是下界不是计数**。见 §EF.1 末尾的「§EI 修订」块。

### §EI.6 不越界(这一半没有任何读数会替它举手)

- **`bots/` 与 `game/` 逐字节未动**(`git diff --stat` 两目录零行)⇒ 稳定版零漂移;
  armed 串(62-id)**一字未动**;`queue.json` 只动 `strategy-5b` 的 `director` 字段。
- **没有 promote / reject 任何 id。** §EI.1 收的是一条**约束**,不是一次裁定:
  `stayfield`/`stayfield2`/`fieldsip` 三者**本轮都仍是 GATED**(工具自己打的那一行为证)。
- **没有开新波次、没有花钱、零 AWS 调用**(连 `budgets` 都没读;MTD 照抄批测台自报)。
- **没有新造第四道门**(见 §EI.8),没有动 `pending_rulings.py` 任何一行。

### §EI.7 诚实边界

1. **`promote_atoms.py` 的注释剥离是词法的**:字符串字面量里的 `--` 会截断该行。
   失效方向**朝少**(更少活调用点 ⇒ 更安静)⇒ 它打的 141 是**活闸点的下界**。
   对 UNKNOWN 那条腿不成立 —— 那条两个方向都响。
2. **"已 promote"读自 `PROMOTED (was soak-candidate 'X')` 注记**,与
   `tests/test_gate_claim_consistency.lua` 同一个字符串。**一个 promote 了却没留注记的 id,
   对形状 (A) 隐形**,在形状 (B) 里报 UNKNOWN(登记册内响,登记册外静)。
   那个缺口是注记的,不是本工具的,**不在这里糊掉**。
2b. 它只回答「**已登记的**约束在**这棵树**上有没有被违反」,**对没人登记的约束无话可说**。
   登记那一行**本身就是被要求的动作**。
3. **§EI.1 的三波读数是 armed 串的读数,不是条件 (a) 的读数**:本轮**没有**去 W24/W25/W26 的
   录像里找持有侧的逐帧证据(那是录像组的活,且是旧语料)。所以本节说的是
   「**没有已知的** (a) 读数」,不是「(a) 读数不存在」。退休条件按这个措辞写。
4. `midsupyield` 那条「已有看守」的结论,依据是协同组 §EF.9 自报的
   「旧文件正好两条断言变红,就是把那句假话编码进去的那两条」+ 本轮串行跑到的 13/13,
   **本轮没有在缺 §EF.4 的树上反向验证过**。

### §EI.8 为什么**不**在本轮补第四道腿(以及下一轮补什么)

补一条「扫 `test_set.md` 里未裁的『提请总监』条目」的腿是显然的下一步,**本轮故意不做**:
它要认的是**散文里的一个意图**(「提请」「交给总监」「球在总监」),
而**判定一条散文项有没有被裁,正是最容易做成一条每轮喊、没人读的腿的形状**
(§EI.3 那条道理的第三次现场)。做它之前要先有**答得出「已裁」的机器键** ——
最省的形状大概是给这类项一个**轻量 `queue.json` 行**(`kind: "ruling_request"`,无波次无入集),
这样两条现成的腿立刻就看得见它们,**而不必发明第四条**。
**下一轮先定这个形状,再写腿。**

### §EI.9 产物与裸读

- `tools/agent/promote_atoms.py`(新);`iterations/promote_atoms.json`(新,1 行原子);
- `tests/test_promote_atoms.py`(**新 19/0**,自动进套件 —— `run_py_tests.sh` 是 `for f in tests/test_*.py`);
- `tools/agent/mutstand_promote_atoms.sh`(**新,9 真发 9 CAUGHT + 2 量程对照 SURVIVED 正确,零 NO-OP,基线还原后 19/0**);
- `tools/agent/routine_selfcheck.sh`(新腿 `promote-atoms`);
- `iterations/owed_executions.json`(**+1 行** `fixture_mock_extrapolated_location`);
...
## §EK 2026-09-04T22:xxZ 协同组 —— **一个缺陷有两半而只有一半看得见时,看得见的那一半是诱饵**:`ignorePickupList` 的放弃机制两处独立坏掉,单修看得见的那半在 5932 个真实句柄上是 **0,与未修树逐位相同**;本节最该被读的是 **§EK.3:那一行修法产出的 diff 读上去像修复、过得了评审、什么也没动**

**零行为改动。**`bots/` 与 `game/` 逐字节未动,armed 串(62-id)一字未动,
`queue.json` 一字未动,无新 soak id、无新闸、无新 fixture。零 AWS、零 S3、零 EC2、零波次。
**不申请入集,不申请波次。**

### §EK.0 一句话

GH #452 把一条路由给本组并明写「下一轮或谁先拿到」。本轮认领它,按 `0PRICE4` 的
主判据**先跑域价钱**。答案有两层:**修的形状是两半一起**(单修一半 = 0/5932,
等于没修),而**那个决定本语料任何价钱都买不到**(掉落物 0/1012 活帧)。
⇒ 不落地任何行为改动,产出是形状 + 价钱 + 那一个缺失的 dumper 字段。

### §EK.1 域(量出来的)

109 fixture / **1012 活英雄帧**(与 §EF.1 / §EH / §EJ 同一语料、同一分母)。
仪器 `tests/_droppick_sweep.lua`(38s 子进程,三值分桶)。
集合探测的分母是 **5932 个真实物品句柄 / 117 个不同物品名**,取自 fixture 的库存。

- 掉落物帧:**0 / 1012**(`GetDroppedItemList()` 在 mock 里是 `{}`,dumper 不写这条记录);
- `ItemOpsThink` 自己那道库存余位闸:**983 / 1012**(97.1%)。

**这个对比才是价钱**:语料带得起这段代码的**其余每一条前提**,缺的恰好是**一个**
字段。所以「买不到」在这里是一句有地址的话,不是一句含糊的没办法。

### §EK.2 机制:四行,两个互相独立的缺陷
...
- **(1) 元数 —— 看得见的一半。**三处读都只传一个实参 ⇒ `set` 绑到物品**名字**、
  `key` 绑到 nil。`("item_aegis")[nil]` 经字符串元表是一次**合法的读**,答 nil ⇒
  闸恒假、`not SetContains(...)` 恒真。**它不抛错** —— 实测 `shipped_raise` = **0/5932**
  —— 这正是它一直没人指着的原因。
- **(2) 两套下标空间 —— 看不见的一半。**写存 `PickedItem.item`(**句柄**)当键,
  三处读全按**名字**问。`0SLOT` / `slotarb` 之后**同族第三次**。

### §EK.3 三条腿(真实 shipped 函数,同一批 5932 个真实句柄)
...
**⭐ 本节的立法级判据(可复用,超出本主题):当一个缺陷有两半而只有一半看得见时,
看得见的那一半不是更便宜的那一半 —— 它是诱饵。**单独落地缺陷 (1) 的修复,产出的
diff 改了源码、过得了评审、**读上去像修复**,而实测**与未修树逐位相同**。
补上那个缺失的实参,正是缺陷 (1) 看上去在要的东西;它仍然**一次都不命中**。

`bothfix` 同时是**反真空对照**:三条腿是同一对 shipped 函数、同一批句柄,只换了
索引空间,而它**能**说「已忽略」——在每一个句柄上。⇒ 那两个 0 是读数,不是卡死的探针。

可操作的一半:**凡「补一个实参 / 改一个名字」型的修复,先问「这个集合是用什么建的键,
读的人拿什么问」** —— 两边不是同一个东西时,元数那一半是装饰。

### §EK.4 第二条发现:重试循环算出来的 desire 谁也收不到(#452 没有这条)
...
⇒ 在出厂树里那个 `return` **根本不是一个 desire,纯粹是一次 skip**。它跳过的是
同一函数下面的**八个家务调用**(`TrySellOrDropItem`、`SwapSmokeSupport`、
六个 `TrySwapInvItemFor*`),**外加 `ConsiderDroppedTime = DotaTime()`** ——
也就是这个函数自己那道 2 秒节流的重新上膛。又因为 `PickedItem` 是模块级、
只在 `OnEnd` 清空,而 §EK.2 那套放弃机制是死的,**触发这次 skip 的条件是黏住的**。

两条都钉成 `[source]` 断言(裸语句调用点 + 八个尾调各一条),
所以将来的改写会**重新推导**它,而不是继承本节的散文。

### §EK.5 诚实边界(写进普查文件头与测试头)

被驱动的 5932 个句柄来自 fixture 的**库存**,不是 `GetDroppedItemList()`。
它们作证的是**集合管路**(句柄 vs 名字,对库存句柄与掉落句柄逐字同形),
**不是那个决定**。这就是为什么这里用替身合法、而给那个决定用替身不合法。

### §EK.6 仪器自查(变异台里三发是本文件存在的理由)

- **M8** 让 `argfix` 腿偷偷按名字建键 ⇒ 它**照常跑、照常出数,而那个数是对照组的**。
  于是「一行修法没用」会拿**不是一行修法**的腿去量,而且会**反方向**报出来、不吭声。
...
`tests/_droppick_sweep.lua`(新);`tests/test_droppick_domain.lua`
(新,**7/7**,`[ratchet]`);`tools/agent/mutstand_droppick.sh`
(新,**12/12 CAUGHT,零 NO-OP**);`state.json:droppick_PRICING_20260904`;
**GH #500**;报告 `iterations/reports/strategy/20260904T223000Z.md`。
铁律 6 静态门:`bash tools/agent/luacheck_gate.sh` ⇒ **GATE_EXIT=0,0 警告**。

交棒两件,都不要钱:

- **(甲) 总监**:`0MOCKHOLE` 把 mock 归总监。本节给出**第二条**要补的 mock 腿
  (`GetDroppedItemList` 一个非空实现),但**请连同 §EJ.2 的顺序约束一起裁** ——
  它是 `^Get -> 0` 兜底的又一个消费点。⚠️ **补上它只能钉 helper 契约,钉不了这个决定**
  (#452 自己就写了这一句):一个手写的掉落物**不是**一帧真实语料,不满足章程第 2 条。
...
(73 帧里随便挑)。**交协同组**(成员是他们写,不是总监)。**载体 = 新开的 GH #503**(`[strategy]`),不是 `queue.json` —— 后者是批测队列,而这件事零 AWS、不需要波次。⚠️ **单开一条 issue 的唯一理由是不让棒掉**:GH #492 本轮关闭,而铁律 9 连带规则的教训正是「一个 issue 一关,它交出去的东西就从所有队列里消失」(拉野死分支掉了 37 轮)。这也正是 §EL.11 说的那个空格:**这类「变成可做了」的交棒今天没有机器可读的格子**。
...
## §EM 围栏那个数在 9 月 1 日失去了推导 —— 裁定 GH #504,并把闸 (iii) 变成程序

**总监 2026-09-05。批测台 00:11Z §二·甲 立案(GH #504),本节是裁定全文。**
零 AWS 花费(两次 `budgets` 读全是免费只读)、零发波、`bots/`+`game/` **零 diff**、
**没有 promote/reject 任何 id**。

### §EM.1 裁定(三句)

1. **批测台是对的,照收。** 九月的围栏是 **`$50`**,不是章程里写着的 `$80`。
2. **但改这个数不是修复**,那正是 08-28 那次已经犯过一遍的错(见 §EM.2)。
   ⇒ **闸 (iii) 从今天起读 `tools/batch_test/soak/wave_fence.py` 打的那一行,
   章程里不再存在一个可抄的围栏数**。与 GH #469(乙)对闸 (i) 是同一味药。
3. **`$90` 刹车线与 `$100` 批准线本轮不动** —— 那是 owner 的地界,工具只**应用**不**推导**;
   燃烧率(GH #505)另走 `DECISIONS_NEEDED`,不在本节裁。

### §EM.2 ⭐⭐⭐ 本节最该被读的:**08-28 那次重写取消了自由度,却把自己的答案写成了字面量**

08-28 的条文自己写得毫不含糊,立法目的就是**取消总监的自由裁量**:

> ⇒ **新写法:围栏 = 下一个尚未跨过的 owner 可见 Budget ACTUAL 告警档。**
> …… 这样围栏不再是猜的 …… **副作用即是目的**:下一次「该不该再抬」不再由总监自由裁量。

**这是一条没有自由参数的规则。** 然后同一段把这条规则**求了一次值**,并把值写了下来:

> `$50` 已跨 …… **下一档就是 $80**。…… **当月唯一的操作数是上面的 $80。**

`dota2bot-batch` 是 `TimeUnit: MONTHLY`。**9-01T00:00Z,ActualSpend 归零,
「已跨过的档」这个集合跟着归零** ⇒ 规则自己的输出变回 `$50`,
**而章程、九月每一轮报告、每一次发波算术,继续读那个 8 月的 `$80`**。

> **一个被推导出来的值被写成了字面量,而它的输入按一张没人在看的日历自己动。**

**三条性质让它隐身,这也正是本轮交付的是程序而不是一个改对的数字的理由**:

| 性质 | 后果 |
|---|---|
| 失效方向是**多给钱**($30),不是少给 | 少给钱的围栏会**卡住某个人**,于是自己举手;多给钱的**没有受害者** |
| 整个 8 月里,错的那一行与对的那一行**逐字节相同** | **没有 diff、没有缺失读数、没有报错** |
| 它只在花得多的时候咬人 | 即**恰好在那封 owner 邮件最近的时候**才生效;按 GH #505 的燃烧率,首次真咬在**九月中旬** |

**同族**:`pullcad` 陷阱(一个 gate 因为别处 promote 而被冻成 FALSE)、
GH #469(闸 (i) 是散文加心算,W12/W20/W44 用同一个机制破了三次)、
#205/#213(一道跑不成的门读成一道通过的门)。**药也同一味**:
让「每月 1 号要重新推导」不再是任何人需要记得的事。

### §EM.3 ⭐ 而 AWS 自己就能答,连百分比都不用写死

章程当初把 `50/80/100%` 手打进条文,理由是 `AGENTS.md` 记着
`dota2bot-agent` **读不了** `DescribeBudgetActionsForBudget`。
**本轮实测:那道权限缺口不覆盖 notifications** ——
`budgets describe-notifications-for-budget` 在本账号 **exit 0**,免费,返回三条 `ACTUAL`。
⇒ **告警档是读出来的,不是断言的**,工具里没有 `50/80/100` 这个常数。

而且它顺带买回一件**比算术更强的证据**:每条 notification 自带
`NotificationState`,**三条全是 `OK`** ⇒ **AWS 自己说九月这三档一封邮件都没发过**,
这是对「$50 未跨」的独立确认,不是我们拿 MTD 再算一遍。

### §EM.4 工具里钉死的三条裁定(留给散文就是本条自己的成因)

- **裁定 1 —— 哪一档是「下一个尚未跨过的」。** 告警是 `ComparisonOperator: GREATER_THAN`
  ⇒ 邮件在**严格超过**时才发 ⇒ **恰好坐在档位上的月份没有发过它** ⇒
  `fence = min{ T : T >= actual }`,且**恰好落在围栏上的一波允许发**。
  保守样子的另一种写法(`T > actual`)会在花费**碰到**档位的那一瞬跳过它,那是更松的方向。
- **裁定 2 —— 百分比还是绝对值。** `ThresholdType` 在响应里**缺省时被省略**,而缺省值是
  `PERCENTAGE` ⇒ 缺省即按 `PERCENTAGE` 读,**并在自己那行里说出来**。
  ⚠️ **这条比看上去重要**:limit 恰好是 `$100`,于是**百分比读法与绝对值读法今天逐位相同** ——
  正是那种在 limit 变化之前一直藏着的巧合。所以**每一轮都打印口径**,不留给读数的人去推断。
- **裁定 3 —— AWS 与我们的算术不一致时,谁都不发波。** 两个方向**都** exit 2(不是 0):
  我们说未跨 / AWS 说 `ALARM` ⇒ 邮件已经发了,围栏建在假前提上;
  我们说已跨 / AWS 说 `OK` ⇒ 我们**跳过了一档还活着的档**,即**更松**。
  **两个方向要么更松要么无法解释,而一次被挡住的发波是安全的失效。**

### §EM.5 ⭐ 变异台:**承重的那一发就是这个缺陷本来的样子**

`tools/agent/mutstand_wave_fence.sh` —— **M1–M5 CAUGHT + M6 控制组 SURVIVED,零 NO-OP,
`STAND_EXIT=0`**,前后基线各绿一次。

**M1 才承重,而 M1 不是一个假想的手误 —— 它就是实际上线的那个 bug**:
把 `pick_fence` 换成「永远返回 $80」。**注意它不破坏什么**:8 月的每一个读数、
每一行输出、刹车、一致性检查**全部照常** ——
**整整一个月里它与正确实现不可区分**。一个抓不住 M1 的用例集,
测的是从来没有人怀疑过的那部分算术。
- **M2** 缓存在另一端(永远取最低档)。它是**保守**方向的,所以需要自己的一发:
  只检查「围栏不会太高」的用例集会放它过去,而裁定是**围栏等于规则的输出,不是手边最安全的数**。
- **M3** `GREATER_THAN` 读成 `>=`;**M4** 百分比分支直接取原始数 —— ⚠️ **它在真预算上什么都不改**
  (50% of $100 = $50),只有 limit 变化时才现形,所以它只能被 `$200` 那组用例抓住。
- **M5** 把不一致降级成「警告但仍然通过」(一个很像样的「别让滞后的告警状态卡住发波」的改法)。
- **M6 是诱饵,必须 SURVIVED**:数字 `80` 出现在**注释**里 —— 而它**真的到处都是**
  (本仓库的章程与报告里,`$80` 作为 8 月的存档答案反复出现)。
  一个 grep 字面量而不驱动推导的台子会把它记成 CAUGHT,
  于是**每次有人引用这段历史就红一次**。⇒ **§EG.5 / §EE.5 / §EL.5 第四次现场:承重用例是诱饵用例不是违规用例。**

### §EM.6 裁定 GH #498:**补跑不受闸 (i) 管**(按批测台交上的三条证据,不重猜)

批测台 §四·乙 交上三条**既有**证据,本轮据此裁,**不新造推理**:
① 章程条文本身(`rerun` 块不开新窗口,且**按 schema 结构性成立** —— `wave_throttle.py` 只读
`machines[]`,不读 `rerun`,不依赖谁诚实填字段);
② **W45 先例可量**:slate `06:30:52Z`,两台补跑机 `07:12:53Z`/`07:12:58Z` = **+42min**
⇒ **若补跑受闸 (i) 管,W45 那一轮就是一次未被记录的突破**,而当轮记录把它写成合规;
③ W45 成本注记的定性:**「一条预先登记的处置被执行,不是一个新的花钱决定」**。

⇒ **裁定:补跑机(`rerun` 块)不受闸 (i) 的 6h 窗口约束。**
**理由是闸 (i) 管的是新的花钱决定的节奏**,而补跑按定义是**买回一粒已经在那一波预算里付过钱的种子**。
**节流它的不是闸 (i),是它自己那两道围栏**:每粒种子**只补跑一次**、**每波最多 2 台按需补跑** ——
两条都是**有界的**,所以豁免闸 (i) 不打开任何无界的花钱通路。
**闸 (iii) 照常适用**(补跑的钱进围栏算术)。
⚠️ **批测台本轮用「等解锁」绕过了这个问题,那是对的** —— 在裁定落地前,
保守默认让两种读法下都合法;**从本节起不必再等**。

### §EM.7 产物、门、以及一处诚实限定

**新**:`tools/batch_test/soak/wave_fence.py`、`tests/test_wave_fence.py`(**25 checks / 0 failed**,
自动进 `run_py_tests.sh`)、`tools/agent/mutstand_wave_fence.sh`(**5 CAUGHT + 1 控制 SURVIVED**)。
**改**:`iterations/streams/batch-desk.md` §4b(iii)(读工具,不读数字)、
`iterations/DECISIONS_NEEDED.md`(GH #505 燃烧率,owner 地界)。

**真机端到端**(`--planned 1.10 --pending 1.76`,即批测台本轮的算术):
`fence $50.00` / `projected $20.633` / `headroom $29.367` / **`WAVE_FENCE: CLEAR (exit 0)`**,
与批测台手算的 `$20.63` 逐位对上。

⚠️ **诚实限定,两条**:
(甲) **本轮 `bots/`+`game/` 一行未改** ⇒ 铁律 6 的**静态半**(`luacheck bots game`)
虽然真跑了且干净,**对本轮改动几乎无话可说**;真正的验收是 `tests/run_py_tests.sh` 与变异台。
(乙) **工具的 AWS 路径今天只在一个月份、一个预算上跑过一次**。
跨月那一面**是用例买的不是真机买的**(§EM.5 M1)——
而这恰恰是本节的论点:**跨月那一刻没有人在场,所以它必须由一个每次重新求值的程序承担,而不是由观察。**
## §EN 2026-09-05T0x:xxZ 协同组 —— **把一个否定结果 ratchet 起来,而不是当结论存档**:§EF.7 那张「四条都作不了证」的表,是被**别人作用域里的一次 harness 修复**解锁的,而协同组 backlog 上没有任何一条会在那天注意到;本节最该被读的是 **§EN.2:替代解释(「上面那条腿本来就挡住了」)是被量掉的,不是被论证掉的 —— 20 个里只有 4 个**

**认领 GH #503**(它不是本组已交棒的:总监开它的**唯一理由**就是「不让一根棒掉在地上」——
它交出去的东西今天没有机器可读的格子,而铁律 9 的连带规则说得很清楚)。
**本轮有行为改动**(前四轮都是零行为改动的域价钱),但只有**一条合取项**。

### §EN.1 主判据(立法级,可复用,超出本主题)

**⭐ 把一个否定结果 ratchet 起来,而不是当结论存档。**
§EH 给 §EF.7 的四条镜像成员定价,答案是「一条都作不了证」。那本可以作为一条结论
归档 —— 然后它会**一直读作四条**。解锁其中一条的修复发生在 **harness 里**
(GH #492,总监补 mock 的 `GetExtrapolatedLocation`,§EL),
**协同组自己的 backlog 上没有任何一条会在它发生的那天注意到**。
`test_midsupmirror_checkability.lua` 的 `[ratchet]` 腿**按设计开火并点了名**,
本轮才存在这个工作单元。
**可操作:一个「今天做不了」的结论,若它的原因住在别的组的作用域里,就必须写成会
自己开火的断言,而不是写成散文。**

### §EN.2 ⭐⭐ 替代解释是被量掉的,不是被论证掉的

敌人进 700 打断扫描,**通常也算「1600 内有团战」**,而 `IsInTeamFight` 就排在这条
新子句上面**一行**。若守卫拒掉的盟友本来就都被它挡住了,这条合取项就是**装饰** ——
正是前一天 `droppick`(§EK)量出来的那个形状(0/5932,与未修树逐位相同)。
所以这条不能靠论证打发,只能量:
**守卫拒掉 20 个候选,其中只有 4 个已被上面那条腿挡住 ⇒ 不被吸收。**

### §EN.3 机制与铁证帧

`J.ShouldTpSupportTowerFight` 在**取响应名额之前**先对自己问一句「有敌人贴脸会打断
这个 TP 吗」(`J.CanEnemyInterruptTpChannel`,即 2026-07-23 已 promote 的 `tpsafe2`
守卫)。逐条镜像它的 `J.HasAvailableSupportResponder` **没有对它即将交出名额的那个
盟友问这一句** ⇒ 让路可以把队伍**唯一**的响应名额交给一个**根本读不完条**的支援。

**铁证帧**:`f_260820_043637_axe_ring_close` **t=393.4** —— 核心 slardar 距它要响应
的那座塔 **15,747**(过了远端闸 3500,所以这是个**可达**的决定瞬间,不只是谓词碰巧
接受的一对),而未修镜像给出的**唯一**支援是
**skywrath_mage,24% 血,axe 在 188 外(攻击距离 300)** —— 已在打击范围内。
条被打断,这条前线**谁也没去**。
**独立第二例**(不同局、不同英雄、不同塔):`f_260820_103216_cm_es_aftershock`
t=473.5,earthshaker,crystal_maiden 在 196。
...
| 接受集 | **2366 → 2215**,且 `ship_true + flips == pre_true` 逐位相等 |
| 盟友级分母 | **285** 个走到打断子句的支援候选 |
| 守卫拒掉 | **20**;其中已被 `IsInTeamFight` 挡住的 **4** |
| 守卫抛错 | **0**(GH #492 之前这里是 257/257) |

**方向是收紧**:合取项 ⇒ 接受集是真子集 ⇒ armed 只会在**更少**的帧上让路,
**只能朝 shipped 走,不会离开它**。这一条不是注释里的声明,是
`ship_true + flips == pre_true` 这条断言。

### §EN.5 不开新 id(pullcad 陷阱),以及仪器第一次跑就读了自己的注释
...
——它查的是调用点存不存在,不是谓词能不能为真。**杠杆是 `midsupyield`,
它从未在任何波次里 armed。** 这与 8 行之上那条配对修复(GH #489)是同一个判断,
本轮把它从散文升级成断言(`MIRROR_SOAKID == 0`)。

⚠️ **而那条断言第一次跑就读出 1,树里却一个 id 也没有** —— 命中的是**修复自己那段
解释「为什么这里没有 id」的注释**。凡「这个谓词**应用**了哪些闸」的论断都必须**读代码
不读散文**;已改为先 `strip_comments` 再解析,把这次实测钉成变异台 **M4**,
并把同族修正一并落到 `test_midsupfar_yield_target.lua` 的孪生计数上。

### §EN.6 ratchet 重取(手写新地板 + 写清理由)

- `test_midsupmirror_checkability.lua`:`short` **4 → 3**,并**新增一条断言:被修的
  必须恰好是能作证的那一条**。光对计数不够 —— 落一条作不了证的腿同样能把计数改对,
  却恰好破坏了这个计数存在的理由(章程第 2 条:行为改动必须带真实帧 fixture)。
- `test_midsupfar_yield_target.lua`:孪生计数 **4 → 3**。
- `[ratchet]` 腿新增 **landed** 断言:**landable 是关于语料的论断,LANDED 是关于树的
  论断,分开记** —— 一次让本文件仍然全绿的回退,会让上面那份定价重新描述一棵不存在
  的树,而那正是 §EF.7 曾经「多读了一天四条」的成因。
- **另三条腿原样不动、仍不可作证**:两条是跨帧会话记忆(单帧 fixture 按构造没有
  历史),一条是 bot-VM 状态(dumper 不 dump)。**不要顺手一起动。**

### §EN.7 诚实边界(继承自 mock 修复,重述以免丢失)

fixture 是**一个瞬间**、不带速度,mock 把每个单位建模成**原地不动** ⇒ 守卫只有
`nNow <= nReach`(「他现在就够得着我」)那一支可达,「他在贴上来」那一支在本语料上
**不可达**。⇒ **上面每一个计数都是打断的下界,不是上界。**
该模型声明在 `tests/mock/replay_fixture.lua`,由
`tests/test_fixture_extrapolation_mock.lua` 量着,所以哪天 fixture 带上速度,
它会**变红而不是变安静**。

### §EN.8 产物

`bots/FunLib/jmz_func.lua`(**唯一**行为改动:一条合取项);
...
## §EQ 2026-09-05T07:xxZ 协同组 —— **一个 GATED 家族把谓词写对了,不构成它旁边那个 PROMOTED 函数问的是同一个问题**:`J.IsFieldRegenSituation` 自 08-22 起就用归属式读法,而对着同一趟回家路的出厂守卫 `J.ShouldStayAndRegen`,在同一个文件里、290 行之外,从来没被带上;本节最该被读的是 **§EQ.4:两发变异体是被**存活**买来的不是被想出来的,其中一发买到的是「本语料按构造抓不住」,于是被声明而不是被补一条会因别的原因开火的断言**

**OWNER_PRIORITIES P4.4**(今日新立:协同组每轮工作单元的**主体必须是一个 `bots/` 行为改动**)+ **P2**(低血不回家,**决策侧,球自 08-22 起在本组**)。
`[strategy]` open issue 本轮扫过 **#511**/#503/#500/#495/#489/#485/#480/#475/#467/#464/#456/#455/#452/#445 —— **无一条未认领**(#511/#503 是本组前两轮已认领并交棒的;#485 的球在总监,其 (乙) 分支已于 09-04T19:41Z 被裁为「本轮不落 `ffeat`」)⇒ 按铁律 9,**优先项凌驾于 issue 流**,本轮取 P4.4 + P2。
**有行为改动,一条合取项宽**:`bots/FunLib/jmz_func.lua`。armed 串一字未动;新 id `stayattr` **提议**入集,**未 armed**。
零 AWS、零 S3、零 EC2、零波次、零 CE 调用。

### §EQ.1 缺陷:一句没有归属的追击读数,住在**出厂**代码里

`J.ShouldStayAndRegen` 是 **PROMOTED**(原 soak id `tphome`,28 局 A/B 后转正)—— 它**在每一局 Turbo 里都活着**。它的追击子句是:

```lua
if bot:WasRecentlyDamagedByAnyHero( 3.0 ) then return false end
```

**没有归属**。一发从地图另一头落下的全局大招,和一个站在身上的英雄,在这一行上读数完全一样 ⇒ 否决触发 ⇒ 「留下喝补给」被撤销 ⇒ 一个安全的残血 bot 被放去走回家或 TP 回家。**这正是 owner 优先项 P2 逐字禁止的那趟路**(owner 原话:「血量低也尽量不要回程」)。

**⭐ 主判据(立法级,可复用,超出本主题):一个 GATED 家族把某个谓词写对了,不构成它旁边那个 PROMOTED 函数问的是同一个问题。**

对着**同一趟回家路**的 gated 家族(`stayfield` / `stayfield2` / `fieldbuy`,谓词 `J.IsFieldRegenSituation`)**自 2026-08-22 起就是归属式的**,源码原话:

> `-- Attributed danger: someone hit me recently AND is still near me.`

⇒ **缺的不是想法** —— 想法就在**同一个文件里,290 行之外**,用仓库自己的话写着。缺的是它**停在了实验代码与出厂代码的边界上**,从来没被带过去。
⇒ 可操作:**「这个仓库知道该怎么问」和「这条出厂路径确实在问」是两句话**。读了 gated 那一半就以为出厂那一半同理,是本节要钉住的那个动作。与 §EO 是同族的反面:那次是**一个守卫在 helper 里已经跑过了**(论断按 token 集合立、按求值集合才为真),这次是**一个守卫在隔壁写对了而这里没写**(两个函数在同一文件里问同一件事、答案不同)。

### §EQ.2 修法:一条合取项,独立闸,方向由构造定死
...
- **未 armed = 出厂读数,靠短路不靠论证**:第二个操作数在**第一个析取项**上短路,否决条件逐字是出厂表达式,**求值顺序也不变**。
- **方向由构造定死**:加一个析取项**只能让否决更难成立** ⇒ armed 的 TRUE 集是 shipped 的**严格超集**。这不是注释里的声明,是 `flip_true_to_false == 0` 那条断言(1012 帧驱动实测 **0**)。
- **独立闸,不与任何 id 合取**(`pullcad` 陷阱)。已从散文升级成断言:`STAY_NIDS` 必须恰好为 **1**。⚠️ 而**这条子句自己的注释里就点了两次 `IsSoakCandidate` 名**(在解释这个陷阱),不剥注释就会被**警告本身**满足 —— 变异体 **M5** 就是这一发,而它是 §EN 首轮真实踩过的那一脚的再犯。
- **turbo 是结构性的**:函数首行已经问过 `J.IsModeTurbo()`,不重复加。
- 新 helper `J.HasNearbyHeroDamager` **不与出厂兄弟扫描共用代码**:把一个出厂调用点重构成共享,会把「一个杠杆」变成「一次出厂行为的重构」(`lanefix` 教训)。**M13 就是这一发**,它 armed 的形状是:所有行为断言照旧通过,而一杠杆的声明已经失效。

### §EQ.3 域价钱 —— 承重的那个数是 **88**,它说的是「收窄」不是「删除」

109 fixture / **1012 活英雄帧**:
...
**88 是本次改动最承重的一个数**:出厂那条子句**十次里对九次以上**。所以这是**收窄**,不是删除 —— 一个「把这行删掉」的修法会同时放掉那 88 帧的真实追击。

7 帧的**互斥四分区**,按函数**自己的子句顺序**取,**数出来不是减出来**(铁律 4(iii)):翻转 **1** / 血量带外 **2** / **1200 环挡住 0** / 补给子句挡住 **4**。四桶之和 `== 7` 由断言钉住 —— 因为「杠杆在这一帧惰性」和「这个情形很罕见」是两句不同的话,只有具名的桶能把它们分开。

**⭐⭐ 铁证帧就在 owner P2 自己钉的那张 fixture 上,读的是上面另一个英雄。**
`tests/fixtures/f_260822_063722_lina_tp_home.lua` **t=349.0**:**jakiro 405/1000**,2.6 秒前吃了 **zuus 从 ~10,400u 外**扔来的雷神之怒 **212+17**(该帧大招 cd 读 **128.3**,即刚放),身上 **tango 在跳**,**1200 环空**。
shipped:否决触发,他被放回家。armed:他留下把 tango 喝完。

### §EQ.4 两发变异体是被**存活**买来的,不是被想出来的(本节最该被读的一条)

`tools/agent/mutstand_stayattr.sh` ⇒ **14 发:13 CAUGHT(含 M14 对照按声明 SURVIVED)+ 1 声明式 UNMEASURABLE,零 NO-OP,零不合规,EXIT=0**。而**第一轮不是这个数**:

- **M8**(把 stub 放宽成 arm **全部** id)**第一轮 SURVIVED**。整个套件**分不清「armed 一个 id」和「armed 全部 141 个」** —— 因为在这份语料上两者给出**同样的数**,而**那正是放宽了也永远不会被发现的条件**。已补 `arm_leak` 计数器(驱动式:问 shipped 代码**能观察到什么**,不读 stub 的源码),现已 CAUGHT。**这是一个真实的断言缺口,由存活发现,不是由复审发现。**
- **M10**(让 1200 环那一桶不可达)**第一轮 SURVIVED**,而查下去发现**本语料上它按构造抓不住**:5 个带内帧**全部**环内无人 ⇒ 杀掉那个分支**改不了任何一条断言看得见的数**。
  ⇒ 诚实做法是**声明**,不是编一条会因**别的原因**开火的断言。已登记为 **UNMEASURABLE**(台子新增这个结局:**不是通过,也不是失败**),并补 `unattr_ring_tested` **可达性**断言,把「分支没跑过」与「世界里是 0」分开(**GH #171 形状**)。语料长出那张帧的那天,台子会**自己打出改判提示**。
- **⚠️ 顺带一发,记在自己账上**:补 `arm_leak` 探针之后 **M6 变成 NO-OP** —— 探针落在它两行锚点的**中间**,于是**一发悄悄不存在了,而台子照旧为它打了一行**。这是规则 2 的**逆命题**。已把锚点改成只钉那一行赋值;**NO-OP 在本台计入「不合规」而不是放行**。

**⇒ 一套第一轮全 CAUGHT 的变异台,通常是没对准任何东西。**

### §EQ.5 诚实边界(四条,写在结论之前)

1. **看不见战争迷雾**:从雾里爆发然后退到 3000 外的敌人,在这一行上与全局大招读数相同 —— **与兄弟扫描逐字相同的边界**。1200 环原封不动,那条子句才管「有没有人站在我身上」。
   ⚠️ **而语料并不佐证这次交接**:5 个带内帧里 **0** 个环内有人 ⇒ 那个 0 是**空洞的**(说的是「没有一帧让环干过活」,不是「环覆盖了这个情形」)。**这正是 M10 抓不住的同一件事**,两处互为证据。
2. **见证帧上最近的敌人是 1,556u 外的 spirit_breaker**,冲刺已转好。他**没有**造成那次伤害(该帧 `recent_damage` 只点了 zuus)⇒ 归属扫描**答得对**;但「1,556 且在冲」是这个杠杆**不衡量**的真实威胁,因为管它的是原封不动的 1200 环。**作为该帧的事实断言下来,不辩解掉。**
3. **P2 自己钉的 lina 本人整函数不翻转** —— 补给子句(`not bHasFlask and GetGold() < 90`)仍然挡着她。而 mock 的 `GetGold` 落在 `^Get -> 0` 兜底上(**GH #495**),**那是一句类型断言不是她真实的钱** ⇒ **真实对局里会不会翻,UNKNOWN,不声称**。她那一帧上被证明的是本杠杆**自己那条子句**:否决触发,且**没有任何打过她的人在附近**(最近敌人 **6,596u**,与源码注释里那个数逐位相同)。
4. **一帧是一个瞬间**。本轮说的是 **t 时刻那个决定**是错的,**不声称**留下的 bot 接下来十秒活了下来。

### §EQ.6 入集提议(请总监裁)

**提议 `stayattr` 入 armed 集**(62 → 63)。

⛔ **一并请收一条**反向**提醒**:本 id 与 `stayfield`/`stayfield2`/`fieldsip` 那个**共同 promote 原子**(`field_hold_needs_magnitude`,§EI.1)**不同族** —— 它动的是 **PROMOTED 守卫**、**不读** `J.HasFieldRegenSource`、也**不碰** `J.IsFieldSipEnough`,所以那条单向约束**不覆盖它**,**不要顺手并进去**。三个 field id 的持有侧问「有没有东西喝」;本 id 问「打我的人还在不在」。同一个 owner 优先项,不同的谓词,不同的原子。

**载体项**:`stayattr` 的唯一闸点在 `bots/FunLib/jmz_func.lua`、消费者是 `mode_retreat_generic.lua` 与 `ability_item_usage_generic.lua` ⇒ **generic**,对载体项**零贡献**(与 `campbind`/`arbheart` 同类);`carrier_terms.py` 请照常自判,不要照抄本句。

**⚠️ 收割前必读**:条件 (a) 的判据是**留没留下**,不是**活没活下来**(诚实边界 4)。armed 腿要看的是「被全局技能打到之后仍留在原地喝补给」的帧,不是胜负也不是存活率。

### §EQ.7 验证

- `bash tools/agent/luacheck_gate.sh` ⇒ **GATE_EXIT=0 CLEAN,0 warnings**(冷启自己装的 `lua-check`)。
- `lua5.1 tests/run_tests.lua stayattr` ⇒ **11 tests, 0 failures**。
- `bash tools/agent/mutstand_stayattr.sh` ⇒ **13 CAUGHT + 1 UNMEASURABLE(声明) + 对照 SURVIVED,零 NO-OP,EXIT=0**。
- 相关腿:`gated_helper_nesting_census` **10/0**(本轮新增一行并按 **(P)** 手读 —— **这一行是开工自检自己点出来的**,不是本轮自查发现的)、`gate_claim_consistency` **16/0**、`smoke_load` **3/0**、`stayfield` **55/0**、`fieldsrc` **8/0**。
...
已开 **GH #518**([harness]),**本轮不自己实现** —— 它值一个自己的工作单元,而本轮已经花掉了。

### §ER.3 顺带:一条**只在全量跑里存在**的 harness 泄漏(已立案 **GH #517**)
...
## §EV 2026-09-05T12:5xZ 总监 —— **我取的那根棒,在它被登记的三小时之前就已经被别人做完了**;本节最该被读的是 **§EV.2:一行 `kind: manual` 在「已执行」与「从没人碰过」之间逐字节相同,而 GH #518 的认领字段对这种失效结构上是瞎的**(立案 **GH #523**)

**执行的是**:`iterations/owed_executions.json:outlatch_check1b_reason`(裁定 §EO.6 交棒 (丙),GH #511)——
上一轮自己排的「下次触发 ①」,并且是 **GH #518 认领字段的第一个真实用户**。
`bots/` + `game/` **零 diff**,零 AWS,无 promote/reject,armed 串 / `queue.json` **一字未动**。

### §EV.1 时间线(全部 2026-09-05,全部裸读得出)
...
**没有人违反任何规则。** 录像组做的是对的(那个文件是它写的,它认为那是**自己的**断言);
协同组写交棒时是对的;总监登记时读的是裁定文本。

### §EV.2 ⭐ 缺陷:`kind: manual` 的一行,在「已执行」与「没人碰过」之间**逐字节相同**

它每轮都报 `OWED`,而那**正是它设计上要做的事**(`pending_rulings.py` 自己打
「no machine check (kind=manual) -- this row is a reminder, not a gate」)。于是登记那一刻
**没有任何东西**会说「这活三小时前就干完了」,登记之后每一轮**继续**报 OWED,读起来跟真欠着一模一样。

⚠️ **GH #518 的认领字段对这种失效结构上是瞎的**:认领挡的是**两个会话同时做**,
而这里的工作**在任何认领可能存在之前就落地了**。两者同根(存量字段分不开状态)但不同形 ——
§ER / #518 是「没有字段记录**已经有人在做**」,本条是「没有字段记录**做没做过**」;
与 backlog §6b(08-25 假弃置)是同一个根的第三面。

### §EV.3 处方:`done_when` 加一种 `kind: "text_absent"`(GH #523,已登记为 `text_absent_done_when_kind`)

那一行的 `done_when_note` 从**正确的前提**推出了 `manual`:「注释说得对不对没有任何断言读得出来」——
**这句话是真的**。**但它从来没问过容易的那一半:那两句写错的话还在不在。**
它们是字面串(`the whole "75% of capture attempts are aborted" finding is about that missing guard` /
`if this fails, the guard landed`),`text_absent` 会在 **07:01Z 当场读 DONE**,那一行**根本不会被登记**。

> **通则:凡是「把这句写错的话改掉」形状的裁定,正面那一半(新说明对不对)不可读,
> 反面那一半(旧说明还在不在)永远可读。**

⛔ **三态,不是两态**:文件读不到 = `UNCERTIFIABLE`(**不是 DONE**);文本还在 = `OWED`;
文件在且文本没了 = `DONE`。**必须同时要求 path 存在** —— 文件被整个删掉时读 DONE 是这种键唯一的假阳,
而「文件没了」已经被 `path_absent` 占走了。失效方向与 GH #171 同一条:**读不出来的条件永不读成已执行**。

### §EV.4 退休的验收:四条对上,第五条**豁免并写清理由**

`done_when_note` 的验收句逐条:(1) `:12-13` 不再声称守卫缺失 ✅(录像组 07:01Z);
(2) `:57` 不再声称变红意味着守卫落地 ✅(同上);(3) 新说明点名 `J.CanNotUseAction` 是真实住址 ✅(同上);
(4) 说明那个 token 落进 `mode_outpost_generic.lua` 的那天是**一个 no-op** —— **本轮之前缺,总监补上**
(纯增 8 行、**零删除**,并指向 `tests/test_outchan_domain.py:168` 那条会在那天开火的断言)。

⛔ (5)「检查 1b 的**断言本身逐字未改**」—— **不成立,而且我不去恢复它**。
...
### §EV.5 ⚠️ 认领字段第一次被真用,而它**没有起到信号作用**(抄给下一个总监)

我按 §ES 写了 `claimed_by`/`claimed_at`,但**在同一个工作单元里就把活干完并退休了** ⇒
认领与成品一起落地,正是 `pending_rulings.py` LIMIT 12 第二句点名的那种「**记录不是信号**」。
**归因不是懒**:取棒后两分钟内就发现活已经做掉了,剩下的只有一处 8 行补写 ——
**一根已经被做完的棒,认领它在信号意义上没有任何东西可以保护**。
这**不构成放松 §ES 的论据**(下一根真要花一轮的棒仍然必须先推认领再开工),
但它说明**认领的价值与剩余工作量成正比,而剩余工作量在取棒那一刻还不知道**。

### §EV.6 铁律 6 / 自检 / 巡检

⚠️ **本节 §EV.6 原写「自检卡在 Lua 检测器腿上」—— 它随后跑完了,真读数见报告第 11 节**(`worst exit 3`,Lua 腿 84 文件 0 失败,python 半边 93/7/2 且七条归属全部查实、零条本轮引入)。
静态半见报告;**本轮最终落地的 diff 里没有一行 Lua**(python 注释 + json + markdown)。
...
### §EX.8 收尾登记(push 之后,按铁律 6 补款 GH #290 的顺序)

- **GH #531 立案**:§EX.4 的「第三种形状」已开成 issue —— *「谓词恒真而答案恒 0 ——
  `check_armed_wiring.py` 结构上看不见它,聚合读数里它跟『已测试且中性』一模一样」*。
  ⚠️ **本轮登记的共同 promote 原子只管 `zusstatic` 这一个 id**(手写的一行,不是检测器);
  #531 要的那一半是**通用检测**:还有几个 armed id 的答案由一个 gated 在别的 id 上的
  句柄/返回值决定,**没有人知道**。已在 issue 里写下已知的最小真值集与建议验收形状。
  ⭐ **顺带验证了 §EX 自己的纪律**:本节初稿在 `promote_atoms.json` 的 `ref` 里写的是
  **`GH #529`**(当时最高是 #528,顺号推出来的),而实际拿到的号是 **#531** ——
  两个流在同一小时里各开了一个。⇒ **顺号推测的引用号会错,而且错得像个普通数字**;
  `claim_precheck.sh` 对**尚不存在**的 issue 号**无话可说**(它解析 `tests/` / `bots/` 路径、
  `state.json:<key>`、`§XX` 和被 git 词锚定的 commit,**不解析 GH 号**)。
  本轮的做法是**先把号从引用里拿掉、push、开 issue、再回填** —— 与 §EV.7 (甲) 同一条顺序。
- **铁律 6 补款本轮遵守了**:发 issue **之前**跑 `claim_precheck.sh`,读数是
  `MISSING section test_set.md §EX` + `DO NOT PUBLISH YET`(§EX 当时只在容器里);
  push 之后复跑 ⇒ `resolved on trunk 8 → 9`,`0 finding(s)`,`OK to publish`,`PRECHECK_EXIT=0`。
  ⚠️ 上一轮(§EV.7 甲)在这里是**违反**的,本轮是**同一个工具、同一个顺序、结果相反**。

### §EX.9 ⚠️ 并发落地的一条读数,**方向与 §EX.1 一致,不改变裁定**

本轮 push 前 rebase 时收到录像组 `fe2fbf90`(16:0xZ):
**W48 全波重跑 `stayattr` 条件 (a) —— 域第一次抵达,(a1) WORKING / (a2) 仍 INDETERMINATE**。

- **不改变 §EX.1 的裁定**:`stayattr` 退集的理由**从来不是证据不足**,是**时机**
  (P4.2 废止 §BB.4,而 §ET 那次入集在澄清落地之前)。⇒ 新证据把 §EX.1 那句
  「§ET 买到的证据逐条仍然成立」**加强**了,没有动摇它。
- **它正是 §EX.1 预告的那件事**:「W48 已在飞不作废,其 `stayattr` 读数按 §ET 归档为
  **额外 (a) 证据**」—— 这份读数就是那一笔,归档处不变。
- **对重新入集的影响**:解冻后重提时,(a1) 已是 WORKING 的真帧读数,
...
- **(甲) 总监**:按 P4.2 冻结把 `'staybag'` 记作 **FROZEN-HOLD**;§FC.2 已立案 **GH #542**
  (`[harness]`:`pullcad` 的第二形态 + 对 nesting census (A) 分类的限定补句)。
- **已发表**:GH **#538** 追评并**关闭**(它自己写的验收行原样引用);GH **#542** / **#543** 新开。
  发表前跑 `claim_precheck.sh` ⇒ **PRECHECK_EXIT=0 / 本地领先 `origin/main` 0 个 commit**。
- **(乙) 批测台**:解冻后本 id 可**单臂**发波(与 `staysrc` 重叠实测 0)。
  另一条直接影响发波设计:**`'bagsalve'` 单臂发波在本语料上按构造读不出任何东西**
  (`flips_bagsalve == 0`)—— 若历史上给它发过单臂波,那一波的零应按
  「域未到达」而不是「已测试且中性」重读(GH #531 同族)。两条已立案 **GH #543**。
  本组本轮**不发波、不排队、不花钱**。
...
## §FD 2026-09-06T02:11Z 协同组 —— **在一棵检测器按源码文本解析的树上,函数签名是一个已发布的接口:动共享谓词的代价由「解析它的声明」的那些文件来付,而它们对这次改动无话可说**;本节最该被读的是 **§FD.6:一个变异体第一次 `CAUGHT` 了,而它砍错了函数 —— `CAUGHT` 掩盖一发打偏的刀,和 `SURVIVED` 暴露它一样彻底**

**产物**:`bots/FunLib/jmz_func.lua` **新增**的 gated 谓词 `J.ShouldFieldBuyRegenHurt`
(`buyband`,turbo-only,**未 armed**,STANDALONE)+ `bots/item_purchase_generic.lua` 购买点一条 **OR 臂**、
...
零 NO-OP、零非声明存活,STAND GREEN,EXIT=0**)、`test_gated_helper_nesting_census.lua` **三行**、
`state.json:buyband_20260906`、**三处检测器修复**(§FD.7,其中一处**先于本轮就红在 trunk 上**)、
报告 `iterations/reports/strategy/20260906T021130Z.md`。
**armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结);零 AWS、零 S3、零 EC2、零波次。**

### §FD.1 立案句:两个常数不同意「什么叫残血」

| 函数 | 状态 | HP 带 |
|---|---|---|
| `J.ShouldStayAndRegen` | **PROMOTED**(`tphome`,每一局 Turbo 都活着) | `nHP < 0.18 or nHP > 0.75` |
| `J.IsFieldRegenSituation` | `stayfield`/`stayfield2`/`fieldbuy` 三家共用 | `nHP < 0.18 or nHP > 0.55` |

上面那行是这棵树**自己**对「伤得够重,回家已经摆上桌面」的定义;下面那行决定**野外方案提不提供**。
⇒ **0.55 与 0.75 之间**:残血、**安全**、**两手空空**的 bot 被 PROMOTED 的否决**放行回家**
(那条路上只剩 `GetGold() < 90`,过了开局几分钟拦不住谁),
而**唯一职责就是「改成买大药」的 id(`fieldbuy`)结构性地说不上话** —— 它的情境谓词认为这 bot 还没受伤。

### §FD.2 域价钱(先跑,再写第一行代码)

同一份语料(109 fixture / **1012 活英雄帧**),前置三数由本轮**独立前缀行走**复算,与 §EY/§FA/§FC **逐位对上**:
turbo **1012/1012**、走到补给子句 **125**、被否决 **112**。

**112 的三分法(断言为恒等式,不是三个独立计数)**:主格位有补给 **44**(`staysrc`)+
背包大药 **2**(`staybag`)+ **什么都没带 66**。
**那 66 帧不是读数缺陷 —— 是真的没东西可读**,owner P2 对它们的答案是供给侧:**买一个**。

| 66 帧里 | 帧 |
|---|---|
| ≤ 0.55(`fieldbuy` 自己的域;其中 **29** 帧它今天就已在说话) | 38 |
| **> 0.55** | **28** |
| ↳ **只被 0.55 上限挡住、别的什么都没挡** | **18** |
| ↳ 被 1600 环挡(真否决,本杠杆不碰) | 11 |
| ↳ 被 1200 内敌塔挡(真否决,本杠杆不碰) | 10 |

**本杠杆域 = 20 帧**,而 `20 = 18 + 1 + 1` **断言为恒等式**,两个余项**数出来而不是解释出来**:
1 帧在 `ShouldStayAndRegen` 的 reach 集外(它的追击行是**未归因**读法 —— `stayattr` 的立案帧形状),
1 帧背着背包大药(GH #123 不对称,原样继承自 `fieldbuy`,由购买点自己的 `FindItemSlot` 拒绝)。

### §FD.3 ⭐ 主判据(立法级,可复用,超出本主题)

**在一棵检测器按源码文本解析的树上,函数签名是一个已发布的接口:
动一个共享谓词的代价,由「解析它的声明」的那些文件来付,而它们对这次改动无话可说。**

到达这个行为有三条路,**两条走不通**:

| 路 | 为什么不走 |
|---|---|
| (a) 直接抬高 `J.IsFieldRegenSituation` 的 0.55 | **一条臂同时动三个家族** —— `lanefix` 的 bundle 形状,两次都测出强负 |
| (b) 从 `J.ShouldFieldBuyRegen` 内部传 gated 上限 | 那函数**第一行就是自己的 gate** ⇒ 新 id 只有 `fieldbuy` **也** armed 才动得了 —— **`pullcad` 第二形态**(§FC / GH #542) |
| (c) 给共享谓词加一个**可选上限参数** | 「看上去最干净」的那条。**写完了,然后撤销了** |

**(c) 的代价是量出来的不是猜的**(那两行重新打在副本上、跑完解析这个函数的 8 个文件、再还原):
`function J.IsFieldRegenSituation( bot )` 与 `nHP < 0.18 or nHP > 0.55` 被当作**字面文本**读,
于是 **8 个里 7 个同时变红**:`healthy_walk_home_gap` / `stayfield_hp_window_reach` /
`stayfield2_marginal_domain` / `fightback_world_assertion` / `itemtrip_wasteful_trip` /
`bagsalve_backpack_source` / `stayattr_global_ult`。**七个里没有一个对这条杠杆有话要说。**

**第 8 个保持绿色的才是信息量所在**:`stayfield_callsite_domain` 读的是**调用点**,
而带默认值的参数让调用点**逐字节不变** ⇒ 代价**精确地**落在解析**声明**的那些文件上。

⇒ 本轮形态:**standalone 函数,重复三条环境子句而不调用兄弟** —— 和 `J.HasNearbyHeroDamager`
当初从同一个扫描里拆出来**完全一样的理由**(改写 shipped 调用点去和 gated 代码共享,
会把一杠杆改动变成一次 shipped 重构)。重复的代价**付在看得见的地方**:
两份拷贝里**每一个常数**(1600/1200/3000/3.0/0.18)都解析出来两两比对,任一边漂移就红。

### §FD.4 单臂可见性(这条设计的全部理由)

| 量 | 读数 |
|---|---|
| `flips_buyband`(**只** arm 本 id) | **20** |
| `hurt_domain`(独立前缀行走) | **20** ✅ 两路一致 |
| `flips_fieldbuy` | 33 |
| `overlap_buy_hurt` | **0**(两域构造性不相交:≤0.55 对 >0.55) |
| `flip_true_to_false` / `arm_leak` / `raises` | **0 / 0 / 0** |
| `overlap_probe_runs` | **== live(1012)** |

⇒ **本 id 单臂波读得到**,与 §FC/GH #543 的 `bagsalve`(**按构造**单臂读不出东西)**恰好相反**。

**⭐ `overlap_probe_runs` 是本轮新加的一列,理由是 GH #171 的形状**:
`overlap_buy_hurt == 0` 是一个**全部内容就是一个零**的断言,而
「探针跑了 1012 帧什么也没找到」与「探针不跑了」**打印出来一模一样**。
探针现在**数自己跑了多少次**并断言等于活帧数;变异体 **M16** 就是这个。

### §FD.5 方向由构造固定

购买点是 `if ( J.ShouldFieldBuyRegen(bot) or J.ShouldFieldBuyRegenHurt(bot) )` ——
**OR 一条新臂进去只能增加 TRUE**;未 armed 时新谓词第一行返回 false(`or` 的单位元),
**shipped 购买顺序逐字节不变**,下面九条引擎子句两臂原样共用。

### §FD.6 ⭐⭐ 本节最该被读的:**`CAUGHT` 掩盖一发打偏的刀,和 `SURVIVED` 暴露它一样彻底**

**M8 第一次跑存活了。** 按证据纪律第 2 条的**逆命题**(先确认这一刀砍在了你以为的地方)查下去:

它的锚是那条光秃秃的 `if #bot:GetNearbyTowers( 1200, true ) > 0 then return false end`,
而这一行在 `jmz_func.lua` 里**出现两次** —— 兄弟 `J.IsFieldRegenSituation`(~5588)与本杠杆(~5852) ——
而 `perl -0pi -e "s///"` **不带 `/g` 改的是第一处**。⇒ **那一刀砍进了兄弟函数,本杠杆从头到尾没被碰过。**

**这次存活付清了自己两次:**

1. 买到一条**真正缺失的断言** `SIT_NIDS == 1`:打偏的那一刀往**共享谓词**里塞了第二条 gated 子句 ——
   正是「一条臂动三个家族」那个本设计全程在躲的危险 —— 而**本套件里没有任何东西在数那里的 id**。
2. 暴露出 **M10 一直是「因为错误的理由而 CAUGHT」**:同一个光秃秃的锚,于是它动的是**兄弟的**常数,
   漂移守卫看见 300 ≠ 1200 就红了 —— **颜色对,函数错**。
   而一行 `CAUGHT` 里**没有任何东西**说得出它砍中了谁。

⇒ **一台绿色的变异台并不证明任何一发打中了它自己声明的对象,只证明有东西把套件弄红了。
锚的唯一性不是正则的实现细节,它是这个声明的一部分。**
建议总监据此给变异台规程补一句:**锚必须唯一,且唯一性要与结论一起登记**。
两发已按「跟在本杠杆那份拷贝后面的、唯一的注释」重新下锚;打偏的那一刀**保留为 M8b**
(它是一个不同的缺陷,有不同的主人),第二次全跑 **20 发全部如声明**。

**声明式 UNMEASURABLE(M18)**:拿掉本杠杆拷贝来的归因否决里那个 `return false`。
20 帧域里**恰好 1 帧**带最近英雄伤害,而那一帧是**未归因**的(global ult 形状)⇒
本语料没有任何一帧能区分两个程序;漂移守卫也看不见(常数没动)。
哪天有「带内 + 被近处敌人打了」的 fixture 落地,**改判为 `caught`** —— 条目写着这句,而不是被删掉。

### §FD.7 顺手修掉三处检测器缺陷(附带,不是主体)

| # | 文件 | 缺陷 | 本轮引入的? |
|---|---|---|---|
| 1 | `test_bagsalve_backpack_source.lua` | `J.HasFieldRegenSource` 调用点数**钉死 `== 2`**,HEAD 上已是 **3** | **不是。先于本轮就红在 trunk 上** |
| 2 | `test_replay_260822_fieldbuy_supply.lua` | 「没有未 gated 调用点」读的是**生源码**,一句注释就弄红它 | 是 |
| 3 | `test_fieldcreep_veto.lua` | **同一条断言的孪生拷贝**,同一缺陷 | 是 |

**第 1 条是「计数不是性质」的教科书例**:那条断言的自述理由是「**新调用者可能没继承 turbo**」——
`J.HasFieldRegenSource` 自己**没有** `IsModeTurbo`,安全性全靠调用顺序。
`staysrc` 在 09-05 加了第三个调用点(行为是对的:`ShouldStayAndRegen` 第一行就问 turbo),
**没人更新这个数**,于是一条职责是「查 turbo 继承」的守卫,有一整天只会说「这个数变了」。
已改成**逐调用者断言那条性质本身**(两条合法路径:先问情境谓词,或自己先问 `IsModeTurbo`),
计数只留作**反真空下限**;两条可证伪性检查改写成 `nCalls ± 1` 而非字面量,
**下次合法新增调用者时不会连它们一起被静默重新基线**。

第 2/3 条同族:断言想说的是「没有未 gated 的**调用点**」,而一句提到谓词名字的**散文不是调用点**。
两处都改成**先 strip 注释**,并各补两条可证伪性检查(真调用仍触发;引用调用形式的注释仍不触发)。

### §FD.8 诚实边界(四条)

1. 翻转的是**购买谓词**,不是一趟被取消的回家:购买点那九条引擎子句 fixture 读不到,那趟路也读不到;
   `WIRE_*` 断言**接线存在**,**没有任何东西断言购买发生了**。
2. 金钱不进 `.dem`(GH #495)⇒ 实测集是**对金钱盲的超集**;方向固定,大小不声称。
3. 域是 **20 帧**,分解见 §FD.2。
4. **`fieldcreep` 那条否决故意不拷贝**(写下另一个候选的 id 会在它 promote 当天把这条子句冻结成 FALSE ——
   `pullcad` 第一形态)。代价:`fieldcreep` armed 期间两臂对「被野怪啃」看法不一致。
   **实测 `hurt_with_creep_damage == 0`** ⇒ 本语料上这个分歧是空的,那一列就是它哪天不空了会举手的地方。

### §FD.9 交棒

- **甲 → 总监**:`buyband` 入集提议,**P4.2 冻结期预期裁定 `FROZEN-HOLD`**(登记、不入集、不视为掉棒);
  另请看 §FD.7 第 1 条(**一条计数式守卫红在 trunk 上一整天没人认领**)与 §FD.6 的规程建议。
- **乙 → 批测台**:本 id **单臂可读**(`flips == 20`,与 `fieldbuy` 重叠 0),
  解冻后可与 `fieldbuy` 同波但**必须分臂**;两者域不相交,读数不会互相污染。
...
## §FG 2026-09-06T07:xxZ 总监 —— **结清 GH #454(连续四轮被点名):spot 单波成本常数重裁,而本节最该被读的不是新数字,是 §FG.1.3 —— 那条自动失效条款按「钱」写,而「钱」与它要抓的东西负相关,于是抢占越频繁常数越不会被复查**;外加 §FG.2 hero-35 搭车裁定

### §FG.1 GH #454:`$0.90` → `$1.10`,并把触发它的判据从「钱」改成「钱 OR 机时」

#### §FG.1.1 立案句(不是本轮的,是批测台的,已挂了四轮)

`$0.90` 那个常数自己带着一条自动失效条款(总监 2026-08-30T22:0xZ 立,`test_set.md` §CO.5):

> 任一波账单侧实测单波成本 > $0.90 ⇒ 本台在该轮报告里点名,**总监下一次触发必须重裁本常数**。
> **这个常数不许靠人记得去复查。**

**W46 触发了它**(账单侧实测 `$0.959–1.022`)。此后 **W47 / W48 / W49 / W50 四轮**,
批测台在每一份波次记录里逐字写着「**总监 still not re-ruled**」(W49 那份甚至自己数到
「named for the third consecutive round」)。⇒ **条款生效了,机制生效了,而被点名的那个人四轮没动。**
本节是那个动作。

⭐ 顺带记一句这条款自己的成绩单:它**没有**靠任何人记得去复查 —— 靠的是批测台每轮点名。
**「这个常数不许靠人记得去复查」这句话本轮被证明是对的**,只是证明的方式是它自己差点又被忘掉。

#### §FG.1.2 (甲) 新常数 `$1.10`,方法照抄 08-30 那次

账单侧实测点(全部,不挑):
`$0.747 / $0.795 / $0.799 / $0.79–0.86 (W29) / $0.824–0.892 (W30) / $0.794–0.859 (W31) /`
**`$0.959–1.022 (W46)`** ` / $0.7413 (W47) / $0.776–0.840 (W48) / $0.70–0.76 (W49)`

⇒ 上确界 **`$1.022`**(W46;现场是现货紧张:两个 AZ 完全没有容量,四个 AZ 里三个越过
`$0.231–0.250` 价带)⇒ 覆盖档 **`$1.10`**(余量 **+7.6%**,与上次 `$0.86 → $0.90` 的 +4.7% 同量级)。

⛔ **不取 `max(费率) × max(价) × 4`** —— 08-30 那次已经否过一次(`$0.948` 是两个从未同时
出现的极大值之积),本轮不重开。

⚠️ **批测台自 W47 起在波次记录里登记的 `~$1.02` 不是常数,是 W46 的实测上确界本身。**
它比 `$0.90` 好(朝安全侧),但**「登记历史最大值」这个做法在市场再紧一点的那一波上恰好不够**
—— 而那正是 `$0.90` 出事的方式。⇒ 常数要**覆盖档**,不是**最大值**。
对 `$50` 围栏的代价:每波比 `$1.02` 多记 `$0.08`。

#### §FG.1.3 ⭐⭐ (乙) 真正的缺陷:**按钱写的判据与它要抓的东西负相关**

这一条是批测台提的(#454「为什么这仍然要报」),总监**照收并认为它比常数本身重要**。

旧条款要的是「**实测单波成本** > $0.90」。而单波成本 = 总机时 × 价,
**一次容量抢占把一台机器在 14 分钟就掐掉,总机时下去,实测成本跟着下去,判据就不触发了。**

W42 就是这么过关的:活下来的三台是 `0.9367 / 0.9175 / 0.9414 h`(均值 **0.9319**,
对章程常数 `0.745` **高 25%**),而那一波实测只有 `$0.701–0.759` —— **线下**。
按活下来那三台的费率补齐四台:`4 × 0.9319 = 3.7274` 机时 ⇒ `$0.861–0.932`,**上端越线**。

⇒ **抢占越频繁,常数越不会被复查。而抢占频繁正是市场紧张、单台费率上涨的时候。**
**失效条件与失效原因负相关。** 这与 `pullcad`「promote 一个 id 会静默冻死引用它的 gate」
是同一族:**门在,但被结构性地绕开**,而且两边的失效方向都是安静的。

**新条款,两条腿,任一条成立即触发**(批测台在该轮报告里点名,总监下一次触发必须重裁):

| 腿 | 判据 | 抓什么 |
|---|---|---|
| **(乙1)** | 任一波**账单侧实测单波成本 > `$1.10`** | 钱真的超了 |
| **(乙2)** | 任一波**「跑完自终」的机器**(SIR `Status.Code = instance-terminated-by-user`)**的机时均值 > `1.00 h/台`** | 费率漂了,**哪怕抢占把总账压回线下** |

**被容量回收的台不进 (乙2) 的均值** —— 它们正是要被绕开的那部分,把它们算进去就等于把
漏洞重新装回来。判据取自 SIR 的 `Status.Code`,不是 EC2 的 `StateReason.Code`(GH #408)。

#### §FG.1.4 ⛔ (乙2) 的阈值**不取** #454 建议的 `0.80 h`,理由是算术

#454 建议把判据改成「每台机时 > 0.80 h」。**方向对,数字不对**,而不对的方式是致命的:

自 W18 起**每一个**「跑完自终」的实测点都在 0.80 之上 ——
W18 `0.89 / 0.90 / 0.909 / 0.920`,W42 `0.9175 / 0.9367 / 0.9414`。

⇒ 那条判据会**每一波都触发**。而**每一波都触发的判据与没有判据等价,只是更吵**
(同族:一个永远红的检测器,读者三轮之内就学会跳过它)。
`0.80` 是对着**旧的、错的**章程常数 `0.745` 写出来的 —— 它继承了那个常数的错。

⇒ 取覆盖全部实测点(上确界 **`0.9414`**)的档 **`1.00 h/台`**(余量 **+6.2%**)。

#### §FG.1.5 (丙) 顺带作废机时常数 `0.745 h/台` → `0.93 h/台`

这一条同时结清批测台 2026-08-28 交棒 ⑤(「总监 —— 章程费率更正请复核」,那根棒也挂了很久)。

`0.745` 是**含被容量回收台**的均值,而被回收的台跑了 14 分钟就没了 ⇒ 它系统性**低估**机时。
低估方向 = **「以为围栏还有余量」**,并且**与「MTD 对 EC2 有 4.3–11.3h 滞后」同向叠加** ——
两个都朝危险侧,两个都安静。取覆盖 7 个「跑完自终」实测点(`0.89–0.9414`)的档 **`0.93`**。

⚠️ **诚实边界,写在这里免得下一轮有人拿它去「复核」(甲)**:
`0.93 × 4 × $0.28/h = $1.04`,与 (甲) 的 `$1.10` **不是同一条推导**,
**也不该用来校对它** —— (甲) 走账单侧实测点的覆盖档,(丙) 走机时侧;
把两者相乘去对账,就是 §FG.1.2 刚否掉的那种「极大值之积」。**两条各自独立登记。**

#### §FG.1.6 落地位置(裁定要落到被裁方读的那一格,§2.5)

- **投递**:`iterations/streams/batch-desk.md` 步骤 2「单波成本基准」 + 「围栏算术一律用上面那张实测表」那一行 —— **批测台每轮读的就是这两处**;
- **档案**:本节;
- **机器键**:`iterations/state.json:wavecost_RERULED_20260906`;
- **线程**:GH #454 追评并关闭。

**零 AWS 增量。本裁定不发波,也不改任何工具的默认值** —— `wave_fence.py` 的 `--planned`
一直由批测台按本常数传入,所以这次重裁是**改章程里那个被传进去的数**,不是改工具。

### §FG.2 hero-35(`axebhrecast`)搭车裁定:**APPROVED-SCAN**

与 hero-32 / hero-33 / hero-34(总监 09-06T04:17Z 裁)**完全同型,同一条款**:
**零 EC2、只读档案遍历,不排波、不入集**,挂到 hero-2 / hero-30 / hero-31 已经在跑的那一次遍历上。

- **产物路径不改名**:`iterations/reports/replay-check/domain_scan_hero_2_30_31.md`
  是 `owed_executions.json` 那一腿唯一读得到的机器键,改名等于让那条腿看不见它;
- **acceptance 一字未改**;
- **METHOD-FAILED 按 §CJ 强制写进**:读不出请求里 (2) 那一列(重放瞬间射程内**还没中招**的
  敌方英雄数)⇒ **INSTRUMENT-BLIND 退回重裁**,**不得**自行套用「(2) 集中在 0 ⇒ 收益不成立、不入集」
  那条预登记 —— 那条预登记假设的是**量具工作而读数为 0**,不是**量具读不出来**。两者在报告里长得一样。

⚠️ **同一腿现在承载第七份读数,而它的 `done_when` 仍只判一个路径存在(LIMIT 11)** ——
与 04:17Z 那三条同一条边界,已写进该行的 `done_when_note`。

### §FG.3 交棒

- **甲 → 批测台**:下一波起围栏算术用 **spot `~$1.10`**(不再是 `$0.90`,也不再是自行登记的 `$1.02`);
  收割轮请把 **(乙2) 的读数**(「跑完自终」台的机时均值)与 (乙1) 一起登记 —— 两条腿都要打,**只打一条等于旧条款**。
- **乙 → 录像组**:hero-35 并进同一次遍历(现在是 **hero-2 / 30 / 31 / 32 / 33 / 34 / 35 七份读数,一个产物**)。
- **丙 → 下一轮总监**(⚠️ 本轮**没有**做,不是做完了):`owed_executions` 的
  `a_evidence_tpdying` 行已被工具标 **DONE 并点名「the director should retire this row」** ——
  产物 `iterations/reports/replay-check/a_evidence_tpdying.md` 已存在(194 行,录像组自判
  `VERIFY id=tpdying verdict=INDETERMINATE episodes=4527`)。退休这一行**需要总监先按 `done_when_note`
  逐条核那份产物**(两腿差分 / 哪一波哪些局 / 铁律 4(i-a) 的 ab-ba 两层读数 / 无 gpm-xpm)
  **再判 WORKING/BUGGY/SILENT**,不是盖个章。**它住在机器 registry 里,所以掉不了** —— 这正是 §DR 立那条的理由。
## §FH 2026-09-06T07:30Z 协同组 —— **一个 armed id 被核验为「执行正确、行为方向错」时,正确的下一步是在它旁边落一个收窄版的新 id,不是改它**;本节最该被读的是 **§FH.5:两次「我预测 UNMEASURABLE、结果 CAUGHT」——那是奉承套件的方向,因此是最不会被复查的方向,而其中一次是我的针根本没扎进去**
> ⚠️ **本节原本写作 §FG,rebase 时改成 §FH**:总监 07:xxZ 那一轮(结清 GH #454)
> 在本轮写作期间也认领了 §FG —— **两轮并行,谁都没抢谁**。冲突时**两节都保留**,
> 本节整体改名,引用它的报告 / 章程 / `state.json` 同步改。
> 这正是 `citation_audit.py` 的 `AMBIGUOUS` 类(两节抢同一个 `§XX` 标题);
> ⚠️ 但本轮**挡下它的是 rebase 冲突,不是检查器** —— 两节都是**追加到文件末尾**的,
> 所以 git 一定会撞;换成两节落在文件不同位置,git 会干干净净地合并出**两个 §FG**。

**认领**:`[strategy]` open issue **#558**(录像组 07:08Z 开,248 个 R_lever episode 帧证据 +
建议钉帧 + 建议验收方式,**未被任何组认领**)。按本组工作流第 1 步取,不是 backlog 自选;
同时满足 **OWNER_PRIORITIES P4.4(i)**(工作单元主体 = 一个 `bots/` 行为改动)。

**产物**:`bots/mode_retreat_generic.lua` X.ShouldRun「前期谨慎冲塔」块**新增**收窄门
`towerring`(turbo-only,**未 armed**,**单臂可读且支配 `towerfear`**)、
`tests/test_towerring_attack_circle.lua`(**18/18**)、
`tools/agent/mutstand_towerring.sh`(**15 发:12 CAUGHT + 对照 M13 SURVIVED + 2 声明式
UNMEASURABLE(M6b/M7b),零 NO-OP、零意外,STAND GREEN,EXIT=0**)、
`state.json:towerring_20260906`、本节、
`tests/_towerfear_sweep.lua` 与 `tests/test_towerfear_clock_leg.lua` **块窗口换机制**、
`test_gated_helper_nesting_census.lua` **两行**、
报告 `iterations/reports/strategy/20260906T073000Z.md`。
**armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结);零 AWS、零 S3、零 EC2、零波次。**

### §FH.1 立案句:条件 (a) 的两半指向同一个环的不同部分

GH #558 对 `towerfear`(把前期怕塔的时钟腿 300s 折半到 150s)逐 episode 帧核 248 个
R_lever episode / 65 个对局:

- **前半(真的执行且生效)买到了**:杠杆释放的矩形里 occ% **+2.45** / dwell **+1.41s** /
  bnc% **−23.88**,三个量**两层同号**;旁边只差等级的对照 `C_low` 三个量**全是 SIGN-SPLIT 噪声**。
- **后半(行为正确)符号是反的**:被释放的 episode 摸进塔 **700u 攻击圈**的比例
  **+12.46pp(ab)/ +35.21pp(ba)**,**两层同号**。帧加权版 70% 来自一个对局,
  按铁律 4(ii) **不作主读数**。
- 录像组明说:**不建议按现状 promote**。

**而这个块自己的注释早就写了为什么。** 700–898u 环带里塔够不着,校准兜底子句
(`GetAttackTarget() == bot`)**结构上无法开火、也不需要**(W8+W9 实测 72.7% 的释放帧在那个环带);
进了 700u 它是**唯一**的兜底,而它要求**炮弹已经在飞**。
⇒ 几何线在 **700u**,而且它是这个块自己的论证里已经有的那个数,不是本轮发明的。

### §FH.2 为什么是**新 id 并列**,不是改 `towerfear`(⭐ 主判据,可复用)

把几何条件塞进 `towerfear` 的门里,会让一个**正在被测量的 id 在 arm 串 md5 不变的情况下换掉语义** ——
W50 已经用那个 id 买到了 (a) 的前半,那半读数会静默变成关于另一个杠杆的。
⇒ 落**并列**的 `towerring`,`towerfear` 的 gate / 代码 / 读数全部保留。

代价是必须回答「两个同时 armed 时谁赢」。本轮把那个答案**做成语句顺序**
(restore 排在 halve **之后**)并**钉在测试里**(DOMINATION,七个承重帧全测),
理由是:**pair 才是 promote 会发的配置**。
M4(把 restore 挪到 halve 之前)就是这条的变异体 —— **每条语句都还在、单独看都对**,
死掉的只有 pair 的读数。

不选另一条路(把新 id 嵌进旧门里)的理由是 **GH #542 / GH #553**:两个 id 在同一条路径上,
哪个都读不出来。本轮的排列反过来:**`towerring` 单独 arm 即可读**(只在环带内折半)。

### §FH.3 域价钱:**先跑,而它给出的是一个零,如实登记**

`tests/_towerfear_sweep.lua`:109 fixture / **1012** 活英雄帧 / **32** 帧 898 环内有敌塔 /
**17** 帧粗子句被问到 / **3** 帧开火 / **仅 1 帧**由时钟腿单独持有 —— 就是 `towerfear` 的钉帧,
**在 727.46u,属于环带** ⇒ **在冻结的这份语料上,`towerring` 的「限制域」是 0 帧**。

**这是关于语料的事实,不是关于杠杆的**:这批 fixture 是为别的理由冻的、批测局在攻塔前自终止,
「2:30–5:00 窗口里站在塔的攻击圈内」在这里被严重欠采样;
GH #558 的 **248 episode** 才是总体读数(armed 腿 **18/55**(ab)、**41/90**(ba) 摸进去过)。

⇒ 因此每一条**限制**断言都在真实帧上**只移动 `DotaTime()`** ——
等级、英雄表、血量、**以及本杠杆读的那个塔距**全是帧自己的。
这个写法不是本轮发明的:`towerfear` 文件本身就用钉帧把时钟拨到 149 论证过。

七个承重帧跨 700 两侧:
**环带** zuus **727.46**(lvl 7,**t=278.5 是帧自己的时刻,零操纵**)/ skeleton_king **787.32** /
ember_spirit **845.08**;**攻击圈** spirit_breaker **174.28** / axe **285.08**
(**lvl 10 —— 校准子句 `botLevel <= 9` 够不到的那一档**)/ lion **328.01** /
obsidian_destroyer **649.41**(内侧边界证人)。级别腿对照 lina lvl 4 @860.01 四个世界全等。

### §FH.4 ⚠️ 一条不空的诚实边界,和两个**长得像对照的非证人**

**700 这个字面量在行为上只被括在 (649.41, 727.46) 之间**,宽 **78u**。
落在这个区间里的改动**帧看不见**,只有源码断言看得见 —— 这正是 **M6/M6b、M7/M7b 成对**要量的东西。

语料里离边界最近的那一帧(juggernaut **701.99u**)**不可用**:ShouldRun 更靠前的一个分支
在 5 个不同时钟上都出价 **0.75**,四个世界全等 ⇒ 它见证不了任何东西。luna **245.20u** 同理。
**两帧都写成断言而不是悄悄丢掉** —— **一个长得像对照、却在每个世界给同一个答案的帧,
正是最容易被当成证据的形状**;写成断言之后,哪天它开始动了,它就自动变成本文件说自己没有的那个紧边界证人。

### §FH.5 ⭐⭐ 变异台规程:**「预测 UNMEASURABLE、结果 CAUGHT」是奉承套件的方向**

**M7**(`<` → `<=`)首跑判 UNMEASURABLE,理由是「语料里没有一帧恰好在 700.00u」——
行为上确实如此,但**源码断言那一半看得见**(`<%s*700` 不匹配 `<= 700`),于是它 **CAUGHT**。
台子一行就说了 `** re-classify **`。

改判之后补 **M7b**:同一处改动**连源码断言一起改**(扮演那个「调了个数、顺手把测试改对」的人),
**这才是真正在问行为半边看不看得见**。
...
**M6/M6b 是同一个成对形状** —— 那对才是「域价钱」的诚实写法:
单独的 M6(700→900)只证明帧对**远处**的改动有牙,单独的 M6b 只会被读成一个洞;
**成对才说得出「牙齿到哪里为止」**。

M4 的一行也值得单记:它的 perl 需要携带两个多行需求量穿过 bash 引号,首跑打印的是
`ENV: unbound variable` —— **不是一个变异体结果**。已改成 python 字面块交换,带
`count(a+b) == 1` 前置断言。

### §FH.6 三个变红的检测器,**三个都对这次改动有话说**

按 §FF.3 立的主判据(**判据不是「改动让几个检测器变红」,是「每个变红的文件对这次改动有没有话说」**)逐条回答:

1. `tests/_towerfear_sweep.lua` 与 2. `tests/test_towerfear_clock_leg.lua` 的**块窗口**是从
   `前期谨慎冲塔` 起算的**字节长度**(3400 → 4800),本轮的论证注释又把它撑爆了。
   **一个失败模式是「有人解释了代码」的窗口不是在量代码。**
   两处都改成**尾锚**(`nEnemyTowers[1]:GetAttackTarget() == bot`,**正是旧 assert 在找的那个东西**)
   + 400 字节尾巴。**这是第三次因散文而 widen,所以这次不是 widen,是换机制。**
3. `tests/test_gated_helper_nesting_census.lua` 两行 gate-inside-a-gate 的 outer-id 列
   `towerfear` → `towerfear,towerring`。分类**仍是 W**,理由逐字写在行上:
   新 id 门的是**同一个块里的同一个时钟局部**,未 armed 时 `nFearClock` 停在出厂 `5*60` ——
   **它加入的那个合取的单位元**,所以单独 arm 任何一个 outer id 测到的东西和落地前一样。

**出厂行为逐字节不变**的证据不是论证是读数:`_towerfear_sweep.lua` 改动前后的普查输出
**byte-identical**(17 ROW + 全部 `C` 计数逐位相同)。

### §FH.7 交棒(三条,都已在 GH #558 下追评)
...
- **丙(录像组)**:GH #558 建议的钉帧 `…_eff51c/20260906_035941_slot6 --t 283.4 --hero sniper`
  ...
  那一帧落成 `tests/fixtures/` 后,§FH.3 的「限制域 = 0 帧」就变成非零。
  **这是本轮唯一一条没有在本容器里付清的账。**

---

## §FI 2026-09-06T09:xxZ 总监 —— **裁定批测台的自裁(它推翻了自己的绿工具,而它是对的),并把那次「靠手算才对」变成工具自己会说的话**;外加 §FI.1 hero-36 搭车裁定与 §FI.2 一条搭车腿的 `done_when` 从「文件在不在」升到「八份读数各自在不在」

本节三件事,而最该被读的是 §FI.0 的形状:**同一种缺陷本轮在两个互不相干的工具上各出现一次,
两次都是「一个派生量被写成了字面量,而字面量在安静的日子里与派生值逐字相同」**
(`wave_fence.py` 的 `pending=0.0`;`owed_executions.json` 那一腿的 `path_exists`)。
两次的失效方向也一样:**朝着「放行」失效**。这不是巧合,是这个仓库的常见病
(`pullcad` / GH #504 / #205 / #213 同族),所以本节把它当一类写,不是当两件事写。

### §FI.0 ⭐⭐ 立案句(批测台 09:12Z 交棒 ②):工具打 CLEAR,而正确答案是 THROTTLED

批测台本轮跑 `wave_fence.py`,读到 `WAVE_FENCE: CLEAR`,**然后没有发波**。
它的理由不是字面条款(字面只覆盖「本台自己近 12h 的波」),是条款的**理由**:
账户里有一台**别人项目**的 `c7a.16xlarge`(`Project=final-table-trainer`)整天在烧,
而 `dota2bot-batch` **没有 cost filter** ⇒ 那台机器的钱会落进我们的预算、吃我们的围栏余量;
`ActualSpend` 又滞后 4.3–11.3h ⇒ MTD 此刻读不到它。批测台手算外推、推翻自己的绿工具、不发波。

**它是对的。而它必须靠手算才对,这件事本身就是缺陷。**

看这个缺陷的形状,它与 GH #504(本文件 §…,`wave_fence.py` 的立案)是**同一个**:

- `--pending` 的默认值是 `0.0`,于是没传它的那一轮打出
  `pending waves    : $0.000   (launched, may not be in MTD yet)` ——
  **这一行读起来像一次测量,实际是一个没人检查过的默认值**;
- 它**朝着多花钱失效**(围栏凭空多出余量),不像朝着少花钱失效的门那样会自己把 bug 报出来;
- 在**没有任何东西在跑**的日子里,默认值与正确值**逐字相同** ⇒ 无 diff、无缺读数、无报错;
- 它**只在有东西在跑的时候咬人**,而那正是它承重的时候。

⇒ **裁定:确认批测台的自裁(不发波是对的),并把它靠手算得到的那一步搬进工具。**

### §FI.1 ⭐ 裁定:`wave_fence.py` 新增 RULING 4 —— `--pending` 是一个**声明**,工具现在会检查它

**明确不做的事(这一条比做了什么更重要)**:**不引入成本模型**。
本工具不知道任意一台机器值多少钱 —— spot 价、on-demand 价、lifecycle 各要一次读,
而在这里猜一个 markup 常数**就是新的自由参数**,正是本文件要消灭的那个病。

能免费确立的是「$0.000 为真」的**前提**:

> `pending == 0` 可核验,当且仅当**没有东西在跑**。

于是新增账户级 `ec2 describe-instances`(免费、**不按 tag 过滤** —— 预算也没过滤):

| 现场 | 结论 |
|---|---|
| 没东西在跑 + 没传 `--pending` | `pending = $0.000`,标 **CERTIFIED**(是读数,不是默认值) |
| 有东西在跑 + 没传 `--pending` | **exit 2**,并**逐台打印**在跑的是什么(操作者才好定价) |
| 传了 `--pending`(含 `0.0`) | 用操作者的数,标 **ASSERTED**,并把它必须覆盖的那几台列在旁边 |
| `--no-accrual-check` | 打一行**「SKIPPED, NOT CERTIFIED」**(GH #213 的 `RULE6_BYPASS` 句式),**要抄进当轮报告** |
| 枚举本身失败 | **exit 2**,不是「没东西在跑」 |

**两条边界写死在 docstring 里,不留给人推断**:

1. 枚举是**账户级、故意的**。今天 `dota2bot-batch` 没有 cost filter ⇒ 账户里任何一台机器
   都吃我们的余量,不管归谁。**若 owner 哪天加上 `Project` 过滤器**
   (`DECISIONS_NEEDED.md` 第 15 条,W36 邮件里唯一真要他动手的一条),
   这条检查就变成**过保守** —— 会因为别人的算力卡住我们。
   **那是「会自己报 bug」的失效方向**(有人被卡住就会喊),而现在这个方向不会;
   工具因此每轮打一行 `budget filters :`,**让预算不再是账户级的那一天是看得见的**。
2. 这里的 exit 2 与本文件其他地方一样是**没跑成**,不是通过。

**批测台从下一轮起的用法**:照常 `--planned`;账户里有东西在跑时工具会 exit 2 并列出来,
自己估一个数传 `--pending`(或 `--no-accrual-check` 并抄那一行进报告)。
**这会让「什么都没跑」的安静轮次继续 exit 0,而 09-06 那种轮次不再打 CLEAR。**

**验收(本轮已付清)**:`tests/test_wave_fence.py` **42 checks / 0 failed**(原 24 → 42);
变异台 `tools/agent/mutstand_wave_fence.sh` 加 M7–M10 后 **9 CAUGHT + 1 控制 SURVIVED,STAND: OK**。
**M7 是「缺陷原样」那一发**:有机器在跑、没传 `--pending`、工具照样交回 `$0.000`——
所有打印行都仍然像模像样,门照常放行,只在有东西在跑的日子里是错的。
M8 是**会有人真去做的那次修改**(把枚举收窄到自己的 `Project` tag —— 听起来对,
在预算没过滤之前是错的,09-06 那台正好就是别人的 tag)。

⚠️ **本轮没有跑活的 AWS 路径**(总监不花 AWS 的钱,且本容器未 bootstrap 凭据):
新增的两个 AWS 读(`describe-instances`、预算的 `CostFilters`)**在本容器里没有被执行过**,
纯核心逻辑与 CLI 接线由上面的 42 checks + 变异台覆盖。**这是覆盖,不是等价** —— 首次实跑
由批测台下一轮完成;若 `describe-instances` 权限不足,它会打 `accrual read : FAILED` 然后
**exit 2 而不是静默放行**,这正是设计的失效方向。

### §FI.2 ⭐ hero-36(`cmrangedhp`)搭车裁定 + 那条腿的 `done_when` 升级

**hero-36 裁 APPROVED-SCAN**,与 hero-32/33/34(04:17Z)、hero-35(07:xxZ)完全同型:
零 EC2、只读档案遍历,不排波、不入集,搭 hero-2/30/31 已在跑的那一次遍历,**产物路径不改名**。
acceptance 一字未改;METHOD-FAILED 按 §CJ 强制写进(读不出「下单瞬间该 ranged lane creep 的
真实血量」那一列 ⇒ INSTRUMENT-BLIND 退回重裁,**不得**自行套用「分布落在 `(460, nCreepCap]`
中段 ⇒ 不入集」)。请求正文里 `DotaTime() > 10*60` 那道分层要求照做。

**但这是第八份读数骑同一条腿,而那条腿的 `done_when` 只判一个路径存不存在。**
前两轮我把这件事写进 `done_when_note` 两次(LIMIT 11),两次都只是**记下来**。
第三次再记一遍不会让它变好:**七份里少六份,这条腿照样绿**,唯一挡在「绿」与「退休」
之间的东西是**总监记得去逐份读** —— 那与 §FI.0 是同一个形状,只是这次的字面量叫
`path_exists`,安静的日子里(产物还没写出来时)它与正确答案也逐字相同。

⇒ **`tools/agent/pending_rulings.py` 新增 `done_when` kind `path_contains_all`**:
产物必须**提到** `contains` 里每一个 id,少一个就报 OWED **并点名少的是哪一个**。
`hero_domain_scan_2_30_31` 改用它,八个 id 全登记。

**故意留下的边界(写进 LIMIT 11,不留给人推断)**:
「**被提到**」不是「**读数正确**」——被提到的 id 可能被并了池、可能答的是错的那一列。
那一层仍然是**总监退休本行前逐份读通**;新 kind 只把「有没有这一份」从**记性**搬进**工具**。
故意用**子串**而不是解析:产物是别的组写的散文,本工具不该规定它的排版。
另一个刻意的后果:**改产物路径或改 id 写法会让这条腿变红** —— 改名本来就该惊动人。

**验收**:`tests/test_pending_rulings.py` **239 checks / 0 failed**(原 225 → 239)。
...
(没人走到的分支证明不了任何事)。
## §FJ 2026-09-06T11:05Z 协同组 —— **「先跑域价钱」有权把杠杆否掉,而它这次就否掉了章程点名的下一格**;落地的是**同一次普查里量出来的最大拒绝桶**:同一个家族对同一个问题ship 了两个半径(PROMOTED 的 1200 与继承来的 1600),`buyring` 只答那 12 帧的差

> ⚠️ **本节原本写作 §FI,rebase 时改成 §FJ**:总监同轮(09:xxZ,裁定批测台自裁)也认领了 §FI。
> **这正是 GH #559 立案的那个形状** —— 挡住它的是 git 不是检查器,而 git 只在两节都追加到
> 文件末尾时才会撞;本轮两节都在末尾,所以确实撞上了,而且是**在 rebase 时**才撞上的。
> **ID**:`buyring`(NEW,turbo-only,STANDALONE,**未 armed**)
> **归属**:OWNER_PRIORITIES **P4.4(i)**(工作单元主体 = 一个 `bots/` 行为改动)+ **P2 供给侧**
> ⛔ **P4.2 入集冻结生效中**:本节是**登记**,不是入集提议的放行。armed 串一字未动,`queue.json` 一字未动。

### §FJ.1 立案句:两个半径,一个问题,都在 shipped 里

「敌人近到这个残血 bot 不能算安全吗」在 P2 的两半上被问,**在同一条 HP 带 `[0.18,0.75]` 上**被问:

| 半边 | 谓词 | 环 | 状态 |
|---|---|---|---|
| 决策(hold) | `J.ShouldStayAndRegen` | **1200** | **PROMOTED —— 每一局 turbo 都在跑** |
| 供给(buy) | `J.IsFieldRegenSituation` + `fieldbuy`/`buyband`/`buytower` | **1600** | shipped / gated |
...
这与 §FF(`buytower`)是**同一个形状**(为另一个消费者写的理由),只是这次落在**环**上而不是塔子句上。

后果是一个以本家族自己的 promoted 数字为地板的洞:一个 **1200 环空**(这棵树今天已经称之为「没被硬追」)
的残血 bot,只要 1200–1600 之间站着一个敌人,就拿不到大药、falls through、回家。
**1200–1600 里没有任何东西与 bot 接触**:1200 上没有英雄能普攻,
而全游戏最长的攻击距离(带射程天赋 + 龙心之刃的远程核心)离 1200 仍差几百单位。那是隔着一波兵。

### §FJ.2 ⭐ 主判据(立法级):域价钱**审的是杠杆本身**,不是给已选定的杠杆算大小

章程 `0TOWERRING`「下一格」点名的是**归因子句**:
「被已归因英雄伤害挡住的 28 帧里,有多少是全局技能/远程一发的形状」。
按章程**先跑价钱**,读数:

- 28 帧里 **25 帧同时也在 1600 环里** ⇒ 环本来就挡,归因不是那 25 帧的约束;
- 再去掉 **2 帧**有敌塔 ⇒ **只剩 1 帧**;
- 那 1 帧的已归因伤害者站在 **1,615.81u** —— **离 1600 环只有 15.81u**。

⇒ **候选域 = 1 帧,且它的前提(「全局技能/远程一发」)在那唯一一帧上不成立。** 不做。

**顺带推翻了章程写下的前提**:供给侧三条臂**早就继承了 `stayattr` 的修复** —— 三条都带已归因扫描,
语料里 3 帧「有英雄伤害但归因不到 3000 内」的帧**今天就已经被放行**。

**这一条的价值不在于省了一次改动**:在于**「章程点名的下一格」不是判决**。
它是上一轮在**没有价钱**的情况下写下的猜测,而域价钱是唯一能审它的东西。
⇒ 建议(交总监):章程「下一格」一律标注为**未定价的猜测**。

换上的杠杆来自**同一次普查**:163 帧两手空空里 **99** 被 1600 环挡(**供给侧最大的单一拒绝桶**),
其中 **12** 帧 1200 环是空的。

### §FJ.3 域价钱(109 fixture / 1012 活英雄帧,turbo 1012/1012)
...
band [0.18,0.75]                305
  └ 主格位什么都没带            163
      ...
      ├ 1200 内有敌塔            22
      └ 什么都没挡               53   ← 两个兄弟今天在管
两个环的差 = 99 − 87 =            12   ← 写成减法断言,不是第四个可漂移的数
      ├ 也被归因挡                0   ← 登记成一条要守的性质(§FJ.5)
...
**就是 OWNER_PRIORITIES P2 点名的那份 063559 回放**:slardar **968/2104 = 46.0%**,
身上 echo_sabre / power_treads / quelling_blade / magic_wand / bracer(**没有任何能喝的**),
最近敌方英雄 tidehunter **1,565.36u** 且**没碰过他**,1200 环空,1200 内无敌塔,最近 3 秒无英雄伤害也无小兵伤害。
shipped 三条买臂**全部拒绝**。

### §FJ.4 单臂可读 + 构造性不相交

三个兄弟臂都要求 1600 环**空**,本臂要求**非空** ⇒ 四域在**整条 HP 带**上构造性不相交,
这正是本杠杆敢横跨 `[0.18,0.75]` 而不是切 0.55 一侧的原因。

`overlap_ring_buy` / `overlap_ring_hurt` / `overlap_ring_tower` 各 **0**;
`overlap_probe_runs == live == 1012`(GH #171 防假零);
`flip_true_to_false` / `arm_leak` / `raises` 全 **0**;带子分布恒等式 **6 + 4 = 10**,两侧都非零。

### §FJ.5 反真空四条

1. `overlap_probe_runs == live` —— 沿用 §FE/§FF;
2. **拒绝集也被驱动**:sweep 逐帧打出被拒的 **153** 帧(87 chase / 64 empty_ring / **0 attr** / 2 tower),
   测试全部重放并断言仍被拒,且断言 `153 == 163 − 10` 是补集;
3. **两个环的差写成减法断言** `nosrc_ring_busy − nosrc_chase_busy == nosrc_annulus`;
4. `nosrc_annulus_attr == 0` **被当成一条要守的性质登记**,而不是留给读者默认 ——
   它一旦不为 0,§FJ.2 里「环在做全部拒绝」那句话就过期了,而**测试会先说**。

### §FJ.6 ⭐⭐ 变异台规程:唯一性不是一次性属性,是每次新增都要重数的

本杠杆的带子**第一版**写作两条语句
`if nHP < 0.18 then return false end` / `if nHP > 0.75 then return false end` ——
...
⇒ 改成 `if nHP > 0.75 or nHP < 0.18 then return false end`(**高位在前的一条语句**),全文件出现 **1** 次。
**上一轮为唯一性选的写法,在下一轮就是下一个人要避开的重复。**

**同一次重数还抓到一处已经坏了的锚**:`mutstand_buytower.sh` 的 M2 needle 结尾是三臂 OR 的**右括号**
`\t\tor J.ShouldFieldBuyRegenTower(bot) )`,第四条臂一进来它就**出现 0 次**;已改成对**臂本身**上锚。
发现它的是本轮写的**只读跨台锚普查**(把每个 `anchor N "$VAR" "needle"` 拿去数一遍)。

**而本台自己的 anchor 检查抓到三处我自己的错误声明**(台子因此跑了三遍)—— 那是它挣饭钱的地方:

- **M9 / M10**:用**光秃秃的环行**上锚,那两行在 `jmz_func.lua` 里分别出现 **2 次**与 **5 次**,
  `perl` 无 `/g` ⇒ 改第一处;第一处**恰好**就是想改的那个函数,于是**两发都打印 CAUGHT** ——
  **颜色对,而它对不对纯属位置的巧合**。已改成**两行 needle**(用邻居把函数点名),各 1 次。
- **M16(三处里最值得记的,因为它长得完全不像重复)**:那一行在 sweep 里只写过两次 ——
  主 stub 一次,disjointness 探针里**把 stub 还原回去**一次,而后者**缩进更深**。
  **一个缩进 24 空格的 needle 是同一行缩进 28 空格时的子串** ⇒ 字面子串计数得 2,
  **而肉眼看不出任何重复**。needle 已改成带上它自己缩进的 `end`。
  ⇒ **「数锚」防的不是手滑,是「同一段文本在另一个缩进层级上再次存在」这种看不见的重复。**

⚠️ **一个操作教训,当场付的**:我在台子**还在跑的时候**编辑了它。`bash` 按**字节偏移**边读边执行,
在当前偏移**之前**插入字节会让后续读取错位 ⇒ **首跑作废、整台重跑**
(SIGTERM 让 EXIT trap 跑完:`INTERRUPTED with bots/FunLib/jmz_func.lua mutated -- restoring before exit`,
`git diff` 复核 `bots/` 只剩本轮的 158 行新增)。**改一个正在跑的 stand 脚本 = 作废这次运行**,
与「改一个正在被测量的 id」(§FH 主判据)同族。

### §FJ.7 ⭐⭐⭐ 工具级(当场付的学费):mock 把 `print` 换成空函数

本轮**第一个**域价钱探针回来时是「**stdout 全空 + 退出码 0**」——
与「跑了、什么都没发现」**逐字节相同**。原因是 `tests/mock` 的 `install()` 把 `print` 换成了空函数
(**GH #546 已立案**);改用 `io.stdout:write` 后同一份代码打出 1012 行。
**记在这里的理由是形状不是工具**:它与 GH #171 的假零同族,而它出现在**每一个新写的一次性 stand 的第一次运行**上。

### §FJ.8 附带变红的三个文件,逐条回答(§FF.3 那把尺)

1. `test_gated_helper_nesting_census` —— 三条新 gate-inside-a-gate 行,**逐条手判 A/I/W**;
2. `test_bagsalve_backpack_source` —— `HasFieldRegenSource` 调用者 5→6,
   新调用者进「**自带 IsModeTurbo**」那一列(**计数只是反真空地板**,性质在那个循环里);
3. `test_buytower_purchase_domain` 的 `WIRE_OR3` —— **第二次因为同一个理由被重读**:
   模式结尾是三臂 OR 的右括号,把**臂数**钉死了;按 §FF 给 `buyband` 的同一处方改成
   **容忍再加臂、仍对替换与 `and` 变红**。

⇒ 三个都对这次改动有话说,三个都是**重读性质**修掉的,**没有一个是抬数字**。

### §FJ.9 诚实边界(两条不空的)

- ⚠️ **10 帧域里 1 帧背包带着大药**:`J.HasFieldRegenSource` 看不到背包(停在 slot 5),
  而购买点的 `bot:FindItemSlot('item_flask') < 0` 看得到(GH #123)⇒ **那一帧谓词翻转但不会真的买**。
  写成精确等值断言 `ring_with_bag_salve == 1` —— 即「翻转集」与「购买集」在本语料上差 1 帧。
- ⚠️ **与 `fieldcreep` 的分歧 1 帧**(`ring_with_creep_damage == 1`):同样没抄那条 gated 的 creep 否决
  (抄了就是在本函数里点另一个候选的名 ⇒ 它 promote 当天这条子句冻死在 FALSE,`pullcad` 第一形态)。
- 1,300u 外的敌人可以在三分之一秒后走进 1200 环:本臂买的是**一瓶大药**,不是「喝下去一定成功」的承诺,
  而喝不喝的决策不在这条路径上。**朝向/速度 fixture 读不到 —— 这是语料侧的缺口**,
  也是本杠杆唯一买不到反例的地方(交录像组)。

### §FJ.10 交棒(三条)

- **甲(总监)**:`buyring` 按 P4.2 请裁 **FROZEN-HOLD**(登记、不入集、不算掉棒)。
  另两条规程建议:(i) 「锚唯一性要在**新增代码时**重数」写进变异台规程,本轮的跨台只读普查是可复制的实现,
  **而它当场抓到一处已坏的锚**;(ii) **章程「下一格」标注为未定价的猜测** —— 本轮它是错的,只有域价钱审得了它。
- **乙(批测台)**:解冻后本 id **单臂可读**(`flips == 10`),与 `fieldbuy` / `buyband` / `buytower` 重叠**各为 0**,
  可同波但**必须分臂**;⚠️ 与 `fieldcreep` 同波要登记 **1 帧**分歧,另有 **1 帧**属于「翻转但不买」(背包大药)。
