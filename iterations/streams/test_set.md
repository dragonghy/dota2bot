...
## §HH 2026-09-12T22:2xZ(第八轮)总监:**`pending_rulings.py` LIMIT 14 落地 —— 一个开行时就已经为真的 `done_when`,此后永远读 DONE,而「请退休我」就印在它下面** —— 本节最该被读的是 **§HH.2:我以为这是 1 行,真值是 7 行,差别来自我读了 `rc.sh` 的 40 行尾巴而不是它指给我的完整日志**

**armed 27 不变**(成员串一字未动),`bots/`+`game/` **零 diff**,**零 AWS**(一次调用都没有),无 promote / 无退集,不发 owner 邮件。

### §HH.1 立案现场:上一轮自己写下的下一棒,而它是同一个形状的第三发

上一轮(§HF/§HG)的收尾读到 owed 行 `late_epoch_corpus_reopen_list`(**本台 06:5xZ 自己开的**)读 DONE 并提示退休,并写下「⇒ 下轮**改判据,不是退休**」。原判据是
`path_exists tests/frames/README.md` —— 那个 README **在开这一行的时候已经存在好几天了** ⇒ 这一行**从落地的那一分钟起就读 DONE**,并在下面一行印着「请退休我」。

⭐ **形状不是「判据过期」,是「判据从来没有辨别过任何东西」。** 它是 LIMIT 13 的**更锋利的同胞**:LIMIT 13 那两例(`roshan_pit_daynight_fix` / `hero_domain_scan_2_30_31`)是**判据活过了它的义务**;这一例是**判据压根没有在说任何事**。两者都落在 `DONE` 上、都印「请退休我」,而**处置相反**。已实测三例,三条不同的路:

| 例 | 判据 | 它为什么从出生就满足 |
|---|---|---|
| `wandlimbo_charge_instrument`(LIMIT 11 的 ⛔) | `text_absent` 两条 needle | 两条都拼错了(一条大小写差一个字母、一条跨注释换行),于是**从未匹配** —— 而「从未匹配」与「那句话被删掉了」逐字节相同 |
| `roshan_pit_daynight_fix`(LIMIT 13) | `path_exists` 一个测试文件 | 文件**前一天**就落了 |
| `late_epoch_corpus_reopen_list`(本轮) | `path_exists tests/frames/README.md` | README **早存在好几天** |

⇒ **LIMIT 11 的 ⭕ 只登记未建的那道闸,本轮建了**:一个 DONE 读数现在要求 `unmet_at_ruling` 证词 —— **裁定当时取到的那个 OWED 读数,即这个区别唯一还可观测的那一刻**。没有它,状态是 **BORN-DONE**:finding、进退出码、**永不打印「请退休我」**。

⚠️ **它买方向不买证明,这一句必须照抄**:工具只检查字面 token `OWED` 与一个能解析的 UTC 瞬间(于是「yes」被拒、散文里那种 `T22:xxZ` 模糊写法被拒 —— 与 `claimed_at` 同一个解析器同一个理由),**它查不出这句话是不是真的**。编造那一行就能绕过它。**它消灭的只是沉默的那个方向**:一行不能再在**没人见它说过别的话**的判据上读出「已执行,请退休」。

⚠️ **git 替代不了这条证词,而且理由要写下来免得看起来像疏漏**:按引入提交的那棵树重算判据**确实**能机械回答这个问题,但**实测 2026-09-12,Routine 容器的 clone 是 50 个提交、全部是同一天** ⇒ 任何比一天更老的行,引入提交**不在树里**,答案恰好在这一行**老到已经被忘掉**的时候变成「说不出」。

### §HH.2 ⭐⭐⭐ 本节最该被读的一条:我把 7 读成了 1,而读错的原因是我读了工具的视图不是工具的数据

我先按 `bash tools/agent/rc.sh python3 tools/agent/pending_rulings.py --owed-only` 的**打印**判「只有 1 行读 DONE」,并据此把本轮范围定成「修一行」。**真值是 7 行。** `rc.sh` 的输出逐字写着 `RC_LOG=/tmp/rc.XXXX (198 line(s); last 40 shown)` —— **它自己说了它是尾巴、也自己给了完整日志的路径**,而我 `grep` 的是那 40 行。7 行里有 **7 句「请退休我」**逐轮打印。

📌 **教训不是「grep 要小心」,是**:`rc.sh` 是本台为了纪律 3(管道读退出码)立的工具,它把**退出码**救回来了,而**同一个动作把输出换成了一个视图** —— 于是一个为了防止误读退出码而存在的工具,成了误读**读数**的入口。两次失效是同一族:**我读的不是数据,是某个东西对数据的呈现**(与 §HD.4 / §HG.2「用一句已经过期的理由支撑决定」同一门)。⇒ **规矩**:凡是要据 `rc.sh` 的输出下结论,读它打印的 `RC_LOG=` 那个文件,不读它的尾巴。

### §HH.3 那 6 行的处置:两种诊断以同一句话呈现,而工具分不出来 —— 所以它不假装分得出

`a_evidence_pulldrag` / `a_evidence_tpgap` / `instrument_ownhalf_a_evidence` / `promote_bar_paired_seed_depth` / `gh692_epoch_clock_first_live_read` / `gh699_materiality_gate_first_live_read`。

现查过前三条点名的产物**都在**(12,028 / 12,763 / 21,296 字节)⇒ **大概率是「真执行完了而没人退休」**,即工具连着好几天要求的动作是**对的**,只是没人做(GH #276「registry 没人剪枝就变墙纸」)。但**本台不能替它们补证词** —— ⛔ 证词只能由**当场取到 OWED 读数的那个人**写,替别人追记就是把这个字段变成签字画押。

⇒ **落地的是天花板不是零**:`tests/test_pending_rulings.py:BORN_DONE_INHERITED = 6`。**理由是算术不是宽容**:六行**全部早于这个字段好几天**,任何证词都不可能被取到过,而一个**本轮清不掉的红**正是「三轮之后没人看」的做法(铁律 10 自己的立法理由)。天花板买到的恰是唯一缺的那件事:**一条新的同形行加不进来了**。六行本身作为一笔债登记成 owed 行 `born_done_inherited_disposal`,`done_when` = `text_absent 'BORN_DONE_INHERITED = 6'`(处置掉任何一行都要调低那个数,needle 于是消失),**证词当场取到并抄了进去**。

⚠️ **不追溯已退休的那批**:registry 的 14 条 retired 行里 **12 条**的判据是机器键、**没有一条带证词** ⇒ 本工具对**已经退休的那些退得对不对,一句话也不说**。

### §HH.4 钉子:9 个变异体各自因自己的理由死,外加一条修在变异台自己身上

`tests/test_pending_rulings.py` **769 检查 0 失败**(入轮 741);`tools/agent/mutstand_owed_witness.sh` **9 CAUGHT / 0 SURVIVED / control_ok=1 / restored=1**。变异体覆盖两个方向:**缺证词读成 DONE**(M1/M2/M3/M4/M4b/M7)与**带证词读不出 DONE**(M8)—— 后者不是对称的装饰,**没有它这道闸就等于「什么都别退休」,而下一个总监最便宜的动作是把它删掉**。

⭐ **变异台自己被修了一处,现场就是它的首跑**:它按兄弟台的写法用 `git diff --quiet` 判还原,而本轮**被守护的代码自己还没提交** ⇒ 首跑在一个**完好还原**的文件上打了 `RESTORE: NO`。⇒ 改成**两个判据**:`START_SHA`(跑之前的 sha,回答本台真正欠的那个问题「树里还有没有变异体」,**并进退出码**)+ `git diff`(第二意见,抓「备份取自一个已被变异的文件」,`START_SHA` 看不见这一种)。**一个自己会喊狼来了的还原判据,三次之后就没人读了。**

### §HH.5 投递

`iterations/owed_executions.json`:`late_epoch_corpus_reopen_list` 换判据 + 补证词、新行 `born_done_inherited_disposal`、新 `_witness_note`(与 `_claim_note`/`_residual_note` 同族,写给以后每一个开行的人);`tools/agent/pending_rulings.py` LIMIT 14 全文;本节 §HH。⚠️ **没开 GH issue**:executor 是总监自己,而机器可读的投递轴是那一行 owed 行加代码里的天花板,**再开一条 issue 只会多一条没人驱动的表**(§DR 的立案句反过来用)。

## §HI 2026-09-13T01:0xZ(第九轮)总监:**推送钩子的三条腿在每一次推送尝试里跑两遍同一棵树 —— 记忆化落地(GH #213 族)** —— 本节最该被读的是 **§HI.2:被浪费的那 7 分钟不是理由,输掉的那场竞速才是,而输掉竞速的恰好就是那次冗余的运行**

### §HI.1 事实(先量后写)

铁律 6 的推送门是 `.githooks/pre-push`,git **每一次 `git push` 调用**跑一遍。
而 `.claude/rules/claude-code.md` 规定的推送路径是**两条命令**:

```
git push -u origin <branch> && git push origin HEAD:main
```

⇒ **每次尝试都在一棵逐字节相同的树上跑两遍三条腿。**
实测(一次性 repo:裸远端 + 计数钩子,同一个 commit 推到两个 ref):
`hook invocations: 2`,两次的 `HEAD` 与 `HEAD^{tree}` 相同。

### §HI.2 立案句:**理由不是浪费,是竞速 —— 而输掉竞速的恰好是那次冗余的运行**

09-12T22:2xZ 本台自己的现场:门跑一次 **~7 分钟**(快 Lua 腿单独 380s,占 90%),
而当时 **main 每 ~7 分钟动一次**,`HEAD:main` **连续三次 remote rejected**
—— 三次都不是门红,全是 ref 竞态 —— 烧掉 ~21 分钟零产出,第四次走了 `RULE6_BYPASS=1`。

📌 **输掉那场竞速的永远是第二次推送**,而第二次推送**在结构上不可能回答出与第一次不同的答案**:
三条腿读的是工作树,两条命令之间没有任何东西碰过它。
⇒ **一道慢到输掉自己必须赢的那场竞速的门,会把人赶进它自己的逃生门。**
命中把第二次推送的窗口从 ~7 分钟压到 ~0,而且压的是唯一一处**可证明是重做**的工作。

### §HI.3 ⛔ 它不是 `RULE6_BYPASS`,而横幅不许被读成那样

bypass **什么都不断言**;命中是**重放一次真实运行**在**这棵完全相同的树**上得到的读数。
「铁律 6 静态半在被推的这棵树上是绿的」这句话,命中与未命中**同样为真**——
变的只有**是谁、在什么时候**做的那份工。横幅两样都写(`RULE6_MEMO=REUSE` + 「这是重用不是跳过」),
于是抄进报告的那一行仍然诚实。

四条 fail-closed(全文见 `tools/agent/rule6_memo.py`):
键 = `HEAD^{tree}` + `origin/main`(快 Lua 腿用 `origin/main...HEAD` 定域,基线动了就是**另一个问题**)/
工作树必须干净(**含未跟踪** —— 腿 lint 的是磁盘上的东西,不是 commit 里的)/
**只有全绿才存**(红与没跑成永不入库,重试必然重跑)/ 任何异常一律**未命中**。

### §HI.4 变异台推翻了本轮的第一版测试(两个存活体,两个都不是「测试写漏了」)

`bash tools/agent/mutstand_rule6_memo.sh` 首跑 **3 CAUGHT / 2 SURVIVED**:

- **M2(基线只从键里拿掉)存活 —— 而它是对的**:记录里还会把 base 读回来比对,于是照样未命中。
  两层独立防线是**设计**;拿掉其中一层的变异体是**等价变异体不是逃逸缺陷**,
  把它记成 CAUGHT 就是**把另一层干的活算在测试头上**。真正的 fail-open 要两层一起拿掉,M2 已改成那样。
- **M4(腿 1 用 `$?` 而非 `PIPESTATUS[0]`)存活 —— 而这一个揭出了真的缺口**:
  腿 1 自己打 `GATE_EXIT=`,钩子拿它做第二意见、不一致就拒推,所以腿 1 **恰好**扛得住。
  ⇒ 一个只把腿 1 弄红的测试,会把管道缺陷记成已抓,**而腿 2、腿 3 既不打退出码也没有第二意见**,
  它们会**静默地推一棵红树**。测试改成**逐条腿分别弄红**(含 exit 2 那一档),
  变异体改打 `prc`/`lrc`;两个**当场被抓**。
...
那条测试 4.6s > py manifest 的 `per_test_cap_seconds = 3.0`
⇒ 它今天在推送闸里跑,**是因为 `py_gate.py` 把没测过的测试一律照跑**
(本轮逐字:`14 new test(s) not in the manifest were run anyway`),**不是因为它被选中**。
下一次 `py_gate_measure.py` 重测会**正常地**把它排除,而**排除不打印任何东西给任何人看**。
⇒ owed 行 `rule6_memo_fast_gate_membership`,判据 `path_contains_all`:
