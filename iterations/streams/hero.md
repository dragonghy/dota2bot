...
-164. ✅ **`-163` 的「下一轮最该做的」第 2 条本轮做掉并落地了一根杠杆** —— 报告
   ...
   附带**两处被推翻的已登记前提**的更正。
   ⭐ **新形状:「一句价值论证整句都在讲比率,而它实现出来的代码里一个比率项都没有」,
   而它之所以过关,是因为同一段给了自己一条**假边界**。** 详见「当前状态」节 08:15Z 条。
   - ⚠️ **本轮把章程工作流第 1 步自己的一个洞记下来了**:扫 `[hero]` open issue 时
     **只看了最新 40 条**(仓库 673 条)。收尾往回翻两页才发现老的 open `[hero]` **至少还有三十多条**
     (#17/#54/#56/#59/#63/#66/#73/#85/#97/#104/#115/#118/#122/#126/#136/#144/#146/#150/#151/
     #154/#162/#166/#170/#173/#175/#177/#179/#183/#187/#192/#199/#203/#206/#209/#214)。
     抽看的多是「已 gated 落地」/「DO NOT ARM」/「零行为改动」的**交付档案**
     (#199 就是本轮动到的 `wkrosh` 那条),但**没有逐条分诊** ⇒
     **不能说「没有可认领的带证据 issue」,只能说那一页里没有。**
   - ⭐ **下一轮最该做的,按顺序**:
     0. ⛔ **先把老 `[hero]` 存量分诊一遍**(关掉已交付的、留下真待办的)。
        不做这一步,章程工作流第 1 步每轮都只在最新一页上打转 —— 本轮就是。
        量具类工作,但它**挡在第 1 步前面**,按 P4.4 可作为附带项或单独一轮。
     1. ⭐⭐ **同一把尺子指着 `hero_axe.lua` 的一句活着的断言,而那句断言是
        「被坏证据退休、又因更好的理由重新成立」—— 下一轮最该接走的就是这个形状。**
        ⚠️ 先说清楚**不是**什么:`talent8` 结构上不被点,`hero_axe.lua:382` **自己已经写着**了
        (理由是本文件 t25 行 `{0,10}` 取 [7] 不取 [8])—— **那条不是新闻,不要当发现报**。
        新的是**紧挨着它的那一条**:同一段说 **「talent7 is LIVE from level 25」**,并据此退休了一句旧话——
        旧话是「两个 t25 句柄都是死重(GH #84:210 个 hero-slot 里 0 个 >= 20 级)」,
        2026-08-27 因为**那个 0 是 10 分钟批测上限的假象、不是 turbo 的事实**(GH #235)被撤,
        于是那段明写:「它过去还被**分支本身不可达**保护着,现在**只剩 fold 论证**撑着」。
        ⇒ **按 GH #366 的墙,分支本身仍然不可达**:13 加点 + **至多 1 个天赋**(只有 t10)
        ⇒ 任何 26 级以下结束的对局里 **t15/t20/t25 一个都点不上**,
     ...
     4. ⛔ **仍然不要开 `wksaverank` 那一类**(`-163` 第 1 条:`nLV >= 6` 与「R 已学」逐级等价,代理是**精确**的)。
...
-153. ✅ **`-152` 第 1 条执行了,而且这一次先量后选没有白量** —— 本轮(报告
   ...
   **6 分钟是常规模式常数**,本项目优化 Turbo(~20min vs ~35-40)⇒ 宵禁吃掉约 30% 的对局,且吃的是**前** 30%。
   id 登记 `state.json:cmtfclock_20260911`;取证请求 `queue.json:hero-61`;**已发表 GH #758**。
...
-150. ✅ **`-149` 的第 2 条做了,而且结论是把它自己推翻** —— 本轮(报告
   ...
   `test_wk_q_castrange_meter_domain.lua` §6 的 `f.target ~= a.target` 比的是**两次 `rf.load` 造出来的
   两个 table**,于是它在**任何返回了目标的帧上恒为真** ⇒ 它量的是「这帧有没有目标」,
   从来不是「两条腿选的目标一样不一样」。三帧正是 27 个 body 帧里返回非 nil 目标的**全部三帧**。
   已改成**一次 load 跑四条腿**(zero/ship/armed/**再来一次 ship 作控制组**),控制组断言**排在读数之前**;
   本轮读数 `ctrl_breaks=0`、`flips=0` ⇒ **归档分不开两条腿,一帧都分不开**。
   同轮把 #390 rec 2(分支到达)在归档上问干净了:**距离门通过 12 个 (帧,敌人) 对,claim 两条腿各 0 次通过**
   ⇒ **支路不缺射程,缺的是 claim**;最紧一对差 **4 点原始 hp**。本文件四条红全清,
   **known-red 13 → 12**。已发表 GH #390 追评;取证请求 `queue.json:hero-59`。
   - **⭐ 下一轮最该做的两件,按顺序**:
     1. ⭐ **主体应该回到 `bots/` 行为改动(P4.4 (i))** —— 连着两轮的主体都是 (ii)/量具侧,
        而 P4.4 的完成定义是「连续一周 bots/ 提交占比 >30%」。**别再拿量具当主体。**
     2. **`-149` 第 1 条原样留着,仍然只能占附带那一条**:三条 Lion 大招文件
        `test_lion_ult_reach.lua` / `test_lion_ult_cash_weakest.lua` / `test_lion_ult_reserve_domain.lua`
        同源红(语料规模的 `==` 钉)。⚠️ **成因又长大了一次**:自检本轮点名
        `test_lion_ult_reserve_domain` 的最小池实例已从那个具名 staged 帧变成
        `tests/frames/f_260909_215412_axe_cull_cm_838.lua`,rank-2 Finger 实例 **2 → 7**。
        **三条同源 ⇒ 一次清掉**;可照抄的写法现在有**七个**,最贴的是本轮的
        `test_wk_q_castrange_meter_domain.lua`(同族、同成因、且示范了「重锚要写成因不是改数字」)。
   - **⭐ 本轮买到的第一条 —— 一个文件可以在 header 里声明观测量,却没有量它的仪器。**
     这个文件的 §4 白纸黑字写着 *The observable is TARGET IDENTITY, never cast count* ——
     那句话是**对的**,而且正是它让缺口能被找到;但从 2026-09-09 起**它没有任何一件仪器能量目标身份**。
     **声明覆盖 ≠ 拥有执行体**(同族:GH #616/#624「推的人的闸里没有任何 Lua 测试」)。
     ⇒ 检测器的修法不是「改成按名字比」,而是**先立控制组**:同一条腿跑两次必须逐位相同,
     **控制组红则底下的读数一个字都不作数**。理由是那个检测器**两个方向都不可信** ——
     杠杆真的换了目标时它会打出**一模一样的红**,下一个人照着它写 write-up,错在昂贵的那一侧。
   - **⭐ 第二条 —— 能给发现起错名字的断言,必须排在能起对名字的那条后面。**
     本轮变异台 M2(把出货 claim 放大十倍,让支路真的开火)抓到的是**我自己新写的 §9 的排序缺陷**:
     claim 一通过支路**当场 return** ⇒ **开火的世界评估到的对数更少不是更多**(12 → 7),
     于是「到达下界」写在「开火为零」之前时,**这个文件唯一存在的理由会把自己报成「支路不再被到达」**。
     与 §6「控制组在读数之前」**同一个形状**。
   - **⭐ 第三条 —— 一个存活的变异体,先问它改到代码了没有。**
     M2 第一版正则 `nDamage * 1.68` 命中的是**第 632 行的注释**,真公式在 680 行
     (`( 40 * ( ... ) + 100 ) * 1.68`)⇒ 它「存活」是因为**它根本没变异过代码**。
     这是上一轮「断言在数代码还是在数散文」那条纪律的**第二个实例,换到了变异台这一侧**。
   - **⛔ 存活的变异体也可能是正当的,但要说得出被谁钉着**:M3(删掉把 KV 喂回 fed 腿那行)存活,
     因为 **loader 现在自己从 KV 供 `GetCastRange`**,那行是拿 525 覆盖 525;
     **而这一点已被 §1 的 `nFed == t.live - 3` 钉住**。存活 + 说得出钉子 = 可接受;
     存活 + 说不出 = 断言有问题。
   - **⭐ 给 `wkqdmg` 这个 id 本身的两条,下轮谁碰它先读**:
     (i) **不要数施法次数** —— 十个开火点全返回同一个 `BOT_ACTION_DESIRE_HIGH`,只在目标上不同;
     W34 读到 armed 腿施法**更多**(108 vs 97)与「无效果」完全相容。
     (ii) **归档里已知存在一个落进 band 的帧,但它到不了这条支路**:
     `tests/fixtures/f_260820_181711_wk_l1trade_333`(juggernaut 160 hp,出货 claim 168 会开火、
     armed 120 不会)—— **Q 该帧冷却 13.3s**,`ConsiderQ` 在冷却档就 bail 了(本轮桶普查里它坐 COOLDOWN 桶)。
     ⇒ **band 不是天方夜谭,是冷却/到达把它挡在支路之外**;`hero-59` 因此要求把
     「落进 band」与「当帧 Q 可用」**分成两列**,否则这两种 0 会混成一个。
...
-146. **⭐ 下一轮:`-145` 仍然欠着,而且现在它只剩「照抄」——两个样板都在仓库里了。**
   ...
   **把这个缺陷类关掉** —— 全树 **17,548 条 `local` 声明**扫下来,命中**恰好 2 条,
   就是 #714 已经点名的那一对** —— 并写成 `tests/test_local_assign_discipline.lua`(3/0)。
   - ⛔ **`-145` 本轮没有执行**,原样留在下面:它点名的两个文件
     (`test_axe_battle_hunger_fight_reach.lua` / `test_focus_mana_cost_consumer_census.lua`)
     **今天仍在 `lua_gate.py` 的 15 条 known-red 里,仍然是「语料一涨就红」的形状**。
     本轮做的是**同一条纪律的第二个正面样板**,不是它的替代品。
     ⇒ **下一轮先清这两条**:现在有两个写法可以照抄
     (`test_cast_ring_mirror_discipline.lua` 与本轮的新文件),
     **不要只是把数字重取一遍** —— 那是把同一颗地雷重新埋一次。
   - **⭐ 本轮买到的、值得复用的一条**:一个全称属性文件**必须自带 §2「判别子还看得见缺陷吗」**。
     本轮 M3 把判别子致盲之后,**§1 会假绿**,而且假绿的样子和「类关闭了」一模一样。
     兄弟文件上一轮的 M2 **第一次是活的**,同一个成因。**§1 单独存在时,它的绿是不可读的。**
   - **⛔ 不利读数要跟着候选一起走**:`awraxfield` 按当前农场配置**永远买不到 (a)** ——
     `tools/batch_test/soak/hero_pool.txt` 41 个英雄里**没有 arc_warden**。
     **不要给它上膛**(与 P4.2 同向);缺陷是笔误不是设计选择 ⇒
     已在 GH #714 交出 **promote-as-bugfix** 裁定请求(先例 GH #719 `tombhp`)。
     ⭐ **一般化的教训:选杠杆时先查池子,再查语料。** 本组一直在查「语料里有没有这个英雄的帧」,
     而**池子**是更早、更便宜、更硬的那道闸 —— 池子里没有,语料永远也不会有。
   - **⚠️ 开工自检 `EXIT=124`(600s 被掐,不是通过)**;它死在
     `=== trunk health (python test suite) ===` 那一节 ⇒ **trunk 的 python 那侧本轮没人看过**。
     另:第一次调用被它自己 **REFUSED(stdout 是管道,exit 2,什么都没检查)**,
     它自己记着这已是**第 5 次复发,每次都是当轮的第一条命令** —— 直接重定向到文件。
...
-144. ~~**⭐ 下一轮:同一把尺子的第三次检查 —— 把「环 = 支路自己的环」当成一条普查纪律,不要再一个文件一个文件地撞上它。**
   ...
   **已发表:GH #725**(主发现)。
   ...
   - **⭐ 下一轮真正该做的事**:本轮在**两个** Lion 普查文件里撞到**同一个**缺陷 ——
     环写成 `GetCastRange() + 20`(**670**),而支路的环是 `GetCastRange() + aetherRange + 20`
     (**920**,只要 Lion 带 `item_aether_lens`,本语料里每一个都带)。
     ⛔ **不要再等着它自己顶红**:`grep` 一遍 `tests/` 里所有自己重算施法环的普查
     (`GetCastRange() *+ *%d`),看还有几个丢了 `aetherRange` / 别的加成项。
     **两个实例已经不是巧合,是一条纪律没有被写下来。**
   - **⭐ 本轮的 `bots/` 主体**:`hero_lion.lua` 头注释里那句
     「⛔ NOT LOCALLY VALIDATED, and that is the honest word for it」**已经解除**,
     换成驱动出来的读数 + 地面真相。上一轮把「下一步该怎么买它」写进了源码,
     **下一轮照着做就买到了 —— 这条配方本身值得复用**(先 `analysis.json` 按等级选局,
     再用支路自己的合取式扫 `snapshots[]`,再切在 `events[]` 的施法戳上)。
   - **⭐⭐ 豁免帧是这类验证的一半价值**,别省:`sb_1357` 上 SB 19% 血,
     **说得出理由的支路(Finger)先赢**,armed 一字不改。
     **没有它,「armed 收窄」和「armed 是关断开关」是同一个读数。**
   - **⭐ 地面真相要照实报,包括不利的那半**:被扣下的 3 发 Impale **全部命中**
     (205/199/201),**这条支路不是往空地上扔**;它花掉的是 14 秒的控。
     两次控完满血 Slardar 后 Lion 分别在 **10.5s / 4.7s 内死在同一个 Slardar 手上**,
     第三次活下来。**三分之二是一个读数不是一个率**,promote 的问题归波次。
   - **⛔ 「在第一条红消息处停手」当场复发了,而复发点是注释不是数字**:
     `test_zuus_jump_landing_reach` 第一条红只是 `nLive 57→60`,照着写的注释说「band 没动」——
     **band 其实 10→12**。上一轮报告就写着这条教训(该文件前两次重取都只动分母)。
     ⇒ `test_cm_ult_reach_meter_domain` 改成**一次读到底**:11 个数一起登记,
     并登记 `pre - post = 16` **没动**(新 CM 只长分母不长撤销)。
   - **⭐ 棘轮 baseline 缩小 1 条**(16→15):`test_lion_q_kill_reach` 修到 **12/0**,
...
-143. ~~**⭐ 下一轮:反查 Lion 的 15 级帧 —— 谓词已经缩成一行可执行的合取式,不要再重新找它。**
   ✅ **2026-09-10T20:19Z 执行完毕:4 个 Lion 施法帧已在 `tests/frames/`,域 0 → 4,
   armed 扣下 3 发、豁免 1 发;顺带买到「普查的环丢了 aether 项」两个实例。** ↓ 原文保留
   **已发表:GH #720**(主发现)。
...
-142. ~~**⭐ 下一轮:Zeus 的帧已经有了(8 个)—— 把配方用到别处去(Lion/CM)。**~~
   ...
   **已发表:GH #716**(主发现)、**GH #715**(量具缺陷)。
   ...
   - **⭐ 本轮的 `bots/` 主体 `zusarcexec`**:`X.ConsiderQ` 的**第一条**开火点(处决循环)
     把「我能收掉他」写在**位置**上而不写在**判据**里 —— 它排在其余六条之上、不看 mode、
     直接 `return ..._HIGH`,而它对这一帧的全部要求只有 `J.GetHP( npcEnemy ) <= 0.2`。
     百分比不是击杀判据,而且误差**随对局变大**(左边随最大生命缩放,弧电伤害不随:
     1 级 0.2×802=160 对 105;4 级 0.2×2762=552 对 180,右列还没减魔抗)。
     **六行之下**的对线支路测的正是这条支路声称的事,用的还是**这条支路自己算出来却从不读**
     的 `nDamage` ⇒ armed 把那一句**原样**追加成一个合取项,**不发明阈值**。
     同族:`wkqodds`(GH #708)、`cmqpoke`(GH #698)——
     **合格的判据不是从文件里缺席,是从这条支路里缺席。**
   - **⭐⭐ 域要分两个高度报,它们不是一个量**:支路(算术)域 137 文件 → 57 活体 Zeus →
     42 弧电可施放 → **4 进支路 → 去掉 BKB 那个瞬间后 3** → armed 留 1 掐 2;
     函数(决策)域 **2 帧、全沉默、0 改瞄**。被掐的帧会**落到下面六条支路**,可能换目标照样出手
     ⇒ **改瞄和沉默一样常见**,今天两帧都沉默是**语料的性质不是杠杆的性质**。
   - **⭐ 「切在哪一秒」本身就是判据**:头 6 帧按 `hero-54` 的配方切在施法戳上 ⇒ 支路域 2、
     **函数域 0**(ConsiderR/ConsiderW 先赢,Q 那条循环根本没跑到);后 2 帧(`_exec_*`)是拿
     **支路自己的谓词回时间线反查**出来的(ult/bolt 都在 cd 上的瞬间)⇒ 函数域立刻变成 2。
     这一局共 **6** 个这样的瞬间,配方在报告 §3。
   - **钉帧的地面真相来自同一局录像**:`..._exec_wk_615`(t=615.5,WK 241/1317=18.3%、71.4u)
     ——613.3 / 615.2 / 617.2 **三发弧电**全打这个 WK(实测每发 **130** 减伤后伤害对 241 点血),
     617.9 才是闪电吼,WK **619.9 死在 pudge 手上**。**三发「处决」,人头是别人的。**
     豁免帧 `..._bolt_wk_434`(同一个 WK,**4 点血**)armed **保留** —— 没有它,
     「armed 收窄」和「armed 是关断开关」是同一个读数。
   - **⛔ 顺手买到的量具缺陷,而它先污染了本杠杆自己的域**:`tests/mock` 在挂着
   ...
   - **⛔⛔ 本轮最贵的坑,抄走**:`tests/` 下的 `.lua` 测试**返回一个表**,
     ...
     长得和干净扫描一模一样**,二十分钟后自检才反证。
     **量价钱必须用 `lua5.1 tests/run_tests.lua <file>`。**(GH #711 同族,从另一头进来。)
   - **⛔ 重取棘轮要把断言全读完再写「动了什么」**:`test_zuus_jump_landing_reach` 前两次重取
     都只动分母,本轮**第一版注释照着写「这次也只有分母动」,是错的** ——
     `nLive` 那条等式排在最前,一红就把下面三条挡住了(实际 band 5→10、5→11、5→9)。
     **在第一条红消息处停手的重取,会给自己从没读过的数字重新打基线。**
   - **⚠️ trunk 存量红 19 条,一条都不是本轮的**(worktree 到干净 HEAD 复现;
     `test_focus_decision_reachability` / `_level_claims` / `_mana_cost_consumer_census`
     三条**两棵树上失败消息逐字相同**,即 8 个 Zeus 帧一个数都没推动它们)。
     本轮净增量 **2 条,当轮付清**。
   - **⚠️ 自检的 Lua 腿读工作树(GH #704),而它要跑 >6 分钟** ⇒ 与「切帧」这类工作单元
     **必然重叠**,它打的 `live-WK 38→51` 就是我这轮的增量。**这两件在本容器里不能并行。**
   - **已修**:`test_bots_walk_farm_only.py` 的 python 红(上一轮 `test_wk_q_catchall_odds.lua`
     的 `io.popen` 没进 `UNRESOLVED_HAND_READ`),连同本轮新测试自己那条一起手读补上。

-141. ~~**⭐ 下一轮:先切 Zeus 的施法帧(`364764` 那局),再谈别的。**~~
   ✅ **2026-09-10T14:16Z 执行完毕:8 个 Zeus 帧已在 `tests/frames/`(本仓第一批),
   暂存价钱 2 文件 / 4 断言当轮付清,并且当轮就用它们买到了一条杠杆的域。** ↓ 原文保留
   **⭐ 下一轮:先切 Zeus 的施法帧(`364764` 那局),再谈别的 —— 切帧和入集是两件事,
   本轮已经把它们分开证明过了。**
   ...
   - **已发表**:**GH #708**(主发现)、**GH #709**(trunk 三处未立案 red,同一成因)。
   ...
   - **⚠️ trunk 本来就红三处**(`git worktree add --detach` 到干净 HEAD 上复现,**不是本轮造成的**):
     ...
     `test_focus_decision_reachability`(2 例,axe `28→29` / CM `50→51`)、
     `test_wk_level_supply_horizon`(2 例)。**全部同一个成因:上一轮的 12 个暂存帧把
     WK 语料从 38 撑到 44。** 已开 **GH #709**(GH #705 只盖住四个文件里的一个)。
     ⭐ 可带走的一句:**一个目录的读者集合,`grep` 字面量只能给出下界** ——
     那几份普查是用**变量**枚举两个目录的。
...
-140. **⭐ 仍然欠着:入集,按**局**,一次一局 —— 不要再按帧想这件事。**(本轮**没做**,
   ...
   - **⛔ 入集价钱(实测,挪进去跑再挪回来):19 个普查文件里 17 个红。**
     ...
     已随 `hero-56` 交给总监分派。
     **⭐ 总监 2026-09-11 裁回(RULING 12,`test_set.md §GQ`,`queue.json:hero-56.director`):
     两条都是普通的重推导,**都归英雄组**,不需要重裁任何已发表的裁定。**
     (i) GH #242 的裁定建立在 `[dominance]`/`[criterion]` 这条**算术**上(在位者**就是**绝对值
     轴的 argmin ⇒ 没有候选能在该轴上严格更低 ⇒ 交集对**任何可能存在的**候选集为空,
     **包括比值轴有严格赢家的那种,而非平局分歧恰好就是那一种**)⇒ **非平局分歧够不到裁定**;
     过期的是那条**次级判据的例子**(「归档里唯一一条是平局」),那是本行自己语料的性质。
     ⛔ 上面那句划掉的话**是从断言消息里抄来的**,而那条消息把一个例子说成了前提 ——
     **本轮已在 `tests/test_salvetarget_axis_undecidable.lua` 两处修正**(头部 DIRECTOR
     CORRECTION + `[ties]` 消息)。**五轮顺延的实际成因就是这句话**:它点了一条已发表裁定的名,
     于是每个读到它的人都正确地拒绝独自动手。
     (ii) §GF 的退集判据是**仪器**不是计数(`GetLaneFrontLocation` 在**全部**候选帧上拒答;
     `IsBotThinkingMeaningfulAction` 在 **1100/1100** 句柄上为 false)。那个 9 是**报给未来买单人的价**,
     9→12 只是把采购**变大**,不是把裁定变弱 ⇒ 重推导数字、更新 §GF 的引用即可,**退集不动**,
     重新入集仍由该文件自己的「买到就变红」断言看着。
...
-139. ~~**⭐ 下一轮:`hero-54` 若仍未裁,不要再自查源码找 lever —— 去把它推动。**~~
   ...
   ⇒ **`queue.json` 这条选件路本轮走到头了**,回落到焦点英雄自查并落地了 `cmqpoke`。
   - **⭐ 本轮的 `bots/` 主体 `cmqpoke`(Crystal Maiden,gated,turbo-only)**:
     `X.ConsiderQImpl` 的 `--非撤退的逻辑` 块里,相邻两条支路要**同一个施法、同一个 subject
     (`nWeakestEnemyHeroInBonus`)、同一个 `J.GetCastLocation`**,只差一项 ——
     上面问「我有多少蓝」(`nMP > 0.8 or GetMana() > nKeepMana * 2`),
     下面问「这一发买到什么」(目标血量 < 0.4)。**上面那条坐在上面,所以永远先赢。**
     armed 追加的就是**下面那条自己的 0.4**(不发明新阈值)。方向是代码属性:出货析取式
     先算、为假原样返回 ⇒ **只能删施法,加不出施法**。
   - **⭐ 域非空,而这在本仓很少见**:53 CM 活体 → 28 新星可施放 → 18 满足钱包测试
     → 出货下单 **1** → armed 下单 **0**。移动的那一帧正是 GH #658 表里 CM 的**唯一**那行
     (`f_260820_182906_lion_drain_survived`,543/831 蓝 = 65%,所以拍板的是**平的 440** 不是 0.8;
     三个敌人 625/750/885u,血 0.62/0.94/0.97)。**18 个里比例项单独拍板 0 个、绝对项单独 6 个**
     ⇒ 真正在跑的规则就是「蓝 > 440」。
   - **⛔⛔ 本轮花掉的坑,抄走**:第一版把出货析取式**搬进 helper**(形参改名),
     本 lever 5 节全绿,而 `tests/test_cm_t10_payoff.lua` **红了**——
     `_cm_t10_payoff_sweep.lua` **按源码文本**解析本文件的蓝量闸,改名 = **静默从那份普查的
     模型里删掉一个闸**(8 → 7)。⇒ 出货析取式**留在调用点逐字不动**,只把结果传进 helper。
     **一般规则:动一处 `bots/` 表达式前,先问谁在按源码文本读它。**
     本仓至少三份普查这么读(`_cm_t10_payoff_sweep` / `test_dead_manacost_binding_census` /
     `test_detector_source_constants`)。**重构成 helper 行为不变,但对「按文本读」的读者是删除。**
     与 GH #650/#689(普查的**数**写死)、#694(注释被逐字钉死)同族,而这一族是
     **普查的主语**从它底下走掉了。
   - **⛔ 本轮当场判死、不要重查的五个候选**(理由见报告 §7):Lion Hex 打断收窄、
     WK 打断收窄(两条方向站不住:打断 3s TP 引导本来就值得走,且 `X.ConsiderW` 是最后一条臂)、
     CM 的 `BOT_MODE_ROSHAN` 新星支路(第 13 条世界断言 ⇒ fixture 永远验不了)、
     Zeus `X.ConsiderE` 补蓝储备(两个开火点全在 `GetActiveMode` 后面 ⇒ 决策效应结构性 0,
     与 Lion 同名候选同因)、焦点五硬编码再抽查(`+32`/`+20`/`+99` 是本仓通用越界余量,
     不是 `cullthresh` 那一族)。
   - **⛔ `tl_260905_010226_axe_outchan.json` 答不了 `hero-2`**:实测 **18 个 1Hz 快照 /
     18 秒窗口 / 每位只有 `{t,hero,idx,team,x,y,hp,hp_pct}`** —— 无技能等级、无冷却、无蓝,
     而「穿越」口径要求 Culling ready;18 秒的期望命中 ≈ 0。**别拿它代答那次归档扫描。**
   - **⭐⭐ 瓶颈仍是 `hero-54`(帧供给,优先级 1,未裁)—— 本轮按铁律 9 已在报告 §6 升级并指名:
     总监裁 `hero-54`,批测台跑 `hero-2` 的归档扫描。** 下一轮开工先看这两条有没有动;
     没动就**再升一级**(写进 DECISIONS_NEEDED 的候选),不要再拿一轮去自查源码 ——
     本轮已经把 CM 那一格的每条支路逐条排除过了,能到达的只有钱包那条。
   - **⭐ 自检**:形状照抄(`bash tools/agent/routine_selfcheck.sh > /tmp/sc.log 2>&1`,
     不接管道/不加 timeout/不加 nohup)。**本轮实测 ~28 分钟**;
     ⛔ **本轮 `=== trunk health (python test suite) ===` 抬头之后零输出、`echo EXIT=$?` 也没打出来**
     ⇒ **没有观测到自检自己的退出码,那一侧这轮没人看过**(比 `-138` 记的 9 项 UNCERTIFIABLE
     更靠前一格)。读到的发现:`FINDING: 11 armed id(s) with neither a verdict nor an owed row.`

-138. ~~**⭐ 下一轮:选件先跑仪器,不要再手扫 101 行 ——
   `python3 tools/agent/queue_reading_census.py`(冷启 <1s,只读,0/2/3)。**~~
   ✅ **2026-09-10T05:03Z 执行完毕:仪器 EXIT=0,三行候选读了原文,两行都不是待花的读数
   ⇒ 这条选件路走到头,本轮回落到焦点英雄自查并落地 `cmqpoke`。** ↓ 原文保留
   ...
   - **(1) 缺 `status` 键 ≠ `!= done`,而它读起来是「可选」。** 全队 101 行里恰好三行完全没有
     ...
     **每轮都会重新选中它**。已补 `delivered-and-consumed`(照 `hero-31`/`hero-35` 的词,
     不造新词,GH #317);`hero-39`/`hero-54` 补 `pending`。**这是仪器唯一升成 exit 3 的发现。**
   - **(2) 一份读数可以坐在四行上。** `hero-23/24/25/29` 共享同一段 W53 家族读数
     (51-id 臂串,gpm −6.80,`comps_better 1/3`,段落自己写着条件 (a) 仍欠)⇒
     **`-137` 候选表里那四行是一份,不是四份**;按行数点它,是铁律 4(i-a) 的行版。
     ⚠️ **整段哈希看不见这一条**(两行带不同的头),只有按 `||` 切段才看得见 ——
     测试里 `2b` 是**负向断言**,专门钉这一点。
   - **⭐ 一般规则(带走):拿到一条「按某个字段筛出来的候选表」,先问那个字段
     **缺席**时读成什么。**`status` 缺席读成「不是 done」,于是花过的钱与没花的钱同形;
     这与 `-136` 的「抬头的前提没进 `if`」、`-137` 的「这一项与上一行共线」是同一族:
     **筛子的失效方向,不在它筛掉的那些行里,在它没写出来的那一格里。**
   - **仪器价钱**:`tools/agent/queue_reading_census.py` + `tests/test_queue_reading_census.py`
     (16 例 0 失败)+ `tools/agent/mutstand_queue_reading_census.sh`(**10/10 CAUGHT**,
     每发锚在本轮移动过的读数上)。class 永远与触发它的 marker 一起打印;打平打 AMBIGUOUS。
   - **下一批真候选(仪器给的,不是手记的)**:`hero-1` / `hero-2` / `hero-10` —— ⚠️ `-137`
     已警告 hero-1/hero-2 的 result 是**裁定**不是读数,**而仪器把它们分类成 DOMAIN-READING**
     (它们的确带了分母);**这不是仪器错,是 LIMITS 第一条的现场**:class 是关于**文本**的
     证据,marker 就印在旁边给你一眼推翻。**下一轮要么去读那两行的原文,要么去做 `hero-10`。**
   - **⛔ 本轮试过并当场判死的两个 `bots/` 候选(不要重查)**:(甲) Lion 补蓝支路从
     `GetNearbyCreeps(1600)` 选靶却当施法单花掉(与 GH #625/#643/#657 同族)——
     **小兵支路在本仓任何一帧上都验不了**(GH #658 §4:语料 `GetNearbyCreeps` 恒 `{}`;
     dumper 的 `creeps[]` 只有 `{t,team,x,y}`,GH #581)⇒ 不写一条落地即无 fixture 的 lever;
     (乙) `hero_lion.lua` `X.ConsiderW`/`X.ConsiderE` 各有一对**死局部**
     (`nDamage`+`nDamageType` 无消费方,`lion_voodoo`/`lion_mana_drain` 都无顶层
     `AbilityDamage` ⇒ 恒 0,GH #175/#644 同族)—— 零行为,不为产出制造 diff。
   - **⭐ 瓶颈仍是 `hero-54`(帧供给,优先级 1,未裁)**:GH #658 的那句话本轮被独立验到第二次。
     下一轮若仍未裁,按铁律 9 在报告里升级并指名下一步。
   - **⭐ 自检**:形状照抄 `-137`(`bash tools/agent/routine_selfcheck.sh > /tmp/sc.log 2>&1; echo "EXIT=$?"`,
     不接管道、不加 timeout、不加 nohup)。**本轮实测 ~30 分钟**(`-137` 记 18 分钟、`-136` 记 13 分钟,
     单调在涨);**python 腿仍 9 项 UNCERTIFIABLE**(GH #507/#358)。
     等待仍用 `until ! pgrep -f "[r]outine_selfcheck.sh" >/dev/null; do sleep 15; done`,
     自检跑着时不跑任何 `tests/` 套件。

-137. ~~**⭐ 下一轮:`-135`/`-136` 那条路仍然在产出 —— 去找**第四份**没人消费的域读数。**~~
   ✅ **2026-09-10T01:48Z 执行完毕,而答案是「先修判据」:那第四份读数不存在 ——
   剩下的四行 `harvested_pending_verification` 共享同一段波次读数,而候选表里还坐着一行
   上一轮刚花掉的 `hero-37`。** ↓ 原文保留
   判据**第三次生效、第三次给出一个 `bots/` 主体**,所以它现在是本组的默认路径,不是一次巧合:
   **`queue.json` 里 `status != done` 而 `result` 已经有东西的 `hero-*` 行,就是买回来了没人花的钱。**
   本轮花掉 `hero-37`(`zusultstrand`,交付三天、`hero-51` 只消费了其中的创建帧那一格)。
   **下一批候选(都还挂着 result、都没被消费)**:`hero-1` / `hero-2` / `hero-3` / `hero-4` /
   `hero-10` / `hero-40` / `hero-51`,以及 `harvested_pending_verification` 的
   `hero-15` / `hero-23` / `hero-24` / `hero-25` / `hero-29`。
   ⚠️ 先读一眼 `hero-1`(`wkqaim` 已裁「不写」)、`hero-2`(Axe 分母结构性为 0)——
   这两条的 result 是**裁定**不是待花的读数,别当第四份。
   - **⭐⭐ 本轮那一课(它把 `-136` 的一般结论推进了一层):**
     `-136` 写的是「先核对**抬头写的前提有没有落在代码里**」。**本轮那个前提落在了代码里** ——
     `if` 里确实有那个环 —— **坏的是它与上一行共线**:外层合取项
     `bot:WasRecentlyDamagedByAnyHero( 2.0 )` 已经蕴含「刚才有敌人进过自己的攻击距离」,
     于是「1600 内有敌方英雄」几乎白写。
     **⭐ 而共线是**可以从读数本身算出来**的,不必读源码就能起疑:**
     1600 切掉 **10.7% 的帧**却只切掉 **1.9% 的 episode**。
     ⇒ **一般规则(带走给下一份比例读数用):拿到「这个过滤器吃进 N%」时,
     先问它在**决策尺度**上吃进多少。两个尺度的裂口不是精度问题,是
     「这一项与它上一行共线」的读数** —— 一条只开火一次的分支,按帧删而按 episode 不删,
     **一个决策都没改**。(这是铁律 4(iii)「连用哪种切法一起登记」的正面用法。)
   - **⭐ `-136` 的「按环 X 切的读数只给按环 X 写的那条腿定价」本轮第二次生效,而且这次
     它决定的是**下一棒的形状**:** 收窄到 700 之后,四列读数里**没有一列**是新腿的域 ⇒
     下一棒不是「要不要入集」,是 **`hero-55`**(同一把仪器、同一份 152 局语料、按新常数重跑)。
     **仪器已经按新常数改好并 76 PASS / 0 FAIL,所以这一棒的边际成本是一次 scan,不是一次调查。**
   - **⛔⛔ 收窄一条 gated 杠杆时,检测器常数与源码常数必须同 commit 移动,而挡路的是
     **把常数写成字面量的自检**。** `test_detector_source_constants.py` 的镜像断言逼着两边同步
     (好事);真正花时间的是检测器**自己的 selfcheck**里四处字面量:`_base_tl` 的敌人默认
     `800`、三条边界 `1601/1600/1400`、幻象检查的 `900`。它们在旧值下都在环内、新值下都在环外,
     于是**常数一动,一批与边界无关的检查一起红**(实测 **75 PASS / 1 FAIL**,红的是幻象那条)。
     ⇒ **改环之前先把 selfcheck 里的距离全部写成 `CHASE_RADIUS` 的表达式**,再改常数。
     同族:输出键名 `any_in_1600` / `ep_in_1600` / `le_1600` —— **留一个叫 1600 而实际是 700
     的名字,就是下一轮误读的现成材料**,已改名(全仓 grep 确认无外部消费方)。
   - **⛔⛔ 环一动,嵌套关系可能翻转,而翻转会**静默改掉别人那一列的含义**。**
     1600 时 chase 环**包含** `ultcash` 的 1200 环;700 时**严格落在 1200 内** ⇒
     几何那半的重叠**按构造变成全覆盖**,`hero-37` 第 (4) 列的 PARTIAL 结论不再可搬。
     处置:检测器加断言 `chase_ring_is_inside_the_ultcash_ring`,有人调回 1200 以上就红。
     **下次动任何一个与别的环比较过的常数,先算一遍两个环的包含方向。**
   - **⛔ 变异台的两发新弹都锚在**收窄 MOVED 过的读数**上**(`-136` 的规矩,本轮遵守):
     M12 = 回滚到 1600(收窄只剩散文);M13 = 把**唯一有差的那一帧**从普查表里剪走
     —— 后者是「只写 `nNew < nOld` 不写精确计数」会静默放过的形状。**13/13 CAUGHT。**
   - **⛔ 仍然不许**排成主体的:见 `-133`/`-134` 那两张清单,本轮未变。
   - **⭐ 自检(本轮零踩坑,形状照抄)**:`bash tools/agent/routine_selfcheck.sh > /tmp/sc.log 2>&1; echo "EXIT=$?"`
     —— 不接管道、不加 `timeout`、不加 `nohup`。**实测 ~18 分钟**(`-136` 记的 13 分钟又长了)。
     `SELFCHECK_EXIT=3`(cadence / owed-executions / a-evidence-owed);
     **python 腿仍 9 项 UNCERTIFIABLE(120s 预算跑不完发现集,GH #507/#358),那一侧这轮没人看过**。
     等待用 `until ! pgrep -f "[r]outine_selfcheck.sh" >/dev/null; do sleep 15; done`,**自检跑着时不跑任何 `tests/` 套件**。
   - **⭐ 第二页上仍没排到的 `[hero]`**:#587 / #567 / #564 / #563 / #560 / #554 / #549
     (#566 已于 2026-09-06 消费完并落地 `liondrainmi`;#562、#274 家族、#593/#564 分别已消费)。

-136. ~~**⭐ 下一轮:`-135` 那条路还没走完 —— 去找**第三份**没人消费的域读数。**~~
   ✅ **2026-09-09T23:08Z 执行完毕。花掉 `hero-37`(`zusultstrand`),主体是一个 `bots/` 行为改动
   (`X.nUltCashChaseRadius` 1600 → 700)。** ↓ 原文保留
   `-135` 的判据本轮第二次生效,而且**这一次读数是正的**,所以那条判据比它自己写的更宽:
   **`queue.json` 里 `status != done` 而 `result` 已经有东西的 `hero-*` 行,不管读数是正是负,都是没人花的钱。**
   本轮花掉 `hero-31`(`wkbonefight`,交付三天没人碰)。**下一批候选(都还挂着 result、都没被消费)**:
   `hero-1` / `hero-2` / `hero-3` / `hero-4` / `hero-10` / `hero-40` / `hero-51`,
   以及 `harvested_pending_verification` 的 `hero-15` / `hero-23` / `hero-24` / `hero-25` / `hero-29`。
   - **⭐⭐ 本轮那一课(它把 `-135` 的那一课推广了):**
     `-135` 写的是「读数证伪的常常不是杠杆,是杠杆的**写法**」,证据是一份 **4:1 的负读数**。
     本轮读数是**正**的(域没空,退回门不触发),**动作却一模一样**,而且答案也一模一样:
     **抬头写的支配性前提没进 `if`。** ⇒ **那一课与读数的符号无关。**
     拿到任何域读数,**先核对抬头写的前提有没有落在代码里**,再决定按读数放行还是判死。
     本轮的具体形状:`wkbonefight` 的 armed 腿按 **1600 视野环**计数,而它的论证
     (「joins a **3v3**」「**hero-facing** attack-seconds」)是一句关于**40s 近战骷髅够得着的人**的话
     ⇒ 收窄到分支自己下一行就在用的 **650**。13 帧上 armed **9 → 6**。
   - **⭐ 一般结论(可带走,给下一份读数用):一份按环 X 切的读数,只给**按环 X 写的那条腿**定价。**
     `hero-31` 的 12,656 是按 1600 切的,收窄后它**不再是这条腿的域**;
     四列读数里**没有一列**是新腿的域 ⇒ 本轮的下一棒不是「要不要入集」,是**要多一列 (3′)**
     (650 环内人数分布,1 与 ≥2 分开)。**给杠杆定价前,先问读数是按哪个谓词切的。**
   - **⛔⛔ 自检:本轮把 `-133` 的两条禁令**分两次各犯了一条** —— 第一条命令接了管道
     (脚本 REFUSED,exit 2,什么都没检查),改用重定向后又**加了 `timeout 300`**
     而实测要 **13 分钟**(EXIT=**124**,又一次什么都没读到)。第三次才跑对。
     **下一轮第一条 Bash 命令只准是**
     `bash tools/agent/routine_selfcheck.sh > /tmp/sc.log 2>&1; echo "EXIT=$?"`
     —— **不接管道、不加 `timeout`、不加 `nohup`**(`nohup` 本轮也试了,4 行就死)。
   - **⛔⛔ GH #507 的范围要改,理由是本轮买到的一个新形状,它比旧形状更会骗人。**
     `-134`/`-135` 把这条记成「**争 CPU**」(后果:9 项 UNCERTIFIABLE,诚实地说「这一侧没人看过」)。
     本轮的实际故障是**争一个共享文件**:我在自检跑着时跑了 `run_tests.lua wk_bone_guard_enemy_count`,
     两边**都写** `bots/Customize/soak_side.lua`(gate 开关)—— python 腿写下开关、列语料、
     断言开关在盘上,我的 lua 跑把它删了 ⇒ 自检报 **TRUNK RED**(`test_lua_corpus_stability.py`),
     **单独重跑 EXIT=0**。**后果比争 CPU 严重得多:那是一个看起来像「主干坏了」的假阳性。**
     ⇒ **规矩改范围不改理由:自检跑着的时候,不要跑任何 `tests/` 套件,不只是不要跑重活。**
     阻塞写法照旧(⚠️ 别用忙等 `do :; done`):
     `until ! pgrep -f "[r]outine_selfcheck.sh" >/dev/null; do sleep 15; done`
   - **⛔⛔ 收窄一条 gated 杠杆时,**不传新参数**的旧断言会静默变成空过 —— `-135` 记过,本轮又撞上一次,
     所以它不是偶然是**形状**。** 本轮 §2 出货普查、§3 阶梯、§4 三个 no-op 对照原来**都不传表**,
     收窄后全都落到**限制性回退**上 ⇒ 无论 gate 开不开都返回出货答案,**对照什么也证明不了**,
     而「21 例全绿」**看不见这件事**。处置:§4 新增 `opener_table()`,**先断言这张表真能打开 armed 腿**
     再拿它做对照;§3 阶梯改扫 **(count, table) 对**(143 对,widening 40 对)。
     **变异台每一发都要锚在收窄 MOVED 过的读数上**(本轮:9→6、点名的 3 帧),
     锚在它没动的东西上就是 `-135` 那三发「因为正确的修复而 SURVIVED」的成因。**10/10 CAUGHT。**
   - **⭐ 孤儿没有重演**:新谓词 `X.wk_CountBoneGuardEngaged` 是被**调用**的,不是只当函数值传
     ⇒ `test_hero_export_reachability.py` 天花板 **34 未动**。`-135` 那条教训生效了,记一笔。
   - **⛔ 仍然不许**排成主体的:见 `-133`/`-134` 那两张清单,本轮未变。
   - **⭐ 第二页上仍没排到的 `[hero]`**:#587 / #567 / #566 / #564 / #563 / #560 /
     #554 / #549(#562、#274 家族已分别于 17:10Z / 本轮消费)。

-135. ~~**⭐ 下一轮:去找**第二份没人消费的域读数**,像本轮花掉 GH #562 那样花掉它。**~~
   ✅ **2026-09-09T21:16Z 执行完毕。花掉 `hero-31`(`wkbonefight`),主体是一个 `bots/` 行为改动。** ↓ 原文保留
   §2.1 仍然绿(`focus` 92 例 0 失败),所以「从源码里找焦点五的新杠杆」这条路**这一轮也没开**;
   本轮走通的是另一条,而且它比 `-134` 那条更可复制:
   **`queue.json` 里 `status != done` 而 `result` 已经有东西的 `hero-*` 行,就是一份买回来了没人花的读数。**
   本轮花掉的是 `hero-35`(GH #562,交付三天没人动过那条杠杆的代码)。
   - **⭐⭐ 本轮那一课(下次拿到任何域读数先问这一句):**
     **读数证伪的常常不是杠杆,是杠杆的写法。** #562 建议「不入集,理由 4:1」;
     但那 4:1 是**未收窄那一版**的读数 —— helper 抬头逐字写着的支配性前提
     (「另有候选时重放才被支配」)**从来没进过 `if`**。把那句话写成合取项,
     121 个纯成本 episode 按构造就不再触发。⇒ **先核对「抬头写的前提有没有落在代码里」,
     再决定是不是要按读数把杠杆判死。**
   - **⛔⛔ 收窄一条 gated 杠杆时必查变异台的 want 串会不会因为修复本身而失效。**
     本轮 `M2`/`M3`/`M7` 三发原来钉在 flee 帧的 e2e 上,而收窄后那一帧**两条腿都站下**
     ⇒ 三发会**因为正确的修复**变成 SURVIVED,而一个只看「11/11 还是绿的」的收尾**看不见**。
     同族两处:§2 的出货普查、§5 的 no-op 对照原来都是**不传表**调用,收窄后会**空过**。
     已全部重新上锚(台子 8 → **11 发,11/11 CAUGHT**)。
   - **⛔ 自检的新形状(`-134` 那条要更新)**:本轮**没有**抢 CPU(自检跑完才动重活),
     python 腿仍然 **9 项 UNCERTIFIABLE** —— 5a0 的 **120s 预算**跑不完 **87 个文件**的发现集
     (NOTE:87 file(s) in 120.1s / budget 120s)。**这不再是「谁在争 CPU」的问题,是预算被发现集长过去了。**
     要么下轮报进 GH(量具类,只能当附带一条),要么就每轮如实记「那一侧没人看过」。
   - **⛔ 仍然不许**排成主体的:见 `-133`/`-134` 那两张清单,本轮未变。
   - **⭐ 第二页上仍没排到的 `[hero]`**:#587 / #567 / #566 / #564 / #563 / #560 /
     #554 / #549(#562 本轮已消费,`hero-35` 的球交给语料侧)。

-134. ~~**⭐ 下一轮:先看 `tests/test_focus_decision_reachability.lua` §2.1 有没有红。**~~
   ✅ **2026-09-09T17:10Z 执行完毕。§2.1 仍绿 ⇒ 不从源码找新杠杆;改去消费一份已交付的域读数
   (GH #562),主体是 (i) 一个 `bots/` 行为改动。** ↓ 原文保留
...
-128. ~~**⭐ 下一轮:按 P4.4 (i) 继续找焦点英雄的 `bots/` 行为改动。**~~
   ...
   - **⛔ 仍然不许**排成主体的(各自在等一份别人手里的供给):`-119`(等录像组造
     ...
     `cmlaneband` 域(`hero-47`)、`zusjumpland` 域(`hero-48`)、`lionqkill` 域(`hero-49`)、
     **新增 `axebhreach` 域(`hero-50`)**;`wkreinctr` 是协同组的(GH #582)。
   - **⛔ 不许**碰 `X.HasSpecialModifier` 的出货名单(Axe):理由见 `-124`,逐字不变。
   - **⛔ 不许**把 `axebhreach` 与 `axebhrecast`(或 `axebhpure`)合取进一个谓词 ——
     那正是 pullcad 陷阱,而这两个 id **就坐在同一个 `if` 里相隔三行**,所以这条特别近;
     变异台 **M9** 就是它。要表达依赖就用 **promote-atom**,不许写进代码。
   - **⭐⭐ 本轮买到的一课,下一轮直接用**:「**惰性**」与「**死接线**」在一份读数里长得
     一模一样,而把它们分开的**不是**更多的 gate 测试(M4 能通过全部 gate 测试),是
     **一对**读数:(甲) 在**缺陷帧**上驱动出**翻转**;(乙) 在**前提为真的真实帧**上端到端
     驱动出**逐位不变**,并**同时断言那 4 帧开了火**(否则「arm 后什么都没变」对四帧
     *本来就什么都没发生*的帧同样成立 —— 那是一句空话)。本轮 §4.1 的
     `nCast == 4` 断言就是为了堵这个空话。
   - **⭐ 本轮(`-128`)顺手看到、没做的两条线索,都在 `bots/BotLib/hero_axe.lua`**:
     (甲) **`X.ConsiderW` 的打野落单点 `bot:GetNearbyNeutralCreeps( nCastRange + 100 )`
     同样无界**,而且它取的是 `J.GetMostHpUnit`(**最高血**,不是最低血)。本轮**故意
     没动**:目标是**小兵**、前提是 `J.IsFarming`,代价故事与团战那条**不同族**,
     合起来就是一次**捆绑**。已被 `tests/test_axe_battle_hunger_fight_reach.lua` §5.4
   ...
   - **⭐ 下一轮开工顺序(这条已连续三轮有效,继续保留)**:开工自检**跑完**再动变异台,
     **不要并行**(GH #507)。本轮遵守了(用 `until ! pgrep -f "[r]outine_selfcheck.sh"`
     阻塞等),没有复发。
   - **⭐ 自检退出码只能用文件重定向读**(`> /tmp/sc.log 2>&1; echo "EXIT=$?"`)。
     用管道读会被脚本当场 REFUSED —— 本轮是**第 12 次**踩到,且**又是本轮第一条命令**。
     **连续两轮同一条**;这条已经不是「提醒」,是「第一条命令的模板」。

-127. ~~**⭐ 下一轮:按 P4.4 (i) 继续找焦点英雄的 `bots/` 行为改动。**~~
   ✅ **2026-09-08T20:20Z 做完:换了英雄(Axe;上一次动他是 09-08T08:16Z 的 `axecullreach`,
   收的是 **R** 的选靶列表,而 **W(Battle Hunger)已登记的两个 id(`axebhpure`/`axebhrecast`)
   一个管伤害类型、一个管重复施法,到达这条链上一个 id 都没有**)。`X.ConsiderW` 八个
   落单点里六个把目标绑在 `nCastRange` 上,团战那个是唯一一个遍历 `nInBonusEnemyList`
   (`nCastRange + 200`)且零距离项的**英雄**落单点。gated `axebhreach` 把那一项补上。
   ...
   - **⭐ 承重的不是又一条到达项,是「这个落单点是一条**选择规则**」**:另外五个英雄落单点
     ...
     **错的是一次交换,不只是一次拉长。** 而且被挤掉的那个**下面没人接得住**,这一条是
     **闭式**的:armed 子集为空 ⇒ 射程内没有敌人通过那四条过滤 ⇒ 而下面两个英雄落单点
     (对线消耗/撤退)读的正是 `nInRangeEnemyList` 并施加**同一组四条过滤**外加各自额外的
     合取 ⇒ 它们也开不了火。
   - **⭐⭐ 端到端那半是零前提注入的,而且它的价值在于「不是空话」**:33 个 Axe 活帧里
     band 6 / 射程内 15 / 前提成立 6,**交集 0**;在那 6 个前提为真的帧上只放开冷却地跑
     `X.SkillsComplement`,**4 帧真的开火**、**那 4 帧选中的目标全在 `nCastRange` 内**、
     **arm 后 6 帧动作逐位不变**。`nCast == 4` 这条断言是**故意加的**:少了它,
     「arm 后什么都没变」对**本来就什么都没发生**的帧同样成立。
   - **⚠️ 第一版 §4 断言「0 帧开火」,当场红了 —— 而红出来的是好消息**:真实供给是 4 帧。
     **改的是断言,不是量具。**
   - **⚠️ 第一版 §1 的三个普查数(前提 5 / 驱动 5)也红了**:我用**原始 fixture 坐标**先估,
     而 mock 会按**可见性**过滤。**先跑再写数**,别把估计写进断言当读数。
   - **⭐⭐⭐ 变异台 M10 是本轮真正的看门狗**:把同一个 helper / id / 闸从**循环过滤**里
     挪到**选出的赢家**上 —— 保留全部 §5 断言、全部 gate 测试、note 一字不改,
     却把 lever 从「换一个够得着的目标」变成「这一帧不打了」,**那是另一条 lever 穿着
     这一条的名字和它整个 (c) 论证**。只有 §3.2 看得见它。
   - **⚠️ 开工自检 EXIT=3(有发现,不是通过)**:cadence / owed-executions,**都不是本轮的**;
     另有 UNCERTIFIABLE(`test_selfcheck_lua_leg.py` 等 120s 没跑完,与前两轮逐字同一条)。
     自检**第 12 次**用管道读退出码被当场拒绝,**且又是本轮第一条命令**。
   - ✅ **「自检与变异台并行」(GH #507)连续三轮没有复发。**

-127b. **⭐ 本条为历史保留(`-127` 原文的线索清单)。**
   - **⛔ 仍然不许**排成主体的(各自在等一份别人手里的供给):`-119`(等录像组造
     timeline)、CM 的 `cmfarcreep` 域(`hero-42`)、`lionultcash` 域(`hero-43`)、
     `lionrreach` 域(`hero-44`)、`wkqlane` 域(`hero-45`)、`axecullreach` 域(`hero-46`)、
...
-120. **⭐ 已由 `-120b` 消费(按 P4.4 (i) 找焦点英雄的 `bots/` 行为改动)。以下线索仍未做:**
   - **理由(2026-09-07T20:05Z 由 `-119` 自己的可行性检查交出)**:`-119` 要的三帧
     ...
     (全盘 `find` 只有 `tests/fixtures/timeline_synthetic.json`)。
     `hero.md:1275` / `:6645` 已两次登记同一件事:**S3 只存 `.dem` + `analysis.json`**。
     ⇒ 供给由 **replay-analyst / 录像组**交(同 `-81` 的处置),本组等它。
   - **⭐ 一条可以现在就做、且是 P4.4 (i) 主体的线索(本轮顺手看到,没做)**:
     `X.ConsiderW` 的冻兵块把**广列表半径 1400** 的兵直接交给
     `ActionQueue_UseAbilityOnEntity`,而 Frostbite 的施法距离是
     `GetCastRange()+30`(块内 0 敌人时被抬到 `GetAttackRange()+60`)⇒ 多开的火可能让
     CM **走过去**冻一只兵。`queue.json:hero-42` 的第 (3) 列就是为它准备的距离读数;
     **先拿到那一列再决定要不要加距离项**,不要凭直觉先加。
   - **⛔ 仍然不许**顺手改 `cmrangedhp` / `cmcreepcap` 的逻辑或 id:它们是同一个块上
     另外两条各自在等域的杠杆,本轮只改了 `test_cm_ranged_creep_health.lua` 的**刮取器**
     (理由见 `-119` 下面那条报告 §6),逻辑一个字节没碰。

-119. **重切那三个没有 abilities 表的 v1 fixture —— 一个**独立的、写明代价的**工作单元,
   不许搭车。⏸️ **2026-09-07T20:05Z 挂起,不是做完**:供给缺口已查清(见 `-120`),
   球交给**录像组**造 timeline;本组在那之前不要再把它排进工作单元(P4.4 也不允许它当主体)。**
   - **是什么**:`f_073148_zuus_lina`、`f_080225_wk_lane`、`f_080225_wk_revive`
     的 WK unit **根本没有 `abilities` 字段**(不是空表)。于是
     `tests/mock/replay_fixture.lua:725` 的 `ipairs(u.abilities or {})` 跑 0 次,
     句柄上什么都没装,四次读全部落到 `bot_api` 的通配
     (`^Get`→0 / `^Is`→false)⇒ **和一个真没点的大招逐位相同**。
     `-118` 已把这条机制驱动下来(`tests/test_wk_rank0_absence_join.lua` §1),
     并证明格式**能**记真的 rank 0(`f_230545_wk_laning_safe` 就记了)⇒
     **修法是 `make_fixture.py` 重切这三个,不是改 dumper。**
   - **⚠️ 代价先写清楚再动手**:这三帧一动,套件里**每一份 WK 普查**都要重读 ——
     至少 `tests/test_wk_save_mana_lock_census.lua`(§1 的 33/3 拆分、§2 的 21/11/6、
     §6 的那个 0)、`tests/test_wkreinctr_untrained.lua` 与
     `tests/_wkreinctr_sweep.lua`(36/24/14/15/23/2 六个数)、
     以及 `-118` 自己那份(§3 的 15/12/3、§5 的三名单)。
     **别把这当成「顺手补个字段」**:它是一次跨文件的重新基线,
     而且会**改变 `wkreinctr` 的域读数本身**(见 GH,`-118` 交出的那条)。
   - **⛔ 仍然不许**顺手改 `wkreinctr` 的注/测试:那是协同组的 id(GH #582)。
     重切 fixture 后由**它的所有者**重读它自己的数。本组只交付重切 + 本组文件的重读。
   - **⭐ 顺带(量具/流程类,至多附带一条,但这一条已经攒够证据了)**:
     GH #507 那条并发撕裂**在 `-118` 一轮之内发作三次**(变异台 vs 自检、
     全量套件 vs 自检、串行脚本被 harness 压行后又一次),其中**两次发生在
     「已经记住要串行」之后**;跨轮看则是 `-114`、`-115`、`-118` 连续三轮自曝。
     ⇒ **靠人记住不成立,要机制**。给总监的具体提案(带失败形状,不只是「记得串行」):
     (i) `bots/Customize/soak_side.lua` 改走 **per-pid / per-run 路径**,
     并发就不再是同一个文件;或 (ii) `run_tests.lua` 与 `routine_selfcheck.sh`
     **各自拿一把锁**,拿不到就 **exit 2(没跑成)而不是继续跑出污染读数**。
     两条都比「每轮在报告里写一遍记住」便宜。
     **⚠️ 附带这条时注意 harness 会把多行 bash 压成一行** —— `-118` 的第三次
     发作就是这么来的:`until … done` 后面的命令被吞进同一行,等待没生效。
     要串行就写成**单行 `&&` 链**或**一个 `.sh` 文件**,别提交多行脚本。

-118. ~~**把 census §1 与 `wkreinctr` 那两个看起来矛盾的读数当成同两帧的两种读法去证伪。**~~
   ✅ **2026-09-07T16:55Z 做完:是同两帧,而且 census 的读法对。`bots/` 零改动。**
   报告 `iterations/reports/hero/20260907T165504Z.md`。新
   `tests/test_wk_rank0_absence_join.lua`(**12 例**)+
   `tools/agent/mutstand_wkrank0join.sh`(**10/10 CAUGHT**)。
   `queue.json:hero-41` 就地补上判读结论(**没有另开请求**)。
   选题依据 OWNER_PRIORITIES **P4.4 (ii)**(本轮 [hero] issue 一条可认领的都没有)。
   - **⭐ 减法先做完了:两个「2」是同两帧**,`f_073148_zuus_lina` 与
     `f_080225_wk_revive`。断言写成**集合相等**不是计数相等(计数正是立案时那个
     答不了问题的量具)。顺带钉住:前者**就是 `test_wkreinctr_untrained.lua` 自己的 PIN 帧**。
   - **⭐⭐ 哪一种读法对,答案在 loader 不在数里**:reader 全装在
     `ipairs(u.abilities or {})` **里面**,没有 abilities 表就一个都不装 ⇒
     `0/false/0/0` 由通配产生。对照:在**计价**帧上读一个树里没有的技能名,
     四个答案**逐位相同**。⇒ **rank 0 在那 3 帧是缺席。**
   - **⭐⭐⭐ 代价:那 14 个 flip 拆成 12 + 2,而调用点自带的等级闸拒掉了全部 12**。
     12 个**真实**的 rank 0 英雄等级全是 1..5 —— 等级 6 之前 Reincarnation
     **根本不能点**,那不是缺陷是规则;唯二越过 `GetLevel() >= 6` 的正是两个缺席帧。
     ⇒ **这份语料里这条杠杆要的形状(等级≥6 且大招点真没花)一帧都没有**,
     而出货注把调用点的 0 **全部**归给 `IsInTeamFight` 恒假,并称那两帧是
     「real replay frames, not a constructed case」—— 真实的是**等级 7**,
     不是「no point in the ultimate」。
   - **census §6 的第三条读数也对上了**:计价 ∧ 等级≥6 ∧ rank 0 = **0**,
     本轮**从 raw rank 重算**(不走 loader)⇒ 两个文件不可能读的是同一个人造物。
     另加 raw↔driven 一致性一例,否则 `raw_rank == nil` 不再等于「loader 无米下锅」。
   - **⚠️ M9 是自己招的**:两个空集合**相等** ⇒ `A ≡ B` 在什么都没量到时照样通过。
     反真空 `#A > 0` 是唯一挡住它的东西。M2/M6 是**行为零移动**、只被源码断言看见的,
     写进台子头注而不是藏起来。
   - **⚠️ 三条操作自曝见报告 §9**:(0) **又踩了一次 GH #507 并发撕裂窗口**
     (第一份自检读数作废,已在安静树上重跑);(1) 第一次自检用管道读退出码,
     脚本**拒绝执行**(证据纪律 3,第 5 次复发);(2) 我写的自检断言**用明文搜自己**,
     那个搜索参数把针放进了草堆,第一次运行就对自己开火;
     (3) **mock 覆盖全局 `print`** ⇒ 第一版探针输出零行、退出码 0,长得像通过。

-118（原文备查）. **⭐ 下一轮的第一件事(由本轮 `-117` 交出):把 census §1 与 `wkreinctr` 那两个
   **看起来矛盾的读数**当成同两帧的两种读法去证伪 —— 而这一条**先做减法**。**
   - **矛盾是什么**:`tests/test_wk_save_mana_lock_census.lua` §1 把 3 个「没有 abilities 表」
     的 WK 帧**排除**掉,并写明「其中 **2 个是 level >= 6**,这正是它们必须被排除而不是
     被记成『储备没开火』的理由」;而 `tests/test_wkreinctr_untrained.lua` 头注写
     「36 个活体 WK 帧里 24 个答 TRUE,其中 **14 个坐在 ability level 0 上**,
     **两个在英雄等级 7**」。**两处的「2」很可能是同两帧**
     (`f_073148_zuus_lina` / `f_080225_wk_revive`,两个都是 level 7、都在 census 的 unpriced 三人组里)。
   - **⚠️ 先做的是减法,不是新读数**:先把这两句**对上号**(是不是同两帧),
     再判**哪一句用错了量** —— 一个把 rank 0 读成**缺席**,另一个把它读成 **0**,
     而 `wkreinctr` 的 14/24 = **58% 的域**是建在后一种读法上的。
     ⛔ **不要**顺手去改 `wkreinctr` 的读数或它的文件:那是协同组落地的 id(GH #582),
     本条要交付的是**哪一种读法对**,连同证据,以 issue 形式交出去。
   - **供给就在本地**,不需要波:两个文件、两组帧,`tests/mock/replay_fixture.lua` 的
     abilities 装载路径决定了空句柄答什么。`-117` 已把「33 个计价帧里 level>=6 且
     R rank 0 的帧数 = 0」复现过一遍,这是矛盾的第三个读数,**也要一起对上号**。
   - **顺带**:`iterations/queue.json:hero-41` 已经把这个数当一列挂出去了
     (「只要那个数,判读留给下一轮」)——**下一轮就是这一轮**,别等批测台。
...
-117. ~~**焦点英雄 Wraith King:`X.ShouldSaveMana` 为一件它从不评估概率的事扣住半个蓝池。**~~
   ...
   **推在开 issue 之前** —— 铁律 6 的 GH #290 那一条)。
   - **选题**:本轮 [hero] issue **一条未认领的都没有**(#570 待总监裁、#577 的拆 id
     ...
     附带一条)⇒ 走章程工作流第 1 步的回落分支。`-116` 降为附带一条,两件都做完。
   - **缺陷**:`X.ShouldSaveMana`(出货、无闸)判五件事 —— 等级、两个非 nil 句柄、
     R 冷却、一道减法 —— **没有一件问「他会不会死」**。而 Reincarnation **不是一次施法**,
     它的 220 蓝在**死亡那一瞬**才被扣走。代价还是双份:`X.ConsiderQ` 与 `X.ConsiderW`
     **各自第一行**都读它(「恰好两个 ability 调用点」已钉),闸一响两个技能同时下架。
   - **⭐ 域是量的**:33 个**计价**帧(切法与 census §1 逐字相同)上储备开火 **6**,
     其中**满血 + 1600u 内零可见敌人**的 **2**(`f_114311_drow_pushguard_silent` lv7
     1044/1044、`f_260820_103216_cm_es_aftershock` lv8 1110/1110);armed 后 true→false **2**、
     false→true **0**。另外 4 帧 1200u 内有 1–2 个可见敌人 —— 储备存在的形状,一个不碰。
   - **⭐⭐ 自己招的一条:血量那一项在本语料上不承重**。6 个开火帧血量 0.69..1.00,
     被释放的两帧是仅有的两个 1.00 ⇒ **(0.87, 1.00] 里任何阈值选出同样这 2 帧**。
     0.95 是为真实对局取的,**语料区分不了它** —— 变异台 **M7**(0.95→0.60)因此
     **行为零移动**,只被源码断言抓住,并写进了台子头注。一个「全杀」而其中一个数
     其实没被量过的台子,是在骗读者哪几个数承重。
   - **⭐⭐⭐ 方向由形状保证不由算术保证**:出货五项**绑定成 `bShipped`**,
     释放只在 `bShipped` 已为真时可达,末句 `return bShipped`。变异台 **M3**
     (去掉 `bShipped and`)**今天行为上零移动** —— 只有源码断言看得见,而它防的是
     将来某次编辑让 armed 腿**发明**一条出货腿从没要过的储备。
   - **顺带查明的一件事,写进边界不写进结论**:Reincarnation 是 **220/110/0**,
     3 级免费 ⇒ 这条储备**本来就会自己塌掉**,杠杆管的是 **rank 1/2 那扇窗**(等级 6–9
     时 220 是 272–399 池子的大半)。hero-41 的地平线注意事项与 hero-10 **方向相反**:
     那条怕语料太早,这条怕语料太晚。
   - **本轮明确没做的**:同一函数里的 rank 盲区(`nLV >= 6` 顶替「R 学过了」)。
     33 个计价帧里 level>=6 且 R rank 0 **为 0**,就地无证据;§7 断言没顺手关掉它。
     由此长出 `-118`。

-116. ~~**把 `-115` 修出来的那条归因,落到它真正会改变行为的那个地方。**~~
   ✅ **2026-09-07T14:09Z 做完,作为 `-117` 的附带一条(P4.4 允许的那一条)。**
   - HONEST BOUNDS 第二条改写:成因是 **loader 没接 `GetNearbyCreeps`** 而不是 dumper
     schema。**判断本身没动**(`-114` 的 NOT TAKEN 不受影响),动的只有成因 ——
     两句话得到同一个 0,**复活条件相反**。
   - 新增一例**把机制本身断言下来**(注释正是这轮出错的载体):(a) loader 提到
     heroes/towers/barracks 而不提 `GetNearbyCreeps`;(b) `bot_api.lua:175` 的通配还在;
     (c) 语料**不是**瓶颈 —— driven fixture 文本里有 `npc_dota_creep_*`。§4 那条 0 的
     失败信息也跟着改写。
   - `tests/test_lion_drain_combat_widen.lua:263` 那条重言加注:它断言的是**mock 的 0
     不是这一帧的 0**,是**绊线**不是发现。措辞抄 `test_cm_frostbite_creep_cap.lua`/
     `test_cm_ranged_creep_health.lua`(按 `-116` 要求没自造新说法)。
   - **⛔ 没有顺手去接 `GetNearbyCreeps`** —— `-116` 明写那是独立的、要写明代价的工作单元。

-116（原文备查）. **⭐ 下一轮的第一件事(由本轮 `-115` 交出):把 `-115` 修出来的那条归因,
   落到它真正会改变行为的那个地方 —— `tests/test_lion_drain_refill_domain.lua`
   的 HONEST BOUNDS 那句「dumper schema 没有 creep 通道」。**
   - **为什么是它**:`-115` 已经证明零的成因是 **loader 没接 `GetNearbyCreeps`**
     (`tests/mock/replay_fixture.lua` 接了 heroes/towers/barracks 三个,唯独没接它;
     于是落到 `tests/mock/bot_api.lua` 的通配 `if key:find('^GetNearby') then return {} end`),
     **不是** dumper schema。两句话得到同一个 0,但**复活条件完全不同**:
     照现在那句写法,谁给 dumper 加了 creep 通道就会以为这些零该动了 —— **它们不会动**。
     `-115` 只在自己文件里把归因写对并加了 M8 做警戒线,**没有去改那句话本身**(不越界改别人刚落的文件)。
   - **⚠️ 先做的仍然是减法**:那个文件是 `-114` 判 NOT TAKEN 的载体,
     **判断本身不动**(两条读数同向,结论不受影响),动的只有那一句**成因**。
     改完把 `tools/agent/mutstand_liondrainrefill.sh` 跑一遍确认 12/12 还在。
   - **顺带**:`tests/test_lion_drain_combat_widen.lua:268` 那条
     `#bot:GetNearbyCreeps(1600, true) == 0` 是同一族的重言,值得加一句注说明它断言的是什么。
     `tests/test_cm_frostbite_creep_cap.lua` 与 `tests/test_cm_ranged_creep_health.lua`
     **早就**把这个零当 mock 的写了,可以直接抄它们的措辞,**别自己新造一套说法**。
   - **⛔ 不要**顺手去接 `GetNearbyCreeps`。那会让补蓝那一支第一次可达,
     连带 `-114` 的 NOT TAKEN、`-115` 的 11% 上界、以及 hero-40 第四列**同时**需要重读;
     真要做,那是一个**独立的、写明代价的**工作单元,不是搭车。

-115. ~~**把 `-114` 的**同一台读法**转到 `X.ConsiderE` 的**第一处早退**。**~~
   ✅ **2026-09-07T10:49Z 做完:读数买到了,判 NOT TAKEN,`bots/` 只动了一处注释。**
   报告 `iterations/reports/hero/20260907T104945Z.md`。新
   `tests/test_lion_considere_earlyreturn_domain.lua`(**13 例**)+
   `tools/agent/mutstand_lionearlyreturn.sh`(**10/10 CAUGHT**)。
   `queue.json:hero-40` **加第五列 + 修一处前提**(**没有另开请求**)。
   - **读数 —— 那道早退是整个 E 下半段的主约束**:27 个活体 Lion 瞬间里 E 可施放 **20**,
     其中 **17 (85%)** 被这行否决,只有 **3** 落穿。
   - **⭐ 两半拆开,答案和 `-115` 防的那个陷阱方向相反**:边际否决
     `IsOtherAbilityFullyCastable()` **15/20** 对 `nSkillLV <= 1` **1/20** ——
     **等级那半近乎冗余**(两个 rank≤1 的瞬间有一个另一半已经否决了),
     做工的确实是 castability 那半,15 比 1。陷阱问对了,不许因此不拆。
   - **供给 castability 那半的是基础技能不是大招**:Q 12/20、W 11/20、R **5/20** ——
     `[liondrain]` 那条注对称地点名三个技能,它们并不对称。
   - **⭐⭐ 给 `liondrainmi` 买到 11% 的域上界**:打架抽蓝那一支在这行**下游**
     (源码字节偏移断言),所以域 ≤ **3/27**;真实值严格更小(中间还隔
     `IsDrainSafeToStart` / `IsInTeamFight` / `IsGoingOnSomeone` 三个合取项,三个都写成了断言)。
   - **⛔ 3/27 是天花板不是 rate,而且这条是被自己的断言改强的**:草稿写「三个里有两个」,
     跑出来是 **3/3** —— 每一个落穿的瞬间都是历轮**因为抽蓝有意思才切下来的**帧。
     按被测结果全选样 ⇒ 作为 rate **向上**偏到不能再偏。归档在原理上答不了 (1) 列的 rate。
   - **判 NOT TAKEN 而且是读数确认了设计**:三个落穿瞬间**全部**恰是 `[liondrain]` 注点名的
     那个状态(Q/W/R 一个都放不出来,逐帧断言不是复述)。加宽它先买到的是
     「在还有控可放的帧上站着引导一个多秒定身技能」,正是那条注已经拒绝过的交易。
     缺席断言两条(无 `IsSoakCandidate` / 谓词未被就地改写),对应 M6 / M3。
   - **⭐⭐⭐ 变异台逼出两个真缺陷,不是走过场**:
     (i) **M4**:文件头 t10 注**逐字引用**了那行早退,于是所有「调用点在下游」的
     字节偏移断言其实是在跟一条**注释**比大小 —— 上提调用点照样通过。已改成按
     **制表符缩进**解析代码行,并断言这样的代码行**恰好一条**。
     (ii) **M6**:检测「有没有人给这行上闸」的断言,本身是按**原文**定位的 ——
     上了闸文本就变了,断言于是**在它唯一该响的变异上消失**。已改成按
     「调用 helper 的那条代码行」定位,与行内还写了什么无关。
   - **一处仓库现存错误,只在自己文件里修正并加警戒线**:见 `-116`。

-115（原文备查）. **⭐ 下一轮的第一件事(由 `-114` 交出):把 `-114` 的**同一台读法**转到
...
-109. ~~**Lion 另外两支的 `J.CanCastOnNonMagicImmune` 是不是过严 —— 一个 WIDENING 杠杆。**~~
   ...
   **本轮买不到的那一半也写清了**:分支**不可达**(`GetTarget`/`GetAttackTarget` 皆 nil,
   ...
   - **来源**:-108 撤下 `liondrainbkb` 时,录像组和本组都确认「三处挑目标对魔免的
     ...
     **引导跑得完**、**蓝真的会掉**(读数见报告 `20260906T171801Z.md` §1)。
   - **⚠️ 三条不许省**:(a) **要自己的 id、自己的域、自己的证据** ——
     `20260906T171801Z.md` 一个字都没测过这一支,不许拿它当立案证据;
     (b) 方向是 **WIDENING**,armed 只会**增加**抽蓝施放,负读数**绝不能**
     读成「少抽了 N 次」;(c) 先想清楚**补蓝那圈**(目标是小兵/野怪)和
     **打架抽蓝**(目标是 botTarget)域完全不同,**很可能是两个杠杆不是一个**。
   - **供给**:`tests/frames/f_260905_004847_lion_drain_bkb.lua`(**staged,不在语料里**)
     是整棵树上**唯一**带 BKB 魔免敌人的 Lion 帧;`tests/fixtures/` 语料本身
     **仍然一个都没有**(40 个敌方英雄实例,魔免 0)。别把这两句合并。

-108. ~~**GH #566 —— `liondrainbkb` 的立案前提在真实帧上被证伪。**~~ ✅ **2026-09-06T17:18Z 做完:
   撤下(不是解释那 8 帧)。** 报告 `iterations/reports/hero/20260906T171801Z.md`,
   `state.json:liondrainbkb_20260906`(新增四个 `_20260906` 字段,立案原文保留不改)。
   `X.lion_IsDrainTargetCastable` 与 gate **已从 `bots/` 删除**,团战吸蓝恢复
   `J.CanCastOnMagicImmune( npcEnemy )`;因该 id **从未 armed**,**对出货零变化**。
   证据由本组**自己从三份 `.dem` 重导**(不是转述录像组的表):下单在魔免亮了 **7.1s**
   后仍被引擎接受(`1db27d` t=1526.0)、引导 **5.1s 整条嵌在 BKB 里**(`b34547`,
   **已冻成 fixture**)、**蓝真的在动**(`d21f34`:Lion **+157** 对基线 +8/s,目标 **−87**)。
   新 fixture + 重写的测试(**10/10**)+ 重写的变异台(**8/8 CAUGHT**)。
   标签建议 **`PREMISE-FALSIFIED`**,**归总监裁**。
   **⚠️ 外溢很大,见报告 §4**:这是语料里**第一个后期帧**。第一版放进 `tests/fixtures/`
   并逐个改别组读数 —— **那条路是错的**,已**全部回滚**,帧改为 staged 在
   **`tests/frames/`**(按名字读),reopen 清单写进那份 README 且**一条都没付**。
   **原始条目(备查):** GH #566 要求本组要么解释那 8 帧,要么撤下/反向重写。
...
- 2026-09-13T08:15Z(报告 `iterations/reports/hero/20260913T081500Z.md`;**backlog:新开 `-164`**;
  ...
  **落地 `wkidleshare`(Wraith King,gated,turbo-only,纯收窄),并更正两处被推翻的已登记前提。零 AWS。**
  - ⭐ **一句价值论证整句都在讲比率,而它实现出来的代码里一个比率项都没有。**
    `wksaveidle` 的抬头论证是「等级 6–9 时储备是**池子的大半**(220 对 272–399),
    『永远扣住』≈『永远不放』」——**整句是一个比率**;而 `X.IsReincarnationReserveIdle`
    的释放条件只有血量 `>= 0.95` 与 1600u 内敌人数 `== 0`,**没有比率项也没有 rank/等级项**。
  - ⛔ **它过关是因为同一段给了自己一条假边界。**「rank 3 时 Reincarnation 免费 ⇒ 储备自己塌掉
    ⇒ 本 lever 只管 rank 1/2 那扇窗、不管后期」:KV 读数(220/110/0)对,**结论错**。
    R 的第三点是**两套 build 行的第 15 位**,而 `tests/test_skill_point_stall_frame.lua` 已把
    **GH #366 判成 (a)**(十个英雄 17–22 级全部停在 **13 加点 + 至多 1 天赋**,第 15 位是一堵墙)
    ⇒ **Reincarnation 永远 rank 1 或 2,储备常驻 220 或 110,从不自己塌掉**;
    「不管后期」是**那段文字的性质,不是代码的性质**。
  - **第二处更正压在一个数上**:`X.GetRoshanManaFloor` 抬头原写「rank 3 免费 ⇒ armed 地板 16 级后
    塌到 blast 自己的价钱」并列出「95..140 at rank 3」作为 armed 地板的**第三列**。
    那一列**到不了** ⇒ `wkrosh` 的 armed 地板只有**两个**可达值(315/330、235/250),后期永远是后者。
    方向不变(235 < 600 是它全部主张),变的是**读者不该期待地板掉到 blast 的价钱**。
  - **读数(全部驱动真 helper)**:33 计价帧上出货规则开火 **6/33**、`wksaveidle` 释放 **2/6**
    (`f_114311_drow_pushguard_silent` 比率 0.587、`f_260820_103216_cm_es_aftershock` 0.568)
    ⇒ ⛔ **加上本 id 之后释放集一帧不变,本语料域为 0**。
    ⭐ 6/33 与 2/6 是对宿主抬头自报数字的**独立复现**,且**不是只对上标题里那个 2** ——
    六个开火帧的血量与敌人数**逐帧**对上(证据纪律 4)。
    语料天花板**英雄 12 级**,仅有的两个 rank-2 帧(比率 0.240)恰好是出货规则**不开火**的帧
    ⇒ 域**整体落在语料之外**,取证走 `queue.json:hero-71`(**零 EC2**)。
  - **地板诚实边界**:比率在地板附近取 …0.467/0.467/0.492/0.492/0.492/0.506/0.506/0.506…
    ⇒ **(0.492, 0.506] 里任何地板选同一批帧**,宽 0.014,**语料分辨不了**。
    取 0.5 是**论证不是拟合**(「大半」=「至少一半」);§4 把这条带量出来并断言它仍然窄。
  - ⚠️ **变异台第一稿 6/8,两个洞都值得记**(最终 **8/8**,`tools/agent/mutstand_wkidleshare.sh`):
    (i) ⭐ **M7 存活的机制正是这个文件自己记过的那一条** —— §6 那条「被划掉的前提不许回来」
    直接匹配源码,而 M7 把那句话**跨两行注释**写回去,子串就看不见了。
    这就是 **GH #235** 在同一文件 `X.ShouldSaveMana` 抬头记下的形状。
    **修法是匹配前把 `---` 续行压平,不是改变异体。**
    ⇒ **凡是拿子串钉一句注释的断言,先把注释压平** —— 重写段落的人**自然**会换行。
    (源码侧同坑也踩了一次:PROMOTE RECIPE 第一稿也跨了行。)
    (ii) **M5(`>=` 翻 `<=`)红了但消息不对** —— §4 的棘轮**自己重算** `share >= floor`,
    于是无论 helper 怎么写它都为真。修法是让棘轮**读真 helper**。
    ⇒ **棘轮重算一遍被测量,就等于没在测。**
  - ⛔ **promote 这条 id 的动作与惯例相反**:gate **未 armed = 不收窄**(permissive),
    所以「把 id 从 armed 串里删掉」会**静默删掉行为**。
    **promote = 删掉那条 guard 语句、保留比率。** 这句话在源码里**写在一行上**并由 §6 钉住。
    这是 pullcad 陷阱的**反向**版本(pullcad:promote 冻死一个 gate;这里:promote 删掉一个行为)。
  - **闸这次拦住的是作者本人**:`tests/test_gated_helper_nesting_census.lua`(结构性普查)
    被新的 gate-套-gate 顶红,已登记为**第一条 WK 行**、分类 **(I)**(未 armed 时
...
- 2026-09-11T14:07Z(报告 `iterations/reports/hero/20260911T140701Z.md`;**backlog:新开 `-150`,
  ...
  **GH #390 要的那一帧不存在:三次「检测到 flip」全部是 harness 造出来的。**
  - **缺陷**:`tests/test_wk_q_castrange_meter_domain.lua` §6 用
    `f.target ~= a.target` 判定两条腿是否分开,而每条腿各做一次 `rf.load`,**每次 load 自己造一套
    unit handle** ⇒ 该项在**任何返回了目标的帧上恒为真**,只在两腿都返回 `nil` 时为假。
    它量的是**「这帧有没有目标」**,从来不是「两条腿选的目标一样不一样」。
    被它点名的三帧(`f_260909_215040_wk_blast_{lion_480,sb_1052,sb_661}`)正是 27 个 body 帧里
    返回非 nil 目标的**全部三帧**,三个世界的 last/desire/目标名**逐位相同**(都在 desire 0.75 的
    团战支路,`wkqdmg` 够不到的行)。**判别子**:同一条腿同样参数跑两次,
    一次 load 内 `t1==t2` 为 **true**,两次 load 间为 **false**(名字都是 lion)。
  - **改动**:`drive_legs(path)` —— **一次 load 跑四条腿**(zero / ship / armed / **再来一次 ship**),
    最后那条是控制组,**控制组断言排在 flip 断言之前**。本轮 `ctrl_breaks=0`、`flips=0`
    ⇒ **归档分不开两条腿,一帧都分不开**。
  - **#390 rec 2 的归档答案(新增 §9)**:Lua 短路 ⇒ `wk_GetBlastKillDamage` **只在距离门已通过时**
    才被调用,数它就把「够不着」和「打不死」分开了。**到达距离门 9 个 body 帧 / 通过并算出 claim
    12 个 (帧,敌人) 对 / claim 通过开火 0 对,两条腿都是 0** ⇒ **支路不缺射程,缺的是 claim。**
    最紧一对 `wk_blast_sb_661` 的 spirit_breaker **172 原始 hp vs 出货 claim 168.0,差 4 点**。
    ⚠️ **与 #390 的「63.6 ehp」不可比**(真发生的施法 vs 支路评估过的全部对;ehp vs 原始 hp)——
    **两者一致的只有那个 0,只有那个 0 被写成结论。**
  - **本地验证**:**9/0**;变异台 5 条 **4 杀 1 存活**,存活那条(M3,删掉把 KV 喂回 fed 腿那行)
    **有正当解释且已被 §1 的 `nFed == t.live - 3` 钉着**(loader 现在自己从 KV 供 `GetCastRange`,
    那行是 525 覆盖 525)。
  - **四条红全清并退出 known-red(13 → 12)**,成因**全部是语料增长**,逐条**重锚 + 写成因**:
    §1 存活 WK 实例 **38 → 51**(+13,按桶劈分 +4 COOLDOWN / +1 SAVEMANA / +8 BODY)、
    §4 cooldown **10 → 14**、savemana **4 → 5**、body **19 → 27**、§5 loop/gate **2 → 9**;
    **§5 `fire_fed` 没动,仍是 0 —— 那才是结论所在。**
  ...
  - **⚠️ 自检**:第一次调用又被它自己 REFUSED(管道,**第 7 次复发**);重定向后跑完,**worst exit 3**。
    两条 Lua 红均非本轮造成;容器无 `lua5.1`/`luacheck`,已装(**包名 `lua-check`**)。
...
- 2026-09-10T20:19Z(报告 `iterations/reports/hero/20260910T201905Z.md`;**backlog:`-143` 完成、
  ...
  **`lionqfight` 的域从 0 变成 4 —— 从「钉空操作」变成本地验证过的杠杆。**
  - **执行的是上一轮写进源码的那一棒**:拿支路自己的合取式(`nLV>=15` ∧ 可施放 ∧ 环内有人)
    回时间线反查,从 soak 局 `20260910_124853_slot1` 切出 **4 个真实施法帧**。
    11 个命中瞬间聚成 4 簇,**每簇末尾都是一次真实的 `lion_impale`**。
  - **读数**:42 活体 Lion → armed 恰好移动 **3** 帧,**每一帧都是变沉默**(兜底是最后一条 Impale 支路);
    `armed ⊆ shipped` 由 §3.4 驱动而非论证。豁免帧 `sb_1357`(SB 19% 血,Finger 先赢)**一字不改**。
  - **地面真相(混合,照实报)**:3 发被扣下的 Impale **全部命中**(205/199/201)——
    不是往空地扔;但两次控完满血 Slardar 后 Lion **10.5s / 4.7s 内死在同一个 Slardar 手上**。
    **三分之二是读数不是率**,promote 归波次。
  - **⭐ 顺带买到的尺子缺陷,两个实例**:普查的环写成 `GetCastRange()+20`(**670**),
    支路的环是 `+aetherRange+20`(**920**,每个 Lion 都带透镜)⇒ 新帧 **3/4 坐在环带里**;
    `test_lion_q_kill_reach` 更把一发 **861.99u** 的合法施法判成「outside 670」,
    而它的注释逐字写着「X.ConsiderQ's own arithmetic」。两处已修成逐帧的环。
    ⚠️ **但它没造出上一轮那个 0**:上一轮就有的 4 个 15 级帧两种环下答案相同(已钉断言)。
  - **暂存价钱:净增红 0,并退掉 2 条存量红**(`test_lion_q_kill_reach` 修到 12/0,
    棘轮 `known_red` 16→15)。⛔ **「第一条红处停手」当场复发**:zuus 那条注释写了
    「band 没动」,实际 **10→12**;cm 那个因此改成一次读到底的 **11 个数**。
  - **验证**:`GATE_EXIT=0`(0 warnings)、`py gate: 95 ran, 0 findings`、
...
- 2026-09-10T14:16Z(报告 `iterations/reports/hero/20260910T141637Z.md`;**backlog:`-141` 完成、
  ...
  **切出本仓第一批 Zeus 真实帧(8 个),并用它们落地 `zusarcexec`(Zeus,gated,turbo-only)。**
  - **为什么这两件是同一个工作单元**:Zeus 在本仓**一直是 0 帧**(已入集两局的 draft 里都没有
    `zuus`)⇒ 任何 Zeus 支路的域读数**只能**是 0。先切帧,处决支路的域**立刻从空变成非空**。
    这是 `hero-54`「那是关于**帧在哪里切**的事实,不是关于机器人的事实」(GH #702)在
    **第二个英雄**上的复现。
  - **杠杆**:`X.ConsiderQ` 第一条开火点只问 `J.GetHP(e) <= 0.2`,而它排在其余六条之上、不看
    mode、直接 HIGH。armed 追加**六行之下那条支路自己在用**的
    `J.WillKillTarget(e, nDamage, DAMAGE_TYPE_MAGICAL, nCastPoint)`,用的是**本支路算出来却从不读**
    的 `nDamage` —— 不发明阈值;出厂合取项逐字不动且先求值 ⇒ gate off 逐字节等价。
  - **域**:支路(算术)4→**3**(去掉 BKB 那个瞬间),armed 留 1 掐 2;
    函数(决策)**2 帧、全沉默、0 改瞄**。⭐ **两个高度必须分列** —— 被掐的帧落到下面六条支路,
    可能换目标照样出手。
  - **地面真相**:钉帧 `..._exec_wk_615` 上,同一局录像里 Zeus 对着一个 241 血的 WK
    **连开三发弧电**(实测每发 130 减伤后伤害),人头 619.9 归 pudge。
  - **⛔ 量具缺陷(已交 [harness])**:mock 的 `IsMagicImmune()` 在挂 BKB 的英雄上答 false
    ⇒ `J.CanCastOnNonMagicImmune` 在每个 fixture 帧上都不否人;**所有带这个合取项的杠杆,
    fixture 域读数都偏大。**
  - **⛔⛔ 空转测量**:`lua5.1 tests/<file>.lua` **一条断言都不执行、退出码 0**。本轮第一次
    量价钱就是这么量的(报 0 红)。**必须走 `tests/run_tests.lua`。**
  - **验证**:`bash tools/agent/luacheck_gate.sh` **exit 0 / 0 warnings**;新测试
    `test_zuus_arc_execute_kill.lua` **14/0**;付清的两条 **15/0** 与 **11/0**;
    `test_bots_walk_farm_only.py` **exit 0**(上一轮留下的红,本轮手读补上)。
    **动态半没有全绿也不可能全绿:trunk 自带 19 条红进本轮,19 条在干净 HEAD 上同样红。**
...
- 2026-09-10T05:03Z(报告 `iterations/reports/hero/20260910T050325Z.md`;**backlog:`-138` 做完、
  ...
  **P4.2 冻结期内不请求入集**)
  **落地 `cmqpoke`(Crystal Maiden,gated,turbo-only):`X.ConsiderQImpl` 的钱包支路。**
  - **缺陷**:`--非撤退的逻辑` 块(唯一的 mode 项是**否定式** `~= BOT_MODE_RETREAT`)里相邻两条
    支路要**同一个施法、同一个 subject、同一个 `J.GetCastLocation`**,只差一项 —— 上面问
    「我有多少蓝」,下面问「这一发买到什么」。上面那条是下面那条把质量测试换成钱包测试,
    **而且坐在上面,所以永远先赢**。与 `cmrcrowd`(GH #649)同族不同形:那条是同一个 `if` 里
    **短路**,这条根本没有可短路的东西。
  - **域(全语料,`tests/fixtures/` + `tests/frames/` 都扫)**:53 活体 → 28 可施放 →
    **18 满足钱包测试(比例项单独拍板 0、绝对项单独 6)** → 出货下单 **1** → armed **0**。
    **本仓第一条 armed 腿真的移动了语料能到达的决策的 CM lever。**
    钉帧 `f_260820_182906_lion_drain_survived`(11 级,543/831 蓝,三敌 625/750/885u,
    血 0.62/0.94/0.97,桌上无人头)。
  - **⛔ 花掉的坑**:出货析取式一旦搬进 helper(形参改名),`_cm_t10_payoff_sweep.lua`
    **按源码文本**解析的蓝量闸就从 8 掉到 7 —— 行为不变,但对「按文本读」的普查是删除。
    表达式已留在调用点逐字不动。**动 `bots/` 表达式前先问谁在按文本读它。**
  - **验证**:`cm_` 331 例 0 失败;新测试 5 例 0 失败;变异台 `mutstand_cmqpoke.sh` 12 发;
    luacheck gate 与 smoke 见报告 §5。全套(~100 分钟,GH #124)未跑完。
  - **⭐ 铁律 9 升级(第三轮点名)**:`hero-54`(帧供给,优先级 1)**仍未裁**。
    指名下一步:**总监裁 `hero-54`**;**批测台跑 `hero-2` 的归档扫描**(§BF.2 早已放行,零 EC2,
    口径数**穿越**不数带内帧,且**不要**拿 `tl_260905_010226_axe_outchan.json` 代答 —— 18 秒 / 无技能字段)。
  - ⛔ **自检的 python trunk-health 腿本轮没有输出、退出码未观测** ⇒ 那一侧这轮没人看过(非通过)。
- 2026-09-10T01:48Z(报告 `iterations/reports/hero/20260910T014840Z.md`;**backlog:`-137` 做完、
  新开 `-138`**;跨五英雄的队列行;OWNER_PRIORITIES **P4.2 冻结期内不请求入集** ——
  主体是**一把仪器 + 一次数据修复**,`bots/` **零改动**,理由见下)
  **`-137` 要的「第四份没人消费的域读数」不存在,而找不到它的原因是判据本身:**
  - **(1) `hero-37`(上一轮刚花掉)在候选表里** —— 它**完全没有 `status` 键**,而判据写的是
    `status != done`;缺席被读成「不是 done」⇒ 花过的钱与没花的钱**逐字节同形**,
    这条判据**每轮都会重新选中它**。全队 101 行里恰好三行缺 `status`,**三行全是 `hero-*`**
    (`hero-37`/`hero-39`/`hero-54`),已分别补 `delivered-and-consumed` / `pending` / `pending`
    (照抄既有词表,**不造新词**,GH #317)。
  - **(2) 剩下四行是一份读数,不是四份** —— `hero-23/24/25/29` 共享同一段 W53 家族读数
    (51-id 臂串,gpm −6.80,`comps_better 1/3`,段落自己写着**条件 (a) 仍欠录像组**),
    按构造给**整串**定价、不给其中任何 id 定价。按行数点它 = 铁律 4(i-a) 的行版
    (报行数,而问题要读数)。⚠️ **整段哈希看不见**(两行带不同的头),只有按 `||` 切段看得见。
  - **⭐ 一般规则(带走):拿到一张按某字段筛出来的候选表,先问那个字段**缺席**时读成什么。**
    与 `-136`「抬头的前提没进 `if`」、`-137`「这一项与上一行共线」同族:
    **筛子的失效方向不在它筛掉的行里,在它没写出来的那一格里。**
  - **仪器**:`tools/agent/queue_reading_census.py`(0 干净 / 2 没跑成 / 3 有发现;
    **唯一升 3 的是缺 `status`**)+ `tests/test_queue_reading_census.py`(**16 例 0 失败**,
    其中 `2b` 是负向断言:整段哈希会漏掉那四行)+ `tools/agent/mutstand_queue_reading_census.sh`
    (**10/10 CAUGHT**)。真实树修复后 **EXIT=0**。
  - **顺带登记两行的陈旧根因**:`hero-3`/`hero-4` 把域上界归给 `SOAK_CAP_MIN=10` 的裁判上限,
    而那道上限已被 **P3 / GH #108** 取下(本仓 2026-08-27 / GH #235 就据此改过五个英雄文件里
    所有引 GH #84 等级普查的段落;旁证:`hero-23` 的 W25/W26 并池读到 WK 局终等级 **100% ≥13**)。
    ⛔ **登记只说「买不到」的理由不成立,不说「现在买得到」。**
  - **`bots/` 为什么零改动**:两个形状对的候选**当场判死在语料上**——(甲) Lion 补蓝支路
    「1600 选靶、按施法单花掉」验不了(语料 `GetNearbyCreeps` 恒 `{}`,GH #658/#581);
    (乙) Lion W/E 两对死局部零行为。**瓶颈是帧供给 `hero-54`(优先级 1,仍未裁)。**
  - **验证**:`luacheck_gate.sh` **GATE_EXIT=0 CLEAN**(本轮 `bots`/`game` 零 diff,是回归确认);
    全量 `run_tests.lua`(~100min)**本轮没跑完 —— 「全量绿」本轮没有人说**;
    自检 **~30 分钟**,**python 腿 9 项 UNCERTIFIABLE(那一侧这轮没人看过)**。
- 2026-09-09T23:08Z(报告 `iterations/reports/hero/20260909T230834Z.md`;**backlog:`-136` 做完、
  新开 `-137`**;焦点英雄 **Zeus**;OWNER_PRIORITIES **P4.4 (i)** —— 主体是一个 `bots/` 行为改动)
  **花掉 `queue.json:hero-37`(`zusultstrand` 域读数,录像组 2026-09-07 交付;`hero-51` 只消费了
  其中的创建帧那一格,第 (3) 列三天无人动过一行代码)。`X.nUltCashChaseRadius` **1600 → 700**。**
  - **读数证伪的是那个环本身**:1600 在这条分支自己的域上吃进 **1,645/1,843 帧(89.3%)**、
    **252/257 episode(98.1%)** ⇒ helper 抬头逐字的
    「NARROWED so the widening is not a blank cheque」**在它被写下的那个值上是假的**。
  - **⭐⭐ 本轮那一课(它把 `-136` 的一般结论推进了一层,拿到任何比例读数先算这一句):**
    **`-136` 说「先核对抬头写的前提有没有落在代码里」;本轮的前提落在了代码里,坏的是它
    与上一行共线。** 外层合取项是 `bot:WasRecentlyDamagedByAnyHero( 2.0 )`,而 2 秒内打过你
    的英雄必然进过它自己的攻击距离 ⇒「1600 内有敌人」几乎被上一行蕴含。
    **⭐ 共线的指纹是可算的,而且它已经印在读数上了**:切掉 **10.7% 的帧**却只切掉
    **1.9% 的 episode** ⇒ 它删的是**同一个 episode 里还有别的帧被放行**的那些帧;
    而这条分支**只开火一次**(HIGH desire ⇒ 大招出手)⇒ **按 episode 不动 = 一个决策都没改。**
    ⇒ **一般规则:拿到一个「过滤器吃进 N%」的读数,先问它在决策尺度上吃进多少;
    两个尺度的裂口不是精度问题,是「这一项与它上一行共线」的读数。**(铁律 4(iii) 的现场。)
  - **收窄到 700 用的是本仓自己量过的环,不是一个新数**:`J.CanEnemyInterruptTpChannel`
    未 armed 搜 **R=700**;`tools/batch_test/behavioral/tpreach_domain.py` 从语料里**量**出
    同一条界(reach>700 需要 `GetAttackRange()>550`,即远程英雄)。
  - **⛔ 故意没用更锋利的 `hEnemy:GetAttackRange()+150`**:`tests/mock/` 下没有任何东西从帧
    数据装这个 getter(dump 没有这一列),loader 会用通配默认 **150** 回答 ⇒ 该项在每个真实
    帧上读成「距离 ≤ 300」**同时长得像在读引擎**。那是 **GH #656** 编目的那一族。
    **常数在这里是诚实的,getter 不是。**
  - **真实帧(9 帧零注入)**:armed 在 1600 放行 **7/9**、700 放行 **6/9**;唯一移动的是
    `f_073148_zuus_lina`(最近敌人 **979.8u**,池子里没人够得着);
    **创建帧 `f_20260827_091703_slot12_zuus_473_1`(Slardar 304.9u)未被挡下。**
  - **⭐ 顺带量到一件改别人读数的结构事实**:1600 时 chase 环**包含** `ultcash` 的 1200 环,
    700 时**严格落在 1200 内** ⇒ 几何那半的重叠**按构造变成全覆盖**。`hero-37` 第 (4) 列的
    PARTIAL(1200 覆盖 80.5% 帧 / 96.1% ep)是**旧嵌套**下的,**不许原样搬**;
    检测器已加断言 `chase_ring_is_inside_the_ultcash_ring`。
  - **仪器随源码同 commit 移动**(`test_detector_source_constants.py` 的镜像断言强制),并修掉
    **把常数写成字面量**的四处(`_base_tl` 默认 800、三条边界 1601/1600/1400、幻象检查的 900
    —— **实测就是它:第一次跑 75 PASS / 1 FAIL**),输出键 `*_1600` → `*_chase`。
    `--selfcheck` **76 PASS / 0 FAIL**。
  - **测试 15 → 18 例(新 §7);变异台 11 → 13 发,13/13 CAUGHT**,新增两发**都锚在收窄
    MOVED 过的读数上**(M12 回滚到 1600;M13 把唯一有差的那一帧从普查表里剪走 —— 那是
    「只写 `nNew < nOld` 不写精确计数」会静默放过的形状)。
...
- 2026-09-09T21:16Z(报告 `iterations/reports/hero/20260909T211626Z.md`;**backlog:`-135` 做完、
  新开 `-136`**;焦点英雄 **Wraith King**;OWNER_PRIORITIES **P4.4 (i)** —— 主体是一个 `bots/` 行为改动)
  **花掉 `queue.json:hero-31`(`wkbonefight` 域读数,录像组 2026-09-06 交付、三天无人消费)。
  读数**没有**证伪这根杠杆(域没空,退回门不触发)—— 它证伪的是**杠杆量的那个圈**。**
  - **读数(录像组量的)**:W 就绪 **112,933** 帧;1600 内敌人数 0/1/≥2 = **65.6%/14.5%/19.9%**;
    就绪+650 内有敌人两层不并池:`n1600==1` 5,093 帧/1,365 ep,**`n1600>=2` 12,656 帧/2,244 ep/118 局全覆盖**
    ⇒ **`DOMAIN-NOT-REACHED` 退回门不触发**;(4) 充能层数**买不到,未用 0 顶替**(INSTRUMENT-BLIND)。
  - **诊断**:armed 腿写 `nEnemies >= 1`,而 `nEnemies` 是 **1600** 视野环人数;可这条杠杆的支配性论证
    (helper 抬头逐字的「a release that joins a **3v3** buys strictly more **hero-facing**
    attack-seconds than one that joins a 1v1」)是一句关于**40s 近战骷髅够得着的英雄**的话,
    **它从来没进过 `if`** ⇒ 1550 开外、正往反方向走的敌人也算进了那个「3v3」。
  - **改动(无新 id)**:新 `X.wk_CountBoneGuardEngaged( tEnemies, nRadius )`;
    `X.IsBoneGuardEnemyCountOk( nEnemies )` → `( nEnemies, tEnemies, nRadius )`,armed 腿 =
    `nEnemies == 1 or X.wk_CountBoneGuardEngaged( tEnemies, nRadius or 650 ) >= 2`;
    `X.ConsiderW` 新增 `local nEngageRange = 650`,**计数环与够得着环从此读同一个数**。
    合取项**只能从 armed 分支内部到达**,gate 关时逐字节仍是出货行为 ⇒ **P4.2 冻结在这里没东西可冻**。
    **方向由构造保证仍成立**(出货判据是字面析取项 ⇒ armed 仍是超集);
    **缺表默认 RESTRICTIVE 不是放行**(放行会让忘传参的调用方静默恢复 1600 那条腿,M4)。
  - **真实帧**:13 个 WK fixture,出货 **2/13**、**旧** armed **9/13**、收窄后 **6/13**;
    丢掉的三帧点名 —— `laning_safe`(2 in view / **1** in reach)、`reincarn_gap`(2/**1**)、
    **`sven_burst`(2 in view / **0** in reach,最干净的病例)**。分支几何内(650 内有人的 7 帧):
    出货 1/7 → armed **5/7**,**仍是出货的五倍**。engaged 直方图 0→6/1→3/2→4,逐帧断言 `engaged <= view`。
  - **测试 17 → 21 例;新变异台 `mutstand_wkbonefight_ring.sh` 10 发 10/10 CAUGHT。**
    ⛔⛔ `-135` 那个坑本轮**正面撞上**:§2/§3/§4 原来**都不传表**,收窄后会落到限制性回退
    ⇒ 无论 gate 开不开都返回出货答案,**对照什么也证明不了**而「全绿」看不见。
    §4 新增 `opener_table()`(**先断言这张表真能打开 armed 腿**),§3 改扫 **(count,table) 对**(143 对/widening 40)。
  - **⭐ 孤儿没有重演**:新谓词是被**调用**的不是只当函数值传 ⇒ `test_hero_export_reachability.py`
    天花板 **34 未动**。
  - `luacheck_gate.sh` **EXIT=0 CLEAN(0 警告)**,未用 `RULE6_BYPASS`;`wk_` **293** / `focus` 92 /
    `smoke_load` 3 / `gate_claim` 16 全 0 失败;四个 python 普查全 EXIT=0。
  - **⛔⛔ 自检**:`-133` 的两条禁令**分两次各犯一条**(先管道 REFUSED exit 2,再 `timeout 300` → EXIT=124,
    实测要 13 分钟),第三次才跑对 ⇒ 真实 **EXIT=3**。**其中的 `trunk-red(python)` 是我自己造的,
    而且成因不是 `-134` 记的「争 CPU」,是**争一个共享文件****:自检的 python 腿与我并发的
    `run_tests.lua` **都写** `bots/Customize/soak_side.lua`(gate 开关)⇒ `test_lua_corpus_stability.py`
    报红,**单独重跑 EXIT=0**。**后果比争 CPU 严重:那是一个像「主干坏了」的假阳性。**
    规矩改范围:**自检跑着时不要跑任何 `tests/` 套件**。
  - **⛔ 新 gated id 0**,armed 串 / `test_set.md` **一字未动**;`state.json` 新条目 1
    (`wkbonefight_ring_20260909`);`queue.json:hero-31` → `delivered-and-consumed` + `next`
    (**下一棒仍在总监,但要裁的东西改小了**:不是「12,656 值不值得入集」,而是**要多一列 (3′)** ——
    650 环内敌方英雄数分布,1 与 ≥2 分开不并池,零 EC2 可并进 hero-2/30..49 那次遍历);
    **零 AWS / 零 EC2 / 零 S3 / 零 CE**。
- 2026-09-09T17:10Z(报告 `iterations/reports/hero/20260909T171044Z.md`;**backlog:`-134` 做完、
  新开 `-135`**;焦点英雄 **Axe**;OWNER_PRIORITIES **P4.4 (i)** —— 主体是一个 `bots/` 行为改动)
  **把 GH #562(`hero-35` 的域读数,交付三天没人消费)花掉:它证伪的不是 `axebhrecast`,
  是它的写法 —— helper 抬头逐字写着的支配性前提「另有候选时重放才被支配」**从来没进过 `if`**。**
  - **读数(录像组量的,不是本轮量的)**:门内 **273** episode,**纯成本 121**(目标活着 + **0** 候选
    ⇒ armed 只是放弃)/ **纯收益 30** / 击杀确认 122;价值列 0 候选 **81.3%** ⇒ **成本 : 收益 ≈ 4 : 1**。
    第 (4) 列 INSTRUMENT-BLIND ⇒ 273 只是三个接线位点域的**上界**。
  - **改动**:`X.axe_IsBattleHungerFresh( hTarget )` → `( hTarget, tCandidates, fExtra )`,armed 腿新增合取项
    `X.axe_HasHungerAlternative`(**无新 id** —— 只能从 armed 分支内部到达,gate 关时结构上不可达);
    新 `X.axe_IsHungerHarassIdle`(把对线支自己的 `GetAttackTarget() == nil` 抬成具名谓词,
    好让候选测试用的**就是那个站点自己的谓词**);三处接线各传**自己遍历的那张表**。
    **接线点仍 3 个,五处字面否决未删,gate 关时逐字节仍是出货行为。收窄 ⇒ armed 接受集变大但仍 ⊆ shipped。**
  - **⚠️ 一处过近似,登记不辩掉**:团战那处的候选测试**故意不带** `axe_IsHungerFightTargetInReach`
    —— 那一项挂着**另一个 id**(`axebhreach`),依赖它的 armed 状态就是 **pullcad 陷阱**;
    残差恰好是 **+200 那条带**。另两处精确。
  - **⭐⭐ §3 的 e2e 断言被翻转,那是产物不是削弱**:同一帧上 armed 原来「一条指令都不发」,
    现在与 shipped **逐字同一条指令** —— 那次 decline 就是 121 个纯成本 episode 的一个实例
    (§1 早就量过:800u 内只有中招的 WK 一个敌人)。**否决本体**改钉在 ring 帧
    (带中招的 skywrath_mage + 未中招敌人),armed 在那里**必须仍然拒绝**,否则「收窄」=「杀死」。
  - **⛔⛔ 三发变异会因为正确的修复变成 SURVIVED**(M2 去 turbo / M3 死接线 / M7 去魔晶前提,
    原来都钉在 flee 帧 e2e 上,而那一帧现在两条腿都站下);§2 出货普查与 §5 no-op 对照原来
    **不传表**、收窄后会**空过**。已全部重新上锚:台子 **8 → 11 发,11/11 CAUGHT**
    (新 M9 = 去掉前提合取项 = 回到 4:1 那版;M10 = 缺表默认翻成放行;M11 = 另一个中招者算成候选)。
  - **⭐ push 门抓到一件真事(不是 flake)**:`test_hero_export_reachability.py` 报
    `X.axe_IsHungerHarassIdle` 是**第 35 个孤儿**(天花板 34)。它是对的 —— 我把它**只当函数值**
    传进 `fExtra`,而站点仍写着字面 `GetAttackTarget() == nil` ⇒ 那是 `bots/BotLib/` 下
    **第一个函数值别名**,闭包看不见。**处置照它自己 LIMITS 写的做:改代码不改量具**
    (「the fix is here, not in the ceiling」)—— 让站点**直接调用**它,无门、行为逐字节相同,
    副产物是那个谓词只写一遍。**天花板仍 34,量具一字未改**;新断言钉「ConsiderW 里必须出现 2 次
    (调用 + 传值)」。⇒ **收窄一条杠杆时新增的具名谓词,只传不调 = 制造孤儿。**
  - 测试 `test_axe_battle_hunger_recast.lua` **20 → 28 例**;`luacheck_gate.sh` **EXIT=0 CLEAN(0 警告)**,
    没用 `RULE6_BYPASS`;分组跑 `axe` **268** / `cm_` 326 / `zuus` 236 / `wk_` 287 / `lion` 222 /
    `replay_` 622 / `focus` 92 等**全 0 失败**;四个相关 python 测试全绿。
  - **自检**:第一条命令**踩了管道**(REFUSED,exit 2,什么都没检查),改成重定向重跑 → **真实 EXIT=3**
    (cadence owed-executions),trunk 快检测器 **87 文件 0 失败**;python 腿 **9 项 UNCERTIFIABLE** ——
    **本轮不是抢 CPU**(`-134` 的坑避开了),是 5a0 的 **120s 预算**跑不完 **87 个文件**的发现集。
  - **⛔ 新 gated id 0,armed 串 / `test_set.md` 一字未动**;`state.json` 新条目 1
    (`axebhrecast_premise_20260909`);`queue.json:hero-35` 改成 `delivered-and-consumed` + `next`
    (下一棒 = 语料侧钉 `a57e1e__20260902_034511_slot4` t=1191.5 那枚**纯收益**帧,⚠️ 视野未核);
    **零 AWS / 零 EC2 / 零 S3 / 零 CE**。
...
- 2026-09-08T20:20Z(报告 `iterations/reports/hero/20260908T202000Z.md`;**backlog:`-127` 做完、
  ...
  `bots/` 行为改动)
  **`X.ConsiderW` 的团战落单点不是一次过长的出价,是一条「搜索集比自己射程宽」的选择规则。
  gated `axebhreach` 把到达项补上,`bots/` 有真代码行。**
  八个落单点里六个把目标绑在 `nCastRange` 上(击杀/对线/撤退读 `nInRangeEnemyList`;
  先手/肉山/Tormentor 直接 `J.IsInRange( ..., nCastRange )`)。团战那个是**唯一**一个遍历
  ...
  **0 红**;census/registry 族 python 检测器 15 个全 0;`test_bots_walk_farm_only.py` 8 checks 0 failed。
  ...
  - **⭐ 承重的是「选择规则 vs 首次命中」**:另外五个英雄落单点都是首次命中循环(错的是一次
    ...
    **把同一个循环刚认证为合法的射程内候选挤掉**。**错的是一次交换,不只是一次拉长。**
    而且被挤掉的那个**下面没人接得住**(闭式:对线/撤退读 `nInRangeEnemyList` 且施加同一组
    四条过滤 + 各自额外合取 ⇒ armed 子集为空时它们也开不了火)。
  - **⭐⭐ 端到端那半零前提注入,而且断言了「不是空话」**:33 个 Axe 活帧里 band **6** /
    射程内 **15** / 前提成立 **6**,**交集 0**;那 6 帧只放开冷却地端到端跑 `X.SkillsComplement`,
    **4 帧真的开火**、**那 4 帧选中的目标全在 `nCastRange` 内**、**arm 后 6 帧动作逐位不变**。
    `nCast == 4` 是**故意加的**断言:少了它,「arm 后什么都没变」对本来就什么都没发生的帧
    同样成立。**「惰性」与「死接线」就是这样分开的,不是靠更多 gate 测试(M4 全过)。**
  - **⭐⭐⭐ 变异台 M10 是本轮的看门狗**:同一个 helper / id / 闸,从**循环过滤**挪到**选出的
    赢家**上 —— 全部 §5 断言与 gate 测试照过,note 一字不改,而 lever 从「换一个够得着的目标」
    变成「这一帧不打了」,**是另一条 lever 穿着这一条的名字和它整个 (c) 论证**。只有 §3.2 看得见。
  - **⚠️ 第一版 §4 断言「0 帧开火」当场红了,而红出来的是好消息**(真实供给 4 帧);第一版 §1
    的普查数也红了,因为我用**原始 fixture 坐标**估而 mock 按**可见性**过滤。**先跑再写数。**
  - **⚠️ 开工自检 EXIT=3(有发现,不是通过)**:cadence / owed-executions,都不是本轮的;另有
    UNCERTIFIABLE(`test_selfcheck_lua_leg.py` 等 120s 没跑完,与前两轮逐字同一条)。自检
    **第 12 次**用管道读退出码被拒,**又是本轮第一条命令**,连续两轮同一条。
  - ✅ **「自检与变异台并行」(GH #507)连续三轮没有复发。**
...
- 2026-09-07T20:05Z(报告 `iterations/reports/hero/20260907T200530Z.md`;**backlog:`-119` 挂起、
  ...
  **零 arm、零入集提议**(P4.2 冻结)。AWS **只做了只读身份自举**(为回答 `-119` 的可行性),
  ...
  ⇒ 走工作流第 1 步回落分支。
  - **⭐ 缺陷**:冻兵块两半各从自己的列表挑兵(近 `nCastRange+100` / 广 `1400`)。
    「再近」那半三个项全读 `Health1`、打 `Creeps1`,**自洽**;「先远」那半打 `Creeps2`、
    460 下界与 `<= nCreepCap` 都读 `Health2`,**唯独带蓝放宽的下界读 `Health1`** ——
    两半里**唯一**一处「拿 A 兵的血量批准砍 B 兵」。
  - **⭐⭐ 咬得最狠的地方恰好是这半存在的理由**:`X.cm_GetStrongestUnit` 的滚动最大值
    初始化成 `GetBot():GetAttackDamage()` 并在无合格单位时**原样返回** ⇒ 近列表为空时
    `Health1` **不是 0、也不是远兵血量**,是 CM 的攻击力(远程基础 39-45,两张购买表都不买
    伤害装)⇒ **远兵是唯一候选的那一刻,放宽下界恒假**,`nMP > 0.45` 什么也买不到。
    ⚠️ **这是属性表+购买表的算术,不是帧读数,两句不许并成一句**:`GetAttackDamage`
    每帧答 0(量具限制),§4 把这个 0 钉成单向绊线并**拒绝**读成「实战里回退值是 0」;
    真实帧上钉的是**回退值的身份**(变异台 M10 就是把初始化式改成 0)。
  - **⭐⭐⭐ 方向由构造保证,不靠单调性**:出货腿**先算、自己 return**,armed 腿只在它答
    false 后可达 ⇒ 对**任意**输入接受集都是严格超集,**没有一句依赖 `Health1 <= Health2`**
    (`cmrangedhp` armed 时那条单调性本来就会破)⇒ 本杠杆方向**与那条杠杆无关**,
    `pullcad` 的教训用在算术上。WIDENING:负读数**永远不能**读成「取消了施法/换了目标」。
  - **⚠️ 变异台 M5 是「把改动无闸发货」那一形**:调用点交换前两个实参,出货腿就去读远兵,
    而所有 gate-off / turbo-only / 方向断言**照样全绿** —— 只有 §1 的**实参顺序**断言看得见。
    一个杠杆的全部内容若是「哪条腿读哪个量」,实参顺序就是它的正文。
  - **⚠️ 顺带改了兄弟文件的刮取器,不是降计数**:`test_cm_ranged_creep_health.lua` 的
    `consumer_floors()` 按内联正则数「四个下界」,390 搬进 helper 实参后**基线当场红**
    (棘轮先于我发现)。修法是让它**也读那个实参**,**计数仍是 4**;降成 3 会让 §3 去认证
    一个文件已经没有的形状,并让 390(500 最险险越过的那个)悄悄退出方向论证。
    `cmrangedhp` 的逻辑/gate/id **一个字节没碰**。
  - **⚠️ `-119` 本轮没做,理由两条(报告 §7)**:P4.4 不允许语料/量具类当主体;且它要的
    timeline **树里没有**(S3 只存 `.dem` + `analysis.json`),供给归录像组。已挂起并开 `-120`。
  - **开工自检 EXIT=3**:trunk RED 是 python `tests/test_stale_waits.py`(本轮零 python 改动,
    与本改动无关);另 2 条 python 腿 + 自检自测 4 条 `UNCERTIFIABLE`(那几格这轮没人看过);
    **fast Lua 检测器这一腿干净**。⚠️ 第一次调用照旧被 `REFUSED: stdout is a pipe` 拒绝执行
    (证据纪律 3,第 5 次复发)—— **那一次什么都没检查**,已改写文件重跑。
    **本轮没有再踩 GH #507**:变异台在自检退出、`lua5.1` 进程归零之后才跑。
- 2026-09-07T16:55Z(报告 `iterations/reports/hero/20260907T165504Z.md`;**backlog `-118`**
  —— 由上一轮 `-117` 直接交出的下一棒;焦点英雄 **Wraith King**;
  OWNER_PRIORITIES **P4.4 (ii)** —— 工作单元主体是一个判定完结所需的最后一块证据)
  **census §1 与 `wkreinctr` 那两个「矛盾」读数是同两帧的两种读法,census 的读法对。
  `bots/` 零改动。** 新 `tests/test_wk_rank0_absence_join.lua`(**12 例**)+
  `tools/agent/mutstand_wkrank0join.sh`(**10/10 CAUGHT**)。
  `queue.json:hero-41` 就地补判读结论(**没有另开请求**)。
  **零 arm、零入集提议**(P4.2 冻结)。AWS **一次都没碰**。
  本轮 [hero] open issue **一条可认领的都没有**(#599 是上一轮自己开的、#570 待总监裁、
  #593 量具类、#577/#587/#566/#564/#560 均已 gated 落地)⇒ 走工作流第 1 步回落分支。
  - **⭐ 减法先做完:两个「2」是同两帧** —— `f_073148_zuus_lina` 与 `f_080225_wk_revive`。
    断言写成**集合相等**不是计数相等(计数正是立案时那个答不了问题的量具,铁律 4 (ii) 同族)。
    顺带钉住:前者**就是 `test_wkreinctr_untrained.lua` 自己的 PIN 帧**。
  - **⭐⭐ 哪一种读法对,答案在 loader 里不在数里**:`tests/mock/replay_fixture.lua:725`
    把每一个 ability reader 装在 `ipairs(u.abilities or {})` **里面** ⇒ 没有 abilities 表
    的 unit 让循环跑 0 次,句柄上**什么都没装**,四次读全部落到 `bot_api` 通配
    (`^Get`→0 :184 / `^Is`→false :167),**与一个真没点的大招逐位相同**。
    对照例:在**计价**帧上读一个树里没有的技能名,四个答案**逐位相同**。
    ⇒ **那 3 帧的 rank 0 是缺席,不是 0。**
  - **⭐⭐⭐ 判读的代价,也是本轮真正的交付物:14 个 flip 拆成 12 + 2**。
    12 个是**计价**帧(rank 0 是真实记录),英雄等级全是 **1..5** —— 等级 6 之前
    Reincarnation **根本不能点**,那不是缺陷是规则,而调用点自带的
    `bot:GetLevel() >= 6` **把这 12 个全拒了**;唯二越过它的正是两个**缺席**帧。
    ⇒ **这份语料里,这条杠杆存在的那个形状(等级≥6 且大招点真没花)一帧都没有。**
    出货注(`jmz_func.lua` honest bounds (2))把调用点的 0 **全部**归给
    `J.IsInTeamFight` 恒假,并称那两帧是「**real replay frames, not a constructed
    case**」—— 前半句真但**不是唯一障碍**(强行置真域也只有 2,且都是缺席),
    「real」的是**英雄等级 7**,不是「no point in the ultimate」。
  - **第三条读数也对上了**:census §6 的「计价 ∧ 等级≥6 ∧ rank 0 = 0」本轮
    **从 raw rank 重算**(不走 loader)⇒ 两个文件不可能读的是同一个人造物。
    另加 raw↔driven 一致性一例,否则 `raw_rank == nil` 不再等于「loader 无米下锅」。
    全称断言代替 `== 12`(GH #106/#127),它变红那天正是语料终于拿到这条杠杆要的那一帧。
  - **⛔ 没碰 `wkreinctr`、它的逻辑、它的测试、`bots/` 一个字节** —— 那是协同组落地的
    id(GH #582),本工作单元交付读数不交付修复(`-118` 原话)。修法(重切三个 v1
    fixture)连同**跨文件重新基线的代价**已作为 **`-119`** 交出。
  - **⚠️ M9 是自己招的**:两个空集合**相等** ⇒ `A ≡ B` 在什么都没量到时照样通过;
    反真空 `#A > 0` 是唯一挡住它的。M2/M6 **行为零移动**、只被源码断言看见,
    写进台子头注而不是藏起来。**过滤器收窄到本文件一个**(与 `wk_` 那几个台子相反):
    三个变异改的是**共享 mock**,宽过滤器下会在本文件断言从没看过一眼时记成 caught。
  - **⚠️ 本轮自曝四条操作事实(报告 §9)**:(0) **又踩了一次 GH #507 并发撕裂窗口** ——
    变异台在开工自检跑完前就改写 `tests/mock/*` 与 `bots/*`;`-114`/`-115` 已各写过一遍
    「下轮记住」,这是**第三轮同形** ⇒ 第一份自检读数作废、已 `pkill` 并在安静树上重跑,
    且把「这条该进总监的规则表」写进 `-119`。(1) 第一次自检**用管道读退出码**,
    脚本自己**拒绝执行**并打 `REFUSED: stdout is a pipe`(证据纪律 3,它说第 5 次复发)
    —— **那一次什么都没检查**。(2) 我写的 §6 自检断言**用明文搜自己的源码**,
    而那个搜索参数**把针放进了草堆**,第一次运行就对自己开火报了一条不存在的 armed leg;
    已改成模式读法并把出错经过写进文件注释。(3) **mock 覆盖全局 `print`** ⇒
    第一版探针**输出零行、退出码 0**,与 GH #601 那条不同源但同形(一个 0 退出码 +
    零输出,读起来像通过);存 `local PRINT = print` 才拿得到输出。
  - **⛔ 并发那条病本轮发作三次,第三次是我的「修法」自己造成的(报告 §9 (2c))**:
    改用「等自检退出 → 删残留 → 再跑套件」的串行脚本提交后台,**harness 把换行压成一行**,
    `until … done` 后面的命令全被吞进同一行 ⇒ 等待没生效,套件与自检第三次抢
    `soak_side.lua`。**处置:两边杀掉、不追污染读数。**⇒ 本轮
    **全量套件「未认证」(既非通过也非红,无终值无退出码)**;
    **开工自检只有部分腿可引用**(python 111 passed/0 failed/1 uncertifiable;
    Lua 检测器 RED = `pgchannel` 族 **GH #607**,协同组同日独立记过;**都不是本轮的**),
    整轮没跑完 ⇒ 按铁律 10 登记 `UNCERTIFIABLE`,**这轮 trunk 那一侧没人完整看过**。
    可引用的动态读数是 **`wk_` 族 277 例 0 失败,安静树上复跑两次**(本轮改动覆盖面
    全在其中)。**三次里有两次发生在我「记住要串行」之后 ⇒ 靠人记住不成立**,
    需要机制(锁文件 / 脚本自拒并发 / `soak_side.lua` 走 per-pid 路径),
    已带着具体失败形状写进 `-119` 交给总监。
- 2026-09-07T10:49Z(报告 `iterations/reports/hero/20260907T104945Z.md`;**backlog `-115`**
  —— 由上一轮 `-114` 直接交出的下一棒;焦点英雄 **Lion**;
  OWNER_PRIORITIES **P4.4 (ii)** —— 工作单元主体是一个判定完结所需的最后一块证据)
  **`X.ConsiderE` 第一处早退量出来了,判 NOT TAKEN,`bots/` 只动一处注释。**
  新 `tests/test_lion_considere_earlyreturn_domain.lua`(**13 例**)+
  `tools/agent/mutstand_lionearlyreturn.sh`(**10/10 CAUGHT**)。
  `queue.json:hero-40` **加第五列 + 修一处前提**(**没有另开请求**)。
  **零 arm、零入集提议**(P4.2 冻结)。AWS **一次都没碰**。
  - **⭐ 读数:那道早退是整个 E 下半段的主约束** —— 27 个活体 Lion 瞬间里 E 可施放 **20**,
    其中 **17(85%)** 被它否决,只有 **3** 落穿。
  - **⭐⭐ 两半拆开,方向与 `-115` 防的陷阱相反**:边际否决
    `IsOtherAbilityFullyCastable()` **15/20** 对 `nSkillLV <= 1` **1/20** ——
    等级那半近乎冗余。陷阱(「别把『E 很少放』读成『冷却卡住了』」)**问对了**,
    答案是 castability 那半确实在做工,15 比 1。供给它的是**基础技能不是大招**:
    Q 12/20、W 11/20、R **5/20** —— `[liondrain]` 注对称点名三个技能,它们不对称。
  - **⭐⭐⭐ `liondrainmi` 拿到 11% 域上界**:打架抽蓝在这行**下游**(字节偏移断言),
    域 ≤ **3/27**,真实值严格更小(中间三个合取项各自写成断言)。
    **⛔ 天花板不是 rate**:那 3 帧**全部 3/3** 是历轮因为抽蓝有意思才切的 ——
    按被测结果全选样,作为 rate 向上偏到不能再偏。归档在原理上答不了 hero-40 (1) 列。
  - **判 NOT TAKEN,而且是读数确认了设计不是推翻它**:三个落穿瞬间**全部**恰是
    `[liondrain]` 注点名的状态(Q/W/R 一个都放不出来,逐帧断言)。缺席断言两条,对应 M6/M3。
  - **⚠️ 变异台逼出两个真缺陷(第一轮 5/10,查完是我的测试错不是变异错)**:
    (i) 文件头 t10 注**逐字引用**了那行早退 ⇒ 所有「调用点在下游」的偏移断言其实在跟**注释**
    比大小,上提调用点照样通过(M4);(ii) 检测「有没有人上闸」的断言按**原文**定位 ⇒
    上了闸文本一变,**它在唯一该响的变异上消失**(M6)。都已改成结构定位并断言唯一性。
  - **⚠️ 修了一处仓库现存的归因错误,但只在自己文件里修 + 加警戒线**:
    `GetNearbyCreeps` 恒为 0 **不是**「dumper schema 没有 creep 通道」,而是
  ...
  - **⚠️ 本轮自曝三条操作事实**:(0) **全量套件也没跑完:`FULL_EXIT=124`**(我自己的
    ...
    **都不是本轮的**(本轮没加任何帧),与 replay-check 09:49Z 记的 GH #594 同族。
    (2) 变异台**没有**与自检/全量套件并发(上一轮自曝的 GH #507 撕裂窗口):
    自检在后台跑期间只写 `tests/` 与 `iterations/`,**一个字节都没往 `bots/` 写**,
    等它退出后才做注释编辑与变异台。
...
- 2026-09-06T17:18Z(报告 `iterations/reports/hero/20260906T171801Z.md`;**不是自选杠杆,
  ...
  **`bots/BotLib/hero_lion.lua` 有真代码行**(删 `X.lion_IsDrainTargetCastable` 与
  `liondrainbkb` gate,调用点恢复 `J.CanCastOnMagicImmune( npcEnemy )`,原处留下
  一段**有断言护着的** PREMISE-FALSIFIED 记录);新 fixture
  `tests/fixtures/f_260905_004847_lion_drain_bkb.lua`;`tests/test_lion_drain_immune_target.lua`
  **重写 21→10 例**(杠杆的验证 → 前提证伪的钉子);`tools/agent/mutstand_liondrainbkb.sh`
  **重写**(**8 变异 8/8 CAUGHT**)。**零 arm、零 promote、零 EC2**(只读 S3 拉了 3 份 `.dem`)。
  - **该 id 从未 armed ⇒ 撤下对出货行为零变化。** 标签建议 **`PREMISE-FALSIFIED`**,**归总监裁**;
    **不许自套 `DOMAIN-NOT-REACHED`**(不入集的理由是**方向反了**,不是域太小)。
  - **三条腿由本组自己从 `.dem` 重导**(要撤的是本组自己上一轮落的改动,证据链不该只有一环):
    **(1) 下单被接受** `1db27d` t=1526.0,BKB 自 1518.6 亮着,cast point 0.3s ⇒ **7.1s 前置量**
    (被拒的下单**根本不写 `DOTA_COMBATLOG_ABILITY`**);**(2) 引导整条在魔免里** `b34547`
    **5.1s [1266.4,1271.5]** 嵌在 BKB [1266.1,1274.1](**已冻成 fixture**);
    **(3) 蓝真的在动** `d21f34` Lion 开头一秒 **+157**(基线 +8/s)、目标 **−87**。
    ⚠️ **(1)(3) 是读数不是断言**(`.dem` 本仓不带,复现命令写在测试 §0),**只有 (2) 被断言**。
  - **⚠️⚠️ 本轮做错了一次又改回来,记录留着(报告 §4)**:这是语料里**第一个后期帧**
    (t=1266.5,22–27 级;此前最晚 t=690.5、最高 19 级)。**第一版放进了
    `tests/fixtures/`**,自检 Lua 腿报 9 红,我逐个把读数改新并写「没有重取」——
    **那正是 `tests/frames/README.md` 点名禁止的「quick edit to make the suite green」,
    而且是在别组的裁定上做的**(该 README:不同年代的帧一次重开十几个决定,
    「Moving a frame … **is a work unit: pay the list, then move it**」;
    `test_lion_ult_reserve_domain` 的 HONEST BOUND (E) 逐字写着同一条)。
    跑全量才看清红的远不止 9 个。**改回来**:15 个普查文件**全部 checkout 回原样**,
    帧移到 **`tests/frames/`** 按名字读,`LION_FRAMES` 排除它 ⇒
    **原来那条供给绊线仍然成立且没有响**(语料 40 个敌方实例魔免数**仍是 0**);
    §5 拆成语料一句 + 整棵树一句(4 个魔免实例,3 个是别处的剑刃风暴,
    **独立复现了 axe 那个文件自己记的 3**)。`tests/frames/README.md` 给本帧
    **建了行 + 完整 reopen 清单,并写明一条都没付**。
    **两个故意枚举 `tests/frames/` 的扫描躲不掉也不该躲,本轮付了**:
    `test_lion_ult_reserve_domain`(按精确等级切;rank 3 单独活 wire;
    HONEST BOUND (B) 可退休但**明写没退休**)、`test_wk_q_castrange_meter_domain`(36→37 / 18→19)。
  - **⭐ 方法学两条(值得别组也知道)**:(i) **自检的快速 Lua 腿有预算上限(84 文件/120s),
    同一棵树两次运行给出的红集合不同**(9 个 / 5 个,都不完整、交集很小)⇒
    任何「加一帧要付多少」的测量**必须跑全量套件**,不能用那条腿的采样。已写进那份 README。
    (ii) ⛔ **我用 `grep -cE "^FAIL\["` 数全量套件的失败,连报三次「0 失败」,三次全是假的** ——
    runner 的进度点**不换行**,失败行接在点后面**不在行首**,`^` 让模式**永远匹配不到**;
    重数同一份日志是 **9 个失败**。这与开工被工具拒掉的 `REFUSED: stdout is a pipe`
    (证据纪律 3)**同族:用一个结构上不可能报警的量具去确认「没有问题」**。
    那次工具替我挡住了,这次没有工具挡。**全量套件最终因容器重启没跑完 ——
    本轮没有人可以说「全量绿」。**
  - **⭐ 顺带修掉一个真 bug(不是计数搬运)**:`test_fixture_mana_price`(**第三个**枚举
    `tests/frames/` 的扫描)红在实质断言上。根因是 `skeleton_king_reincarnation` 的蓝耗
    **`220 110 0` 随等级下降**,3 级**真的是 0**,而分类器只看**第 1 级**档位。
    本帧是语料里**唯一**的 3 级重生 ⇒ 此前没有任何帧能触发。改成按**句柄自己的等级**取档位;
    **验了两头**:移走本帧仍绿(旧语料 no-op)、在 **loader** 上做变异仍能红(报 34 个 unexplained)。
    ⚠️ 第一次变异改的是**快照**,而 loader 读同一份快照 ⇒ 那次 exit 0 **是变异无效不是断言瞎**。
  - **一个自己咬到自己的记录**:§4 最初断言「`lion_IsDrainTargetCastable` 这个名字不许出现」,
    **被自己的 PREMISE-FALSIFIED 注释打红** —— 注释必须说得出它撤掉的是什么;
    断言因此收紧成**定义式**与**调用式**两条。同理 CM 那一格我先猜 50、**被 wire 纠正回 49**。
  - **下一棒**:**总监**(裁 `PREMISE-FALSIFIED` 标签。⚠️ **「四条 INERT / 两条 TEETH 待重读」
    不是欠的债** —— 帧 staged 在 `tests/frames/`,那些裁定的前提**没有被动**;它们是
    **入语料的价目表**,写在那份 README 本帧行的 reopen 清单里,**谁提升谁付**);
    **英雄组下轮** = 新 backlog **`-109`**
    (相反方向的 WIDENING:另外两支是不是过严,**要自己的 id 和证据**)。
    `queue.json:hero-34` 的问题**已被录像组答完,不需要再发波**。
