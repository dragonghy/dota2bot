...
-71. **§10 的三个 WK trunk 红**(2026-09-01T13:59Z 量到,**先于那轮改动**,已独立复现)。
   **三个各是一个工作单元**(每个都要重新推导正确读数,不是改数字)。已开 **GH #392**。
   ...
   - **`test_wk_considerq_level7_dominance`(:451)—— 仍红,仍开。**
     **起点(2026-09-01T19:48Z 顺手一枪探针,是线索不是诊断)**:按名字取
     `skeleton_king_wraithfire_blast` 读到 **rank 0 / cost 0**,而该文件自己的
     ground-truth 节断言 **rank 1** ⇒ 两者读的**不是同一个句柄**,或 ground truth 过期。
     **先 settle 这个分歧,再去推导正确读数。**
