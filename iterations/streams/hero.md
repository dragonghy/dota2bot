...
-109. **⭐ 下一轮英雄组的第一个自选杠杆(方向与 -108 相反,别把它当成已经判过):
   ...
   - **供给**:`tests/fixtures/f_260905_004847_lion_drain_bkb.lua` 是语料里
     目前**唯一**带魔免敌人的 Lion 帧(45 个敌方英雄实例里 1 个)。
...
-108. ~~**GH #566 —— `liondrainbkb` 的立案前提在真实帧上被证伪。**~~ ✅ **2026-09-06T17:18Z 做完:
   ...
   **⚠️ 外溢很大,见报告 §4**:这是语料里**第一个后期帧**,打红了 **7 个别组的普查文件**
   (最重的是 `test_level_gate_census`:`ge20` 0→**10**、18:00 后的帧 0→**1** ⇒
   四条 INERT 裁定的前提**不再成立**、两条 TEETH 裁定**待重读**)。
   本轮**只登记不重读**,重读的棒**交给总监**。
...
- 2026-09-06T17:18Z(报告 `iterations/reports/hero/20260906T171801Z.md`;**不是自选杠杆,
  ...
  **重写**(**8 变异 8/8 CAUGHT**)。**零 arm、零 promote、零 EC2**(只读 S3 拉了 3 份 `.dem`)。
  ...
  - **⚠️⚠️ 最值钱的副产品在别人家:这是语料里第一个后期帧(t=1266.5,22–27 级),
    打红了 7 个别组普查文件** —— 先挪走 fixture 跑基线确认**七个全是本轮造成的**。
    最重的 `test_level_gate_census`:`ge20` **0→10**、高水位 **19→27**、18:00 后的帧 **0→1**
    ⇒ **四条 INERT 裁定的前提「语料里没有这种英雄」不再成立**,`J.IsLateGame()` 不再恒假,
    `mode_farm_generic:393/:507` 两条 TEETH **待重读**。`test_turbo_ternary_dominance`
    **终于能钉真实帧了(本轮没钉)**;`test_focus_mana_cost_consumer_census` 里 lion
    **翻转计数本身多了一格**(不只是分母)。**本轮一条都没替别组重读,全部「登记新读数 +
    明写没有重取」并留活 wire。**
  ...
  - **下一棒**:**总监**(裁 `PREMISE-FALSIFIED` 标签 + 决定四条 INERT / 两条 TEETH 谁去重读 ——
    前提是被**这一帧**推翻的,责任不会自动落回原作者);**英雄组下轮** = 新 backlog **`-109`**
