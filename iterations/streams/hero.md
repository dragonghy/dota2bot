...
18. **焦点五天赋梯子重锚(2026-08-22T14:00Z 起)** —— #104 的方法推到其余四个焦点英雄。
    ...
    - **下一根(登记、没做)**:`X.ConsiderR` 的 `nKillDamage = 150 + 100*lv` = **250/350/450**,
      而 Culling Blade 今天是 **275/375/475** ⇒ bot 低估自己处决技 **25 点纯伤害**,
      在那 25 点带内**拒绝本来能杀的一发**。修法是从技能读(`GetSpecialValueInt('damage')`)。
      **它是「加动作」不是「减动作」⇒ 要真实帧 + 量过的域**,预注册域已写进代码注释
      (Culling 就绪 + 目标 175 内 + 血量落在 `(150+100*lv, 技能伤害]`,**数 episode**)。
