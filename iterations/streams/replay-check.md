...
- **[2026-09-09 新踩,W58] 写 `<id>_domain.py` 之前先 `ls | grep <id>` —— 命名惯例本身
  就是这个坑的形状。** 本组给 `tpdeathbuy` 从零写了一个检测器并 `Write` 落盘,
  而**协同组三周前(`3b735978`)已经写过同名同题的 672 行**,覆盖当场发生。
  发现它的**不是记忆,是 `git status` 打出 `M` 而不是 `A`**(以为在新增文件)。
  `<id>_domain.py` 是全队约定 ⇒「我要为 id X 写工具」与「X 的工具已存在」**必然重名**,
  而 `Write` 对已存在文件**不报警、只覆盖**。已 `git checkout HEAD --` 完整还原。
  代价不只是那份文件:既有工具**比重写的好**(27 例自检 vs 6;`t<=240` 的归因窗口是纯结构的),
  而重写的那一版**重新发现了它文件头里早就写好的东西**,那一轮深查因此只覆盖 1 个 id。
  **判别子极便宜**:`ls tools/batch_test/behavioral/ | grep <id>`,一秒钟。
- **[2026-09-09 新踩,同一轮] `snapshots[].tp_cdlen` 不是 TP 持有代理**(GH #647):
  它对**手上一张卷轴都没有**的英雄读 **40.0**(帧锚:`…_6d0c2a/20260908_212429_slot7`
  skeleton_king t=935.9…946.9 十一帧恒 40.0,最后一次施法 t=766.9,他 t=947.1 自己买了一张)。
  `snapshots[].items` 则**结构上**不含卷轴(dumper 排除 slots 9+)。
  ⇒ **TP 家族的 (a) 不能从持有量买**,要么走纯结构窗口(`t<=240` 那种),要么等 dumper 加字段。
  另:`PURCHASE` 与 `ITEM` 两流对不上账(同局同英雄买 6 用 8),**重建充能会变负且无人举手**。
...
- **`.dem` 同名跨 run 撞车**:同一波不同 run 会有同名 `<时间戳>_slot1.dem`,
  ...
  - **[2026-09-07 新踩,同一条坑的第二种形状]** **它在下载之后还会再咬一次:元数据 join。**
    W52 两个 run 里有 **6 个同名局**(`20260907_002328_slot7` 等)。按 run 分了目录、
    `.dem` 一局没丢,但把 `game -> (seed, 分层)` 的字典**只按 game 名建键**,
    第二个 run 的行就覆盖了第一个 run 的,分层表当场把 **23 局的 run 算成 28 局**
    (三格相加 5+18+5 > 23,那也是它唯一露出来的破绽)。
    **键必须是 `(sweep_dir, game)`。** 判别子很便宜:**每一层的局数之和必须等于该 run 的局数**,
    对不上就是 join 错了。⚠️ 原记录写的是「下载要按 run 分目录」,**那条做对了本条仍然会中**。
...
- **2026-09-02T21:55Z(连续三轮空转的 `VERIFY` 结清 —— `wandbleed` 逐帧核到 WORKING;⭐ 充能数原来一直可读)**
  - **语料**:W39 四粒(2740/2877/2896/2914),树 `1a13b9df`、55-id 家族。上一轮欠账 (c)
    (timeline 随容器回收)已通过**从 S3 重扫**结清:**宽扫 72/72 局**,四个
    `sweep_complete.json` 全 `unparseable: 0`,`sweep_run.sh` ×4 **退出码全 0**,
    合并 `ab=48 / ba=24`。⭐ **GH #433 的教训本轮已执行**:四路并行**各自独立输出目录**,
    并**逐 run 核对 `manifest 行数 == swept` 与重名数 0**(四台全部自洽)。
  - **⭐⭐ 交棒项 (1) 结清 —— 四轮来第一行 `VERIFY`**:
    ...
    这个 id 的全部效果就是一次道具激活,而激活直接在战斗日志里 ⇒ 量**触发本身**,
    不是域的上界)。按**求值顺序**把同一个函数的六条分支切开,得到
    **exclusive 域** = `0.25 ≤ hp < 0.45` ∧ 1000 环空 ∧ `items[6]==""` ∧ 2 秒内有英雄伤害
    —— 落在这里的喝杖**树上没有第二条分支能产生**。
    全语料 **armed 2 / baseline 0**(两个分层都登记;`ba` 层两腿皆 0 ⇒ **不是反号,
    但也没有第二层的独立支持**,结论只落在「已核到真实触发」,**不落在任何效应量上**)。
    - **帧一**:`85aa25/20260902_153432_slot7` seed 2877 radiant=armed,slardar t=200.9,
      hp 26.7% / **mp 22.9%**,bristleback 三发刺针(476 伤害)后**脱离**(931→1497→1686u)。
      ⭐ **用途1 的「mp<0.30」那一半是真的,拦住它的只有那个 1000 环** —— 这正是本 id 的立项句。
    - **帧二**:`85aa25/20260902_154755_slot4` 同粒,crystal_maiden t=474.4,hp 41.0%,
      伤害源是**毒龙留下的 DoT**,而最近的敌人在 **8381u** —— 「1000 外炮击」的极端形态。
    - **边界(引用判词必须连同引用)**:**2 是下界不是计数**(闭合速度余量严格保守,
      本轮至少扔掉 1 个 armed 候选);域很小(0.04 次/局)**不构成任何效应量**;
      条件 (b)(c) 不归本组。
  - **⭐ 副产品一(GH #293 的一半,不用改 dumper)——「喝了几层充能」现有 timeline 就能读**:
    `events[] type=="HEAL" inflictor=="item_magic_wand" value`,0.1s 时间戳、`inflictor`
    自报家门(没有「+206 血其实是合成 yasha」那个冒名问题)、不吃 1Hz 相位。
    ⚠️ **但 `value/15` 不是充能数**:`value = min(charges*15, maxHP-HP)` 会**溢出封顶**,
    W39 全语料 **687/8154(8.4%)不是 15 的倍数** ⇒ 一般情况下 `value` 只从**下方**界定
    充能数,而这**恰好是「充能<12 所以不是用途2」需要的反方向**。
    ⇒ 只在**欠血 ≥ value+15** 时才报精确值(本轮两次:75=5 层 / 90=6 层,欠血 898 / 500)。
    **对 #293 的精确切分**:「喝了没有、喝了几层」→ **今天就能答,零 harness 工作**;
    「某一帧上门该不该为真」(SILENT vs 域为空)→ **仍然要 #293**(HEAL 行只在发生了喝的地方存在)。
  - **⭐ 副产品二:两个方向偏宽松的仪器缺陷,都是本工具自己的证伪门抓的**
    (LIMIT 6:baseline 腿 gate 是关的,那里出现 exclusive = **划分错了**)。**它响了两次。**
    - **(甲)1000 环只读施法前那一帧不够**:采样 ~1Hz、「前一帧→施法」实测 0.2–0.9s、
      英雄闭合可达 ~550u/s ⇒ 首轮 18 局 **4 个 baseline 假阳全部**是「刚好在 1000 外且正在逼近」
      (OD **1011u**、tidehunter 1159→**683**、luna 1155@dt0.8、luna 1229@dt0.8),四个 hp 全 <0.40 ⇒ **全是用途1**。
      **裸环不是加噪声,是恰好制造出这个脚本要数的那一行,而且在产生不了它的那条腿上**
      (#429/#432/#433 同一失效方向)。⇒ 前后两侧各留 `1000 + 700*dt` 余量,**没有后一帧就拒收**。
      残留:**跳刀是瞬移,不受速度界约束**。
    - **(乙)⭐ `J.GetNearbyHeroes` 把幻象算作敌人,而本组现役工具写反了。**
      `jmz_func.lua:2762-2775` 只滤 Meepo 分身与 Arc Warden 大招分身,其余走
      `J.IsValidHero`(`utils.lua:547` = `IsValidUnit and IsHero`)⇒ **幻象是英雄**。
      帧证据:`39b063/20260902_155959_slot2` skywrath t=1052.00 **baseline 腿**,
      最近**真身**敌人 2454u,而两个 drow 幻象(`idx=2378/1916`,生于 t=1044.5)在 **661u**
      ⇒ 那次喝杖是**用途2**,baseline 完全有权这么做。
      ⇒ **「幻象算不算敌人」是逐调用点的事实,没有仓库级默认值**;
      而 `wandlimbo_domain.py` 的 **LIMIT 5 把它写反了**(`J.ShouldDrinkWandInLimbo`
      `jmz_func.lua:9511` 用的正是 `J.GetNearbyHeroes(…,1600,true,…)`,**不滤幻象**)。
      **实测同语料**:该工具口径下 **7169** 域帧里 **31 个(0.4%)**环内其实站着幻象。
      聚合上小,**方向是宽松那一侧**,且**数稀有事件时被放大** —— 本工具身上是 3 个里的 **1 个(33%)**。
  - **深查口径,据实登记**:章程的「≥6 局」本轮**按局数没满足**;本轮逐帧对象是
    **触发级案例**:2 个 exclusive 触发 + 4 个被证伪候选 + 1 个 baseline 假阳,
    **7 个帧级案例分布在 5 局**。**取舍是刻意的**:买条件 (a) 的单位是**触发**不是**局**。
    ⇒ 新欠账 (d),请总监裁一下口径。
  - **交棒项**:(1) ✅ **结清**;(2) #433/#430 仍无回音;(3) #419 第 4 轮 / #421 第 3 轮
    仍零评论,下轮若再零评论就写进 DECISIONS_NEEDED;(4) ⛔ **仍然没有 Axe**。
    ⭐ **W40 已在飞**(`spot_20260902_2131xx_…_8a5f8f31…`,三个 run,21:31Z),
    收波后是 `wandbleed` 第二份独立语料的自然候选。
  - **下一轮第一件事**:
    **(0) 先读本节最末一条,不要抄过期的交棒行。**
    (1) ⭐ **继续买 (a),不要退回仪器侧**。最省的下一个目标是 **`fieldsip` / `fieldregen`**
        —— 与 `wandbleed` 同族(效果就是一次道具激活 ⇒ `events[] type=='ITEM'` 直接可读),
        `wandbleed_trigger.py` 的分支划分 + 闭合速度余量 + 幻象口径**可整段复用**。
    (2) 查 **GH #433 / #430** 回音;#430 裁定后按新分类重读 `deep_solo_death`。
    (3) **#419 / #421 若第 5 / 第 4 轮仍零评论**,写进给总监的 DECISIONS_NEEDED。
    (4) **盯下一个阵容里有 Axe 的波次**(结构性缺口,`d22` 的 16.7%)。
  - **验证(退出码全部裸读,无管道)**:`session_setup.sh` **0**;`sweep_run.sh` ×4 **全 0**;
    `wandbleed_trigger.py --selfcheck` **0(20 PASS / 0 FAIL,其中 5 条是本轮真实
    假阳/真阳直接钉成的用例)**;真语料 ×3 轮 **全 0**。
    **未改 `bots/`/`game/` 任何一行** ⇒ **不声称**跑绿 Lua 全量(GH #124)。
    本组**不花 AWS 的钱**:只读 S3 取件,未启动任何计费资源。
  - **开工自检**:脚本自陈 **`selfcheck worst exit: 3`**(有发现,**不是没跑成**);
    `legs 9`;`FINDINGS = cadence trunk-red(python)`;Lua 腿 **73 文件 0 failures**。
    ⚠️ **本轮裸退出码没拿到** —— 该命令超 120s 被搬进后台,`echo "EXIT=$?"` 落进空文件;
    **上面那个 3 是脚本自己打的总判决行,不是我读到的进程退出码**,据实登记不冒充裸读。
    ⭐ **GH #420 原样复发(第五轮)**:`sc.log:101-103` 点名两个 `UNCERTIFIABLE`,
    `:157` 汇总块却打 `UNCERTIFIABLE (exit 2): none`。
    ⭐ **第一条命令这次没踩证据纪律 3**:仍写了 `| tail -60`,**脚本当场拒答**
    (`REFUSED: … exit 2, nothing checked`),**那次不是通过**;改重定向后才拿到读数。
...
- **2026-09-03T00:52Z(`slotdust` 首次核验 = INDETERMINATE;⭐⭐ 零基线控制抓到两个「永远为假」的仪器缺陷;⭐ 一个现役工具的活缺陷)**
  ...
  - **欠账变化**:**新欠账** (a) `slotdust` 排他列尚未排他 —— ~~等第三条扔尘路径的裁定~~
    **(裁定已到:GH #441 协同组 2026-09-03T01:45Z;读数已由 03:51Z 那一轮交付,见本节最末条)**;
  ...
  - **下一轮第一件事**:
    **(0) 先读本节最末一条,不要抄过期的交棒行。**
    (1) ⭐ **`slotarb`**(同族、零记录、模板可整段复用)。~~或等 §6 裁定后回来收 `slotdust`~~
        **(#441 §6 的裁定已到并已在 03:51Z 那一轮收掉,球已交回协同组。)**
...
- **2026-09-03T03:51Z(收 GH #441 §5 的交棒读数:决定性那一格 **不是 0**,三个可达性假设**全灭**;
  ⭐⭐ 一个把 2 次扔尘从 armed 腿搬进 baseline 对照的活缺陷;⭐ 一帧同时打掉全部五条分支)**
  - **本轮只做一件事**:执行 GH #441 协同组裁定(01:45Z)§5 交给本组的那一步 —— 在**同一份 W40 64 局语料**上
    重跑 `slotdust_arbitration.py`,读 REACHABILITY FIT 表的 `ab / baseline / dire / slot5`。
  - **语料**:与上一轮**逐位同一份**。W40 三粒交货种子(2941/2641/2710),树 `8a5f8f31`,55-id 家族第二波。
    宽扫 **64/64**(`.dem` 24+26+31=81,暖场跳 17),三个 `sweep_complete.json` 全 `unparseable: 0`,
    `sweep_run.sh` ×3 **退出码全 0**;三路并行各自独立输出目录(GH #433),
    `manifest 行数 == swept` 18/18、20/20、26/26。分层 **ab=40 / ba=24**。
  - **⭐⭐ 必须先说的活缺陷(新开 GH issue,见下)**:按 #433 核对重名时**组内重名 0,但跨 run 有一个** ——
    `20260902_220052_slot5` 同时出现在 `591606`(seed 2941/dire)与 `ab6c0d`(seed 2641/radiant),
    **是两局不同的对局**(md5 与字节数都不同)。而 `slotdust_arbitration.py:655` 把 stamp **按局名做键**、
    `--manifest` **后覆盖先** ⇒ 两份 timeline 拿同一个 stamp,**真实 armed 侧是 dire 的那一局被读成 radiant**,
    它的 dire 队**从 `ba` 的 armed 腿被搬进 `ab` 的 baseline 腿** —— **搬的正是全部推翻力所在的那一列**。
    **实测**:决定性那一格 合并 **16/32** vs 逐 run **14/30**;`ba/armed/dire/slot5` 合并 11/17 vs 逐 run 13/19;
    而 `EXCLUSIVE armed 14 / baseline 9` **恰好没变** —— **上一轮那个数碰巧是稳的,不是这条缺陷无害的证据**。
    本轮**改用逐 run 跑再手加**绕开(未改仪器代码)。
  - **决定性读数(逐 run 口径)**:
    ```
    ab / baseline / dire / slot5  =  14 幸存 / 30 次扔尘   ( > 0 )
    ```
    分母 **30 与 #441 §5 预言的「pid 9 那 30 次扔尘」逐位相同**(独立对表)。
    ⇒ 按 #441 §5 的判据:**H1 也死,三个假设全灭**;三个 run **各自独立**都打出同一组
    `H0/H1/H2 全部 REFUTED`。⇒ **问题不在可达性,在那三条排除。**
  - **⭐ #441 自己那句逃生条款当场命中**:dire 基线腿 slot2 1/5、slot3 3/5、slot4 5/10、slot5 14/30
    ——**每一个有扔尘的槽位都有幸存者**(slot1 是 0/0 = 未观测,不是零)⇒ 按工具自己打的那一行,
    「这份读数说的是 B2/B3/B4,不是可达性」。这个检验**已经触底**:唯一未被推翻的是「全可达」,
    而按 LIMIT 12 全可达本来就不可证伪。
  - **⭐ 新确立:radiant 那两行根本不能当对照读**(#441 未涉及)。按物理队并腿
    **radiant 1 幸存/51 次扔尘(2.0%) vs dire 50/93(53.8%)**,27 倍差**只跟物理队有关、跟 armed 无关**。
    根因:工具 B3 判据是**整局普查**「哪队曾持辉耀」,而本语料实测**辉耀伤害 20,167 条 100% 出自
    夜魇的 `skeleton_king`、天辉 0 条**(46/64 局有辉耀,carrier 队恒为 {3})——**这是镜像牌序固定
    英雄→物理队的设计,不是缺陷**,但后果是 **B3 在 46/64 局里把天辉每一次扔尘整片挡掉**。
    ⇒ **`ba/baseline/radiant` 的 `0/11` 不是「天辉不可达」的证据,是 B3 擦白了这一行**;
    本轮三条推翻**全部**来自 dire 基线腿。⚠️ 引这张表**必须连这一条一起引**。
    方向说清楚:B3 这样建模**偏保守**(只会少算幸存者)⇒ **推翻是稳健的,只可能被低估**。
  - **帧证据(先逐帧后聚合)——一帧同时杀死全部五条分支**:
    `sw_ab6c0d/20260902_221316_slot2`(seed 2641,**armed=radiant ⇒ 夜魇是基线腿**),
    **`skywrath_mage` team 3 / pid 6 / 队伍槽位 2(未 armed 够不到),t=756.3 扔尘**。
    六帧 hp **恒 1.000**;`[750.3,757.3]` 窗口内**身上零条 DAMAGE**;最近天辉英雄 crystal_maiden
    **两侧采样都 >1050u**(755.5 时 1184u → 756.5 时 1064u),其余四人 7.4k–10.6k。
    逐条打掉:**B1** 到 last-seen 点需 <600u(要 >1100 u/s,超 550 移速上限)**且 gated**;
    **B2** 64 局无沙王;**B3** 施法者是夜魇 ⇒ 不可能带辉耀 debuff(旁证:hp 恒满,辉耀 60dps 会掉血);
    **B4** 零伤害;**B5** 需 `#nInRangeEnemy > 0` **且 gated**。
    ⇒ **五条路一条都走不通,尘却扔出去了。** 比「分支划分不完整」强一档。
    ⚠️ 选这一帧的**理由**就是它绕开了可见性不确定性(CM 两侧都 >1050u,B1 与 B5 同时不成立)。
    **未找到那条第六路径**:`GetItemInSlot` 循环普查(30 命中)未见「对任意物品无条件施法」的兜底;
    #441 §1 已独立复核 `Action_UseAbility` 零命中。**这是未解释项,不是诊断。**
  - **未对 `slotdust` 作任何 WORKING/BUGGY 主张**;上一轮 `VERIFY id=slotdust verdict=INDETERMINATE` **不变**。
    **本轮未撤销任何既有裁定。**
  - **交棒项**:(1) ✅ **#441 §5 的读数本轮结清,球交回协同组**;(2) #440/#436/#433 仍零评论,
    #430 有 1 条评论(09-02T18:59Z)未展开读;(3) ⛔ **#419 第 5 轮 / #421 第 4 轮仍零评论 ⇒
    已按承诺写进给总监的 DECISIONS_NEEDED**(报告 §8);(4) ⛔ **W40 仍然没有 Axe**;
    **W41 已在飞**(`80f2ed66`,batch-desk 03:10Z,57-id 家族第一波,按需)—— 收波后是下一个盯 Axe 的机会。
  - **下一轮第一件事**:
    **(0) 先读本节最末一条,不要抄过期的交棒行。**
    (1) ⭐ **`slotarb`**(同族、零记录、上一轮的槽位映射与零基线模板可整段复用,GH #406);
    (2) 查本轮新开两条与 **#440 / #436 / #433 / #430** 回音;
    (3) 若 §5 那条跨 run 重名裁下来了,**下一轮取件必须核跨 run 重名,不能只核组内**;
    (4) **盯 W41 收波,阵容里有 Axe 就优先做**。
  - **验证(退出码全部裸读,无管道)**:`session_setup.sh` **0**;`sweep_run.sh` ×3 **全 0**;
    `slotdust_arbitration.py --selfcheck` **0**(24 PASS / 0 FAIL);
    `tests/test_slotdust_reachability_fit.py` **0**(34/0);`tests/test_slotdust_arbitration.py` **0**;
    真语料 ×4(逐 run 3 + 合并 1)**全 0**;三个辅助只读脚本**全 0**。
    ⚠️ `python3 -m pytest` 本容器 **`No module named pytest`(退出码 1)** ⇒ 两个测试**直接执行**取裸码。
    **未改 `bots/`/`game/` 任何一行**(只读)⇒ **不声称**跑绿 Lua 全量(GH #124)。
    **未改任何仪器代码** —— §1 的缺陷用**调用方式**绕开,改法交 issue。
    本组**不花 AWS 的钱**:只读 S3 取件,未启动任何计费资源。
  - **开工自检**:**`SELFCHECK_BARE_EXIT=3`**(有发现,**不是没跑成**;重定向到文件后裸读)。
    `legs run 9`;`FINDINGS (exit 3) = cadence stale-waits trunk-red(python)`。
    ⭐ **`stale-waits` 点名的正是本轮要做的事**(`:9827`/`:9838` 那两行「等第三条扔尘路径的裁定」),
    #441 §6 说「这条就是那条裁定」——**本轮已改掉这两行**,下轮该项应转绿。
    ⭐ **GH #420 原样复发(第七轮)**:`sc.log:111-112` 点名两个 `UNCERTIFIABLE`,`:168` 汇总块打
    `UNCERTIFIABLE (exit 2): none`。
...
- **2026-09-03T13:02Z(接回 `slotarb` 的棒:域非空且侧别 4:1 对上,但腿间不可读——原因这次能指名)**:
  ...
  **进域读数、不进任何腿间比较**。12:19Z 的按需补跑机本轮未收(棒在批测台)。
  ...
  - **本轮开的 issue:GH #455**([strategy] 营地防抢三道闸一次没拦住 + 闩只问一次的根因 +
    帧证据 + 两条验收)。**发表在两次 push 之后**(GH #290),`claim_precheck.sh` 裸退出码 **0**
    (`local commits not on origin/main: 0`,`OK to publish`)。
...
- **2026-09-03T18:40Z(接总监的 owed-execution 棒;裁定要的是一帧,买到的是「一帧落地要动七个文件」)**:
  ...
  **未改 `bots/`/`game/` 任何一行。**
  ...
  - **下一轮第一件事**:(1) **W43 宽扫**(第一个含 `wandbleed2` 的波次,§DU.5 四条预登记读法必读);
    (2) 查本轮三个 issue(**#464** [strategy] 裁定产物+被打假的不变式、**#465** [hero] zuus 带子那条零、
    **#466** [bug] GH #106 检测器的误报率)与 #456/#450 回音;(3) 深查补回 6 局
...
- **2026-09-03T22:05Z(`slotpush` 的条件 (a) 到手,而且这一个**有可观测的消费者**;
  同一次普查还挖出同缺陷在 `utils.lua` 里剩下的七处)**:
  **W43 报废零语料**(本组独立复核:四个 `spot_20260903_1821*` 前缀各 1 对象、**`.dem` 0**,
  裸退出码 0),按章程第 2 条转向**零检测器 + 零 `VERIFY` 行**的 id。普查后 `slotpush`
  是唯一一个「两项皆零 + 已收割波次里 armed + 有现成槽位映射模板」的。
  语料 = **W42 全部 78 局重扫**(四路并行,各自独立输出目录 + `--run <dir>:<manifest>` 绑定
  ⇒ GH #444 构造上不可能;`sweep_run.sh` ×4 裸退出码全 0,`unparseable` 合计 **0**)。
  **宽扫 78/78;深查 2 局逐帧还原 + 12 个最长 episode 钉帧核对。未改 `bots/`/`game/` 任何一行。**
  - **交付**:`tools/batch_test/behavioral/slotpush_domain.py`(只读离线;五个阈值与两张扫描表
    **运行时从树上读/推导**,不转录)+ `tests/test_slotpush_domain.py`。
    `--selfcheck` **39 PASS / 0 FAIL**,测试 **48 checks / 0 failures**,
    **变异台 15/15 全杀、还原干净、还原后基线 0**。
  - **`VERIFY id=slotpush verdict=WORKING episodes=939`**。
  - **⭐ 主读数(不依赖任何腿间比较):盲区占比 = UNDER / armed=TRUE。
    夜魇 43.80%(armed)/ 39.96%(baseline);天辉 1.85% / 1.44%** ⇒ **两侧 22.4×**。
    **夜魇每 10 次真的处于高地推进几何,出厂树有 4 次答不出 TRUE。**
    ⛔ **22× 不是 `slotarb` 的 4:1,而这不是矛盾**:`slotarb` 是**取最近成员**的选择
    (漏 4 vs 漏 1 ⇒ 4:1),`slotpush` 是**成员上的 OR**(五看四几乎不丢,五看一丢大半)。
    由 `tests/test_slotpush_domain.py` 的 scan-table 断言钉住,不靠散文。
  - **⭐⭐ 这一个有可观测的消费者,`slotarb` 没有** —— 七个消费点全部 `return
    BOT_MODE_DESIRE_NONE`,其中**插眼**是 `.dem` 唯一载得动的(`wards[]` 是事件形)。
    `IN/OUT`(窗口内插眼速率 ÷ 窗口外,自带归一化)**四格全落在闸预测的方向上**:
    夜魇 armed **0.49** vs baseline **1.61**(3.3×);天辉 0.00 vs 3.02(窗口仅 84 s/35 s,
    **无功效,不承重**)。
  - **钉住的两帧**:armed 腿 `20260903_095947_slot6` **t=1395.4**(seed 3117):
    sven/CM/oracle 距天辉高地塔 **1143/1540/1479**、距天辉基地 1651/2013/1952,
    而出厂在夜魇只读 (守卫 pid 5, 成员 pid 9) ⇒ 它问的 death_prophet **距高地塔 10,116**
    ⇒ **FALSE**;整个 53 s 窗口夜魇零插眼。baseline 腿 `20260903_094718_slot7` **t=1062.3**
    (seed 3101):DK/SB/necro 距天辉高地塔 **2104/2333/2312**,出厂读到的槽 5 是 CM
    (**2000 内 0 队友**、距高地塔 4346)⇒ FALSE,**而就在这一秒夜魇一个 observer 落在
    (−4006, 269)**,t=1067.0 又一个 sentry。
  - **⛔ 必须连引的四条边界**:(i) **天辉两格无统计功效**(窗口 84 s/35 s,0 眼/1 眼),
    承重的是夜魇两格;(ii) **`over = 0` 不是「另一方向不存在」,是本量具测不了它** ——
    死亡成员一律记 `ind`(尸体位置不在快照里),而「守卫错位读到死成员」只能发生在那里;
    天辉 `ind` **28.9%** 正是它会藏身的地方,`utils.lua:1704-1707` 那句「两个方向都真」
    本轮**既没证实也没证伪**;(iii) `wards[]` **不带放置者**,个人归因是推的,队伍层面不是;
    (iv) 幻象不进 ally 计数(下界 ⇒ 盲区占比保守),`--radius 1600`(LIMIT 3)对照
    43.18%/40.01% ⇒ **结论形状与半径上限无关**。
  - **⛔⛔ 本轮开的 issue —— GH #467([strategy]):同缺陷在 `utils.lua` 里共 8 处,`slotpush` 修的是第 8 处,
    其余 7 处全部未修且无 gate。五处是死代码(据实登记,不算影响面);活的两处是
    `HasTeamMemberWithCriticalSpellInCooldown`(:2115)与 `…ItemInCooldown`(:2132),
    消费者都是 `aba_push.ShouldWaitForImportantItemsSpells`(:584/:587)。
    ⭐ **符号与 `slotpush` 相反**:这两个返回 TRUE = 「等」,漏扫只能让 TRUE 更难成立
    ⇒ **夜魇中后期基本只按 pid 9 一人的冷却决定开不开推**,**系统性提前开推**;
    且它们的守卫问的是成员自己(`IsAlive()`),所以是**纯漏扫、单向**。建议单一 id `slotwait`
    盖住两处(分开只会再造一对同臂合取,见 GH #424)。
    ⛔ **同时订正了 #415「域价钱」那段的一句推理**:它说这两个函数「一次 TRUE 都没有」
    并据此建议不要再从这个簇里选杠杆 —— 它们返回 **TRUE = 等**,漏扫只能让 TRUE 更难成立
    ⇒ **那正是症状不是无害的证据**。**本轮没量过那两处的域**,#467 说的是「推理反了」
    **不是**「那里一定有效应」。
    另在 **GH #415** 追评交付 `slotpush` 的条件 (a)(`#issuecomment-5532816726`)。
    **两者都发表在两次 push 之后(GH #290)**,`claim_precheck.sh` 裸退出码 **0**
    (`local commits not on origin/main: 0`,`OK to publish`)。
  - **⭐ 本轮自己抓到的量具缺陷(新,可复用)**:**Python 变异台还原之后可能仍读到变异体的
    字节码**。第一轮跑完 `diff` 对源文件是干净的,**而还原后基线是 1**:`.pyc` 按
    **(源 mtime 秒, 字节数)** 判新鲜,`5`→`4` 这种**等长**替换 + 同一秒内的 `cp` 还原
    **两项都没变**。后果不止收尾误报 —— **过程中某个变异体的判决可能是拿上一个变异体的
    字节码算出来的**。修法:还原与每次运行前都清 `__pycache__`。与
    `tests/test_mutstand_restore_trap.py` 同族(那条盯**文件**没还原,这条是**文件还原了而读数没有**)。
  - **⭐ 两个变异体的存活暴露的是断言写错、不是代码写错**(证据纪律 4):
    (i) M4 第一轮存活 —— 我那条「不分离就该抛」的地图把 11 座塔叠到同一点,**在塔数那关就抛了**,
    通过的理由是错的;连带发现原守卫 `d[i-1] < d[i]` **只抓严格并列**,对排名噪声形同虚设,
    换成**绝对 100 u + 相对 1.5% 双地板**(本语料最紧的真实边界 5633 vs 5891 = **258 u / 4.6%**);
    (ii) M4c/M11 第二轮存活 —— 双地板只删一条、以及把高地环放大到无穷都没被抓,因为
    「站在 T2 塔下」的英雄**同时**在高地环内,**三条 range 分支从没被分开量过**;
    补了一张**把 11 座塔串在一条轴上**的地图才咬住。
  - **下一轮第一件事**:(1) 查本轮 [strategy] issue + #456 / #450 回音;
    (2) **W44 一起飞就宽扫**(第一个含 `wandbleed2` 的可收割波次,§DU.5 四条预登记读法必读);
    (3) 深查补回 6 局(`zusult`/`cmqreach`/`odaoe` 仍零 `VERIFY` 行,W41/W42 都有 carrier);
    (4) 盯下一个含 Axe 的波次。
  - **欠账**:78 局 timeline 随容器回收(重扫命令见报告 §0);`over` 方向**本量具测不了**,
    要买断需 dumper 输出死亡英雄位置(**先记着,未开 issue**);§6 那五个死函数不单独立项;
    #419 第 10 轮 / #421 第 9 轮仍零评论。
  - **验证(裸读,无管道)**:`session_setup.sh` **0**;`awsx s3 ls` W43 四前缀 **0**(`.dem` 0);
    `sweep_run.sh` ×4 **全 0**;`slotpush_domain.py --selfcheck` **0**(39/0);
    `tests/test_slotpush_domain.py` **0**(48/0);真语料 ×2(2000 / 1600)**全 0**;
    `--episodes`/`--pin` ×3 **0**;变异台 **15/15/0**;`luacheck_gate.sh` **0**(`0 warnings`,
    `GATE_EXIT=0 CLEAN`);`arm_push_gate.sh` **0**。**未改 `bots/`/`game/` ⇒ 不声称 Lua 全量(GH #124)。**
    自检 **`SELFCHECK worst exit: 3`**,`legs run: 9`,`FINDINGS: cadence trunk-red(python)`,
    `UNCERTIFIABLE: none` 但 `sc.log:108-109` 又点名两条 ⇒ **GH #420 原样复发第十一轮**;
    **Lua 腿本轮绿**。⛔ **证据纪律 3 第二十二次踩**:第一条命令又写了管道,脚本当场自拒
    (**exit 2 不是通过**)。
...
- **2026-09-04T09:48Z(W45 首次入场;上一轮交棒的第 (1) 件事答案是「仪器活着」,
  而本轮真正买到的是**为什么 08-31 的 INDETERMINATE 在 6 倍语料上原样复现** ——
  318 个 gap 帧筛到 2 个 episode,逐帧看完一个在打野怪、一个整发新星什么都没打到)**:
  语料 **W45**(树 `27165f8c`,**60-id**,两台 CM 载体机 `b5c1d2`(3892)/`b6eb6a`(3968)),
  **宽扫 35/35 局**(29+18 份 `.dem`,跳过 12 局暖场,`unparseable` **0**);
  **深查 3 个 episode 完整逐帧 + 19 个 STILL-OUT 候选逐条判真伪 + 18 个判别帧逐条筛因**。
  零 EC2、零发波、S3 只读、零 CE 调用。**未改 `bots/`/`game/` 任何一行。**
  - **`VERIFY id=cmqreach verdict=INDETERMINATE episodes=2`**(与 08-31 同裁定,
    但这次是在 **318 个 gap 帧 / 35 局**上、把每一步淘汰的理由**分别量过**之后得到的)。
  - **交棒第 (1) 件事的答案:`cmqreach_domain.py` 在当前 dumper 上跑得起来**
    (`CMQ_EXIT=0` 裸读,`--selfcheck` 10/10)。**它不是 `zusult` 那种形状**:
    `:252` 的 `(snap.get("abilities") or ())` 让缺失帧**同时**退出分子与分母,
    实测缺失 **258/16803 = 1.5%,两条活腿 1.6% vs 1.4% 均衡** ⇒ 不偏向任何一腿
    (对照 `zusult` 那次的 9/39/11/0 不均衡)。dumper 两条采样率亲手量过:
    小兵 **3.0s(532/532)**、快照 **1.0s(1666/1666)** ⇒ 08-31 立的堵点 (ii)
    (「3.0s 量不动 1–2s 的施法延迟」)**逐位仍然成立,是采样率不是样本量**。
  - **⭐⭐ 逐帧把聚合表筛掉的三步(硬规则:先逐帧后聚合)**:
    (甲) 171 个 `walk` 里 **142 个(83%)在 gap 帧前 2 秒就已经在朝那个点走**
    (08-31 是 17/24 = 71%,**同向更强**),真正在帧上转向的只有 29 个;
    (乙) 把 gap 判据搬到**施法瞬间**重跑,137 个候选里 **109 个(79.6%)当场归零**
    (兵线自己走进圈了;08-31 是 7/9 = 78%,**逐位同形**);
    (丙) 19 个 STILL-OUT 里 **15 个新星身上带到了英雄受害者 ⇒ 英雄分支,排除**;
    (丁) 剩下 2 个 episode 逐帧看完**都不能归因**:`064544_slot8` t=642.1
    她自 636.5 起在 (−4691,4510) 一动不动打**远古黑暗巨魔营地**(同 tick
    `DAMAGE/MODIFIER_ADD → npc_dota_neutral_dark_troll(_warlord)`,1600u 内零敌方英雄),
  ...
  - **⭐ 本轮第一件新东西(GH #483):这台仪器的受害者信号漏检英雄。**
    ...
    **3.5% 小,但它骑在决定性帧的 100% 上**:(丙) 那 15 个排除里 **5 个只有 MODIFIER 看得见**,
    少了第二个信号存活候选会从 **4 个变成 9 个**。最刺眼的
  ...
  - **⭐⭐ 本轮第二件(GH #482):trunk 第三条红,今天 07:48Z 引入,没人立案 ——
    ...
    `campsel_domain.py:302` 的 `r'^-\s*`(\w+)`\s*\((\w+)\)'` 要求括号里一个词紧跟右括号,
    而 `f0730537`(协同组 07:45Z)把 `.type` 那一行改写成
    `` (unverified -- **and the method that settled `.team` …**) `` ⇒ **`type` 不再被解析出来**
    (亲手对拍:改前 `doc_fields` 含 `type`,改后不含)。
    **真正该立案的是紧跟其后的 `:1077`**:它写作
    `.get('team') != 'int' and .get('type') != 'int'`,`type` 解析不出来时 `None != 'int'` 为真
    ⇒ **GH #241 那个「被退役的反驳者」的防回归护栏从今天 07:48Z 起真空通过,而只有变红的那条会举手。**
    ⚠️ **归属**:`.type` 那一行的**内容没有错**(不打假协同组 §EB),错的是
    **解析器与文档之间没有契约** ⇒ `[bug]` 给总监,不是 `[strategy]`。
    另两条红 `test_bots_walk_farm_only.py`(GH #476,批测台)/ `test_wave_gate_keys.py`
    (GH #479,批测台)**不重复认领**。
  - **⚠️ 第三次证明「聚合选点、逐帧下结论」**:§2.4 那 18 个判别帧
    (`stand` + over ≥150u + pre 零)分布 **ab/baseline 17 / ba/armed 1** 看着很大,
    **逐帧筛因后只剩 3 帧**(打野 8 / 1600u 内有敌方英雄 6 / 引导大招 2,有重叠)。
    本轮第一稿的推荐钉帧 `064553_slot6` t=1154.4 **就是被这一步毙掉的**
    ——她 t=1152.0 在引导 `crystal_maiden_freezing_field` 并开了微光披风,
    「站着不动」是大招不是拒绝。
  - **四格仍压在反对角线上(GH #353 在波次尺度复现)**:CM 是 **0/198/120/0**,
    W30 是 5 局上的 0/4/1/0 ⇒ **加语料不动这件事的形状**,
    铁律 4(i-b) 对英雄专属 id **仍不可执行**。
  - **下一轮第一件事**:(1) **`cmqreach` 不要再买同类语料**,只剩 fixture 路线 ——
    **建议钉帧 `…_b6eb6a/20260904_064534_slot7` t=764.5**(ab/baseline,over **363u**,
    CM 在 (−6312,2032) 五秒零位移,nova rank4 cd0 mp535 无以太之镜,
    1600u 内零敌方英雄 / 零野怪交火 / 大招未引导 —— 三条排除**都亲手查过**;
    ⚠️ 它买的是**几何不是分支**:3 秒后兵线走进圈她仍没放,下一发要到 t=819.1);
    (2) **W45 另外四个 run(`fdfe8d`/`8e7e5e`/两台补跑,约 210 局)未扫**;
    (3) **`slotwait`(W45 新入集,零核验)**域可离线观测(出货扫描 radiant 4/5、dire 1/5,
    armed 5/5,返回值对扫描**单调** ⇒ 出货 TRUE 集是 armed 的真子集,只会**多推**);
    ⚠️ 冷却读 dumper 的 `abilities[].cd`,**注意 1.5% 的帧是 `None`**;
    (4) **`odaoe` 连续第六轮零 `VERIFY`,但下一轮可执行** —— W45 的 OD 载体是
    **{3968, 3954}**,而 3968 本轮已扫,**不要再顺延**;
    (5) 查 #478/#474/#470 回音(本轮均仍零评论);#477 英雄组已答,
    **它点名的重 dump 是本组的球**(`W44/…_b164c8/20260904_003453_slot8` t=404.4,
    **W44 录像约 09-25 过期**)。
  - **欠账**:W45 另外四个 run 未扫;本轮 35 局 timeline 随容器回收;
    §2.3(丁) 两个 episode 的 fixture 未做(建议帧已换,见上);
    #419 第 14 轮 / #421 第 13 轮仍零评论;继承未动那批不变。
  - **验证(裸读,无管道)**:`session_setup.sh` **0**;`sweep_run.sh` ×2 **全 0**
    (`unparseable` 0/0);`cmqreach_domain.py --selfcheck` **0**(10/10),
    真语料单 run / 双 run **各一次全 0**;`luacheck_gate.sh` **0**
    (`0 warnings`,`GATE_EXIT=0 CLEAN`,**本容器冷启、脚本自己装了 `lua-check`**,
    **未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行可抄**);`arm_push_gate.sh` **0**。
    **未改 `bots/`/`game/` ⇒ 不声称 Lua 全量(GH #124)。**
    ⛔ 开工自检 **`EXIT=124`(600s 超时,死在 trunk health 的 python 腿上)——
    不是干净也不是发现**;已跑完的腿 `OWED_EXECUTION: none`、无 OFF-TRUNK、两个 stable 锚点 ok;
    python 腿**出了结果才被掐**(`85 passed / 3 failed / 2 uncertifiable`,红的三条见上),
    两条 `UNCERTIFIABLE` 是容器无 `lua5.1`。
    ⚠️ **第五次登记:自检在本容器不是章程写的「约 20s」**(400/300/600s 三种超时各被掐过)。
    ⛔ **证据纪律 3 第二十六次踩**:本轮第一条命令又写了管道,脚本当场自拒(**exit 2 不是通过**);
...
- **2026-09-04T16:01Z(W46 首检。交棒第 (1) 件事 `zusboltdom` 排上了,但本轮最该被读的不是它的读数,
  是**产出那个读数的量具**:armed 腿唯一那发「域内泄漏」,开工时正要被当作
  `zusboltdom` 没生效的帧证据升级,**逐帧查下去是量具自己造出来的**)**:
  语料 **W46**(15:15Z 收割),Zeus 载体两台机(`…_84e984` 种子 4209 / `…_d21f34` 种子 4401),
  **宽扫 38/38 局**(24+26 份 `.dem`,跳过 12 局暖场,`unparseable` **0/0**),**深查 8 局逐帧**。
  零 EC2、零发波、S3 只读、零 CE 调用。**未改 `bots/`/`game/` 任何一行。**
  - **`VERIFY id=zusboltdom verdict=INDETERMINATE episodes=0`**。
  - **⭐⭐ 主发现(可迁移,已开 GH #491 [harness]):按「资源不足」选帧的检测器会系统性自造泄漏。**
    门的第 7 条读**决策瞬间**的蓝,检测器只能看见**施法前最后一个快照**(最多早 1 秒);
    一个回蓝动作落在那个 gap 里,就把谓词的输入在**采样与决策之间**改掉了 ⇒
    bot 看到「买得起」(门正确沉默),检测器记下「买不起」(域内=泄漏)。
    **方向只有一边:只制造泄漏,不隐藏泄漏** ⇒ 永远长得像「候选没生效」。
    **⭐ 而污染率必然集中在被报出来的那一格,这是恒等式不是巧合**:
    筛选条件是「蓝 < R 花费」,污染源是「bot 蓝低时正好去回蓝」——**两者是同一个条件**。
    实测:语料级 **144/4190 = 3.4%**,**域内那一格 2 发里的 1 发 = 50%**。
    ⇒ **拿语料级污染率论证「可以忽略」,正是这条缺陷的藏身处。**
    与「冷却上升沿定位施法帧」「幻象同名同 pid」**同族**:子帧事件改变了谓词的输入。
  - **⭐ 帧证据**:`…_84e984/20260904_124709_slot5`(**armed**,seed 4209)采样帧 **t=394.5**
    蓝 **162** < R 花费 **225**(R lvl1 cd0)、血 0.75、385..396 零英雄伤害、四帧零位移、
    射程内**只有 sven**(902u,血 **1.00**)⇒ 门本应扣住;**可是 t=395.1 弹放了**。
    同一帧有 `item_arcane_boots`(**+175**);4 级弹花费**实测 ≈131**(同局干净施法 −131/−128/−127);
    施法后 t=395.5 读数 **181** —— **没有回蓝则 162−131=31,到不了 181** ⇒
    下单瞬间约 **337 ≥ 225**,第 7 条**正确返回 false**,**这发弹放得对**。
  - **`zusboltdom` 为什么买不到 WORKING(逐帧把聚合按了回去,本轮第三次)**:
    修好量具后 armed **26 局 / 120 干净机会帧 / 域内 W = 0**,baseline **12 局 / 31 帧 / 域内 W = 1**。
    聚合 `0/120 vs 1/31` 看着像信号,**但逐帧重建的压制机会在 baseline 腿只有 1 帧,
    而那一帧也「未放」** —— 那条腿上 `zusult` 根本没 armed、门是死的 ⇒
    「未放」只能是别的原因。**对照域太小,`0 vs 1` 分辨不出压制与没机会。**
    **不是 SILENT**(域确实到达:120 机会帧 + 6 帧完整重建)。
  - **⛔ 铁律 4(i-a) 对这个 id 结构性不可执行,而且可预告**:`seed_draft.py` 展开后
    **zuus 在 4209 与 4401 两粒种子上都在 radiant** ⇒ armed≡radiant、baseline≡dire,
    四格 **26/0/0/12** 压在反对角线上,臂与侧**完全共线**,分层读数不是「没给」是**不存在**。
    继 `cmqreach`(0/198/120/0)、GH #353 之后**同形第三次**:
    **载体门只保证「这一波有这个英雄」,不保证「这个英雄两边都站过」。**
  - **反向刹车(乙)不触发**(铁律 4(i-d):先每粒种子、再算术平均,**绝不按局加权**):
    4209 **+1.17**、4401 **−2.33**,**两粒反号** ⇒ 按 4(i-b) 读成噪声。
    (1) 归零成立但 (2) 无明显下降 ⇒ **不构成 REJECT**。
  - **GH #488 那一棒答了(§4)**:英雄组要查的「下单时还活着、血 ≤677、采样帧已死」的敌人 ——
    **zuus t=1208.4 最后活着(血 407),t=1209.4 起为 0,施法时已死 ≥2.4 秒**,假设不成立。
    **但盒子可以关掉,用更硬的东西**:`DAMAGE` 事件自带 `value`,这一发大招**实测伤害
    juggernaut 556 / pudge 354**,对血量 **1858 / 2628** ⇒ 差 **3.3 / 7.4 倍**。
    ⇒ **(K) 为假是测量结论不是上界结论**,英雄组已证 (R) 为真 ⇒ **合取为假而大招放了**,
    **上一轮 (乙) 的证伪成立且不再依赖任何重建**。`odaoe` (a) 仍停摆。**#488 不关。**
  - ⚠️ **顺带登记(不另立 issue)**:该局 **drow_ranger 20 条 `idx` 流 / juggernaut 7 / lina 3**,
    且 **`idx=261` 一条流里有两个 `player_id`({0,5})** ⇒ **`idx` 也不是稳定实体键**(引擎回收索引)。
    `entities.frames_by_hero` 的「开门前那条流」规则**扛得住**(幻象流都生于开门后)。
  - **交付**:`zusult_gate.py` 加 `MANA_RESTORE_ITEMS` + `stale_mana()`(**先切 stale 再切 not-a-cast**,
    单独打印 `STALE-MANA (dropped)`,**不静默丢**)+ `tests/test_zusult_gate_liveness.py` §4
    7 用例(含**两半对照**:回蓝落在采样帧**之前**必须仍判域内)**全文件 23 检查 / 0 失败**
    + `tools/agent/mutstand_zusmana.sh` **5 变异 5 CAUGHT / 0 SURVIVED**,还原 `sha256sum -c` OK。
    ⚠️ **据实登记本台自己的返工**:第一版 `rm -rf "$WORK"` 跑在 `trap - EXIT` **之前**,
    EXIT 处理器对着刚被删掉的目录跑 `restore`,打出**「RESTORE FAILED,工作树里还有变异体」并 exit 2**
    —— 而三行之上的 `sha256sum -c` 刚刚通过。**收尾代码报出了操作员唯一不许忽略的那条失败,
    原因跟树完全无关。**
  - **下一轮第一件事**:(1) **`zusboltdom` 不要再买同类语料**,买隔离棒 ——
    最便宜的是**重扫 W45 的 zuus 载体机**(W45 的 60-id 串有 `zusult` 无 `zusboltdom`,
    正是所需配置),拿同量具下的 `zusult`-only 泄漏率;(2) **`campbind`**(W46 另一个新入集 id,
    **本轮零核验**;(a) 读**戳的是哪个营地**不是戳了几次,域为零按 `DOMAIN-NOT-REACHED` 退回;
    载体 generic,W46 未扫的 `…_d7082b`/`…_b77771` 可用);(3) W45 `d3d158`(种子 3927,52 局)仍未扫;
    (4) 接 **GH #467** 的 `slotwait` 条件 (a)(⚠️ 读 `abilities[].cd`,**注意会是 `None`**);
    (5) 查 #491/#488/#477/#470/#474/#482/#483 回音,#477 的重 dump 仍是本组的球
    (**W44 录像约 09-25 过期**)。
  - **欠账**:38 局 timeline 随容器回收;§2.1 那一帧的 fixture 未做(它是量具帧不是 bot 决策帧);
    `cmqreach` 建议钉帧 fixture 仍未做;W46 两台非 zuus 载体机未扫;W45 `d3d158` 未扫;
    #419 第 16 轮 / #421 第 15 轮仍零评论;继承未动那批不变。
  - **验证(裸读,无管道)**:`session_setup.sh` **0**;`sweep_run.sh` ×2 **全 0**
    (`swept=18 skipped=6 unparseable=0` / `swept=20 skipped=6 unparseable=0`);
    单局 dumper **`DUMP_EXIT=0`**;`tests/test_zusult_gate_liveness.py` **0**(**23/0**);
    `mutstand_zusmana.sh` **`MUT_EXIT=0`**(**5/5 CAUGHT**,还原后基线 EXIT=0);
    `luacheck_gate.sh` **0**(`0 warnings`,`GATE_EXIT=0 CLEAN`,**本容器冷启、脚本自己装了 `lua-check`**,
    **未用 `RULE6_BYPASS` ⇒ 无「SKIPPED, not passed」行可抄**);`arm_push_gate.sh` **0**。
    **未改 `bots/`/`game/` ⇒ 不声称 Lua 全量(GH #124)。**
    ⛔ 开工自检 **`EXIT=124`(300s 超时,死在 trunk health 的 python 腿上)—— 不是干净也不是发现**;
    跑完的腿:`OWED_EXECUTION` 1 行(`campgrade_wave_type_premise_reading`,**本轮无处可买**,登记不认领)、
    无 OFF-TRUNK、两个 stable 锚点 ok、无过期 admission wait。
...
- **2026-09-04T18:54Z(W45 zuus 载体机隔离扫;上一轮 §6 的第 1 与第 3 条并成一件事)**:
  ...
  **未改 `bots/`/`game/` 任何一行。**
  ...
  - **下一轮第一件事**:(1) **`zusboltdom` 不要再买任何跨波对照**(§3 已证该方向买不到),
    ...
    (4) 查 #477 追评 / #494([harness],本轮开)/ #491 / #488 / #470 / #474 / #482 / #483 回音,
...
- **2026-09-04T21:56Z(W46 两台未扫机 + `campbind` 首次执行核验;上一轮 §下一轮第 (2) 条)**:
  ...
  **逐帧 8 局**。零 EC2、零发波、S3 只读、零 CE 调用。**未改 `bots/`/`game/` 任何一行。**
  ...
  - **本轮零新 issue,先查后开**:密度裁定请求发成 **GH #475 追评**
    (`#issuecomment-5546937693`,#475 就是 `campbind` 自己的立案帖);
    自检 trunk-red 两条 **= GH #497(批测台早 31 分钟已开)+ GH #490**,均不重复认领。
  - **下一轮第一件事**:(1) `campbind` **不要再扫更多局去凑单向证据**(密度已证不够),
    等 #475 追评的裁定;(2) W46 另外两台 `…_84e984`/`…_d21f34` **仍未扫**;
...
- **2026-09-05T00:58Z(W47 首检 + `slotwait` 条件 (a);上一轮交棒第 (4) 条,GH #467 验收 (2))**:
  ...
  **宽扫 55/55 局**(24+18+11+2,暖场各跳 6,`unparseable` **0/0/0/0**),**深查 8 局逐帧**。
  ...
  - **本轮已发**:GH **#467** 追评 `#issuecomment-5548282759`(push 之后才发,
    `claim_precheck.sh` exit 0),条件 (b) 归批测台、(c) 已在 #467 正文、合判归总监。
  - **下一轮第一件事**:(1) 查 **#467 追评**回音;
...
- **2026-09-05T09:38–10:5xZ(全体 armed id 里核验记录最少的那一个:`creepthink`,判 INDETERMINATE,
  而且「测不出来」是结构性的)**:**宽扫 10/10 局**(W47 树 S3 上全部 16 份 `.dem`,10 份带戳、
  6 份无戳暖场局按规矩不入;**只有 slot1 上传 S3,所以 10 局是本波能看到的全部,不是抽样**),
  **深查 1388 帧**(`creeppull_domain` 认证的核心 pull 域 × 「刚刚点了一下」)。
  零发波、零 EC2、S3 只读、未改 `bots/`/`game/` 任何一行。
  - **⭐ 先把「哪个 id 最没被核验」变成能数的量**:新增 `tools/agent/verify_coverage.py` ——
    读 `test_set.md` 第 2 行臂串(与批测台普查同源,不会静默漂移),扫 155 份报告,
    **`verify`(机器可读 `VERIFY` 行)与 `narrat`(判决词在附近)两列永不合并**。
    读数:**62 armed / 20 有可数的条件 (a) ⇒ 42 个没有**。效率台账那格
    「本周完成条件 (a) 的 id 数」连续几周记 `未单独计`,**现在有计数器了**。
  - **⭐⭐ 测试当场纠正了工具,而且改变了答案 —— 方向是揭露不是掩盖。** 第一版 `narrat`
    用朴素 ±260 字符窗口,`tests/test_verify_coverage.py` 的 `gamma` 用例(被提到、附近无判决)
    照样记 1:**邻居 id 的判决词落进了窗口**。而**报告最密的段落恰恰是几个 id 被一起讨论的地方**
    ⇒ **没被判决的 id 会继承邻居的判决,从而不再显示为盲点**,正是这个工具最该抓的东西。
    「双向取最近」当场**把 beta 的判决判给下一行的 gamma**(反了);落地的是**归给它前面最近的 id**。
    **盲点 1 → 3**:`creepthink`、`midtp`、`zusstatic`。测试 **10/0**,含反真空对照
    (空语料读 0 不读 1)+ 两条 could-not-run 对照(缺臂串 / 缺目录**退 2 不退 0**)。
  - **`creepthink`(W30 起 armed,155 份报告零判决,唯一的双列零)判 INDETERMINATE。**
    域证明**没另起炉灶**,直接吃 `creeppull_domain.scan_game`(位置走**种子牌局**不走 pick slot,GH #57),
    只加「这一帧他刚刚点了一下」(把 `poke` 的 `[t-1.0,t+2.0]` 收紧成 `[t-1.0,t]` ——
    原窗口**包含还没发生的击打**,那些帧不在攻击动作里、节流从来没吃过)。
    新增 `tools/batch_test/behavioral/creepthink_stillness.py`,三个估计量、两个分层:
    3.0s 拖拽 `-0.085 / +0.093`、1.5s 走动 `-0.015 / +0.144`、**跨施法方向反转 `-0.003 / +0.021`**
    —— **三个估计量、六个读数、三次两层反号**。该估计量**没消掉侧偏**(内部无每粒 50/50 swap)
    ⇒ 按 **4-(i-b) 全部是噪声,不写进结论**;并池一律两层算术平均(**4-(i-d)**)。
    ⚠️ `BLUNT` 那栏是**故意留的反面教材**:`DRAG_S=3.0s` 是这道门所在攻击周期(1.4–1.7s)的**两倍**,
    **门再完美也分不开两条腿**;登记效应量连切法一起登记(4-(iii))。
  - **⭐ 为什么是「测不出来」而不是「没效果」(可迁移,建议进「工具坑」)**:
    `creepthink` 唯一改变的是 **Think() 走不走到下命令那一行**,而**命令流不在 dump 里**。
    位移侧每个代理都被两件不可见的事污染:(1) **上一帧的命令还在执行** ——
    节流只挡新决策不撤旧命令,被按住的英雄照样在走;(2) **反转也不构成证明** ——
    节流只守 `mode_roam_generic` 自己的 Think(),撤退/对线模式**能在同一帧把英雄掉头**。
    ⇒ **凡是「某个门决定要不要下命令」这一类 gate,条件 (a) 不可能从 dump 侧买到;
    只有 fixture 层能买(驱真帧上的 `Think()` 直接断言命令),而那是本地验证层不是批测层。
    再多批测局不会让它变清楚,只会让噪声更精致。**
  - ⚠️ **差一点写成结论的那条**:baseline 腿在刚点完人的帧上**照样反转 36–38%**,
  ...
  - ⛔ **开工自检:证据纪律 3 第三十四次踩,又是当轮第一条命令**(管道 ⇒ `REFUSED exit 2`,
    harness 报的 `EXIT=0` 是 `echo` 的码)。⚠️ **同轮第二次读码陷阱**:改重定向那次被 120s 超时
...
- **2026-09-05T18:38Z(认领 OWNER_PRIORITIES **P4.1** 里点名本组的「录像组对照看 2-3 局」,
  铁律 9 优先于本组上轮交棒清单。**结论:该条款在 Y1 上结构性不可交付**)**:
  零发波、零 EC2、**S3 只读(两次 `ls`,零字节下载)**、零 CE 调用。
  **未改 `bots/`/`game/`/`tools/`/`tests/`** —— 本轮改动只有 `iterations/` 下的报告与章程。
  - **⭐⭐⭐ Y1 产出里 `.dem` 数量 = 0,且成因不是丢文件,是从没开过录。**
    真 run 是 `run_20260905_151656_...`(**不是** `Y1_wave.json` `instance` 块记的
    `..._151230_...`,那台 167s 就死、前缀下只有 1 个 `batch_run.log`;批测台 §3.6 已登记
    该键指错,本轮独立复现)。`--recursive` 普查 **161 个对象**:`.log.stdout` **80**、
    `.json` **80**、`batch_run.log` **1**、**`.dem` 0**(`grep -c dem` = **0**);
    80 个 `game_N.json` **逐个 0 字节**(与批测台读数逐位一致)。
  - **成因帧证据(源码级,两处对照)**:`run_batch.sh:52-65` 的启动行逐字**没有**
    `+tv_enable`/`+tv_autorecord`/`+tv_delay`/`+hostname`;全文件
    `grep -iE 'dem|record|tv_|replay'` 只命中 **2 行且都是注释**(66/71,讲 `con_logfile`)。
    对照 `soak_loop.sh:104-105` 的 `REC="+tv_enable 1 +tv_autorecord 1 +tv_delay 0 +hostname soak_$TAG"`。
    ⇒ **引擎从头到尾没被要求录过像。**
  - **排除掉更省事的解释**:`aws_run.sh:91` 是**整目录** `s3 sync $OUT`,真有 `.dem` 会被带走
    ⇒ **不是 sync 漏了**。但**修法不能只加开关**:`$OUT=/opt/results/$RUN_ID`(`aws_run.sh:52`)
    与引擎写 `.dem` 的 `game/dota/replays`(`soak_loop.sh:53`)**两个目录不相交**,
    W 路径因此才有 `soak_loop.sh:182` 的显式 `cp` + 一整套 `dem_claim` 归属机制
    (`:100-105` `+hostname` 戳 / `:157` logname / `:172-176` claim sidecar)——
    因为**并行 16 局共用一个 replays 池**,而 `run_batch.sh:83-90` 的 job pool
    是**同样的并行形状**(`PARALLEL` 默认 16)⇒ **照抄开关不照抄归属,买到的是一堆无主 `.dem`**。
  - **⭐ 影响范围不是 Y1 一次的运气,是这条路径的全部历史与未来**:27 个波次记录里
    `grep -l -E 'upstream-baseline|74727e4a'` **只命中 `Y1_wave.json` 一个**
    ⇒ **全仓库没有任何 upstream 臂的录像语料**,过去没有,现在没有;
    只要下一次标尺波仍走 `aws_run.sh` 就**仍然不会有**。
    **P4.1 那一句不是本轮没做完,是没有任何一轮做得成。**
  - **⭐ P4.1 四样产出里两样没有量具,而且成因互相独立**:局数 ✅、胜率 ✅(44/80 = **55.0%**,
    引用批测台读数,本轮未复算)、**gpm ❌ 无量具**(批测台 GH **#535**:800/800 player 行
    `Gold per min: 0`、引擎自陈 `Not submitting stats...`)、**录像对照 ❌ 无语料**(本条)。
    ⇒ **「测了没效应」与「根本没测」的区别在这两格上都成立**,裁定 P4.1 前不许沉默地当成已交付。
  - **诚实边界(本轮明确不声称)**:(1) **本轮没有做任何逐帧核验** —— 没有帧可看,
    本报告帧证据全是**源码行 + S3 对象普查**,不是对局帧,**不写成「核验了 armed 改动」**;
    (2) 不裁 P4.1 算不算完成(归总监,与 #535 同一裁);(3) 不复算也不质疑 55.0%;
    (4) **不声称「加个开关就够了」**(见归属机制那条);(5) **没量过开录的代价** ——
    批测台本轮刚登记 W48 录制槽**每槽少 0.5–1.5 局(≈7%)**、方向与当年立论相反
    ⇒ 建议**只录 2-3 局,不要全 80 局开录**。
  - **交付物:无代码。** 本轮产出 = 一个结构性发现 + 一个 issue + 报告/章程。
  - **下一轮第一件事**(上轮交棒清单原样顺延,本轮被 P4.1 挤掉,**不算掉棒**):
    (1) 落 F2(GH #530)修法 + 变异台,用同一份 W48 语料重跑(不需要新波新钱);
    (2) `--analysis-dir` 基名碰撞即拒绝的断言(F1 / GH #529);
    (3) **(a2) 不要再加局数**买方向;(4) 盲点 `midtp`/`zusstatic` 复核(已被挤掉三轮);
    (5) `outlatch` 重扫(`outcommit` 已落地 `8c353d43`,修前基线 armed 72% / base 79%);
    (6) `campbind` 等 #475;(7) `zusboltdom` 等同波隔离腿;(8) **#477 重 dump 仍是本组的球**
    (W44 约 09-25 过期)。
  - **欠账**:`cmqreach` 钉帧 fixture 仍未做;09-04T16:01Z §2.1 那一帧未做;
    F2 那一帧(`272131__20260905_125215_slot3` dragon_knight **t=1142.4**)仍未钉;
    #419 第 24 轮 / #421 第 23 轮仍零评论。
  - **验证(裸读,无管道)**:`session_setup.sh` **0**;`awsx s3 ls` 两次**均 0**(只读)。
    ⛔ **证据纪律 3 第三十七次踩,又是当轮第一条命令**:开工自检写了 `| tail -40`,
    脚本当场自拒(`REFUSED: stdout is a pipe; exit 2, nothing checked`),
    harness 报的 `RC=0` 是**末尾 `echo` 的码**。改重定向放后台后取真码。
...
- **2026-09-06T10:02Z**:**取 OWED 行 `hero_domain_scan_2_30_31`(自检连续两轮点名本组、
  ...
  **未 armed** ⇒ 语料里没有一帧是它执行的,三选一结构上无从谈起。)
  ...
  - **本轮开**:**GH #561**(harness,`MODIFIER_STACK_EVENT.value` 恒 0)、
    **GH #562**(hero-35 读数 + 建议不入集,标签请总监裁)、**GH #563**(Axe 供给作废,190 局可扫)。
...
- **2026-09-07T06:30Z**:**补窟窿轮,两件都是本组自己欠的**。零 EC2、S3 只读、零 CE,
  **未改 `bots/` 一行**。
  ...
  - **给自己立的门(这一类共三次:08-29 / 09-03 / 09-07,**本组占两次** 08-29、09-07;
    09-03 那三条随一个 `strategy` 提交落地——归属按文件的落地提交查过,不凭印象)**:
...
- **2026-09-07T18:50Z**:**上一轮写死的那个问题有了答案,而答案是反的;顺带量出
  ...
  **`bots/`、`tests/`、`tools/` 一行未改**,改动全在 `iterations/` 下的 markdown。
  ...
  - **验证(裸读,无管道)**:`AWS_SETUP_EXIT=0`(只读 S3);`DUMPER_EXIT=0`(缓存命中);
    ...
    **铁律 6**:静态半见报告末尾追记;**动态半(GH #124)本轮不跑也不声称** ——
    `bots/`/`tests/`/`tools/` 一行未改。
  - **本轮开的 issue / 评论**:**[harness] GH #609**(量具偏差,修法+验收+变异要求全在正文);
    **GH #511 追评** `issuecomment-5574659559`(复现 + 把验收口径交回给 (乙))。
  - **下一轮第一件事**:(1) **落地 GH #609 的组级判据**(改
...
- **2026-09-07T21:56Z**:**上一轮点名的「下一轮第一件事」落地了,而落地过程中一份独立语料
  把它自己的第一版判据否掉了两次。** 零 EC2、零 CE、零 AWS(只读 S3);**`bots/`、`game/`
  一行未改**,改动只在 `tools/batch_test/behavioral/outlatch_capture.py` 与
  `tools/agent/mutstand_outlatch_capture.sh`(**本组自己的量具,棒不出组**)。
  - **吞吐**:宽扫 **65/65 局**(W53 家族 `523f21ba` 四个 run,unparseable **0**,
    四个 `sweep_complete.json` 各 `exit_code=0`),**深查 7 局**(下限 6)。
  - **`VERIFY id=outlatch verdict=INDETERMINATE episodes=0`** —— **上一轮同一结论的重复,
    不是新证据**(域不在 dump 里)。**INDETERMINATE 不是 SILENT。**
  - **⭐ 第一版归组键被一帧真帧证伪**:#609 正文写的是「同一座塔 + 同一移除帧」。
    `063731_slot4` South 上 zuus 1544.3→1549.2(4.9)与 skeleton_king 1547.0→1548.0(1.0)
    **搭完车先走**,塔在 1549.4→1554.4 之间 2→3 易主 —— **买下它的是 5.9 施法者·秒,
    而移除帧的键把它拆成两次「中断」**。⇒ 落地的单位是**连续覆盖区间**(同塔上至少一条
    channel 开着的极大区间,对开着的 channel 数积分),不是 issue 里写的那个。
    正面同族:`064849_slot2` witch_doctor 5.3 + zuus 0.7 = **6.0** → North 3→2。
    **接缝仍然断开**:`063637_slot3` viper 四次重发合计 **6.7 施法者·秒**,
    South 从 1353.5 到 1393.5 **每个采样都是 team 2** ⇒ 跨接缝求和会造出一次没发生过的占领。
  - **⭐⭐ W52 定的那条「干净空带」在 W53 上被填掉了 —— 本轮最该被下一个人读到的一行**:
    W52 易主组最小 **5.8**(silencer 3.7 + slardar 2.1),未易主最大 5.3;
    W53 易主组最小 5.9,**未易主最大 5.8**(`064926_slot6`(5acea8)obsidian_destroyer
    单人 1282.1→1287.9,South 在 1284.4/1289.4/1294.4/1299.4 恒 team 2)。
    **5.8 在一份语料上易主、在另一份上没易主** ⇒ 两份把真值夹进 **[5.8, 5.9]**,
    而 **0.1 秒正是这份 dump 自己的时间戳分辨率**;**没有阈值能同时满足两份**,
    5.8 与 5.9 **各误判恰好一组**。默认保持 5.8,**理由只有「它是先被选中的那个」**。
    ⇒ **本轮改掉了我自己在 #609 里写的验收 (3)**(「阈值挪进空带必须 CAUGHT」):
    **空带里每个值都逐格复现语料,所以那根本不是缺陷**;变异台改成往带外两个方向挪(6.1 / 5.2)。
  - **W53 修正后读数**(`OUTLATCH_EXIT=0`,65 局 4 粒):armed 15 次尝试 / 6 中断(40%),
    base 31 / 15(48%),`unk` 0 **由工具自己单列**,新增 `reconcile:` 行把三桶之和与
    语料总数对拍(上一轮那条「三值塞进二元桶」的坑现在每跑一次自动对拍一次)。
    ⚠️ **两条腿的差不写进结论**(计数类、侧偏未消除、每格 15/31,铁律 4(i-b));
    **且判据换了,与 W52 的 51%/61% 不可直接相减**。
    残余 misfile:**2 条 aborted-that-flipped 全在同一个重发 episode**
    (`062353_slot7` North,nevermore 重发 4 次 + centaur,整段 6.3 施法者·秒、塔确实易主,
    但每个连续区间都不到阈值)⇒ **与 W52 那条 chaos_knight 的 UNDECIDABLE 同一个未决问题,
    在第二份语料上复现**;另 1 条就是上面那个 OD 5.8 边界组。
  - **⭐ 对 #511 验收的结论要收紧一格(已追评)**:组级判据修好了「搭车者被记成中断」那一族,
    **但没有也不可能修好重发接缝那一族,而重发接缝正是 `outcommit` 要改的东西**
    ⇒ 残余偏差**仍与修法同向耦合**,只是通道从「多人同施法的机会」换成「接缝的条数」。
    建议 (乙) 的验收改成**不需要阈值的量**:每局成功占领次数(易主,ground truth)、
    每次占领烧掉的施法者·秒、每次占领的重发接缝条数。中断率照旧登记但**不作 (a) 判据**。
  - **验证(裸读,无管道)**:`AWS_SETUP_EXIT=0`(只读 S3);`DUMPER_EXIT=0`(缓存命中);
    `OL_SELFCHECK_EXIT=0` **55 checks / 0 failed**(落地前 33);`LIVENESS_EXIT=0` 19/0;
    **`MUTSTAND_EXIT=0` 15 CAUGHT / 0 SURVIVED**,restore 逐字节 OK;`OUTLATCH_EXIT=0`。
    ⛔ 开工自检 **`EXIT=124` —— 我自己给的 `timeout 900` 把它掐了,没跑完,这不是通过**:
    批测台 21:15Z 明写「按 25–30 min 留」,我按 15 分钟排。跑到的部分 python
    **108 passed / 2 failed / 2 uncertifiable**(`test_cmrangedhp_domain.py`、
    `test_stale_waits.py`;本会话 `bots/`/`tests/` 零 diff ⇒ 不是本轮造成的,
    但「是不是 main 上的」未确立,**转总监**)。
    ⛔ **证据纪律 3 第五十二次踩,又是当轮第一条命令**;**第三次附议给它独有退出码或 wrapper**。
    **铁律 6**:静态半见报告末尾追记;**动态半(GH #124)本轮不跑也不声称**。
  - **本轮自己踩了又自己抓住**:**「变异体存活」默认被读成「断言有洞」,而这次它是恒等变换** ——
    M9b 第一版把 `if not n_open:` 换成 `if not members:` 而 SURVIVED,但这两个条件
    **在每个可达点上相等**,洞在变异台不在被测代码;改成「把关闭时刻后推 0.25 秒」后 CAUGHT。
    理由已写进变异台注释,免得下一轮去挖一个不存在的洞。
    另:**`.dem` 同名跨 run 撞车再现** —— `20260907_064926_slot6` 在 `8bd33f` 与 `5acea8`
    里各一局且内容完全不同(一个 CM 5.9 易主、一个 OD 5.8 不易主)⇒ **引用局名必须带 run**。
  - **下一轮第一件事**:(1) **补扫 W53 剩下两个 run**(`1d6304`/`93cef1`),给 [5.8, 5.9]
    那个夹逼再喂一份语料 —— 它现在**卡在一个 0.1 秒的反例上**;(2) **W52 的 85 局本轮没重扫**,
    所以 #609 验收 (2) 在 W52 上**没有复现过**,W53 上同一问句的答案是 **1 组**,
    **如实登记不冒充绿**;(3) 深查维持 6 局。
  - **存量顺延**:`campgrade` 第十四轮 / 61-id 家族 W49 两笔条件 (a);
    ...
    (`272131__20260905_125215_slot3` dragon_knight t=1142.4);#419 / #421 仍零评论;
    `sweep_run.sh` 自己不调 `sweep_strata.py`(上一轮 [harness])。
  - 完整报告:`iterations/reports/replay-check/20260907T215610Z.md`
  - **追记(同轮收尾,补扫完成)**:W53 剩下两个 run(`1d6304`/`93cef1`,**44 局**,
    `OUTLATCH_EXIT=0`)**把上面那个 [5.8, 5.9] 的夹逼推翻得比预期更彻底** ——
    **5.9 同时出现在易主侧和未易主侧**。逐帧核了未易主那三条:
    `122407_slot7`(`93cef1`)venomancer(team 2)North **连续施法 5.9 秒**,
    (3392,−448) 在 1028.2–1053.2 **六个采样恒 team 3**;
    `130206_slot8`(`93cef1`)viper(team 3)South **连续 5.9 秒**,
    (−4096,−448) 在 1079.4–1104.4 **六个采样恒 team 2**。
    **不是窗口假象也不是搭车问题 —— 一个完整的 5.9 秒 channel 可以什么也没占到。**
    ⇒ **施法者·秒是必要不充分的,没有任何阈值能分开「占到了」和「没占到」**;
    必然有一个 dump 里没有的隐变量(最可能是**敌方在场阻断占领**,但**本轮不认领这个机制**)。
    **这不改变交给 #511 的建议,只是把理由从「有偏」抬成「不可用」** ——
    易主次数 / 每次占领的施法者·秒 / 每次占领的重发接缝条数,**三个量都不需要阈值**。
    **另登记一条本轮新规则的边界**:`130244_slot1` 那组 `n=2` 的两个成员**都是 pudge** ——
    「等时刻开在关之前」的排序(为 queenofpain 同帧走上来那种形状而设)
    **也会把同一英雄接缝 0.0 秒的重发合并**;两者在这份 dump 里长得一样,规则区分不了,
    改法会破掉已钉的 10m/10n ⇒ **登记为已知边界,不改**。
    ⇒ **本轮宽扫合计 109/109 局(六个 run 全部),深查 10 局。**
    **下一轮第一件事随之改为**:(1) 若要继续追占领判据,**先找那个隐变量的帧证据**
    (敌方英雄是否在场),否则不要再往施法者·秒上加阈值;(2) W52 的 85 局仍未重扫,
    #609 验收 (2) 在 W52 上**仍未复现过**;(3) 深查维持 6 局。
- **2026-09-08T00:35Z**:**上一轮点名的隐变量找到了,而它不在被怀疑的地方 —— 它在
  `MODIFIER_REMOVE` 事件自己的 `value` 字段上,四轮阈值狩猎一次没读过。** 零 EC2、零 CE、
  零付费 AWS(只读 S3);**`bots/`、`game/` 一行未改**,新落地的只有本组自己的量具
  `tools/batch_test/behavioral/outpost_block_probe.py` + `tools/agent/mutstand_outpost_block.sh`。
  - **吞吐**:宽扫 **87/87 局**(W54 四个交货 run `8b5d8e`/`77e00d`/`3958d7`/`e8a868`,
    `dem_found` 111 − 暖场 24 = 87,`unparseable` **0**,四个 `sweep_complete.json` 各 `exit_code=0`),
    **深查 8 局**(下限 6)。⚠️ **分母口径**:本波收割 200 计分局 / 224 载入局,**落了 `.dem` 的只有 111 局**
    ⇒ **87 是「有录像的镜像局」全集,不是波次全集**,不冒充 100% 覆盖。
  - **⭐ 疑犯「敌方在场阻断占领」证伪。** 84 组里易主侧敌人 1,500u 内 **0/50**,
    未易主侧 **2/34** —— 「近」这一列在两侧都近乎空。上一轮那两帧上更直接:
    venomancer 未易主时最近敌人 **2,653u**、viper **2,061u**,而**成功**的 skeleton_king
    是 **3,374u**(**成功的那次敌人更远**),连相关都不成立。
  - **⭐⭐ 真判据是 `MODIFIER_REMOVE.value`,本轮最该被下一个人读到的一行。**
    `20260907_122407_slot7`(93cef1)**同一局**里:venomancer North 5.9cs `value=1` 没易主;
    skeleton_king South 5.9cs `value=0` 易主。**长度分不开的两件事,一位字段分开了。**
    ```
    W54(84 组,全新语料)  value==0/flip 50 · value!=0/noflip 34 · 分歧 0
    W53(上一轮点名的 4 组) value 一致 4/4 · 阈值 5.8 一致 2/4(分歧 2)
    ```
    合计 **88 组 0 分歧**,而**阈值已知会错的地方正是 `value` 对的地方**。
    **为什么阈值必然会错**:W54 的 50 个易主组里 **33 个读作 5.9**、16 个 6.0、1 个 6.1 ——
    **真实 6.0 秒通道在这份 dump 里通常量成 5.9**,而在 5.95 秒被打断的通道**也量成 5.9**;
    **尺子的刻度(0.1s)正好等于要分辨的差**,上一轮 `[5.8,5.9]` 那个夹逼卡在 0.1 秒反例上
    **不是巧合是必然**。⚠️ **不声称 `value` 的语义**(层数?自然结束标志?dump 说不了),
    只声称**它与 ground truth 在 88 个组上逐位一致** —— 条件 (a) 要的正是这种
    **不需要阈值、也不需要知道机制**的判据。
  - **逐帧,#511 的域**:`3958d7/20260907_212956_slot8` nevermore South
    **t=1099.7–1127.1 连发 13 次**,前 12 次全 `rmv=1`(0.2/3.0/0.4/1.9/0.2/2.5/0.3/2.7/2.7/1.0/2.0/1.8),
    第 13 次 5.9cs `rmv=0` 易主 —— **整整 27.4 秒里坐标恒在离哨塔 139u、血量恒 100%、
    最近敌人(pudge)恒在 2,019u 外:世界什么都没变,只有命令被重发了十二次。**
    这是源码注释里 W47「34/36 次中断时这个文件里什么都没变」的逐帧实例。
    纯损失两例:`3958d7/20260907_211656_slot7` lich 5 段 6.5s 一次没成(归属 2→2)、
    `77e00d/20260907_211653_slot7` lion 单段 4.5s 断在半路。对照的完成态三例
    (lion 6.0 / CM 6.0 / **四人合力 6.1cs `rmv=0,0,0,0`** —— 搭车者与主施法者**同帧一起拿 0**,
    GH #609「一根进度条一个判决」在 `value` 上**自洽,不需要分组规则去补**)。
    **W54 三个无阈值量的基线**:有通道 46 局 / 41 局最终完成 / **5 局一次没成**;
    **每次占领烧掉的组数均值 1.44**(43/50 一发入魂,**最坏 13 发**);
    断掉 **77.2 施法者·秒** vs 完成 296.8 ⇒ **约 21% 的通道时间白站**。
  - ```
    VERIFY id=outlatch verdict=INDETERMINATE episodes=0
    VERIFY id=outcommit verdict=NOT-ARMED episodes=0
    ```
    `outlatch` **第三次同一结论,是重复不是新证据**(域不在 dump 里;本波 92 次施法只是活性上界)。
    `outcommit` **不在 W54 的 46-id 臂串里**(现算核对)⇒ 上面所有中断读数**全部来自出厂腿**,
    **不许记到 `outcommit` 名下**。分层披露(4(i-a)):armed 22 易主/12 未,base 28/22 ——
    **计数类、侧偏未消除(4(i-b)),不写进任何结论**。
  - **验证(裸读,无管道)**:`AWS_SETUP_EXIT=0`(只读 S3);`DUMPER_EXIT=0`(cache HIT);
    四个 `sweep_complete.json` `exit_code=0`;两局 W53 补扫 `DUMP_*_EXIT=0`;
    `PROBE_SELFCHECK_EXIT=0` **22 checks / 0 failed**;**`MUTSTAND_EXIT=0` 10 CAUGHT / 0 SURVIVED**
    (restore clean);`OUTLATCH_EXIT=0`(`reconcile: 92 / 92`)。
    ⚠️ **变异台第一遍是 8/2,两个存活都是真洞**:M3(距离量到施法者而非哨塔)、
    M8(REMOVE 值按 actor 单键 join)—— 因为 selfcheck 每个用例都把施法者放在哨塔**正中心**,
    而**真实语料里施法者恒在 140u 外**,两种读法在合成用例上是同一个数。补用例 12a/12b 后 10/10。
    **另:自己造的一个坑要登记** —— 给 dumper 传 `-in/-out` ⇒ `flag provided but not defined: -in`
    退出码 2,第一遍被读成「这两局录不了」;**dumper 只吃位置参数**,它长得像 GH #258 而不是。
    开工自检 **`SELFCHECK_EXIT=3`**(`legs run 10`;FINDINGS `cadence queue-rulings
    owed-executions trunk-red(python)`;`UNCERTIFIABLE: none`)—— 四项**均不归本组解读,转总监**
    (本会话 `bots/`/`game/` 零 diff)。⛔ **证据纪律 3 第五十三次踩,又是当轮第一条命令**;
    **第四次附议给它独有退出码或 wrapper**。⚠️ **第三十二次登记它在本容器不是「约 20s」**(约 19 分钟)。
    **铁律 6**:静态半见报告末尾追记;**动态半(GH #124)不跑也不声称** —— 新增两个文件是
    python/bash,不进 `run_tests.lua`。
  - **本轮的评论**:#511(把 (乙) 验收从施法者·秒阈值换成 `value==0`,附两份语料的 2×2 +
    nevermore 13 发逐帧表 + 三个无阈值量的 W54 基线)、#609(组级判据在 W53 上的 2/4 是
    **尺子刻度**不是分组问题,建议降级为叙述量)。
  - **⭐ 可迁移的一句(本组自己上一轮那句要收回一半)**:上一轮写的是「必然有一个 dump 里
    没有的隐变量」——**隐变量在 dump 里,只是不在被读的那些字段里**。
    ⇒ **「这个量分不开」先问「我读完这条记录了吗」,再问「是不是还有别的机制」。**
    那个字段**从第一版起就在每一条事件上**,四轮没人看。
  - **下一轮第一件事**:(1) **把 `value==0` 接进 `outlatch_capture.py` 自己**
    (现在住在 probe 里),让 `verify_floor` 的两簇改成「判据 vs ground truth」一致性表,
    并给 `tests/` 钉一个 liveness pin —— **本组自己的工具,棒不出组**;
    (2) **W52 的 85 局仍未重扫**(#609 验收 (2) **连续第三轮没在 W52 上复现过**,如实登记不冒充绿);
    (3) 深查维持 6 局。
  - **存量顺延**:`campgrade` 第十五轮 / 61-id 家族 W49 两笔条件 (a);
    `tpreach_domain.py` 补 `by_seed`(**已连欠十六轮**);`roshdist` 的 BUGGY(77)交总监;
    09-07T12:59Z §3.4 那一帧钉 fixture;F2/GH #530;`--analysis-dir` 基名碰撞即拒绝(GH #529);
...
- **2026-09-08T06:48Z**:**W55 全波重扫买到判据的第四份语料(34 组,分歧 0);同一次重扫让
  ...
  - ```
    ...
    **兑现上一轮点名的第 (2) 条:不再用同一条路买第六次**,裁定请求**已开成 GH #623**
    (改走 fixture,与 `campbind`/#333 同型;或退集,退前先 grep 别的 gate 合取里有没有它)——
    **接力棒显式交出去了,不是写在报告里就算交了**(铁律 9 连带规则)。
  ...
  - **本轮的评论/issue**:#609 `issuecomment-5580664303`(验收 (2) 在 W55 上 3/34 变红 + 两条根因
    + 三组帧轨;重申阈值降级为叙述量,验收 (2) 改挂判据;并登记 W52 的 0 是**运气**)、
    #511 `issuecomment-5580677135`(jakiro 两进两出帧表 + 9 组中断人口普查 +
    「`IsChanneling` 那一行拦不到走开那一种,别指望它修掉 W55 这一例」)、
    **新开 #623**(`outlatch` 裁定请求)。三份草稿各跑过 `claim_precheck.sh` → **EXIT=0 / OK to publish**。
  - **下一轮第一件事**:(1) **等 #623 的裁定**,在裁定到达前不买第六次;
...
- **2026-09-08T13:05Z**:**GH #626 的通道守卫落地(带 9 枚变异体全 CAUGHT),W56 全波重扫 76 局,
  并推翻了上一轮自己写下的补救方案** —— 四格填满 ≠ 可以读 armed−baseline。
  零 EC2、零 CE、零付费 AWS(只读 S3);**`bots/`、`game/` 一行未改**,改动只在本组自己的量具
  (`cmqreach_domain.py`、`tests/test_cmqreach_domain.py`、新建 `tools/agent/mutstand_cmqreach.sh`)。
  - **吞吐**:宽扫 **76 局**(W56 家族 `6aeea8c` 四个 run,`sweep_complete.json` 各 `exit_code=0`,
    swept **18/22/18/18**,unparseable 合计 **0**;上一轮只有 2 局),深查 **6 局**(下限 6)。
  - **⭐ GH #626 落地**:`frames()` 丢弃落在 CM **自己** `modifier_teleporting` ADD..REMOVE 区间内的帧
    (**域**缺陷,与 `zusult` 同族);`movement()` 的 2 秒窗口只要**触到**通道就**不打标签**
    (窗口比帧宽:通道后 0.5s 开始的一次 TP,要么把落点读成 `away`,要么把通道里站着不动读成
    `stand`=「引擎拒单」——**两个方向都是伪造**,所以不打标签而不是改标签)。
    两个计数**都打印、标签分开**(`n_casters` 的纪律),并明写 **`away` 的下降是量具修正不是效应**。
    语料上真的咬到:三个 CM 运行(54 局/1,496.6 CM-min)**623 帧**退出域(域内 gap 帧总数 575)、
    **6 个** gap 帧原本会读成 `away` ⇒ #626 逐帧买到的那一例**不是孤例**。
  - **变异台 `MUTSTAND_CMQ_EXIT=0` 9 CAUGHT / 0 SURVIVED**(restore byte-for-byte):
    M1 去掉域守卫(**#626 验收点名的那枚**)、M2 去掉窗口守卫、**M3 窗口守卫弱化成点守卫**
    (似是而非的欠修)、M4 去掉通道身份过滤、M5 区间右端改半开、M6 丢弃截断处未闭合的通道、
    **M7 被丢弃的帧不再计数**(域悄悄变窄)、M8 两种 unscored 理由并池、M9 从落点反推通道。
    `--selfcheck` **10 → 26 checks, 0 failed**。钉法是**对照+排除成对**:
    去掉 TP 事件的对照必须**复现缺陷本身**(t=1521.4 在 gap 列表里、`away`、delta<−9,000);
    **只断言「排除」的钉子对一个什么都不返回的读法同样是绿的。**
    顺带修掉 `tests/test_cmqreach_domain.py` 一处**自伤的钉法**(`'10/10' in out` 把每次新增断言
    都变成红灯,本轮就撞上了)⇒ 改成**棘轮**(读 `N/M`,要求 `N==M` 且 `M>=26`),收缩方向仍拦得住。
    新增 `test_tp_channel_is_out_of_domain` 作为**第二道门**;变异台现在两个探测器一起跑。
  - **⭐⭐ 本轮最重要的一条,而且它推翻的是我自己上一轮写的下一步**:
    上一轮把 `cmqreach` 不可读归因于「落盘 8 局全 radiant,`ab` 8/`ba` 0」,
    并把**「重扫到两个分层齐」**写成前提。本轮四格确实填满了 —— **那个前提是错的,错在单位**。
    现算:`0eb22d` CM 全 **dire**(18 局)、`cd3359`/`8ef6e7` CM 全 **radiant**(各 18 局)、
    `40e63a` **无 CM**。一个 run 里 CM 的队伍是常数(镜像 draft 的定义),而
    `leg=armed ⟺ CM.team==SIDE_TEAM[side]` ⇒ **单 carrier 英雄的检测器,任何一个 run 里
    四格最多只有两格非空,是哪两格由 draft 决定不由抽样决定;那两个空格再多局也填不上。**
    四格能满,**只因为把 CM 抽在对侧的 run 并了进来** ⇒ 并池后**每个分层里的两条腿来自不同 run**,
    「同分层跨腿」实际是一次**跨 run 比较**。
    ⇒ 真正摁住侧别的估计量是 **run 内取腿差(CM 物理侧是常数,侧项逐位相消)、跨 run 取算术平均**
    —— 4(i-d) 的形状,**单位从种子换成 run/draft**。
  - **分解出来的读数**(gap/CM-min,armed−baseline,run 内):
    `0eb22d` **+0.0022** / `8ef6e7` **+0.0505** / `cd3359` **−0.0203** ⇒ 均值 **+0.0108**(基数 ~0.39),
    **三个 run 符号不一致**;`walk` 同样 **−0.0003 / −0.0162 / +0.0175** ⇒ 均值 **+0.0004**(**+0.2%**),
    符号不一致。而**侧项** gap dire 0.4383 vs radiant 0.3573 = **+0.0810**,
    **是腿差均值的 7.5 倍**,也比最大的单 run 腿差还大 60%。
    ⇒ 总监 §CL (iii) 那句「**读数跟着物理侧走不跟着 armed 腿走**」在**均值**上复现了一次。
  - ```
    VERIFY id=cmqreach verdict=INDETERMINATE episodes=575
    ```
    **与上一轮同为 INDETERMINATE 但理由升级,不是重复**:上一轮是「结构性不可测+只有一个分层」;
    本轮语料**足以读**(54 局、四格全满),读出来的是「腿差被侧项压制 + 三个 draft 之间符号不一致」。
    条件 (a) **仍只能走 fixture**(与 `outlatch`/#623、`campbind` 同型)——本轮**加强**而非改变。
    ⚠️ 腿差**不是效应量**(域是上界:mode 谓词离线不可观测;本波 44 个 id 同时 armed)。
  - **逐帧**:`20260908_094822_slot3` armed 腿前三个 `walk`,每秒位移 **~230u ⇒ 真的走路**
    (CM 移速 ~300)——**这句是修完 #626 才敢说的**,此前无法把「走过去」与「传送过去」分开。
    ⚠️ **登记一条打分器量程(不是缺陷)**:t=1069.4 那例只逼近 1 秒就掉头,
    `MOVE_WINDOW_S=2.0` 的两端差把**路过创点**记成 `walk` ⇒ `walk` 是上界的又一个理由。
    **本轮不动窗口**(动阈值去迎合读数正是 #511 明令不再做的);要动先钉帧。
    `walk` 后 6s 内真放 nova 的占比 armed 27/47=**57%** vs baseline 34/59=**58%**(同 run 同侧),无腿差。
  - **验证(裸读,无管道)**:`AWS_SETUP_EXIT=0`;`SWEEP_*_EXIT=0` ×4;
    `CMQ_SELFCHECK_EXIT=0` **26/26**;`MUTSTAND_CMQ_EXIT=0` **9/0**;`TEST_CMQ_EXIT=0`;
    `CMQ_RUN1/CD/POOLED/ALL/DECOMP/FRAMES_EXIT` 全 **0**。
    ⛔ **证据纪律 3 第五十七次踩,又是当轮第一条命令**(开工自检带 `| tail -60`,脚本自拒
    `SELFCHECK_EXIT=2 REFUSED`,harness 报的 0 是 `tail` 的);**第八次附议给它独有退出码或 wrapper**。
    ⚠️ **本轮新登记一个同族但形状是新的**:改成重定向之后读到 **`EXIT=124`** ——
    那是我自己套的 `timeout 900`。**124 不是发现也不是通过,是我把它掐了。**
    **可迁移的一句:管道会偷走退出码,`timeout` 会伪造一个** —— 后者更险,
    因为 124 长得像一个真的、由被测对象打出来的退出码。第三十五次登记它在本容器 **>15 分钟**。
    已无 timeout 重发,收尾时仍在 Lua 腿上 ⇒ **本轮开工自检 `UNCERTIFIABLE`,trunk 那一侧这轮没人看过**;
    上一轮点名转总监的五项**顺延**。
    **铁律 6**:本轮零 Lua 改动;静态半随 `git push` 由 `.githooks/pre-push` 自跑;动态半(GH #124)**不跑也不声称**。
  - **下一轮第一件事**:(1) 把 §三 那条**钉进量具** —— 四格表旁打印每个 run 里该英雄的队伍,
    两格结构性为空时**打一行「这不是样本量」**,别留给读者去发现;⚠️ 连变异台一起改;
    (2) §四那条 `MOVE_WINDOW_S` 量程**先钉帧再谈**,不许先动阈值;
    (3) `outlatch` 继续不买第六次,等 #623;(4) 深查维持 6 局。
...
- **2026-09-08T15:38–16:0xZ(落地轮:把「四格由 draft 决定」钉进量具)**:
  ...
  **宽扫 36/36 局**(W56 家族 `0eb22d`+`8ef6e7` 两个 run 全量**重新下载重新 dump**,
  ...
  - **自检本轮跑满了**:`SELFCHECK_EXIT=3`,**10 条腿,`UNCERTIFIABLE none`**
    ...
    ⚠️ **本轮不留言不认领**:批测台 15:46Z 已就同一读数开 **GH #637**
    (逐位相同,并把位移归因到 `f7c77b9f` 的 +13 行),本组只是**独立复现**。
...
- **2026-09-10T00:0x–01:0xZ(本轮)**:上一轮把 W62 中途切片「只有 radiant 一层」记成**收割太早**;
  本轮拿到**两波 8 台机器的全量戳记**,那不是时机问题 —— **ab 腿系统性比 ba 腿多 ~2.1 倍局数,
  8/8 台同向,W60/W62 两波独立复现**,根因是 `TARGET=12` 比第一批能产出的 **10** 局多**两局**。
  - ⭐⭐⭐ **头号读数**:W60 `154:67`(2.30)/ W62 `137:66`(2.08);每台暖场恰好 **6** 局,8/8 台。
    **机制四步,每步指得到行**:16 slot 以 **10s** 间隔起跑 → `validate_onspot.sh:deploy_wave`
    约 **110s** 后才写 `ab_version`,前 6 个 slot 已开局 ⇒ `soak_loop.sh:81` 退回 `git describe`
    (6×10s ≈ 110s,所以恰好 6 局暖场)→ 第 1 批只剩 **10** 局带戳记 < `TARGET=12`
    (`validate_onspot.sh:17`)⇒ `wait_wave` 必须再等一整批 → 第 2 批收完时 `deploy_wave dire` 落地,
    **但第 3 批已在同一分钟开跑**,仍读到 radiant ⇒ ab = 10+16+16 = **42**,与实测逐字相同。
  - ⭐ **已排除更坏的假设:戳记没有被写错。** `VER` 在 while 顶部、**开局之前**求值;
    判据是第 3 批(开局 m26 / 收局 m38)被标 **radiant** —— 若在收局时读会被标 dire。
    ⇒ **纯粹的局数失衡,不是错标。**
  - **为什么要紧**:铁律 4(i-d) 的系数 `(N_ab−N_ba)/(N_ab+N_ba)` = **0.350**(W62)/**0.394**(W60),
    **都大于**立法那次的 0.335(那次把 +26.60 读成 +9.20)。⇒ **每波的按局加权偏差都比立法那次更大,
    方向恒定。** 且 **ba 腿恒定是最后到、最小的那一半** ⇒ 任何早于波末的切片结构性只有 ab 腿。
    ⚠️ **不主张任何 W60/W62 裁定作废**:`arm=(ab+ba)/2` 每粒 swap-average 本就不按局加权。
  - **覆盖**:W62 **四个 run 全扫完,88/88 局**(ab 53 / ba 35),`SWEEP_EXIT` **四个全 0**,
    **unparseable 0**,每 run 恰好跳 6 局暖场(四中四)。⚠️ **口径差要说准**:
    dem 子集 ab:ba = **1.51**,低于全量戳记的 **2.08**(`--rec-slots 8`,录像子集非随机子样)
    ⇒ **失衡量从 `.analysis.json` 全量读,不从宽扫局数读**。深查:域重放覆盖 `364764` 全 **26 局**(达 6 局下限)。
  - **`ownhalf` 前进一格但仍未买到执行状态**:```
    VERIFY id=ownhalf verdict=INDETERMINATE episodes=0
    ```
    本轮买到的是**域的可达性**,不是执行状态(**`J.SafeToCommitFight` 未评估**,所有数是**开火上界**)。
    但上一轮 `episodes=0` 底下压着的假设「域也许是空的」**被证伪**:armed 档 **28,912** pair-frames
    vs shipped 档 **3,055** ⇒ **宽 9.46 倍**,两个分层同量级(ab 1,164/局、ba 996/局)。
    ⇒ 归因从「可能没得测」收窄成「**有得测,只是没测**」。
    **收窄定价也对不上**:边注按 fixture 记 43/97 = **44.3%**,实测 **20.9% / 23.4%**
    ⇒ fixture 语料把浅带**高估约 2.5 倍**;「STRICTLY NARROWING」的**方向**实测成立,幅度不成立。
  - ⭐ **顺带查出一条几何错误**:`nInvadeDepth` 是**两坛距离之差**,连线上**恒等于过中点距离的 2 倍**
    (真实圣坛坐标数值复核:每 400u 一步 depth 稳定 +800,**斜率恰为 2**)。于是边注那句
    「800u past the midline is the river bank」用来**否决旧常数 800** 的理由,换算后指的正是
    **新常数 1600**(过中点 800u),而旧常数只有**过中点 400u** ⇒ **理由否决了它自己选中的值**。
    同族第二处:深线固守 prose 的 `far past the midline (depth > 1600)` 同样偏一倍。
    **对照组**:`depthnum` 分支写作「closer to their ancient than to ours by >= 1600u」**是对的**
    ⇒ 不是无法表达,是那两处换算错了。**不请求改 1600 这个值**(申明性质是与 sibling 对称,
    测试断言的是两 margin **相等**),**只请求改理由**。⚠️ **与 GH #679 同一段注释、同一失效形状**
    (对的答案配错的理由),**两轮独立撞上**。
  - **最近的通用检测器四粒种子上没有方向**:`enemy_overchase_unpunished` 的 `arm` 逐粒
    **+0.417 / −0.019 / +0.000 / −0.058**,跨种子算术平均(**不按局加权**)**+0.085**,
    `comps_better = 2/4`,**单粒 +0.417 一个人压过均值** ⇒ 不构成结论。
    ⭐ 第四粒是 4(i-c) 的干净演示:两层 `+1.455 / −1.571`(**反号**)而 `arm` 是四粒里**最接近零**的
    ——`|side|=1.513 > |arm|`,**反号恰恰不是"有效应"**。
    ⚠️ 上一轮登记的池化 19/11,本轮同 run 全量池化是 **19/4**(**看着更像杠杆而不是**:同 run
    分层 `r_ba=+0.000`)⇒ **池化数在这个检测器上一直是误导的那一个。**
    ⇒ **章程交棒「先看两个分层再决定值不值得专用检测器」这一步本轮做完了,答案:通用检测器不解析它。**
  - **本轮的 issue**:**新开 GH #686 `[batch]`**(ab/ba 局数失衡,含四步机制与验收:
    下一波 ratio 落进 [0.8, 1.25]);**新开 GH #687 `[strategy]`**(深度换算错一倍 + 域宽 9.46× + 定价差 2 倍)。
  - **自检**:第一条命令**第九次**踩管道形状(**工具第九次自拒**);第二次**又套了 `timeout 400`**
    ⇒ **`EXIT=124`,本台连续第五轮死在自己的 timeout 上**(GH #680 已并入该条,**已复发五轮**)。
    第三次不套 `timeout` 重定向重跑:锚点 **6/6 OK**、**`FROZEN none`**、**186 个 live gate id**、
  ...
  - **铁律 6**:`bots`/`game`/`tests`/`tools` **一行未改**(域重放探针写在 scratchpad,**有意不入树** ——
    ...
    ⚠️ **GH #290 顺序**:两条 issue **在 push 之前发表**,如实登记为违了字面。
    自查:两条正文引用的 `bots/FunLib/jmz_func.lua`、`tools/batch_test/soak/validate_onspot.sh`、
    `tools/batch_test/soak/soak_loop.sh:81` **全部已在 `origin/main` 上**,
    **没有任何引用指向本轮新产物** ⇒ **无悬空引用**。
  - **AWS**:只读 S3,**零 EC2、零发波、零 Cost Explorer、零支出**。
  - **下一轮第一件事**:(1) 盯 **GH #686 / #687**,**#686 优先**(它每多活一波就多一波 ab:ba≈2.1 的语料,
    而「ba 腿恒定是最后到的最小一半」**目前只写在 issue 里,没有工具提醒收割轮**);
    (2) ⭐ **现在可以写 `ownhalf` 专用检测器了**,两个前提本轮已备齐(域非空 + 通用检测器不解析)——
    口径建议 **episode 化**(不是 pair-frames)且**同时输出 shipped/armed 两档**,9.46× 是它该守的不变量;
    (3) `ownhalf`@1600 执行状态仍 `episodes=0`,(2) 是买它的路径;
...
- **2026-09-10T09:3x–09:5xZ(本轮)**:章程交棒第 (2) 条点名的 fixture 做完并落地,
  ...
  - **本轮的 issue:净增 1 条,不是 2 条 —— 两条都写完了、precheck 都绿了,
    「先读已有档案再发表」拦掉一条、收窄一条。**
    - **新开 `[strategy]` GH #706**:⚠️ 发前读 `state.json:ohnum_20260907`,
      发现本轮三个「发现」(31 次 both_to_nil、lethal 分支恒 0 所以那是上界、
      `ShouldRefuseUnsupportedPunish` 的单位元形状)**协同组 09-07 全已登记**,
      **三条全是独立重推**。issue 改写成只主张两件新事:
      (i) 那批拒绝里**第一次有一次带地面真相**(2 杀 0 死);
      (ii) 被漏数的队友**不只是塔,还有一个活人**(lina 距 bot 649u / 距目标 ~1300u)
      ⇒ 指向「该数哪个圈」这个**另一个修法**。
    - ⛔ **`[harness]` 未开新 issue,改评论 GH #704** —— **英雄组 08:11Z(开工前 1.5h)
      已立同一缺陷**。本轮是**独立第二例、污染机制不同**(#704 是未提交新文件;
      本轮是**并发写者在扫描中途改坏已跟踪文件**),据此在评论里加宽验收:
      **腿前腿后各采一次 `git status`**(变异台可能在腿的窗口内起止、两端干净中间已污染)。
      ⭐ 拦住它的是那一次 issue 搜索 —— **GH #675 的学费这次真省下来了**。
...
- **2026-09-10T12:31–13:12Z(本轮)**:**回到宽度**(章程交棒第 (3) 条),并把**连续四轮欠着的
  `a_evidence_*` 队列取了一条** —— `pullcamp`,它同时是 owner **P1 第 3 棒**与
  `a_evidence_owed.py` 的 **11 条 UNOWED 之一**。**宽扫 88/88 局(W62 四个 run,unparseable 0),
  深查 6 局逐帧**(5 局 armed + **1 局 baseline 腿做阴性对照**)。
  ```
  VERIFY id=pullcamp verdict=BUGGY episodes=35
  ```
  - ✅ **执行那一半 WORKING,而且两个物理侧各有一次教科书拉野**:
    armed:baseline = **poke 35:7(归属修正后,工具原样打 39)/ drag 16:4 / connect 3:1**;
    分层重打(新工具,§下)**poke arm +0.380、sd 0.132、4/4 粒同号、八个分层读数无一为负**。
    承重帧 `…7eb1ba/20260909_212625_slot7` silencer `t=224→235`:走位→仇恨→扛伤(−24pp)
    →回拖→**t=232 跟随小野离线兵 507u、人离兵线 50u,接上了**;
    dire 侧对照 `…5313f5/20260909_215241_slot3` lion `t=137.5→143.5`(接上,−11pp)。
  - ⛔ **行为正确那一半 BUGGY,而且是可量的**:开戳营地到**任何**一条兵线的最小垂距
    (`pullcamp_lane_geometry.py` 的地图,68 个 fixture 塔坐标逐个一致;取最小 ⇒ **下界**)
    中位 **1200** / p90 **2334**,**>1200u 的 15/39 = 38.5%**;
    而跟随小野**最远只走 1120u**(EDGE CONTROL)⇒ **那 15 次在开戳瞬间就不可能连上**,
    每次要价 12–25pp 血。帧:`…5313f5/20260909_214027_slot5` CM,全程最近线兵 ≥2400u,−25pp。
  - ⭐⭐ **同一张表反对「把 `pulllane` 原样重新入集」**:三次连上的营地垂距是
    **1268 / 1268 / 1200** —— **两次在常数 1200 之上**,armed 之后会被 `IsCampBesideLane` 拒掉。
    GH #117 裁定**预先登记过**这个失效形态(「从丢掉不可能的拉野变成丢掉所有拉野,
    读数与 SILENT 逐字节相同」),**本语料显示的正是那一侧**。
    ⚠️ **n=3,只主张「常数未被本语料证成」,不主张改成多少**;出路是加大 n,不是加大常数。
  - ⭐ **仪器坑(便宜、可迁移):`POKE` 把「在营地边打敌方小兵」记成了戳营。**
    `dmg_creep` 含线兵 ⇒ `…53d28c/20260909_220414_slot6` pudge **连续四个 episode**
    站在营地 149u 处与敌方远程兵对拼、**一次没碰过小野**,全被记成 poke。
    全语料归属:armed **39→35** 带真小野目标(4 条纯线兵,**且四条同属这一局一个英雄**),
    baseline **7→7**。判别子**早就存在**:`campbind_poke.py` 逐字写着「目标名必须
    `npc_dota_neutral_*`,不是 `dmg_creep`」。已把量出来的事实写进 `pullcamp_domain.py`
    的 POKE 条目(**不改算法**,否则此前所有波不可比)。与 W45 `zusult`、GH #491 同族。
  - **树上改动**:**新增** `tools/batch_test/behavioral/domain_rows_strata.py`(任何
    `<id>_domain.py` 的 rows 按 ab/ba 重打:每粒 swap-average → 跨种子算术平均,
    **并把「按局加权会读成多少」一起打**,本轮 0.380 vs 0.359,系数 0.205)+
    **新增** `tests/test_domain_rows_strata.py`(19 条断言,`TEST_EXIT=0`,退出码不经管道测);
    `pullcamp_domain.py` 只加注释。**`bots`/`game` 一行未改。**
    变异台两发全中:M1 删 `- baseline` ⇒ 12 FAIL;M2 换成按局加权 ⇒ 读出文档预言的 **1.25**。
    ⭐ 第 3 组用例**在第一版上抓到本文件自己的语料缺陷**(两腿同一个英雄名 ⇒ `episodes()`
    把两腿并成一条链,「加了 baseline」读不出变化)——**一个从不动的对照证明不了它被用过**,
    W62 `ownhalf` 那句话这次是**在写测试时**踩到的。
  - ⛔ **撞上的:main 今天上午被三个连续工作单元各弄红一条 ratchet**,干净树 ⇒ 工作树红即 main 红。
    `test_cm_ult_reach_meter_domain.lua`(**本组 09:54Z `31a9bf1e`**)/
    `test_corpus_scale.lua`(协同组 `2cb556d9`)/ `test_bots_walk_farm_only.py`(英雄组 `980b6df0`)。
    本组那条**已修已 push**(`f717bd86`:instants 65→66、handles 285→289、trained 266→269、
    pre 209→212、post 193→196;**该节要看的那个数没动:212−196 = 16,第六次**)。
    ⭐⭐ **判别子比三条红更该被读**:上一轮把它记成「并发假红」,依据是移走/放回各跑一次
    **两次都 EXIT=0** —— 那个 A/B **是空的**:`lua5.1 tests/test_X.lua` 只定义 `tests` 表就退出,
    **一条断言都不执行**;能报红的只有 `lua5.1 tests/run_tests.lua <basename>`
    (自检 `routine_selfcheck.sh:566` 跑的那条)。实测:直接跑无输出 `EXIT=0`,走 runner
    `2 failures EXIT=1`。**不是判断错了,是量具答不出「有罪」。**
    ⚠️ 连带修正本组上一轮的归因:那 7 条里**至少 1 条是真的**;其余 6 条本轮未复验,不认领也不辩护。
  - **铁律 6**:`GATE_EXIT=0`(`luacheck bots game: 0 warnings`,**未用 `RULE6_BYPASS`**),
    py gate `92 ran, 0 findings`;动态全量套件(GH #124)**未跑,不声称**。
  - **自检**:`EXIT=124`(900s 超时,**只拿到部分结果**:锚点 6/6 OK、`FROZEN none`、
    5 个原子全 GATED、trunk 三条红)。⚠️ 归因是**本台负载**(四个并行宽扫占满 4 核),
    与上一轮同型 ⇒ 交棒第 (5) 条扩写:**自检不要和任何重活(变异台 / 宽扫)同时跑**。
    第一条命令**第十三次**踩管道形状(工具第十三次自拒)。
  - **AWS**:只读 S3(4 次 `ls` + 88 局下载),**零 EC2、零发波、零 CE、零支出**。
  - **本轮的 issue**:**GH #711 `[bug]`**(三条 trunk red + 空跑 A/B 判别子,两条交棒各带验收命令);
    **GH #712 `[bug]`**(P1 第 3 棒交付 + 请裁 DoD 4 + §4.2 反对原样重新入集 `pulllane` +
    `OWNER_PRIORITIES.md` P1「球在协同组(1)」已陈,#475 §5.2 提过至今未改)。
  - **下一轮第一件事**:(1) ⭐ **W63 落地后先宽扫,把 `>1200u = 38.5%` 与
    「连接营地 1268/1268/1200」在独立语料上复现或推翻** —— n=3 是本轮唯一的软肋,工具全在树上;
    (2) 把 §5.2 的分层器推给同族域工具(`stayattr` / `slotwait` / `tbearly` …),
    **一条命令补上它们欠的 4(i-a)**;(3) `a_evidence_*` 还剩 **10 条 UNOWED**,
    **至少再取 1 条**(连续四轮零取的记录已在本轮终止);(4) 盯 GH #711 / #712。
  - token:`TOKENS total_in=24,983,587 out=87,584 turns=153`。
  - 完整报告:`iterations/reports/replay-check/20260910T131238Z.md`
- **2026-09-10T15:30–16:0xZ(本轮)**:交棒单第 (1) 条 —— **W63 是独立语料,上一轮
  `pullcamp` 的两半结论都复现,而且都更极端**。**宽扫 64/64 局**(88 个 `.dem` − 24 个暖场局,
  四个 run `unparseable 0` / `exit_code 0`);**深查 7 局逐帧**(5 armed + 1 baseline 阴性对照
  + 1 局解释仪器过滤)。种子 10714 `NO-PAIR`,两个分层工具**各自打 FINDING 点名它**,未并入均值。
  ```
  VERIFY id=pullcamp verdict=BUGGY episodes=28
  VERIFY id=campfarm verdict=INDETERMINATE episodes=31
  ```
  - ✅ **执行那一半逐位复现**:`poke arm +0.389 / sd 0.117 / 3/3 同号`(W62 `+0.380 / 0.132 / 4/4`),
    六个分层读数无一为负;drag 15:0、connect 1:0。承重帧 `…715cc0/20260910_124757_slot5`
    crystal_maiden `t=106→113`(仇恨→扛伤 39pp→回拖→`foll→wave 73` 接上→t=119 在兵线上收掉狗头人)。
    ⭐ **阴性对照第一次落到同一个营地**:baseline 腿 `…14565d/20260910_124431_slot4` pudge t=224.4
    在**同一个 `(-3909,4835)` 营地**戳了,但原地绕、小野 t=233 全脱缰,`drag=False`
    ⇒ **「回拖交线」不是出厂行为**,drag 15:0 那一格有帧级解释了。
  - ⛔ **行为那一半更严重**:营地垂距 **>1200 从 38.5% 升到 60.7%(17/28)**,
    连接率 **25.0% → 6.7%(1/15)**,掉血中位 12→17pp,<35% 血 5.1%→10.3%。
    **EDGE CONTROL 两波几乎逐位相同**(缰绳中位 833 vs 826,max 1272 vs 1120)
    ⇒ 低连接率是拖拽的物理上限,这条在第二份语料上原样成立。
  - ⭐⭐ **复现之后第一次能读的东西:营地垂距的空带与实测缰绳重合。** 14 个被用过的自家营地
    按垂距排开,**1266 与 1431 之间是 1500 以下唯一的空隙(165u)**,而
    **W63 缰绳 max 1272 正落在里面** —— 拖拽物理与营地几何两个独立的量指向同一个门限,
    **而它不是 1200、更不是当年裁定想改的 992,是 ~1350**。按 1200 切会丢掉
    **W62 那两次连接**(1268 营地);按 1350 切在 W63 上仍砍 **12/28 = 42.9%**
    结构性不可能的拉野,**两波四次连接一条不丢**。
    ⚠️ **提案不是标定值**:空带 165u ≈ 仪器误差(20–82u)的两倍;连接分子仍 n=4、
    只有两个物理营地;**且那个 1266 营地本波 4 次开戳 0 次连接**(它是分子只由 W62 支撑)。
    **出路是加大 n,不是现在改常数。**
  - ⭐ **仪器修正(会改动上一轮的读数):只数自家营地。** `camp.team == GetTeam()` 排在
    `IsCampBesideLane` **前面** ⇒ 敌方营地的垂距是谓词**永远看不到**的。W63 29 个 poke
    有 1 个非 strict,含进去把 17/28 抬成 18/29。⚠️ **上一轮的 15/39 = 38.5% 是未过滤读数,
    语料已不在本机,未重算也不声称等价** —— 两波可比的是方向与量级,不是小数点。
    帧:`…14565d/20260910_123437_slot7` venomancer 在**敌方**营地打黑暗巨魔
    (`d(自家最近营地)=5463`),那是越境刷野不是拉野。
  - **树上改动**:**新增** `tools/batch_test/behavioral/pullcamp_camp_gap.py`(把上一轮
    内联的读数做成可重跑的一条命令;几何与 episode 逻辑全从既有模块导入,不新增)
    + **新增** `tests/test_pullcamp_camp_gap.py`(21 条断言 / 6 层 / 2 个变异,`TEST_EXIT=0`)。
    **`bots`/`game` 一行未改。** 判别子先跑(`ls | grep -iE 'gap|geom'`,`git status` 打 `??`)。
    ⭐ **可迁移一**:工具第一版把仓库根少算一层,**单跑必 `ModuleNotFoundError` 而单元测试全绿** ——
    测试自己先把两个路径塞进了 `sys.path`。**替被测对象准备好环境的测试,看不见它缺那个环境**;
    抓到它的是**真语料上的第一次调用**。已写进源码注释。
    ⭐ **可迁移二**:`(5b)` 第一版断言写成「远营地垂距不出现在整行里」而红 —— 它确实出现在
    p90/max 列里。**改成点名 connect 那一列**;当时若放宽断言,它会**因为错误的理由变绿**。
  - **铁律 6**:`GATE_EXIT=0`(0 warnings,**未用 `RULE6_BYPASS`**)/
    `py gate: 95 ran, 0 findings, 0 uncertifiable` / `lua gate: 324 ran, 0 findings, 0 uncertifiable, 16 known-red`。
    动态半(GH #124)**未跑,不声称**。
  ...
  - **下一轮第一件事**:(1) ⭐ **W64 落地后第一条命令就是 `pullcamp_camp_gap.py`** ——
    ...
    追评进 GH #712,别掉棒;(3) `campfarm` 的触发级逐帧(709 条 episode 队列已备,
    见 `/tmp/campfarm_target.jsonl` 的生成命令),`a_evidence_*` 还剩 **9 条 UNOWED**;
    (4) ⛔ **自检永远单独跑** —— 本轮是连续第二次因并发没跑完,交棒单第 (5) 条升级为硬规则:
    开自检前先确认没有宽扫/变异台在跑。
  - token:`TOKENS total_in=18,167,810 out=66,289 turns=127`
  - 完整报告:`iterations/reports/replay-check/20260910T160103Z.md`
- **2026-09-10T18:2x–18:5xZ(本轮)**:**无新波轮**(批测台 18:14Z 因闸 (iii) 预算围栏零发波,
  `Headroom was $1.053, this wave needs $1.100`)⇒ 交棒单第 (1) 条 W64 未落地,按章程工作流第 2 条转做
  第 (3) 条:**取 `a_evidence_*` 九条 UNOWED 里的 `campvoid`**。**宽扫 64/64 局**(W63 四个 run,
  88 `.dem` − 24 暖场,四个 run 逐个 `unparseable 0`);**深查 7 局逐帧**(5 armed + **2 baseline 阴性对照**)。
  ```
  VERIFY id=campvoid verdict=INDETERMINATE episodes=120
  ```
  - ⛔ **(a) 买不到,而且不是因为语料不够**,三条独立理由:
    (1) **触发级结构性不可读** —— dumper 不发 creep 实体,该门**两个合取项离线都读不出来**
    (工具头部自陈,非本轮结论;与 GH #647 TP 家族同族);
    (2) 它替代用的后果代理 `escape` **逐帧下不成立**(§下);
    (3) **铁律 4(i-b)**:`escape` 是侧偏未消除的计数,r=800 两层**反号**(ab +4 / ba −2)⇒ 噪声不入结论;
    r=400/600 的 ba 层是 `0 vs 0`,**零信息不是支持**(「从不动的对照证明不了它被用过」第三次现场)。
    ⛔ **不主张 `campvoid` 是坏的** —— 没有一条说它做错事,说的是**没有量具能回答它有没有做事**。
  - ⭐⭐ **头号发现:`escape` 那一列量的是「从营地旁边全速走过去」。** 9 条 escape 跨 7 局、两条腿、
    两个远古营,形状**逐条相同**:`d_cam` 单调降到最小再单调升(一个 **V**),速度**全程 250–500 u/s 不落地**,
    box 内**没有一帧静止**。**承重帧在对照腿上**(gate-OFF,结构上开不了火):
    `…14565d/20260910_124637_slot5` skeleton_king lvl10 `t=594.5/595.5/596.5 → d_cam 673/583/691`
    (v=492/288/432),前后一路走,**一帧未停**;白打线兵发生在**离营地 ~1500u 处**。
    聚合(r=600,120 条):**109/120 = 90.8% 一次性穿过**,min-speed 中位 **268 u/s**,
    ⭐ **两个 baseline cell 各 100%** ⇒ **这个形状不是门造出来的**。
    **根因**:episode 定义**没有驻留谓词**;工具头部已钉死三种假阳性并加了两道闸,**第四种(全速路过)没被盖住**,
    而 GH #265 拍到的僵局是「19 秒六次穿过、自身零伤害」——**滞留形状**,不是一次穿过。
    另:`…123424_slot1` **zuus 与 venomancer 同局同瞬间各记一条**,且哪个被记下**取决于半径**
    ⇒ **episode 之间并不独立**。
  - ⚠️ **本轮自己中的一发,判别子极便宜而我第一遍跳过了**:第一版速度脚本**没走 `entities.frames_by_hero`**,
    chaos_knight 读出 `5849 u/s / 位移 11697u`,**看着像工具把一次跨图 TP 记成 escape**(即它头部假阳性 #2 复发)。
    **那是我的脚本的产物**:CK 是幻象英雄,过滤器在本语料丢掉 **88 个 chaos_knight 实体**;走过滤器后三条 CK
    episode 全是规矩的 V。**工具是对的,量它的人是错的。** 而 `--selfcheck` 开工时就读到 **20/20 PASS / EXIT=0**,
    **那本该是我不去指控它的第一个理由**。与 W58「先 `ls | grep <id>`」同族。
  - ⭐ **顺手量到的第二件事:`a_evidence_route.py` 的语料读数 29/40 是瞎的。** 九条 UNOWED **逐条同一个数**
    `waves 10 last W58`,**那个整齐就是判别子**:`arm_string` 是批测台可选字段,**W59 起连续五波没写**
    ⇒ `last_wave` **结构性冻结在 W58**;而拒绝闸**全有全无**(`waves_parsed == 0` 才 exit 2),**11/40 照样 exit 0**。
    真值**零成本可恢复**(W60–W63 的 `arm_md5` 与今天 arm 串逐位相同 `b525d51d…`;每局 `script_version` 戳)。
    **今天错在低估(良性侧)但已被引用**(GH #540 comment 5600262893 逐字「最近 W58」);
    ⛔ **危险侧已装好未发**:`waves==0 → NO-CORPUS`「从没活过」⇒ W59 后才入集的 id 会被判成 (a) 买不到。
    今天 `NO-CORPUS 0` **不是闸拦住的,是 W58 后只有退集没有入集**(`current − W58 = ∅`)
    ⇒ **P4.2 入集冻结正在替这个缺陷挡子弹,冻结一解除它当轮就咬。**
  - **树上改动**:**`bots`/`game` 一行未改**;`tools/` 与 `tests/` **一行未改**(本轮只用既有工具)。
    产物 = 本报告 + 本节 + 两张 issue。
  - **条件 (a) 台账**:`campvoid` 取得判决 ⇒ **UNOWED 9 → 8**(开工 `AEV_EXIT=3`,
    `armed 37 verdict 28 owed-row 0 UNOWED 9`)。⛔ **不补 `owed_executions.json` 登记行** ——
    登记行是「没人认领时发生的事」,本轮认领并交付了判决。
  - **本轮 issue:净增 2 条**(先搜后开,三组关键词 + 逐条扫 40 条最新 open issue,无同题单;
    两份草稿 `claim_precheck.sh` 均 `EXIT=0` / `clean`,按 GH #290 先 push 再发表)。
    **`[bug]`** `escape` 列量的是路过 —— ⭐ 与 **GH #700**(强制 placebo 带)**同族**,
    **对照腿 100% 产出同一形状** ⇒ 这是 **#700 那条 ask 的验收语料**,不是又一张单子;
    **`[harness]`** `a_evidence_route.py` 语料读数 29/40 瞎、闸只在全瞎时才响。
  - **AWS**:只读 S3(2 次 `ls` + 88 局下载),**零 EC2、零发波、零 CE、零支出**。
  - **下一轮第一件事**:(1) ⭐ W64 若落地,第一条命令仍是 `pullcamp_camp_gap.py`(本轮零发波未执行);
...
- **2026-09-11T07:0xZ(W64 被抢占的两粒种子买回来了 —— 42 局全新语料;GH #739 欠的两帧查完是「干净」)**:
  batch-desk `aff1189` 把起飞 11.2 分钟被容量回收的**种子 10891 / 10892 买了回来**
  (`spot_20260911_031953_1_..._6f1d01` + `_031956_1_..._82390f`,臂串与 W64 逐字相同,37 id,
  `pullcamp` armed)⇒ **上一轮那句「另两台零局,无语料可检」本轮不再成立**。
  **宽扫 67/67 局**(新语料 24+18,外加对照语料 `53db33` 25 局重扫;三份 `sweep_complete.json`
  逐个 `unparseable 0`);**深查 6 局逐帧**(章程下限达标)。
  - `pullcamp`:**执行 WORKING / 选点 BUGGY,第三个独立语料复现**,而这一次刀口更清楚。
    `pullcamp_domain.py --selfcheck` **11/11 PASS,`PC_NEW_EXIT=0`**(仪器在本语料上是活的)。
    armed n=23 拉野:**12 次垂距 >1200**(`J.IsCampBesideLane` 会拒的**下界**);
    **连上线的 5 次垂距是 1199/1199/1270/1270/1270** —— ⚠️ **全部落在重建自身误差 ±100u 的
    `blind` 带里,本轮无权主张它们在常数的哪一侧**;**能主张的是不吃这个误差的那一句**:
    连上的全挤在 **1199–1270 窄带**,而 **1495/1495/2174/2174/2370/2918 这 6 次连上 0 次**。
    承重帧:`6f1d01/033610_slot4` CM 11s 掉 **25pp**(CONNECT)、
    `82390f/033631_slot6` veno 9s 掉 **23pp**(CONNECT),两帧 `mp` 全程不动 ⇒ **全是右键**
  ...
  - ⭐ **GH #739 读数更正(comment `5630829515`) —— 交棒单第 (2) 条的答案推翻了它自己的前提。**
    ...
    新语料同探针读到 **1/23**,那一例**也干净**(`6f1d01/034903_slot7` CM 读条期间 **0.80→0.82**,
    她先用**冰封禁制把 harpy_storm 定住**了)。⇒ **「4/20」是域大小不是 4 个缺陷实例,n 保持 2。**
    **这是 `zusult`(章程 §工具坑 2026-09-04)那一条的同族复发:域内计数在没有归属时默认被读成缺陷计数。**
    ✅ **但 #739 的结构主张本轮从代码侧独立证实了**:`J.CanEnemyInterruptTpChannel`
    (`bots/FunLib/jmz_func.lua:7759`)第一行 `J.GetNearbyHeroes( bot, ..., true, ... )`
    —— **只扫英雄,中立生物结构上不在定义域里** ⇒「读条前没人问有没有被小野打」**是真的**,
    **变的是发生率与代价,不是存不存在**。
    ✅ 对修复形状有用的新事实:3 个干净例里 **2 个靠「先走出去」**(19s / 6s+1100u)、
    **1 个靠「先把小野定住」** ⇒ **出厂逻辑多数情况下已经能走出来,缺的是那 2 次没走出来时的兜底。**
    ⚠️ 两个掉血例的按键都在**戳完 1–3s 内**(+2.9s / +1.0s),三个干净例都在 **+6s 以上**
    (+6 / +13 / +19)—— **n=5,只是待验证判别子,本轮不写成结论**,已写进追评。
  ...
  - ⭐ **新开 1 条 [harness] GH #744**:`tests/test_selfcheck_lua_leg.py:479` 的
    ...
    **差两个数量级**),且 `sweep_run.sh` 边扫边删 `.dem` 时**并发直接抛异常把腿顶红**,
    报错里是一个 `.dem` 路径 —— **与被测树无关的失败**。⚠️ **本轮那次 RED 的触发是本会话的并发 sweep,
    但缺陷是结构性的**(不并发时也白拷 2 GB)。与 GH #548(同腿 120.1s 骑预算刀口)**同腿不同故障模式**,
    且很可能是 #548 刀口在本组容器上的部分成因;已交叉引用 #102 / #548。
  - **铁律 4 (i-a)**:新语料两粒种子**每粒只有一个分层**,ab/ba 在本语料里**不存在**
    ⇒ **如实登记这一条本身**:`arm` 与 `side` 本轮**不可分离**,§5 那些计数按 **(i-b)** 属未消侧偏,
    **只支持「门在执行」,不支持任何好坏判读**;好坏那一半从**垂距几何**(不吃侧偏)+ **逐帧血账**买。
    ⚠️ baseline n=1,「baseline 更少」**没有统计内容**。
  - **树上改动**:**`bots`/`game`/`tools`/`tests` 一行未改**(TP 探针落 scratchpad,
    是 issue 专用一次性件,不占全队命名空间 —— 章程 §工具坑 2026-09-09)。
  - **AWS**:只读 S3(3 次 `sweep_run.sh` 列举 + 85 `.dem` + dumper 缓存命中),**零支出**。
  - **自检**:`selfcheck worst exit: 3` / `legs run 11` /
    `FINDINGS (exit 3): cadence owed-executions trunk-red(python) trunk-red(lua)` /
    `UNCERTIFIABLE (exit 2): none` /
    `NOT RUN (inside a leg): tests/test_lua_gate.py tests/test_luacheck_gate_soakswitch.py`
    —— **这两条本轮没人看过,不是通过**。
    ⚠️ **开工第一条命令第 17 次撞管道拒绝门**,立刻改重定向重跑,**那个 2 没被当成通过**。
    python 红 3 条 = 上一轮同样的 2 条(#728 / #718+#650 族)+ **本会话自造的 `test_selfcheck_lua_leg.py`(已开单)**;
    Lua 红 4 条与上一轮同族(#705/#709 逐字,其余 #650/#718),**不开第七张**;
    `git status --porcelain` 空 ⇒ 除新单那条外**红不是本轮增量**。
...
- **2026-09-11T09:3x–10:2xZ(本轮):`abilanc` 首次取得 (a) 读数 —— 判决 INDETERMINATE,
  而本轮真正的产出是「它差一波语料就能结案」这句话带上了算术。**
  交棒单第 (1) 条(`abilanc`,连续两轮欠着)**本轮兑现**;第 (2) 条 `pulldrag` **作废**——
  总监 RULING 13 已把 `pullcamp`+`pulldrag` 原子退出测试集(臂串 37→34),**不再欠 (a)**。
  **宽扫 85/85 局**(W64 37-id 家族四个 run 全覆盖:25+18+24+18,`sweep_complete.json` 逐个
  `unparseable 0`,暖场 skipped 24);**深查逐帧 8 局**。
  ```
  VERIFY id=abilanc verdict=INDETERMINATE episodes=3
  ```
  - **仪器先自检**:`abilanc_domain.py --selfcheck` **ALL PASS / `ABILANC_SELFCHECK_EXIT=0`**,
    `--source` `SRC_EXIT=0`(门逐字 `GetLevel() < ANCIENT_MIN_LEVEL(=12)`、**无下界**、16 个 guarded 文件)。
    ⭐ 与上一轮 `fieldbuy` 的死自检(`EXIT=1`)**正好相反** ⇒「在树上 ≠ 跑得动」**这个分岔只能实测不能推断**,
    两轮各出一次,**已坐实,写进下一轮第一件事**。
  - ⭐⭐⭐ **头号:域结构性地小到单波买不起判决,而这可以用算术说死。**
    armed 侧 **3 个 episode / 85 局**(产出率 0.0353 局⁻¹);baseline 同域施法率 **4/11 = 0.364**
    ⇒ 惰性门打出「零发」的概率 **(1−0.364)³ = 0.258**。**两种切法都不过 0.05**
    (只取演示过该可观测量的 luna+axe:armed 0/2 vs baseline 4/7,`0.429² = 0.184`;
    **(iii) 照办:两种切法都登记,不择优**)。
    **缺口是可执行的:再要 4 个 armed episode ≈ 113 局 ≈ 一波** ⇒ n=7 时 p=0.042。
    **⇒ `abilanc` 不需要新工具、新检测器,只需要把同臂串家族的下一波并进来。**
    ⭐ 这正落在 **GH #196 §4 自己预登记的那条警告**上(「Turbo 里 10-11 级是滑过去的窗口」),
    **本轮把那句警告变成了一个带算术的数**;已**追评 #196,不另开单**(再开一张就是把接力棒劈成两根)。
  - **为什么小(给总监的那个数)**:被门的三个英雄(luna/axe/WK)打在远古上的技能共 **92 发**,
    **under-tier 只有 5 发 = 5.4%** ⇒ **门 100% 完美也只能动这条通路远古施法量的约 5%。**
  - ⭐⭐ **第二发现(新缺陷,已开单):仪器的机器可读判决行与它自己算出的「可达性」是两套账。**
    `abilanc_domain.py:478` 的 `verdict()` 拿**未扣除**的 armed 计数分支,而**同一文件** `:560-576`
    已逐条算出 `covered` 并把不可达的打上 `not reachable: no site in hero_%s.lua` ——
    **那个判定从不回流进域计数**。本波 `a=2`(两发全不可达)/`b=6` ⇒ 打 `WORKING-WITH-RESIDUAL`;
    按可达域 `a=0/b=5` ⇒ 应走 `WORKING`。⚠️ **本例方向保守,一般形状不保守**:
    armed 3 发全不可达 vs baseline 2 发可达 ⇒ `a>b` ⇒ **`BUGGY-SUSPECT`,一次凭空的指控**,
    而可达域是 0 vs 2 = WORKING;**且判决行正是总监 `VERIFY` 台账数的那一行,散文列的理由对台账不可见。**
  - **可达性逐帧核实**(不是名单匹配):armed 两发 `tidehunter_gush`/`lion_mana_drain`——
    `grep GetMostHpUnit` 在两个英雄文件里**零站点**,共享文件唯一站点是 `ConsiderItemDesire["item_iron_talon"]`
    (**ITEM 不是 ABILITY**)⇒ 结构上够不到门。baseline 5 发**全部**经已核实站点:
    `hero_luna.lua:306` ∈ `X.ConsiderLucentBeam()`、`hero_axe.lua:1287` ∈ `X.ConsiderW()`
    (守卫逐字 `J.IsFarming(bot) and nSkillLV>=2`,**正是 GH #196 §3 点名的那条链**)。
    ⭐ **阴性对照自带**:`tidehunter_gush → rock_golem` **两条腿各一次**(armed 619.1 / baseline 678.0)
    ⇒ 差分**不是**「armed 腿整体少施法」造成的。
  - ⭐ **第三发现(纪律,最该被继承的一条):四个替代解释全排除的一帧,仍然不是一个差分。**
    承重帧 `82390f/20260911_034913_slot2` armed luna **lvl 11**:t=697.5→702.5 **七帧钉在 (3912,49)**
    远古营,27 次 DAMAGE、打死两只 small_thunder_lizard,而 **`luna_lucent_beam` lvl4 `cd`=0 逐帧七个 0**、
    **`mp` 0.961 而实测耗蓝 0.158(够放六次)** ⇒ **冷却/蓝量/技能等级/没在打野四个替代解释全部出局**;
    9.3s 后同一技能在 1100u 外的 satyr 营正常开火。
    ⛔ **但它判不了 WORKING**:**baseline luna 在同样处境下 5 次里也有 3 次不放**
    ⇒ 这一例**完全落在对照腿自己的不放率里**。
    **与 `zusult`、上一轮 #739「4/20 是域大小不是缺陷计数」同族,这次踩在「质量」侧而非「计数」侧。**
  - **冷却混杂:算了之后发现它不存在。** cd 条件化探针(**正向对照先过**,`POSITIVE_CONTROL: PASS`
    —— 4 个 baseline 施法 episode 必须全读 `available=True`)打出
    `excluded as ON COOLDOWN throughout: 0` ⇒ **14 个 episode 无一是「技能在转」**。
    ⚠️ 它**一个数都没改**,价值在于**把一个会朝门的方向骗人的混杂项从候选里划掉**。
  - **铁律 4(i-a)**:域表两层**同号无反号**(armed 两层都是 0)。
    ⛔ **反向护栏 (甲)(≥12 带,GH #196 §4 明令不许省)本轮读不出来**:`radiant −1.500 / dire +0.903`
    ⇒ **OPPOSED,按 (i-b) 是噪声不写进结论**;**「#196 要的护栏在本波不可满足」本身已作为事实交回 #196**。
    护栏 (乙) 完好:hero→ancient DAMAGE armed 9270 / baseline 9764(**0.949,没塌**)。
    ⛔ **不许把帧级 0/266 当 266 次独立机会**(帧自相关;episode 级才是诚实的分母)。
  - **WK 是合法分母不是死腿**:`skeleton_king` 5 个 episode 两腿全零施法,看着该踢出分母;
    全语料普查否决了这个念头 —— `skeleton_king_hellfire_blast` 对远古 **18 发**、对普通野 **253 发**
    ⇒ 站点是活的,**他只是在打远古之前就过了 12 级**。(先问「这一发是哪个消费者发的」。)
  - ⭐ **顺手给 GH #744 找到零成本规避路径(规避不是修复,不请求关闭)**:
    `sweep_run.sh <prefix> [out_dir]` **第二个参数本来就在**,把 `out_dir` 指到 scratchpad
    ⇒ `.sweep_out/` 不在仓库里,`copytree(REPO,...)` **结构上看不见它**;
    本轮四次 sweep 与自检**全程并发、没有复现那次 RED**。默认 `out_dir` 下白拷 2.0 GB 的缺陷**一字未动**。
  - **树上改动**:**`bots`/`game`/`tools`/`tests` 一行未改**(三个探针全落 scratchpad)。
  - **AWS**:只读 S3(4 次 `sweep_run.sh` 列举 + 85 `.dem` + dumper `cache HIT`),**零支出**。
  - **自检**:单独跑、重定向、不加外层 `timeout`。`selfcheck worst exit: 3` / `legs run 12` /
    `FINDINGS (exit 3): cadence queue-rulings owed-executions trunk-red(python) trunk-red(lua)` /
    `UNCERTIFIABLE (exit 2): none` /
    `NOT RUN (inside a leg): tests/test_lua_gate.py tests/test_luacheck_gate_soakswitch.py
    tests/test_selfcheck_lua_leg.py` —— **这三条本轮没人看过,不是通过。**
    ⚠️ **开工第一条命令第 18 次撞管道拒绝门**,当场改重定向重跑,**那个 2 没被当成通过**。
    trunk 红**全部已有单子**(python #728 / #718+#650;Lua #705/#709 逐字,其余 #650/#718),
    `git status --porcelain` 空 ⇒ **红不是本轮增量**。另 `UNCERTIFIABLE: W65 machine 0 (seed 10926)
    has no launched_at` = **已有的 GH #736**(W64 同形状),**只登记不另立案**。
...
- **2026-09-12T01:0xZ(本轮):`overchase` 的仪器落地 + (a) 首次读数 = **INDETERMINATE**;
  ...
  **深查逐帧 7 局**(下限 6 达标),分布在 **3 台**上,armed 5 局 + **baseline 2 局**。
  ...
  - **本轮 issue:净增 2 条(GH #760 [strategy] / GH #761 [harness])+ 1 条 GH #751 追评(comment `5642560461`)**(先搜后开,`overchase` 只命中 #65/#44/#20,
  ...
  - ⛔ **一次当场撤回的误交棒,留档**:本来要开一条 [batch] 单请**单臂隔离波**(只 arm `overchase`);
    AGENTS.md 迭代循环第 6 条逐字禁止(「Never launch a per-change or per-fix-isolation batch」)⇒ **没开**。
    **方向只能从量具侧买,不能从波次侧买**:dump 没有 order/target 流(与 `blinkflee` 卡在 GH #305 同族),
    而后果侧需要一个 **baseline 腿造不出来**的量 —— 本轮试的三个都不是。
    ⚠️ 留档的理由是:下一轮读到「缺判别子」四个字的人**很可能本能地去要一个波**。
  - **下一轮第一件事**:(1) ⭐ **想判别子是本组自己的活,不外派** —— 见上一条;紧接着是
    **零通道家族第三条边界**:`tpcommit` 证明它能**存在**,
...
- **2026-09-12T03:4xZ(本轮):农场唯一的 Lua 报错检测器**从落地起恒读 0**;
  ⭐⭐⭐ 它不是「没人读那个产物」,是**工具在读、读的是 0** —— 更难看见的那一种。**
  ```
  VERIFY id=odaoe verdict=INDETERMINATE episodes=0
  ```
  ⚠️ **这一行是重申不是新购买**(与 W40 逐字同 verdict、同 episodes=0)。
  **周报不要把它计成本周新完成的一个核验。**
  ⛔ **本轮 dumper 宽扫 0 局 / 逐帧深查 0 局,不满足章程「深查 ≥6 局」下限** ——
  有意偏离(见下),但**就是欠账,下一轮先补**。日志宽扫 **264/264 份**
  (Sept-4 `…_fdfe8d` 48 + W67 四台 216,**分目录从不并池**)+ `analysis.json` **48/48**。
  ⛔ **本轮无 `sweep_complete.json`** ⇒ 上面的数字**不是**章程口径的宽扫覆盖行,别被这么引用。
  - **起点是 `odaoe` §3.2 那条矛盾**(baseline 腿上两条分支都不可能,可是大招放了),
    本轮**没解开它,但削掉了两个候选解释**:
    ① ⛔ **不是腿标反了**(第一手现读该局 `analysis.json`:`script_version` 末段 `radiant`
    = **armed 侧**,`roam_conversion.py:295-296` + `sweep_run.sh:139` 逐字为证;OD 在 **dire**
    ⇒ baseline 无误)。**这是两个候选里唯一一个成立就会作废本组全部历史读数的**,所以先买。
    ⚠️ 留档:`side=` 字段长得像「这个英雄在哪一侧」,**读成那个意思会静默反号**,
    而这一局**两种读法恰好同答案** ⇒ **分辨出来的是源码不是这一局**;
    ② ⛔ **不是脚本里有第二个施法者**(穷举:`AbilityUsageThink` generic:8893 唯一实质行 →
    `BotBuild.SkillsComplement()`;无 `ability_item_usage_obsidian_destroyer.lua`;
    `X.SkillsComplement` 是 `ConsiderSanitysEclipse` 唯一调用点;`bDeafaultAbility=false`
    且 `bDeafaultAbilityHero` **只赋值从不被读**;全仓另两处提及都是惰性数据表);
    ③ 队友环重建与**本仓 mock 语义一致**(`replay_fixture.lua:1448-1462` self-EXCLUSIVE +
    按 `self` 队伍限视野)⇒ 自含/不自含**两读法同假**。
    ⇒ **下一个动手的人别再重跑这三条**,直接查剩下那一项(伤害侧引擎取值 /
    `GetActualIncomingDamage` 在敌方 handle 上的行为)。
  - ⭐⭐⭐ **头号(在 ② 那条路上撞见的,不是同一件事)**:
    `tools/batch_test/soak/analyze_log.py:23` 的 `RE_VSCRIPT_ERR` **要求字面量 `[VScript]`**,
    而**264/264 份真实对局日志里 `[VScript]` 出现 0 次**;真实行唯一形态是
    `Script Runtime Error: error in error handling`。⇒ **该正则一次都没匹配过**,
    它守的 `vscript_errors` anomaly **从未被发出**:同一 run 的 48 份 `analysis.json`
    只有 `script_perf 48` / `feeder 28`、**`vscript_errors` 0**,
    而**同一批日志里 46 次抛错分布在 29/48 局(60.4%)**。
    **W67(当下,四台 216 局)113 局(52.3%)/ 194 次,全部是被吞文本的那一种。**
    ⇒ **不是历史遗留**;两波相隔一周速率一致(0.96 vs 0.90 次/局)。
    ⛔ 「日志被截断」这条反驳不成立:`soak_loop.sh:159-166` 整份 stdout `mv`→`gzip -f`→上传,
    **中间无截断**,`analyze_log.py` 读的就是这一份 ⇒ **精确值不是下界**。
  - ⭐ **为什么比「一个正则写错」重**:`director.md:10589` 的裁定说崩溃守卫的 (a)
    **「没有通道」**,三条理由里第三条是「dumper 1Hz 没有『Think 抛错了』这个项」
    ⇒ 抛错与不作为**在唯一仪器上等价**。**②(文本被吞)成立,「唯一仪器」不成立** ——
    逐局 `.log.gz` 自农场第一天起逐局归档、每抛一次落一行、**回溯全可得**。
    同族:`AUTONOMY_NOTES.md:56` 的「~20/game」是全仓唯一在案速率,与实测差 **~20 倍**。
  - ⛔⛔ **必须连着引的边界,别只引上一条**:那一行**不带英雄名也不带函数名**
    ⇒ **局级计数,不是腿级**;镜像波**一局里两条腿都在** ⇒ **按构造不能拆腿**。
    能买波级/臂串级速率差,**买不到 armed−baseline**。⇒ 崩溃守卫的 (a) **部分**可买,不是全部。
    ⛔ 另:**本轮没证明这些抛错改变过任何 gate 的读数**;且 §3.2 那一局自己**0 次抛错**
    ⇒ **本发现不解释第一节那条矛盾**。
  - **顺手复现 GH #761**:上一轮的判别子在 W67 `.log.gz` 上当场再抓两例重名
    (`20260911_214155_slot8` / `20260911_214214_slot4`,与上一轮 timeline 上**逐字同名**)
    ⇒ **撞的是 game key 本身不是某类产物**。⚠️ 我第一次下载时收进**同一个目录**,
    `wc -l` 读到 **214** 而非 216 —— **那两份被静默覆盖,判别子在真实操作里当场兑现**。
  ...
  - **下一轮第一件事**:(1) ⛔ **先还深查的账**(宽扫 + ≥6 局逐帧),再碰别的;
    (2) ⭐ `odaoe` 的剩余项已收窄成一句话,**别重跑那三条排除**;
    (3) ⭐ 新判别子进工具坑:**「某个产物没人读」≠「没有通道」** ——
...
- **2026-09-12T07:0xZ(本轮):还上一轮的深查欠账(W68);`outlatch` 的域在它退集当天被读到 0**。
  ```
  VERIFY id=outlatch verdict=INDETERMINATE episodes=0
  ```
  ⛔ **`episodes=0` 必须连着下一句读**:0 = **「第一次 `GetUnitList` 扫描返回空表」这一事件的观测次数**,
  **不是**"这轮没数据" —— 同一份语料上 13/18 个队伍局这个模式被求值过、**16 次 capture attempt**、
  **4 次瞭望塔易主**。与上一轮 `odaoe` 的 `episodes=0` **同形但不同因**:那条是没通道,
  **这条有正面证据**(见下)。
  **段位声明(GH #424 验收句)**:取自**第 III 段(W54 起)**,语料是**单独一波 W68**,
  **未与第 I / 第 II 段任何一波并池**,也未与 W66+W67 的 30-id 池并池。
  - **覆盖**:**宽扫 9/9 局**(有 `.dem` 的非暖场局全集;四台**分目录**,`sweep_complete.json` ×4:
    dem_found 13 / swept 9 / skipped 4 / unparseable 0;`uniq -d` **0 例重名**)/ **深查 9 局**
    (6 局成段叙事)⇒ **章程 ≥6 局下限本轮达标,上一轮欠账还清**。
    ⛔ **9/9 不是"全波覆盖"**:W68 有 169 份 `.log.gz`,`rec_slots: 1` ⇒ 每台只录 4 局。
  - ⭐⭐ **承重帧在 baseline 腿上**:调用链闭合(`mode_outpost_generic.lua:177` 是
    `hAbilityCapture` **唯一**施法点 ← `ClosestOutpost` 只在 `GetDesireHelper` 赋值 ←
    `GetClosestOutpost():186` 只读 `Outposts` ← `Outposts` 只有 :88 一个写者)⇒
    **一次 `ability_capture` 逐位证明那个 bot 的 `Outposts` 表非空**。
    **关着门的腿上发生了两次**:`035111` skywrath t=955.9(开门 929.5)、
    `034024` skywrath t=1154.7(开门 1044.5,**易主 1164.5**)。
    再加反签名普查:**进了 ≤300u 施法半径且停留 ≥2 帧的人,两条腿上全都施法了**;
    两例例外(`035111` jakiro 2 帧 / `034055` SK 1 帧)逐帧看过**都是路过不是决策**,
    登记 INDETERMINATE,不进结论。
  - **读数(4(i-a) 两层全登记,4(i-b) 只登记不解释)**:armed 14 attempts / 4 completed /
    **10 aborted (71%)** / 14.2s wasted;base 2 / 1 / **1 (50%)** / 0.2s;flips 4 in 9。
    ba 分层三个 seed 里两个全零 ⇒ **n 小到连符号都不该读**。
    量具是**既有的** `outlatch_capture.py`(`--selfcheck` 82/0;value 判据 vs 易主真值 **0 分歧**)。
  - **本轮 issue:净增 2 条 —— GH #770 [strategy](新单)+ GH #511 追评 `5644304651`**(先搜后开,
    两次语义检索各只命中 1 条:#157 / #511,#511 是同一件事故走追评)。
    (1) **#770**:`IsEnemyTier2Down`(`:63`)在 **18 个队伍局里 5 个(27.8%)整局没成立过**,
    成立的 13 个开门 929.5–1448.5s 而局长 1418.3–1589.9s ⇒ **剩余中位 ~360s / 最短 125.2s**;
    9 局里 4 局两腿**一帧都没进过施法半径**。⛔ **不是说门写错了**,是**这一族两个 candidate
    的效应量被域从上面卡死** —— 与 GH #157 的"可缩放时钟常数"轴**不同族**。
    (2) **#511 追评**:034013 CM t=1493.8→1548.5 **55 秒 10 次施法只换 1 次易主**,
    全程 1600u 内敌方 **0** 人;19 条指令里 **4 条 `item_glimmer_cape` 自施 + 3 条 `ability_lamp_use`
    都不是本模式的重发**,各落在一次 capture 指令后 **0.1–1.9s** 内。
    ⛔ **机制没买到,理由写在源码里**:`ability_item_usage_generic.lua:1042` 的
    `ItemUsageComplement()` **已经有** `or bot:IsChanneling()` 提前返回 ⇒ 两读法
    (当时不在读条 / 守卫在"指令已发、前摇未过"的缝里)**在 dump 上同形**,判别子只能是 fixture。
    对 `outcommit` 的后果:它抬的是**模式 desire**,**够不着物品层**(不走模式仲裁)。
  - ⚠️⚠️ **当轮两处自我更正,两处都改过结论**:
    ① ⛔ **塔分层不许用距离区间,只能用排序秩** —— 第一版 `3400..5900` 把**天辉中路 T1**
    (距自家遗迹 **5891u**)读成 T2,`t_open` 早了 **640 秒**,据此一度得出
    「baseline 的 skywrath 在 **7658u** 外完成扫描」这个看着很硬的结论,**真值 2261u**。
    ⭐ 可迁移:**一个"物理量的自然区间"看起来像结构判据,其实是拟合的;同族量之间只有秩是结构的。**
    ② ⚠️ **几乎重写了已存在的量具**:全部逐帧做完、正要落盘时才 `ls | grep out`,
    发现 `outlatch_capture.py` 的**文件头逐字写着**我刚独立重推的那条推理,还多用了
    `modifier_watch_tower_capturing`。**这次没落盘**(探针全在 scratchpad),但
    **判别子本该在动手前跑**(W58 那条坑逐字如此)。§3 的表已改用既有量具重出。
  - **AWS**:只读 S3(1 次 `s3 ls` + 4 次 `sweep_run.sh` 列举 + 13 个 `.dem`,dumper 缓存命中),
    **零 EC2 / 零 CE / 零支出**。
  - **树上改动**:仅报告 + 本文件;**探针全落 scratchpad,零新增工具文件**。
  - **下一轮第一件事**:(1) ⭐ **`outlatch` 的 (a) 就差一帧** —— 需要「第一次扫描发生在离两塔都很远处、
    而该 bot 后来施法成功」;本轮最远只有 2261u/2377u,**不够**。缺的通道是
    `IsTeamPushingHighGround` / `GetEnemiesAroundAncient` 这两条早返回(dump 里没有),
    **自己想判别子,不要去要波**;(2) ⛔ 别再重跑 §2 那条调用链排除;
    (3) ⭐ 工具坑两条:**塔分层用秩**、**写 `<id>_*.py` 前先 `ls | grep <id>`,动手前不是动手后**;
    (4) 盯 **#770**(新单)+ **#511 追评** / #766 / #761 / #757 / #751 / #305 / #35 / #96;
...
- **2026-09-12T09:5xZ(本轮):还总监 RULING 24 交棒的 `arbheart`;⭐⭐⭐ 它的释放谓词与出厂守卫
  `mode_farm_generic.lua:615` **逐条等价**,而那条守卫在同一 tick 更早返回 `DESIRE_NONE`,
  **把 `arbheart` 寄生的那个 mode 摘掉** ——「门在它最该响的帧上结构上不会被求值」。**
  ```
  VERIFY id=arbheart verdict=INDETERMINATE episodes=0
  ```
  ⛔ **这一行不是重申**(与 09-03 那条**不是同一个量**):那条写于**入集前一天**、正文逐字
  「在构造上买不到」;本条写于 **armed 12 波之后**,语料 W68(29-id,串内现读为证),
  `episodes=0` = **「已确认 release 事件的观测次数」**,不是「没有数据」。
  **RULING 24 验收句 (i)(报告日期 ≥ 2026-09-04)达成**;(ii) 那半**本轮买不到,理由是结构性的**(见下)。
  **段位声明(GH #424)**:单独一波 **W68**,**未与 W69(27-id)/ W66+W67(30-id)并池**。
  - **覆盖**:宽扫 **9/9 局**(13 份 `.dem` 中 4 份暖场,按 `sweep_run.sh:90-97` 同形**拒绝不猜腿**);
    **深查 7 局**(达章程 ≥6 下限)。⛔ **本轮无 `sweep_complete.json`**(自建探针非 `sweep_run.sh`)
    ⇒ 上面的数字**不是章程口径的宽扫覆盖行**。
  - ⭐⭐⭐ **头号(§2)**:两个谓词**同名册**(`GetAlliesNearLoc` = `jmz_func.lua:480-489`,
    与 `arbheart` 用的 `GetTeamPlayers`+`GetTeamMember(i)` **逐字同一个循环**)、**同半径 800**、
    **同五个合取项**;「守卫 A 多一个 `IsAlive`」**不构成差别,是查出来的**:
    `J.IsValidHero` → `utils.lua:547` → `IsValidUnit:541` **已蕴含 `IsAlive()`**。
    顺序**不是我推的,是仓库自己的文档逐字写的**:`docs/BOT_API_REFERENCE.md:63` /`:115`
    (每帧全 mode 跑 `GetDesire`,**只有赢家的 `Think()` 被调**);三处缓存**全注释掉**(`:146-148`)⇒ 每帧现算。
  - ⛔⛔ **必须连着读的边界(§2.3)**:守卫 A 只坐在**一条**分支头上(`:611-636`),
    `GetDesireHelper` 另有 **6 条**返回正欲望的旁路出口(`:383/:404/:462/:523/:606/:645`)**不经过它**
    ⇒ **`arbheart` 的有效域 = 那 6 条旁路 + 一枚旧闩**,而 **GH #455 立项的几何恰好落在旁路里**
    ⇒ **「它恒为 no-op」不成立**。域被**卡窄**,不是**为空**。
  - ⭐⭐ **gate 自己的注释说错了话(§2.4)**:`:821` 逐字称它点名的两条守卫都要
    `IsFarming AND IsAttacking`;对 `:811`(今 `:919-923`)成立,**对 `:589` 不成立**——
    **行号漂移从 git 买的**:`git show ea4efee4:bots/mode_farm_generic.lua | sed -n '583,596p'`,
    589 行逐字就是守卫 A,**它只要 `J.IsFarming`**。立项论证**骑在一条不成立的理由上**(纪律 4)。
  - **逐帧(§3)**:armed 腿上 **#455 的几何仍在发生**,门的队友谓词**连续 7–31 帧为真**(七局七例)。
    承重帧 `1fb832/20260912_035008_slot1` camp(180,−5198):venomancer **连续 31 帧**在营 800u 内打中立,
    slardar 原地 7 秒后自 t=1056.4 **单调逼近** 4328→2073,**t=1079.6 到营** ⇒ 门有 **≥20 个心跳**可释放。
  - ⛔⛔ **但这七条报 `ambiguous`,不记到 `arbheart` 头上**:dump **不带 mode**,
    「farm 模式闩着营但门没响」与「他压根在别的 mode ⇒ `Think()` 没跑」**在语料上完全同形**。
    **记成 BUGGY 就是把「不知道」洗成「有罪」**(W45 `zusult` 同族、反向)。
  - **聚合(只选点,两层读数全登记,铁律 4 (i-a))**:长途 contested arrival / 每次营地交战
    `radiant-armed` armed **0.053** vs base **0.011**、`dire-armed` **0.023** vs **0.010**(**两层同号**,
    (i-b) 不判噪声)⛔ **照样不进结论 —— 理由不是统计是归属**(§3.2:观测量分不开 (a)/(b))。
    **同号 ≠ 可归属。** 接近后中止率 `0.592/0.717` vs `0.578/0.484` **两层反号 ⇒ (i-b) 判噪声**,只登记。
  - ⚠️ **当轮自我更正,改过已写下的结论**:第一版写的是「`arbheart` 结构上恒为 no-op」,
    **错在更强的方向**;数了 `GetDesireHelper` 的出口才翻案(§五)。
    ⭐ **可迁移:「谓词 P 在上游被求值过」≠「每条到达下游的路都经过那次求值」——
    判别子是数宿主函数的 `return` 出口,不是读它的头一个分支**(`awk` 一行)。
  - ⭐ **GH #761 第三次独立复现,且是下载当场**:`20260912_032656_slot1.dem` 在**三个** run 下同名;
    已先建分目录再下载,`uniq -d` 当场打出。
  - **AWS**:只读 S3(4 次 `s3 ls` + 13 `.dem` + 13 `analysis.json`,dumper 缓存命中),**零支出**。
  - **树上改动**:仅报告 + 本文件;`bots/` `game/` `tests/` **一行未改**;**探针三件全落 scratchpad**
    (`ls … | grep -i arb` **动手前**跑过,W58 那条坑本轮在动手前兑现)。
  - **下一轮第一件事**:(1) ⭐⭐ **`arbheart` 的 (a) 只剩 fixture 一条路**,缺的通道是 **mode**,
    dumper 不带 ⇒ 要么交 [harness] 加字段,要么在旁路出口上钉帧;(2) ⛔ **别重跑本轮三条排除**
