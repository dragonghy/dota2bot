...
0S. **【2026-08-22T10:xxZ 新增,流程条,不产 gate】「我以为我碰了哪些文件」不是清单,
   ...
   **而那 8 条里有 1 条不是分母**:`crash_2597 == 179` 在**今天的四棵树上**
   (`cac2fa5`/`41df6b4`/`477d0d4`/`3dc30f2`,98 枚语料)**读数全是 177** ⇒
   **本轮之前就红了 −2,且不是本组造成**(100 枚语料下带不带本轮 `bots/` 改动都读 178/32/210,
   gate 关着 helper 第二行就返回)。它落在**已 promote 的 `tpsafe2` 路**上,是行为变化不是分母,
   已写进该文件注释并交总监(GH #100 系,`state.json:itemdesire_CRASH_PIN_STALE_20260822`)。
