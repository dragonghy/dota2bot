...
0SIP. **【2026-08-29T10:4xZ 新增;认领 **owner 优先项 P2** 里本组自己 2026-08-22 登记、
   此后**零轮认领**的那一格(`0P2` 第 (c) 段末:「留下之后够不够」);一条可复用主判据 +
   **一个已落地的 gated id `fieldsip`** + 两条方法自伤 + 一条 trunk RED 的旁证。
   ...
   **⚠️ 顺带发现一条与本轮无关的 trunk RED**(分块跑全套时撞到,已在**净 HEAD `1ccdeaa7`** 上复现):
   `tests/test_cm_arcane_aura_passive.lua:226` —— **本组不认领**(CM 技能,`[hero]`/`[bug]` 族),
   它**不在自检快 Lua 腿的覆盖里**,与 GH #267 同族。
...
## 当前状态(每次触发后更新)
- 2026-08-29T10:43Z(**owner 优先项 P2**;入集提议 `test_set.md` **§CE**):**一个 presence 谓词
  ...
  **⚠️ 顺带撞到一条与本轮无关的 trunk RED,已在净 `HEAD 1ccdeaa7` 上复现**:
  `tests/test_cm_arcane_aura_passive.lua:226`(「armed 时 PASSIVE mask 必须拒绝出厂允许的施法」)——
  **本组不认领**(CM 技能,`[hero]`/`[bug]` 族);它**不在自检快 Lua 腿的覆盖里**,与 GH #267 同族。
  **交棒**:**总监**(① 裁 `fieldsip` 入集,建议**入**;② **请连 §CE.6 那个次序问题一起裁** ——
  ...
  ⑤ 上面那条 CM 的 trunk RED 请派人)、**录像组**((a) 判读点 = armed 腿上只带 tango/faerie_fire
