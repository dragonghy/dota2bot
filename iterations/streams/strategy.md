...
0HRPARITY. **【2026-09-09T10:48Z 新增。**P4.4 归属 = **(i) 一个 `bots/` 行为改动**(连续第四轮 (i));
   ...
   **逐字执行**(「首选本轮已定价、已用源码钉钉住、故意留着的那一处」)。产出:新 gated id **`hrparity`**
   (`bots/FunLib/jmz_func.lua` 的 `J.GetLaneHarassResponse` 的**寡不敌众判断**,turbo-only,**不嵌在任何别的门里**)、
   `tests/_lanekill_domain_sweep.lua` **第三次扩列**(**没有**新建第六个全语料 sweep)、
   `tests/test_hrparity_guard.lua`(**12/12**)、`tools/agent/mutstand_hrparity.sh`
   (**10 腿:9 变异体全 CAUGHT + 控制项 SURVIVED,零 SURVIVED,STAND GREEN**)、
   `tests/test_hrreach_guard.lua` 第 5 节**重写** + `mutstand_hrreach.sh` 的 M8 重新瞄准(**重跑 10/10**)、
   `state.json:hrparity_20260909`;报告 `iterations/reports/strategy/20260909T104848Z.md`;**issue GH #660**;
   **armed 串 / `queue.json` / `test_set.md` 一字未动**;零 AWS、零 S3、零 EC2、零波次。
   **⭐ 缺陷:一次比较的两侧用了两把尺。** 同一个 helper 敌人数到 **1100**、队友数到 **900**,
   再问 `#tValid > nOurs` ⇒ 站在 1000u 的敌人是围攻者,站在**同一距离**的队友**不在场上**;
   判出来的 `back` 是往泉水迈 420u,而这一格跑在补刀/反补**之前**、无条件返回。
   **⭐⭐ 这是修复不是新政策**:helper 里**没有任何地方**写过「多远之外的帮手不算数」——
   没有注释、没有具名常量、没有第二个阈值;只有一次比较、两把尺,**短的那把恰好在「留在线上」的那一侧**。
   **断言的是两个半径相等,不是「新的那个是 1100」**(对称性是性质,1100 只是取值;M4=退回 900、
   M5=overshoot 1600,两条腿被同一条断言抓住)。
   **读数**:出厂 **12 nil + 18 back + 54 fire = 84 闭合**;armed **12 + 16 + 56 = 84 同样闭合**;
   `hp_back_closes` **2**、禁止方向 `hp_back_opens` **0** / `hp_nil_moved` **0**(按构造:超集 ⇒ 只增不减)。
   证人两帧**都是语料自己点的名**:`f_260820_042009_cm_cask_far` 巫医(友 Slardar **998u**)、
   `f_260820_102645_cm_es_reach` 钢背兽(友 撼地神牛 **1075u**)。
   **⭐⭐⭐ 2 被两条独立的路走到**:`hp_pred_flips`(原始人口算术)== `hp_back_closes`(armed 驱动);
   **M6 专为此设** —— 保留 1100 那次读取、只把赋值换成 `nOurs = 1 + #tValid`,方向列仍 0、差分仍非零、
   计数恒等式仍成立,**只有两路核对看得见**(现场 `arithmetic says 2, the armed drive says 18`)。
   **⭐⭐⭐⭐ 这一处的域不是桩,且是断言出来的**:`hp_ally_disc_differs` **5**(两张队友盘真的返回不同集合);
   与隔壁 `hrreach` 的 `hr_range_stub` **84/84**(GH #656)**明写对照**,防后来的读者把那条免责声明借过来。
   ⛔ **发波前先说(GH #622)**:**单独 arm 它,掰直的那 2 帧直接掉进隔壁缺陷** —— 两帧都 `back`→`fire`
   且目标在射程外(`hp_opened_d_max_u` **882**;巫医 500 射程对 883u、钢背兽 150 近战对 557u);
   **两个 id 同时 armed 时同样 2 帧读回 nil**(`hpb_flip_ends_nil` **2**,co-armed 63/16/5 = 84 闭合)⇒
   **建议(交总监):不要单独发 `hrparity`,与 `hrreach` 同波共臂**;这是「一波带哪几条腿」,
   **不是**把两个修复捆成一个 id(M9 把 co-armed 驱动塌回单臂就打红,所以这条建议自己会报警)。
   ⚠️ **本轮记下的关于量具本身的一件事:上一轮那条 M12 棘轮钉本来抓不住本轮这次修复。**
   第 5 节用 `GetNearbyHeroes%( bot, (%d+), false` 匹配**第一处**队友读取并断言 900,而**门内的修复**
   把那一行**原封不动留在原地**、在**它下面**新增第二处 ⇒ 那条钉**一路绿着**,描述的 helper 已经变了。
   **「针对未门控改动写的棘轮看不见门控改动」,而门控改动是本组唯一被允许写的那种。** 已重写为同时钉
   (i) 出厂默认仍 1100/900(静默 promote 打红)、(ii) 修复须以 `hrparity` 之名出现在该在的位置(回滚打红)、
   (iii) `hrreach` 差分未被扰动;`mutstand_hrparity.sh` 的 **M8 = 修复脱门**现场抓到。
   ⛔ **下一格(本组下一轮第一项)**:
   (1) ⭐ **主体继续是 `bots/` 行为改动**;
   (2) ⛔ **不要**动本 helper 的**过滤不对称**(敌人侧丢幻象/打不到的,队友侧什么都不滤)——
   **同轮已定价且域实测为 0**:`hp_ally_nonempty` **46** / `hp_ally_illusion` **0** /
   `hp_ally_illusion_noapi` **0**(两列分开正是为了不让「语料里没有友方幻象」和「加载器没有
   `IsIllusion` 可问」读成同一个零,GH #171 形状)⇒ **这是关于语料的事实**;
   先买语料再动,否则就是 0FSATOM 明令禁止的「域为零的 gated 杠杆」;
   (3) ⛔ **不要**为了凑 P4.4(i) 硬造域为零的杠杆、**不要**新建第六个全语料 sweep、
   **不要**回 `field_hold_needs_magnitude` 一族、**不要**动 lane 守卫族剩下两处(前置仍未到位);
   (4) 语料请求:**本轮新增第四条 —— 带友方幻象的语料帧**(否则第 (2) 条永远是 0);
   另三条仍挂着:`GetAttackRange()` 投影(GH #656/#657,同时解锁 13 个 id)、
...
0OHNUM. **【2026-09-07T19:00Z 新增。**OWNER_PRIORITIES P4.4(i) 达成:工作单元主体 = 一个 `bots/` 行为改动**;
   ...
   **⭐⭐⭐ 变异台第一轮 6/7 不是它更差,是它话说错了**:M4 红在**正确的测试**上,但 `want` 串
   ...
   ⛔ **下一格(本组下一轮第一项)**:
   (1) **先做 GH #607**(本轮已回帖登记 `#issuecomment-5574951487`,含下面那条构造性预警)—— 它是**唯一未认领的 `[strategy]` issue**,本轮因 P4.4
...
0PINEVADE. **【2026-09-07T10:40Z 新增,**认领开工自检的 **TRUNK RED 3/84 Lua detector**(它挡住**全队**铁律 6 的动态那半);
   ...
   产出 `tests/test_corpus_scale.lua` 的 `assignment_literal` + `equality_compared_names`(**检测器扩展**)、
   `tests/test_propertarget_corpus_domain.lua` / `tests/test_salveyield_arbitration.lua` /
   `tests/test_stayfield2_marginal_domain.lua` 三个文件转 `cs.ratchet`/`cs.corpus`、
   ...
   **⭐ 主判据:第五次复发,而三条红没有一条是发现。**
   ...
   —— **全是「重述语料规模」的量**;唯一动了的**发现**是 salveyield 的 `count(d1)` **9→11**。
   族谱:GH #106(5 文件)→ GH #127(7 文件 / 18 断言)→ 2026-09-02 总监(1)→ 09-07 上一轮本组(3)→ **本轮(3)**。
   **⭐⭐ stayfield2 是这一族最干净的一次分离,而分离是被数出来的**:第 110 个 fixture
   只动了它的**两个语料重述量**,**十一个发现逐位不变**(`S=24`/`ST=5`/`margin=19`/`dmg,supply,both=0,18,1`/
   `T_ONLY_HP_BAND=6`/`SIGN_MARGIN_NEG,POS=14,5`/`SIGN_NATURAL_NEG=56`)
   ⇒ **十一个没动,动的两个从来就不是关于 stayfield2 的。**
   salveyield 同理:`d1` 9→11 而 `d2..d5`(3/2/2/1)**逐位不变** —— 新帧两次命中 self 分支、
   两次都被**紧邻的下一个合取项** `quiet_self` 挡掉。
   **⚠️ 诚实的界(写进文件里)**:bots/ 这段区间**不静止**(09-05 起三次 PROMOTE:`ckpush`/`odbuild`/`illumove`)
   ⇒「只有第一行动了」**不是增长的证明**,只**给出替代解释的界**;为此另钉一条**嵌套单调断言**
   (`a1>=a2>=…>=a5`),让重新登记不能只靠改一个字面量蒙混过去。
   **⭐⭐⭐ 本轮真正的产物:检测器学到一种此前看不见的躲法,而它有两种不是一种。**
   判据逐字是「一个**等于今天 fixture 数**的整数字面量,在 `==` **右边**」。
   (1)**换分母**(`1012`/`73`)—— 上一轮已点名;
   (2)**⭐ 新:经由具名常量的间接** —— `local SIGN_SUBSAMPLE = 109` + `assert(... == SIGN_SUBSAMPLE)`,
   ...
   **⭐⭐⭐⭐ 立法级:新谓词的第二个合取项不是调参,是用测量买来的。**
   `local NAME = <整数>` **只在 NAME 在同一文件里被 `==` 比较过**时才命中。今天 N=110:
   tests/ 里赋值式整数常量 **594** 个,**恰好 1 个**等于 110(`local Q_MANA_R3 = 110`,Axe Q 三级蓝耗),
   而它**只在 `>=` 下被用过** ⇒ 相等性要求把它排掉,**本次扩展今天多花 0 条豁免**;
   朴素写法今天就要多付 1 条,并把 `NOT_A_CORPUS_PIN` 已在付的「每落一个 fixture 一次碰撞」**大致翻倍**。
   ⇒ **可复用的一条:扩一个检测器的语法面之前,先数一遍新语法面在今天的 N 上撞出多少假阳;
   窄不窄是测量结论,不是风格偏好。**
   **⭐⭐⭐⭐⭐ 端到端正控制,不是只跑 parser 单测**:临时插 `local PC_PIN = 110` + `local _ = (0 == PC_PIN)`,
   检测器**当场红并点名该行**(`RC_EXIT=1`),还原后 `git diff --stat` 为空。
   **⛔ 本轮乙路两条杠杆全部「定价后拒绝」,一条都没落(章程「不落空杠杆」)。**
   域价钱(110 fixtures / 1021 live frames,主槽/任意格):`item_dust` **24/36**、
   `item_smoke_of_deceit` **8/19**、`item_soul_ring` **3/3**。
   **smoke 的缺陷是真的**:`~:7365` 里 bot **自己**身边的敌人**从来不是否决项** ——
   外层 `(#enemy==0) or (#tower==0)` 只决定要不要扫**队友**,而那个扫描只会把
   `isThereEnemyNearby` 置**真**、从不读 bot 自己;于是「敌方英雄**和**塔都在 1200 内」
   **整个跳过扫描**,然后照常开雾,而迷雾在敌人进 **1025** 时就破。变量名和外层条件都在说
   作者想问「**我**干净吗」,**而写出来的代码从没问过**。**但域 = 0 且是真读数**:
   8 个主槽携带帧上 `J.GetAllyList(bot,1200)` **全是 0**。
   **dust 是 UNMEASURABLE 不是 EMPTY**:分支 1 要 `time_since_seen ∈ (0.2,0.5)`,
   而 `replay_fixture.lua` **自己声明**只会发 **0 或 999** ⇒ 结构性不可达;
   分支 3 实测 `IsUnitWillGoInvisible` **0/5**;分支 2 走 `^Was -> false` 泛默认。
   **⭐ 先查后写,没有重造**:顺手核到 `bot:GetActiveMode()` 在 **1021** 帧上恒 **0**,
   **这已经是第十三条 world assertion**(`test_activemode_world_assertion.lua` +
   `test_activemode_call_site_census.lua`,`compare_lines==209`)。
   **唯一未登记的增量、已交出去**:那 209 行**按算符分层**是 `==`(压成恒 FALSE)与
   `~=`(压成恒 **TRUE** = **空过的守卫**)两类,而那份散文**只写了 FALSE 那一半**;
   本组粗扫读到 **129/87**(**未**过它的 `strip_line_comment`,**不可直接相减**)。
   **⛔ P4.4 主体配额本轮未达成、如实登记**:主体是测试与检测器,`bots/` 一字未动。
   三条理由并列:(1) 红是**全队**铁律 6 动态那半的门且是本组自己这一族第五次;
   (2) 本组范围内**唯二**可动的 `bots/` 杠杆跑完域价钱**都是 0**,落任何一条都是明文禁止的**空杠杆**;
   (3) P4.2 冻结期落地也只能 FROZEN-HOLD。
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue(带帧证据的优先);
   否则 —— ⛔ **先读这一句**:本组已**连续两轮**主体不是 `bots/`,**下一轮的主体必须是一个
   `bots/` 行为改动**,且**不许再从 `ConsiderItemDesire` 的单引号族里找**(8 条里属本组范围的
   三条已全部定价:smoke 域 0、dust 不可测、soul_ring 3 帧)。
   改从**本组自己的范围**里取一个**域一定非零**的谓词:TP 纪律 / 支援仲裁 / 推进-防守姿态
   一族的调用点(`midtp`/`suptp`/`tpcommit`/`ownhalf`/`overchase`/`pushguard`),
   它们读的是英雄位置、血量、建筑——**全是 fixture 的 ground truth**,不经过任何泛默认。
   **先跑域价钱,再动手;域为 0 就换下一个,但这一轮必须落一条杠杆。**

0GRENHARASS. **【2026-09-07T07:30Z 新增,**OWNER_PRIORITIES P4.4(i) 达成:工作单元主体 = 一个 `bots/` 行为改动**;
   工作流第 1 步扫 `[strategy]` open issue —— `#582`/`#578`/`#575`/`#572`/`#568`(本组前五轮已交付、等总监裁)
   与 `#558`(已认领并交回)⇒ **无未认领的带帧证据条目**;章程 `0CORPUSPIN` 的「下一格」**甲路**
   (`J.IsInTeamFight` 团战帧)**未到位**(扫了录像组最新四份报告)⇒ 走**乙路**;
   产出 `bots/ability_item_usage_generic.lua` 的 `grenharass` 块(turbo **显式**,**未 armed**,STANDALONE)、
   `tests/test_grenharass_domain.lua`(**12/12**)、`tests/_grenharass_sweep.lua`、
   `tools/agent/mutstand_grenharass.sh`(**15/15 GREEN**)、`tools/agent/chain_member_census.py`(GH #574 编辑费)、
   `state.json:grenharass_20260907`、`test_set.md §FS`;报告 `iterations/reports/strategy/20260907T073000Z.md`;
   **armed 串一字未动、`queue.json` 一字未动**。零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲)+ 批测台(乙)+ 录像组(丙)+ 英雄组(丁),总线 = GH #590。**】**
   **⭐ 先纠一句本组自己的记法:那条缝的根因不是「没有 consider 函数」。**
   章程把它记作「100 携带者 / 0 次投掷」,本轮开工第一遍 `grep 'ConsiderItemDesire\["'`
   **读回「这个道具根本没有 entry」—— 那是错的**:它用的是**单引号**。
   **数出来的**:该文件双引号键 **168** 个、单引号 **8** 个(`blood_grenade`/`disperser`/`dust`/`harpoon`/
   `pavise`/`pirate_hat`/`smoke_of_deceit`/`soul_ring`)⇒ **按双引号做的普查漏掉的是 8 个道具的整个决策层。**
   **⭐⭐ 真正的根因:entry 在,但两条分支都是击杀确认。**
   loop 1 要 `J.CanKillTarget(e, totalDmg)` —— 手雷自己的 **125** 必须够杀(275 那档坐在
   `IsFacingLocation(loc, 15)` 这个 **15 度**锥形后面);loop 2 要**队友自己的伤害已经够杀**
   (`GetTotalEstimatedDamageToTarget(allies) >= e:GetHealth()`)⇒ 一个 **25 金、最大囤 2、
   整个对线期反复补货、51 个英雄购买表都有**的对线消耗品,**只会被当处决用**。
   域价钱:1012 帧 / 109 fixture ⇒ 携带任意格 **238** → 主槽 **100** → 900 内有敌 **40**
   → 过滤器一帧没删(cand == loop)→ **`CanKillTarget(e,125)` 命中 `0`**(275 那档 3)。
   杠杆写在**道具自己的尺子**上:`GetHealth() <= totalDmg * 3`,域 **8**(目标血量 225–363)。
   **⭐⭐⭐ 两把独立量具落在同一个数上,而断言的是等式**:前缀行走 `domain` **8**
   == 驱动引擎动作 `cast_armed` **8**,`cast_ship` **0**(M8 专打这条)。
   驱动那列要给**手雷把手**补 IsTrained/IsActivated/IsFullyCastable(第十六条世界断言),
   **只补这一个把手**;**M13** 把它拿掉 ⇒ `cast_armed` 读回 **0**,**和「杠杆什么都没做」印出来一模一样**。
   **⭐⭐⭐⭐ 立法级:本轮最强的变异体在本语料上不动任何一个数字。**
   方向来自**位置**(追加在两条已发货 `return` 之后)⇒ **M7** 把块原样搬到两条分支**上面**:
   语法没问题、`domain`/`cast_armed`/`flips` **逐位不变**,抓住它的**只有结构断言**。
   ⇒ **当方向来自位置而不是谓词,守住方向的断言必须是位置断言;任何行为计数对它都是瞎的。**
   **⭐⭐⭐⭐⭐ 变异台第一版 M12 幸存,而缺陷在断言不在变异体**:sweep 自己那半结构解析
   **算了但没人读**(测试用自己的 parser 重读了一遍)⇒ **死仪表**。修法不是删,是新增
   「**两个 parser 对表**」section —— 理由是算术:域读数是**用 sweep 解析出来的数**量的。
   **⭐ 同轮结清一条 trunk 红,而它是 GH #574 的第三次实例**:+82 行把一条 judged 行的行号
   `8341 → 8423` 顶走;**工具是绿的**(GH #442 之后按链文本做键,只打 `LINE NOTE`),
   **收费的只有新鲜度断言**。付了这笔编辑费并写进注释 ⇒ **#574 的答案现在是测量不是意见:
   同一个文件三次插入、三次改这个字面量、每次都由一个跟该 finding 毫无关系的轮次买单。**
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue(带帧证据的优先);
   否则按 **P4.4(i)** —— ⛔ **但先读这一句**:`0CORPUSPIN` 的**甲路仍然是最高价值的一格**
   (`J.IsInTeamFight` 从未被本语料照亮过),它**只缺录像组的团战帧**,本轮已第二次交棒;
   若仍未到位,乙路的**下一条**候选是 §1 点名的那 **8 个单引号 entry** ——
   `item_dust`/`item_smoke_of_deceit`/`item_soul_ring` 三条属于本组范围(视野/开雾/续航),
   **先跑域价钱再决定动哪一条**,域为 0 就登记「定价后拒绝」并**再交一次语料棒**,**不落空杠杆**。

0CORPUSPIN. **【2026-09-07T05:00Z 新增,**认领 trunk 的 Lua 红(铁律 6),而它是本组
   01:20Z 自己落地的文件**;工作流第 1 步扫 `[strategy]` open issue ——
   `#582`/`#578`/`#575`/`#572`/`#568`(本组前五轮已交付、等总监裁)与 `#558`(已认领并交回)
   ⇒ **无未认领的带帧证据条目**;
   产出 `tests/test_wkreinctr_untrained.lua`、`tests/test_waitclar_mana_trip.lua`、
   `tests/test_stayfamily_singlearm.lua` 三个文件的分母改走 `tests/corpus_scale.lua`、
   `state.json:corpuspin_20260907`;报告 `iterations/reports/strategy/20260907T050000Z.md`;
   **`bots/` 一字未动、armed 串一字未动、`queue.json` 一字未动**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲:检测器判据 + 丁:关 GH #585)+ 批测台(乙)+ 录像组(丙:团战帧,第二次交)。**】**
   **⭐ 主判据:检测器报了 1 条,同一个缺陷在树上有 4 条。**
   `test_corpus_scale.lua` 的判据逐字是「**一个等于今天 fixture 数的整数字面量**」,
   于是它的**召回率是语料规模的函数** —— 这一点它自己的注释只写了**误报**那一半
   (落地第 109 个 fixture 把它瞄准了一个伤害总和 109),本轮量到的是**漏报**那一半。
   按 `HEAD` 逐字数:`C.fixtures == 109`(**看得见**)+ `C.live == 1012` **三处**(**看不见**),
   **1/4**。
   **⭐ 这是这一族的第 4 次,前一次就在五天前**(`state.json:corpuspin_trunkred_20260902`):
   GH #106(5 文件)→ GH #127(7 文件 / **18 断言**)→ **2026-09-02 总监**修
   `test_slotdust_dust_arbitration.lua:645` 的 `assert(nFixtures == 107)` → 本轮。
   四次并排读出一句单看任何一次都读不出的话:**两次被检测器抓到(107、109)都是因为钉的值
   恰好等于当天的 fixture 数;而「换个分母」那一变种(`C.live == 1012`)至今没被任何东西抓到过** ——
   它躲过了 09-02 那次修复、躲过了此后每一轮自检,**今天也不是被检测器发现的,
   是修它隔壁那条时顺手 grep 出来的。**
   三处漏网的不是别的缺陷,是**同一个缺陷换了个分母**(`live` 是帧数、`fixtures` 是文件数,
   都是「语料有多大」的重述),躲过去**只因为 1012 今天不等于 109**。
   **而三个文件全部是本组落地的,只用了连续两轮**(`git log --diff-filter=A`:
   `waitclar` 那轮 **22:55Z 一次落两个**,`wkreinctr` 那轮 **01:20Z 落第三个**)
   ⇒ **连续两轮复制同一个缺陷三遍,拦住第三遍的唯一原因是数字碰巧撞上了;
   而一次落两个那回,连间隔都没有。**
   **⭐⭐ 修法,以及为什么域内计数一个都不软化(理由是算术不是保守)。**
   分母走 `cs.ratchet(C.live, 1012, …)` / `cs.corpus(C.fixtures, …)`;
   `wk_frames`/`ship_true`/`flips`/`stop_*`/`wk_lv6`/`ship_true_untrained_lv6`/`pair_ne_arm`
   与全部恒零、全部恒等式**留等号**。`corpus_scale.lua` 的立论「行为改动表现为计数**下降**」
   是对**追加 fixture** 说的,**对变异体不成立**:本杠杆变异台的 **M6**
   (`GetLevel() >= 0` 换 `IsTrained()`)让 `flips` **14 → 24 涨**,**M7**(删调用点的 `GetLevel() >= 6`)
   让 `ship_true_untrained_lv6` **2 → 14 涨**,**ratchet 对上涨一言不发**。
   ⇒ **可复用的一条:`ratchet` 只对"重述语料规模"的量成立;对"这是我的发现"的量,等号是它的量程。**
   (已写进三个文件的行内注释,**不新增规则**——P4.3 元规则。)
   **⭐⭐⭐ 负控制的一个坑:一次运行里两个变异体,只能证明排在前面的那个。**
   第一版把 `ratchet` 与 `corpus` 两个负控制放进同一次运行,`ratchet` 先抛 ⇒
   `corpus()` **那一行从未被求值**,而整轮读数照样是 `EXIT=1`,
   **和「两个守卫都响了」印出来一模一样**。拆开重跑才拿到 `SHRANK` 那一行。
   **⭐⭐⭐⭐ 立法级(量具):`routine_selfcheck.sh` 的 trunk-red 腿与任何 `mutstand_*.sh` 不可并发。**
   本轮自检报 **3/84** Lua 文件红,其中**两条是假红**
   (`test_campbind_coarm_vacuous` 报 `jmz_func.lua:13078: unexpected symbol near '<eof>'`、
   `test_blinkflee_scope_ruling` 报 `no DONE line`)——**当场取证**:变异台跑到 M5 时
   `git diff --stat bots/` ⇒ `bots/FunLib/jmz_func.lua | 2 +-`,**树上那一刻带着一个变异体**,
   而同一棵树的 `luacheck_gate.sh` 几分钟前刚打 0 warnings。
   前者**读**工作树、后者**写**工作树 ⇒ **并发跑出来的 trunk-red 是噪声不是发现**,
   且它红得**跟真缺陷一模一样**(报的是检测器自己的用例名,一个字不提有人在改文件)。
   自检自带的那句「re-run after `git stash`」**防不住这个**:`git stash` 会把变异体一起藏走,
   重跑照样绿,**而那个绿同样不是真的**。
   **⭐⭐ rebase 撞车,而撞出来的是本条结论的独立实证。** `push origin HEAD:main` 被拒 → rebase 在
   `test_wkreinctr_untrained.lua` 上冲突,对面是**总监 04:45Z 的 `e472ebcc`**(同一小时),
   **两边逐字修的是同一处**(同样两行 `cs.ratchet`+`cs.corpus`,只有 label 与注释不同)。
   **重点不是撞车,是对面停在哪里**:`git show origin/main:` 查两个兄弟文件,
   `C.live == 1012` **各还在**(grep -c ⇒ 1/1)—— **总监修的正好是检测器点名的那一行,
   另外三条一条没动。不是疏忽,是没有任何东西点它们的名。**
   「召回率是语料规模的函数」本是本组 grep 出来的论断,这一小时里**另一个组在不知情下把它演了一遍**。
   ⇒ 按键名的判据**有了一次独立复现**。冲突解法:该 hunk 取 `origin/main`(总监先落地),
   本组只保留另外三个文件的修复;两边判据一致,差别仅 label 与注释,无信息损失。
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue(带帧证据的优先);
   否则按 **P4.4(i)** ——⛔ **但先读这一句**:`0WKREINCTR` 已付过价钱的结论仍然有效,
   现有语料(**1012 帧 / 109 fixture**)对「野外续航 / 回城 TP」这条缝**跑干了**
   (五条候选路域 0/0/3(假阳性)/0/0),且 `J.IsInTeamFight` 这条线**整个没被本语料照亮过**
   (36 个 WK 帧 `fight_true == 0`)。⇒ (甲)**若录像组的团战帧到位**,在
   `J.IsInTeamFight` 的**单个调用点**上取一个 gated 合取项(⚠️ 350+ 调用点,**绝不动 helper 本身**);
   (乙)**若未到位**,先跑 `item_blood_grenade`「100 携带者 / 0 次投掷」那条缝的**域价钱**
   (⚠️ 那需要一条**新行为**不是一次收窄,**方向不由构造固定**,按大捆绑教训格外小心),
   域为 0 就登记「定价后拒绝」并**再交一次语料棒** —— **不落空杠杆**。
...
0URNSELF. **【2026-09-06T16:53Z 新增,**OWNER_PRIORITIES P4.4(i) + P2(**决策/施法侧**,球在本组)**;
   ...
   更早的 `#385/#300/#254/#201/#198/#26` 同为存量或无帧证据 ⇒ **无未认领的带帧证据条目**,按铁律 9 取优先项;
   产出 `bots/ability_item_usage_generic.lua` 的 `X.ConsiderItemDesire["item_urn_of_shadows"]` 内
   **新增 gated self 分支**(`urnself`,turbo-only,**未 armed**,STANDALONE)、
   `tests/test_urnself_self_patient.lua`(**12/12**)、`tests/_urnself_sweep.lua`、
   `tools/agent/mutstand_urnself.sh`(**17 发**)、`state.json:urnself_20260906`、`test_set.md §FM`;
   报告 `iterations/reports/strategy/20260906T165337Z.md`;
   **armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结)**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲)+ 批测台(乙)+ 录像组(丙),总线 = GH #572。**】**
   **⭐ 主判据(承接上一轮,第三次用,而这次它的用法是新的):域价钱**答完**章程点名的那一格之后,
   **下一个候选也必须先付价钱 —— 而它否掉了最直接的那条路。**
   `0STAYTOWER` 点名的普查跑了:把 PROMOTED 的 `J.ShouldStayAndRegen` 四条 supply 杠杆
   (`staysrc`+`staybottle`+`staybag`+`bagsalve`)**同时 armed**,沿它自己的前缀走 1012 活体帧 ——
   到达 supply 子句 **125**、出厂放行 **13**、四杠杆全 armed 放行 **60** ⇒ **仍被否决 65**。
   逐件数那 65 帧:**49 帧真的两手空空**(归 `fieldbuy`)、约 **9 帧背包 tango/faerie_fire**(`bagsalve` 已裁)、
   **6 帧空瓶**(正确)、**8 帧带影之灵龛** —— 这一族的回复词表里**从来没有过它**,
   其中 **2 帧 `modifier_item_urn_heal` 是活的**(**正在被灵龛治疗**,而整条 supply 链读它们「两手空空」)。
   ⇒ **那一格的答案不是「没有了」,是「问得太窄」**:漏的不是 docstring 那句话的第四半,**是词表本身少一味药**。
   ⇒ 而最直接的动作(把灵龛加进 `J.HasFieldRegenSource`)**当场被 `bagsalve` 自己的理由否掉**:
   灵龛的病人循环是 `bot:GetNearbyHeroes(...)`,**引擎这个调用不返回调用者** ⇒ **没有可用的施法路径**,
   数它就是「把 bot 按在一个它永远不会喝的东西旁边」。**先把药能喝下去。**
   **⭐⭐ 落地的杠杆来自同一张表的兄弟条目**:`X.ConsiderItemDesire["item_flask"]`(400 血 / 13s,可自施)
   **有** self 分支 `hEffectTarget = bot`,还为「自己 vs 队友」单开了**三个 id**
   (`salveyield`/`salvepool`/`salveally`);`item_urn_of_shadows`(**400 血 / 8s,同样可自施**)**只有 ally 分支**。
   `urnself` 在 ally 分支的 `return` **之后**追加 self 分支,**每一条合取项都是那个 ally 循环的原话**
   (800 / 3.1 / 三个治疗 modifier / 450 / 空 `hNearbyEnemyHeroList` / 非魔免可施),外加**显式** `J.IsModeTurbo()`
   (**此路径上没有任何一处在它之上问过 turbo,写「结构性」会是假话**)。
   「只在没有队友够格时才开火」**是控制流不是合取项**;自己与队友都够格的 **2 帧故意留给队友**,本轮**不做仲裁**。
   域:带灵龛 **81** → 瓶颈是**缺血 >450 的 17**(不是危险子句)⇒ **自己够格 6**(无队友竞争 **4**),**6/6 在带内**;
   驱动读数 **c1 出厂 3(ally,控制组)/ armed 7** ⇒ **gain 4、loss 0**,且 **`gain` == `domain_selfonly` == 4 逐位相等**。
   **⭐⭐⭐ 第三条(立法级,变异台/量具):三样会让 `CAUGHT`/`SURVIVED` 撒谎的针,三样都是锚检查抓的。**
   台子**第一次跑 STAND RED,3 个 unexpected,而两个是锚不是杠杆**:(1) **不存在的针**(M3 少写了 `sCastType`);
   (2) ⛔ **不唯一的针 = GH #550 活体复现** —— 那行 450 在本文件**出现两次**(药膏那条逐字相同),
   `perl s///` 不带 `/g` ⇒ **改的是药膏**、灵龛的 450 原封不动 ⇒ 测试照样绿 ⇒ **SURVIVED**;
   **没有那行 `ANCHOR ... occurs 2 time(s)`,这次 SURVIVED 会被读成「套件有个洞」,然后有人去修一个不存在的洞**;
   (3) ⭐ **被 shell 语义压扁的针**:把针写成 `"...450$(printf '\n')...== 0 $(printf '\n')"`,**锚检查回来是 0** ——
   **`$(...)` 会剥掉结尾换行**,两个都展开成空串,**针被静默压成一行**,而它**长得像一个写好了的多行针**;
   正确写法是 **`$'\n'`**。附:灵龛那份针的第二行**带一个行尾空格**、药膏那份没有 ——
   **一个肉眼完全看不见的差别在承重**,这正是「**数**锚」而不是「**看**锚」的理由。
   **⭐⭐⭐⭐ 第四条:一个注释可以移动结构切分的边界。**
   sweep 第一次跑,**每一个结构事实都读作「这个杠杆不存在」**(`URN_NIDS 0`/`SELF_ASSIGNS_BOT 0`/
   `SELF_AFTER_ALLY_RETURN 0` —— **逐字就是杠杆从未写过的那份读数**),因为切块沿用兄弟 sweep 的
   `find('X.ConsiderItemDesire[')`,而**本杠杆自己的注释里就有 `X.ConsiderItemDesire["item_flask"]`**
   ⇒ **切块在自己的注释处被切断**;改成**行首锚定**,做成 M15 CAUGHT。
   同族:`find('hEffectTarget = bot')` 命中十二行外 `hEffectTarget = botTarget` 的**前缀** ——
   侦察探针因此对**未打补丁的文件**报了 `SELF_MENTION=1`。**针必须自带终结符。**
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue(带帧证据的优先);
   否则按 **P4.4 在 `bots/` 上取一个小杠杆**,候选(**标注:未定价的猜测,先跑域价钱,价钱有权否掉它**)——
   **`urnself` 打开之后,灵龛才有资格被问「要不要进 `J.HasFieldRegenSource` 的词表」**,
   那是本轮**明确拒绝回答**的问题,而拒绝的理由(没有可用的施法路径)正是本轮消掉的;
   **先跑域价钱**:那 65 帧里带灵龛的 8 帧,有几帧同时满足 `urnself` 的域?
   **答案很小就把这条路判到此为止**,不要因为「顺理成章」就落一个 id。
   **照旧先跑域价钱、每个变异体的锚先数一遍(连同它是否与上一轮为唯一性选的写法重复)、恒零的断言先证明它数得动。**
...
0BUYRING. **【2026-09-06T11:05Z 新增,**OWNER_PRIORITIES P4.4(i)(工作单元主体 = 一个 `bots/` 行为改动)+ P2(**供给侧**,球在本组)**;
   ...
   **无未认领条目** ⇒ 按铁律 9 取优先项;产出
   `bots/FunLib/jmz_func.lua` **新增** `J.ShouldFieldBuyRegenRing`(`buyring`,turbo-only,**未 armed**,STANDALONE)
   + `bots/item_purchase_generic.lua` 购买点**第四条 OR 臂**、
   `tests/test_buyring_purchase_domain.lua`(**19/19**)、`tests/_buyring_sweep.lua`、
   `tools/agent/mutstand_buyring.sh`、`state.json:buyring_20260906`、`test_set.md §FJ`、
   nesting census **三行**、`tests/test_bagsalve_backpack_source.lua`(调用者 5→6)、
   `tests/_buytower_sweep.lua` 的 `WIRE_OR3`(**容忍再加臂**)、`tools/agent/mutstand_buytower.sh` **M2 重新上锚**;
   报告 `iterations/reports/strategy/20260906T110500Z.md`;
   **armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结)**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲:FROZEN-HOLD + 两条规程建议)+ 批测台(乙:单臂可读,与三个兄弟重叠各 0)+ 录像组(丙:朝向/速度 fixture 读不到)。**】**
   **⭐ 主判据(立法级,可复用):「先跑域价钱」有权把杠杆否掉,而它这次就否掉了上一格点名的下一格。**
   `0TOWERRING` 点名的是**归因子句**(「被已归因伤害挡住的 28 帧里,多少是全局/远程一发的形状」)。
   先跑价钱:28 帧里 **25 帧同时也在 1600 环里**(环本来就挡)、再去掉 2 帧有塔 ⇒ **只剩 1 帧**;
   而那 1 帧的已归因伤害者站在 **1,615.81u**,**离环只有 15.81u** —— 按任何读法都不是「全局技能/远程一发」。
   ⇒ 候选**域 = 1 帧且前提在那一帧上不成立**,不做。**顺带推翻了上一格写下的前提**:
   供给侧三条臂**早就继承了 `stayattr` 的修复**(三条都带已归因扫描;语料里 3 帧「有英雄伤害但归因不到」今天就已放行)。
   ⇒ **「章程点名的下一格」不是判决,是上一轮在没有价钱的情况下写下的猜测,而域价钱是唯一能审它的东西。**
   已把这一条作为规程建议交给总监。
   **⭐⭐ 落地的杠杆来自同一次普查(最大拒绝桶)**:同一个家族对**同一个问题**在**同一条 HP 带**上 ship 了两个半径 ——
   决策侧 `J.ShouldStayAndRegen`(**PROMOTED**)量 **1200**,供给侧量 **1600**,
   而 1600 的理由写在它自己的注释里:「the ring **the guarded branch itself measures**」——
   一句关于**被取消的那条 retreat bid** 的话,**而买臂不取消任何东西**(§FF `buytower` 的同一个形状,这次落在环上)。
   域价钱:带内 305 → 两手空空 163 →(1600 环 99 / **1200 环 87** / 归因 28 / 有塔 22 / 什么都没挡 53)
   ⇒ **两环之差 99−87 = 12**(写成**减法断言**,不是第四个可漂移的数),减去归因 **0**、塔 **2** ⇒ **域 10 帧**;
   `flips_buyring 10 == ring_domain 10`,三个 overlap 全 **0**,`overlap_probe_runs == live == 1012`,`6+4=10` 两侧非零。
   钉帧就是 **P2 点名的那份 063559**:slardar 46.0%、身上没有任何能喝的、最近敌人 tidehunter **1,565.36u** 且没碰过他。
   **⭐⭐⭐ 第三条(变异台规程级):锚唯一性不是一次性属性,是每次新增都要重数的。**
   本杠杆带子第一版写作两条语句 —— 那正是**上一轮 `buytower` 为唯一性选的写法**,于是与它**逐字节相同**,
   而 `mutstand_buytower.sh` 的 M5/M6 用的就是它们;改成 `if nHP > 0.75 or nHP < 0.18 ...` 一条语句(全文件 1 次)。
   同一次重数**还抓到一处已经坏了的锚**:该台 M2 的 needle 结尾是**三臂 OR 的右括号**,第四条臂一进来就**出现 0 次**;已改。
   **本台自己的 anchor 检查又抓到我自己的三处错误声明**(台子因此跑了三遍),其中最值得记的是 **M16**:
   那一行在 sweep 里只写过两次(主 stub + 探针里把 stub 还原回去),而后者**缩进更深** ——
   **缩进 24 空格的 needle 是同一行缩进 28 空格时的子串** ⇒ 计数得 2,**而肉眼看不出任何重复**。
   ⇒ **「数锚」防的不是手滑,是「同一段文本在另一个缩进层级上再次存在」这种看不见的重复。**
   ⚠️ **另一条操作教训**:我在台子**还在跑的时候**编辑了它 —— `bash` 按**字节偏移**边读边执行,
   在当前偏移之前插入字节会让后续读取错位 ⇒ **那次运行作废、整台重跑**
   (SIGTERM 让 EXIT trap 跑完并复原,`git diff` 复核 `bots/` 干净)。**改一个正在跑的 stand = 作废这次运行。**
   **附带(工具,一条)**:本轮第一个域价钱探针回来是「**stdout 全空 + 退出码 0**」——
   mock 的 `install()` 把 `print` 换成空函数(**GH #546**),与「跑了什么都没发现」逐字节相同;改用 `io.stdout:write` 后打出 1012 行。
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue;否则按 **P4.4 在 `bots/` 上取一个小杠杆**,
   候选(**标注:未定价的猜测,先跑域价钱,价钱有权否掉它**)——
   **`fieldbuy` 那 99 帧里被 1200 环挡住的 87 帧**:PROMOTED 的 hold 侧在这 87 帧上**也**拒绝,
   所以这里没有「家族自相矛盾」可用,**要另找理由或判定这条路到此为止**;
   **照旧先跑域价钱、每个变异体的锚先数一遍(连同它是否与上一轮为唯一性选的写法重复)。**

0TOWERRING. **【2026-09-06T07:30Z 新增,**认领 `[strategy]` open issue #558**(录像组 07:08Z 开,
   248 个 R_lever episode 帧证据 + 建议钉帧 + 建议验收方式,**未被任何组认领**)——
   本组工作流第 1 步(扫 issue,优先带帧证据的)就命中,**不是 backlog 自选**;
   同时满足 **OWNER_PRIORITIES P4.4(i)**(主体 = 一个 `bots/` 行为改动);
   产出 `bots/mode_retreat_generic.lua` X.ShouldRun「前期谨慎冲塔」块**新增**收窄门
   ...
   **armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结)**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲:FROZEN-HOLD + 一并裁 #558 对 `towerfear` 的建议)+ 批测台(乙:单臂可读,
   与 `towerfear` 同波必须分臂或按支配读)+ 录像组(丙:GH #558 的钉帧本容器取不到)。**】**
   **⭐ 主判据(立法级,可复用):一个已 armed 的 id 被核验为「执行正确、行为方向错」时,
   正确的下一步不是改它,是在它旁边落一个收窄版的新 id。** 改它 = 让一个**正在被测量的 id 在
   arm 串 md5 不变的情况下换掉语义**,W50 已经用 `towerfear` 买到的 (a) 前半读数会静默变成
   关于另一个杠杆的。代价是必须回答「两个同时 armed 时谁赢」,而本轮把那个答案**做成语句顺序**
   (restore 排在 halve 之后)并**钉在测试里**(DOMINATION,七帧全测),因为**pair 才是 promote 会发的配置**。
   M4(把 restore 挪到 halve 之前)就是它的变异体:**每条语句都还在、单独看都对**,死掉的只有 pair 的读数。
   不嵌进旧门里的理由是 GH #542/#553(两个 id 同路径 ⇒ 都读不出来)。
   **⭐⭐ 第二条(变异台规程级):「我预测 UNMEASURABLE、结果 CAUGHT」是奉承套件的那个方向,
   因此是最不会被复查的方向。** M7(`<`→`<=`)首跑判 UNMEASURABLE(行为上确实没有一帧在 700.00u),
   但**源码断言那半看得见**(`<%s*700` 不匹配 `<= 700`)⇒ CAUGHT,台子一行就说了 `re-classify`。
   改判后补 **M7b**(连源码断言一起改 —— 扮演「调了个数顺手把测试改对」的人),**那才是在问行为半边**;
   而 **M7b 自己的第一版是 NO-OP**:perl 针把 Lua 模式的 `%)` 写成 `)`,一处没匹配上,
   于是它只是 M7 的第二份拷贝并打印 CAUGHT —— **颜色对、程序错,GH #550 同族**,
   只是错的不是锚的唯一性而是**拼写**,两者失败输出**逐字节相同**。已改成带 `count == 3` 断言的字面替换。
   **M6/M6b 同一形状** —— 成对才说得出「牙齿到哪里为止」。
   **⭐⭐⭐ 域价钱(先跑,而它给出的是一个零,如实登记)**:1012 活帧 / 32 帧 898 环内有塔 /
   17 帧粗子句被问到 / 3 帧开火 / **仅 1 帧由时钟腿单独持有,而它在 727.46u 属于环带**
   ⇒ **本轮杠杆的「限制域」在冻结语料上是 0 帧**。这是**语料**的事实不是杠杆的
   (fixture 为别的理由冻、批测局攻塔前自终止;GH #558 的 248 episode 才是总体读数)。
   ⇒ 每条**限制**断言只移动 `DotaTime()`,**塔距是帧自己的**;`towerfear` 文件本身就用过这个写法。
   ⚠️ **700 在行为上只被括在 (649.41, 727.46),宽 78u**;离边界最近的 juggernaut **701.99u**
   **不可用**(更靠前的分支在 5 个时钟上都出价 0.75,四世界全等),luna 245.20u 同理 ——
   **两帧写成断言而不是悄悄丢掉**:一个长得像对照、却在每个世界给同一个答案的帧,
   正是最容易被当成证据的形状。出厂不变的证据是**读数不是论证**:普查输出前后 **byte-identical**。
   **附带(量具,一条)**:开了 **GH #559** —— 两轮同时认领同一个 `§XX` 时,挡住它的是 **git 不是检查器**,而 git **只在两节都追加到文件末尾时**才会撞;附了一个 `uniq -d` 级别的只读检查与验收。
   **下一格(本组下一轮第一项)**:先扫未认领 `[strategy]` issue(本轮证明这条路是活的);
   否则按 **P4.4 在 `bots/` 上取一个小杠杆** —— 候选:上一格 `0BUYTOWER` 点名的那一条仍未做,
   即**「被已归因英雄伤害挡住的 28 帧里,有多少是全局技能/远程一发的形状」**
   (`stayattr` 在**决策侧**已证、**供给侧三条臂都还没继承**的同一个缺陷);
   **照旧先跑域价钱、先断言控制帧够得着被控制的代码,并且每个变异体的锚先数一遍、拼写也数一遍。**

0BUYTOWER. **【2026-09-06T04:35Z 新增,**OWNER_PRIORITIES P4.4 + P2(**供给侧**,球在本组)**;
   `[strategy]` open issue 本轮扫过 **#521**(及章程已记录的 #511 / #503)—— **无一条未认领**
   ⇒ 按铁律 9 取优先项,并做章程上一格(`0BUYBAND`)点名的两个候选之一;
   **P4.4 归属:主体是 (i) 一个 `bots/` 行为改动**;
   产出 `bots/FunLib/jmz_func.lua` **新增**谓词 `J.ShouldFieldBuyRegenTower`(`buytower`,turbo-only,**未 armed**,
   STANDALONE)+ `bots/item_purchase_generic.lua` 购买点**第三条 OR 臂**、
   `tests/test_buytower_purchase_domain.lua`(**18/18**)、`tests/_buytower_sweep.lua`、
   `tools/agent/mutstand_buytower.sh`(**带新的 `anchor` 前置检查**)、`state.json:buytower_20260906`、
   `test_set.md §FF`、nesting census **三行**、`tools/agent/mutstand_buyband.sh` **三处重新上锚(M2/M9/M17)**;
   报告 `iterations/reports/strategy/20260906T043525Z.md`;
   **armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结)**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲:FROZEN-HOLD + 「锚唯一性」变成机制的规程建议)+ 批测台(乙:单臂可读,与两个兄弟重叠各 0,
   与 `fieldcreep` 同波要登记 2 帧分歧)+ 录像组/语料侧(丙)。**】**
   **⭐ 主判据(立法级,可复用,超出本主题):判据不是「这次改动让几个检测器变红」,
   是「每一个变红的文件对这次改动有没有话说」。**
   上一轮(`0BUYBAND`)量到 7 个变红、**七个里没有一个对那条杠杆有话要说** ⇒ **撤销**;
   本轮对真正做出的改动用同一把尺:恰好 **3 个**变红(`nesting_census` / `bagsalve_backpack_source` /
   `buyband_hp_band` 的 `WIRE_OR`),**三个都在说这次改动的事**,于是三个都是**重读性质**修掉的、
   **没有一个是抬数字**。⇒ **七个说不上话 = 重新基线化(该撤销);三个说得上话 = 这棵树在审这次改动。**
   **⭐⭐ 第二条(变异台规程级,这次是量出来的、不是被存活撞出来的):GH #550 当时还有两处活的,在同一台台子上。**
   锚 `if #J.GetNearbyHeroes( bot, 1600, ... ) > 0 ...` + 紧跟的 `-- Attributed danger` 在 `jmz_func.lua` 里
   **出现三次**,而 `mutstand_buyband.sh` 的 **M9 与 M17 用的正是它** ⇒ 两发一直砍在**共享谓词**上,
   M9 靠漂移守卫、M17 靠无限递归各自变红 —— **颜色对,函数错**。已重新上锚;
   并把这条**从教训变成机制**:新台子带 `anchor <期望次数> <文件> <锚>` **前置检查**,不等就判**台子失败**。
   ⚠️ 该检查**第一版自己就是错的**(`grep -F -c` 把多行锚当成多个模式、按行计数,
   一个出现 1 次的两行锚被报成 **13133** 次),已改成整文件字面子串计数 —— 一个能把 1 报成五位数的计数器不是检查。
   本轮杠杆的带子**故意写成两条语句**同理:写成一行会与 `J.ShouldStayAndRegen` 的带子行逐字节相同。
   **⭐⭐⭐ 域价钱**:`[0.18,0.75]` 带内 **305** → 主格位空手 **163** →(环 99 / 归因 28 / 有塔 22 / 什么都没挡 53)
   ⇒ **只被敌塔挡 = 本杠杆域 8 帧**。**不相交靠「取反」不靠带子**(兄弟要求 1200 环空,本臂要求非空)
   ⇒ 三域在**整条 HP 带**上不相交,`overlap_tower_buy` / `overlap_tower_hurt` 各 **0**,
   `overlap_probe_runs == live == 1012`;`flips_buytower 8 == tower_domain 8 == nosrc_tower_only 8`;
   `flip_true_to_false` / `arm_leak` / `raises` 全 **0**;`4 + 4 = 8` 跨 0.55 两侧且**两侧都非零**。
   ⚠️ **一条不空的诚实边界**(与上一轮的差别):没抄的 `fieldcreep` 分歧在本语料上**不是空的** ——
   8 帧里 **2 帧**吃了小兵伤害,已写成精确等值断言。
   ⚠️ **另一处自我更正,发表前抓到**:草稿写「8 帧全部在塔的 700 射程外」是**错的**,
   实测 8 个最近敌塔距离 **221/688/787/987/1021/1039/1090/1130** ⇒ **2 帧本来就在射程里**。
   **下一格(本组下一轮第一项)**:取未认领 `[strategy]` issue;否则按 **P4.4 在 `bots/` 上取一个小杠杆** ——
   候选:**163 帧里被 1600 环挡住的 99 帧**里,`fieldbuy` 的注释把「环空」登记成了**购买有没有意义的前提**,
   那条论证只覆盖**敌方英雄**;**环里只有敌方小兵/野怪时它并不成立**(1600 环读的是英雄,所以这不是那一条)——
   真正该问的是**「被已归因英雄伤害挡住的 28 帧里,有多少是全局技能/远程一发的形状」**
   (那是 `stayattr` 在**决策侧**已经证过、而**供给侧三条臂都还没继承**的同一个缺陷);
   **照旧先跑域价钱、先断言控制帧够得着被控制的代码,并且每个变异体的锚先数一遍。**
...
0OUTCOMMIT. **【2026-09-05T13:4xZ 新增,**接下 GH #511 交棒 (乙)**(录像组 06:5xZ 把结构物 modifier
   ...
   **P4.4 归属:主体是 (i) 一个 `bots/` 行为改动**;
   产出 `bots/mode_outpost_generic.lua` 的 gated 块 `outcommit`(turbo-only,**未 armed**)、
   `tests/test_outcommit_channel_hold.lua`(**11/11**)、
   `tests/fixtures/outchan/f_260905_010205_luna_{channel,precast}.lua`、
   `tools/agent/mutstand_outcommit.sh`(**16/16 CAUGHT,零 NO-OP,零 SURVIVED**)、
   `state.json:outcommit_20260905`、`test_set.md §EW`、
   报告 `iterations/reports/strategy/20260905T134234Z.md`;
   **armed 串一字未动、`queue.json` 一字未动(P4.2 入集冻结)**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(甲)+ 批测台(乙,冻结解除后单臂)+ 录像组/语料侧(丙:替身遗迹)。**】**
   **⭐ 主判据(立法级,可复用,超出本主题):「缺 X 守卫」和「出价没给沉没成本定价」
   落在同一个文件里、指着同一段录像、读起来一样,而只有后者动得了东西。**
   ⇒ 一个「缺守卫」的论断被推翻之后,不要就地找第二个守卫,要**往上一层**问
   「这个模式有没有为它已经付出的代价出价」。
   **⭐⭐ 读数**:真帧上出厂 **0.7197** → armed **0.9**;四个对照逐位等于出厂;
   闸块在**每一条出厂否决之下**(行号断言 + M8)。
   **⭐⭐⭐ 两处自伤**:第一版反真空对照**是真空的**(M5 SURVIVED —— 控制帧早五条子句就返回 NONE);
   收窄兄弟棘轮后 **M6b 当场 SURVIVED**(收窄把变异体挪出射程,**表现是「更绿了」**)。均已修 + 各加一条断言。
   **下一格(本组下一轮第一项)**:取未认领 `[strategy]` issue;否则按 **P4.4 在 `bots/` 上取一个小杠杆**,
   优先 **P2 决策侧的下一格** —— `J.ShouldStayAndRegen` 的补给子句 `bot:GetGold() < 90` 在 Turbo 里
   是否还是对的门(注意 GH #495:mock 的 `GetGold` 落在 `^Get -> 0` 兜底上,**先解决怎么读到真钱**);
   **照旧先跑域价钱**,并**先断言控制帧够得着被控制的代码**(本轮 §EW.3 的账)。
...
0MIDSUPINT. **【2026-09-05T01:30Z 新增,**认领 GH #503**(P1 (甲) 卡 `strategy-43`、P2/P3 球在总监;
   ...
   **#503 是 00:14Z 新开、不是本组已交棒的**,且它逐字把成员的书写归给本组
   (「成员是协同组写,不是总监」)⇒ 章程「下一格」第一条成立,**本轮没有换域**);
   **前四轮都是零行为改动的域价钱,本轮有行为改动,但只有一条合取项**:
   `bots/FunLib/jmz_func.lua` 的 `J.HasAvailableSupportResponder` 补上
   `and not J.CanEnemyInterruptTpChannel( hAlly )`;
   产出 `tests/_midsupint_sweep.lua`;`tests/test_midsupint_mirror_interrupt.lua`(**8/8**);
   `tools/agent/mutstand_midsupint.sh`;两处 ratchet 重取;
   `state.json:midsupint_20260905`;`test_set.md §EN`;
   报告 `iterations/reports/strategy/20260905T013000Z.md`;
   **armed 串一字未动、`queue.json` 一字未动、无新 id/闸**;零 AWS、零 S3、零 EC2、零波次。
   **已交棒总监(`midsupyield` 入不入 armed 集)。**】**
   **⭐ 主判据(立法级,可复用,超出本主题):把一个否定结果 **ratchet** 起来,而不是当结论存档
   —— 因为解锁它的修复可能发生在**别人的作用域**里,而那一天你自己的 backlog 上没有任何东西在看。**
   §EH 把 §EF.7 的四条镜像成员定价为「一条都作不了证」。那本可以作为结论归档 ——
   然后它会**一直读作四条**。解锁其中一条的修复是 **harness 的**(GH #492,总监补 mock 的
   `GetExtrapolatedLocation`,§EL),协同组 backlog 上**没有任何一条会在那天注意到**;
   是 `test_midsupmirror_checkability.lua` 的 `[ratchet]` 腿**按设计开火并点了名**。
   可操作:**一个「今天做不了」的结论,若它的原因住在别的组的作用域里,就必须写成会自己
   开火的断言,而不是写成散文。**
   **⭐⭐ 替代解释是被量掉的,不是被论证掉的。** 敌人进 700 打断扫描通常也算「1600 内有团战」,
   而 `IsInTeamFight` 就排在这条新子句上面**一行** ⇒ 「本来就被挡住了」是活的替代解释,
   而它正是前一天 `droppick`(§EK)量出来的那个形状(0/5932,与未修树逐位相同)。
   实测:守卫拒掉 **20** 个候选,**其中只有 4 个**已被上面那条腿挡住 ⇒ **不被吸收**。
   **⭐⭐⭐ 域价钱**:**7048** 个 (英雄, 塔) 对里 **151 对翻转**,分布在 **20** 个活帧;
   接受集 **2366 → 2215**,且 `ship_true + flips == pre_true` **逐位相等** ⇒
   **方向是收紧,只能朝 shipped 走**(这不是注释里的声明,是那条断言)。
   反漂移 `shipped_disagrees` **0/7048**。
   **⭐⭐⭐⭐ 铁证帧**:`f_260820_043637_axe_ring_close` **t=393.4** —— 核心 slardar 距塔 **15,747**
   (过了远端闸 ⇒ **可达**的决定瞬间),而未修镜像给出的**唯一**支援是
   **skywrath_mage,24% 血,axe 在 188 外(攻击距离 300)** ⇒ 条被打断,这条前线**谁也没去**。
   独立第二例 `f_260820_103216_cm_es_aftershock` t=473.5(earthshaker,crystal_maiden 在 196)。
   **⭐⭐⭐⭐⭐ 不开新 id 是 pullcad 陷阱的直接后果**:这个谓词**唯一**的消费点已被
   `IsSoakCandidate('midsupyield')` 闸住 ⇒ 这里再加一个 id,活条件就成 `midsupint and midsupyield`,
   **任一被 promote 的那天冻成 FALSE**,而 `check_armed_wiring.py` 仍叫它 **WIRED**。
   已从散文升级成断言(`MIRROR_SOAKID == 0`)并钉成 M3。
   ⚠️ **一处仪器自伤,当场收账**:那条断言**第一次跑就读出 1**,树里却一个 id 也没有 ——
   命中的是**修复自己那段解释「为什么这里没有 id」的注释**。⇒ 凡「这个谓词**应用**了哪些闸」
   的论断必须**读代码不读散文**;已 `strip_comments`,钉成 **M4**,同族修正落到孪生计数上。
   ⚠️ **开工自检与本轮编辑并发,它那两条 lua 红是工件不是读数**(读到的是 `short` 4→3 的
   中间态)—— **一次跑到一半被换了树的全量,产出的是工件**,§EK 刚记过,本轮自己又制造了一次;
   结算读数以定树上重跑的铁律 6 为准。
   **下一格**:球在**总监** —— `midsupyield` 要不要入 armed 集(**在它 armed 之前,这条成员在
   真实对局里没有可观察行为**,所以录像组的条件 (a) 排在它后面,不是前面)。
   §EF.7 剩下三条**不要动**(不可作证,理由已定价)。
   **本组下一轮**:取未认领 `[strategy]` issue,否则换域,照旧先跑域价钱。

0DROPPRICE. **【2026-09-04T22:30Z 新增,**认领 GH #452**(P1 (甲) 卡 `strategy-43`、P2/P3 球在总监;
   `[strategy]` open issue 本轮扫过 #495/#492/#489/#485/#480/#475/#467/#464/#456/#455/**#452**/#445,
   **#452 不是本组已交棒的** —— 它的交棒节逐字写着「**本组(下一轮或谁先拿到)**」⇒
   章程「下一格」的第一条(取未认领 `[strategy]` issue)在这里成立,**本轮没有换域**;
   认领后照 `0PRICE4` 先跑域价钱,**没碰 §EF.7 那四条,也没补 mock 的任何一条腿**);
   **产出是一份真帧普查 + 一份真帧用例 7/7 + 一台新变异台 12/12 CAUGHT(零 NO-OP)
   + 两条交棒,零行为改动**:
   `tests/_droppick_sweep.lua`;`tests/test_droppick_domain.lua`;
   `tools/agent/mutstand_droppick.sh`;`state.json:droppick_PRICING_20260904`;
   `test_set.md §EK`;**GH #500**(另开 **GH #501** 记一条 trunk 红);
   报告 `iterations/reports/strategy/20260904T223000Z.md`;
   `bots/`/`game/` 逐字节未动、armed 串一字未动、`queue.json` 一字未动、无新 id/闸/fixture;
   零 AWS、零 S3、零 EC2、零波次。**已交棒总监(甲)+ 录像组/dumper(乙)。**】**
   **⭐ 主判据(立法级,可复用,超出本主题):当一个缺陷有两半而只有一半看得见时,
   看得见的那一半不是更便宜的那一半 —— 它是诱饵。**单独落地它产出的 diff **改了源码、
   过得了评审、读上去像修复**,而实测**与未修树逐位相同**。可操作:**凡「补一个实参 /
   改一个名字」型的修复,先问「这个集合是用什么建的键,读的人拿什么问」** ——
   两边不是同一个东西时,元数那一半是装饰。
   **⭐⭐ 机制:四行,两个互相独立的缺陷。**`bots/mode_team_roam_generic.lua` 的
   `:1771`/`:1819`/`:1823` 三处读 + `:1821` 一处写,对着 `utils.lua:1260` 的
   `SetContains(set, key)`。**(1) 元数**:三处读只传一个实参 ⇒ `set` 绑到物品**名字**、
   `key` 绑到 nil;`("item_aegis")[nil]` 经字符串元表是**合法的读**、答 nil ⇒ 闸恒假,
   **且不抛错**(`shipped_raise` **0/5932**),这正是它一直没人指着的原因。
   **(2) 两套下标空间**:写存 `PickedItem.item`(**句柄**)当键,三处读全按**名字**问 ⇒
   补上那个缺失的实参**仍然一次都不命中**。`0SLOT`/`slotarb` 之后**同族第三次**。
   **⭐⭐⭐ 三条腿(真实 shipped 函数,5932 个真实句柄 / 117 个名字)**:
   `shipped` **0/5932**;`argfix`(= 一行修法)**0/5932**;`bothfix` **5932/5932**。
   `bothfix` 同时是**反真空对照** —— 同一对函数、同一批句柄、只换索引空间,而它**能**说
   「已忽略」,在每一个句柄上 ⇒ **两个 0 是读数不是卡死的探针**。**修法是两半一起,否则等于没修。**
   **⭐⭐⭐⭐ 第二条发现(#452 没有):重试循环算出来的 desire 谁也收不到。**
   `ItemOpsDesire` 在循环里 `return RemapValClamped(...)`,而**唯一**调用点
   `GetDesireHelper:271` 把它当**裸语句**调用、把值扔掉 ⇒ 那个 `return` **不是 desire,
   纯粹是一次 skip**;跳过的是**八个家务调用**(`TrySellOrDropItem`/`SwapSmokeSupport`/
   六个 `TrySwapInvItemFor*`)**外加 `ConsiderDroppedTime = DotaTime()`** ——
   这个函数自己那道 2 秒节流的重新上膛。又因 `PickedItem` 是模块级、只在 `OnEnd` 清空,
   而放弃机制是死的,**触发这次 skip 的条件是黏住的**。两条都钉成 `[source]` 断言。
   **⭐⭐⭐⭐⭐ 不落地是结构性的**:那个**决定**在本语料买不到(`GetDroppedItemList()`
   mock 答 `{}`、dumper 不写这条记录 ⇒ **0/1012 活帧**);但语料**带得起其余每一条前提**
   (`ItemOpsThink` 的库存余位闸 **983/1012**)⇒ **价钱指名了恰好一个缺失字段**,
   「买不到」在这里是一句**有地址**的话。**诚实边界**:5932 个句柄来自 fixture 的**库存**,
   作证的是**集合管路**(句柄 vs 名字),**不是那个决定** —— 已写进普查头与测试头。
   **⭐⭐⭐⭐⭐⭐ 变异台里三发是这台存在的理由**:**M8** 让 `argfix` 腿偷偷按名字建键 ⇒
   **照常跑、照常出数,而那个数是对照组的**,于是「一行修法没用」会拿**不是一行修法**的腿去量、
   **反方向**报出来还不吭声;**M9** 让对照腿按谁也不问的键建 ⇒ 三条腿全读 0、**内部自洽**、
   与真读数不可分辨;**M12** 钉最危险的方向 —— `drop_frames == 0` 是「不许落地」的依据,
   若这个 0 能由**没去问**产生,那条裁定就是**不可证伪**而非被测量的 ⇒ 普查文件本身也被
   `[source]` 断言钉住必须真的 `pcall(GetDroppedItemList)`。
   ⚠️ 开工自检 **EXIT=3**:`cadence`/`owed-executions`/`trunk-red(python)`;python 三条
   **都不是本轮引入**(本轮零 python 改动):#490、#497,以及第三条
   `test_mutstand_restore_trap.py` —— 唯一失败行是
   `mutstand_campbind_poke.sh traps a function that exists (cp)`,**未见 open issue,本轮补开 #501**。
   **它是误报**:那条 trap 写作 `trap 'cp "$SAFE/orig.py" "$SRC"; rm -rf "$SAFE"' EXIT`,
   **先还原后删副本、顺序正确**,而检测器的正则把首个 token `cp` 当函数名 ⇒ 打出
   「the trap fires and does nothing」,**诊断与事实相反**。这正是 **GH #468 自己命名的
   「检测器只认得一种拼法」** 的**第二次现场,而且就在同一个文件里、隔一天**。
   ⚠️ 第一次调自检又用了 `| tail`,脚本自己拒绝并打 **EXIT=2**(REFUSED 不是通过),
   改重定向重跑 —— **同一脚印第三次**。
   ⚠️ **动态半没有有效读数**:全量在飞行途中被 rebase 换了树 ⇒ 它那条 `test_fixture_kv_getters.lua :: 9b`
   **是工件不是发现**(rebase 后单独重跑 31/0)。**跑到一半被换了树的全量,产出的是工件。**
   ✅ **GH #290 顺序本轮守住了**:先 `git push`,再跑 `claim_precheck.sh`(两份草稿各 EXIT=0),
   最后才发表 issue —— **#500 / #501,编号与已推送的引用一致**。
   **下一格**:(甲) 在总监手上(mock 的 `GetDroppedItemList` 补不补,**要连同 §EJ.2 的顺序约束
   一起裁**);(乙) 在录像组/dumper 手上(写一条掉落物记录才真正解锁验证)。
   **在任一回来之前,不要单独落地元数那一半** —— 已量出它是 0。
   **本组下一轮**:取未认领 `[strategy]` issue,否则换域,照旧先跑域价钱。

0MOCKPRICE. **【2026-09-04T19:30Z 新增,**自驱**(P1 (甲) 卡 `strategy-43`、P2 球在总监、P3 责在总监;
   `[strategy]` open issue 本轮扫过一遍(#492/#489/#485/#480/#475/#467/#464/#456/#455/#415)
   **全是本组已交棒的**;取 `0PRICE4` 「下一格」写死的 **换域 + 先跑域价钱**,
   且**没有碰 §EF.7 那四条**);
   **换到哪个域是被 `0PRICE4` 自己的主判据挑出来的** —— 它的结论是「决定能不能落地的是
   **语料能不能作证**」,而**挡住那条轴的正是 mock 的兜底规则**,所以本轮的题就是
   **那条兜底规则本身值多少钱**;
   **产出是一份真帧普查 + 一份真帧用例 7/7 + 一台新变异台 12/12 CAUGHT(零 NO-OP)
   + 两条交总监的 harness 裁定,零行为改动**:
   `tests/_mockscalar_sweep.lua`;`tests/test_mockscalar_return_shape.lua`;
   `tools/agent/mutstand_mockscalar.sh`;`state.json:mockscalar_PRICING_20260904`;
   `test_set.md §EJ`;**GH #495**;报告 `iterations/reports/strategy/20260904T193000Z.md`;
   `bots/`/`game/` 逐字节未动、armed 串一字未动、`queue.json` 一字未动、无新 id/闸/fixture;
   零 AWS、零 S3、零 EC2、零波次。**已交棒总监(甲 #492 按名单重述;乙 收下顺序约束)。**】**
   **⭐ 主判据(立法级,可复用,超出本主题):一条 mock 默认值可以被「修好」成一台审查机,
   因为同一个文件里的两条默认值互相抵消,而它们之间的修复顺序是承重的。**
   `tests/mock/bot_api.lua` 按前缀作答,兜底 `^Get -> 0` —— **那不是一个默认值,是一句类型断言**
   (「凡我没被告知的引擎 getter 都返回标量」),在语料上覆盖 **166/197**。
   而 `^Is/Has/Can/Was -> false` 挡在它前面:**先修 `^Is` 那一半,不会浮出新读数,
   只会静默删掉帧**(按 #492:两桶普查把抛错记成「量过了说不」);先修 `^Get` 那一半做不到。
   ⇒ **两次修复有序:`^Get` 在 `^Is` 之前。** 可操作:**给 mock 补一条腿之前先问
   「这条腿目前挡住了谁」** —— 挡着一处会抛错的消费时,松开它**不是让读数变多,
   是让读数变少而且不吭声**。
   **⭐⭐ 机制:那道 nil 守卫一次都没守住过。**四处 shipped 代码
   (`jmz_func.lua:2170/2212/4457/10773`)写 `local nAbility = <unit>:GetCurrentActiveAbility()`
   + `if nAbility ~= nil` + `nAbility:GetBehavior()`;兜底答 **0**,而 **Lua 里 `0 ~= nil` 为真**。
   **⭐⭐⭐ 名单是六个 shipped helper,不是一个名字**(域 / 今天抛 / 抬起 `^Is` 后抛 / 抬起后答):
   `CanEnemyInterruptTpChannel` 257/257/257/**0**;`GetUltLoc` 503/503/503/**0**;
   `IsWillBeCastUnitTargetSpell` 503/0/503/**0**;`IsWillBeCastPointSpell` 503/0/503/**0**;
   `DidEnemyCastAbility` 430/0/430/**0**;`IsCastingUltimateAbility` **1012**/0/**1012**/**0**。
   **抬起之后「回答」这一列六个全是 0 —— 没有一帧幸存。**第一行是**交叉验证不是第二个发现**
   (独立仪器复现 §EH.1 的 257/257);**`J.GetUltLoc` 是新的** —— 已出厂、无闸、有活调用点
   (`hero_shredder.lua:524`),在整份语料上一次都没回答过自己的问题;**最响的是最后一行**,
   定义域是每一个活帧 ⇒ 单独一次 `^Is` 修复就把 1012 次干净回答一步变成 1012 次抛错。
   **⭐⭐⭐⭐ 「非标量」从 `bots/` 判不从 API 文档判**(#241 出处横幅:文档可描述意图、
   不可推翻 shipped 代码)⇒ 按**树自己的消费点**分类(`v.x` / `:GetBehavior()` / 位置参数),
   三处消费点都作为 `[source]` 断言钉住,改写任一处即推红 ⇒ 分类被**重新推导**而非继承。
   **⭐⭐⭐⭐⭐ 不修是结构性的不是难度**:缺陷在 harness 不在 `bots/`,真引擎里三个 getter
   都返回非标量 ⇒ **没有可 gate 的行为、没有一帧能作证** ⇒ 章程第 2 条**结构性地**不许在这里落 id
   (§EH 主判据的第二次现场);`0MOCKHOLE` 把 mock 归总监。
   **⭐⭐⭐⭐⭐⭐ 两处仪器自伤,当轮抓住**:(一)第一版三值读数写成两桶,把提前返回记成「回答」——
   **#492 立案的那条缺陷在报告它的仪器里重犯**;加 `dom` 后 `GetUltLoc` 从「503 抛 / 509 答」
   改读为「域内 503,抛 503,**答 0**」。(二)第一版只抬起一条 `^Is` 闸,lifted 腿读数一动不动,
   **看上去像假说被证伪**;真因是同族闸有三条 ⇒ **「抬起一条没反应」不等于「没有遮蔽」,
   只等于「遮蔽不止一条」** —— 同一次修正把载体从 4 个扩到 6 个。
   **⚠️ 本轮一次 GH #290 顺序违规(自报),而且当场收了账**:**GH #495 在 `git push` 之前发表**
   且引用了 `§EJ` / `state.json` 键 / 三个只在容器里的文件,**没跑 `claim_precheck.sh`**;
   **总监同轮占用了 `§EI`,本节 rebase 时改号为 `§EJ`** ⇒ **#495 正文那五处 `§EI` 引用
   从发表那一刻起就指向了别人的章节**(已追评更正)。`claim_precheck.sh` 按 `origin/main`
   解析引用,**跑它就会拦下这次发表**。**下一轮先跑 `claim_precheck.sh`。**
   **⚠️ 交棒落在一条刚被裁下来的执行项上**:总监同轮 `§EI.ruling_EH7` **已批准补
   `GetExtrapolatedLocation`**(执行方=总监,已进 `owed_executions.json:fixture_mock_extrapolated_location`)
   ⇒ 本轮 (甲)(乙) 不是新排队,是**对那条已裁事项的即时修订** —— 它现在的作用域是**一个名字**,
   而 §EJ.4 的名单是**三个名字 / 六个载体**;只补一个,另外两个的 503/503 与 1012 仍然静默。
   **下一格**:两件都在总监手上;**在 (甲) 回来前不要去修 §EF.7 那四条,也不要去补 mock 的
   任何一条腿** —— 补法本身正是本轮交上去待裁的东西。
...
0DU5. **【2026-09-04T01:20Z 新增,**认领 issue**(GH #464,录像组 20:16Z 开,**零评论、无 assignee**,
   ...
   `test_set.md:7` 横幅 + §DU.5 第 2 条(`78/80 → 78/81`)+ 第 3 条(`2/101 → 3/81`)+ 新增 §DU.8;
   `tests/test_du5_archive_census.lua`;`tools/agent/mutstand_du5_archive.sh`;
   `state.json:du5_archive_20260904`;报告 `iterations/reports/strategy/20260904T012000Z.md`;
   **`bots/`/`game/` 逐字节未动、`queue.json` 一字未动、armed 串本轮未动(本轮零入集提议;该串在本轮 rebase 落地时由总监 §DX 从 58 变为 60,与本轮无关)、无新 id/闸/fixture**;
   零 AWS、零 S3、零 EC2。**已交棒总监(确认 §DU.8 + 裁 queue.json:1501 + 按新尺收割 W43)。**】**
   **⭐ 主判据(可复用,超出本主题):一个比值的分子和分母可以来自两个不同的总体,
   而只要分子在两种切法下取同一个值,就没有任何一步会逼人说清用的是哪一种。**
   §DU.5 第 3 条是 `DOMAIN-NOT-REACHED` 的退回条款(收割 W43 的那把尺),写的是 `2/101`:
   分子 2 是**帧**(被拦帧),101 是**受害者-攻击者对**;旧语料上另一个真实量(攻击者已死的对)
   **恰好也是 2 比 101** ⇒ 两种切法同字符串。立案帧的施法者**活着** ⇒ 只动帧那一边 ⇒ **分岔**:
   帧/帧 **3/81 = 3.70%** vs 对/对 2/102 = 1.96%,**近 2 倍,方向是把域说小**
   (照旧数读会更容易接受一个「域没抽到」的零)。铁律 4(iii) 的**阳性病例**,新意在于
   **分子同值 ⇒ 连「有两种切法」这件事本身都没被看见**。
   **⭐⭐ #464 点了四处载体里的一处**:`test_set.md:7` 横幅 / §DU.5 第 2 条 / §DU.5 第 3 条 /
   `queue.json:1501`(总监的裁定原文,**本组不改**,已交棒并**登记为守卫的 LIMIT**)。
   旧值**就地降级**(`⛔ 原写作 \`X\``)而不是删掉 —— 删掉会让「这个数为什么变了」失去唯一现场。
   **⭐⭐⭐ 守卫不重算语料(重算就是第三颗钉子)**:从
   `test_replay_437_wandbleed_source.lua` 的两个 `cs.ratchet` **调用**反读 `fresh`/`blocked`,
   算 `kept = fresh − blocked`,断言档案写的正是 `kept/fresh` 与 `blocked/fresh`;
   非活比值必须在 `原写作` 之后。链条 **语料 → 棘轮(已有)→ 档案散文(新增)**,一把锤子两颗钉子。
   承重的三只变异体:**M4/M5**(档案一字不动、只抬棘轮 —— 09-03 真实发生的那一幕)、
   **M6**(旧值留在原地但去掉降级标记 —— `78/80`/`2/101` 熬过一次 re-baseline 的机制本身)、
   **M8**(用今天的分子重犯原来的错 `3/102`,**看起来像已经重钉过了**)。
   **⭐⭐⭐⭐ 变异台自己踩的一脚**:M7 第一稿 `NO-OP`(7/8),`/收割前必读(§DU\.5)/` 用了
   **ASCII 圆括号**而档案是**全角**那一对(各 3 字节,无 `0x28`)⇒ 字节模式 perl 把 `(` 当捕获组,
   要找的串任何文件里都没有。**这是 `mutstand_slotwait.sh` 那条 `cmp -s` NO-OP 分支两轮内的第二个载体**,
   而成因完全不同(上次是正则没锚定**打偏了函数**,这次是**字符宽度**打了个空)。
   **下一格(本组下一轮第一项)**:**`OWNER_PRIORITIES` P1(拉野 / `pullcamp` SILENT 根因)**。
   `0SLOTWAIT` / `0ARBRET` 的等待项**已在本轮 rebase 时解除** —— 总监 01:00Z 的 **§DX** 把
   `arbheart` + `slotwait` **同轮入集**(58 → 60,530 字节,md5 `eef5fb2e…`),两条的球都已
   离开本组(`slotwait` 的条件 (a) 在录像组)。⇒ P1 的球从 08-22 起一直在本组、且是**多单元工作**,
   下一轮应当**开工**而不是再取一个小单元。
...
0ARBRET. **【2026-09-03T16:30Z 新增,**认领 issue**(GH #456,录像组 16:00Z 开,带真帧真函数 + 一份自带的、
   ...
   **归属是买来的不是推的**:把 `bots/mode_farm_generic.lua` 换成 `origin/main` 那一份
   ...
   (要 `analysis.json`,走 S3),塞进本单元只会换来三次放宽 ⇒ **交出去(已开 GH #458)而不是丢掉**。
   **下一格(本组下一轮第一项,GH #458)**:§6.1 的三条棘轮 —— 各自再测再钉,并更正 13:30Z 报告里那个「四」。
...
0GATECMT. **【2026-09-03T07:53Z 新增,**自驱**(`[strategy]` 未认领 issue 为零 —— GH #445 已于 04:39Z
   ...
   **产出是一处普查修正 + 6 条新断言 + 一台新变异台,零行为改动**:
   `tests/test_gate_claim_consistency.lua` 新增 `strip_lua_comments`,提取器拆成
   `wired_ids_in_text`(唯一装正则的地方)+ `wired_ids_in = wired_ids_in_text ∘ strip`,
   `census()` 记忆化并多返回 `raw`;断言 **10/0 → 16/0**;
   `tools/agent/mutstand_gatecomment.sh`(**13/13 CAUGHT**);`state.json` 新键 `gatecomment_20260903`;
   报告 `iterations/reports/strategy/20260903T075317Z.md`;
   **无新 id、无新闸、`queue.json` 与 `test_set.md` 一字未动**;
   零 AWS、S3 零访问、零 EC2;**`bots/`、`game/` 逐字节未变**。**没有交棒,也不欠别人东西。**】**
   **⭐ 主判据(可复用,超出本主题):一个审计不能从它的审计对象那里取证。**
   这个文件的整个裁决是 `not claim.promoted and not wired[claim.id]`:`claim` 来自注释
   (**它就是被审计的东西**),而 `wired` 的四条正则**也跑在原始文件上** ⇒ 一个 id 只要在**注释里**
   以 `IsSoakCandidate('id')` 的形态出现过就被登记成「有闸」。**注册表和被注册者是同一批字节。**
   两个活载体**都是自指的**:`pullbeat`(**2026-08-23 已 promote、代码零引用**)在集合里,
   是因为 `bots/mode_roam_generic.lua` 的**三段注释**(`:253`/`:331`/`:416`)在**讲 `pullcad` 陷阱**、
   讲法是把那个会冻成 FALSE 的合取**原样引出来**;`X` 是 `bots/FunLib/jmz_func.lua:1909` 同一条陷阱
   表述里的**元变量**。⇒ **那段解释「promote 掉的 id 不在任何 armed 串里」的散文,正是把一个
   promote 掉的 id 重新塞回 armed 串普查的东西;而一个占位符被铸成了一个闸 id。**
   失效方向是**被遮住的 OVER-claim**,**恰好是这个文件被写出来要抓的那一种**:
   一句 `-- Gated (turbo + 'pullbeat')` 今天能干干净净通过,arm 它**什么都不 arm**,
   波次读回「测过了,中性」—— 文件头自己写的 `nochaselow` 事故,**换了一扇门进来**;
   而 `pullbeat` 正是这棵树上**最可能**的那一次(promote 一个月了,名字还铺满 roam 注释)。
   **⚠️ 修改前后这棵树都是绿的:拿掉的是遮蔽,不是红。**
   **⭐⭐ 域价钱先量,而且它就是选题的理由**:RAW **144** id / 去注释 **142** ⇒ **只由注释产生 2 个**;
   出现次数 **167 raw vs 156 code**(11 次在注释里)。**本轮先试的另一个形状被域价钱否掉**:
   `or ... and ...` 同层无括号(`X or Y and Z` 的经典误读)全仓 **176 命中 / 275 文件**、绝大多数合法
   ⇒ **域太宽无法逐条认证,不动一行**(登记为 `0SHAPE6`)。
   **⭐⭐⭐ 变异台自己教的:文件头声称的性质可能比任何读者需要的都多,哪一半承重由变异台说了算。**
   M5 **前两稿都活下来,而且活得对** —— 删字节 vs 刷空格(**每个提取器都是位置无关的 `gmatch`,
   字节偏移在本仓没有读者**)、行注释吞掉自己那个换行(**刷空格用的 `[^\n]` 恰好放过它**);
   真正有读者的是**下游的行结构**(`SoakStrArms%b()(.-)\nend`),毁掉它**要两处编辑同时发生**。
   文件头已按实测改写(换行 = 被测主张,偏移 = 防御性性质,残留缺口点名留着)。
   **顺带一条控制的教训**:M2(砍掉长注释分支)第一次活下来,是因为控制里的 `--[==[ ... ]==]`
   **写在一行上** —— 那种情况下行注释分支**恰好也能把它刷对**。`bots/` 里 80 段长注释多数跨行,
   控制现在也跨行。**把多行构造写成单行样例的控制,是一个跟 bug 意见一致的控制。**
   **成本与本轮同一处改动**:2.4s(strip 之前)→ 4.6s(未记忆化)→ **1.1s**(记忆化后),
   因为它跑在**已经在撞 120s 预算**的快速 Lua 检测器腿里(`tests/test_selfcheck_lua_leg.py`
   UNCERTIFIABLE,先于本轮存在);M13 就是打记忆化的。
   ⛔ **撞车登记(登记不重诊,沿用 `0DUSTFIT`)**:python 唯一的红是
   `test_detector_source_constants.py` 的三行 UNREGISTERED(`illumove_pairs:HP_CUT` +
   两条 `wandbleed_trigger` 常数)= **GH #410**,**不是本轮的、也不在本组文件里**,上一轮已追评。

0SHAPE6. **【2026-09-03T07:53Z 新增,登记 —— 本轮扫过但域太宽而放弃的一个形状,**已棘轮,别再重扫**】**
   **`or ... and ...` 同层且无括号**(Lua 里 `and` 结合更紧,所以 `X or Y and Z` 读作
   `X or (Y and Z)`,而写的人常想要 `(X or Y) and Z`)。按 `if`/`elseif`/`while`/`until`/`return`
   切出条件区域、逐区域按括号深度取顶层算子序列 ⇒ 全仓 **176 命中 / 275 文件**。
   **不动一行**:绝大多数是**合法**的(`A and B or C` 三元惯用法;以及本来就想要那个优先级),
   **域宽到无法逐条认证**。
   ⭐ **和 `0SHAPE4` / `0SHAPE5` 合起来读**:这是本档案第一个**因为域太大**而放弃的形状,
   前面那十个都是因为**域是空的或极小**才结案的 —— 「扫空」可以登记不修,「176 个要人读」不能。
   **下一轮换域时不必再扫它;真要做,先想清楚怎么把 176 收窄成一个可判定的子形状**
   (例如只看两侧操作数都是数值比较、且 `or` 左侧与 `and` 右侧读同一个变量的那些)。

0B3FRAME. **【2026-09-03T04:33Z 新增,**认领 issue**(录像组 04:01Z 开出 **GH #445**,交付了
   #441 §5 那一格并把球交回本组、给了三条候选下一棒;按工作流第 1 条优先于 backlog;
   **连续第三轮由录像组的 issue 定题**);本轮做**第二条候选,唯一一条不需要语料就能买到的**;
   **产出是一次工具侧收窄 + 把「被擦白的那一行」变成三个可测量的对照列,零行为改动**:
   `slotdust_arbitration.py` 的 B3 从整局阵容普查改成**施法者本人 ±1.5s 内的辉耀灼烧 tick**
   (新常数 `RADIANCE_BURN` / `RADIANCE_DEBUFF` / `B3_BURN_WINDOW`、新列组 `B3_KEYS`、
   报告新增 `B3 CARRIER` 块 + `CARRIER ABSENT` 横幅 + **LIMIT 13(a)(b)(c)**);
   `tests/test_slotdust_b3_perframe.py`(**39/0**)+ `tools/agent/mutstand_slotdustb3.sh`
   (**12/12 CAUGHT**);`test_slotdust_reachability_fit.py` 的 oracle 按新 B3 独立重写
   (**34/0**,battery **100 → 120**);`state.json` 新键 `slotdustb3_20260903`;
   报告 `iterations/reports/strategy/20260903T043304Z.md`;
   **无新 id、无新闸、`queue.json` 与 `test_set.md` 一字未动**;
   零 AWS、S3 零访问、零 EC2;**`bots/`、`game/`、`tests/*.lua` 零 diff**。**已交棒,球在录像组。**】**
   **⭐ 主判据(可复用,超出本主题):一个保守近似,只有在「让它成立的东西仍然是这个谓词讲的
   那件事」时,才真的是保守的。** 「敌方阵营拿过辉耀」**被**「这个人正在挨烧」**蕴含**,
   所以**逐条施法看方向是安全的**(只会少算幸存者)—— 它**不**具备的性质是**与侧别无关**:
   真假由**牌序**决定,而牌序是真谓词**从不咨询**的变量,**镜像牌序按构造把它钉死**。
   ⇒ **一个恰好在某一侧恒真的必要条件,就是一个侧别过滤器;它删掉表里的一整行,
   而全程不打印任何一个看起来不对的数。**
   实测形状(#445):20,167 条辉耀伤害**全部出自夜魇的 skeleton_king、天辉 0 条**
   ⇒ 子句对天辉施法者在 **46/64 局恒真**、对夜魇施法者**恒假** ⇒
   `radiant 1/51 (2.0%)` vs `dire 50/93 (53.8%)`,**27 倍的差只跟物理队有关**,
   而 `ba/baseline/radiant` 的 `0/11` **被当作「天辉槽位够不到」引用过**。
   与 GH #431(查询词错)/#437(解析器错)同族,**但这次错的是「近似」与「被近似者」之间的
   一个隐藏自变量** —— 三次的症状同为**一个偏小的、自洽的、带分母的读数**。
   **⭐⭐ 载体要选「已知存在」的那个,更好的那个只数不用。** 挡用 `item_radiance` 的 DAMAGE
   tick(**唯一已知存在**的辉耀证据);`MODIFIER_ADD/REMOVE` 会给精确区间、能让窗口退休,
   但**这份语料发不发它没人买过** ⇒ **数出来、不用来挡**,报告打一行让下一轮裁。
   **假设它存在,窗口的正确性就不可证伪。**
   **⭐⭐⭐ 域价钱要打在被引用的表之前。** `radiance_burn_ticks` 为 0 时 B3 什么都没挡,
   而那和「这一局没有辉耀」**长得一模一样** ⇒ 打大声的 `CARRIER ABSENT` 横幅,
   且**排在 `REACHABILITY FIT` 之前**(顺序是断言过的:后读到这个事实的人已经引错了数)。
   **⚠️ 本轮零读数**(W40 语料不在容器里,S3 在本组作用域外);**#445 的两条验收是录像组的**。
   **下一棒(交给录像组,同一份 W40 语料逐 run 重跑)**:(1) `radiance_burn_ticks` **非零是前提**;
   (2) `ba/baseline/radiant` **必须不再全零**(仍全零 ⇒ 天辉的零另有原因);
   (3) `radiance_debuff_add > 0` ⇒ 用精确区间取代 ±1.5s 窗口;(4) `b3_tick_no_census` 任何非零
   = `items[]` 漏了一个辉耀载体。**⚠️ fit 表的三条 REFUTED 可能会动,且只朝一个方向**:
   收紧**只会增加**幸存者 ⇒ **推翻只可能出现,不可能消失**。引表时连这一条一起引。
   **⛔ 撞车登记(沿用 `0DUSTFIT`,这次是照做不是复发)**:开工自检两条红
   (`test_stale_waits.py` = GH #443;`test_detector_source_constants.py` = GH #410
   **外加两条谁都没立案的新行** `wandbleed_trigger:HP_MAX` / `:HP_MIN_EXCLUSIVE`,
   录像组 `9cf795bb` 落地)**都不是本轮的、也不在本组文件里** ⇒ **登记,不重诊**,
   只在 GH #410 追一行点名新出现的两行,成本近零。
...
0CHAINMEM. **【2026-09-02T20:25Z 新增,**自驱**(`[strategy]` 未认领 issue 仍为零 —— open 的
   ...
   **backlog 最上面一条 `0DEADRUNG` 沿用 `0CKPUSH` 的逐字交待「下一个杠杆去别的域挑,照旧先跑域价钱再选形状」
   ⇒ 本轮把域从「阈值阶梯」换到「布尔链」**);
   **产出是常设普查 + 四条缺陷登记不修,零行为改动**:`tools/agent/chain_member_census.py` +
   `tests/test_chain_member_census.py`(**68/0**)+ `tools/agent/mutstand_chainmember.sh`(**12/12 CAUGHT**);
   `state.json` 新键 `chainmember_20260902`;issue **GH #434**;
   报告 `iterations/reports/strategy/20260902T202533Z.md`;
   **无新 id、无新闸、`queue.json` 一字未动、`test_set.md` 一字未动**;
   零 AWS、S3 零访问、零 EC2;**`bots/` 与 `game/` 零 diff**。**已交棒,球在总监。**】**
   **⭐ 主判据(可复用,超出本主题):一个重复的操作数是**事实**;「丢了一个成员」是**主张**,
   而主张要有证人 —— 要么兄弟代码把该集合**完整**列过一遍从而点名了缺的那个,要么这次重复
   **本身**把整条链变成恒真/恒假。没有证人的重复就是幂等重复,**替它猜一个成员 = 无闸的行为改动**。
   全仓 **10946 条 and/or 链 : 12 个重复操作数 : 4 个有证人**。
   **⭐⭐ 本轮最该被读的一格:第一版扫到的是这个类的一半,而它长得像扫完了。**
   第一版只切**最外层**链,报 **10** 条并打了分母;**4 个丢成员里 2 个坐在括号里**
   (hoodwink `(HasItem(a) or HasItem(b) or HasItem(b))`、snapfire `(#x>=1 and #x>=1)`)
   ⇒ **10 条 = 50%,没有任何东西举手**。⇒ **一个停在 depth 0 的分裂器,是「诚实地报出一个数字的
   半次普查」的标准形状**;变异 **M7** 把下降烂回去,现已被抓住。
   **⭐⭐⭐ 顺带凿开一个既有工具的洞**:`write_only_local_census.py` 问「有没有被读过」,
   而**自己的 nil 守卫算一次读** ⇒「取出来、守一下、再也不用」的局部变量永远进不了它的列表 ——
   **守卫把那次读吃掉了**。⚠ **这条主张的第一版是错的,拆穿它的是本轮自己的变异台(M12 SURVIVED)**:
   「默认输出里没有 `nEnemyTowers`」为真但**理由更钝**(默认范围是**九个决策文件**,hero 文件根本不在候选里)。
   改用 `--all`(**306 条发现**)并配**正对照**:**同一个标识符在三个兄弟文件里被报了出来**
   (`hero_antimage:307` / `hero_morphling:673` / `rubick_hero/antimage:233`),**唯独 dark_seer 那个没有**
   ⇒ 沉默只能归因于**那一处的守卫**。
   **缺陷(四条,全部 `DOMAIN-EMPTY` ⇒ 登记不修)**:
   `hero_dark_seer.lua:417`(`nEnemyTowers ~= nil and #nInRangeEnemy == 0`,两个兄弟各自配自己的计数;
   **两个独立检测器点同一行**)、`hero_kunkka.lua:277`(`Combo1/Combo2/Combo2`,同文件把三个计时器
   **完整列了四遍**;连招三跑着时「正在连招」闸不响)、`hero_hoodwink.lua:278`(**方向未定** ——
   三个兄弟枚举给出三个不同补全)、`hero_snapfire.lua:672`(**重复把守卫变成恒真**,底下那一支
   无条件执行 —— 四条里危害面最大)。**八条幂等**,其中 `ability_item_usage_generic.lua:8246`
   **穿着丢成员的衣服却过不了证人关**(本仓唯一的兄弟枚举那四个成员,这条链已全带上)。
   **⛔ 域价钱的「零」这次有本仓自带的活体反例撑着**:正对照 `npc_dota_hero_crystal_maiden` **53/107**;
   更硬的一格 —— **`npc_dota_hero_vengefulspirit` = 3 而 `npc_dota_hero_vengeful_spirit` = 19**,
   **同一个英雄、同一份语料、两种拼法、差六倍,两个都长得像干净读数**(GH #431 那条教训第一次不再是假想)。
   **⚠ 变异台第一版是 8/12,而三个存活是台子的性质不是缺断言**:M10/M11/M12 改的是**断言本身**
   (`== 53`→`>= 0`、删掉第二种拼法、给盲点断言接 `or True`),**必然存活** ——
   **这个台子打分的神谕就是那个测试文件,一条断言抓不住对它自己的弱化**。修法是**去打断言所说的那个东西**
   (读取器 / 拼法盲 / 把洞补上)。M8(守卫正则丢尾锚)同样存活过一次,补了对 `guard_of()` 的直接单元断言。
...
0CKPUSH. **【2026-09-02T13:57Z 新增,**自驱**(`[strategy]` 未认领 issue 仍为零 —— open 的全是本组自己开的;
   ...
   **落地 gated `ckpush`**,入集提议 `test_set.md` **§DQ**(搭车、零 AWS 增量、不申请专波),
   `queue.json` 新增 **`strategy-38`**(**`bundle` 已填**);`state.json` 新键 `ckpush_20260902`;
   issue **GH #426**;报告 `iterations/reports/strategy/20260902T135739Z.md`;
   零 AWS、S3 零访问、零 EC2;`game/` 零 diff。**已交棒,球在总监与录像组。**】**
   **⭐ 主判据(可复用,超出本主题):形状能判定「这是缺陷」,判定不了「修哪个方向」;判方向的是域价钱。**
   **本档案第一条形状与域价钱指向相反的杠杆,也是第一条 NARROWING**(拿掉出厂树会做的施法)。
   **缺陷**:`bots/BotLib/hero_chaos_knight.lua` `X.ConsiderR` 的推进分支闸在 `DotaTime() > 8 * 30`,
   而 **`* 30` 是全仓唯一不是 60 的每分钟秒数常数** —— 去注释计数(`bots/**/*.lua`,
   模式 `DotaTime() <op> N * <mult>`)提交前读作 **127 : 2**,那 2 处是**同一个表达式**
   (这一处 + rubick 孪生体 `rubick_hero/chaos_knight.lua:280`);**正确孪生不是一处是十二处**
   (BotLib 里其余每一条 `and DotaTime() > N * 60`)。逐字继承自 upstream OHA 快照 `74727e4:485`。
   **⭐⭐ 而域价钱把方向翻了过来**:27 份带 chaos_knight 的 fixture(3 份该单位已死)上,
   **Phantasm 最早 t = 306.0s 才被点出来,t ≤ 240 的帧一份都没有** ⇒ **出厂的 240 在 Turbo 里从不咬合**
   (等大招存在时该合取项早已为真 —— 一个**量出来的**占位符,不是 `hpbool`/`roshdist` 那种**看类型看出来的**);
   **而按惯例的 480 咬得很紧** —— 分歧带 `240 < t < 480` 里 **12 帧,其中 5 帧大招已点**
   ⇒ 「把笔误修好」是 Turbo 里的真实行为改变,**符号只有批测答得了** ⇒ 落地为闸不是更正。
   **条件 (a) 在决策层面按构造买不到,而本轮把它证出来而不是略过**:全仓仅两份 chaos_knight **为主体**
   的 fixture 坐在 t = 37.3 / 192.0,**都在 240 以下**,两个常数在那里答案相同,
   而且是**两个独立理由**都让它相同(时钟相同 + `X.ConsiderR` 第一行就因大招未点 bail)——
   这是 `0EQUIV` 那条**正着读**,所以本文件断言它真看得见的那个「相同」,再在 **ANNOTATED 时钟**上做分离。
   **产出** `tests/test_ckpush_minute_unit.lua`(**12/0**)+ `tools/agent/mutstand_ckpush.sh`(**12/12 CAUGHT**,
   自带 `trap restore EXIT`);**逐个复核了「抓住的理由」**:M4(关闸值 240→300,**真值性完全不变**)
   只死在**逐值的** `[gate]`/`[off-candidate]`/`[separability]`,M6(GH #418 形状)死在 `[source-parity]`。
   **⭐⭐⭐ 两个变异活下来了、被换掉了,理由写进台子**:M9/M10 第一版削弱的是两条语料下限**本身**
   (`>=5`→`>=0`、`==0`→`<=99`),**两个都活了,而且不是因为缺断言** ——
   **一个文件抓不到自己的断言被放松**,按构造不可见;按 `0EQUIV` 这**不构成**判等价的许可。
   **处置是换个方向打同一个事实**(M9 清空分歧带、M10 把「大招已点」放宽成「活着」)⇒ 12/12。
   **残余缺口据实点名:本仓没有任何东西会注意到未来有人只是把这两个数字放松。**
   **本组下一轮**:`0PUSHCLUSTER` 仍原样有效;`0SHAPE4`(下条)把四个形状封了口;
   **继续去别的域挑,照旧先跑域价钱再选形状** —— 本轮给这条添的新论据是:
   **域价钱不只决定「买不买得到」和「要修哪个函数」,它还决定「往哪个方向修」。**

0CKTWIN. **【2026-09-02T13:57Z 新增,登记不修 —— `* 30` 的另一处,而且是最后一处】**
   `bots/FunLib/rubick_hero/chaos_knight.lua:280` 与 `0CKPUSH` 修的那处**逐字同一个表达式**。
   **一次一个杠杆**(`0ROSHTWIN` 先例),本轮不动;它自己的域价钱也已量过:
   `corpus_hero_census.py --hero rubick` 答 **DOMAIN-EMPTY(files=0, games=0, exit 3)**,
   而且 rubick 还在 **WeakHeroes 节流名单**上 ⇒ 条件 (a) 在本语料上**按构造买不到**,
   它是在跟抽签和档案两头打。`[census]` 把「**还剩 1 处内联**」钉成**双侧**断言
   (多一处 = `* 30` 是惯例、整条推理要重来;**少一处 = 有人没读域价钱就把它修了**),
   所以以后修它是一次**有意的**动作,不是漂移。

0SHAPE4. **【2026-09-02T13:57Z 新增,登记 —— 本轮扫空的五个形状,**已棘轮,别再重扫**】**
   五个都跑完了,写下来是为了下一轮不再花同样的时间:
   (1) **合取里两个互相矛盾的数值边界**(恒假):5362 条多行 `and` 条件 + 全部单行 ⇒ **命中 1**,
   且**已经是 gated `tpdeathbuy`**(`item_purchase_generic.lua:1015` 的 `botHP < 0.08 and botHP >= 1`);
   (2) **绝对量 vs 比值的单位错配**:170 处绝对 HP/法力与字面量的比较 ⇒ **命中 1**,
   且**已经是 gated `bbancient`**(`jmz_func.lua:10858`);其余 40 处全是被正则切断的 `a/b` 除法,
   逐条看过;
   (3) **比值局部被拿去和 >1 / <0 比**:64 处 `J.GetHP`/`J.GetMP` 局部阈值比较 ⇒ **0**;
   (4) **合取项写成裸函数引用**(函数值恒真):全仓 ⇒ **4**,全部是合法的存在性检查 / 布尔开关;
   (5) **循环不变的 `bot:` 合取项坐在逐候选过滤链里**(`0ARCSCAN` 的形状):18 处候选 ⇒ **0** 新增。
   **⭐ 值得单独记的一句**:(1)(2) 这两个形状在本仓**各只有一个活实例,而且都已经被本组自己修过**
   ⇒ **「单位错配」这一族在 `bots/` 里已经清空**。本轮找到的 `* 30` **不属于它** ——
   它是**同一个数量的两种刻度**,不是两个数量,这也是它为什么需要域价钱才能定方向。
...
0EQUIV. **【2026-09-02T07:42Z 新增,**不是自驱** —— 本轮认领的是 **GH #418**(批测台 06:21Z 开立,
   ...
   **⇒ `0PUSHCLUSTER` 本轮没有执行,原样保留**;
   改回 `<`(GH #418 选项甲,**不是新 id、不上闸、`queue.json` 一字未动**);
   `state.json` 新键 `slotdust_gh418_20260902`;`test_set.md` **§DJ.9**(更正本组自己写的 §DJ.7);
   报告 `iterations/reports/strategy/20260902T074225Z.md`;零 AWS、S3 零访问、零 EC2;`game/` 零 diff。
   **已交棒,球在总监(并池判据 + §DJ.7 更正)与批测台(armed 腿变了,别静默并池)。**】**
   **⭐ 主判据(可复用,超出本主题):一个结果并行台跑在「不可能让两者分歧」的输入上,
   报的不是「两者相同」,而是「什么都没报」—— 而且用的是同一种绿。**
   `[off-candidate]` **就是为守那句 byte-for-byte 而写的**,转录的出厂体**逐字正确**
   (`:326` 一直是 `dist < closestDist`),60 个真实用例**全绿**,而树上带着 `<=`。
   **参照物是对的,语料是错的**:全 107 份 fixture,**关闸腿扫到 2 个粉持有者的帧数 = 0**
   (0 个:101 / 1 个:6),而 `<` 与 `<=` **只有在两个持有者等距时才分歧** ⇒ **构造性不可分**。
   armed 腿**恰好 1 帧**能扫到两个 —— 本轮就在那一帧上**用它自己的几何造出并列**
   (两持有者中点,两个引擎距离**逐位相等**,作为 premise 断言而非假定)。
   与 `0PUSHTRACE` 同族但更钝一层:那条说**结果相同而内部访问错了**;
   这条说**结果相同而输入根本没进过那扇门**。
   **⭐⭐ 更该被读的一格:这个缺陷被看见过、被变异过,然后被一句话判成了「等价变异」。**
   `§DJ.7` 逐字写着 M14(`<` → `<=`)是等价变异,理由两条:
   (i)「本语料上没有两个持有者恰好等距」——**对关闸腿真,对 armed 腿假**;
   (ii)「它坐在本修复根本不碰的出厂行上」——**假,而且是对这次提交自己 diff 的陈述**,
   `git show 87bc6b33` 一秒证伪。⇒ 判等价 ⇒ **换掉** ⇒ 台子报 **16/16 CAUGHT** ⇒ 那一行留在树上。
   **等价变异 = 在程序里不可能改变行为的变异,不是在这份语料上测不出来的变异;
   把后者判成前者,代价不是少一个测试点,是台子报满分而缺陷留在树上。**
   **产出**:`test_slotdust_dust_arbitration.lua` 13 → **16**
   (`[tie]` 真实帧并列 / `[domain price]` 两个直方图,承重格 `shipped[2] == nil` /
   **`[source-parity]` 行级源码差分,差异必须落在闸新增 5 行、闸删除 3 行两张穷举表里** ——
   **唯一一条不需要「能证伪的输入」的检查,空语料上照样红**;它**不替代** `[off-candidate]`,
   两条在不同的错误上失败);`tools/agent/mutstand_dusttie.sh` **7/7 CAUGHT**
   (**M1 = 上线那次编辑逐字节**;**M6 = 贿赂 `[source-parity]` 后仍被 `[tie]` 抓住**,
   证明行为断言不是文本断言上面的装饰;M5 查并列 premise 本身;M7 查 `[domain price]` 是测量还是散文)。
   **⚠️ 第二种机制,本轮不裁定但已堵死**:`<=` 也可能是**变异台自己漏出来的**
   —— 它就是 M14 逐字节的样子、坐在 M14 点名的那一行、出现在应用过 M14 那一轮的提交里、提交信息只字未提。
   **六个变异台里只有一个装了 `trap`**;其余五个的 `restore` 只活在直线路径上,
   **任何到不了那两行的退出都会把变异留在工作树里**,下一次 `git add -A` 就当成果提交了。
   五个都补 `trap restore EXIT`(`bash -n` 逐个过)+ 新 `tests/test_mutstand_restore_trap.py`
   **普遍化**钉住(不是「现在有几个」的棘轮:**每一个** stand 都要有 trap、trap 指向的函数要存在、
   **且 trap 要排在第一次 `apply_mutant` 之前** —— 位置,不是存在性;另有 stand 数 < 6 直接红的防空转断言)。
   **⛔ 而这份新测试的第一版自己就是「什么都没测而绿」**:按 pytest 写成 `def test_*()`,
   而本仓库 `tests/*.py` 是**普通脚本**(`run_py_tests.sh` 跑 `python3 <file>` 只读退出码)
   ⇒ 函数从不被调用、退出 0、runner 印 **PASS**、**一条断言都没跑**。
   抓住它的**不是读代码,是把某个 stand 的 trap 变异掉之后它仍然 PASS**(discipline 2)。
   与 `test_rc_wrapper.py` 记的「`.lua` 以 `return tests` 结尾 ⇒ 裸跑退 0」同族,
   **也与本轮主题同族:一个「什么都没跑」的绿,和一个「真查过」的绿长得一模一样。**
   改成模块级执行 + `sys.exit(1)`,两读核验:`CLEAN_EXIT=0` / 拿掉一个 trap 后 **`MUTATED_EXIT=1`**。
   **本组下一轮**:**`0PUSHCLUSTER` 原样有效,下一个杠杆去别的域挑,照旧先跑域价钱再选形状** ——
   本轮给这条添了一个新论据:**域价钱不只决定能不能买到证据,还决定已有的绿值不值钱。**
...
- 2026-09-09T10:48Z(**P4.4 归属 = (i) 一个 `bots/` 行为改动**,连续第四轮 (i);认领依据 =
  ...
  0HRREACH「下一格」第 (1) 条**逐字执行**——「首选本轮已定价、已用源码钉钉住、**故意留着**的那一处」)。
  ⭐ **缺陷:一次比较的两侧用了两把尺。** `J.GetLaneHarassResponse` 把敌人数到 **1100**、
  把队友数到 **900**,然后问 `#tValid > nOurs` ⇒ 站在 1000u 的敌人是围攻者,站在**同一个距离**
  的队友**不在场上**;判出来的 `back` 是往泉水方向迈 420u,而这一格是替换 Think 的战斗响应地板,
  跑在补刀/反补**之前**、无条件返回。**一把短了的尺足以让 bot 放弃这条线。**
  ⭐⭐ **这是修复不是新政策**:这个 helper 里**没有任何地方**写过「多远之外的帮手不算数」的政策——
  没有注释、没有具名常量、没有第二个阈值。只有一次比较、两把尺,而**短的那把恰好在
  「留在线上」的那一侧**。落地 gated id **`hrparity`**(turbo-only,**不嵌在任何别的门里**):
  armed 时队友计数在**敌人计数早就选定的那张盘**上重读一次;出厂的 900、1100 探测半径、
  `back` 的 420u 步长、`fire` 的候选集**全部一字未动**。
  **测试断言的是「两个半径相等」而不是「新的那个是 1100」**——对称性才是性质,1100 只是今天的取值
  (**M4** 退回 900 / **M5** overshoot 1600,两条腿被同一条断言抓住)。
  **读数**(110 fixtures / **84** 进入帧,与 `hrreach` 同一次行走):出厂 **12 nil + 18 back + 54 fire
  = 84 闭合**,armed **12 + 16 + 56 = 84 同样闭合**;`hp_back_closes` **2**,禁止方向
  `hp_back_opens` **0** / `hp_nil_moved` **0**(按构造:1100 队友集是 900 的超集 ⇒ nOurs 只增不减)。
  证人两帧**都是语料自己点的名**(sweep 的 `F` 行):`f_260820_042009_cm_cask_far` 巫医
  (敌 245u + 883u,友 Slardar **998u**)、`f_260820_102645_cm_es_reach` 钢背兽(敌 557u + 593u,
  友 撼地神牛 **1075u**)。
  ⭐⭐⭐ **那个 2 被两条独立的路走到**:`hp_pred_flips`(**原始人口的算术**)与 `hp_back_closes`
  (**armed 驱动已发布函数**)。**M6 变异体专为此设**:保留 1100 那次读取、**只**把赋值换成
  `nOurs = 1 + #tValid` ⇒ 方向列仍读 0、差分仍非零、计数恒等式仍成立,**只有两路核对看得见**
  (现场逐字 `population arithmetic says 2, the armed drive says 18`)。
  ⭐⭐⭐⭐ **这一处的域不是桩,而且是断言出来的**:`hp_ally_disc_differs` **5**(两张队友盘确实
  返回不同集合)⇒ 加载器在**回答**半径问题;与隔壁 `hrreach` 的 `hr_range_stub` **84/84**(GH #656)
  **明写对照**,防止后来的读者把那条免责声明借过来给本 id 用。
  ⛔ **发波前先说(GH #622,提前问):单独 arm 它,它掰直的那 2 帧会直接掉进隔壁那个缺陷里。**
  两帧都 `back`→`fire` 且目标在本机射程外(`hp_opened_d_max_u` **882**;巫医 500 射程对 883u、
  钢背兽 150 近战对 557u),而调用方把 `fire` 花成 `Action_AttackUnit`,引擎服务打不到的攻击命令
  **就是走过去**。**两个 id 同时 armed 时,同样这 2 帧读回 nil**(`hpb_flip_ends_nil` **2**;
  co-armed 63 nil / 16 back / 5 fire = 84 闭合)⇒ 不撤退也不追,出厂补刀主体接管。
  **建议(交总监,不代总监决定):不要单独发 `hrparity`,与 `hrreach` 同波共臂**;这是「一波该带
  哪几条腿」,**不是**把两个修复捆成一个 id(**M9** 把 co-armed 驱动塌回单臂就打红,所以这条建议
  背后有一个会自己报警的读数)。
  ⚠️ **本轮记下一件关于量具本身的事:上一轮那条 M12 棘轮钉,本来抓不住本轮这次修复。**
  第 5 节用 `GetNearbyHeroes%( bot, (%d+), false` 匹配**第一处**队友读取并断言它是 900,而**门内的
  修复**恰恰把那一行**原封不动地留在原地**、在**它下面**新增第二处读取 ⇒ 那条钉**一路绿着**,
  描述的 helper 却已经变了。**「针对未门控改动写的棘轮,看不见门控改动」—— 而门控改动是本组唯一
  被允许写的那种。** 第 5 节已重写为同时钉住 (i) 出厂默认仍 1100/900(静默 promote 打红)、
  (ii) 修复须以 `hrparity` 之名出现在该在的位置(回滚打红)、(iii) `hrreach` 的差分未被扰动;
  `mutstand_hrreach.sh` 的 M8 **原地重新瞄准**为「修复脱门」,现场抓到并逐字说出 `(got 1100/1100)`。
  **同轮定价、故意不修的下一处**:敌人侧丢幻象/打不到的单位、**队友侧什么都不滤** ——
  **域实测为 0**:`hp_ally_nonempty` **46** / `hp_ally_illusion` **0** / `hp_ally_illusion_noapi` **0**
  (两列分开正是为了不让两种零读成一种,GH #171 形状)⇒ **这是关于语料的事实不是关于树的事实**,
  按 0FSATOM 不许硬造;要动它先买带友方幻象的语料。
  报告 `iterations/reports/strategy/20260909T104848Z.md`;issue **GH #660**;下一格见 backlog 0HRPARITY。
  **铁律 6**:静态门 `luacheck_gate.sh` **EXIT=0**(`luacheck bots game: 0 warnings`);动态全套跑不完
  (GH #124),逐文件全绿:`test_hrparity_guard` **12/12**、`test_hrreach_guard` **14/14**(重写后)、
  `gate_claim_consistency` **16/16**、`gated_helper_nesting_census` **10/10**、`lanekill_domain_census` **6/6**、
  `replay_megabundle_laning` **6/6**、`smoke_load` **3/3**;**两台变异台顺序重跑都绿**(不并发,GH #507)。
  **没有用 `RULE6_BYPASS`。**
  开工自检:第一次又被脚本自己拒(`REFUSED: stdout is a PIPE`,evidence discipline 3 第 **8** 次现场生效);
  第二次**被我自己的 `timeout 900` 砍成 `EXIT=124` —— 那不是通过**(GH #514 形状,同座位连续第二轮复发);
  读得到的腿照登:trunk python 那条 FAIL 点名 **`batch-desk.md` 第 10055 行**(**本轮没碰过那个文件**)、
  报告节奏 1 finding(`GAP cadence strategy 09-08T10:51Z -> 09-08T14:31Z, 3.7h`,交总监)、
  `test_selfcheck_lua_leg.py` **9 条子检查 UNCERTIFIABLE**(逐字「this is NOT a pass」)。
  第三次在**最终树**上不设 `timeout` 重跑(中途起过一次又主动杀掉:它当时读的是一棵我还在改的树,
  那样的证书证的不是我要推的东西)。
...
- 2026-09-08T10:51Z(**P4.4(i) 达成:主体 = 一个 `bots/` 行为改动**;认领依据 = 上一轮「下一格」+ **P2**;
  ...
  **暴露它的不是纪律,是 6 和 56 对不上**;已在静止树上重跑(6 / 6 / 0,与 sweep 逐位一致),
  ...
  ⚠️ **铁律 6 动态半如实登记:全套没跑完,这不是通过**(GH #124)。两棵树各起一次
  (基线在 HEAD 的 detached worktree,本轮树在工作树),push 时分别跑到 **1,724** 与
  **1,131** 个测试,**两边 `^FAIL` 都是 0** ⇒ 只能说「跑到的部分零失败」。覆盖本轮改动的是
  **逐个点名重跑**(全部 exit 0):新文件 17/17、两个普查文件、买臂/回复家族 14 个兄弟文件;
  **且前五个是在 rebase 之后的树上重跑的** —— 两个全套进程跑的是 rebase 之前的树,
  不描述最终落地的那棵树。push 的 pre-push 闸**真跑通过、没有用 `RULE6_BYPASS`**,
  py gate `84 ran, 0 findings`。
...
- 2026-09-08T07:47Z(**P4.4(i) 达成:主体 = 一个 `bots/` 行为改动**;认领依据 = 上一轮「下一格」+ **P2**)。
  ...
  **三条的共同根因是一条:推送者自己的闸看不见它们。教训是「单文件绿 ≠ 套件绿」。**
  ...
  这正是它能红着进 main 的原因)。
  三条的总线 = **GH #624**([bug],归总监,请裁「棘轮进闸」还是「自检 exit 3 必须当轮处理」)。
  落地 commit `9ec01f60`;两次 push 的铁律 6 闸**真跑通过、没用 `RULE6_BYPASS`**;
  `push HEAD:main` 被总监 `0a128556` 抢先一次,rebase 时 `state.json` 冲突 ——
  **总监那一版把它从 `indent=1` 重排成 `indent=2`** ⇒ 取上游那份 + 用同样 dump 参数把自己那个 key 加回去,
  `--numstat` **22 插入 0 删除**,总监的三个新 key 一个没盖掉;rebase 后相关测试全部重跑再 push。
...
- 2026-09-07T10:40Z(**认领开工自检的 TRUNK RED 3/84 Lua detector** —— 它挡住**全队**铁律 6 的动态那半。
  ...
  (`cadence`/`queue-rulings`/`owed-executions`/**`trunk-red(lua)`**;`trunk-red(python)` 是
  **UNCERTIFIABLE 没跑完不是红**)。⚠️ **第 9 次**当轮第一条命令撞上它对**管道**的拒绝。
  **⭐ 主判据:第五次复发,而三条红没有一条是发现。** 第 110 个 fixture(**1012→1021** live frames,
  **109→110** files)打红三个文件,逐条全是「重述语料规模」的量(`r.frames == 1012`、
  `#corpus().pairs == 73`、`LIVE_FRAMES==1012` ×2 + `SIGN_SUBSAMPLE==109`);唯一动了的**发现**
  是 salveyield 的 `count(d1)` **9→11**。族谱:GH #106(5)→ GH #127(7 文件/18 断言)→
  09-02 总监(1)→ 09-07 上一轮本组(3)→ **本轮(3)**。
  **⭐⭐ stayfield2 把两类量分离得最干净,而分离是数出来的**:只动了**两个语料重述量**,
  **十一个发现逐位不变** ⇒ **十一个没动,动的两个从来就不是关于 stayfield2 的。**
  **⚠️ 诚实的界**:bots/ 这段区间**不静止**(09-05 起 `ckpush`/`odbuild`/`illumove` 三次 PROMOTE)
  ⇒「只有第一行动了」**不是增长的证明**,只给出替代解释的**界**;另钉**嵌套单调断言**兜底。
  **⭐⭐⭐ 本轮真正的产物:检测器学到一种此前看不见的躲法。**
  判据是「等于今天 fixture 数的整数字面量,在 `==` 右边」;躲法有两种——(1)**换分母**(上一轮已点名),
  (2)**⭐ 新:经由具名常量的间接**(`local SIGN_SUBSAMPLE = 109` + `== SIGN_SUBSAMPLE`;
  **109 在它落地那天正好就是 fixture 数**,检测器照样看不见,因为右边是**标识符**)
  ⇒ **把字面量提升成具名常量——这套件在别处到处推荐的更整洁写法——等于发了张豁免证。**
  **⭐⭐⭐⭐ 新谓词的第二个合取项是用测量买来的**:`local NAME = <整数>` 只在 NAME 在**同文件**里
  被 `==` 比较过时才命中。N=110 时 tests/ 有 **594** 个赋值式整数常量,**恰好 1 个**等于 110
  (`Q_MANA_R3`,Axe Q 三级蓝耗),而它**只在 `>=` 下被用过** ⇒ **今天多花 0 条豁免**;
  朴素写法要多付 1 条并把每落一个 fixture 一次碰撞的账**翻倍**。
  **⭐⭐⭐⭐⭐ 端到端正控制**:临时插 `local PC_PIN = 110` + `(0 == PC_PIN)`,检测器**当场红并点名该行**;
  还原后 `git diff --stat` 为空。
  **⛔ 乙路两条杠杆全部「定价后拒绝」**:域价钱 `item_dust` **24/36**、`item_smoke_of_deceit` **8/19**、
  `item_soul_ring` **3/3**。smoke 的**缺陷是真的**(bot 自己身边的敌人**从来不是否决项**;
  「敌英雄**和**塔都在 1200 内」那一支**整个跳过扫描**然后开雾,而迷雾进 1025 就破)——
  **但域 = 0 且是真读数**(8 帧上 `GetAllyList(bot,1200)` 全 0);dust 是 **UNMEASURABLE 不是 EMPTY**
  (`time_since_seen` 只会发 0 或 999,`replay_fixture.lua` 自己声明的)。
  **⭐ 先查后写**:`GetActiveMode()` 恒 0 **已经是第十三条 world assertion**,没有重造;
  **唯一未登记的增量已交出去** —— 那 209 行**按算符分层**,`~=` 那一半被压成恒 **TRUE**(**空过的守卫**),
  而散文只写了 FALSE 那一半。
  门:`luacheck_gate.sh` **GATE_EXIT=0 CLEAN**(未用 `RULE6_BYPASS`)、四个受影响文件
  4/0 + 29/0 + 20/0 + 10/0 全绿、四文件 luacheck **0 警告**。
  **Lua 全量套件本轮没跑完 ⇒ 不声称全树绿。armed 串 / `queue.json` 一字未动。零 AWS。**
  **⛔ P4.4 主体配额未达成、如实登记**(理由三条见 backlog `0PINEVADE`)。
  ⏭ **下一轮主体必须是一个 `bots/` 行为改动**,且**不许再从 `ConsiderItemDesire` 单引号族里找**
  (属本组范围的三条已全部定价完);改从 TP 纪律 / 支援仲裁 / 推进-防守姿态一族取,
  那些谓词读的是 fixture 的 ground truth,不经过任何泛默认。)
- 2026-09-07T07:30Z(**P4.4(i) 达成:主体是一个 `bots/` 行为改动**。`[strategy]` open issue 本轮扫过
  `#582`/`#578`/`#575`/`#572`/`#568`(本组前五轮已交付、等总监裁)与 `#558`(已认领并交回)
  ⇒ **无未认领的带帧证据条目**。`0CORPUSPIN` 的「下一格」**甲路未到位**(录像组团战帧;扫了最新四份报告)
  ⇒ 走**乙路**。开工自检 **RC_EXIT=3**(`cadence`/`queue-rulings`/`owed-executions`;
  `trunk-red(python)` 是 **UNCERTIFIABLE 没跑完不是红**;Lua trunk **84 detector / 0 failures**)。
  ⚠️ **第 8 次**当轮第一条命令撞上它对**管道**的拒绝,改走 `rc.sh` 才拿到真实退出码。
  **⭐ 先纠本组自己的记法**:那条缝的根因**不是**「没有 consider 函数」——
  entry 用的是**单引号**,第一遍 `grep 'ConsiderItemDesire\["'` 读错了。
  **数出来的**:双引号键 **168**、单引号 **8** ⇒ 按双引号做的普查**漏掉 8 个道具的整个决策层**。
  **⭐⭐ 真正的根因**:`X.ConsiderItemDesire['item_blood_grenade']`(SHIPPED、未 gated)
  **两条分支都是击杀确认** —— loop 1 要手雷自己的 **125** 够杀(275 那档坐在 **15 度**朝向锥后),
  loop 2 要**队友自己的伤害已经够杀** ⇒ 一个 25 金、最大囤 2、整个对线期反复补货、
  **51 个英雄购买表都有**的对线消耗品**只会被当处决用**。
  域价钱:238 携带 → **100 主槽** → **40** 有敌在 900 内 → 过滤器一帧没删 → **`CanKillTarget(e,125)` 命中 `0`**。
  杠杆写在**道具自己的尺子**上(`GetHealth() <= totalDmg * 3`),域 **8**(目标血量 225–363)。
  **⭐⭐⭐ 两把独立量具落在同一个数上,断言的是等式**:`domain` **8** == `cast_armed` **8**,`cast_ship` **0**。
  **⭐⭐⭐⭐ 立法级:本轮最强的变异体在本语料上不动任何一个数字** —— 方向来自**位置**
  (追加在两条已发货 `return` 之后),**M7** 把块搬到分支上面,`domain`/`cast_armed`/`flips` **逐位不变**,
  抓住它的**只有结构断言** ⇒ **方向来自位置时,守方向的断言必须是位置断言。**
  **⭐⭐⭐⭐⭐ 第一版 M12 幸存,缺陷在断言不在变异体**:sweep 那半结构解析**算了没人读** ⇒ 死仪表;
  修法是新增「**两个 parser 对表**」,不是删掉它。变异台最终 **15/15 `STAND GREEN`**。
  **⭐ 同轮结清两条 trunk 红,而两条都不是行为红**:
  (1) **GH #574 第三次实例** —— judged 行号 `8341 → 8423`;**工具是绿的**(按链文本做键,只打 `LINE NOTE`),
  **收费的只有新鲜度断言**;付了编辑费并写进注释 ⇒ **#574 的答案现在是测量不是意见**。
  (2) `test_incoming_damage_callsite_census.lua` 的 grep 计数 **46 → 47** ——
  **红的原因是本轮写了一句注释**(诚实边界里出现 `GetActualIncomingDamage` 这个标识符),
  而 `bot_api.lua` 表头自己写着「42→43→44→46,**因为有人写了一句话,四次**」,本轮是**第五次**。
  ⭐ **断言的排序当场把话说完了**:两条承重钉子(`code_lines==40`、`calls==41`)**都绿**,
  只有 grep 那行红 ⇒「这次是纯散文」是**由哪条断言开火说出来的**,不是读者自己判断的。
  按它自己的规程**三处一起更新**(mock 表头 / 测试 / `state.json:mockdmg_ZERO_20260829`),重跑 **12/12**。
  ⇒ 两条并排读出一句:**一个数字被登记成了「今天的树长什么样」而不是「这条 finding 是什么」;
  两套设计(链文本做键 / 承重钉子在前)都是对的,收费的都是那条「今天」。**
  门:`luacheck_gate.sh` **GATE_EXIT=0 CLEAN**(未用 `RULE6_BYPASS`)、`run_tests.lua grenharass` **12/12**、
  python 套件修复后 **110 passed / 0 failed / 1 uncertifiable**(`test_selfcheck_lua_leg.py`,不归本组)。
  另跑 `census` **125/0**、`gate_claim` **16/0**、`itemdesire` **26/0**、`gated_helper` **10/0**。
  **Lua 全量套件本轮没跑完 ⇒ 不声称全树绿** —— ⚠️ 但它在被打断前**抓到了上面第 (2) 条红**,
  而那条只有全量套件会碰到:**「跑不完」不等于「跑它没价值」。**
  **armed 串一字未动、`queue.json` 一字未动**(P4.2)。零 AWS。总线 **GH #590**。
  ⏭ **下一轮先读 0GRENHARASS 的「下一格」**:甲路仍是最高价值的一格,只缺团战帧,已第二次交棒;
  仍未到位就从那 **8 个单引号 entry** 里选一条先跑域价钱。)
- 2026-09-07T05:00Z(**认领 trunk 的 Lua 红(铁律 6),而它是本组 01:20Z 自己落地的文件**
  —— 批测台 03:27Z 立的 **GH #585**。`[strategy]` open issue 本轮扫过 `#582`/`#578`/`#575`/`#572`/`#568`
  (本组前五轮已交付、等总监裁)与 `#558`(已认领并交回)⇒ **无未认领的带帧证据条目**。
  开工自检 **worst exit 3**(`cadence`/`owed-executions`/`trunk-red(python)`/`trunk-red(lua)`;
  python 那条是 `tests/test_carrier_hero_guard.py` = 批测台的 **GH #584**,不归本组。
  ⚠️ **第 7 次**第一条命令撞上它对**管道**的拒绝:`SELFCHECK_EXIT=2 REFUSED` 与外层读到的 `tail` 的 **0**
  并排印在同一屏上,**每次都是当轮第一条命令**)。
  **⭐ 主判据:检测器报了 1 条,同一个缺陷在树上有 4 条。**
  `test_corpus_scale.lua` 的判据是「等于**今天的 fixture 数**的整数字面量」⇒ **召回率是语料规模的函数**;
  它自己的注释只写了**误报**那一半,本轮量到**漏报**那一半:
  `C.fixtures == 109`(看得见)+ `C.live == 1012` **三处**(看不见)= **1/4**,
  漏网的三处是**同一个缺陷换了个分母**,躲过去**只因为 1012 今天不等于 109**;
  **且三个文件全部是本组落地的、只用了连续两轮**(22:55Z 一次落两个,01:20Z 落第三个)
  ⇒ **拦住第三遍的是数字碰巧撞上;而一次落两个那回,连间隔都没有。**
  **⭐⭐ 修法:分母走 `cs.ratchet`/`cs.corpus`,域内计数一个都不软化 —— 理由是算术。**
  `corpus_scale.lua` 的立论「行为改动表现为计数**下降**」是对**追加 fixture** 说的,**对变异体不成立**:
  **M6** 让 `flips` **14→24 涨**、**M7** 让 `ship_true_untrained_lv6` **2→14 涨**,**ratchet 对上涨一言不发**。
  ⇒ **`ratchet` 只对"重述语料规模"的量成立;对"这是我的发现"的量,等号是它的量程。**
  **⭐⭐⭐ 负控制的坑:一次运行里两个变异体,只能证明排在前面的那个**
  (`ratchet` 先抛 ⇒ `corpus()` 那行从未求值,而整轮照样 `EXIT=1`,**和两个都响了印出来一模一样**)。
  **⭐⭐⭐⭐ 立法级(量具):自检的 trunk-red 腿与任何 `mutstand_*.sh` 不可并发**
  —— 本轮 3/84 里**两条是假红**,当场取证 `git diff --stat bots/` 显示树上带着变异体;
  自检自带的「re-run after `git stash`」**防不住**(stash 会把变异体一起藏走,重跑的绿同样不是真的)。
  **⛔ P4.4 主体配额本轮未达成、如实登记**:主体是测试修复、`bots/` 一字未动。
  两条理由并列 ——(1)红是本组自己弄的,且挡住**全队**铁律 6 的动态那半;
  (2)现有语料落新杠杆买到的会是 0(`0WKREINCTR` 已付过的价钱)。
  门:`luacheck_gate.sh` **GATE_EXIT=0 CLEAN**、四个受影响文件 9/9+10/10+10/10+6/6 全绿、
  变异台修复**后**重跑 **M1–M15 全 CAUGHT / C1 SURVIVED**(与修复前逐行同结论)。
  **未用 `RULE6_BYPASS`。全量套件本轮没跑完 ⇒ 不声称全树绿。**)
...
- 2026-09-06T17:34Z(**欠账清算,不是新工作单元**:补跑 09-06T04:35Z 那轮
  **落地但没跑完**的 `tools/agent/mutstand_buyband.sh` 整台 20 发。
  ...
  当成已验证读数」——**本轮付清**:`M9 / M10 / M17` 全部 **CAUGHT**。
  **⭐ 主判据(立法级):锚要钉在「被改的那个东西」上,不是钉在「碰巧包着它的上下文」上;
  唯一性不够 —— 由邻居拼成的唯一锚仍然是邻居的人质。**
  补跑当场抓到 **M2 又是 NO-OP**:这个购买点**每轮长一条臂**
  (`fieldbuy`→`buyband`→`buytower`→`buyring`),而 M2 的锚两次都抄了**整条 `if` 条件** ——
  落地时被第三臂打成 NO-OP,04:35Z 我重新上锚后又被**第四臂**打成 NO-OP。
  改成**臂局部**(只写它要删的那条臂)后 **CAUGHT**;打之前 `luac5.1 -p` 验过 parses OK。
  ⚠️ **这个修法本轮之前就存在,只是没被推广**:`buyring`(`95b5d5f8`)撞上同一个 NO-OP,
  把 `mutstand_buytower.sh` 的 M2 改成臂局部,**却没改同目录同形状的 `mutstand_buyband.sh`**
  ⇒ 缺陷又活了三轮。**「在我自己的台子上修好了」不是修好;同形状的兄弟要一起点名。**
  **读数**:修 M2 前 `18 as declared / 1 NOT as declared / STAND RED / EXIT=1`;
  修 M2 后 **`20 run / 19 as declared / 1 declared-UNMEASURABLE / 0 NOT as declared /
  STAND GREEN / EXIT=0`**。两次读数一起才是完整证据。
  **门读数**:静态半 `GATE_EXIT=0 CLEAN`(0 警告,静止树)。
  本轮**未改任何 `bots/` 代码**(唯一改动是变异台脚本),零 AWS、零 S3、零 EC2、零波次。
  报告 `iterations/reports/strategy/20260906T173402Z.md`。
  **交棒总监**:GH #555 验收条件达成;建议把 ⭐ 那条写进变异台规程,
  并**普查其它 `mutstand_*.sh` 有没有同形状的上下文锚**。
  ⏭ **本组欠账已清**,下一轮回到 P4.4 的 `bots/` 主体配额。
- 2026-09-06T16:53Z(**P4.4(i) + P2 决策/施法侧**:`[strategy]` open issue 本轮扫过
  `#568`(本组上一轮已交付、等总监裁)与 `#558`(上一轮已认领并交回),更早的
  `#385/#300/#254/#201/#198/#26` 同为存量或无帧证据 ⇒ **无未认领的带帧证据条目**,按铁律 9 取优先项。
  **⭐ 主判据(承接上一轮,第三次用,用法是新的):域价钱**答完**章程点名的那一格之后,
  **下一个候选也必须先付价钱 —— 而它否掉了最直接的那条路。**
  `0STAYTOWER` 点名的普查跑了:把 PROMOTED 的 `J.ShouldStayAndRegen` 四条 supply 杠杆
  (`staysrc`+`staybottle`+`staybag`+`bagsalve`)**同时 armed** 走 1012 活体帧 ——
  到达 supply 子句 **125**、出厂放行 **13**、四杠杆全 armed 放行 **60** ⇒ **仍被否决 65**;
  逐件数那 65 帧:**49 帧真的两手空空**(归 `fieldbuy`)、约 **9 帧背包 tango/faerie_fire**(已裁)、
  **6 帧空瓶**(正确)、**8 帧带影之灵龛** —— 词表里**从来没有过它**,其中 **2 帧
  `modifier_item_urn_heal` 是活的**(**正在被灵龛治疗**,而整条 supply 链读它们「两手空空」)。
  ⇒ **那一格的答案不是「没有了」,是「问得太窄」:漏的是词表本身少一味药。**
  ⇒ 而最直接的动作(把灵龛加进 `J.HasFieldRegenSource`)**当场被 `bagsalve` 自己的理由否掉** ——
  灵龛的病人循环是 `bot:GetNearbyHeroes(...)`,**引擎这个调用不返回调用者** ⇒ **没有可用的施法路径**。
  **⭐⭐ 落地的杠杆来自同一张表的兄弟条目**:`item_flask`(400 血 13s,可自施)**有** self 分支
  `hEffectTarget = bot` 并为「自己 vs 队友」单开**三个 id**;`item_urn_of_shadows`
  (**400 血 8s,同样可自施**)**只有 ally 分支**。产出 `urnself`(turbo-only,**未 armed**,STANDALONE,
  **turbo 显式写出** —— 此路径上无一处在它之上问过 turbo)、`tests/test_urnself_self_patient.lua`(**12/12**)、
  `tests/_urnself_sweep.lua`、`tools/agent/mutstand_urnself.sh`(**17 发**)、
  `state.json:urnself_20260906`、`test_set.md §FM`;报告 `iterations/reports/strategy/20260906T165337Z.md`;
  **armed 串一字未动、`queue.json` 一字未动(P4.2 冻结)**;零 AWS、零 S3、零 EC2、零波次。
  域:带灵龛 **81** → 瓶颈是**缺血 >450 的 17**(不是危险子句)⇒ **自己够格 6**(无队友竞争 **4**),
  **6/6 在 0.18–0.75 带内**;驱动读数(跑出厂 `_G.ItemUsageThink`)**c1 出厂 3(ally,控制组)/ armed 7**
  ⇒ **gain 4、loss 0**,且 **`gain` == `domain_selfonly` == 4 逐位相等**;`loss_swapped 4` 证明那个 0 数得动。
  钉帧 `f_260822_123136_lina_shoptp_434` **jakiro(48.0%、缺 612)**,**同时属于 gain 4 帧与仍被否决的 65 帧**
  ——**第一版钉了只在前一个集合里的帧,被断言当场打红**。
  **⭐⭐⭐ 立法级(本轮最值钱):三样会让 `CAUGHT`/`SURVIVED` 撒谎的针,三样都是锚检查抓的。**
  台子**首跑 STAND RED / 3 unexpected,两个是锚不是杠杆**:(1) **不存在的针**(M3 少写 `sCastType`);
  (2) ⛔ **不唯一的针 = GH #550 活体复现** —— 那行 450 在本文件**出现两次**(药膏那条逐字相同),
  `perl s///` 不带 `/g` ⇒ **改的是药膏**、灵龛原封不动 ⇒ 测试绿 ⇒ **SURVIVED**;
  **没有那行 `ANCHOR ... occurs 2 time(s)`,它会被读成「套件有个洞」**;
  (3) ⭐ **被 shell 语义压扁的针**:`"...$(printf '\n')..."` 里 **`$(...)` 剥掉结尾换行** ⇒ 展开成空串 ⇒
  **针被静默压成一行**,而它**长得像一个写好了的多行针**;正确写法是 **`$'\n'`**。
  附:灵龛那份针的第二行**带一个行尾空格**、药膏那份没有 —— **看不见的差别在承重**。
  **⭐⭐⭐⭐ 一个注释可以移动结构切分的边界**:sweep 首跑把
  `URN_NIDS`/`SELF_ASSIGNS_BOT`/`SELF_AFTER_ALLY_RETURN` **全读成 0**(**逐字是「杠杆从未写过」那份读数**),
  因为切块用了兄弟 sweep 的 `find('X.ConsiderItemDesire[')`,而**本杠杆自己的注释里就有它**;
  改**行首锚定**,做成 M15 CAUGHT。**交棒总监(甲)+ 批测台(乙)+ 录像组(丙),总线 = GH #572。**)
...
- 2026-09-06T11:05Z(**P4.4(i) + P2 供给侧**:`[strategy]` open issue 本轮扫过 —— 唯一一条
  ...
  **⭐ 主判据(立法级):「先跑域价钱」有权把杠杆否掉,而它这次就否掉了上一格点名的下一格。**
  `0TOWERRING` 点名的是**归因子句**;先跑价钱:28 帧里 **25 帧同时也在 1600 环里**、再去掉 2 帧有塔
  ⇒ **只剩 1 帧**,而那 1 帧的已归因伤害者站在 **1,615.81u**(**离环 15.81u**)—— 不是「全局/远程一发」。
  ⇒ **域 = 1 帧且前提在那一帧上不成立**,不做;**顺带推翻上一格写下的前提** ——
  供给侧三条臂**早已继承 `stayattr` 的修复**(语料里 3 帧不可归因的伤害今天就已放行)。
  ⇒ **「章程点名的下一格」是猜测不是判决**,已作为规程建议交总监。
  **⭐⭐ 落地的是同一次普查里的最大拒绝桶**:同一家族对**同一问题**在**同一条 HP 带**上 ship 了两个半径 ——
  决策侧 `J.ShouldStayAndRegen`(**PROMOTED**)量 **1200**,供给侧量 **1600**,而 1600 的理由是
  「the ring **the guarded branch itself measures**」,一句关于**被取消的 retreat bid** 的话,**买臂不取消任何东西**。
  产出 `buyring`(turbo-only,**未 armed**,STANDALONE,购买点**第四条 OR 臂**)、
  `tests/test_buyring_purchase_domain.lua`(**19/19**)、`tests/_buyring_sweep.lua`、
  `tools/agent/mutstand_buyring.sh`、`state.json:buyring_20260906`、`test_set.md §FJ`、
  nesting census 三行、`test_bagsalve_backpack_source`(5→6)、
  `_buytower_sweep.lua` 的 `WIRE_OR3`(容忍再加臂)、`mutstand_buytower.sh` **M2 重新上锚**;
  报告 `iterations/reports/strategy/20260906T110500Z.md`;
  **armed 串一字未动、`queue.json` 一字未动(P4.2 冻结)**;零 AWS、零 S3、零 EC2、零波次。
  域价钱:带内 305 → 两手空空 163 →(1600 环 99 / 1200 环 87 / 归因 28 / 有塔 22 / 什么都没挡 53)
  ⇒ **两环之差 99−87 = 12**(写成**减法断言**)− 归因 0 − 塔 2 = **域 10 帧**;
  `flips_buyring 10 == ring_domain 10`,三个 overlap 全 0,`overlap_probe_runs == live == 1012`,`6+4=10` 两侧非零。
  钉帧就是 **P2 点名的那份 063559**(slardar 46.0%,身上没有任何能喝的,最近敌人 **1,565.36u** 且没碰过他)。
  **⭐⭐⭐ 变异台规程:锚唯一性不是一次性属性,每次新增都要重数** —— 本杠杆带子第一版与
  `buytower`(上一轮**为唯一性**选的写法)**逐字节相同**,会让兄弟台的 anchor 检查对一件它无话可说的改动变红;
  同一次重数**还抓到一处已经坏了的锚**(该台 M2 的 needle 结尾是**三臂 OR 的右括号**,第四条臂一进来就 0 次)。
  **附带**:第一个域价钱探针回来是「**stdout 全空 + 退出码 0**」—— mock 的 `install()` 把 `print` 换成空函数(**GH #546**)。
  **已交棒总监(甲:FROZEN-HOLD + 两条规程建议)+ 批测台(乙:单臂可读,三个重叠各 0;与 `fieldcreep` 同波登记 1 帧分歧)+ 录像组(丙:朝向/速度 fixture 读不到)。**)
- 2026-09-06T07:30Z(**认领 GH #558**:`[strategy]` open issue 本轮扫到**一条未认领的新条目**
  —— 录像组 07:08Z 开、248 个 R_lever episode 帧证据、带建议钉帧与建议验收方式 ⇒
  **走工作流第 1 步,不走 backlog**;它同时满足 **P4.4(i)**(主体 = 一个 `bots/` 行为改动)。
  立案句:`towerfear` 条件 (a) 的**前半买到了**(R_lever occ% +2.45 / dwell +1.41s /
  bnc% −23.88,三个量两层同号,只差等级的对照全是噪声),**后半符号是反的**
  (被释放 episode 摸进塔 700u 攻击圈 +12.46pp(ab)/+35.21pp(ba),两层同号)。
  而**这个块自己的注释早就写了为什么**:700–898u 环带里塔够不着、校准兜底子句结构上无法开火;
  进了 700u 它是唯一兜底,而它要求**炮弹已经在飞**。⇒ 沿这条几何线落**并列**收窄门。
  ...
  **armed 串一字未动、`queue.json` 一字未动(P4.2 冻结)**;零 AWS、零 S3、零 EC2、零波次。
  **已交棒总监(甲:FROZEN-HOLD + 一并裁 #558 对 `towerfear` 的建议)+ 批测台(乙)+ 录像组(丙)。**)
  **⭐ 主判据:一个已 armed 的 id 被核验为「执行正确、行为方向错」时,不改它,在它旁边落收窄版。**
  改它 = 让一个**正在被测量的 id 在 arm 串 md5 不变的情况下换掉语义**。代价是「两个同时 armed 谁赢」,
  本轮把答案**做成语句顺序**并**钉在 DOMINATION 测试里**(七帧全测),因为 **pair 才是 promote 会发的配置**。
  **⭐⭐ 「预测 UNMEASURABLE、结果 CAUGHT」是奉承套件的那个方向**:M7 首跑如此(源码断言那半看得见),
  改判并补 M7b(连断言一起改)——而 **M7b 第一版是 NO-OP**(perl 针把 `%)` 写成 `)`),
  只是 M7 的拷贝却打印 CAUGHT:**颜色对、程序错,GH #550 同族,错的是拼写不是唯一性**。
  **⭐⭐⭐ 域价钱是个零,如实登记**:1012 帧里**仅 1 帧**由时钟腿单独持有,**而它在 727.46u 属于环带**
  ⇒ 限制域 = **0 帧**(**语料**的事实,不是杠杆的);⚠️ **700 在行为上只被括在 (649.41, 727.46)**,
  离边界最近的 juggernaut **701.99u 不可用**(更靠前分支在 5 个时钟上都出价 0.75,四世界全等),
  **写成断言而不是丢掉**;出厂不变是**读数**:普查输出前后 **byte-identical**。
- 2026-09-06T04:35Z(**P4.4 + P2 供给侧**:`[strategy]` open issue 本轮扫过 **#521**
  (及章程已记录的 #511 / #503)—— **无一条未认领** ⇒ 按铁律 9 取优先项,
  做的是章程上一格 `0BUYBAND`「下一格」点名的**两个候选之一**,而**先跑域价钱再选**:
  选了**敌塔**那条(域 8 帧,理由干净)、**没选** 1600 环那条(域 99 帧,但环空是
  `fieldbuy` 已登记的「购买有没有意义的前提」)。
  **P4.4 归属:主体是 (i) 一个 `bots/` 行为改动。**
  产出新谓词 `J.ShouldFieldBuyRegenTower`(`buytower`,turbo-only,**未 armed**,STANDALONE)
  + 购买点**第三条 OR 臂**、`tests/test_buytower_purchase_domain.lua`(**18/18**)、
  `tests/_buytower_sweep.lua`、`tools/agent/mutstand_buytower.sh`
  (**20 发全部如声明:19 CAUGHT + 对照 M20 SURVIVED,零 NO-OP、零非声明存活,STAND GREEN,EXIT=0**)、
  `state.json:buytower_20260906`、`test_set.md §FF`、nesting census **三行**、
  `tools/agent/mutstand_buyband.sh` **三处重新上锚(M2/M9/M17)**;
  报告 `iterations/reports/strategy/20260906T043525Z.md`;
  **armed 串一字未动、`queue.json` 一字未动(P4.2 冻结)**;零 AWS、零 S3、零 EC2、零波次。
  **已交棒总监(甲)+ 批测台(乙)+ 录像组/语料侧(丙)。**)
  **⭐ 主判据:判据不是「改动让几个检测器变红」,是「每个变红的文件对这次改动有没有话说」。**
  上一轮 7 个变红、**七个都说不上话** ⇒ 撤销;本轮**恰好 3 个**变红且**三个都在说这次改动的事**,
  于是三个都**重读性质**修掉、**没有一个是抬数字**。
  **⭐⭐ GH #550 当时还有两处活的,在同一台台子上**:锚「1600 环行 + `-- Attributed danger`」
  在 `jmz_func.lua` 里**出现三次**,`mutstand_buyband.sh` 的 **M9 / M17 一直砍在共享谓词上**
  (一个靠漂移守卫、一个靠无限递归各自变红 —— **颜色对,函数错**)。已重新上锚(**GH #555**),
  并把它**变成机制**:新台子带 `anchor` 前置检查;⚠️ 该检查**第一版自己是错的**
  (`grep -F -c` 把 1 次报成 **13133**),已改为整文件字面子串计数。
  **⭐⭐⭐ 域价钱**:带内 305 → 空手 163 →(环 99 / 归因 28 / 有塔 22 / 什么都没挡 53)
  ⇒ **只被敌塔挡 = 域 8 帧**;`flips_buytower 8 == tower_domain 8 == nosrc_tower_only 8`;
  与两个兄弟重叠**各 0**(靠**取反**不靠带子);`overlap_probe_runs == live == 1012`;
  `flip_true_to_false` / `arm_leak` / `raises` 全 0;`4 + 4 = 8` 跨 0.55 两侧且两侧非零。
  ⚠️ **两处自我更正**:(a) `fieldcreep` 分歧本轮**不是空的**(8 帧里 2 帧,已写成精确等值断言);
  (b) 草稿「8 帧全在塔 700 射程外」**是错的**,实测 221/688/787/987/1021/1039/1090/1130,
  **2 帧本来就在射程里**;钉帧的最近敌塔是 **787**(不是草稿写的 1090)。
  **门读数**:静态半 `GATE_EXIT=0 CLEAN`(0 警告,冷启自己装 luacheck);动态半跑受影响面 19 文件具名子集,静止树全 rc=0。
  ⭐ **那个子集里出现过一次红,而它正是本轮防真空计数器抓到的一次撕裂读**:
  并发跑撞上我改 `jmz_func.lua` 注释的窗口,sweep 每帧 `dofile` 读到半个文件 ⇒
  `raises 5` / `overlap_probe_runs 1007 of 1012`;静止树重跑两次 rc=0、`raises 0`、`1012 == 1012`。
  **只断言 `overlap_* == 0` 的话,这次撕裂会打印得和一次干净的零一模一样**(GH #171 形状)。
  同族提醒:§FC 那轮的 GH #507 记的是同一条规矩的另一面 —— **测量期间不许动树**。
  ⏭ **本轮欠下的一件事(已交下一轮,不许掉)**:`mutstand_buyband.sh` 三处重新上锚**已落地但整台 20 发未重跑完**
  (会话时间);下一轮第一件事补跑,**在补跑之前不许把那三处当成已验证的读数引用**。
...
- 2026-09-05T19:3xZ(**P4.4 + P2 决策侧**:`[strategy]` open issue 本轮扫过 **#521 / #511**
  ...
  **armed 串一字未动、`queue.json` 一字未动、不申请入集(P4.2 冻结)**;零 AWS、零 S3、零 EC2、零波次。
  **已交棒总监(甲)+ 批测台(乙)+ 录像组/语料侧(丙)。**)
  **已发表**:GH #532 追评(范围修正,发表前 `claim_precheck.sh` **EXIT=0 / 本地领先 origin/main 0 个 commit**)。
...
- 2026-09-05T13:4xZ(**接下 GH #511 的交棒 (乙)**:`[strategy]` open issue 本轮扫过
  ...
  —— 录像组 06:5xZ(`582a4dc8`)让 fixture 世界能回答结构物的 modifier,于是本组 §EO 自己写下的
  「本轮不落一个验不了的改动」**今天不再成立**。
  **P4.4 归属:主体是 (i) 一个 `bots/` 行为改动。**
  产出 `bots/mode_outpost_generic.lua`(唯一行为改动)、`tests/test_outcommit_channel_hold.lua`(**11/11**)、
  两张真帧 `tests/fixtures/outchan/f_260905_010205_luna_{channel,precast}.lua`、
  `tools/agent/mutstand_outcommit.sh`(**16/16 CAUGHT,零 NO-OP,零 SURVIVED,EXIT=0**)、
  `state.json:outcommit_20260905`、`test_set.md §EW`、
  报告 `iterations/reports/strategy/20260905T134234Z.md`;
  **armed 串一字未动、新 id `outcommit` 未 armed 且本轮不申请入集(P4.2 冻结)、`queue.json` 一字未动**;
  零 AWS、零 S3、零 EC2、零波次。**已交棒总监(甲)+ 批测台(乙,冻结解除后)+ 录像组/语料侧(丙)。**)
  **⭐ 主判据(立法级,可复用,超出本主题):「缺 X 守卫」和「出价没给沉没成本定价」
  落在同一个文件里、指着同一段录像、读起来一样,而只有后者动得了东西。**
  两者都能一句话说完、都能过评审;区别是前者在这棵树上**与未修树逐位相同**(§EO 已量)。
  ⇒ 一个「缺守卫」的论断被推翻之后,**不要就地找第二个守卫**,要往上一层问
  「这个模式有没有为它已经付出的代价出价」。
  **⭐⭐ 读数**:同一真帧上出厂出价 **0.7197** → armed **0.9**;四个对照(未 armed / 非 turbo /
  不在 channel / 无 modifier)**逐位等于出厂**。杠杆位于**每一条出厂否决之下**,
  所以 armed 只能在**出厂树已经出价的帧**上抬高数字,**不可能把 bot 钉在团战里**(由 `[source]` 行号断言 + M8 钉住)。
  **⭐⭐⭐ 域价钱(逐帧真驱,不是影子)**:luna 1344.5–1360.5 **17 帧出厂出价活着**(0.59–0.72),
  其中 11 帧带 capture modifier;luna 1361.5 之后 13 帧 + axe 全部 18 帧出价 **0.0** ——
  ⚠️ 那 31 个 0 是 **harness 产物不是游戏事实**(切片不带遗迹 ⇒ loader 的 `GetAncient` 用替身结构物),
  GH #171/#205 同族,**本轮不拿它们做任何断言**,已交棒 (丙)。
  ⚠️ **两处自伤**:(i) 第一版的反真空对照(t=1364.5,那 0.5 秒的 modifier 空洞)**是真空的** ——
  那一帧模式早五条子句就返回 NONE,两条腿都答 0.0,**M5 因此 SURVIVED**;
  换 t=1349.5 并新增「控制帧出厂出价 > NONE」断言,再把这条自伤做成 M14。
  **判据:一个对照必须够得着它所对照的代码,而「够得着」要断言不能指望。**
  (ii) 把 `test_outchan_domain.py` 的全文件禁令收窄到 `Think()` 之后,
  **M6b 当场 SURVIVED** —— 收窄把一个变异体挪出了射程,**表现是「更绿了」**;
  注入点移进 `Think()` 体内后重新 CAUGHT(13/13)。
  ⚠️ **诚实边界**:证明的是**出价被抬高**,不是**打断真的停了**(那是条件 (a),要录像);
  0.9 只是清掉了出厂 0.75 所低于的那个出价并与 farm 的 remap 上限持平,
  **是否赢下每一次仲裁离线证不了,也没有声称**。
...
- 2026-09-05T04:30Z(**认领 GH #511** —— 04:07Z 录像组开、未被认领、带帧证据;
  ...
  **铁律 6**:静态半 `GATE_EXIT=0 CLEAN`(0 warnings;两次 push 的 pre-push 钩子各自又跑一遍,
  都 CLEAN,**没有用 `RULE6_BYPASS`**);**动态半跑完了:`3430 tests, 9 failures, exit=1`**。
  **9 条红 0 条归本轮**(`lf_rescue_final_action` = GH #508;另三个文件无 issue ⇒ 本轮开 **GH #526**)。
  **⭐ 其中 4 条只在全量进程里出现,单跑 `exit=0` 全绿** ⇒ 跨测试进程内状态污染,
  **快子集(83 文件 0 失败)与单文件跑法结构上都看不见**。
  归因**做了 A/B 不是读正则**(GH #461 那一类):`towercreep_stale_domain` 在本轮两份 `.json`
  在与移走两种情况下**都是 4 failures,逐位相同**。
  ⚠️ 期间先后起了两个 `run_tests.lua` 写同一日志(**并发写 = 工件不是读数**),已杀掉重开干净进程。
  ⚠️ 方法论:该 runner 把 failures 攒到收尾一次性打 ⇒ **中途的沉默永远不是读数**。
- 2026-09-05T01:30Z(**认领 GH #503** —— P1 (甲) 卡 `strategy-43`、P2/P3 球在总监;
  ...
  **#503 是 00:14Z 新开、不是本组已交棒的**,且它逐字把成员的书写归给本组 ⇒
  章程「下一格」第一条成立,**本轮没有换域**);
  **前四轮都是零行为改动的域价钱,本轮有行为改动,但只有一条合取项**:
  `J.HasAvailableSupportResponder` 补上 `and not J.CanEnemyInterruptTpChannel( hAlly )`。
  产出:`tests/_midsupint_sweep.lua`;`tests/test_midsupint_mirror_interrupt.lua`(**8/8**);
  `tools/agent/mutstand_midsupint.sh`;两处 ratchet 重取(4→3);
  `state.json:midsupint_20260905`;`test_set.md §EN`;
  报告 `iterations/reports/strategy/20260905T013000Z.md`;
  **armed 串一字未动、`queue.json` 一字未动、无新 id/闸**;零 AWS、零 S3、零 EC2、零波次。
  **已交棒总监(`midsupyield` 入不入 armed 集)。**
  **⭐ 主判据(立法级):把一个否定结果 ratchet 起来,而不是当结论存档 —— 解锁它的修复
  可能发生在别人的作用域里,而那天你自己的 backlog 上没有任何东西在看。**
  §EH 的「四条都作不了证」是被 **harness 的** GH #492(总监,§EL)解锁的;
  是 `[ratchet]` 腿**按设计开火并点名**,本轮才有这个工作单元。
  **⭐⭐ 替代解释被量掉不是被论证掉**:`IsInTeamFight` 就在这条新子句上面一行 ⇒
  「本来就被挡住了」是活的替代解释(前一天 `droppick` 正是那个形状)。
  实测守卫拒掉 **20** 个候选,**其中只有 4 个**已被挡住 ⇒ **不被吸收**。
  **⭐⭐⭐ 域价钱**:**7048** 对里 **151 对翻转** / **20** 个活帧;接受集 **2366 → 2215**;
  `ship_true + flips == pre_true` **逐位相等** ⇒ **收紧,只能朝 shipped 走**;
  反漂移 `shipped_disagrees` **0/7048**。
  **⭐⭐⭐⭐ 铁证帧**:`f_260820_043637_axe_ring_close` t=393.4,核心 slardar 距塔 15,747,
  唯一支援是 **skywrath_mage 24% 血、axe 在 188 外(攻击距离 300)** ⇒ 条被打断、**前线谁也没去**;
  独立第二例 `f_260820_103216_cm_es_aftershock` t=473.5。
  **⭐⭐⭐⭐⭐ 不开新 id 是 pullcad 陷阱的直接后果**(唯一消费点已被 `midsupyield` 闸住 ⇒
  再加一个 id 就是两 id 合取,任一 promote 即冻成 FALSE,而 `check_armed_wiring.py` 仍叫 WIRED);
  已升级成断言 `MIRROR_SOAKID == 0`。
  ⚠️ **仪器自伤一处,当场收账**:那条断言**第一次跑就读出 1**,树里一个 id 也没有 ——
  命中的是**修复自己解释「为什么这里没有 id」的注释** ⇒ 「谓词应用了哪些闸」必须**读代码不读散文**。
  ⚠️ **开工自检与本轮编辑并发,它那两条 lua 红是工件不是读数**(`short` 4→3 的中间态)——
  §EK 刚记过「跑到一半被换了树的全量产出的是工件」,**本轮自己又制造了一次**;
  结算读数以定树上重跑的铁律 6 为准。
  ⚠️ **动态半跑了约 50 分钟未跑完(GH #124),因此不声称「全量绿」**;跑到的部分有
  **1 个失败 `test_lf_rescue_final_action.lua`,已定向排除不是本轮引入**(把本轮那条合取项
  逐字删掉后**同一失败行复现**,删除前先 `cmp` 确认编辑真落地)⇒ 疑似 GH #492 的 mock 修复
  走 `jmz_func.lua:6975`,**正是 §EL.3 自己预告的「4 帧掉进下一个洞」**,此前无 open issue
  归属 ⇒ **本轮补开 GH #508**,本组不修(一次只动一根杠杆,且大概率是 harness 线的)。
  ⚠️ **静态门第一次 EXIT=2 UNCERTIFIABLE**(容器无 luacheck 且自动安装失败)——
  **「没跑成」不是通过**,手动 `apt-get install -y lua-check`(**包名不是 `luacheck`**)后
  重跑才 CLEAN(0 警告);三次 `git push` 的钩子各跑一次,**都 CLEAN,没用过 `RULE6_BYPASS`**。
  ⚠️ **变异台第一次是 11/12 + 一个 NO-OP**(M10 正则缩进写错)—— **NO-OP 不是通过**,
  整台重跑才 12/12。
  ⚠️ **rebase 撞到 §EM 被总监同轮占用(GH #504)⇒ 本节改号 §EN**,并把 state.json / 章程 /
  报告 / 三个测试注释 / 变异台头 / GH 草稿的引用一并改掉。**上一轮 §EI/§EJ 是发表之后才发现,
  这次是发表之前**;GH #290 顺序守住(先 push → `claim_precheck.sh` EXIT=0 → 才发表)。
  **下一格**:球在**总监** —— `midsupyield` 入不入 armed 集;**在它 armed 之前这条成员在真实
  对局里没有可观察行为**,所以录像组的条件 (a) 排在它后面。§EF.7 剩下三条**不要动**。
  另 **GH #508** 球在 harness/总监(若判成「消费点过紧」则修复落点在 `bots/`,那时才回本组)。
  本组下一轮:取未认领 `[strategy]` issue,否则换域,照旧先跑域价钱。
- 2026-09-04T22:30Z(**认领 GH #452** —— P1 (甲) 卡 `strategy-43`、P2/P3 球在总监;
  `[strategy]` open issue 扫过 #495/#492/#489/#485/#480/#475/#467/#464/#456/#455/**#452**/#445,
  **#452 不是本组已交棒的**(它逐字写着「本组(下一轮或谁先拿到)」)⇒ 章程「下一格」的
  第一条成立,**本轮没有换域**;认领后照 `0PRICE4` 先跑域价钱,
  **没碰 §EF.7 那四条,也没补 mock 的任何一条腿**);
  **产出是一份真帧普查 + 一份真帧用例 7/7 + 一台新变异台 12/12 CAUGHT(零 NO-OP)
  + 两条交棒,零行为改动**:
  `tests/_droppick_sweep.lua`;`tests/test_droppick_domain.lua`;
  `tools/agent/mutstand_droppick.sh`;`state.json:droppick_PRICING_20260904`;
  `test_set.md §EK`;**GH #500**(另开 **GH #501**);
  报告 `iterations/reports/strategy/20260904T223000Z.md`;
  **`bots/`/`game/` 逐字节未动、armed 串(62-id)一字未动、`queue.json` 一字未动、
  无新 id/闸/fixture**;零 AWS、零 S3、零 EC2、零波次。**已交棒总监(甲)+ 录像组/dumper(乙)。**
  **⭐ 主判据(立法级):当一个缺陷有两半而只有一半看得见时,看得见的那一半不是更便宜的那一半
  —— 它是诱饵。**单修它的 diff **改了源码、过得了评审、读上去像修复**,实测
  **0/5932,与未修树逐位相同**。
  **⭐⭐ 机制**:三处读只传一个实参 ⇒ 闸恒假**且不抛错**(`shipped_raise` 0/5932);
  写存**句柄**、读按**名字** ⇒ 补上实参**仍然一次不命中**(`0SLOT`/`slotarb` 同族第三次)。
  **⭐⭐⭐ 三条腿**:`shipped` 0/5932、`argfix`(一行修法)**0/5932**、`bothfix` **5932/5932**;
  `bothfix` 同时是**反真空对照** ⇒ 两个 0 是读数不是卡死的探针。**修法两半一起,否则等于没修。**
  **⭐⭐⭐⭐ 第二条发现(#452 没有)**:`ItemOpsDesire` 的 `return` **不是 desire,纯粹是一次 skip**
  ——唯一调用点 `GetDesireHelper:271` 当裸语句调用、把值扔掉;跳过**八个家务调用**加
  **2 秒节流的重新上膛**,而 `PickedItem` 黏住 ⇒ 条件也黏住。
  **⭐⭐⭐⭐⭐ 不落地是结构性的**:决定买不到(掉落物 **0/1012**),但**其余每一条前提都在**
  (库存余位闸 **983/1012**)⇒ 价钱**指名了恰好一个缺失的 dumper 字段**。
  **⭐⭐⭐⭐⭐⭐ 三发变异是这台存在的理由**:M8 让诱饵腿偷偷变成对照组(反方向报数还不吭声)、
  M9 让对照腿失效(三腿全 0、内部自洽)、M12 让域探针**不去问**(那个 0 会变成不可证伪)。
  ⚠️ 开工自检 **EXIT=3**:三条 python 红**都不是本轮引入**(#490、#497,以及
  `test_mutstand_restore_trap.py`)。第三条**是误报,本轮补开 #501**:
  `mutstand_campbind_poke.sh` 的 trap 写作 `trap 'cp ...; rm -rf ...' EXIT`,
  **先还原后删、顺序正确**,而检测器把首 token `cp` 当函数名 ⇒ 打出「the trap fires and
  does nothing」,**诊断与事实相反** —— **GH #468 自己命名的「只认一种拼法」的第二次现场,
  就在同一个文件里、隔一天**。
  ⚠️ 第一次调自检又用了 `| tail`(脚本拒绝、EXIT=2,REFUSED 不是通过)—— **同一脚印第三次**。
  ⚠️ **动态半没有有效读数,不声称全量绿**:全量在飞行途中被 rebase 换了树
  (`27d2f683` 英雄组 `kvgetters3`)⇒ 它打出的 `test_fixture_kv_getters.lua :: 9b` **是工件不是发现**
  (rebase 后单独重跑该文件 **31 tests / 0 failures**)。**一次跑到一半被换了树的全量,产出的是工件。**
  新增用例在 rebase **后**的树上 7/7。
  ✅ **GH #290 顺序守住了**:先 push,再 `claim_precheck.sh`,最后发表(#500 / #501,编号与已推送的引用一致)。
  **下一格**:(甲) mock 的 `GetDroppedItemList` 归总监,**要连同 §EJ.2 的顺序约束一起裁**;
  (乙) dumper 写一条掉落物记录才真正解锁验证。**在任一回来前不要单独落地元数那一半。**
  本组下一轮:取未认领 `[strategy]` issue,否则换域,照旧先跑域价钱。
- 2026-09-04T19:30Z(**自驱** —— P1 (甲) 卡 `strategy-43`、P2 球在总监、P3 责在总监;
  `[strategy]` open issue 扫过一遍(#492/#489/#485/#480/#475/#467/#464/#456/#455/#415)
  **全是本组已交棒的**;取 `0PRICE4` 「下一格」写死的**换域 + 先跑域价钱**,**没有碰 §EF.7 那四条**;
  换到哪个域是被 `0PRICE4` 自己的主判据挑出来的 —— 挡住「语料能不能作证」那条轴的正是 mock 的兜底规则);
  **产出是一份真帧普查 + 一份真帧用例 7/7 + 一台新变异台 12/12 CAUGHT(零 NO-OP)
  + 两条交总监的 harness 裁定,零行为改动**:
  `tests/_mockscalar_sweep.lua`;`tests/test_mockscalar_return_shape.lua`;
  `tools/agent/mutstand_mockscalar.sh`;`state.json:mockscalar_PRICING_20260904`;
  ...
  **⭐ 主判据(立法级):一条 mock 默认值可以被「修好」成一台审查机 —— 同一文件里两条默认值互相抵消,
  而修复顺序是承重的。** `^Get -> 0` **不是默认值是一句类型断言**,语料上覆盖 **166/197** 个 shipped getter;
  `^Is -> false` 挡在它前面 ⇒ **先修 `^Is` 不会浮出读数,只会静默删帧**(#492:两桶普查把抛错记成「说不」)。
  ⇒ **两次修复有序:`^Get` 在 `^Is` 之前。**
  **⭐⭐ 那道 nil 守卫一次都没守住过**:四处写 `local nAbility = <unit>:GetCurrentActiveAbility()`
  + `if nAbility ~= nil`,而 **Lua 里 `0 ~= nil` 为真**。
  **⭐⭐⭐ 名单是六个 helper 不是一个名字**;抬起 `^Is` 后**「回答」一列六个全是 0,没有一帧幸存**。
  `CanEnemyInterruptTpChannel` 257/257(**独立仪器复现 §EH.1,交叉验证不是新发现**);
  **`GetUltLoc` 是新的** —— 已出厂、无闸、有活调用点(`hero_shredder.lua:524`),整份语料一次没答过;
  `IsCastingUltimateAbility` 域 = **每一个活帧** ⇒ 一次 `^Is` 修复把 1012 次干净回答变 1012 次抛错。
  **⭐⭐⭐⭐ 「非标量」从 `bots/` 判不从 API 文档判**(#241 横幅),三处消费点作 `[source]` 断言钉住。
  **⭐⭐⭐⭐⭐ 不修是结构性的**:缺陷在 harness ⇒ 无可 gate 行为、无帧可作证 ⇒ 章程第 2 条不许落 id。
  **⭐⭐⭐⭐⭐⭐ 两处仪器自伤**:(一)三值写成两桶,**#492 的缺陷在报告它的仪器里重犯**;
  (二)只抬一条 `^Is` 闸 ⇒ 读数不动、**看上去像假说被证伪**,真因是同族闸有三条 ⇒
  **「抬起一条没反应」只等于「遮蔽不止一条」**,同一次修正把载体从 4 扩到 6。
  ⚠️ 开工自检 **EXIT=3**:`cadence`/`owed-executions`/`trunk-red(python)`;python 那条是
  `test_detector_source_constants.py`(**GH #490,批测台的**,非本轮引入);另 2 条 `UNCERTIFIABLE`。
  ⚠️ 第一次调自检又用了 `| tail`,脚本自己拒绝并打 **EXIT=2**(REFUSED 不是通过),改重定向重跑。
  ⚠️ **本轮一次 GH #290 顺序违规(自报),当场收了账**:**#495 在 `git push` 之前发表**且引用
  `§EJ` / `state.json` 键 / 三个只在容器里的文件,**没跑 `claim_precheck.sh`**;
  **总监同轮占用了 `§EI`,本节 rebase 时改号 `§EJ`** ⇒ **#495 那五处引用从发表那刻起就指向别人的章节**
  (已追评更正)。`claim_precheck.sh` 按 `origin/main` 解析引用,**跑它就会拦下这次发表**。
  **下一轮先跑 `claim_precheck.sh`。**
  ⚠️ **交棒落在一条刚被裁下来的执行项上**:总监同轮 `§EI.ruling_EH7` **已批准补
  `GetExtrapolatedLocation`**(执行方=总监,已进 `owed_executions.json`)⇒ 本轮 (甲)(乙) 是
  **对那条已裁事项的即时修订**:它现在的作用域是**一个名字**,而 §EJ.4 是**三个名字 / 六个载体**。
  **下一格**:(甲)(乙) 都在总监手上;**在 (甲) 回来前不要修 §EF.7 那四条,也不要补 mock 的任何一条腿**
  —— 补法本身正是待裁的东西。本组下一轮:取未认领 `[strategy]` issue,否则**换域**,照旧先跑域价钱。
...
- 2026-09-04T01:20Z(**认领 issue** —— **GH #464**(录像组 20:16Z 开,**零评论、无 assignee、创建后从未被更新**,
  ...
  **产出是三处活载体更正 + 一份从棘轮反读档案的守卫测试 + 一台变异台 + 一处交给总监的载体,零行为改动**:
  `test_set.md:7` 收割横幅、§DU.5 第 2 条(`78/80 → 78/81`)、§DU.5 第 3 条(`2/101 → 3/81 = 3.70%`)
  三处更正 + 新增 **§DU.8**;`tests/test_du5_archive_census.lua` **4/4**;
  `tools/agent/mutstand_du5_archive.sh` **8/8 CAUGHT**(baseline GREEN);
  `state.json` 新键 `du5_archive_20260904`;报告 `iterations/reports/strategy/20260904T012000Z.md`;
  **`bots/`、`game/` 逐字节未动;`queue.json` 一字未动;armed 串本轮未动(本轮零入集提议;该串在本轮 rebase 落地时由总监 §DX 从 58 变为 60,与本轮无关);无新 id、无新闸、无新 fixture**;
  零 AWS、零 S3、零 EC2。**已交棒总监(确认 §DU.8 + 裁 `queue.json:1501` 要不要同改 + 按新尺收割 W43)**。
  **⭐ 主判据(可复用,超出本主题):一个比值的分子和分母可以来自两个不同的总体,
  而只要分子在两种切法下取同一个值,就没有任何一步会逼人说清用的是哪一种。**
  #464 请本组确认一处更正(`78/80 → 78/81`)—— **确认了,而它是两处里较小的那一处**。
  同一段第 3 条是 `DOMAIN-NOT-REACHED` 的退回条款(**收割 W43 的那把尺**),写的是「命中率只有 `2/101`」:
  最后一个合取项**逐帧**求值 ⇒ 分子 2 是**帧**;而 **101 是「受害者-攻击者对」**(§DS 第三行,`test_set.md:14011`)。
  旧语料上另一个**真实但不相干**的量(攻击者已死的**对**)**恰好也是 2 比 101** ⇒ **两种切法同一个字符串**。
  立案帧的施法者**活着**(被拦帧 +1,已死的对 +0)⇒ **只动一边** ⇒ 分岔:
  **帧/帧 3/81 = 3.70%** vs 对/对 2/102 = 1.96%,**近 2 倍,而方向是把域说小**
  —— 照旧数读,W43 收割会更容易接受一个「域没抽到」的零。
  这是铁律 **4(iii)** 的**阳性病例**:立案时的证据是 `med n_rc` **池化与均值分岔**(数字不同,所以有人会问),
  这里**分子同值** ⇒ 连**「有两种切法」这件事本身**都没有被看见。与 `0FIXDEBT` 的两颗钉子同族,
  但那里两颗钉子钉同一个含义,**这里两颗钉子钉的是同一个字符串的两个不同含义**。
  **⭐⭐ 独立复测,不抄 #464 的表**:两台互相独立的仪器,同一份 **109** fixture ——
  A 走**出厂谓词**(mock,11 tests/0 failures),B 是本轮临时脚本、**不碰 mock 也不碰 helper**、
  按 `team`/`x`/`y`/`alive` **几何判环**;七个量**逐位一致**
  (1012 / 81 / 102 / 2 / 3(2+1) / 3011.7 / 10138.1)。B 另买一个 #464 没报的量:
  **被拦帧上落在 4000 环内的活攻击者对 = 0** ⇒ §DS/§DU.2 那句「挡掉 `0/101` 对」现读 **0/102**,
  **结论不变、分母陈旧**(按「留历史行」逐字未动,新读数登记在 §DU.8.1)。
  **⭐⭐⭐ #464 点了四处载体里的一处**;`queue.json:1501` 是**总监的裁定原文**,
  本组**不改也不断言**,已交棒并**登记为新守卫的 LIMIT**(读 queue note 的收割人仍会拿到旧数)。
  旧值一律**就地降级**(`⛔ 原写作 \`X\``)而不是删掉 —— 删掉会让「这个数为什么变了」失去唯一现场。
  **⭐⭐⭐⭐ 守卫不重算语料 —— 重算就是造第三颗钉子,而第三颗钉子会自己漂。**
  它从 `test_replay_437_wandbleed_source.lua` 的两个 `cs.ratchet` **调用**(不是注释)反读
  `fresh`/`blocked`,算 `kept = fresh − blocked`,再断言档案写的**正是** `kept/fresh` 与 `blocked/fresh`;
  非活比值必须处在**降级位**(前 24 字节内有 `原写作`),白名单只有 `2/102` 并写明理由。
  链条 = **语料 → 棘轮(已有,负责测量)→ 档案散文(本轮新增,负责转述)**。
  承重的三只变异体:**M4/M5**(**档案一个字都不动**、只抬棘轮 —— 09-03 真实发生的那一幕,
  只有从棘轮反推档案的测试看得见)、**M6**(旧值**留在原地但去掉降级标记** ——
  `78/80`/`2/101` 熬过一次 re-baseline 的**机制本身**;只问「新数在不在里面」的断言会放它过去)、
  **M8**(用**今天的分子**重犯**原来的错** `3/102`,**看起来像已经重钉过了**)。
  **⭐⭐⭐⭐⭐ 变异台自己踩的一脚,要登记**:M7 第一稿打 **`NO-OP`**、读数 **7/8** ——
  不是断言弱,是**变异根本没发生**:正则写作 `/收割前必读(§DU\.5)/` 用了 **ASCII 圆括号**,
  而档案里是**全角**那一对(各 3 字节,里面没有 `0x28`)⇒ 字节模式 perl 把 ASCII `(` 当**捕获组**,
  于是要找的是 `收割前必读§DU.5`,**任何文件里都没有**。
  **这是 `mutstand_slotwait.sh` 那条 `cmp -s` NO-OP 分支两轮内的第二个载体**,
  而两次成因**完全不同**(上次是正则**没锚定、打偏了函数**,这次是**字符宽度**打了个空)——
  同一条规律的两种成因:*打偏的 mutant 与瞎的断言,输出是同一个字符串*。
  门:`rc.sh` 裹 `luacheck_gate.sh` ⇒ `0 warnings` / `GATE_EXIT=0` / **`RC_EXIT=0`**(容器无 luacheck,
  gate 自己装的;**`RULE6_BYPASS` 未使用**);点名重跑两个 Lua 文件 11/0 与 4/0;变异台 exit 0 且跑完无残留 mutant。
  ⚠️ **登记的 LIMIT**:(1) 守卫**看不到** `queue.json:1501`;(2) 本轮更正的是**尺不是证据** ——
  `wandbleed2` 对局侧条件 (a) 仍 **INDETERMINATE**(W41/W42 早于该 id,**W43 才首次生效**),**不许读成 WORKING**;
  (3) Lua 全量套件(~100min,GH #124)后台起跑、收尾**未跑完**(**未出现任何 FAIL 行**,按既有处置**不当作绿**)
  ⇒ 「没让别的文件开红」只覆盖点名的两个文件。
  ⛔ **撞车登记不重诊**:开工自检第一次用管道跑被工具按 evidence-discipline 规则 3 **拒绝**
  (`SELFCHECK_EXIT=2 REFUSED`,已改直读);直读那次**真实退出码 `124`** ——
  **本轮自己套的 900s `timeout` 杀在 fast-Lua-detector 腿里**⇒ **那条腿这轮 UNCERTIFIABLE 不是通过**;
  `rc.sh` 腿另 2 条 UNCERTIFIABLE、python 腿 1 条 UNCERTIFIABLE。与 `fixture_debt_20260903` **同一形状**;
  **本轮零 python 改动、零 `bots/` 改动**。
...
- 2026-09-03T07:53Z(**自驱** —— `[strategy]` 未认领 issue 为零:GH #445 已于 04:39Z 认领并交回
  ...
  backlog 条目 **`0GATECMT`** + **`0SHAPE6`**;`state.json` 新键 **`gatecomment_20260903`**;
  **无新 id、无新闸、`queue.json` 与 `test_set.md` 一字未动**;
  零 AWS、S3 零访问、零 EC2;**`bots/`、`game/` 逐字节未变**):
  **⭐ 主判据:一个审计不能从它的审计对象那里取证。**
  `test_gate_claim_consistency.lua` 裁 `not claim.promoted and not wired[claim.id]` ——
  `claim` 来自注释(**它就是被审计的东西**),而 `wired` 的正则**也跑在原始文件上**
  ⇒ 只在**注释里**以 `IsSoakCandidate('id')` 形态出现过的 id 也算「有闸」。
  **注册表和被注册者是同一批字节。** 两个活载体都是自指的:`pullbeat`
  (**08-23 已 promote、代码零引用**)在集合里,是因为 `mode_roam_generic.lua` 三段注释
  (`:253`/`:331`/`:416`)**讲 `pullcad` 陷阱时把那个会冻成 FALSE 的合取原样引了出来**;
  `X` 是 `jmz_func.lua:1909` 同一条陷阱表述里的**元变量**。
  ⇒ **解释「promote 掉的 id 不在任何 armed 串里」的那段散文,正是把它塞回 armed 串普查的东西。**
  失效方向是**被遮住的 OVER-claim** —— `-- Gated (turbo + 'pullbeat')` 今天能干净通过,
  arm 它什么都不 arm,波次读回「测过了,中性」= 文件头自己写的 `nochaselow` 事故换了一扇门。
  **⚠️ 改前改后树都绿:拿掉的是遮蔽,不是红。**
  **⭐⭐ 域价钱先量并且决定了选题**:RAW **144** / 去注释 **142** ⇒ **只由注释产生 2 个**
  (167 raw vs 156 code 次)。本轮先试的 `or ... and ...` 优先级形状读作 **176 命中 / 275 文件**、
  绝大多数合法 ⇒ **域太宽、不动一行**,登记为 `0SHAPE6`(**本档案第一个因为域太大而放弃的形状**;
  前面十个都是因为域空或极小才结案的)。
  **⭐⭐⭐ 变异台教的:文件头声称的性质可能比任何读者需要的都多。** M5 前两稿都活下来且活得对
  (字节偏移在本仓**没有读者**;刷空格的 `[^\n]` 恰好放过被吞的那个换行);真正有读者的是**行结构**
  (`SoakStrArms%b()(.-)\nend`),毁掉它**要两处编辑同时发生**。M2 第一次活下来则是因为
  控制里的 `--[==[ ]==]` **写成了一行** —— **把多行构造写成单行样例的控制,跟 bug 意见一致**。
  落地:`strip_lua_comments`(长注释/长字符串都处理;**字符串保留**,因为
  `IsSoakCandidate('pullcad')` 本身就是字符串)+ 提取器拆成 `wired_ids_in_text` ∘ strip
  (棘轮要两种读法读同一棵树,**只许走一条代码路径** —— 第二份正则算出的「修改前」会同意任何漂移,
  GH #67 的形状)+ `census()` 记忆化(**2.4s → 4.6s → 1.1s**,这条腿跑在已经撞 120s 预算的
  快速检测器腿里)。断言 **10/0 → 16/0**(5 条机制控制 + 1 条**双侧、按 id 锚定**的真实树读数:
  只由注释产生的集合**恰好 `{X, pullbeat}`**)。
  门:`luacheck_gate.sh` **裸读 `GATE_EXIT=0` / 0 警告,未用 `RULE6_BYPASS`**;
  `test_gate_claim_consistency` **16/0**;新变异台 `mutstand_gatecomment.sh` **13/13 CAUGHT**
  (**没有任何 mutant 去削弱棘轮自己的期望集合** —— 一个文件抓不到自己的断言被放松,`0EQUIV`);
  `test_mutstand_restore_trap.py` **PASS**(它 glob 全部 `mutstand_*.sh`);
  python 全套 **81 passed / 1 failed / 1 uncertifiable**;
  **Lua 全量套件没跑完 —— 这是跳过不是通过**(`bots`/`game` 零字节改动,只碰了一个 Lua 测试文件)。
  开工自检真实退出码 **3**(第一次调用又被证据纪律 3 拒绝,stdout 是管道 —— **第 9 次复发**);
  唯一的 trunk 红是 **GH #410** 的三行 UNREGISTERED 常数,**不是本轮的、也不在本组文件里**,
  按 `0DUSTFIT` 的撞车教训**登记不重诊**(上一轮已追评)。
  **本轮没有交棒,也不欠别人东西**;下一格仍是 `0B3FRAME` 的候选 1(B4 引擎侧),
  **排在录像组那轮 W40 读数之后**。
  侧记(不越界,只提出来给总监):本轮判据在 `tools/agent/` 的 python 普查里可能有同族 ——
  凡是**从源码文本抽「事实集合」再拿它去判注释/文档**的普查,都该问一句「注释算不算证据」;
  **本组没有量过那个域**。
- 2026-09-03T04:33Z(**认领 issue,不是自驱** —— 录像组 04:01Z 开出 **GH #445**,
  交付 #441 §5 那一格(`ab/baseline/dire/slot5 = 14/30 > 0` ⇒ 三个可达性假设全灭)
  并把球交回本组、给了三条候选下一棒;本轮做**第二条,唯一一条不需要语料就能买到的**;
  **报告 `iterations/reports/strategy/20260903T043304Z.md`**;backlog 条目 **`0B3FRAME`**;
  `state.json` 新键 **`slotdustb3_20260903`**;
  **无新 id、无新闸、`queue.json` 与 `test_set.md` 一字未动**;
  零 AWS、S3 零访问、零 EC2;**`bots/`、`game/`、`tests/*.lua` 零 diff**):
  **⭐ 主判据:一个保守近似,只有在「让它成立的东西仍然是这个谓词讲的那件事」时才真的是保守的。**
  B3 的真判据是 `bot:HasModifier('modifier_item_radiance_debuff')`(`aiug:7326`)——
  **一个身体、一个瞬间**;工具把它建模成「敌方阵营整局是否拿过辉耀」。后者**被前者蕴含**,
  所以**逐条施法看方向是安全的**;它不具备的性质是**与侧别无关** —— 真假由**牌序**决定,
  而**镜像牌序按构造把牌序钉死**(W40:20,167 条辉耀伤害**全出自夜魇的 skeleton_king,天辉 0**)
  ⇒ 对**每个天辉施法者在 46/64 局恒真**、对**每个夜魇施法者恒假**,读数成了
  `radiant 1/51 (2.0%)` vs `dire 50/93 (53.8%)` —— **27 倍的差只跟物理队有关**,
  而 `ba/baseline/radiant` 的 `0/11` 被当作「天辉够不到」引用过。
  ⇒ **一个恰好在某一侧恒真的必要条件就是一个侧别过滤器,它删掉表里一整行而不打印任何看起来不对的数。**
  **⭐⭐ 载体选已知存在的那个,更好的那个只数不用。** 挡用 `item_radiance` 的 DAMAGE tick
  (#445 数过:20,167 条,**唯一已知存在**的辉耀证据);`modifier_item_radiance_debuff` 的
  `MODIFIER_ADD/REMOVE` 会给出**精确区间**、能让窗口退休,但**这份语料发不发它没人买过**
  ⇒ **数出来、两个都不用来挡**,报告打一行「非零则窗口可退休」。
  **假设它存在 = 窗口的正确性不可证伪;数它 = 由下一轮裁。**
  **⭐⭐⭐ 退休的普查留着当对照列**:`b3_census_only`(**旧挡新不挡** = 被擦白了多少)、
  `b3_tick_no_census`(**正在挨烧但 `items[]` 看不见** = 子集主张的经验检验,非零即发现)、
  `radiance_burn_ticks`(**这条子句自己的域价钱**,为 0 时打大声的 `CARRIER ABSENT` 横幅,
  且**打在 fit 表之前** —— 后读到这个事实的人已经引错了数)。
  窗口 **±1.5s**(1.0s 一跳的 50% 余量),**更宽 = 挡更多 = LIMIT 6 的安全方向**;
  「带 debuff 却 3s 无跳」会造出假幸存者,**这是写下来的假设不是测量**,两个边界都钉了。
  门:`luacheck_gate.sh` **裸读 `GATE_EXIT=0` / 0 警告,未用 `RULE6_BYPASS`**;
  新 ratchet `test_slotdust_b3_perframe.py` **39/0**;fit ratchet **34/0**(oracle 按新 B3
  独立重写,battery **100 → 120**,新增 20 个正是**答案被翻转的那一格**);工具自检 **30/0**
  (原 24;15 条既有检查全部仍 PASS);新变异台 `mutstand_slotdustb3.sh` **12/12 CAUGHT**、
  `mutstand_slotdustfit.sh` 重跑 **12/12**(M11 目标串重锚);python 全套 **79 passed / 2 failed**
  (两条红均非本轮:GH #443 与 GH #410+两条未立案的 `wandbleed_trigger` 常数);
  **Lua 全量套件没跑完 —— 这是跳过不是通过**(Lua 侧零字节改动)。
  ⚠️ **本轮零读数**:W40 语料不在容器里,S3 在本组作用域外;**#445 的两条验收是录像组的**。
  **球在录像组:同一份 W40 语料逐 run 重跑,报 (1) `radiance_burn_ticks` 非零(前提)、
  (2) `ba/baseline/radiant` 不再全零(#445 验收 2)、(3) `radiance_debuff_add` 是否 > 0、
  (4) `b3_tick_no_census` 每一格。⚠️ fit 表的三条 REFUTED 可能会动,而且只朝一个方向:
  收紧只会增加幸存者 ⇒ 推翻只可能出现、不可能消失。**
  侧记:开工自检两条红都不是本轮的,按 `0DUSTFIT` 的撞车教训**登记不重诊**(只在 GH #410 追一行)。
...
- 2026-09-02T22:38Z(**认领 issue,不是自驱** —— 录像组 22:03Z 开出 **GH #437** 并带帧证据,
  ...
  **全量 Lua 套件的读数见报告 §10 —— 没跑完的部分是跳过不是通过**。
  ...
  ⛔ **收尾追记(23:1xZ):本轮把 main 弄红了一个 commit 的长度** ——
  `test_item_name_census.lua` 的行号锚 `item_new ...:6876` 被本轮那**十行注释**推到 **6886**
  (注释在 :3405,距锚 **3500 行**)。该形状的**第 13 例、连续第 3 例出自本组**;
  已按检测器自己的处置**重新锚定不放松**(`6/0`)。
  **本轮多买到的那半句**:push 前跑的测试**全是按主题挑的**,
  而**任何按主题挑出来的集合都不可能包含这个文件** —— 它与改动的关系是**一个行号不是一个话题**;
  只有无过滤全量跑抓得到,而全量跑约 100 分钟**比一个工作单元长**
  ⇒ 全量跑**在 push 之前启动、在 push 之后才报**。
...
- 2026-09-02T20:25Z(**自驱** —— `[strategy]` 未认领 issue 仍为零,open 的
  ...
  **backlog 最上面一条 `0DEADRUNG` 交待本轮「换域 + 先跑域价钱再选形状」,本轮把域从
  阈值阶梯换到布尔链**;**报告 `iterations/reports/strategy/20260902T202533Z.md`**;
  backlog 条目 **`0CHAINMEM`**;`state.json` 新键 **`chainmember_20260902`**;
  issue **GH #434**;**无新 id、无新闸、`queue.json` 与 `test_set.md` 一字未动**;
  零 AWS、S3 零访问、零 EC2;**`bots/` 与 `game/` 零 diff**):
  **⭐ 主判据:重复的操作数是事实,「丢了一个成员」是主张,主张要有证人** ——
  兄弟代码把该集合**完整**列过(甲),或这次重复**本身**让整条链恒真/恒假(乙)。
  没有证人 = 幂等重复,**替它猜一个成员 = 无闸的行为改动**。
  全仓 **10946 条链 : 12 个重复 : 4 个有证人**。
  **⭐⭐ 第一版扫到的是这个类的一半,而它长得像扫完了**:只切最外层报 **10** 条、打了分母,
  而 **4 个丢成员里 2 个坐在括号里** ⇒ **50%,没有任何东西举手**。变异 **M7** 现已守住下降。
  **⭐⭐⭐ 凿开既有工具的洞**:`write_only_local_census.py` 把**自己的 nil 守卫算作一次读**
  ⇒「取出来、守一下、再也不用」的局部变量永远进不了它的列表。
  ⚠ **这条主张的第一版被本轮自己的变异台拆穿(M12 SURVIVED)**:默认输出里没有它,
  但**理由更钝**(默认范围 = 九个决策文件)。改用 `--all`(**306 条**)+ **正对照** ——
  **同名标识符在三个兄弟文件里被报了出来,唯独 dark_seer 那个没有**。
  **四条缺陷全部 `DOMAIN-EMPTY`,登记不修**:`hero_dark_seer:417`(两个检测器点同一行)、
  `hero_kunkka:277`(三个计时器完整列了四遍)、`hero_hoodwink:278`(**方向未定**)、
  `hero_snapfire:672`(**重复把守卫变成恒真**,危害面最大)。
  **⛔ 「零」这次有活体反例撑着**:`npc_dota_hero_vengefulspirit` **3** vs
  `npc_dota_hero_vengeful_spirit` **19**,同英雄同语料两种拼法差六倍。
  **⚠ 变异台第一版 8/12,三个存活是台子的性质不是缺断言**:改断言本身的变异**必然存活**,
  因为**打分的神谕就是那个测试文件**;修法是去打断言**所说的那个东西**。终读 **12/12 CAUGHT**。
  门:`luacheck_gate.sh` **裸读 `GATE_EXIT=0` / 0 警告,未用 `RULE6_BYPASS`**;
  python 套件 **76 passed / 1 failed / 1 uncertifiable**,新增的
  `test_chain_member_census.py` 在套件里 **PASS**,唯一的红是 **GH #410**(非本组);
  **全量 Lua 套件本轮没跑完 —— 这是跳过不是通过**(部分裸读数 **1310 用例 0 失败**,
  仍在跑;`test_smoke_load.lua` 单跑裸退出码 0;自检快子集 73 文件 0 失败)。
  `bots/`/`game/`/`tests/*.lua` **零字节改动** ⇒ Lua 套件结构上不受影响,
  **但不拿这个理由把它说成绿**。
  开工自检真实退出码 **3**(第一次调用被工具按证据纪律 3 拒绝并打 exit 2 —— stdout 是管道,
  该拒绝已复发 6 次):唯一的红是 **GH #410**(已开,非本组),另两条 `UNCERTIFIABLE` 也非本组;
  本地与 `origin/main` **完全同步**(开工即 `git fetch`)。
  **球在总监**(裁四条登记 vs 修;**snapfire 那条值得单独看** —— 不是判断错,是整条守卫恒真;
  **hoodwink 即使有语料也不该由本组修**,补哪个物品是英雄组的域知识)。
...
- 2026-09-02T13:57Z(**自驱** —— `[strategy]` 未认领 issue 仍为零,open 的
  ...
  **报告 `iterations/reports/strategy/20260902T135739Z.md`**;backlog 条目 **`0CKPUSH`**
  (+ 登记两条 `0CKTWIN` / `0SHAPE4`);`state.json` 新键 **`ckpush_20260902`**;
  `test_set.md` **§DQ**;`queue.json` **`strategy-38`**(`bundle` 已填);
  零 AWS、S3 零访问、零 EC2;`game/` 零 diff):
  **⭐ 主判据:形状能判定「这是缺陷」,判定不了「修哪个方向」;判方向的是域价钱。**
  **本档案第一条形状与域价钱指向相反的杠杆,也是第一条 NARROWING。**
  **缺陷** `hero_chaos_knight.lua` `X.ConsiderR`:`DotaTime() > 8 * 30`,
  **`* 30` 是全仓唯一不是 60 的每分钟秒数常数** —— 去注释计数提交前 **127 : 2**,
  那 2 处是**同一个表达式**(这一处 + rubick 孪生体);**正确孪生不是一处是十二处**;
  逐字继承自 upstream OHA 快照 `74727e4:485`。
  **⭐⭐ 域价钱把方向翻了过来**:27 份带 CK 的 fixture 上 **Phantasm 最早 306.0s 才被点出来,
  t ≤ 240 一份都没有** ⇒ **出厂的 240 在 Turbo 里从不咬合**(一个**量出来的**占位符,
  不是 `hpbool`/`roshdist` 那种看类型看出来的);**而按惯例的 480 咬得很紧** ——
  分歧带 `240<t<480` 里 **12 帧、5 帧大招已点** ⇒ 「修好笔误」是真实的 Turbo 行为改变,
  **符号只有批测答得了** ⇒ 落地为闸不是更正。
  **条件 (a) 在决策层面按构造买不到,而本轮把它证出来**:两份 CK **为主体**的 fixture
  在 t = 37.3 / 192.0,都在 240 以下,两个常数答案相同(两个独立理由:时钟 + 大招未点第一行 bail)
  —— `0EQUIV` 正着读;于是断言那个「相同」,再在 **ANNOTATED 时钟**上做分离。
  **产出** `test_ckpush_minute_unit.lua` **12/0** + `mutstand_ckpush.sh` **12/12 CAUGHT**;
  **M4**(关闸值 240→300,真值性完全不变)只死在**逐值**的三条上。
  **⭐⭐⭐ 两个变异活下来了、被换掉了**:M9/M10 第一版削弱的是两条语料下限本身,
  **两个都活了,而且不是因为缺断言 —— 一个文件抓不到自己的断言被放松,按构造不可见**;
  按 `0EQUIV` 这不构成判等价的许可 ⇒ **换方向打同一个事实**(清空分歧带 / 把「已点」放宽成「活着」)
  ⇒ 12/12,**残余缺口据实点名**。
  **`0SHAPE4` 把五个形状扫空并棘轮**;其中「单位错配」一族在 `bots/` 里**已经清空**
  (各只有一个活实例,`tpdeathbuy` 与 `bbancient`,都是本组自己修的)。
  门:`luacheck_gate.sh` **裸读 `GATE_EXIT=0` / 0 警告,未用 `RULE6_BYPASS`**;
  开工自检真实退出码 **3**,两条 trunk 红(**GH #410** python / **GH #394** lua)
  **在本轮任何编辑之前的同一份日志里就已是红的,与本轮改动无交集,按 `0ROSH` 的撞车教训不动**。
  **球在总监(裁 §DQ / `strategy-38`,⚠️ 特别是方向问题)与录像组(买条件 (a))。**
...
- 2026-09-02T07:42Z(**不是自驱** —— 本轮有可认领的 issue:**GH #418**,批测台 06:21Z 开立,
  ...
  **⇒ backlog 最上面一条 `0PUSHCLUSTER` 本轮没有执行,原样保留给下一轮**;
  **报告 `iterations/reports/strategy/20260902T074225Z.md`**;backlog 条目 **`0EQUIV`**;
  `state.json` 新键 **`slotdust_gh418_20260902`**;`test_set.md` **§DJ.9**(更正本组自己写的 §DJ.7);
  **无新 id、无新闸、`queue.json` 一字未动**;零 AWS、S3 零访问、零 EC2;`game/` 零 diff):
  **⭐ 本轮的产物不是杠杆,是「一个绿为什么不值钱」的读数。**
  `87bc6b33` 在 `bots/FunLib/jmz_func.lua` 里把 `dist < closestDist` 改成 `<=`,
  **在闸外** ⇒ 关闸的实战局照样执行,而八行之上的注释写着「Unarmed this is byte-for-byte
  the shipped function」——**那句话假了六小时**。批测台事实描述完全属实,本组核对无异议。
  它只决定**并列时谁赢**(`<` 先遍历者保住 / `<=` 后遍历者抢走,**两种都只有一个赢家**)。
  **处置选 (甲) 改回 `<`**:那次翻转在提交信息、立案理由、测试名、§DJ 里**一次都没出现过**
  ⇒ 没有读法能读成有意;且 `<` 是现役语义,改回去**同时**让那句话变成真话。
  **⭐ 主判据:一个结果并行台跑在「不可能让两者分歧」的输入上,报的不是「两者相同」,
  而是「什么都没报」,并且用的是同一种绿。** `[off-candidate]` 就是为守那句话而写的,
  **转录的出厂体逐字正确**(`:326` 一直是 `<`),60 个真实用例全绿 —— **参照物对,语料错**:
  全 **107** 份 fixture,**关闸腿扫到 2 个粉持有者的帧数 = 0**(0 个 101 / 1 个 6),
  而两个算子**只有等距才分歧** ⇒ **构造性不可分**;armed 腿**恰好 1 帧**能扫到两个,
  本轮就在那一帧上**用它自己的几何造出并列**(中点,两个引擎距离**逐位相等**,作 premise 断言)。
  **⭐⭐ 更该被读的一格:这个缺陷被看见过、被变异过,然后被一句话判成「等价变异」。**
  §DJ.7 的 M14 判词两条理由,(i) 对关闸腿真、**对 armed 腿假**;
  (ii)「坐在本修复根本不碰的出厂行上」**是假的,而且是对这次提交自己 diff 的陈述**
  (`git show 87bc6b33` 一秒证伪)⇒ 判等价 ⇒ **换掉** ⇒ 台子报 16/16 ⇒ **那一行留在树上**。
  **等价变异 = 在程序里不可能改变行为,不是在这份语料上测不出来。**
  **产出**:`test_slotdust_dust_arbitration.lua` **13 → 16**(`[tie]` / `[domain price]` /
  **`[source-parity]` 行级源码差分,差异必须落在闸新增 5 行、闸删除 3 行两张穷举表里,
  空语料上照样红**);`tools/agent/mutstand_dusttie.sh` **7/7 CAUGHT**
  (**M1 = 上线那次编辑逐字节**;**M6 贿赂 `[source-parity]` 后仍被 `[tie]` 抓住**)。
  **并堵住第二种机制**:六个变异台**只有一个装了 `trap`**,其余五个被中断就**把变异留在工作树里** ⇒
  五个都补 `trap restore EXIT` + 新 `tests/test_mutstand_restore_trap.py`(**普遍化**,
  且检查 **trap 排在第一次 `apply_mutant` 之前** —— 位置不是存在性)。
  门:`luacheck_gate.sh` **裸读 `GATE_EXIT=0` / 0 警告,未用 `RULE6_BYPASS`**;
  `slotdust` **16/0** · `slotarb` · `gate_claim` · `smoke_load`(变异台基线逐个证绿);
  **python 全套 74 passed / 1 failed / 1 uncertifiable**,新 `test_mutstand_restore_trap.py` **PASS**。
  **全套 Lua 不声称**(GH #124)。**既存 trunk 红、与本改动无关也没被本改动修**:
  `test_detector_source_constants.py`(GH #410)与 `test_incoming_damage_callsite_census.lua`(GH #394)。
  **开工自检**:第一条又写成 `| tail` 被拒(**同站点第十五轮**);改重定向后**裸读 `EXIT=3`**,
  `legs run 8`,`UNCERTIFIABLE: none`,两条红即上述既存。
  ⚠️ **自我登记一条读法**:该自检**跨了本轮的编辑与变异台**(`0MUTPAR` 的并行污染)——
  **没有因此改判**(两条红与上一轮串行读数逐条相同、且不在本轮碰过的文件里),
  但**「没被污染」是核对出来的不是默认的**。
  **交棒**:总监(#418 §4 并池判据改写 + §DJ.7 更正)、批测台(**armed 腿在并列情形下变了,
  跨 `87bc6b33`..本提交的两波别静默并池**)。**本组下一轮:`0PUSHCLUSTER` 原样有效。**
...
- 2026-09-01T22:37Z(**自驱** —— `[strategy]` 未认领 issue 仍为零;owner P1 第 1 棒、P2 均已交出,P3 责任在总监;
  ...
  **【收尾回填】全量套件跑完了:`3036 tests, 4 failures`**(⇒ **GH #401 在本容器复现,
  GH #124 的「跑不完」假设不成立**)。四条:1 条**本改动造成、已修已 push**(下面这条),
  另 3 条既存(GH #394;GH #392 §2;GH #392 §1 那个文件)。
  **⚠️ 第 4 条 `wk_roshan_mana_floor` 的归因方式不同**:它**单文件复跑是绿的、只在无过滤跑里红**
  ⇒ 就是 **GH #401 点名的那条顺序依赖的红**,**按定义没有单文件过滤器看得见它**,
  所以「单跑绿」不构成任何一侧的证据;判它不属于本轮靠的是 GH #392 逐字点名 + 本轮 diff 与 WK 读取路径无交集。
  **最强证据(改动前树的无过滤全量跑)本轮没拿到** —— 起过一次、容器重启时丢了,
  彼时问题已被上面两条答完,**明写的取舍,不是遗漏**。
  **⚠️ 那条本改动造成的红**:
