...
0P3. **【2026-08-22T14:0xZ 新增】owner P2 的补给侧已落地为 gated `fieldbuy`,本条留着盯回程。**
   **本轮真正的发现是域的形状**:整条 situation 谓词成立的 **50** 个真实英雄帧里,
   **22 帧包里有回复品(`stayfield` 能动的域)、28 帧没有** ⇒ **没人管的那一半是大的那一半**;
   而现有采购路够不到它的原因是**结构性**的:`fieldregen` 的 `not J.IsInLaningPhase()`
   在 Turbo 里等于**一局 ~20 分钟的前 8 分钟按构造静默**(28 帧里 **18** 帧在与净值无关的硬地板下),
   出厂对线补给块卡 `botLevel < 6`(28 里 **22** 帧 ≥6 级),**12 帧同时落在两个洞里**;
   `fieldregen` 的 0.45 上限再排掉 13 帧 —— **owner 自己钉的 Slardar 帧(46.0%)就差 1.0 个百分点**。
   **第一次在真实帧上断言一次采购**(物品**使用**层是黑的,GH #100;**采购**入口能跑)。
   **交出去的三棒**:总监(test_set.md 顶部提议,请与 `stayfield`/`stayfield2` **同波** ——
   分波会互相遮蔽,决策侧最后一条子句就是本 id 的域的补集)/ 批测台(不新开单,随「野区续航」波,
   `--rec-slots ≥ 8`)/ 录像组(阴性判据写在 GH #110 追评)。
   **本轮明确不做、留给下一个杠杆的**:(1) 要不要把 **faerie_fire 也算进采购点的「已经有了」**
   —— 帧 B 的仙灵之火在背包里、换出来 6 秒,**先要一份它真跑起来的录像**;
   (2) **不动 `fieldregen`** —— 它在 armed 集里,改它 = 悄悄换掉在飞波次在测的东西
   (已配 `[reverse]` 用例钉住它两条子句还在,将来放宽必须**故意**去红它)。
   **顺带撞到两条世界事实**(§3.2/§3.3 of the report):`ItemPurchaseThink` 的
   `bot.lastItemPurchaseFrameProcessTime` 是**第十四条(GH #91)在全新入口上的复发**
   —— 盖章的时钟就是它稍后要比的那个 ⇒ 一次调用的 VM 里**整个采购层直接返回**;
   `bot:DistanceFromSecretShop()` **没打桩答 nil** ⇒ 任何 >6 级英雄在 `item_purchase_generic:1109` 崩溃,
   **而那一行只有靠第十五条(GH #93)才够得到**(门是 `GetGameMode() ~= 23 and botLevel > 6`)
   ⇒ **7/8/9 级英雄的那次崩溃是纯拼写产物**。已开 `[harness]` issue 交总监。
   **一条负面结果如实记下(M5)**:拿掉敌塔子句**只红了普查计数**,两枚钉帧 1200 内都没塔
   ⇒ **该子句在本文件里没有孤立行为见证**(见证在姊妹 stayfield2 文件的 zuus_ult_denied 帧)。
   **「只动计数器的变异」= 计数器有牙 **且** 缺一条行为断言,两半要一起说。**
   ⭐ **0S 本轮当场兑现了一次,而且给出了它的机制版**:新文件单跑 **21/21 全绿**、
   luacheck 0 警告、9 条变异全部点名 ⇒ 看起来能提交了;**全套跑读出 1234 例 1 失败,
   而那一条是我自己弄红的** —— `test_replay_260822_lina_tp_home :: wiring: ... turbo-only`。
   本轮把 situation 那几条**再往下拆了一层**,姊妹文件的 `[reverse]` 钉子**按函数体**
   断言「turbo-only + 三个半径」,行搬走了钉子就落空(它红得对,只是盯错了函数体)。
   ⇒ **一次纯提取重构不改任何行为,却会让所有按函数体钉的源码断言失准** ——
   而这类断言正是本组防「gate 悄悄消失」的主力。**今后凡是拆/合一个被 `[reverse]` 钉过的函数,
   收尾必须 grep 该函数名的所有钉子并逐个搬**;单文件跑永远发现不了,
   因为**被打破的那个文件不是你在改的那个文件**。已修 + 多钉一条(两个无 gate 的层
   **都**不许长出自己的 gate),三个姊妹文件合跑 **60/60**。
0R. ~~**【2026-08-22T10:xxZ 新增】本组自己的普查文件在 main 上是红的**~~ **已结案
   (2026-08-22T14:0xZ 复核)**:`1443e2a`(11:xxZ,采信总监 `bef59de` 的 bisect)已把
   `crash_2597`/`crash_3325` 重钉成 **178/32**,本轮开工在 `a03f17d`(= `origin/main`)上单跑该文件
   **24 例 0 失败**。根因是本组自己 05:30Z 的 `cf7bb4c` 治疗了一枚 fixture(同一个 `--t` 落到早
   ~0.33s 的采样)⇒ **共享语料不只被「加一枚」打破,也被「治一枚」打破**(GH #106 的后半句)。
   **保留在案的方法学**(下面原文不删):
0Rx. **【原 0R 全文,保留作方法学】本组自己的普查文件在 main 上是红的,
...
## 当前状态(每次触发后更新)
- 2026-08-22T14:00Z:**owner 优先项 P2 的补给侧 —— 新 gate `fieldbuy`,而且是本仓库
  第一次在真实帧上断言一次采购。**
  **认领依据**:两个 owner 优先项本组这一棒都已交出(P1 球在总监/录像组,replay-check 13:15Z
  已把 `creeppull` 的 (a) 买到;P2 决策侧两个 id 的入集球在总监)⇒ 取 charter 11:26Z 自己点名的
  「下一个杠杆」。开工按铁律 10 跑了 `routine_selfcheck.sh`(唯一 finding 是英雄组的 cadence 洞),
  按 0m 先 fetch,`origin/main` = `a03f17d` = 本地 HEAD。**零 EC2 支出**;**不新提批测请求**。
  **先量再改**(100 fixture / **930 真实存活英雄帧**,跑的是 shipped helper,四个常数全从源码读):
  **150** 帧在血量带内 → **82** 帧**六个可用格里没有任何能喝的东西**;整条 situation 成立的 **50** 帧
  里 **22 有 / 28 没有** ⇒ **`stayfield` 能动的域是 22,没人管的那半是 28** ——
  **补给侧才是大的那半**。`situation == 22 + 28` 已成断言,两个 helper 从此不会悄悄漂开。
  **够不到的原因是结构不是阈值**:`fieldregen` 带 `not J.IsInLaningPhase()`,Turbo 对线地板 **8:00**
  ⇒ **一局 ~20 分钟里前 40% 的时间它按构造不存在**(28 里 **18** 帧在与净值无关的硬地板下);
  出厂对线补给块卡 `botLevel < 6`(28 里 **22** 帧 ≥6)⇒ **12 帧同时在两个洞里**;
  0.45 上限再排掉 13 帧,**owner 自己钉的 Slardar 帧(46.0%)差 1.0 个百分点**。
  **结构**:`J.ShouldRegenNotGoHome` 拆出**无 gate 的核心** `J.IsFieldRegenSituation`,
  新 gated 包装 `J.ShouldBuyFieldRegen` 调**同一个谓词**、把包里那条**反过来**;
  采购点接在 `fieldregen` 块正下方。**共用谓词是承重设计**:补给规则自带阈值就会与决策带漂开。
  **测的是最终动作**:`ItemPurchaseThink` 真实帧驱动,`ActionImmediate_PurchaseItem('item_flask')`
  **armed 有 / 不 armed 没有**,两枚帧各一次;阴性对照有牙(不 armed 那次 Think **仍买了别的**)。
  **两枚钉帧各隔离一个出厂阻塞点**:帧 A `f_260820_102645_cm_es_reach`/zuus(7 级 39.5%,敌人 1727,
  **边距只有 127**)= 只被对线子句挡;帧 B `f_260820_043637_axe_ring_alone`/viper(15 级 54.9%,已过对线)
  = 只被 0.45 上限挡。**两条一直只有注释没有见证的子句本轮各拿到一枚真实帧**:
  帧 A 的 4 号格**字面就是 `item_empty_bottle``**(瓶子腿永远答不了 TRUE 的实证)、
  帧 B 的仙灵之火在 **6 号格 = 背包**(背包里的药喝不了的实证)。
  **驱动采购层要三条声明,写在测试里不藏进 loader**(GH #61):`GetGameState`、库存、以及
  **第十四条世界断言(GH #91)在全新入口上的复发** —— `bot.lastItemPurchaseFrameProcessTime`
  盖章用的就是它稍后要比的那个时钟 ⇒ **一次调用的 VM 里整个采购层直接 return**。
  **没声明因此没测到的那条明说了**:金钱子句(mock 的 `GetItemCost` 答 0,恒真)。
  **顺手撞到的第二条**:`bot:DistanceFromSecretShop()` 没打桩答 nil ⇒ 任何 **>6 级**英雄在
  `item_purchase_generic:1109` 崩溃,**而那一行只有靠第十五条(GH #93)才够得到**
  (门是 `GetGameMode() ~= 23 and botLevel > 6`,fixture 世界按名字是 Turbo、按字面 23 不是;
  诚实世界走另一腿要 >9 级)⇒ **7/8/9 级英雄那次崩溃是纯拼写产物**;本组要的采购**在崩溃上游**,
  用例**断言了这个次序**而不是吞掉它。已开 `[harness]` issue。
  **验收**:luacheck **0 警告**;新文件 **20 例 0 失败**;**9 条变异逐条 apply+rollback**
  (回滚走**内存副本**,不走 `git checkout --`),每条点名它该红的用例。
  **一条负面结果(M5)**:拿掉敌塔子句**只红了普查计数** —— 两枚钉帧 1200 内都没塔,
  ⇒ **该子句在本文件没有孤立行为见证**(见证在姊妹文件的 `zuus_ult_denied` 帧)。
  **「只动计数器的变异」= 计数器有牙 **且** 缺一条行为断言 —— 两半必须一起说**,
  这是 0m 第四种形状的下一层:变异落在了被测量上,但落的是计数那一面。
  **条件 (c) 已检索**:药膏 100 金 / 13 秒 390 血,标准建议明确说「前 10 分钟不该回泉、
  不必要的回程是浪费时间」——正是本杠杆补的洞;而且**药膏被敌方英雄伤害打断就作废**
  ⇒ situation 里的「1600 环空 / 无归因受击」**不只是保守,是药膏能生效的前提**。
  **接力棒**:总监(test_set.md 顶部提议:请把 `fieldbuy` 与 `stayfield`/`stayfield2` **同波** ——
  分波会互相遮蔽;**顺带 `stayfield2` 的 11:2xZ 提议仍挂着**)→ 批测台(不新开单,随「野区续航」波,
  `--rec-slots ≥ 8`)→ 录像组(阴性判据在 GH #110 追评)。
  **backlog 头条 0R 本轮复核为绿并结案**(`1443e2a` 已重钉 178/32,`a03f17d` 上 24 例 0 失败);
  已在 GH #112 留言说明「红没了但**不由本组关闭**」——正文定的那份 `crash_2597`
  **per-fixture 分解至今没写**,它属于 #106 §4 的形状,是一个独立工作单元。
  `state.json:fieldbuy_20260822`,详见 `iterations/reports/strategy/20260822T140000Z.md`。
