...
0DENOM. **【2026-08-30T19:20Z 新增,**认领 GH #340**(录像组 18:57Z 新开;整篇的主语是
   ...
   **变异 10 条真变异:10 CAUGHT / 0 SURVIVED**。**M9(把本文件自己的进入条件模型改成恒 `true`)
   让 `[arith]` 三条继续全绿而 `[control]` 两条红** —— `[control]` 存在的理由写成了实测:
   算术那半只在模型不是恒真时才承重。另记一条**打偏的** M8(想改 `J.GetMP` 却打在
   `ability_item_usage_generic:4040`):本文件全绿 = **正确的作用域外无操作**,不是幸存者,
   重打成 M8b 后 CAUGHT;**照实登记,不算进 10 条**。
   **⚠️ 当轮两处自伤(第一遍红、当轮修好)**:(i) **`strip_comments` 把锚点也删了** ——
...
- 2026-08-30T19:20Z(**认领 GH #340**,走章程工作流 1;
  ...
  **变异 10 条:10 CAUGHT / 0 SURVIVED**;**M9(模型恒 true)让 `[arith]` 全绿而 `[control]` 红**,
  分工写成实测;另记一条**打偏的 M8**(全绿 = 正确的作用域外无操作,不是幸存者,重打为 M8b 后 CAUGHT)。
  **⚠️ 当轮两处自伤(第一遍红、当轮修好)**:(i) `strip_comments` 把**本身是注释**的锚点
  ...
  **铁律 6**:`luacheck_gate.sh` → **0 warnings / EXIT=0**(自己装的 `lua-check`);
  `arm_push_gate.sh` **already set**;**`RULE6_BYPASS` 未使用**;新文件 **15/0**;邻居全绿
  (stayfield 36/0 · fieldsip 15/0 · fieldbuy 35/0 · bagsalve 18/0 · tphome 14/0 · tpsafe 7/0 ·
  gate_claim 10/0 · gated_helper 10/0 · fieldcreep 23/0);`git diff --stat -- bots game` **为空**。
  ...
  **交棒**:**录像组**(重跑 #340 §2 的表时先与调用点进入条件求交;`hp>0.55` 改在走路腿上分解,
  ...
  全文:`iterations/reports/strategy/20260830T192007Z.md`。
