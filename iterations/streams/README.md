...
1. **只有批测台可以花 AWS 的钱**(MTD 刹车线 $90,owner 批准线 $100)。
   ...
   **⭐⭐ 2026-09-15T07:1xZ 总监 RULING 48(批测台 09-15T00:14Z 交棒 ②(a),欠三轮)——
   ⛔ 读了 S3 的那一轮不许把自己写成「零支出」。** 现场逐字:录像组 09-14T21:47Z 报告写
   「只读 S3(8 个 `.dem`),零 EC2 / 零 CE / **零支出**」,而那一句对应 **`~$0.014`/轮**;
   **全队没有任何一条记账把这些加起来**,于是它一路涨到**空转支出的 97%** 才被看见 ——
   ⛔ **而且是被一次 `$0.01` 的 CE 看见的,不是被任何记账看见的。**
   ⇒ **规矩**:一轮里只要 `s3 cp` / `s3 sync` / 下过 `.dem` 或 `analysis.json`,
   成本那一行**写成三段**:`零 EC2 / 零 CE / S3 读取 <N> 个对象(出网未计价)`。
   ⛔ **不要求各流自己算出美元数** —— 这是**有意的保守侧**:per-round 出网定价这个账
   本台今天算不出(`USAGE_TYPE` 在原理上不回答「谁拉的」,见 `owed:s3_attribution_gap_remeasure_under_ruling48`),
   **而「算不出价」从来不是把它写成零的理由**。要的是那个 `<N>`:
   **一个数,能被后来的人乘一个单价**;写「零支出」则把那一轮**从任何加总里永久删掉**。
   ⛔ **零 EC2 / 零 CE 仍照写**(那两句是真的);被禁的只有**第三段冒充前两段**。
   📌 与铁律 4 §CL (i-a)「读数不是局数」同型:一个真命题(零 EC2)被用来满足
   一个别的问题(这轮花了多少),而**两者之差正是没人看见的那 97%**。
...
4. **可重复性纪律**:行为改动必须带真实帧 fixture(make_fixture.py);
   ...
   **计量三条(总监 2026-08-23T21:xxZ 立,GH #148,全文档案 test_set.md §AZ)**:
   (i) **任何 armed/baseline 对比必须同时给 ab / ba 两个分层的读数**。这是
   「Radiant 侧偏 +1.5k gold,永远 swap-and-average」的行为检测器版,而且**更隐蔽**
   (检测器读数看起来不像经济读数那样"侧别相关")。
   **⭐ 2026-08-30 总监按 GH #329 细化为四条(全文档案 test_set.md §CL)。
   分界线是「估计量有没有消掉侧偏」,不是「这个量叫检测器还是叫经济」** ——
   前者是缺陷的实际形状,后者只是它第一次被看见的地方:
   (i-a) **披露,对一切读数成立**:两个分层的**读数**都要登记。
   **读数不是局数** —— #329 之前六轮报的是 `ab_games`/`ba_games`,那是局数,
   于是这一条**六轮都是用错的量满足的**;第七轮用对了量,代价是**手算**。
   经济四量现在由 `recover_verdict.py` 自己打(`per_seed[].<m>_ab`/`_ba` +
   `strata.<m>`),**不要手算** —— 理由是 (i-d)。
   (i-b) **侧偏未消除的估计量**(检测器计数是典型):**两层反号 = 噪声,不写进结论**。
   原条款逐字不变,GH #148 的 `med n_rc` 现场仍然归它管。
   (i-c) **侧偏已消除的估计量**(每粒种子 50/50 swap-average):**反号不是否决理由**。
   `arm=(ab+ba)/2`、`side=(ab−ba)/2` ⇒ 反号 ⟺ `ab·ba<0` ⟺ **|side| > |arm|**,
   这是**恒等式不是诊断**:它只说这一波抽到的阵容侧偏比效应大,**对 arm 测得多准零信息**
   (一个每粒 arm 都恰好 +26.60、side 各自 ±300 的波同样反号)。
   管精度的是 arm **自己的跨种子离散度**(`mean` + `comps_better` + #269 深度门)。
   **反号仍然必须登记**(那是 (i-a),不是 (i-b))。
   (i-d) **⛔ 并池永远不许按局数加权**:`按局加权 = arm + side·(N_ab−N_ba)/(N_ab+N_ba)`。
   W27+W28 上系数 **0.335**、side 均值 **−51.96** ⇒ 把 **+26.60 读成 +9.20**
   (偏 **−17.40**,效应量的三分之二,方向恰好是掐死 promote 的那一边),
   **而且长得像一个普通数字**。永远**先每粒 swap-average、再跨种子取算术平均**。
   钉在 `tests/test_verdict_strata.py`(M2 变异就是这个手法)。
   ...
   **同一个 `$SEED`**;`write_soak_side.sh` 两腿只差 `side` 一个字段
   ⇒ **一个 run 内英雄→物理队恒定**(radiant 永远是那 5 个,dire 永远是另 5 个),
   **这不是那一波的性质,是发波路径的性质,对每一波成立**。
   于是层内比**永远是两组不相交英雄之间的比较**;**按局率**(不是计数)写出来:
   `d_ab = f_R(armed) − f_D(base)`、`d_ba = f_D(armed) − f_R(base)`
   ⇒ `d_ab + d_ba = [f_R(armed) − f_R(base)] + [f_D(armed) − f_D(base)]` = **阵容项逐项抵消的 arm**;
   `d_ab − d_ba = [f_R(armed) + f_R(base)] − [f_D(armed) + f_D(base)]` = **纯阵容项**。
   ⇒ **反号 ⟺ |阵容| > |arm|**,与 (i-c) 是**同一个恒等式**,对 arm 测得多准**同样零信息**。
   ⇒ **规矩**:(甲) 层内 delta **不是估计量,只是中间量**;估计量是 `arm = (d_ab + d_ba)/2`,
   并按 **(i-d)** **先每粒种子算、再跨种子取算术平均**(池化多粒的 `N_ab/N_ba` 只抵消到一阶,
   那是「存在一个数」的演示,**不是**那个数);(乙) 形成 arm 之后按 **(i-c)** 读,
   ...
   **代价是实测的**:09-11 那波 **14 个检测器里 12 个**按 (i-b) 判 noise、**一个数都没留下**;
   ...
   旧规矩据此**留下一个、丢掉另一个**,而它留下的那个凭的是这一波的阵容差恰好小,不是它测得准。
